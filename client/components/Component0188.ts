// client/components/Component0188.ts
// Benchmark component #188 — identical logic to TodoPage, scoped class names.
...
import "./Component0188.css";

type Filter = "all" | "active" | "done";

export const Component0188 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0188] mounted");
    return () => console.log("[Component0188] unmounted");
  });
  ...
  return () => html`
    <div class="app-188">
      <header class="header-188">
        <h1>todos — Component0188</h1>
      </header>

      <main class="main-188">
        <input
          class="add-input-188 ${error() ? "add-input--error-188" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-188" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-188">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-188">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-188 ${filter() === f ? "filter-btn--active-188" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-188">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-188">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-188">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-188 ${todo.done ? "todo-item--done-188" : ""}">
                    <button
                      class="todo-check-188 ${todo.done ? "todo-check--done-188" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-188" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-188"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-188" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-188" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-188">
            <span class="todo-count-188">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-188" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
