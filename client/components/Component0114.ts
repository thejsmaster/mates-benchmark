// client/components/Component0114.ts
// Benchmark component #114 — identical logic to TodoPage, scoped class names.
...
import "./Component0114.css";

type Filter = "all" | "active" | "done";

export const Component0114 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0114] mounted");
    return () => console.log("[Component0114] unmounted");
  });
  ...
  return () => html`
    <div class="app-114">
      <header class="header-114">
        <h1>todos — Component0114</h1>
      </header>

      <main class="main-114">
        <input
          class="add-input-114 ${error() ? "add-input--error-114" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-114" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-114">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-114">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-114 ${filter() === f ? "filter-btn--active-114" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-114">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-114">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-114">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-114 ${todo.done ? "todo-item--done-114" : ""}">
                    <button
                      class="todo-check-114 ${todo.done ? "todo-check--done-114" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-114" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-114"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-114" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-114" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-114">
            <span class="todo-count-114">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-114" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
