// client/components/Component0018.ts
// Benchmark component #18 — identical logic to TodoPage, scoped class names.
...
import "./Component0018.css";

type Filter = "all" | "active" | "done";

export const Component0018 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0018] mounted");
    return () => console.log("[Component0018] unmounted");
  });
  ...
  return () => html`
    <div class="app-18">
      <header class="header-18">
        <h1>todos — Component0018</h1>
      </header>

      <main class="main-18">
        <input
          class="add-input-18 ${error() ? "add-input--error-18" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-18" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-18">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-18">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-18 ${filter() === f ? "filter-btn--active-18" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-18">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-18">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-18">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-18 ${todo.done ? "todo-item--done-18" : ""}">
                    <button
                      class="todo-check-18 ${todo.done ? "todo-check--done-18" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-18" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-18"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-18" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-18" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-18">
            <span class="todo-count-18">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-18" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
