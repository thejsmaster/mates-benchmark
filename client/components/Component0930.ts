// client/components/Component0930.ts
// Benchmark component #930 — identical logic to TodoPage, scoped class names.
...
import "./Component0930.css";

type Filter = "all" | "active" | "done";

export const Component0930 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0930] mounted");
    return () => console.log("[Component0930] unmounted");
  });
  ...
  return () => html`
    <div class="app-930">
      <header class="header-930">
        <h1>todos — Component0930</h1>
      </header>

      <main class="main-930">
        <input
          class="add-input-930 ${error() ? "add-input--error-930" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-930" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-930">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-930">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-930 ${filter() === f ? "filter-btn--active-930" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-930">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-930">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-930">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-930 ${todo.done ? "todo-item--done-930" : ""}">
                    <button
                      class="todo-check-930 ${todo.done ? "todo-check--done-930" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-930" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-930"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-930" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-930" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-930">
            <span class="todo-count-930">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-930" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
