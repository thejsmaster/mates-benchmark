// client/components/Component0212.ts
// Benchmark component #212 — identical logic to TodoPage, scoped class names.
...
import "./Component0212.css";

type Filter = "all" | "active" | "done";

export const Component0212 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0212] mounted");
    return () => console.log("[Component0212] unmounted");
  });
  ...
  return () => html`
    <div class="app-212">
      <header class="header-212">
        <h1>todos — Component0212</h1>
      </header>

      <main class="main-212">
        <input
          class="add-input-212 ${error() ? "add-input--error-212" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-212" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-212">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-212">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-212 ${filter() === f ? "filter-btn--active-212" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-212">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-212">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-212">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-212 ${todo.done ? "todo-item--done-212" : ""}">
                    <button
                      class="todo-check-212 ${todo.done ? "todo-check--done-212" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-212" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-212"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-212" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-212" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-212">
            <span class="todo-count-212">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-212" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
