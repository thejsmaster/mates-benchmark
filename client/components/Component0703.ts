// client/components/Component0703.ts
// Benchmark component #703 — identical logic to TodoPage, scoped class names.
...
import "./Component0703.css";

type Filter = "all" | "active" | "done";

export const Component0703 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0703] mounted");
    return () => console.log("[Component0703] unmounted");
  });
  ...
  return () => html`
    <div class="app-703">
      <header class="header-703">
        <h1>todos — Component0703</h1>
      </header>

      <main class="main-703">
        <input
          class="add-input-703 ${error() ? "add-input--error-703" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-703" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-703">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-703">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-703 ${filter() === f ? "filter-btn--active-703" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-703">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-703">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-703">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-703 ${todo.done ? "todo-item--done-703" : ""}">
                    <button
                      class="todo-check-703 ${todo.done ? "todo-check--done-703" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-703" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-703"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-703" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-703" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-703">
            <span class="todo-count-703">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-703" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
