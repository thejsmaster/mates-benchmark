// client/components/Component0790.ts
// Benchmark component #790 — identical logic to TodoPage, scoped class names.
...
import "./Component0790.css";

type Filter = "all" | "active" | "done";

export const Component0790 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0790] mounted");
    return () => console.log("[Component0790] unmounted");
  });
  ...
  return () => html`
    <div class="app-790">
      <header class="header-790">
        <h1>todos — Component0790</h1>
      </header>

      <main class="main-790">
        <input
          class="add-input-790 ${error() ? "add-input--error-790" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-790" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-790">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-790">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-790 ${filter() === f ? "filter-btn--active-790" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-790">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-790">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-790">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-790 ${todo.done ? "todo-item--done-790" : ""}">
                    <button
                      class="todo-check-790 ${todo.done ? "todo-check--done-790" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-790" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-790"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-790" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-790" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-790">
            <span class="todo-count-790">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-790" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
