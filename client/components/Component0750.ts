// client/components/Component0750.ts
// Benchmark component #750 — identical logic to TodoPage, scoped class names.
...
import "./Component0750.css";

type Filter = "all" | "active" | "done";

export const Component0750 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0750] mounted");
    return () => console.log("[Component0750] unmounted");
  });
  ...
  return () => html`
    <div class="app-750">
      <header class="header-750">
        <h1>todos — Component0750</h1>
      </header>

      <main class="main-750">
        <input
          class="add-input-750 ${error() ? "add-input--error-750" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-750" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-750">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-750">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-750 ${filter() === f ? "filter-btn--active-750" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-750">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-750">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-750">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-750 ${todo.done ? "todo-item--done-750" : ""}">
                    <button
                      class="todo-check-750 ${todo.done ? "todo-check--done-750" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-750" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-750"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-750" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-750" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-750">
            <span class="todo-count-750">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-750" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
