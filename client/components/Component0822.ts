// client/components/Component0822.ts
// Benchmark component #822 — identical logic to TodoPage, scoped class names.
...
import "./Component0822.css";

type Filter = "all" | "active" | "done";

export const Component0822 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0822] mounted");
    return () => console.log("[Component0822] unmounted");
  });
  ...
  return () => html`
    <div class="app-822">
      <header class="header-822">
        <h1>todos — Component0822</h1>
      </header>

      <main class="main-822">
        <input
          class="add-input-822 ${error() ? "add-input--error-822" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-822" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-822">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-822">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-822 ${filter() === f ? "filter-btn--active-822" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-822">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-822">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-822">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-822 ${todo.done ? "todo-item--done-822" : ""}">
                    <button
                      class="todo-check-822 ${todo.done ? "todo-check--done-822" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-822" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-822"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-822" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-822" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-822">
            <span class="todo-count-822">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-822" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
