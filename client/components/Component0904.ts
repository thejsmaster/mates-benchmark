// client/components/Component0904.ts
// Benchmark component #904 — identical logic to TodoPage, scoped class names.
...
import "./Component0904.css";

type Filter = "all" | "active" | "done";

export const Component0904 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0904] mounted");
    return () => console.log("[Component0904] unmounted");
  });
  ...
  return () => html`
    <div class="app-904">
      <header class="header-904">
        <h1>todos — Component0904</h1>
      </header>

      <main class="main-904">
        <input
          class="add-input-904 ${error() ? "add-input--error-904" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-904" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-904">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-904">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-904 ${filter() === f ? "filter-btn--active-904" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-904">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-904">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-904">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-904 ${todo.done ? "todo-item--done-904" : ""}">
                    <button
                      class="todo-check-904 ${todo.done ? "todo-check--done-904" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-904" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-904"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-904" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-904" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-904">
            <span class="todo-count-904">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-904" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
