// client/components/Component0177.ts
// Benchmark component #177 — identical logic to TodoPage, scoped class names.
...
import "./Component0177.css";

type Filter = "all" | "active" | "done";

export const Component0177 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0177] mounted");
    return () => console.log("[Component0177] unmounted");
  });
  ...
  return () => html`
    <div class="app-177">
      <header class="header-177">
        <h1>todos — Component0177</h1>
      </header>

      <main class="main-177">
        <input
          class="add-input-177 ${error() ? "add-input--error-177" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-177" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-177">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-177">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-177 ${filter() === f ? "filter-btn--active-177" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-177">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-177">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-177">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-177 ${todo.done ? "todo-item--done-177" : ""}">
                    <button
                      class="todo-check-177 ${todo.done ? "todo-check--done-177" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-177" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-177"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-177" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-177" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-177">
            <span class="todo-count-177">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-177" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
