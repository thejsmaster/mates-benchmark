// client/components/Component0902.ts
// Benchmark component #902 — identical logic to TodoPage, scoped class names.
...
import "./Component0902.css";

type Filter = "all" | "active" | "done";

export const Component0902 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0902] mounted");
    return () => console.log("[Component0902] unmounted");
  });
  ...
  return () => html`
    <div class="app-902">
      <header class="header-902">
        <h1>todos — Component0902</h1>
      </header>

      <main class="main-902">
        <input
          class="add-input-902 ${error() ? "add-input--error-902" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-902" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-902">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-902">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-902 ${filter() === f ? "filter-btn--active-902" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-902">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-902">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-902">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-902 ${todo.done ? "todo-item--done-902" : ""}">
                    <button
                      class="todo-check-902 ${todo.done ? "todo-check--done-902" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-902" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-902"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-902" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-902" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-902">
            <span class="todo-count-902">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-902" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
