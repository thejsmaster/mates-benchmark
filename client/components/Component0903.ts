// client/components/Component0903.ts
// Benchmark component #903 — identical logic to TodoPage, scoped class names.
...
import "./Component0903.css";

type Filter = "all" | "active" | "done";

export const Component0903 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0903] mounted");
    return () => console.log("[Component0903] unmounted");
  });
  ...
  return () => html`
    <div class="app-903">
      <header class="header-903">
        <h1>todos — Component0903</h1>
      </header>

      <main class="main-903">
        <input
          class="add-input-903 ${error() ? "add-input--error-903" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-903" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-903">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-903">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-903 ${filter() === f ? "filter-btn--active-903" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-903">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-903">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-903">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-903 ${todo.done ? "todo-item--done-903" : ""}">
                    <button
                      class="todo-check-903 ${todo.done ? "todo-check--done-903" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-903" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-903"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-903" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-903" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-903">
            <span class="todo-count-903">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-903" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
