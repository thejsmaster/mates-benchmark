// client/components/Component0198.ts
// Benchmark component #198 — identical logic to TodoPage, scoped class names.
...
import "./Component0198.css";

type Filter = "all" | "active" | "done";

export const Component0198 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0198] mounted");
    return () => console.log("[Component0198] unmounted");
  });
  ...
  return () => html`
    <div class="app-198">
      <header class="header-198">
        <h1>todos — Component0198</h1>
      </header>

      <main class="main-198">
        <input
          class="add-input-198 ${error() ? "add-input--error-198" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-198" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-198">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-198">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-198 ${filter() === f ? "filter-btn--active-198" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-198">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-198">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-198">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-198 ${todo.done ? "todo-item--done-198" : ""}">
                    <button
                      class="todo-check-198 ${todo.done ? "todo-check--done-198" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-198" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-198"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-198" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-198" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-198">
            <span class="todo-count-198">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-198" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
