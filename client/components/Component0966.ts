// client/components/Component0966.ts
// Benchmark component #966 — identical logic to TodoPage, scoped class names.
...
import "./Component0966.css";

type Filter = "all" | "active" | "done";

export const Component0966 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0966] mounted");
    return () => console.log("[Component0966] unmounted");
  });
  ...
  return () => html`
    <div class="app-966">
      <header class="header-966">
        <h1>todos — Component0966</h1>
      </header>

      <main class="main-966">
        <input
          class="add-input-966 ${error() ? "add-input--error-966" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-966" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-966">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-966">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-966 ${filter() === f ? "filter-btn--active-966" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-966">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-966">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-966">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-966 ${todo.done ? "todo-item--done-966" : ""}">
                    <button
                      class="todo-check-966 ${todo.done ? "todo-check--done-966" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-966" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-966"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-966" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-966" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-966">
            <span class="todo-count-966">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-966" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
