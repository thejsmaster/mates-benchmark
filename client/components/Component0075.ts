// client/components/Component0075.ts
// Benchmark component #75 — identical logic to TodoPage, scoped class names.
...
import "./Component0075.css";

type Filter = "all" | "active" | "done";

export const Component0075 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0075] mounted");
    return () => console.log("[Component0075] unmounted");
  });
  ...
  return () => html`
    <div class="app-75">
      <header class="header-75">
        <h1>todos — Component0075</h1>
      </header>

      <main class="main-75">
        <input
          class="add-input-75 ${error() ? "add-input--error-75" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-75" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-75">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-75">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-75 ${filter() === f ? "filter-btn--active-75" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-75">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-75">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-75">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-75 ${todo.done ? "todo-item--done-75" : ""}">
                    <button
                      class="todo-check-75 ${todo.done ? "todo-check--done-75" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-75" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-75"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-75" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-75" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-75">
            <span class="todo-count-75">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-75" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
