// client/components/Component0770.ts
// Benchmark component #770 — identical logic to TodoPage, scoped class names.
...
import "./Component0770.css";

type Filter = "all" | "active" | "done";

export const Component0770 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0770] mounted");
    return () => console.log("[Component0770] unmounted");
  });
  ...
  return () => html`
    <div class="app-770">
      <header class="header-770">
        <h1>todos — Component0770</h1>
      </header>

      <main class="main-770">
        <input
          class="add-input-770 ${error() ? "add-input--error-770" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-770" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-770">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-770">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-770 ${filter() === f ? "filter-btn--active-770" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-770">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-770">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-770">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-770 ${todo.done ? "todo-item--done-770" : ""}">
                    <button
                      class="todo-check-770 ${todo.done ? "todo-check--done-770" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-770" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-770"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-770" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-770" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-770">
            <span class="todo-count-770">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-770" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
