// client/components/Component0911.ts
// Benchmark component #911 — identical logic to TodoPage, scoped class names.
...
import "./Component0911.css";

type Filter = "all" | "active" | "done";

export const Component0911 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0911] mounted");
    return () => console.log("[Component0911] unmounted");
  });
  ...
  return () => html`
    <div class="app-911">
      <header class="header-911">
        <h1>todos — Component0911</h1>
      </header>

      <main class="main-911">
        <input
          class="add-input-911 ${error() ? "add-input--error-911" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-911" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-911">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-911">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-911 ${filter() === f ? "filter-btn--active-911" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-911">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-911">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-911">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-911 ${todo.done ? "todo-item--done-911" : ""}">
                    <button
                      class="todo-check-911 ${todo.done ? "todo-check--done-911" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-911" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-911"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-911" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-911" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-911">
            <span class="todo-count-911">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-911" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
