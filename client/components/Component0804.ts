// client/components/Component0804.ts
// Benchmark component #804 — identical logic to TodoPage, scoped class names.
...
import "./Component0804.css";

type Filter = "all" | "active" | "done";

export const Component0804 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0804] mounted");
    return () => console.log("[Component0804] unmounted");
  });
  ...
  return () => html`
    <div class="app-804">
      <header class="header-804">
        <h1>todos — Component0804</h1>
      </header>

      <main class="main-804">
        <input
          class="add-input-804 ${error() ? "add-input--error-804" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-804" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-804">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-804">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-804 ${filter() === f ? "filter-btn--active-804" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-804">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-804">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-804">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-804 ${todo.done ? "todo-item--done-804" : ""}">
                    <button
                      class="todo-check-804 ${todo.done ? "todo-check--done-804" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-804" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-804"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-804" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-804" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-804">
            <span class="todo-count-804">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-804" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
