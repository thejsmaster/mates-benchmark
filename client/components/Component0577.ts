// client/components/Component0577.ts
// Benchmark component #577 — identical logic to TodoPage, scoped class names.
...
import "./Component0577.css";

type Filter = "all" | "active" | "done";

export const Component0577 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0577] mounted");
    return () => console.log("[Component0577] unmounted");
  });
  ...
  return () => html`
    <div class="app-577">
      <header class="header-577">
        <h1>todos — Component0577</h1>
      </header>

      <main class="main-577">
        <input
          class="add-input-577 ${error() ? "add-input--error-577" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-577" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-577">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-577">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-577 ${filter() === f ? "filter-btn--active-577" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-577">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-577">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-577">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-577 ${todo.done ? "todo-item--done-577" : ""}">
                    <button
                      class="todo-check-577 ${todo.done ? "todo-check--done-577" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-577" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-577"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-577" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-577" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-577">
            <span class="todo-count-577">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-577" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
