// client/components/Component0801.ts
// Benchmark component #801 — identical logic to TodoPage, scoped class names.
...
import "./Component0801.css";

type Filter = "all" | "active" | "done";

export const Component0801 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0801] mounted");
    return () => console.log("[Component0801] unmounted");
  });
  ...
  return () => html`
    <div class="app-801">
      <header class="header-801">
        <h1>todos — Component0801</h1>
      </header>

      <main class="main-801">
        <input
          class="add-input-801 ${error() ? "add-input--error-801" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-801" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-801">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-801">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-801 ${filter() === f ? "filter-btn--active-801" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-801">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-801">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-801">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-801 ${todo.done ? "todo-item--done-801" : ""}">
                    <button
                      class="todo-check-801 ${todo.done ? "todo-check--done-801" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-801" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-801"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-801" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-801" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-801">
            <span class="todo-count-801">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-801" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
