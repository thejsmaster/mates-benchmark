// client/components/Component0818.ts
// Benchmark component #818 — identical logic to TodoPage, scoped class names.
...
import "./Component0818.css";

type Filter = "all" | "active" | "done";

export const Component0818 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0818] mounted");
    return () => console.log("[Component0818] unmounted");
  });
  ...
  return () => html`
    <div class="app-818">
      <header class="header-818">
        <h1>todos — Component0818</h1>
      </header>

      <main class="main-818">
        <input
          class="add-input-818 ${error() ? "add-input--error-818" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-818" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-818">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-818">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-818 ${filter() === f ? "filter-btn--active-818" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-818">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-818">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-818">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-818 ${todo.done ? "todo-item--done-818" : ""}">
                    <button
                      class="todo-check-818 ${todo.done ? "todo-check--done-818" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-818" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-818"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-818" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-818" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-818">
            <span class="todo-count-818">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-818" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
