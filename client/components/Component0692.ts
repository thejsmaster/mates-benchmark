// client/components/Component0692.ts
// Benchmark component #692 — identical logic to TodoPage, scoped class names.
...
import "./Component0692.css";

type Filter = "all" | "active" | "done";

export const Component0692 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0692] mounted");
    return () => console.log("[Component0692] unmounted");
  });
  ...
  return () => html`
    <div class="app-692">
      <header class="header-692">
        <h1>todos — Component0692</h1>
      </header>

      <main class="main-692">
        <input
          class="add-input-692 ${error() ? "add-input--error-692" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-692" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-692">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-692">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-692 ${filter() === f ? "filter-btn--active-692" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-692">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-692">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-692">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-692 ${todo.done ? "todo-item--done-692" : ""}">
                    <button
                      class="todo-check-692 ${todo.done ? "todo-check--done-692" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-692" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-692"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-692" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-692" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-692">
            <span class="todo-count-692">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-692" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
