// client/components/Component0151.ts
// Benchmark component #151 — identical logic to TodoPage, scoped class names.
...
import "./Component0151.css";

type Filter = "all" | "active" | "done";

export const Component0151 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0151] mounted");
    return () => console.log("[Component0151] unmounted");
  });
  ...
  return () => html`
    <div class="app-151">
      <header class="header-151">
        <h1>todos — Component0151</h1>
      </header>

      <main class="main-151">
        <input
          class="add-input-151 ${error() ? "add-input--error-151" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-151" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-151">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-151">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-151 ${filter() === f ? "filter-btn--active-151" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-151">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-151">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-151">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-151 ${todo.done ? "todo-item--done-151" : ""}">
                    <button
                      class="todo-check-151 ${todo.done ? "todo-check--done-151" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-151" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-151"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-151" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-151" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-151">
            <span class="todo-count-151">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-151" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
