// client/components/Component0101.ts
// Benchmark component #101 — identical logic to TodoPage, scoped class names.
...
import "./Component0101.css";

type Filter = "all" | "active" | "done";

export const Component0101 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0101] mounted");
    return () => console.log("[Component0101] unmounted");
  });
  ...
  return () => html`
    <div class="app-101">
      <header class="header-101">
        <h1>todos — Component0101</h1>
      </header>

      <main class="main-101">
        <input
          class="add-input-101 ${error() ? "add-input--error-101" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-101" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-101">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-101">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-101 ${filter() === f ? "filter-btn--active-101" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-101">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-101">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-101">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-101 ${todo.done ? "todo-item--done-101" : ""}">
                    <button
                      class="todo-check-101 ${todo.done ? "todo-check--done-101" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-101" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-101"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-101" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-101" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-101">
            <span class="todo-count-101">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-101" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
