// client/components/Component0642.ts
// Benchmark component #642 — identical logic to TodoPage, scoped class names.
...
import "./Component0642.css";

type Filter = "all" | "active" | "done";

export const Component0642 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0642] mounted");
    return () => console.log("[Component0642] unmounted");
  });
  ...
  return () => html`
    <div class="app-642">
      <header class="header-642">
        <h1>todos — Component0642</h1>
      </header>

      <main class="main-642">
        <input
          class="add-input-642 ${error() ? "add-input--error-642" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-642" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-642">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-642">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-642 ${filter() === f ? "filter-btn--active-642" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-642">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-642">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-642">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-642 ${todo.done ? "todo-item--done-642" : ""}">
                    <button
                      class="todo-check-642 ${todo.done ? "todo-check--done-642" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-642" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-642"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-642" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-642" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-642">
            <span class="todo-count-642">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-642" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
