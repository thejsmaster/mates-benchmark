// client/components/Component0346.ts
// Benchmark component #346 — identical logic to TodoPage, scoped class names.
...
import "./Component0346.css";

type Filter = "all" | "active" | "done";

export const Component0346 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0346] mounted");
    return () => console.log("[Component0346] unmounted");
  });
  ...
  return () => html`
    <div class="app-346">
      <header class="header-346">
        <h1>todos — Component0346</h1>
      </header>

      <main class="main-346">
        <input
          class="add-input-346 ${error() ? "add-input--error-346" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-346" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-346">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-346">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-346 ${filter() === f ? "filter-btn--active-346" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-346">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-346">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-346">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-346 ${todo.done ? "todo-item--done-346" : ""}">
                    <button
                      class="todo-check-346 ${todo.done ? "todo-check--done-346" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-346" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-346"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-346" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-346" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-346">
            <span class="todo-count-346">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-346" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
