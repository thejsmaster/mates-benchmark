// client/components/Component0222.ts
// Benchmark component #222 — identical logic to TodoPage, scoped class names.
...
import "./Component0222.css";

type Filter = "all" | "active" | "done";

export const Component0222 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0222] mounted");
    return () => console.log("[Component0222] unmounted");
  });
  ...
  return () => html`
    <div class="app-222">
      <header class="header-222">
        <h1>todos — Component0222</h1>
      </header>

      <main class="main-222">
        <input
          class="add-input-222 ${error() ? "add-input--error-222" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-222" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-222">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-222">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-222 ${filter() === f ? "filter-btn--active-222" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-222">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-222">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-222">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-222 ${todo.done ? "todo-item--done-222" : ""}">
                    <button
                      class="todo-check-222 ${todo.done ? "todo-check--done-222" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-222" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-222"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-222" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-222" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-222">
            <span class="todo-count-222">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-222" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
