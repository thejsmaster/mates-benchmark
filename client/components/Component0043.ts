// client/components/Component0043.ts
// Benchmark component #43 — identical logic to TodoPage, scoped class names.
...
import "./Component0043.css";

type Filter = "all" | "active" | "done";

export const Component0043 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0043] mounted");
    return () => console.log("[Component0043] unmounted");
  });
  ...
  return () => html`
    <div class="app-43">
      <header class="header-43">
        <h1>todos — Component0043</h1>
      </header>

      <main class="main-43">
        <input
          class="add-input-43 ${error() ? "add-input--error-43" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-43" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-43">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-43">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-43 ${filter() === f ? "filter-btn--active-43" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-43">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-43">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-43">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-43 ${todo.done ? "todo-item--done-43" : ""}">
                    <button
                      class="todo-check-43 ${todo.done ? "todo-check--done-43" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-43" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-43"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-43" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-43" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-43">
            <span class="todo-count-43">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-43" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
