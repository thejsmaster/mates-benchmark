// client/components/Component0150.ts
// Benchmark component #150 — identical logic to TodoPage, scoped class names.
...
import "./Component0150.css";

type Filter = "all" | "active" | "done";

export const Component0150 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0150] mounted");
    return () => console.log("[Component0150] unmounted");
  });
  ...
  return () => html`
    <div class="app-150">
      <header class="header-150">
        <h1>todos — Component0150</h1>
      </header>

      <main class="main-150">
        <input
          class="add-input-150 ${error() ? "add-input--error-150" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-150" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-150">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-150">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-150 ${filter() === f ? "filter-btn--active-150" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-150">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-150">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-150">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-150 ${todo.done ? "todo-item--done-150" : ""}">
                    <button
                      class="todo-check-150 ${todo.done ? "todo-check--done-150" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-150" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-150"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-150" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-150" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-150">
            <span class="todo-count-150">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-150" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
