// client/components/Component0843.ts
// Benchmark component #843 — identical logic to TodoPage, scoped class names.
...
import "./Component0843.css";

type Filter = "all" | "active" | "done";

export const Component0843 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0843] mounted");
    return () => console.log("[Component0843] unmounted");
  });
  ...
  return () => html`
    <div class="app-843">
      <header class="header-843">
        <h1>todos — Component0843</h1>
      </header>

      <main class="main-843">
        <input
          class="add-input-843 ${error() ? "add-input--error-843" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-843" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-843">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-843">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-843 ${filter() === f ? "filter-btn--active-843" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-843">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-843">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-843">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-843 ${todo.done ? "todo-item--done-843" : ""}">
                    <button
                      class="todo-check-843 ${todo.done ? "todo-check--done-843" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-843" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-843"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-843" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-843" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-843">
            <span class="todo-count-843">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-843" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
