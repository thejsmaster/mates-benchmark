// client/components/Component0788.ts
// Benchmark component #788 — identical logic to TodoPage, scoped class names.
...
import "./Component0788.css";

type Filter = "all" | "active" | "done";

export const Component0788 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0788] mounted");
    return () => console.log("[Component0788] unmounted");
  });
  ...
  return () => html`
    <div class="app-788">
      <header class="header-788">
        <h1>todos — Component0788</h1>
      </header>

      <main class="main-788">
        <input
          class="add-input-788 ${error() ? "add-input--error-788" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-788" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-788">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-788">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-788 ${filter() === f ? "filter-btn--active-788" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-788">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-788">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-788">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-788 ${todo.done ? "todo-item--done-788" : ""}">
                    <button
                      class="todo-check-788 ${todo.done ? "todo-check--done-788" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-788" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-788"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-788" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-788" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-788">
            <span class="todo-count-788">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-788" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
