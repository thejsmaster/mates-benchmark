// client/components/Component0015.ts
// Benchmark component #15 — identical logic to TodoPage, scoped class names.
...
import "./Component0015.css";

type Filter = "all" | "active" | "done";

export const Component0015 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0015] mounted");
    return () => console.log("[Component0015] unmounted");
  });
  ...
  return () => html`
    <div class="app-15">
      <header class="header-15">
        <h1>todos — Component0015</h1>
      </header>

      <main class="main-15">
        <input
          class="add-input-15 ${error() ? "add-input--error-15" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-15" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-15">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-15">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-15 ${filter() === f ? "filter-btn--active-15" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-15">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-15">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-15">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-15 ${todo.done ? "todo-item--done-15" : ""}">
                    <button
                      class="todo-check-15 ${todo.done ? "todo-check--done-15" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-15" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-15"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-15" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-15" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-15">
            <span class="todo-count-15">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-15" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
