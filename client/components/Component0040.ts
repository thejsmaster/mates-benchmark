// client/components/Component0040.ts
// Benchmark component #40 — identical logic to TodoPage, scoped class names.
...
import "./Component0040.css";

type Filter = "all" | "active" | "done";

export const Component0040 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0040] mounted");
    return () => console.log("[Component0040] unmounted");
  });
  ...
  return () => html`
    <div class="app-40">
      <header class="header-40">
        <h1>todos — Component0040</h1>
      </header>

      <main class="main-40">
        <input
          class="add-input-40 ${error() ? "add-input--error-40" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-40" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-40">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-40">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-40 ${filter() === f ? "filter-btn--active-40" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-40">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-40">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-40">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-40 ${todo.done ? "todo-item--done-40" : ""}">
                    <button
                      class="todo-check-40 ${todo.done ? "todo-check--done-40" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-40" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-40"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-40" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-40" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-40">
            <span class="todo-count-40">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-40" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
