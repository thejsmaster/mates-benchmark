// client/components/Component0319.ts
// Benchmark component #319 — identical logic to TodoPage, scoped class names.
...
import "./Component0319.css";

type Filter = "all" | "active" | "done";

export const Component0319 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0319] mounted");
    return () => console.log("[Component0319] unmounted");
  });
  ...
  return () => html`
    <div class="app-319">
      <header class="header-319">
        <h1>todos — Component0319</h1>
      </header>

      <main class="main-319">
        <input
          class="add-input-319 ${error() ? "add-input--error-319" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-319" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-319">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-319">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-319 ${filter() === f ? "filter-btn--active-319" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-319">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-319">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-319">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-319 ${todo.done ? "todo-item--done-319" : ""}">
                    <button
                      class="todo-check-319 ${todo.done ? "todo-check--done-319" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-319" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-319"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-319" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-319" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-319">
            <span class="todo-count-319">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-319" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
