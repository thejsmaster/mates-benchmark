// client/components/Component0249.ts
// Benchmark component #249 — identical logic to TodoPage, scoped class names.
...
import "./Component0249.css";

type Filter = "all" | "active" | "done";

export const Component0249 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0249] mounted");
    return () => console.log("[Component0249] unmounted");
  });
  ...
  return () => html`
    <div class="app-249">
      <header class="header-249">
        <h1>todos — Component0249</h1>
      </header>

      <main class="main-249">
        <input
          class="add-input-249 ${error() ? "add-input--error-249" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-249" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-249">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-249">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-249 ${filter() === f ? "filter-btn--active-249" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-249">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-249">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-249">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-249 ${todo.done ? "todo-item--done-249" : ""}">
                    <button
                      class="todo-check-249 ${todo.done ? "todo-check--done-249" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-249" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-249"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-249" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-249" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-249">
            <span class="todo-count-249">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-249" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
