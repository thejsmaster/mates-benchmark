// client/components/Component0356.ts
// Benchmark component #356 — identical logic to TodoPage, scoped class names.
...
import "./Component0356.css";

type Filter = "all" | "active" | "done";

export const Component0356 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0356] mounted");
    return () => console.log("[Component0356] unmounted");
  });
  ...
  return () => html`
    <div class="app-356">
      <header class="header-356">
        <h1>todos — Component0356</h1>
      </header>

      <main class="main-356">
        <input
          class="add-input-356 ${error() ? "add-input--error-356" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-356" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-356">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-356">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-356 ${filter() === f ? "filter-btn--active-356" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-356">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-356">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-356">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-356 ${todo.done ? "todo-item--done-356" : ""}">
                    <button
                      class="todo-check-356 ${todo.done ? "todo-check--done-356" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-356" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-356"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-356" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-356" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-356">
            <span class="todo-count-356">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-356" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
