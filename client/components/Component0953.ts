// client/components/Component0953.ts
// Benchmark component #953 — identical logic to TodoPage, scoped class names.
...
import "./Component0953.css";

type Filter = "all" | "active" | "done";

export const Component0953 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0953] mounted");
    return () => console.log("[Component0953] unmounted");
  });
  ...
  return () => html`
    <div class="app-953">
      <header class="header-953">
        <h1>todos — Component0953</h1>
      </header>

      <main class="main-953">
        <input
          class="add-input-953 ${error() ? "add-input--error-953" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-953" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-953">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-953">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-953 ${filter() === f ? "filter-btn--active-953" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-953">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-953">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-953">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-953 ${todo.done ? "todo-item--done-953" : ""}">
                    <button
                      class="todo-check-953 ${todo.done ? "todo-check--done-953" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-953" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-953"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-953" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-953" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-953">
            <span class="todo-count-953">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-953" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
