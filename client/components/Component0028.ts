// client/components/Component0028.ts
// Benchmark component #28 — identical logic to TodoPage, scoped class names.
...
import "./Component0028.css";

type Filter = "all" | "active" | "done";

export const Component0028 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0028] mounted");
    return () => console.log("[Component0028] unmounted");
  });
  ...
  return () => html`
    <div class="app-28">
      <header class="header-28">
        <h1>todos — Component0028</h1>
      </header>

      <main class="main-28">
        <input
          class="add-input-28 ${error() ? "add-input--error-28" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-28" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-28">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-28">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-28 ${filter() === f ? "filter-btn--active-28" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-28">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-28">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-28">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-28 ${todo.done ? "todo-item--done-28" : ""}">
                    <button
                      class="todo-check-28 ${todo.done ? "todo-check--done-28" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-28" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-28"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-28" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-28" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-28">
            <span class="todo-count-28">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-28" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
