// client/components/Component0762.ts
// Benchmark component #762 — identical logic to TodoPage, scoped class names.
...
import "./Component0762.css";

type Filter = "all" | "active" | "done";

export const Component0762 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0762] mounted");
    return () => console.log("[Component0762] unmounted");
  });
  ...
  return () => html`
    <div class="app-762">
      <header class="header-762">
        <h1>todos — Component0762</h1>
      </header>

      <main class="main-762">
        <input
          class="add-input-762 ${error() ? "add-input--error-762" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-762" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-762">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-762">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-762 ${filter() === f ? "filter-btn--active-762" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-762">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-762">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-762">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-762 ${todo.done ? "todo-item--done-762" : ""}">
                    <button
                      class="todo-check-762 ${todo.done ? "todo-check--done-762" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-762" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-762"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-762" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-762" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-762">
            <span class="todo-count-762">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-762" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
