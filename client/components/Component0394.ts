// client/components/Component0394.ts
// Benchmark component #394 — identical logic to TodoPage, scoped class names.
...
import "./Component0394.css";

type Filter = "all" | "active" | "done";

export const Component0394 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0394] mounted");
    return () => console.log("[Component0394] unmounted");
  });
  ...
  return () => html`
    <div class="app-394">
      <header class="header-394">
        <h1>todos — Component0394</h1>
      </header>

      <main class="main-394">
        <input
          class="add-input-394 ${error() ? "add-input--error-394" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-394" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-394">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-394">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-394 ${filter() === f ? "filter-btn--active-394" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-394">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-394">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-394">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-394 ${todo.done ? "todo-item--done-394" : ""}">
                    <button
                      class="todo-check-394 ${todo.done ? "todo-check--done-394" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-394" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-394"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-394" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-394" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-394">
            <span class="todo-count-394">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-394" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
