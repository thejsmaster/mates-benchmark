// client/components/Component0934.ts
// Benchmark component #934 — identical logic to TodoPage, scoped class names.
...
import "./Component0934.css";

type Filter = "all" | "active" | "done";

export const Component0934 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0934] mounted");
    return () => console.log("[Component0934] unmounted");
  });
  ...
  return () => html`
    <div class="app-934">
      <header class="header-934">
        <h1>todos — Component0934</h1>
      </header>

      <main class="main-934">
        <input
          class="add-input-934 ${error() ? "add-input--error-934" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-934" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-934">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-934">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-934 ${filter() === f ? "filter-btn--active-934" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-934">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-934">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-934">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-934 ${todo.done ? "todo-item--done-934" : ""}">
                    <button
                      class="todo-check-934 ${todo.done ? "todo-check--done-934" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-934" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-934"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-934" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-934" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-934">
            <span class="todo-count-934">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-934" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
