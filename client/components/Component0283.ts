// client/components/Component0283.ts
// Benchmark component #283 — identical logic to TodoPage, scoped class names.
...
import "./Component0283.css";

type Filter = "all" | "active" | "done";

export const Component0283 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0283] mounted");
    return () => console.log("[Component0283] unmounted");
  });
  ...
  return () => html`
    <div class="app-283">
      <header class="header-283">
        <h1>todos — Component0283</h1>
      </header>

      <main class="main-283">
        <input
          class="add-input-283 ${error() ? "add-input--error-283" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-283" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-283">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-283">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-283 ${filter() === f ? "filter-btn--active-283" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-283">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-283">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-283">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-283 ${todo.done ? "todo-item--done-283" : ""}">
                    <button
                      class="todo-check-283 ${todo.done ? "todo-check--done-283" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-283" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-283"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-283" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-283" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-283">
            <span class="todo-count-283">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-283" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
