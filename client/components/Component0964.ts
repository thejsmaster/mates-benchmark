// client/components/Component0964.ts
// Benchmark component #964 — identical logic to TodoPage, scoped class names.
...
import "./Component0964.css";

type Filter = "all" | "active" | "done";

export const Component0964 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0964] mounted");
    return () => console.log("[Component0964] unmounted");
  });
  ...
  return () => html`
    <div class="app-964">
      <header class="header-964">
        <h1>todos — Component0964</h1>
      </header>

      <main class="main-964">
        <input
          class="add-input-964 ${error() ? "add-input--error-964" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-964" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-964">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-964">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-964 ${filter() === f ? "filter-btn--active-964" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-964">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-964">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-964">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-964 ${todo.done ? "todo-item--done-964" : ""}">
                    <button
                      class="todo-check-964 ${todo.done ? "todo-check--done-964" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-964" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-964"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-964" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-964" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-964">
            <span class="todo-count-964">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-964" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
