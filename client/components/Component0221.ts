// client/components/Component0221.ts
// Benchmark component #221 — identical logic to TodoPage, scoped class names.
...
import "./Component0221.css";

type Filter = "all" | "active" | "done";

export const Component0221 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0221] mounted");
    return () => console.log("[Component0221] unmounted");
  });
  ...
  return () => html`
    <div class="app-221">
      <header class="header-221">
        <h1>todos — Component0221</h1>
      </header>

      <main class="main-221">
        <input
          class="add-input-221 ${error() ? "add-input--error-221" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-221" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-221">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-221">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-221 ${filter() === f ? "filter-btn--active-221" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-221">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-221">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-221">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-221 ${todo.done ? "todo-item--done-221" : ""}">
                    <button
                      class="todo-check-221 ${todo.done ? "todo-check--done-221" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-221" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-221"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-221" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-221" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-221">
            <span class="todo-count-221">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-221" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
