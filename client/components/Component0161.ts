// client/components/Component0161.ts
// Benchmark component #161 — identical logic to TodoPage, scoped class names.
...
import "./Component0161.css";

type Filter = "all" | "active" | "done";

export const Component0161 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0161] mounted");
    return () => console.log("[Component0161] unmounted");
  });
  ...
  return () => html`
    <div class="app-161">
      <header class="header-161">
        <h1>todos — Component0161</h1>
      </header>

      <main class="main-161">
        <input
          class="add-input-161 ${error() ? "add-input--error-161" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-161" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-161">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-161">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-161 ${filter() === f ? "filter-btn--active-161" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-161">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-161">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-161">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-161 ${todo.done ? "todo-item--done-161" : ""}">
                    <button
                      class="todo-check-161 ${todo.done ? "todo-check--done-161" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-161" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-161"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-161" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-161" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-161">
            <span class="todo-count-161">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-161" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
