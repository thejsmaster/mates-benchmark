// client/components/Component0295.ts
// Benchmark component #295 — identical logic to TodoPage, scoped class names.
...
import "./Component0295.css";

type Filter = "all" | "active" | "done";

export const Component0295 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0295] mounted");
    return () => console.log("[Component0295] unmounted");
  });
  ...
  return () => html`
    <div class="app-295">
      <header class="header-295">
        <h1>todos — Component0295</h1>
      </header>

      <main class="main-295">
        <input
          class="add-input-295 ${error() ? "add-input--error-295" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-295" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-295">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-295">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-295 ${filter() === f ? "filter-btn--active-295" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-295">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-295">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-295">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-295 ${todo.done ? "todo-item--done-295" : ""}">
                    <button
                      class="todo-check-295 ${todo.done ? "todo-check--done-295" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-295" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-295"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-295" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-295" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-295">
            <span class="todo-count-295">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-295" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
