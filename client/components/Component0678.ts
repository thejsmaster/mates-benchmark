// client/components/Component0678.ts
// Benchmark component #678 — identical logic to TodoPage, scoped class names.
...
import "./Component0678.css";

type Filter = "all" | "active" | "done";

export const Component0678 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0678] mounted");
    return () => console.log("[Component0678] unmounted");
  });
  ...
  return () => html`
    <div class="app-678">
      <header class="header-678">
        <h1>todos — Component0678</h1>
      </header>

      <main class="main-678">
        <input
          class="add-input-678 ${error() ? "add-input--error-678" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-678" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-678">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-678">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-678 ${filter() === f ? "filter-btn--active-678" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-678">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-678">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-678">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-678 ${todo.done ? "todo-item--done-678" : ""}">
                    <button
                      class="todo-check-678 ${todo.done ? "todo-check--done-678" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-678" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-678"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-678" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-678" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-678">
            <span class="todo-count-678">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-678" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
