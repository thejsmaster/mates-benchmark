// client/components/Component0402.ts
// Benchmark component #402 — identical logic to TodoPage, scoped class names.
...
import "./Component0402.css";

type Filter = "all" | "active" | "done";

export const Component0402 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0402] mounted");
    return () => console.log("[Component0402] unmounted");
  });
  ...
  return () => html`
    <div class="app-402">
      <header class="header-402">
        <h1>todos — Component0402</h1>
      </header>

      <main class="main-402">
        <input
          class="add-input-402 ${error() ? "add-input--error-402" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-402" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-402">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-402">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-402 ${filter() === f ? "filter-btn--active-402" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-402">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-402">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-402">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-402 ${todo.done ? "todo-item--done-402" : ""}">
                    <button
                      class="todo-check-402 ${todo.done ? "todo-check--done-402" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-402" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-402"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-402" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-402" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-402">
            <span class="todo-count-402">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-402" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
