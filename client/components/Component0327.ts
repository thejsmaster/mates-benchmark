// client/components/Component0327.ts
// Benchmark component #327 — identical logic to TodoPage, scoped class names.
...
import "./Component0327.css";

type Filter = "all" | "active" | "done";

export const Component0327 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0327] mounted");
    return () => console.log("[Component0327] unmounted");
  });
  ...
  return () => html`
    <div class="app-327">
      <header class="header-327">
        <h1>todos — Component0327</h1>
      </header>

      <main class="main-327">
        <input
          class="add-input-327 ${error() ? "add-input--error-327" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-327" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-327">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-327">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-327 ${filter() === f ? "filter-btn--active-327" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-327">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-327">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-327">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-327 ${todo.done ? "todo-item--done-327" : ""}">
                    <button
                      class="todo-check-327 ${todo.done ? "todo-check--done-327" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-327" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-327"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-327" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-327" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-327">
            <span class="todo-count-327">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-327" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
