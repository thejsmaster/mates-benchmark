// client/components/Component0768.ts
// Benchmark component #768 — identical logic to TodoPage, scoped class names.
...
import "./Component0768.css";

type Filter = "all" | "active" | "done";

export const Component0768 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0768] mounted");
    return () => console.log("[Component0768] unmounted");
  });
  ...
  return () => html`
    <div class="app-768">
      <header class="header-768">
        <h1>todos — Component0768</h1>
      </header>

      <main class="main-768">
        <input
          class="add-input-768 ${error() ? "add-input--error-768" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-768" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-768">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-768">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-768 ${filter() === f ? "filter-btn--active-768" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-768">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-768">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-768">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-768 ${todo.done ? "todo-item--done-768" : ""}">
                    <button
                      class="todo-check-768 ${todo.done ? "todo-check--done-768" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-768" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-768"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-768" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-768" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-768">
            <span class="todo-count-768">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-768" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
