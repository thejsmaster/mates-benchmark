// client/components/Component0738.ts
// Benchmark component #738 — identical logic to TodoPage, scoped class names.
...
import "./Component0738.css";

type Filter = "all" | "active" | "done";

export const Component0738 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0738] mounted");
    return () => console.log("[Component0738] unmounted");
  });
  ...
  return () => html`
    <div class="app-738">
      <header class="header-738">
        <h1>todos — Component0738</h1>
      </header>

      <main class="main-738">
        <input
          class="add-input-738 ${error() ? "add-input--error-738" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-738" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-738">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-738">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-738 ${filter() === f ? "filter-btn--active-738" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-738">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-738">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-738">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-738 ${todo.done ? "todo-item--done-738" : ""}">
                    <button
                      class="todo-check-738 ${todo.done ? "todo-check--done-738" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-738" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-738"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-738" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-738" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-738">
            <span class="todo-count-738">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-738" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
