// client/components/Component0888.ts
// Benchmark component #888 — identical logic to TodoPage, scoped class names.
...
import "./Component0888.css";

type Filter = "all" | "active" | "done";

export const Component0888 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0888] mounted");
    return () => console.log("[Component0888] unmounted");
  });
  ...
  return () => html`
    <div class="app-888">
      <header class="header-888">
        <h1>todos — Component0888</h1>
      </header>

      <main class="main-888">
        <input
          class="add-input-888 ${error() ? "add-input--error-888" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-888" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-888">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-888">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-888 ${filter() === f ? "filter-btn--active-888" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-888">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-888">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-888">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-888 ${todo.done ? "todo-item--done-888" : ""}">
                    <button
                      class="todo-check-888 ${todo.done ? "todo-check--done-888" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-888" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-888"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-888" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-888" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-888">
            <span class="todo-count-888">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-888" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
