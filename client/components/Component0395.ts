// client/components/Component0395.ts
// Benchmark component #395 — identical logic to TodoPage, scoped class names.
...
import "./Component0395.css";

type Filter = "all" | "active" | "done";

export const Component0395 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0395] mounted");
    return () => console.log("[Component0395] unmounted");
  });
  ...
  return () => html`
    <div class="app-395">
      <header class="header-395">
        <h1>todos — Component0395</h1>
      </header>

      <main class="main-395">
        <input
          class="add-input-395 ${error() ? "add-input--error-395" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-395" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-395">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-395">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-395 ${filter() === f ? "filter-btn--active-395" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-395">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-395">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-395">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-395 ${todo.done ? "todo-item--done-395" : ""}">
                    <button
                      class="todo-check-395 ${todo.done ? "todo-check--done-395" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-395" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-395"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-395" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-395" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-395">
            <span class="todo-count-395">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-395" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
