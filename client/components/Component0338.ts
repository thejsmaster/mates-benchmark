// client/components/Component0338.ts
// Benchmark component #338 — identical logic to TodoPage, scoped class names.
...
import "./Component0338.css";

type Filter = "all" | "active" | "done";

export const Component0338 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0338] mounted");
    return () => console.log("[Component0338] unmounted");
  });
  ...
  return () => html`
    <div class="app-338">
      <header class="header-338">
        <h1>todos — Component0338</h1>
      </header>

      <main class="main-338">
        <input
          class="add-input-338 ${error() ? "add-input--error-338" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-338" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-338">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-338">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-338 ${filter() === f ? "filter-btn--active-338" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-338">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-338">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-338">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-338 ${todo.done ? "todo-item--done-338" : ""}">
                    <button
                      class="todo-check-338 ${todo.done ? "todo-check--done-338" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-338" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-338"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-338" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-338" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-338">
            <span class="todo-count-338">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-338" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
