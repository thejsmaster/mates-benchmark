// client/components/Component0136.ts
// Benchmark component #136 — identical logic to TodoPage, scoped class names.
...
import "./Component0136.css";

type Filter = "all" | "active" | "done";

export const Component0136 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0136] mounted");
    return () => console.log("[Component0136] unmounted");
  });
  ...
  return () => html`
    <div class="app-136">
      <header class="header-136">
        <h1>todos — Component0136</h1>
      </header>

      <main class="main-136">
        <input
          class="add-input-136 ${error() ? "add-input--error-136" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-136" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-136">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-136">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-136 ${filter() === f ? "filter-btn--active-136" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-136">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-136">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-136">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-136 ${todo.done ? "todo-item--done-136" : ""}">
                    <button
                      class="todo-check-136 ${todo.done ? "todo-check--done-136" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-136" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-136"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-136" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-136" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-136">
            <span class="todo-count-136">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-136" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
