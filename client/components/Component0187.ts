// client/components/Component0187.ts
// Benchmark component #187 — identical logic to TodoPage, scoped class names.
...
import "./Component0187.css";

type Filter = "all" | "active" | "done";

export const Component0187 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0187] mounted");
    return () => console.log("[Component0187] unmounted");
  });
  ...
  return () => html`
    <div class="app-187">
      <header class="header-187">
        <h1>todos — Component0187</h1>
      </header>

      <main class="main-187">
        <input
          class="add-input-187 ${error() ? "add-input--error-187" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-187" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-187">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-187">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-187 ${filter() === f ? "filter-btn--active-187" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-187">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-187">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-187">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-187 ${todo.done ? "todo-item--done-187" : ""}">
                    <button
                      class="todo-check-187 ${todo.done ? "todo-check--done-187" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-187" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-187"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-187" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-187" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-187">
            <span class="todo-count-187">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-187" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
