// client/components/Component0581.ts
// Benchmark component #581 — identical logic to TodoPage, scoped class names.
...
import "./Component0581.css";

type Filter = "all" | "active" | "done";

export const Component0581 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0581] mounted");
    return () => console.log("[Component0581] unmounted");
  });
  ...
  return () => html`
    <div class="app-581">
      <header class="header-581">
        <h1>todos — Component0581</h1>
      </header>

      <main class="main-581">
        <input
          class="add-input-581 ${error() ? "add-input--error-581" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-581" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-581">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-581">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-581 ${filter() === f ? "filter-btn--active-581" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-581">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-581">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-581">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-581 ${todo.done ? "todo-item--done-581" : ""}">
                    <button
                      class="todo-check-581 ${todo.done ? "todo-check--done-581" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-581" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-581"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-581" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-581" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-581">
            <span class="todo-count-581">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-581" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
