// client/components/Component0458.ts
// Benchmark component #458 — identical logic to TodoPage, scoped class names.
...
import "./Component0458.css";

type Filter = "all" | "active" | "done";

export const Component0458 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0458] mounted");
    return () => console.log("[Component0458] unmounted");
  });
  ...
  return () => html`
    <div class="app-458">
      <header class="header-458">
        <h1>todos — Component0458</h1>
      </header>

      <main class="main-458">
        <input
          class="add-input-458 ${error() ? "add-input--error-458" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-458" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-458">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-458">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-458 ${filter() === f ? "filter-btn--active-458" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-458">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-458">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-458">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-458 ${todo.done ? "todo-item--done-458" : ""}">
                    <button
                      class="todo-check-458 ${todo.done ? "todo-check--done-458" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-458" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-458"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-458" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-458" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-458">
            <span class="todo-count-458">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-458" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
