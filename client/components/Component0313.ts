// client/components/Component0313.ts
// Benchmark component #313 — identical logic to TodoPage, scoped class names.
...
import "./Component0313.css";

type Filter = "all" | "active" | "done";

export const Component0313 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0313] mounted");
    return () => console.log("[Component0313] unmounted");
  });
  ...
  return () => html`
    <div class="app-313">
      <header class="header-313">
        <h1>todos — Component0313</h1>
      </header>

      <main class="main-313">
        <input
          class="add-input-313 ${error() ? "add-input--error-313" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-313" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-313">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-313">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-313 ${filter() === f ? "filter-btn--active-313" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-313">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-313">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-313">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-313 ${todo.done ? "todo-item--done-313" : ""}">
                    <button
                      class="todo-check-313 ${todo.done ? "todo-check--done-313" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-313" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-313"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-313" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-313" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-313">
            <span class="todo-count-313">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-313" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
