// client/components/Component0399.ts
// Benchmark component #399 — identical logic to TodoPage, scoped class names.
...
import "./Component0399.css";

type Filter = "all" | "active" | "done";

export const Component0399 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0399] mounted");
    return () => console.log("[Component0399] unmounted");
  });
  ...
  return () => html`
    <div class="app-399">
      <header class="header-399">
        <h1>todos — Component0399</h1>
      </header>

      <main class="main-399">
        <input
          class="add-input-399 ${error() ? "add-input--error-399" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-399" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-399">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-399">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-399 ${filter() === f ? "filter-btn--active-399" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-399">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-399">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-399">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-399 ${todo.done ? "todo-item--done-399" : ""}">
                    <button
                      class="todo-check-399 ${todo.done ? "todo-check--done-399" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-399" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-399"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-399" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-399" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-399">
            <span class="todo-count-399">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-399" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
