// client/components/Component0456.ts
// Benchmark component #456 — identical logic to TodoPage, scoped class names.
...
import "./Component0456.css";

type Filter = "all" | "active" | "done";

export const Component0456 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0456] mounted");
    return () => console.log("[Component0456] unmounted");
  });
  ...
  return () => html`
    <div class="app-456">
      <header class="header-456">
        <h1>todos — Component0456</h1>
      </header>

      <main class="main-456">
        <input
          class="add-input-456 ${error() ? "add-input--error-456" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-456" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-456">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-456">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-456 ${filter() === f ? "filter-btn--active-456" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-456">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-456">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-456">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-456 ${todo.done ? "todo-item--done-456" : ""}">
                    <button
                      class="todo-check-456 ${todo.done ? "todo-check--done-456" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-456" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-456"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-456" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-456" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-456">
            <span class="todo-count-456">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-456" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
