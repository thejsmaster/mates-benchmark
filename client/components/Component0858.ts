// client/components/Component0858.ts
// Benchmark component #858 — identical logic to TodoPage, scoped class names.
...
import "./Component0858.css";

type Filter = "all" | "active" | "done";

export const Component0858 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0858] mounted");
    return () => console.log("[Component0858] unmounted");
  });
  ...
  return () => html`
    <div class="app-858">
      <header class="header-858">
        <h1>todos — Component0858</h1>
      </header>

      <main class="main-858">
        <input
          class="add-input-858 ${error() ? "add-input--error-858" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-858" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-858">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-858">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-858 ${filter() === f ? "filter-btn--active-858" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-858">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-858">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-858">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-858 ${todo.done ? "todo-item--done-858" : ""}">
                    <button
                      class="todo-check-858 ${todo.done ? "todo-check--done-858" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-858" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-858"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-858" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-858" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-858">
            <span class="todo-count-858">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-858" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
