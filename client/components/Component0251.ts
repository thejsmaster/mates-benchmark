// client/components/Component0251.ts
// Benchmark component #251 — identical logic to TodoPage, scoped class names.
...
import "./Component0251.css";

type Filter = "all" | "active" | "done";

export const Component0251 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0251] mounted");
    return () => console.log("[Component0251] unmounted");
  });
  ...
  return () => html`
    <div class="app-251">
      <header class="header-251">
        <h1>todos — Component0251</h1>
      </header>

      <main class="main-251">
        <input
          class="add-input-251 ${error() ? "add-input--error-251" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-251" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-251">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-251">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-251 ${filter() === f ? "filter-btn--active-251" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-251">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-251">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-251">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-251 ${todo.done ? "todo-item--done-251" : ""}">
                    <button
                      class="todo-check-251 ${todo.done ? "todo-check--done-251" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-251" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-251"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-251" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-251" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-251">
            <span class="todo-count-251">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-251" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
