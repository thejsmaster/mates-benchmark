// client/components/Component0176.ts
// Benchmark component #176 — identical logic to TodoPage, scoped class names.
...
import "./Component0176.css";

type Filter = "all" | "active" | "done";

export const Component0176 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0176] mounted");
    return () => console.log("[Component0176] unmounted");
  });
  ...
  return () => html`
    <div class="app-176">
      <header class="header-176">
        <h1>todos — Component0176</h1>
      </header>

      <main class="main-176">
        <input
          class="add-input-176 ${error() ? "add-input--error-176" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-176" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-176">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-176">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-176 ${filter() === f ? "filter-btn--active-176" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-176">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-176">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-176">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-176 ${todo.done ? "todo-item--done-176" : ""}">
                    <button
                      class="todo-check-176 ${todo.done ? "todo-check--done-176" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-176" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-176"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-176" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-176" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-176">
            <span class="todo-count-176">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-176" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
