// client/components/Component0952.ts
// Benchmark component #952 — identical logic to TodoPage, scoped class names.
...
import "./Component0952.css";

type Filter = "all" | "active" | "done";

export const Component0952 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0952] mounted");
    return () => console.log("[Component0952] unmounted");
  });
  ...
  return () => html`
    <div class="app-952">
      <header class="header-952">
        <h1>todos — Component0952</h1>
      </header>

      <main class="main-952">
        <input
          class="add-input-952 ${error() ? "add-input--error-952" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-952" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-952">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-952">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-952 ${filter() === f ? "filter-btn--active-952" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-952">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-952">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-952">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-952 ${todo.done ? "todo-item--done-952" : ""}">
                    <button
                      class="todo-check-952 ${todo.done ? "todo-check--done-952" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-952" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-952"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-952" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-952" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-952">
            <span class="todo-count-952">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-952" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
