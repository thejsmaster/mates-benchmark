// client/components/Component0884.ts
// Benchmark component #884 — identical logic to TodoPage, scoped class names.
...
import "./Component0884.css";

type Filter = "all" | "active" | "done";

export const Component0884 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0884] mounted");
    return () => console.log("[Component0884] unmounted");
  });
  ...
  return () => html`
    <div class="app-884">
      <header class="header-884">
        <h1>todos — Component0884</h1>
      </header>

      <main class="main-884">
        <input
          class="add-input-884 ${error() ? "add-input--error-884" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-884" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-884">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-884">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-884 ${filter() === f ? "filter-btn--active-884" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-884">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-884">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-884">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-884 ${todo.done ? "todo-item--done-884" : ""}">
                    <button
                      class="todo-check-884 ${todo.done ? "todo-check--done-884" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-884" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-884"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-884" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-884" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-884">
            <span class="todo-count-884">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-884" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
