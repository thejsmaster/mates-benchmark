// client/components/Component0349.ts
// Benchmark component #349 — identical logic to TodoPage, scoped class names.
...
import "./Component0349.css";

type Filter = "all" | "active" | "done";

export const Component0349 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0349] mounted");
    return () => console.log("[Component0349] unmounted");
  });
  ...
  return () => html`
    <div class="app-349">
      <header class="header-349">
        <h1>todos — Component0349</h1>
      </header>

      <main class="main-349">
        <input
          class="add-input-349 ${error() ? "add-input--error-349" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-349" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-349">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-349">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-349 ${filter() === f ? "filter-btn--active-349" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-349">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-349">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-349">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-349 ${todo.done ? "todo-item--done-349" : ""}">
                    <button
                      class="todo-check-349 ${todo.done ? "todo-check--done-349" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-349" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-349"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-349" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-349" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-349">
            <span class="todo-count-349">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-349" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
