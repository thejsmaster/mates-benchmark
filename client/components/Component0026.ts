// client/components/Component0026.ts
// Benchmark component #26 — identical logic to TodoPage, scoped class names.
...
import "./Component0026.css";

type Filter = "all" | "active" | "done";

export const Component0026 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0026] mounted");
    return () => console.log("[Component0026] unmounted");
  });
  ...
  return () => html`
    <div class="app-26">
      <header class="header-26">
        <h1>todos — Component0026</h1>
      </header>

      <main class="main-26">
        <input
          class="add-input-26 ${error() ? "add-input--error-26" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-26" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-26">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-26">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-26 ${filter() === f ? "filter-btn--active-26" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-26">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-26">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-26">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-26 ${todo.done ? "todo-item--done-26" : ""}">
                    <button
                      class="todo-check-26 ${todo.done ? "todo-check--done-26" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-26" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-26"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-26" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-26" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-26">
            <span class="todo-count-26">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-26" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
