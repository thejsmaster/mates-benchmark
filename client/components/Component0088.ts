// client/components/Component0088.ts
// Benchmark component #88 — identical logic to TodoPage, scoped class names.
...
import "./Component0088.css";

type Filter = "all" | "active" | "done";

export const Component0088 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0088] mounted");
    return () => console.log("[Component0088] unmounted");
  });
  ...
  return () => html`
    <div class="app-88">
      <header class="header-88">
        <h1>todos — Component0088</h1>
      </header>

      <main class="main-88">
        <input
          class="add-input-88 ${error() ? "add-input--error-88" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-88" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-88">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-88">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-88 ${filter() === f ? "filter-btn--active-88" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-88">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-88">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-88">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-88 ${todo.done ? "todo-item--done-88" : ""}">
                    <button
                      class="todo-check-88 ${todo.done ? "todo-check--done-88" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-88" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-88"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-88" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-88" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-88">
            <span class="todo-count-88">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-88" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
