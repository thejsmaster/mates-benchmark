// client/components/Component0179.ts
// Benchmark component #179 — identical logic to TodoPage, scoped class names.
...
import "./Component0179.css";

type Filter = "all" | "active" | "done";

export const Component0179 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0179] mounted");
    return () => console.log("[Component0179] unmounted");
  });
  ...
  return () => html`
    <div class="app-179">
      <header class="header-179">
        <h1>todos — Component0179</h1>
      </header>

      <main class="main-179">
        <input
          class="add-input-179 ${error() ? "add-input--error-179" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-179" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-179">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-179">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-179 ${filter() === f ? "filter-btn--active-179" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-179">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-179">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-179">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-179 ${todo.done ? "todo-item--done-179" : ""}">
                    <button
                      class="todo-check-179 ${todo.done ? "todo-check--done-179" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-179" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-179"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-179" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-179" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-179">
            <span class="todo-count-179">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-179" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
