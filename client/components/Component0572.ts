// client/components/Component0572.ts
// Benchmark component #572 — identical logic to TodoPage, scoped class names.
...
import "./Component0572.css";

type Filter = "all" | "active" | "done";

export const Component0572 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0572] mounted");
    return () => console.log("[Component0572] unmounted");
  });
  ...
  return () => html`
    <div class="app-572">
      <header class="header-572">
        <h1>todos — Component0572</h1>
      </header>

      <main class="main-572">
        <input
          class="add-input-572 ${error() ? "add-input--error-572" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-572" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-572">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-572">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-572 ${filter() === f ? "filter-btn--active-572" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-572">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-572">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-572">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-572 ${todo.done ? "todo-item--done-572" : ""}">
                    <button
                      class="todo-check-572 ${todo.done ? "todo-check--done-572" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-572" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-572"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-572" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-572" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-572">
            <span class="todo-count-572">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-572" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
