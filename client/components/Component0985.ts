// client/components/Component0985.ts
// Benchmark component #985 — identical logic to TodoPage, scoped class names.
...
import "./Component0985.css";

type Filter = "all" | "active" | "done";

export const Component0985 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0985] mounted");
    return () => console.log("[Component0985] unmounted");
  });
  ...
  return () => html`
    <div class="app-985">
      <header class="header-985">
        <h1>todos — Component0985</h1>
      </header>

      <main class="main-985">
        <input
          class="add-input-985 ${error() ? "add-input--error-985" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-985" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-985">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-985">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-985 ${filter() === f ? "filter-btn--active-985" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-985">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-985">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-985">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-985 ${todo.done ? "todo-item--done-985" : ""}">
                    <button
                      class="todo-check-985 ${todo.done ? "todo-check--done-985" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-985" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-985"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-985" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-985" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-985">
            <span class="todo-count-985">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-985" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
