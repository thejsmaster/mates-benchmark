// client/components/Component0647.ts
// Benchmark component #647 — identical logic to TodoPage, scoped class names.
...
import "./Component0647.css";

type Filter = "all" | "active" | "done";

export const Component0647 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0647] mounted");
    return () => console.log("[Component0647] unmounted");
  });
  ...
  return () => html`
    <div class="app-647">
      <header class="header-647">
        <h1>todos — Component0647</h1>
      </header>

      <main class="main-647">
        <input
          class="add-input-647 ${error() ? "add-input--error-647" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-647" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-647">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-647">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-647 ${filter() === f ? "filter-btn--active-647" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-647">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-647">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-647">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-647 ${todo.done ? "todo-item--done-647" : ""}">
                    <button
                      class="todo-check-647 ${todo.done ? "todo-check--done-647" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-647" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-647"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-647" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-647" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-647">
            <span class="todo-count-647">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-647" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
