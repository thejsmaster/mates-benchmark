// client/components/Component0314.ts
// Benchmark component #314 — identical logic to TodoPage, scoped class names.
...
import "./Component0314.css";

type Filter = "all" | "active" | "done";

export const Component0314 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0314] mounted");
    return () => console.log("[Component0314] unmounted");
  });
  ...
  return () => html`
    <div class="app-314">
      <header class="header-314">
        <h1>todos — Component0314</h1>
      </header>

      <main class="main-314">
        <input
          class="add-input-314 ${error() ? "add-input--error-314" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-314" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-314">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-314">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-314 ${filter() === f ? "filter-btn--active-314" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-314">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-314">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-314">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-314 ${todo.done ? "todo-item--done-314" : ""}">
                    <button
                      class="todo-check-314 ${todo.done ? "todo-check--done-314" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-314" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-314"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-314" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-314" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-314">
            <span class="todo-count-314">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-314" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
