// client/components/Component0341.ts
// Benchmark component #341 — identical logic to TodoPage, scoped class names.
...
import "./Component0341.css";

type Filter = "all" | "active" | "done";

export const Component0341 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0341] mounted");
    return () => console.log("[Component0341] unmounted");
  });
  ...
  return () => html`
    <div class="app-341">
      <header class="header-341">
        <h1>todos — Component0341</h1>
      </header>

      <main class="main-341">
        <input
          class="add-input-341 ${error() ? "add-input--error-341" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-341" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-341">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-341">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-341 ${filter() === f ? "filter-btn--active-341" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-341">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-341">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-341">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-341 ${todo.done ? "todo-item--done-341" : ""}">
                    <button
                      class="todo-check-341 ${todo.done ? "todo-check--done-341" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-341" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-341"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-341" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-341" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-341">
            <span class="todo-count-341">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-341" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
