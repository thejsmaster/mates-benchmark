// client/components/Component0590.ts
// Benchmark component #590 — identical logic to TodoPage, scoped class names.
...
import "./Component0590.css";

type Filter = "all" | "active" | "done";

export const Component0590 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0590] mounted");
    return () => console.log("[Component0590] unmounted");
  });
  ...
  return () => html`
    <div class="app-590">
      <header class="header-590">
        <h1>todos — Component0590</h1>
      </header>

      <main class="main-590">
        <input
          class="add-input-590 ${error() ? "add-input--error-590" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-590" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-590">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-590">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-590 ${filter() === f ? "filter-btn--active-590" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-590">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-590">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-590">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-590 ${todo.done ? "todo-item--done-590" : ""}">
                    <button
                      class="todo-check-590 ${todo.done ? "todo-check--done-590" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-590" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-590"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-590" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-590" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-590">
            <span class="todo-count-590">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-590" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
