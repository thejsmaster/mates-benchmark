// client/components/Component0941.ts
// Benchmark component #941 — identical logic to TodoPage, scoped class names.
...
import "./Component0941.css";

type Filter = "all" | "active" | "done";

export const Component0941 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0941] mounted");
    return () => console.log("[Component0941] unmounted");
  });
  ...
  return () => html`
    <div class="app-941">
      <header class="header-941">
        <h1>todos — Component0941</h1>
      </header>

      <main class="main-941">
        <input
          class="add-input-941 ${error() ? "add-input--error-941" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-941" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-941">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-941">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-941 ${filter() === f ? "filter-btn--active-941" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-941">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-941">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-941">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-941 ${todo.done ? "todo-item--done-941" : ""}">
                    <button
                      class="todo-check-941 ${todo.done ? "todo-check--done-941" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-941" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-941"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-941" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-941" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-941">
            <span class="todo-count-941">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-941" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
