// client/components/Component0123.ts
// Benchmark component #123 — identical logic to TodoPage, scoped class names.
...
import "./Component0123.css";

type Filter = "all" | "active" | "done";

export const Component0123 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0123] mounted");
    return () => console.log("[Component0123] unmounted");
  });
  ...
  return () => html`
    <div class="app-123">
      <header class="header-123">
        <h1>todos — Component0123</h1>
      </header>

      <main class="main-123">
        <input
          class="add-input-123 ${error() ? "add-input--error-123" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-123" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-123">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-123">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-123 ${filter() === f ? "filter-btn--active-123" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-123">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-123">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-123">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-123 ${todo.done ? "todo-item--done-123" : ""}">
                    <button
                      class="todo-check-123 ${todo.done ? "todo-check--done-123" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-123" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-123"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-123" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-123" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-123">
            <span class="todo-count-123">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-123" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
