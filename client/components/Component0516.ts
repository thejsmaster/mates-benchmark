// client/components/Component0516.ts
// Benchmark component #516 — identical logic to TodoPage, scoped class names.
...
import "./Component0516.css";

type Filter = "all" | "active" | "done";

export const Component0516 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0516] mounted");
    return () => console.log("[Component0516] unmounted");
  });
  ...
  return () => html`
    <div class="app-516">
      <header class="header-516">
        <h1>todos — Component0516</h1>
      </header>

      <main class="main-516">
        <input
          class="add-input-516 ${error() ? "add-input--error-516" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-516" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-516">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-516">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-516 ${filter() === f ? "filter-btn--active-516" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-516">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-516">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-516">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-516 ${todo.done ? "todo-item--done-516" : ""}">
                    <button
                      class="todo-check-516 ${todo.done ? "todo-check--done-516" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-516" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-516"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-516" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-516" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-516">
            <span class="todo-count-516">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-516" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
