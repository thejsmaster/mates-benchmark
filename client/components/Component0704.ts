// client/components/Component0704.ts
// Benchmark component #704 — identical logic to TodoPage, scoped class names.
...
import "./Component0704.css";

type Filter = "all" | "active" | "done";

export const Component0704 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0704] mounted");
    return () => console.log("[Component0704] unmounted");
  });
  ...
  return () => html`
    <div class="app-704">
      <header class="header-704">
        <h1>todos — Component0704</h1>
      </header>

      <main class="main-704">
        <input
          class="add-input-704 ${error() ? "add-input--error-704" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-704" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-704">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-704">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-704 ${filter() === f ? "filter-btn--active-704" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-704">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-704">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-704">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-704 ${todo.done ? "todo-item--done-704" : ""}">
                    <button
                      class="todo-check-704 ${todo.done ? "todo-check--done-704" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-704" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-704"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-704" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-704" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-704">
            <span class="todo-count-704">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-704" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
