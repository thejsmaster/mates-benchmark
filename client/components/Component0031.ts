// client/components/Component0031.ts
// Benchmark component #31 — identical logic to TodoPage, scoped class names.
...
import "./Component0031.css";

type Filter = "all" | "active" | "done";

export const Component0031 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0031] mounted");
    return () => console.log("[Component0031] unmounted");
  });
  ...
  return () => html`
    <div class="app-31">
      <header class="header-31">
        <h1>todos — Component0031</h1>
      </header>

      <main class="main-31">
        <input
          class="add-input-31 ${error() ? "add-input--error-31" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-31" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-31">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-31">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-31 ${filter() === f ? "filter-btn--active-31" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-31">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-31">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-31">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-31 ${todo.done ? "todo-item--done-31" : ""}">
                    <button
                      class="todo-check-31 ${todo.done ? "todo-check--done-31" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-31" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-31"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-31" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-31" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-31">
            <span class="todo-count-31">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-31" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
