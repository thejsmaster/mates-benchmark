// client/components/Component0253.ts
// Benchmark component #253 — identical logic to TodoPage, scoped class names.
...
import "./Component0253.css";

type Filter = "all" | "active" | "done";

export const Component0253 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0253] mounted");
    return () => console.log("[Component0253] unmounted");
  });
  ...
  return () => html`
    <div class="app-253">
      <header class="header-253">
        <h1>todos — Component0253</h1>
      </header>

      <main class="main-253">
        <input
          class="add-input-253 ${error() ? "add-input--error-253" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-253" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-253">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-253">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-253 ${filter() === f ? "filter-btn--active-253" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-253">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-253">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-253">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-253 ${todo.done ? "todo-item--done-253" : ""}">
                    <button
                      class="todo-check-253 ${todo.done ? "todo-check--done-253" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-253" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-253"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-253" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-253" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-253">
            <span class="todo-count-253">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-253" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
