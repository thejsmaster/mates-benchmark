// client/components/Component0255.ts
// Benchmark component #255 — identical logic to TodoPage, scoped class names.
...
import "./Component0255.css";

type Filter = "all" | "active" | "done";

export const Component0255 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0255] mounted");
    return () => console.log("[Component0255] unmounted");
  });
  ...
  return () => html`
    <div class="app-255">
      <header class="header-255">
        <h1>todos — Component0255</h1>
      </header>

      <main class="main-255">
        <input
          class="add-input-255 ${error() ? "add-input--error-255" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-255" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-255">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-255">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-255 ${filter() === f ? "filter-btn--active-255" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-255">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-255">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-255">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-255 ${todo.done ? "todo-item--done-255" : ""}">
                    <button
                      class="todo-check-255 ${todo.done ? "todo-check--done-255" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-255" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-255"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-255" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-255" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-255">
            <span class="todo-count-255">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-255" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
