// client/components/Component0586.ts
// Benchmark component #586 — identical logic to TodoPage, scoped class names.
...
import "./Component0586.css";

type Filter = "all" | "active" | "done";

export const Component0586 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0586] mounted");
    return () => console.log("[Component0586] unmounted");
  });
  ...
  return () => html`
    <div class="app-586">
      <header class="header-586">
        <h1>todos — Component0586</h1>
      </header>

      <main class="main-586">
        <input
          class="add-input-586 ${error() ? "add-input--error-586" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-586" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-586">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-586">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-586 ${filter() === f ? "filter-btn--active-586" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-586">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-586">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-586">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-586 ${todo.done ? "todo-item--done-586" : ""}">
                    <button
                      class="todo-check-586 ${todo.done ? "todo-check--done-586" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-586" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-586"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-586" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-586" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-586">
            <span class="todo-count-586">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-586" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
