// client/components/Component0429.ts
// Benchmark component #429 — identical logic to TodoPage, scoped class names.
...
import "./Component0429.css";

type Filter = "all" | "active" | "done";

export const Component0429 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0429] mounted");
    return () => console.log("[Component0429] unmounted");
  });
  ...
  return () => html`
    <div class="app-429">
      <header class="header-429">
        <h1>todos — Component0429</h1>
      </header>

      <main class="main-429">
        <input
          class="add-input-429 ${error() ? "add-input--error-429" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-429" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-429">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-429">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-429 ${filter() === f ? "filter-btn--active-429" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-429">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-429">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-429">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-429 ${todo.done ? "todo-item--done-429" : ""}">
                    <button
                      class="todo-check-429 ${todo.done ? "todo-check--done-429" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-429" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-429"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-429" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-429" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-429">
            <span class="todo-count-429">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-429" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
