// client/components/Component0157.ts
// Benchmark component #157 — identical logic to TodoPage, scoped class names.
...
import "./Component0157.css";

type Filter = "all" | "active" | "done";

export const Component0157 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0157] mounted");
    return () => console.log("[Component0157] unmounted");
  });
  ...
  return () => html`
    <div class="app-157">
      <header class="header-157">
        <h1>todos — Component0157</h1>
      </header>

      <main class="main-157">
        <input
          class="add-input-157 ${error() ? "add-input--error-157" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-157" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-157">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-157">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-157 ${filter() === f ? "filter-btn--active-157" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-157">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-157">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-157">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-157 ${todo.done ? "todo-item--done-157" : ""}">
                    <button
                      class="todo-check-157 ${todo.done ? "todo-check--done-157" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-157" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-157"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-157" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-157" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-157">
            <span class="todo-count-157">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-157" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
