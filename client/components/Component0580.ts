// client/components/Component0580.ts
// Benchmark component #580 — identical logic to TodoPage, scoped class names.
...
import "./Component0580.css";

type Filter = "all" | "active" | "done";

export const Component0580 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0580] mounted");
    return () => console.log("[Component0580] unmounted");
  });
  ...
  return () => html`
    <div class="app-580">
      <header class="header-580">
        <h1>todos — Component0580</h1>
      </header>

      <main class="main-580">
        <input
          class="add-input-580 ${error() ? "add-input--error-580" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-580" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-580">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-580">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-580 ${filter() === f ? "filter-btn--active-580" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-580">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-580">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-580">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-580 ${todo.done ? "todo-item--done-580" : ""}">
                    <button
                      class="todo-check-580 ${todo.done ? "todo-check--done-580" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-580" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-580"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-580" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-580" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-580">
            <span class="todo-count-580">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-580" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
