// client/components/Component0970.ts
// Benchmark component #970 — identical logic to TodoPage, scoped class names.
...
import "./Component0970.css";

type Filter = "all" | "active" | "done";

export const Component0970 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0970] mounted");
    return () => console.log("[Component0970] unmounted");
  });
  ...
  return () => html`
    <div class="app-970">
      <header class="header-970">
        <h1>todos — Component0970</h1>
      </header>

      <main class="main-970">
        <input
          class="add-input-970 ${error() ? "add-input--error-970" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-970" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-970">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-970">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-970 ${filter() === f ? "filter-btn--active-970" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-970">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-970">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-970">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-970 ${todo.done ? "todo-item--done-970" : ""}">
                    <button
                      class="todo-check-970 ${todo.done ? "todo-check--done-970" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-970" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-970"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-970" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-970" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-970">
            <span class="todo-count-970">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-970" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
