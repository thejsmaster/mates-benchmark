// client/components/Component0783.ts
// Benchmark component #783 — identical logic to TodoPage, scoped class names.
...
import "./Component0783.css";

type Filter = "all" | "active" | "done";

export const Component0783 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0783] mounted");
    return () => console.log("[Component0783] unmounted");
  });
  ...
  return () => html`
    <div class="app-783">
      <header class="header-783">
        <h1>todos — Component0783</h1>
      </header>

      <main class="main-783">
        <input
          class="add-input-783 ${error() ? "add-input--error-783" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-783" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-783">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-783">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-783 ${filter() === f ? "filter-btn--active-783" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-783">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-783">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-783">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-783 ${todo.done ? "todo-item--done-783" : ""}">
                    <button
                      class="todo-check-783 ${todo.done ? "todo-check--done-783" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-783" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-783"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-783" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-783" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-783">
            <span class="todo-count-783">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-783" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
