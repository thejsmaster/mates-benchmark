// client/components/Component0791.ts
// Benchmark component #791 — identical logic to TodoPage, scoped class names.
...
import "./Component0791.css";

type Filter = "all" | "active" | "done";

export const Component0791 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0791] mounted");
    return () => console.log("[Component0791] unmounted");
  });
  ...
  return () => html`
    <div class="app-791">
      <header class="header-791">
        <h1>todos — Component0791</h1>
      </header>

      <main class="main-791">
        <input
          class="add-input-791 ${error() ? "add-input--error-791" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-791" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-791">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-791">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-791 ${filter() === f ? "filter-btn--active-791" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-791">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-791">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-791">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-791 ${todo.done ? "todo-item--done-791" : ""}">
                    <button
                      class="todo-check-791 ${todo.done ? "todo-check--done-791" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-791" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-791"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-791" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-791" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-791">
            <span class="todo-count-791">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-791" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
