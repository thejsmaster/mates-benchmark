// client/components/Component0832.ts
// Benchmark component #832 — identical logic to TodoPage, scoped class names.
...
import "./Component0832.css";

type Filter = "all" | "active" | "done";

export const Component0832 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0832] mounted");
    return () => console.log("[Component0832] unmounted");
  });
  ...
  return () => html`
    <div class="app-832">
      <header class="header-832">
        <h1>todos — Component0832</h1>
      </header>

      <main class="main-832">
        <input
          class="add-input-832 ${error() ? "add-input--error-832" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-832" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-832">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-832">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-832 ${filter() === f ? "filter-btn--active-832" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-832">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-832">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-832">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-832 ${todo.done ? "todo-item--done-832" : ""}">
                    <button
                      class="todo-check-832 ${todo.done ? "todo-check--done-832" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-832" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-832"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-832" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-832" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-832">
            <span class="todo-count-832">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-832" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
