// client/components/Component0694.ts
// Benchmark component #694 — identical logic to TodoPage, scoped class names.
...
import "./Component0694.css";

type Filter = "all" | "active" | "done";

export const Component0694 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0694] mounted");
    return () => console.log("[Component0694] unmounted");
  });
  ...
  return () => html`
    <div class="app-694">
      <header class="header-694">
        <h1>todos — Component0694</h1>
      </header>

      <main class="main-694">
        <input
          class="add-input-694 ${error() ? "add-input--error-694" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-694" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-694">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-694">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-694 ${filter() === f ? "filter-btn--active-694" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-694">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-694">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-694">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-694 ${todo.done ? "todo-item--done-694" : ""}">
                    <button
                      class="todo-check-694 ${todo.done ? "todo-check--done-694" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-694" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-694"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-694" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-694" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-694">
            <span class="todo-count-694">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-694" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
