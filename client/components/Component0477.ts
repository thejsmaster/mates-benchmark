// client/components/Component0477.ts
// Benchmark component #477 — identical logic to TodoPage, scoped class names.
...
import "./Component0477.css";

type Filter = "all" | "active" | "done";

export const Component0477 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0477] mounted");
    return () => console.log("[Component0477] unmounted");
  });
  ...
  return () => html`
    <div class="app-477">
      <header class="header-477">
        <h1>todos — Component0477</h1>
      </header>

      <main class="main-477">
        <input
          class="add-input-477 ${error() ? "add-input--error-477" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-477" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-477">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-477">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-477 ${filter() === f ? "filter-btn--active-477" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-477">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-477">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-477">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-477 ${todo.done ? "todo-item--done-477" : ""}">
                    <button
                      class="todo-check-477 ${todo.done ? "todo-check--done-477" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-477" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-477"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-477" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-477" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-477">
            <span class="todo-count-477">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-477" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
