// client/components/Component0320.ts
// Benchmark component #320 — identical logic to TodoPage, scoped class names.
...
import "./Component0320.css";

type Filter = "all" | "active" | "done";

export const Component0320 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0320] mounted");
    return () => console.log("[Component0320] unmounted");
  });
  ...
  return () => html`
    <div class="app-320">
      <header class="header-320">
        <h1>todos — Component0320</h1>
      </header>

      <main class="main-320">
        <input
          class="add-input-320 ${error() ? "add-input--error-320" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-320" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-320">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-320">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-320 ${filter() === f ? "filter-btn--active-320" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-320">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-320">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-320">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-320 ${todo.done ? "todo-item--done-320" : ""}">
                    <button
                      class="todo-check-320 ${todo.done ? "todo-check--done-320" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-320" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-320"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-320" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-320" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-320">
            <span class="todo-count-320">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-320" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
