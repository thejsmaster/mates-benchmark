// client/components/Component0758.ts
// Benchmark component #758 — identical logic to TodoPage, scoped class names.
...
import "./Component0758.css";

type Filter = "all" | "active" | "done";

export const Component0758 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0758] mounted");
    return () => console.log("[Component0758] unmounted");
  });
  ...
  return () => html`
    <div class="app-758">
      <header class="header-758">
        <h1>todos — Component0758</h1>
      </header>

      <main class="main-758">
        <input
          class="add-input-758 ${error() ? "add-input--error-758" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-758" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-758">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-758">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-758 ${filter() === f ? "filter-btn--active-758" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-758">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-758">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-758">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-758 ${todo.done ? "todo-item--done-758" : ""}">
                    <button
                      class="todo-check-758 ${todo.done ? "todo-check--done-758" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-758" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-758"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-758" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-758" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-758">
            <span class="todo-count-758">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-758" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
