// client/components/Component0595.ts
// Benchmark component #595 — identical logic to TodoPage, scoped class names.
...
import "./Component0595.css";

type Filter = "all" | "active" | "done";

export const Component0595 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0595] mounted");
    return () => console.log("[Component0595] unmounted");
  });
  ...
  return () => html`
    <div class="app-595">
      <header class="header-595">
        <h1>todos — Component0595</h1>
      </header>

      <main class="main-595">
        <input
          class="add-input-595 ${error() ? "add-input--error-595" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-595" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-595">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-595">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-595 ${filter() === f ? "filter-btn--active-595" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-595">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-595">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-595">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-595 ${todo.done ? "todo-item--done-595" : ""}">
                    <button
                      class="todo-check-595 ${todo.done ? "todo-check--done-595" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-595" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-595"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-595" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-595" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-595">
            <span class="todo-count-595">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-595" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
