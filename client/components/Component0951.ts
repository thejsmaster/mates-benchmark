// client/components/Component0951.ts
// Benchmark component #951 — identical logic to TodoPage, scoped class names.
...
import "./Component0951.css";

type Filter = "all" | "active" | "done";

export const Component0951 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0951] mounted");
    return () => console.log("[Component0951] unmounted");
  });
  ...
  return () => html`
    <div class="app-951">
      <header class="header-951">
        <h1>todos — Component0951</h1>
      </header>

      <main class="main-951">
        <input
          class="add-input-951 ${error() ? "add-input--error-951" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-951" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-951">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-951">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-951 ${filter() === f ? "filter-btn--active-951" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-951">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-951">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-951">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-951 ${todo.done ? "todo-item--done-951" : ""}">
                    <button
                      class="todo-check-951 ${todo.done ? "todo-check--done-951" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-951" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-951"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-951" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-951" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-951">
            <span class="todo-count-951">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-951" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
