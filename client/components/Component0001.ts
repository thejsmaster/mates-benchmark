// client/components/Component0001.ts
// Benchmark component #1 — identical logic to TodoPage, scoped class names.
...
import "./Component0001.css";

type Filter = "all" | "active" | "done";

export const Component0001 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0001] mounted");
    return () => console.log("[Component0001] unmounted");
  });
  ...
  return () => html`
    <div class="app-1">
      <header class="header-1">
        <h1>todos — Component0001</h1>
      </header>

      <main class="main-1">
        <input
          class="add-input-1 ${error() ? "add-input--error-1" : ""}"
          type="text"
          placeholder="What needs to be testing something?"
          .value=${input()}
          @input=${(e: Event) => {
        ...
        />
        <button class="add-btn-1" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-1">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-1">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-1 ${filter() === f ? "filter-btn--active-1" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-1">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-1">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-1">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-1 ${todo.done ? "todo-item--done-1" : ""}">
                    <button
                      class="todo-check-1 ${todo.done ? "todo-check--done-1" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-1" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-1"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-1" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-1" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-1">
            <span class="todo-count-1">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-1" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
