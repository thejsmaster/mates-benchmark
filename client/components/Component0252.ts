// client/components/Component0252.ts
// Benchmark component #252 — identical logic to TodoPage, scoped class names.
...
import "./Component0252.css";

type Filter = "all" | "active" | "done";

export const Component0252 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0252] mounted");
    return () => console.log("[Component0252] unmounted");
  });
  ...
  return () => html`
    <div class="app-252">
      <header class="header-252">
        <h1>todos — Component0252</h1>
      </header>

      <main class="main-252">
        <input
          class="add-input-252 ${error() ? "add-input--error-252" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-252" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-252">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-252">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-252 ${filter() === f ? "filter-btn--active-252" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-252">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-252">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-252">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-252 ${todo.done ? "todo-item--done-252" : ""}">
                    <button
                      class="todo-check-252 ${todo.done ? "todo-check--done-252" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-252" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-252"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-252" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-252" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-252">
            <span class="todo-count-252">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-252" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
