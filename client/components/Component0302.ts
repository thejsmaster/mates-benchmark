// client/components/Component0302.ts
// Benchmark component #302 — identical logic to TodoPage, scoped class names.
...
import "./Component0302.css";

type Filter = "all" | "active" | "done";

export const Component0302 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0302] mounted");
    return () => console.log("[Component0302] unmounted");
  });
  ...
  return () => html`
    <div class="app-302">
      <header class="header-302">
        <h1>todos — Component0302</h1>
      </header>

      <main class="main-302">
        <input
          class="add-input-302 ${error() ? "add-input--error-302" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-302" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-302">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-302">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-302 ${filter() === f ? "filter-btn--active-302" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-302">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-302">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-302">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-302 ${todo.done ? "todo-item--done-302" : ""}">
                    <button
                      class="todo-check-302 ${todo.done ? "todo-check--done-302" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-302" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-302"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-302" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-302" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-302">
            <span class="todo-count-302">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-302" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
