// client/components/Component0852.ts
// Benchmark component #852 — identical logic to TodoPage, scoped class names.
...
import "./Component0852.css";

type Filter = "all" | "active" | "done";

export const Component0852 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0852] mounted");
    return () => console.log("[Component0852] unmounted");
  });
  ...
  return () => html`
    <div class="app-852">
      <header class="header-852">
        <h1>todos — Component0852</h1>
      </header>

      <main class="main-852">
        <input
          class="add-input-852 ${error() ? "add-input--error-852" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-852" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-852">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-852">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-852 ${filter() === f ? "filter-btn--active-852" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-852">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-852">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-852">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-852 ${todo.done ? "todo-item--done-852" : ""}">
                    <button
                      class="todo-check-852 ${todo.done ? "todo-check--done-852" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-852" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-852"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-852" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-852" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-852">
            <span class="todo-count-852">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-852" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
