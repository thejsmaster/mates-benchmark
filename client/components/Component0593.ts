// client/components/Component0593.ts
// Benchmark component #593 — identical logic to TodoPage, scoped class names.
...
import "./Component0593.css";

type Filter = "all" | "active" | "done";

export const Component0593 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0593] mounted");
    return () => console.log("[Component0593] unmounted");
  });
  ...
  return () => html`
    <div class="app-593">
      <header class="header-593">
        <h1>todos — Component0593</h1>
      </header>

      <main class="main-593">
        <input
          class="add-input-593 ${error() ? "add-input--error-593" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-593" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-593">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-593">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-593 ${filter() === f ? "filter-btn--active-593" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-593">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-593">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-593">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-593 ${todo.done ? "todo-item--done-593" : ""}">
                    <button
                      class="todo-check-593 ${todo.done ? "todo-check--done-593" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-593" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-593"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-593" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-593" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-593">
            <span class="todo-count-593">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-593" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
