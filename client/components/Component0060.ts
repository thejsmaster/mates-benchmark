// client/components/Component0060.ts
// Benchmark component #60 — identical logic to TodoPage, scoped class names.
...
import "./Component0060.css";

type Filter = "all" | "active" | "done";

export const Component0060 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0060] mounted");
    return () => console.log("[Component0060] unmounted");
  });
  ...
  return () => html`
    <div class="app-60">
      <header class="header-60">
        <h1>todos — Component0060</h1>
      </header>

      <main class="main-60">
        <input
          class="add-input-60 ${error() ? "add-input--error-60" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-60" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-60">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-60">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-60 ${filter() === f ? "filter-btn--active-60" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-60">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-60">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-60">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-60 ${todo.done ? "todo-item--done-60" : ""}">
                    <button
                      class="todo-check-60 ${todo.done ? "todo-check--done-60" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-60" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-60"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-60" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-60" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-60">
            <span class="todo-count-60">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-60" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
