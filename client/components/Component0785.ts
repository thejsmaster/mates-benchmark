// client/components/Component0785.ts
// Benchmark component #785 — identical logic to TodoPage, scoped class names.
...
import "./Component0785.css";

type Filter = "all" | "active" | "done";

export const Component0785 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0785] mounted");
    return () => console.log("[Component0785] unmounted");
  });
  ...
  return () => html`
    <div class="app-785">
      <header class="header-785">
        <h1>todos — Component0785</h1>
      </header>

      <main class="main-785">
        <input
          class="add-input-785 ${error() ? "add-input--error-785" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-785" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-785">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-785">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-785 ${filter() === f ? "filter-btn--active-785" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-785">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-785">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-785">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-785 ${todo.done ? "todo-item--done-785" : ""}">
                    <button
                      class="todo-check-785 ${todo.done ? "todo-check--done-785" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-785" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-785"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-785" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-785" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-785">
            <span class="todo-count-785">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-785" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
