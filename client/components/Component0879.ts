// client/components/Component0879.ts
// Benchmark component #879 — identical logic to TodoPage, scoped class names.
...
import "./Component0879.css";

type Filter = "all" | "active" | "done";

export const Component0879 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0879] mounted");
    return () => console.log("[Component0879] unmounted");
  });
  ...
  return () => html`
    <div class="app-879">
      <header class="header-879">
        <h1>todos — Component0879</h1>
      </header>

      <main class="main-879">
        <input
          class="add-input-879 ${error() ? "add-input--error-879" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-879" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-879">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-879">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-879 ${filter() === f ? "filter-btn--active-879" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-879">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-879">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-879">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-879 ${todo.done ? "todo-item--done-879" : ""}">
                    <button
                      class="todo-check-879 ${todo.done ? "todo-check--done-879" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-879" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-879"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-879" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-879" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-879">
            <span class="todo-count-879">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-879" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
