// client/components/Component0943.ts
// Benchmark component #943 — identical logic to TodoPage, scoped class names.
...
import "./Component0943.css";

type Filter = "all" | "active" | "done";

export const Component0943 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0943] mounted");
    return () => console.log("[Component0943] unmounted");
  });
  ...
  return () => html`
    <div class="app-943">
      <header class="header-943">
        <h1>todos — Component0943</h1>
      </header>

      <main class="main-943">
        <input
          class="add-input-943 ${error() ? "add-input--error-943" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-943" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-943">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-943">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-943 ${filter() === f ? "filter-btn--active-943" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-943">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-943">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-943">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-943 ${todo.done ? "todo-item--done-943" : ""}">
                    <button
                      class="todo-check-943 ${todo.done ? "todo-check--done-943" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-943" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-943"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-943" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-943" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-943">
            <span class="todo-count-943">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-943" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
