// client/components/Component0620.ts
// Benchmark component #620 — identical logic to TodoPage, scoped class names.
...
import "./Component0620.css";

type Filter = "all" | "active" | "done";

export const Component0620 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0620] mounted");
    return () => console.log("[Component0620] unmounted");
  });
  ...
  return () => html`
    <div class="app-620">
      <header class="header-620">
        <h1>todos — Component0620</h1>
      </header>

      <main class="main-620">
        <input
          class="add-input-620 ${error() ? "add-input--error-620" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-620" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-620">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-620">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-620 ${filter() === f ? "filter-btn--active-620" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-620">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-620">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-620">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-620 ${todo.done ? "todo-item--done-620" : ""}">
                    <button
                      class="todo-check-620 ${todo.done ? "todo-check--done-620" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-620" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-620"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-620" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-620" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-620">
            <span class="todo-count-620">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-620" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
