// client/components/Component0874.ts
// Benchmark component #874 — identical logic to TodoPage, scoped class names.
...
import "./Component0874.css";

type Filter = "all" | "active" | "done";

export const Component0874 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0874] mounted");
    return () => console.log("[Component0874] unmounted");
  });
  ...
  return () => html`
    <div class="app-874">
      <header class="header-874">
        <h1>todos — Component0874</h1>
      </header>

      <main class="main-874">
        <input
          class="add-input-874 ${error() ? "add-input--error-874" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-874" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-874">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-874">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-874 ${filter() === f ? "filter-btn--active-874" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-874">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-874">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-874">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-874 ${todo.done ? "todo-item--done-874" : ""}">
                    <button
                      class="todo-check-874 ${todo.done ? "todo-check--done-874" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-874" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-874"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-874" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-874" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-874">
            <span class="todo-count-874">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-874" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
