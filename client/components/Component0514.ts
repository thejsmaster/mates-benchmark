// client/components/Component0514.ts
// Benchmark component #514 — identical logic to TodoPage, scoped class names.
...
import "./Component0514.css";

type Filter = "all" | "active" | "done";

export const Component0514 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0514] mounted");
    return () => console.log("[Component0514] unmounted");
  });
  ...
  return () => html`
    <div class="app-514">
      <header class="header-514">
        <h1>todos — Component0514</h1>
      </header>

      <main class="main-514">
        <input
          class="add-input-514 ${error() ? "add-input--error-514" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-514" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-514">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-514">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-514 ${filter() === f ? "filter-btn--active-514" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-514">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-514">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-514">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-514 ${todo.done ? "todo-item--done-514" : ""}">
                    <button
                      class="todo-check-514 ${todo.done ? "todo-check--done-514" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-514" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-514"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-514" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-514" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-514">
            <span class="todo-count-514">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-514" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
