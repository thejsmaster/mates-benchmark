// client/components/Component0523.ts
// Benchmark component #523 — identical logic to TodoPage, scoped class names.
...
import "./Component0523.css";

type Filter = "all" | "active" | "done";

export const Component0523 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0523] mounted");
    return () => console.log("[Component0523] unmounted");
  });
  ...
  return () => html`
    <div class="app-523">
      <header class="header-523">
        <h1>todos — Component0523</h1>
      </header>

      <main class="main-523">
        <input
          class="add-input-523 ${error() ? "add-input--error-523" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-523" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-523">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-523">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-523 ${filter() === f ? "filter-btn--active-523" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-523">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-523">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-523">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-523 ${todo.done ? "todo-item--done-523" : ""}">
                    <button
                      class="todo-check-523 ${todo.done ? "todo-check--done-523" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-523" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-523"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-523" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-523" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-523">
            <span class="todo-count-523">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-523" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
