// client/components/Component0100.ts
// Benchmark component #100 — identical logic to TodoPage, scoped class names.
...
import "./Component0100.css";

type Filter = "all" | "active" | "done";

export const Component0100 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0100] mounted");
    return () => console.log("[Component0100] unmounted");
  });
  ...
  return () => html`
    <div class="app-100">
      <header class="header-100">
        <h1>todos — Component0100</h1>
      </header>

      <main class="main-100">
        <input
          class="add-input-100 ${error() ? "add-input--error-100" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-100" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-100">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-100">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-100 ${filter() === f ? "filter-btn--active-100" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-100">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-100">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-100">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-100 ${todo.done ? "todo-item--done-100" : ""}">
                    <button
                      class="todo-check-100 ${todo.done ? "todo-check--done-100" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-100" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-100"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-100" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-100" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-100">
            <span class="todo-count-100">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-100" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
