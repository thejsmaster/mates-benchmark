// client/components/Component0005.ts
// Benchmark component #5 — identical logic to TodoPage, scoped class names.
...
import "./Component0005.css";

type Filter = "all" | "active" | "done";

export const Component0005 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0005] mounted");
    return () => console.log("[Component0005] unmounted");
  });
  ...
  return () => html`
    <div class="app-5">
      <header class="header-5">
        <h1>todos — Component0005</h1>
      </header>

      <main class="main-5">
        <input
          class="add-input-5 ${error() ? "add-input--error-5" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-5" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-5">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-5">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-5 ${filter() === f ? "filter-btn--active-5" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-5">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-5">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-5">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-5 ${todo.done ? "todo-item--done-5" : ""}">
                    <button
                      class="todo-check-5 ${todo.done ? "todo-check--done-5" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-5" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-5"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-5" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-5" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-5">
            <span class="todo-count-5">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-5" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
