// client/components/Component0471.ts
// Benchmark component #471 — identical logic to TodoPage, scoped class names.
...
import "./Component0471.css";

type Filter = "all" | "active" | "done";

export const Component0471 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0471] mounted");
    return () => console.log("[Component0471] unmounted");
  });
  ...
  return () => html`
    <div class="app-471">
      <header class="header-471">
        <h1>todos — Component0471</h1>
      </header>

      <main class="main-471">
        <input
          class="add-input-471 ${error() ? "add-input--error-471" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-471" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-471">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-471">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-471 ${filter() === f ? "filter-btn--active-471" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-471">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-471">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-471">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-471 ${todo.done ? "todo-item--done-471" : ""}">
                    <button
                      class="todo-check-471 ${todo.done ? "todo-check--done-471" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-471" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-471"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-471" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-471" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-471">
            <span class="todo-count-471">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-471" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
