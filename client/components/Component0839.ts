// client/components/Component0839.ts
// Benchmark component #839 — identical logic to TodoPage, scoped class names.
...
import "./Component0839.css";

type Filter = "all" | "active" | "done";

export const Component0839 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0839] mounted");
    return () => console.log("[Component0839] unmounted");
  });
  ...
  return () => html`
    <div class="app-839">
      <header class="header-839">
        <h1>todos — Component0839</h1>
      </header>

      <main class="main-839">
        <input
          class="add-input-839 ${error() ? "add-input--error-839" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-839" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-839">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-839">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-839 ${filter() === f ? "filter-btn--active-839" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-839">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-839">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-839">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-839 ${todo.done ? "todo-item--done-839" : ""}">
                    <button
                      class="todo-check-839 ${todo.done ? "todo-check--done-839" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-839" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-839"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-839" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-839" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-839">
            <span class="todo-count-839">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-839" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
