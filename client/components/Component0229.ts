// client/components/Component0229.ts
// Benchmark component #229 — identical logic to TodoPage, scoped class names.
...
import "./Component0229.css";

type Filter = "all" | "active" | "done";

export const Component0229 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0229] mounted");
    return () => console.log("[Component0229] unmounted");
  });
  ...
  return () => html`
    <div class="app-229">
      <header class="header-229">
        <h1>todos — Component0229</h1>
      </header>

      <main class="main-229">
        <input
          class="add-input-229 ${error() ? "add-input--error-229" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-229" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-229">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-229">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-229 ${filter() === f ? "filter-btn--active-229" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-229">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-229">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-229">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-229 ${todo.done ? "todo-item--done-229" : ""}">
                    <button
                      class="todo-check-229 ${todo.done ? "todo-check--done-229" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-229" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-229"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-229" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-229" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-229">
            <span class="todo-count-229">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-229" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
