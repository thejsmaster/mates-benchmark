// client/components/Component0733.ts
// Benchmark component #733 — identical logic to TodoPage, scoped class names.
...
import "./Component0733.css";

type Filter = "all" | "active" | "done";

export const Component0733 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0733] mounted");
    return () => console.log("[Component0733] unmounted");
  });
  ...
  return () => html`
    <div class="app-733">
      <header class="header-733">
        <h1>todos — Component0733</h1>
      </header>

      <main class="main-733">
        <input
          class="add-input-733 ${error() ? "add-input--error-733" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-733" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-733">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-733">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-733 ${filter() === f ? "filter-btn--active-733" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-733">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-733">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-733">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-733 ${todo.done ? "todo-item--done-733" : ""}">
                    <button
                      class="todo-check-733 ${todo.done ? "todo-check--done-733" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-733" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-733"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-733" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-733" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-733">
            <span class="todo-count-733">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-733" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
