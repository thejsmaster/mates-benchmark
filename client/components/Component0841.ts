// client/components/Component0841.ts
// Benchmark component #841 — identical logic to TodoPage, scoped class names.
...
import "./Component0841.css";

type Filter = "all" | "active" | "done";

export const Component0841 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0841] mounted");
    return () => console.log("[Component0841] unmounted");
  });
  ...
  return () => html`
    <div class="app-841">
      <header class="header-841">
        <h1>todos — Component0841</h1>
      </header>

      <main class="main-841">
        <input
          class="add-input-841 ${error() ? "add-input--error-841" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-841" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-841">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-841">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-841 ${filter() === f ? "filter-btn--active-841" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-841">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-841">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-841">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-841 ${todo.done ? "todo-item--done-841" : ""}">
                    <button
                      class="todo-check-841 ${todo.done ? "todo-check--done-841" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-841" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-841"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-841" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-841" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-841">
            <span class="todo-count-841">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-841" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
