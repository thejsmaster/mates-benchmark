// client/components/Component0937.ts
// Benchmark component #937 — identical logic to TodoPage, scoped class names.
...
import "./Component0937.css";

type Filter = "all" | "active" | "done";

export const Component0937 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0937] mounted");
    return () => console.log("[Component0937] unmounted");
  });
  ...
  return () => html`
    <div class="app-937">
      <header class="header-937">
        <h1>todos — Component0937</h1>
      </header>

      <main class="main-937">
        <input
          class="add-input-937 ${error() ? "add-input--error-937" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-937" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-937">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-937">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-937 ${filter() === f ? "filter-btn--active-937" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-937">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-937">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-937">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-937 ${todo.done ? "todo-item--done-937" : ""}">
                    <button
                      class="todo-check-937 ${todo.done ? "todo-check--done-937" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-937" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-937"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-937" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-937" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-937">
            <span class="todo-count-937">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-937" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
