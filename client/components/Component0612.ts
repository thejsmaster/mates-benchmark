// client/components/Component0612.ts
// Benchmark component #612 — identical logic to TodoPage, scoped class names.
...
import "./Component0612.css";

type Filter = "all" | "active" | "done";

export const Component0612 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0612] mounted");
    return () => console.log("[Component0612] unmounted");
  });
  ...
  return () => html`
    <div class="app-612">
      <header class="header-612">
        <h1>todos — Component0612</h1>
      </header>

      <main class="main-612">
        <input
          class="add-input-612 ${error() ? "add-input--error-612" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-612" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-612">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-612">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-612 ${filter() === f ? "filter-btn--active-612" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-612">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-612">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-612">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-612 ${todo.done ? "todo-item--done-612" : ""}">
                    <button
                      class="todo-check-612 ${todo.done ? "todo-check--done-612" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-612" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-612"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-612" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-612" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-612">
            <span class="todo-count-612">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-612" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
