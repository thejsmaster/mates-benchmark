// client/components/Component0840.ts
// Benchmark component #840 — identical logic to TodoPage, scoped class names.
...
import "./Component0840.css";

type Filter = "all" | "active" | "done";

export const Component0840 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0840] mounted");
    return () => console.log("[Component0840] unmounted");
  });
  ...
  return () => html`
    <div class="app-840">
      <header class="header-840">
        <h1>todos — Component0840</h1>
      </header>

      <main class="main-840">
        <input
          class="add-input-840 ${error() ? "add-input--error-840" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-840" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-840">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-840">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-840 ${filter() === f ? "filter-btn--active-840" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-840">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-840">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-840">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-840 ${todo.done ? "todo-item--done-840" : ""}">
                    <button
                      class="todo-check-840 ${todo.done ? "todo-check--done-840" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-840" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-840"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-840" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-840" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-840">
            <span class="todo-count-840">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-840" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
