// client/components/Component0512.ts
// Benchmark component #512 — identical logic to TodoPage, scoped class names.
...
import "./Component0512.css";

type Filter = "all" | "active" | "done";

export const Component0512 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0512] mounted");
    return () => console.log("[Component0512] unmounted");
  });
  ...
  return () => html`
    <div class="app-512">
      <header class="header-512">
        <h1>todos — Component0512</h1>
      </header>

      <main class="main-512">
        <input
          class="add-input-512 ${error() ? "add-input--error-512" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-512" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-512">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-512">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-512 ${filter() === f ? "filter-btn--active-512" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-512">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-512">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-512">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-512 ${todo.done ? "todo-item--done-512" : ""}">
                    <button
                      class="todo-check-512 ${todo.done ? "todo-check--done-512" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-512" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-512"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-512" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-512" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-512">
            <span class="todo-count-512">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-512" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
