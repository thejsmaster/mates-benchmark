// client/components/Component0540.ts
// Benchmark component #540 — identical logic to TodoPage, scoped class names.
...
import "./Component0540.css";

type Filter = "all" | "active" | "done";

export const Component0540 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0540] mounted");
    return () => console.log("[Component0540] unmounted");
  });
  ...
  return () => html`
    <div class="app-540">
      <header class="header-540">
        <h1>todos — Component0540</h1>
      </header>

      <main class="main-540">
        <input
          class="add-input-540 ${error() ? "add-input--error-540" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-540" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-540">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-540">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-540 ${filter() === f ? "filter-btn--active-540" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-540">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-540">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-540">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-540 ${todo.done ? "todo-item--done-540" : ""}">
                    <button
                      class="todo-check-540 ${todo.done ? "todo-check--done-540" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-540" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-540"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-540" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-540" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-540">
            <span class="todo-count-540">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-540" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
