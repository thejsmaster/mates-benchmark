// client/components/Component0111.ts
// Benchmark component #111 — identical logic to TodoPage, scoped class names.
...
import "./Component0111.css";

type Filter = "all" | "active" | "done";

export const Component0111 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0111] mounted");
    return () => console.log("[Component0111] unmounted");
  });
  ...
  return () => html`
    <div class="app-111">
      <header class="header-111">
        <h1>todos — Component0111</h1>
      </header>

      <main class="main-111">
        <input
          class="add-input-111 ${error() ? "add-input--error-111" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-111" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-111">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-111">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-111 ${filter() === f ? "filter-btn--active-111" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-111">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-111">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-111">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-111 ${todo.done ? "todo-item--done-111" : ""}">
                    <button
                      class="todo-check-111 ${todo.done ? "todo-check--done-111" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-111" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-111"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-111" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-111" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-111">
            <span class="todo-count-111">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-111" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
