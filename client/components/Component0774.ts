// client/components/Component0774.ts
// Benchmark component #774 — identical logic to TodoPage, scoped class names.
...
import "./Component0774.css";

type Filter = "all" | "active" | "done";

export const Component0774 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0774] mounted");
    return () => console.log("[Component0774] unmounted");
  });
  ...
  return () => html`
    <div class="app-774">
      <header class="header-774">
        <h1>todos — Component0774</h1>
      </header>

      <main class="main-774">
        <input
          class="add-input-774 ${error() ? "add-input--error-774" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-774" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-774">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-774">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-774 ${filter() === f ? "filter-btn--active-774" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-774">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-774">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-774">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-774 ${todo.done ? "todo-item--done-774" : ""}">
                    <button
                      class="todo-check-774 ${todo.done ? "todo-check--done-774" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-774" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-774"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-774" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-774" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-774">
            <span class="todo-count-774">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-774" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
