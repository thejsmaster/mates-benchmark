// client/components/Component0162.ts
// Benchmark component #162 — identical logic to TodoPage, scoped class names.
...
import "./Component0162.css";

type Filter = "all" | "active" | "done";

export const Component0162 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0162] mounted");
    return () => console.log("[Component0162] unmounted");
  });
  ...
  return () => html`
    <div class="app-162">
      <header class="header-162">
        <h1>todos — Component0162</h1>
      </header>

      <main class="main-162">
        <input
          class="add-input-162 ${error() ? "add-input--error-162" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-162" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-162">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-162">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-162 ${filter() === f ? "filter-btn--active-162" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-162">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-162">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-162">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-162 ${todo.done ? "todo-item--done-162" : ""}">
                    <button
                      class="todo-check-162 ${todo.done ? "todo-check--done-162" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-162" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-162"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-162" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-162" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-162">
            <span class="todo-count-162">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-162" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
