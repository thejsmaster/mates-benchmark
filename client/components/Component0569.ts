// client/components/Component0569.ts
// Benchmark component #569 — identical logic to TodoPage, scoped class names.
...
import "./Component0569.css";

type Filter = "all" | "active" | "done";

export const Component0569 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0569] mounted");
    return () => console.log("[Component0569] unmounted");
  });
  ...
  return () => html`
    <div class="app-569">
      <header class="header-569">
        <h1>todos — Component0569</h1>
      </header>

      <main class="main-569">
        <input
          class="add-input-569 ${error() ? "add-input--error-569" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-569" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-569">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-569">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-569 ${filter() === f ? "filter-btn--active-569" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-569">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-569">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-569">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-569 ${todo.done ? "todo-item--done-569" : ""}">
                    <button
                      class="todo-check-569 ${todo.done ? "todo-check--done-569" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-569" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-569"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-569" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-569" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-569">
            <span class="todo-count-569">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-569" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
