// client/components/Component0641.ts
// Benchmark component #641 — identical logic to TodoPage, scoped class names.
...
import "./Component0641.css";

type Filter = "all" | "active" | "done";

export const Component0641 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0641] mounted");
    return () => console.log("[Component0641] unmounted");
  });
  ...
  return () => html`
    <div class="app-641">
      <header class="header-641">
        <h1>todos — Component0641</h1>
      </header>

      <main class="main-641">
        <input
          class="add-input-641 ${error() ? "add-input--error-641" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-641" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-641">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-641">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-641 ${filter() === f ? "filter-btn--active-641" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-641">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-641">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-641">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-641 ${todo.done ? "todo-item--done-641" : ""}">
                    <button
                      class="todo-check-641 ${todo.done ? "todo-check--done-641" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-641" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-641"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-641" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-641" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-641">
            <span class="todo-count-641">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-641" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
