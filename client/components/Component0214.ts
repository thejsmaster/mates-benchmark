// client/components/Component0214.ts
// Benchmark component #214 — identical logic to TodoPage, scoped class names.
...
import "./Component0214.css";

type Filter = "all" | "active" | "done";

export const Component0214 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0214] mounted");
    return () => console.log("[Component0214] unmounted");
  });
  ...
  return () => html`
    <div class="app-214">
      <header class="header-214">
        <h1>todos — Component0214</h1>
      </header>

      <main class="main-214">
        <input
          class="add-input-214 ${error() ? "add-input--error-214" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-214" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-214">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-214">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-214 ${filter() === f ? "filter-btn--active-214" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-214">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-214">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-214">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-214 ${todo.done ? "todo-item--done-214" : ""}">
                    <button
                      class="todo-check-214 ${todo.done ? "todo-check--done-214" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-214" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-214"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-214" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-214" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-214">
            <span class="todo-count-214">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-214" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
