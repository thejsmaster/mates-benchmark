// client/components/Component0525.ts
// Benchmark component #525 — identical logic to TodoPage, scoped class names.
...
import "./Component0525.css";

type Filter = "all" | "active" | "done";

export const Component0525 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0525] mounted");
    return () => console.log("[Component0525] unmounted");
  });
  ...
  return () => html`
    <div class="app-525">
      <header class="header-525">
        <h1>todos — Component0525</h1>
      </header>

      <main class="main-525">
        <input
          class="add-input-525 ${error() ? "add-input--error-525" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-525" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-525">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-525">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-525 ${filter() === f ? "filter-btn--active-525" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-525">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-525">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-525">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-525 ${todo.done ? "todo-item--done-525" : ""}">
                    <button
                      class="todo-check-525 ${todo.done ? "todo-check--done-525" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-525" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-525"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-525" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-525" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-525">
            <span class="todo-count-525">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-525" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
