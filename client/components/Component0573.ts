// client/components/Component0573.ts
// Benchmark component #573 — identical logic to TodoPage, scoped class names.
...
import "./Component0573.css";

type Filter = "all" | "active" | "done";

export const Component0573 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0573] mounted");
    return () => console.log("[Component0573] unmounted");
  });
  ...
  return () => html`
    <div class="app-573">
      <header class="header-573">
        <h1>todos — Component0573</h1>
      </header>

      <main class="main-573">
        <input
          class="add-input-573 ${error() ? "add-input--error-573" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-573" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-573">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-573">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-573 ${filter() === f ? "filter-btn--active-573" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-573">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-573">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-573">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-573 ${todo.done ? "todo-item--done-573" : ""}">
                    <button
                      class="todo-check-573 ${todo.done ? "todo-check--done-573" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-573" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-573"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-573" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-573" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-573">
            <span class="todo-count-573">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-573" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
