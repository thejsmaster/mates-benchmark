// client/components/Component0856.ts
// Benchmark component #856 — identical logic to TodoPage, scoped class names.
...
import "./Component0856.css";

type Filter = "all" | "active" | "done";

export const Component0856 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0856] mounted");
    return () => console.log("[Component0856] unmounted");
  });
  ...
  return () => html`
    <div class="app-856">
      <header class="header-856">
        <h1>todos — Component0856</h1>
      </header>

      <main class="main-856">
        <input
          class="add-input-856 ${error() ? "add-input--error-856" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-856" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-856">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-856">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-856 ${filter() === f ? "filter-btn--active-856" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-856">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-856">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-856">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-856 ${todo.done ? "todo-item--done-856" : ""}">
                    <button
                      class="todo-check-856 ${todo.done ? "todo-check--done-856" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-856" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-856"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-856" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-856" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-856">
            <span class="todo-count-856">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-856" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
