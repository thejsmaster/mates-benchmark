// client/components/Component0278.ts
// Benchmark component #278 — identical logic to TodoPage, scoped class names.
...
import "./Component0278.css";

type Filter = "all" | "active" | "done";

export const Component0278 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0278] mounted");
    return () => console.log("[Component0278] unmounted");
  });
  ...
  return () => html`
    <div class="app-278">
      <header class="header-278">
        <h1>todos — Component0278</h1>
      </header>

      <main class="main-278">
        <input
          class="add-input-278 ${error() ? "add-input--error-278" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-278" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-278">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-278">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-278 ${filter() === f ? "filter-btn--active-278" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-278">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-278">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-278">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-278 ${todo.done ? "todo-item--done-278" : ""}">
                    <button
                      class="todo-check-278 ${todo.done ? "todo-check--done-278" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-278" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-278"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-278" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-278" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-278">
            <span class="todo-count-278">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-278" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
