// client/components/Component0347.ts
// Benchmark component #347 — identical logic to TodoPage, scoped class names.
...
import "./Component0347.css";

type Filter = "all" | "active" | "done";

export const Component0347 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0347] mounted");
    return () => console.log("[Component0347] unmounted");
  });
  ...
  return () => html`
    <div class="app-347">
      <header class="header-347">
        <h1>todos — Component0347</h1>
      </header>

      <main class="main-347">
        <input
          class="add-input-347 ${error() ? "add-input--error-347" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-347" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-347">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-347">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-347 ${filter() === f ? "filter-btn--active-347" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-347">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-347">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-347">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-347 ${todo.done ? "todo-item--done-347" : ""}">
                    <button
                      class="todo-check-347 ${todo.done ? "todo-check--done-347" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-347" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-347"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-347" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-347" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-347">
            <span class="todo-count-347">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-347" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
