// client/components/Component0821.ts
// Benchmark component #821 — identical logic to TodoPage, scoped class names.
...
import "./Component0821.css";

type Filter = "all" | "active" | "done";

export const Component0821 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0821] mounted");
    return () => console.log("[Component0821] unmounted");
  });
  ...
  return () => html`
    <div class="app-821">
      <header class="header-821">
        <h1>todos — Component0821</h1>
      </header>

      <main class="main-821">
        <input
          class="add-input-821 ${error() ? "add-input--error-821" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-821" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-821">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-821">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-821 ${filter() === f ? "filter-btn--active-821" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-821">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-821">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-821">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-821 ${todo.done ? "todo-item--done-821" : ""}">
                    <button
                      class="todo-check-821 ${todo.done ? "todo-check--done-821" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-821" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-821"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-821" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-821" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-821">
            <span class="todo-count-821">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-821" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
