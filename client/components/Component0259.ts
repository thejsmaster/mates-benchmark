// client/components/Component0259.ts
// Benchmark component #259 — identical logic to TodoPage, scoped class names.
...
import "./Component0259.css";

type Filter = "all" | "active" | "done";

export const Component0259 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0259] mounted");
    return () => console.log("[Component0259] unmounted");
  });
  ...
  return () => html`
    <div class="app-259">
      <header class="header-259">
        <h1>todos — Component0259</h1>
      </header>

      <main class="main-259">
        <input
          class="add-input-259 ${error() ? "add-input--error-259" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-259" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-259">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-259">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-259 ${filter() === f ? "filter-btn--active-259" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-259">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-259">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-259">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-259 ${todo.done ? "todo-item--done-259" : ""}">
                    <button
                      class="todo-check-259 ${todo.done ? "todo-check--done-259" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-259" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-259"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-259" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-259" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-259">
            <span class="todo-count-259">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-259" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
