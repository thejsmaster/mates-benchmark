// client/components/Component0061.ts
// Benchmark component #61 — identical logic to TodoPage, scoped class names.
...
import "./Component0061.css";

type Filter = "all" | "active" | "done";

export const Component0061 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0061] mounted");
    return () => console.log("[Component0061] unmounted");
  });
  ...
  return () => html`
    <div class="app-61">
      <header class="header-61">
        <h1>todos — Component0061</h1>
      </header>

      <main class="main-61">
        <input
          class="add-input-61 ${error() ? "add-input--error-61" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-61" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-61">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-61">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-61 ${filter() === f ? "filter-btn--active-61" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-61">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-61">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-61">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-61 ${todo.done ? "todo-item--done-61" : ""}">
                    <button
                      class="todo-check-61 ${todo.done ? "todo-check--done-61" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-61" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-61"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-61" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-61" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-61">
            <span class="todo-count-61">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-61" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
