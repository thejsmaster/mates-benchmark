// client/components/Component0968.ts
// Benchmark component #968 — identical logic to TodoPage, scoped class names.
...
import "./Component0968.css";

type Filter = "all" | "active" | "done";

export const Component0968 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0968] mounted");
    return () => console.log("[Component0968] unmounted");
  });
  ...
  return () => html`
    <div class="app-968">
      <header class="header-968">
        <h1>todos — Component0968</h1>
      </header>

      <main class="main-968">
        <input
          class="add-input-968 ${error() ? "add-input--error-968" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-968" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-968">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-968">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-968 ${filter() === f ? "filter-btn--active-968" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-968">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-968">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-968">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-968 ${todo.done ? "todo-item--done-968" : ""}">
                    <button
                      class="todo-check-968 ${todo.done ? "todo-check--done-968" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-968" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-968"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-968" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-968" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-968">
            <span class="todo-count-968">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-968" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
