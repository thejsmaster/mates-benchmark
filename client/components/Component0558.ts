// client/components/Component0558.ts
// Benchmark component #558 — identical logic to TodoPage, scoped class names.
...
import "./Component0558.css";

type Filter = "all" | "active" | "done";

export const Component0558 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0558] mounted");
    return () => console.log("[Component0558] unmounted");
  });
  ...
  return () => html`
    <div class="app-558">
      <header class="header-558">
        <h1>todos — Component0558</h1>
      </header>

      <main class="main-558">
        <input
          class="add-input-558 ${error() ? "add-input--error-558" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-558" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-558">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-558">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-558 ${filter() === f ? "filter-btn--active-558" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-558">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-558">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-558">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-558 ${todo.done ? "todo-item--done-558" : ""}">
                    <button
                      class="todo-check-558 ${todo.done ? "todo-check--done-558" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-558" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-558"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-558" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-558" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-558">
            <span class="todo-count-558">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-558" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
