// client/components/Component0436.ts
// Benchmark component #436 — identical logic to TodoPage, scoped class names.
...
import "./Component0436.css";

type Filter = "all" | "active" | "done";

export const Component0436 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0436] mounted");
    return () => console.log("[Component0436] unmounted");
  });
  ...
  return () => html`
    <div class="app-436">
      <header class="header-436">
        <h1>todos — Component0436</h1>
      </header>

      <main class="main-436">
        <input
          class="add-input-436 ${error() ? "add-input--error-436" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-436" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-436">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-436">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-436 ${filter() === f ? "filter-btn--active-436" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-436">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-436">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-436">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-436 ${todo.done ? "todo-item--done-436" : ""}">
                    <button
                      class="todo-check-436 ${todo.done ? "todo-check--done-436" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-436" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-436"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-436" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-436" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-436">
            <span class="todo-count-436">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-436" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
