// client/components/Component0203.ts
// Benchmark component #203 — identical logic to TodoPage, scoped class names.
...
import "./Component0203.css";

type Filter = "all" | "active" | "done";

export const Component0203 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0203] mounted");
    return () => console.log("[Component0203] unmounted");
  });
  ...
  return () => html`
    <div class="app-203">
      <header class="header-203">
        <h1>todos — Component0203</h1>
      </header>

      <main class="main-203">
        <input
          class="add-input-203 ${error() ? "add-input--error-203" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-203" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-203">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-203">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-203 ${filter() === f ? "filter-btn--active-203" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-203">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-203">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-203">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-203 ${todo.done ? "todo-item--done-203" : ""}">
                    <button
                      class="todo-check-203 ${todo.done ? "todo-check--done-203" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-203" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-203"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-203" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-203" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-203">
            <span class="todo-count-203">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-203" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
