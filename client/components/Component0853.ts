// client/components/Component0853.ts
// Benchmark component #853 — identical logic to TodoPage, scoped class names.
...
import "./Component0853.css";

type Filter = "all" | "active" | "done";

export const Component0853 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0853] mounted");
    return () => console.log("[Component0853] unmounted");
  });
  ...
  return () => html`
    <div class="app-853">
      <header class="header-853">
        <h1>todos — Component0853</h1>
      </header>

      <main class="main-853">
        <input
          class="add-input-853 ${error() ? "add-input--error-853" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-853" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-853">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-853">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-853 ${filter() === f ? "filter-btn--active-853" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-853">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-853">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-853">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-853 ${todo.done ? "todo-item--done-853" : ""}">
                    <button
                      class="todo-check-853 ${todo.done ? "todo-check--done-853" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-853" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-853"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-853" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-853" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-853">
            <span class="todo-count-853">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-853" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
