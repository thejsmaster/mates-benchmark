// client/components/Component0328.ts
// Benchmark component #328 — identical logic to TodoPage, scoped class names.
...
import "./Component0328.css";

type Filter = "all" | "active" | "done";

export const Component0328 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0328] mounted");
    return () => console.log("[Component0328] unmounted");
  });
  ...
  return () => html`
    <div class="app-328">
      <header class="header-328">
        <h1>todos — Component0328</h1>
      </header>

      <main class="main-328">
        <input
          class="add-input-328 ${error() ? "add-input--error-328" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-328" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-328">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-328">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-328 ${filter() === f ? "filter-btn--active-328" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-328">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-328">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-328">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-328 ${todo.done ? "todo-item--done-328" : ""}">
                    <button
                      class="todo-check-328 ${todo.done ? "todo-check--done-328" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-328" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-328"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-328" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-328" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-328">
            <span class="todo-count-328">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-328" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
