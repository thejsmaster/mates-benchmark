// client/components/Component0798.ts
// Benchmark component #798 — identical logic to TodoPage, scoped class names.
...
import "./Component0798.css";

type Filter = "all" | "active" | "done";

export const Component0798 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0798] mounted");
    return () => console.log("[Component0798] unmounted");
  });
  ...
  return () => html`
    <div class="app-798">
      <header class="header-798">
        <h1>todos — Component0798</h1>
      </header>

      <main class="main-798">
        <input
          class="add-input-798 ${error() ? "add-input--error-798" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-798" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-798">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-798">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-798 ${filter() === f ? "filter-btn--active-798" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-798">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-798">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-798">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-798 ${todo.done ? "todo-item--done-798" : ""}">
                    <button
                      class="todo-check-798 ${todo.done ? "todo-check--done-798" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-798" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-798"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-798" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-798" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-798">
            <span class="todo-count-798">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-798" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
