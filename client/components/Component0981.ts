// client/components/Component0981.ts
// Benchmark component #981 — identical logic to TodoPage, scoped class names.
...
import "./Component0981.css";

type Filter = "all" | "active" | "done";

export const Component0981 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0981] mounted");
    return () => console.log("[Component0981] unmounted");
  });
  ...
  return () => html`
    <div class="app-981">
      <header class="header-981">
        <h1>todos — Component0981</h1>
      </header>

      <main class="main-981">
        <input
          class="add-input-981 ${error() ? "add-input--error-981" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-981" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-981">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-981">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-981 ${filter() === f ? "filter-btn--active-981" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-981">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-981">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-981">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-981 ${todo.done ? "todo-item--done-981" : ""}">
                    <button
                      class="todo-check-981 ${todo.done ? "todo-check--done-981" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-981" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-981"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-981" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-981" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-981">
            <span class="todo-count-981">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-981" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
