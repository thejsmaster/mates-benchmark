// client/components/Component0498.ts
// Benchmark component #498 — identical logic to TodoPage, scoped class names.
...
import "./Component0498.css";

type Filter = "all" | "active" | "done";

export const Component0498 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0498] mounted");
    return () => console.log("[Component0498] unmounted");
  });
  ...
  return () => html`
    <div class="app-498">
      <header class="header-498">
        <h1>todos — Component0498</h1>
      </header>

      <main class="main-498">
        <input
          class="add-input-498 ${error() ? "add-input--error-498" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-498" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-498">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-498">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-498 ${filter() === f ? "filter-btn--active-498" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-498">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-498">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-498">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-498 ${todo.done ? "todo-item--done-498" : ""}">
                    <button
                      class="todo-check-498 ${todo.done ? "todo-check--done-498" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-498" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-498"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-498" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-498" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-498">
            <span class="todo-count-498">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-498" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
