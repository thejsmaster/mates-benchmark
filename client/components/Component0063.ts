// client/components/Component0063.ts
// Benchmark component #63 — identical logic to TodoPage, scoped class names.
...
import "./Component0063.css";

type Filter = "all" | "active" | "done";

export const Component0063 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0063] mounted");
    return () => console.log("[Component0063] unmounted");
  });
  ...
  return () => html`
    <div class="app-63">
      <header class="header-63">
        <h1>todos — Component0063</h1>
      </header>

      <main class="main-63">
        <input
          class="add-input-63 ${error() ? "add-input--error-63" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-63" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-63">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-63">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-63 ${filter() === f ? "filter-btn--active-63" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-63">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-63">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-63">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-63 ${todo.done ? "todo-item--done-63" : ""}">
                    <button
                      class="todo-check-63 ${todo.done ? "todo-check--done-63" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-63" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-63"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-63" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-63" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-63">
            <span class="todo-count-63">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-63" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
