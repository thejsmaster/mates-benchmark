// client/components/Component0143.ts
// Benchmark component #143 — identical logic to TodoPage, scoped class names.
...
import "./Component0143.css";

type Filter = "all" | "active" | "done";

export const Component0143 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0143] mounted");
    return () => console.log("[Component0143] unmounted");
  });
  ...
  return () => html`
    <div class="app-143">
      <header class="header-143">
        <h1>todos — Component0143</h1>
      </header>

      <main class="main-143">
        <input
          class="add-input-143 ${error() ? "add-input--error-143" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-143" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-143">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-143">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-143 ${filter() === f ? "filter-btn--active-143" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-143">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-143">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-143">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-143 ${todo.done ? "todo-item--done-143" : ""}">
                    <button
                      class="todo-check-143 ${todo.done ? "todo-check--done-143" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-143" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-143"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-143" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-143" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-143">
            <span class="todo-count-143">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-143" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
