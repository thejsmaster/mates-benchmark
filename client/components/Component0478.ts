// client/components/Component0478.ts
// Benchmark component #478 — identical logic to TodoPage, scoped class names.
...
import "./Component0478.css";

type Filter = "all" | "active" | "done";

export const Component0478 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0478] mounted");
    return () => console.log("[Component0478] unmounted");
  });
  ...
  return () => html`
    <div class="app-478">
      <header class="header-478">
        <h1>todos — Component0478</h1>
      </header>

      <main class="main-478">
        <input
          class="add-input-478 ${error() ? "add-input--error-478" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-478" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-478">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-478">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-478 ${filter() === f ? "filter-btn--active-478" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-478">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-478">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-478">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-478 ${todo.done ? "todo-item--done-478" : ""}">
                    <button
                      class="todo-check-478 ${todo.done ? "todo-check--done-478" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-478" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-478"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-478" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-478" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-478">
            <span class="todo-count-478">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-478" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
