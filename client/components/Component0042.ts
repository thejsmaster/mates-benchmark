// client/components/Component0042.ts
// Benchmark component #42 — identical logic to TodoPage, scoped class names.
...
import "./Component0042.css";

type Filter = "all" | "active" | "done";

export const Component0042 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0042] mounted");
    return () => console.log("[Component0042] unmounted");
  });
  ...
  return () => html`
    <div class="app-42">
      <header class="header-42">
        <h1>todos — Component0042</h1>
      </header>

      <main class="main-42">
        <input
          class="add-input-42 ${error() ? "add-input--error-42" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-42" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-42">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-42">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-42 ${filter() === f ? "filter-btn--active-42" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-42">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-42">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-42">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-42 ${todo.done ? "todo-item--done-42" : ""}">
                    <button
                      class="todo-check-42 ${todo.done ? "todo-check--done-42" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-42" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-42"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-42" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-42" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-42">
            <span class="todo-count-42">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-42" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
