// client/components/Component0036.ts
// Benchmark component #36 — identical logic to TodoPage, scoped class names.
...
import "./Component0036.css";

type Filter = "all" | "active" | "done";

export const Component0036 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0036] mounted");
    return () => console.log("[Component0036] unmounted");
  });
  ...
  return () => html`
    <div class="app-36">
      <header class="header-36">
        <h1>todos — Component0036</h1>
      </header>

      <main class="main-36">
        <input
          class="add-input-36 ${error() ? "add-input--error-36" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-36" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-36">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-36">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-36 ${filter() === f ? "filter-btn--active-36" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-36">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-36">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-36">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-36 ${todo.done ? "todo-item--done-36" : ""}">
                    <button
                      class="todo-check-36 ${todo.done ? "todo-check--done-36" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-36" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-36"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-36" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-36" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-36">
            <span class="todo-count-36">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-36" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
