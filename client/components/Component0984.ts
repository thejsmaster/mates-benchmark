// client/components/Component0984.ts
// Benchmark component #984 — identical logic to TodoPage, scoped class names.
...
import "./Component0984.css";

type Filter = "all" | "active" | "done";

export const Component0984 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0984] mounted");
    return () => console.log("[Component0984] unmounted");
  });
  ...
  return () => html`
    <div class="app-984">
      <header class="header-984">
        <h1>todos — Component0984</h1>
      </header>

      <main class="main-984">
        <input
          class="add-input-984 ${error() ? "add-input--error-984" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-984" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-984">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-984">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-984 ${filter() === f ? "filter-btn--active-984" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-984">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-984">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-984">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-984 ${todo.done ? "todo-item--done-984" : ""}">
                    <button
                      class="todo-check-984 ${todo.done ? "todo-check--done-984" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-984" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-984"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-984" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-984" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-984">
            <span class="todo-count-984">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-984" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
