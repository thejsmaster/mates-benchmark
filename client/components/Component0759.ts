// client/components/Component0759.ts
// Benchmark component #759 — identical logic to TodoPage, scoped class names.
...
import "./Component0759.css";

type Filter = "all" | "active" | "done";

export const Component0759 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0759] mounted");
    return () => console.log("[Component0759] unmounted");
  });
  ...
  return () => html`
    <div class="app-759">
      <header class="header-759">
        <h1>todos — Component0759</h1>
      </header>

      <main class="main-759">
        <input
          class="add-input-759 ${error() ? "add-input--error-759" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-759" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-759">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-759">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-759 ${filter() === f ? "filter-btn--active-759" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-759">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-759">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-759">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-759 ${todo.done ? "todo-item--done-759" : ""}">
                    <button
                      class="todo-check-759 ${todo.done ? "todo-check--done-759" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-759" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-759"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-759" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-759" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-759">
            <span class="todo-count-759">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-759" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
