// client/components/Component0219.ts
// Benchmark component #219 — identical logic to TodoPage, scoped class names.
...
import "./Component0219.css";

type Filter = "all" | "active" | "done";

export const Component0219 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0219] mounted");
    return () => console.log("[Component0219] unmounted");
  });
  ...
  return () => html`
    <div class="app-219">
      <header class="header-219">
        <h1>todos — Component0219</h1>
      </header>

      <main class="main-219">
        <input
          class="add-input-219 ${error() ? "add-input--error-219" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-219" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-219">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-219">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-219 ${filter() === f ? "filter-btn--active-219" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-219">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-219">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-219">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-219 ${todo.done ? "todo-item--done-219" : ""}">
                    <button
                      class="todo-check-219 ${todo.done ? "todo-check--done-219" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-219" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-219"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-219" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-219" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-219">
            <span class="todo-count-219">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-219" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
