// client/components/Component0119.ts
// Benchmark component #119 — identical logic to TodoPage, scoped class names.
...
import "./Component0119.css";

type Filter = "all" | "active" | "done";

export const Component0119 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0119] mounted");
    return () => console.log("[Component0119] unmounted");
  });
  ...
  return () => html`
    <div class="app-119">
      <header class="header-119">
        <h1>todos — Component0119</h1>
      </header>

      <main class="main-119">
        <input
          class="add-input-119 ${error() ? "add-input--error-119" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-119" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-119">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-119">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-119 ${filter() === f ? "filter-btn--active-119" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-119">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-119">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-119">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-119 ${todo.done ? "todo-item--done-119" : ""}">
                    <button
                      class="todo-check-119 ${todo.done ? "todo-check--done-119" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-119" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-119"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-119" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-119" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-119">
            <span class="todo-count-119">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-119" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
