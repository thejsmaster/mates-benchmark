// client/components/Component0513.ts
// Benchmark component #513 — identical logic to TodoPage, scoped class names.
...
import "./Component0513.css";

type Filter = "all" | "active" | "done";

export const Component0513 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0513] mounted");
    return () => console.log("[Component0513] unmounted");
  });
  ...
  return () => html`
    <div class="app-513">
      <header class="header-513">
        <h1>todos — Component0513</h1>
      </header>

      <main class="main-513">
        <input
          class="add-input-513 ${error() ? "add-input--error-513" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-513" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-513">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-513">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-513 ${filter() === f ? "filter-btn--active-513" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-513">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-513">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-513">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-513 ${todo.done ? "todo-item--done-513" : ""}">
                    <button
                      class="todo-check-513 ${todo.done ? "todo-check--done-513" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-513" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-513"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-513" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-513" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-513">
            <span class="todo-count-513">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-513" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
