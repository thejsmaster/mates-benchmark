// client/components/Component0210.ts
// Benchmark component #210 — identical logic to TodoPage, scoped class names.
...
import "./Component0210.css";

type Filter = "all" | "active" | "done";

export const Component0210 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0210] mounted");
    return () => console.log("[Component0210] unmounted");
  });
  ...
  return () => html`
    <div class="app-210">
      <header class="header-210">
        <h1>todos — Component0210</h1>
      </header>

      <main class="main-210">
        <input
          class="add-input-210 ${error() ? "add-input--error-210" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-210" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-210">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-210">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-210 ${filter() === f ? "filter-btn--active-210" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-210">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-210">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-210">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-210 ${todo.done ? "todo-item--done-210" : ""}">
                    <button
                      class="todo-check-210 ${todo.done ? "todo-check--done-210" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-210" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-210"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-210" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-210" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-210">
            <span class="todo-count-210">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-210" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
