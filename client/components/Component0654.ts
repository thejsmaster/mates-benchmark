// client/components/Component0654.ts
// Benchmark component #654 — identical logic to TodoPage, scoped class names.
...
import "./Component0654.css";

type Filter = "all" | "active" | "done";

export const Component0654 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0654] mounted");
    return () => console.log("[Component0654] unmounted");
  });
  ...
  return () => html`
    <div class="app-654">
      <header class="header-654">
        <h1>todos — Component0654</h1>
      </header>

      <main class="main-654">
        <input
          class="add-input-654 ${error() ? "add-input--error-654" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-654" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-654">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-654">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-654 ${filter() === f ? "filter-btn--active-654" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-654">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-654">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-654">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-654 ${todo.done ? "todo-item--done-654" : ""}">
                    <button
                      class="todo-check-654 ${todo.done ? "todo-check--done-654" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-654" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-654"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-654" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-654" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-654">
            <span class="todo-count-654">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-654" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
