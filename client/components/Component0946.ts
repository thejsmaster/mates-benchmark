// client/components/Component0946.ts
// Benchmark component #946 — identical logic to TodoPage, scoped class names.
...
import "./Component0946.css";

type Filter = "all" | "active" | "done";

export const Component0946 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0946] mounted");
    return () => console.log("[Component0946] unmounted");
  });
  ...
  return () => html`
    <div class="app-946">
      <header class="header-946">
        <h1>todos — Component0946</h1>
      </header>

      <main class="main-946">
        <input
          class="add-input-946 ${error() ? "add-input--error-946" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-946" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-946">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-946">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-946 ${filter() === f ? "filter-btn--active-946" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-946">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-946">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-946">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-946 ${todo.done ? "todo-item--done-946" : ""}">
                    <button
                      class="todo-check-946 ${todo.done ? "todo-check--done-946" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-946" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-946"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-946" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-946" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-946">
            <span class="todo-count-946">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-946" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
