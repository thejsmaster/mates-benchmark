// client/components/Component0017.ts
// Benchmark component #17 — identical logic to TodoPage, scoped class names.
...
import "./Component0017.css";

type Filter = "all" | "active" | "done";

export const Component0017 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0017] mounted");
    return () => console.log("[Component0017] unmounted");
  });
  ...
  return () => html`
    <div class="app-17">
      <header class="header-17">
        <h1>todos — Component0017</h1>
      </header>

      <main class="main-17">
        <input
          class="add-input-17 ${error() ? "add-input--error-17" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-17" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-17">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-17">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-17 ${filter() === f ? "filter-btn--active-17" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-17">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-17">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-17">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-17 ${todo.done ? "todo-item--done-17" : ""}">
                    <button
                      class="todo-check-17 ${todo.done ? "todo-check--done-17" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-17" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-17"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-17" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-17" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-17">
            <span class="todo-count-17">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-17" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
