// client/components/Component0401.ts
// Benchmark component #401 — identical logic to TodoPage, scoped class names.
...
import "./Component0401.css";

type Filter = "all" | "active" | "done";

export const Component0401 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0401] mounted");
    return () => console.log("[Component0401] unmounted");
  });
  ...
  return () => html`
    <div class="app-401">
      <header class="header-401">
        <h1>todos — Component0401</h1>
      </header>

      <main class="main-401">
        <input
          class="add-input-401 ${error() ? "add-input--error-401" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-401" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-401">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-401">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-401 ${filter() === f ? "filter-btn--active-401" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-401">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-401">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-401">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-401 ${todo.done ? "todo-item--done-401" : ""}">
                    <button
                      class="todo-check-401 ${todo.done ? "todo-check--done-401" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-401" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-401"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-401" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-401" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-401">
            <span class="todo-count-401">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-401" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
