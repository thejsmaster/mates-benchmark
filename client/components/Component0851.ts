// client/components/Component0851.ts
// Benchmark component #851 — identical logic to TodoPage, scoped class names.
...
import "./Component0851.css";

type Filter = "all" | "active" | "done";

export const Component0851 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0851] mounted");
    return () => console.log("[Component0851] unmounted");
  });
  ...
  return () => html`
    <div class="app-851">
      <header class="header-851">
        <h1>todos — Component0851</h1>
      </header>

      <main class="main-851">
        <input
          class="add-input-851 ${error() ? "add-input--error-851" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-851" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-851">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-851">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-851 ${filter() === f ? "filter-btn--active-851" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-851">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-851">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-851">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-851 ${todo.done ? "todo-item--done-851" : ""}">
                    <button
                      class="todo-check-851 ${todo.done ? "todo-check--done-851" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-851" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-851"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-851" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-851" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-851">
            <span class="todo-count-851">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-851" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
