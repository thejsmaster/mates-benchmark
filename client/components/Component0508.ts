// client/components/Component0508.ts
// Benchmark component #508 — identical logic to TodoPage, scoped class names.
...
import "./Component0508.css";

type Filter = "all" | "active" | "done";

export const Component0508 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0508] mounted");
    return () => console.log("[Component0508] unmounted");
  });
  ...
  return () => html`
    <div class="app-508">
      <header class="header-508">
        <h1>todos — Component0508</h1>
      </header>

      <main class="main-508">
        <input
          class="add-input-508 ${error() ? "add-input--error-508" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-508" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-508">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-508">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-508 ${filter() === f ? "filter-btn--active-508" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-508">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-508">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-508">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-508 ${todo.done ? "todo-item--done-508" : ""}">
                    <button
                      class="todo-check-508 ${todo.done ? "todo-check--done-508" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-508" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-508"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-508" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-508" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-508">
            <span class="todo-count-508">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-508" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
