// client/components/Component0304.ts
// Benchmark component #304 — identical logic to TodoPage, scoped class names.
...
import "./Component0304.css";

type Filter = "all" | "active" | "done";

export const Component0304 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0304] mounted");
    return () => console.log("[Component0304] unmounted");
  });
  ...
  return () => html`
    <div class="app-304">
      <header class="header-304">
        <h1>todos — Component0304</h1>
      </header>

      <main class="main-304">
        <input
          class="add-input-304 ${error() ? "add-input--error-304" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-304" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-304">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-304">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-304 ${filter() === f ? "filter-btn--active-304" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-304">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-304">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-304">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-304 ${todo.done ? "todo-item--done-304" : ""}">
                    <button
                      class="todo-check-304 ${todo.done ? "todo-check--done-304" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-304" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-304"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-304" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-304" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-304">
            <span class="todo-count-304">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-304" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
