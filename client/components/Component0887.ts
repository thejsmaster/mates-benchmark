// client/components/Component0887.ts
// Benchmark component #887 — identical logic to TodoPage, scoped class names.
...
import "./Component0887.css";

type Filter = "all" | "active" | "done";

export const Component0887 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0887] mounted");
    return () => console.log("[Component0887] unmounted");
  });
  ...
  return () => html`
    <div class="app-887">
      <header class="header-887">
        <h1>todos — Component0887</h1>
      </header>

      <main class="main-887">
        <input
          class="add-input-887 ${error() ? "add-input--error-887" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-887" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-887">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-887">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-887 ${filter() === f ? "filter-btn--active-887" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-887">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-887">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-887">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-887 ${todo.done ? "todo-item--done-887" : ""}">
                    <button
                      class="todo-check-887 ${todo.done ? "todo-check--done-887" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-887" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-887"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-887" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-887" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-887">
            <span class="todo-count-887">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-887" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
