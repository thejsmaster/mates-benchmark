// client/components/Component0355.ts
// Benchmark component #355 — identical logic to TodoPage, scoped class names.
...
import "./Component0355.css";

type Filter = "all" | "active" | "done";

export const Component0355 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0355] mounted");
    return () => console.log("[Component0355] unmounted");
  });
  ...
  return () => html`
    <div class="app-355">
      <header class="header-355">
        <h1>todos — Component0355</h1>
      </header>

      <main class="main-355">
        <input
          class="add-input-355 ${error() ? "add-input--error-355" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-355" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-355">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-355">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-355 ${filter() === f ? "filter-btn--active-355" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-355">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-355">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-355">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-355 ${todo.done ? "todo-item--done-355" : ""}">
                    <button
                      class="todo-check-355 ${todo.done ? "todo-check--done-355" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-355" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-355"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-355" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-355" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-355">
            <span class="todo-count-355">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-355" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
