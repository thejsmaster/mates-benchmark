// client/components/Component0766.ts
// Benchmark component #766 — identical logic to TodoPage, scoped class names.
...
import "./Component0766.css";

type Filter = "all" | "active" | "done";

export const Component0766 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0766] mounted");
    return () => console.log("[Component0766] unmounted");
  });
  ...
  return () => html`
    <div class="app-766">
      <header class="header-766">
        <h1>todos — Component0766</h1>
      </header>

      <main class="main-766">
        <input
          class="add-input-766 ${error() ? "add-input--error-766" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-766" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-766">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-766">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-766 ${filter() === f ? "filter-btn--active-766" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-766">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-766">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-766">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-766 ${todo.done ? "todo-item--done-766" : ""}">
                    <button
                      class="todo-check-766 ${todo.done ? "todo-check--done-766" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-766" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-766"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-766" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-766" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-766">
            <span class="todo-count-766">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-766" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
