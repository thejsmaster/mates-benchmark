// client/components/Component0452.ts
// Benchmark component #452 — identical logic to TodoPage, scoped class names.
...
import "./Component0452.css";

type Filter = "all" | "active" | "done";

export const Component0452 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0452] mounted");
    return () => console.log("[Component0452] unmounted");
  });
  ...
  return () => html`
    <div class="app-452">
      <header class="header-452">
        <h1>todos — Component0452</h1>
      </header>

      <main class="main-452">
        <input
          class="add-input-452 ${error() ? "add-input--error-452" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-452" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-452">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-452">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-452 ${filter() === f ? "filter-btn--active-452" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-452">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-452">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-452">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-452 ${todo.done ? "todo-item--done-452" : ""}">
                    <button
                      class="todo-check-452 ${todo.done ? "todo-check--done-452" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-452" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-452"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-452" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-452" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-452">
            <span class="todo-count-452">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-452" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
