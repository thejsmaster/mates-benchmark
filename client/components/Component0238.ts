// client/components/Component0238.ts
// Benchmark component #238 — identical logic to TodoPage, scoped class names.
...
import "./Component0238.css";

type Filter = "all" | "active" | "done";

export const Component0238 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0238] mounted");
    return () => console.log("[Component0238] unmounted");
  });
  ...
  return () => html`
    <div class="app-238">
      <header class="header-238">
        <h1>todos — Component0238</h1>
      </header>

      <main class="main-238">
        <input
          class="add-input-238 ${error() ? "add-input--error-238" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-238" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-238">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-238">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-238 ${filter() === f ? "filter-btn--active-238" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-238">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-238">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-238">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-238 ${todo.done ? "todo-item--done-238" : ""}">
                    <button
                      class="todo-check-238 ${todo.done ? "todo-check--done-238" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-238" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-238"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-238" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-238" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-238">
            <span class="todo-count-238">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-238" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
