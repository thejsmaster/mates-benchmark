// client/components/Component0057.ts
// Benchmark component #57 — identical logic to TodoPage, scoped class names.
...
import "./Component0057.css";

type Filter = "all" | "active" | "done";

export const Component0057 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0057] mounted");
    return () => console.log("[Component0057] unmounted");
  });
  ...
  return () => html`
    <div class="app-57">
      <header class="header-57">
        <h1>todos — Component0057</h1>
      </header>

      <main class="main-57">
        <input
          class="add-input-57 ${error() ? "add-input--error-57" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-57" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-57">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-57">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-57 ${filter() === f ? "filter-btn--active-57" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-57">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-57">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-57">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-57 ${todo.done ? "todo-item--done-57" : ""}">
                    <button
                      class="todo-check-57 ${todo.done ? "todo-check--done-57" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-57" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-57"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-57" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-57" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-57">
            <span class="todo-count-57">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-57" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
