// client/components/Component0488.ts
// Benchmark component #488 — identical logic to TodoPage, scoped class names.
...
import "./Component0488.css";

type Filter = "all" | "active" | "done";

export const Component0488 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0488] mounted");
    return () => console.log("[Component0488] unmounted");
  });
  ...
  return () => html`
    <div class="app-488">
      <header class="header-488">
        <h1>todos — Component0488</h1>
      </header>

      <main class="main-488">
        <input
          class="add-input-488 ${error() ? "add-input--error-488" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-488" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-488">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-488">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-488 ${filter() === f ? "filter-btn--active-488" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-488">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-488">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-488">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-488 ${todo.done ? "todo-item--done-488" : ""}">
                    <button
                      class="todo-check-488 ${todo.done ? "todo-check--done-488" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-488" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-488"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-488" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-488" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-488">
            <span class="todo-count-488">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-488" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
