// client/components/Component0021.ts
// Benchmark component #21 — identical logic to TodoPage, scoped class names.
...
import "./Component0021.css";

type Filter = "all" | "active" | "done";

export const Component0021 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0021] mounted");
    return () => console.log("[Component0021] unmounted");
  });
  ...
  return () => html`
    <div class="app-21">
      <header class="header-21">
        <h1>todos — Component0021</h1>
      </header>

      <main class="main-21">
        <input
          class="add-input-21 ${error() ? "add-input--error-21" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-21" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-21">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-21">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-21 ${filter() === f ? "filter-btn--active-21" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-21">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-21">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-21">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-21 ${todo.done ? "todo-item--done-21" : ""}">
                    <button
                      class="todo-check-21 ${todo.done ? "todo-check--done-21" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-21" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-21"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-21" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-21" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-21">
            <span class="todo-count-21">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-21" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
