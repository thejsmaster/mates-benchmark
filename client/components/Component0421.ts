// client/components/Component0421.ts
// Benchmark component #421 — identical logic to TodoPage, scoped class names.
...
import "./Component0421.css";

type Filter = "all" | "active" | "done";

export const Component0421 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0421] mounted");
    return () => console.log("[Component0421] unmounted");
  });
  ...
  return () => html`
    <div class="app-421">
      <header class="header-421">
        <h1>todos — Component0421</h1>
      </header>

      <main class="main-421">
        <input
          class="add-input-421 ${error() ? "add-input--error-421" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-421" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-421">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-421">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-421 ${filter() === f ? "filter-btn--active-421" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-421">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-421">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-421">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-421 ${todo.done ? "todo-item--done-421" : ""}">
                    <button
                      class="todo-check-421 ${todo.done ? "todo-check--done-421" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-421" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-421"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-421" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-421" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-421">
            <span class="todo-count-421">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-421" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
