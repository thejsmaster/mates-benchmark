// client/components/Component0784.ts
// Benchmark component #784 — identical logic to TodoPage, scoped class names.
...
import "./Component0784.css";

type Filter = "all" | "active" | "done";

export const Component0784 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0784] mounted");
    return () => console.log("[Component0784] unmounted");
  });
  ...
  return () => html`
    <div class="app-784">
      <header class="header-784">
        <h1>todos — Component0784</h1>
      </header>

      <main class="main-784">
        <input
          class="add-input-784 ${error() ? "add-input--error-784" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-784" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-784">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-784">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-784 ${filter() === f ? "filter-btn--active-784" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-784">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-784">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-784">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-784 ${todo.done ? "todo-item--done-784" : ""}">
                    <button
                      class="todo-check-784 ${todo.done ? "todo-check--done-784" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-784" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-784"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-784" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-784" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-784">
            <span class="todo-count-784">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-784" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
