// client/components/Component0317.ts
// Benchmark component #317 — identical logic to TodoPage, scoped class names.
...
import "./Component0317.css";

type Filter = "all" | "active" | "done";

export const Component0317 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0317] mounted");
    return () => console.log("[Component0317] unmounted");
  });
  ...
  return () => html`
    <div class="app-317">
      <header class="header-317">
        <h1>todos — Component0317</h1>
      </header>

      <main class="main-317">
        <input
          class="add-input-317 ${error() ? "add-input--error-317" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-317" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-317">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-317">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-317 ${filter() === f ? "filter-btn--active-317" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-317">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-317">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-317">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-317 ${todo.done ? "todo-item--done-317" : ""}">
                    <button
                      class="todo-check-317 ${todo.done ? "todo-check--done-317" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-317" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-317"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-317" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-317" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-317">
            <span class="todo-count-317">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-317" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
