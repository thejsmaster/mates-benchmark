// client/components/Component0340.ts
// Benchmark component #340 — identical logic to TodoPage, scoped class names.
...
import "./Component0340.css";

type Filter = "all" | "active" | "done";

export const Component0340 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0340] mounted");
    return () => console.log("[Component0340] unmounted");
  });
  ...
  return () => html`
    <div class="app-340">
      <header class="header-340">
        <h1>todos — Component0340</h1>
      </header>

      <main class="main-340">
        <input
          class="add-input-340 ${error() ? "add-input--error-340" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-340" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-340">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-340">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-340 ${filter() === f ? "filter-btn--active-340" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-340">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-340">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-340">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-340 ${todo.done ? "todo-item--done-340" : ""}">
                    <button
                      class="todo-check-340 ${todo.done ? "todo-check--done-340" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-340" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-340"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-340" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-340" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-340">
            <span class="todo-count-340">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-340" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
