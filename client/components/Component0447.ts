// client/components/Component0447.ts
// Benchmark component #447 — identical logic to TodoPage, scoped class names.
...
import "./Component0447.css";

type Filter = "all" | "active" | "done";

export const Component0447 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0447] mounted");
    return () => console.log("[Component0447] unmounted");
  });
  ...
  return () => html`
    <div class="app-447">
      <header class="header-447">
        <h1>todos — Component0447</h1>
      </header>

      <main class="main-447">
        <input
          class="add-input-447 ${error() ? "add-input--error-447" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-447" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-447">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-447">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-447 ${filter() === f ? "filter-btn--active-447" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-447">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-447">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-447">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-447 ${todo.done ? "todo-item--done-447" : ""}">
                    <button
                      class="todo-check-447 ${todo.done ? "todo-check--done-447" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-447" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-447"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-447" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-447" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-447">
            <span class="todo-count-447">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-447" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
