// client/components/Component0873.ts
// Benchmark component #873 — identical logic to TodoPage, scoped class names.
...
import "./Component0873.css";

type Filter = "all" | "active" | "done";

export const Component0873 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0873] mounted");
    return () => console.log("[Component0873] unmounted");
  });
  ...
  return () => html`
    <div class="app-873">
      <header class="header-873">
        <h1>todos — Component0873</h1>
      </header>

      <main class="main-873">
        <input
          class="add-input-873 ${error() ? "add-input--error-873" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-873" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-873">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-873">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-873 ${filter() === f ? "filter-btn--active-873" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-873">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-873">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-873">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-873 ${todo.done ? "todo-item--done-873" : ""}">
                    <button
                      class="todo-check-873 ${todo.done ? "todo-check--done-873" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-873" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-873"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-873" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-873" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-873">
            <span class="todo-count-873">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-873" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
