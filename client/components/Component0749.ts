// client/components/Component0749.ts
// Benchmark component #749 — identical logic to TodoPage, scoped class names.
...
import "./Component0749.css";

type Filter = "all" | "active" | "done";

export const Component0749 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0749] mounted");
    return () => console.log("[Component0749] unmounted");
  });
  ...
  return () => html`
    <div class="app-749">
      <header class="header-749">
        <h1>todos — Component0749</h1>
      </header>

      <main class="main-749">
        <input
          class="add-input-749 ${error() ? "add-input--error-749" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-749" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-749">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-749">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-749 ${filter() === f ? "filter-btn--active-749" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-749">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-749">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-749">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-749 ${todo.done ? "todo-item--done-749" : ""}">
                    <button
                      class="todo-check-749 ${todo.done ? "todo-check--done-749" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-749" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-749"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-749" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-749" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-749">
            <span class="todo-count-749">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-749" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
