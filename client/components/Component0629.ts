// client/components/Component0629.ts
// Benchmark component #629 — identical logic to TodoPage, scoped class names.
...
import "./Component0629.css";

type Filter = "all" | "active" | "done";

export const Component0629 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0629] mounted");
    return () => console.log("[Component0629] unmounted");
  });
  ...
  return () => html`
    <div class="app-629">
      <header class="header-629">
        <h1>todos — Component0629</h1>
      </header>

      <main class="main-629">
        <input
          class="add-input-629 ${error() ? "add-input--error-629" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-629" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-629">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-629">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-629 ${filter() === f ? "filter-btn--active-629" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-629">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-629">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-629">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-629 ${todo.done ? "todo-item--done-629" : ""}">
                    <button
                      class="todo-check-629 ${todo.done ? "todo-check--done-629" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-629" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-629"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-629" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-629" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-629">
            <span class="todo-count-629">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-629" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
