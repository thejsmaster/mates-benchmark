// client/components/Component0411.ts
// Benchmark component #411 — identical logic to TodoPage, scoped class names.
...
import "./Component0411.css";

type Filter = "all" | "active" | "done";

export const Component0411 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0411] mounted");
    return () => console.log("[Component0411] unmounted");
  });
  ...
  return () => html`
    <div class="app-411">
      <header class="header-411">
        <h1>todos — Component0411</h1>
      </header>

      <main class="main-411">
        <input
          class="add-input-411 ${error() ? "add-input--error-411" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-411" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-411">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-411">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-411 ${filter() === f ? "filter-btn--active-411" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-411">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-411">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-411">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-411 ${todo.done ? "todo-item--done-411" : ""}">
                    <button
                      class="todo-check-411 ${todo.done ? "todo-check--done-411" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-411" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-411"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-411" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-411" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-411">
            <span class="todo-count-411">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-411" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
