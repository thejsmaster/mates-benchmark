// client/components/Component0224.ts
// Benchmark component #224 — identical logic to TodoPage, scoped class names.
...
import "./Component0224.css";

type Filter = "all" | "active" | "done";

export const Component0224 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0224] mounted");
    return () => console.log("[Component0224] unmounted");
  });
  ...
  return () => html`
    <div class="app-224">
      <header class="header-224">
        <h1>todos — Component0224</h1>
      </header>

      <main class="main-224">
        <input
          class="add-input-224 ${error() ? "add-input--error-224" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-224" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-224">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-224">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-224 ${filter() === f ? "filter-btn--active-224" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-224">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-224">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-224">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-224 ${todo.done ? "todo-item--done-224" : ""}">
                    <button
                      class="todo-check-224 ${todo.done ? "todo-check--done-224" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-224" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-224"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-224" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-224" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-224">
            <span class="todo-count-224">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-224" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
