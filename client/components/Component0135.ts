// client/components/Component0135.ts
// Benchmark component #135 — identical logic to TodoPage, scoped class names.
...
import "./Component0135.css";

type Filter = "all" | "active" | "done";

export const Component0135 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0135] mounted");
    return () => console.log("[Component0135] unmounted");
  });
  ...
  return () => html`
    <div class="app-135">
      <header class="header-135">
        <h1>todos — Component0135</h1>
      </header>

      <main class="main-135">
        <input
          class="add-input-135 ${error() ? "add-input--error-135" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-135" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-135">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-135">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-135 ${filter() === f ? "filter-btn--active-135" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-135">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-135">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-135">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-135 ${todo.done ? "todo-item--done-135" : ""}">
                    <button
                      class="todo-check-135 ${todo.done ? "todo-check--done-135" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-135" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-135"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-135" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-135" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-135">
            <span class="todo-count-135">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-135" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
