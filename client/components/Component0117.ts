// client/components/Component0117.ts
// Benchmark component #117 — identical logic to TodoPage, scoped class names.
...
import "./Component0117.css";

type Filter = "all" | "active" | "done";

export const Component0117 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0117] mounted");
    return () => console.log("[Component0117] unmounted");
  });
  ...
  return () => html`
    <div class="app-117">
      <header class="header-117">
        <h1>todos — Component0117</h1>
      </header>

      <main class="main-117">
        <input
          class="add-input-117 ${error() ? "add-input--error-117" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-117" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-117">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-117">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-117 ${filter() === f ? "filter-btn--active-117" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-117">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-117">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-117">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-117 ${todo.done ? "todo-item--done-117" : ""}">
                    <button
                      class="todo-check-117 ${todo.done ? "todo-check--done-117" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-117" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-117"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-117" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-117" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-117">
            <span class="todo-count-117">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-117" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
