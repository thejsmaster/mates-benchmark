// client/components/Component0359.ts
// Benchmark component #359 — identical logic to TodoPage, scoped class names.
...
import "./Component0359.css";

type Filter = "all" | "active" | "done";

export const Component0359 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0359] mounted");
    return () => console.log("[Component0359] unmounted");
  });
  ...
  return () => html`
    <div class="app-359">
      <header class="header-359">
        <h1>todos — Component0359</h1>
      </header>

      <main class="main-359">
        <input
          class="add-input-359 ${error() ? "add-input--error-359" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-359" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-359">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-359">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-359 ${filter() === f ? "filter-btn--active-359" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-359">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-359">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-359">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-359 ${todo.done ? "todo-item--done-359" : ""}">
                    <button
                      class="todo-check-359 ${todo.done ? "todo-check--done-359" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-359" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-359"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-359" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-359" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-359">
            <span class="todo-count-359">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-359" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
