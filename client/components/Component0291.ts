// client/components/Component0291.ts
// Benchmark component #291 — identical logic to TodoPage, scoped class names.
...
import "./Component0291.css";

type Filter = "all" | "active" | "done";

export const Component0291 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0291] mounted");
    return () => console.log("[Component0291] unmounted");
  });
  ...
  return () => html`
    <div class="app-291">
      <header class="header-291">
        <h1>todos — Component0291</h1>
      </header>

      <main class="main-291">
        <input
          class="add-input-291 ${error() ? "add-input--error-291" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-291" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-291">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-291">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-291 ${filter() === f ? "filter-btn--active-291" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-291">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-291">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-291">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-291 ${todo.done ? "todo-item--done-291" : ""}">
                    <button
                      class="todo-check-291 ${todo.done ? "todo-check--done-291" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-291" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-291"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-291" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-291" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-291">
            <span class="todo-count-291">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-291" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
