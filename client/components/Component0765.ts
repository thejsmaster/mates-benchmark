// client/components/Component0765.ts
// Benchmark component #765 — identical logic to TodoPage, scoped class names.
...
import "./Component0765.css";

type Filter = "all" | "active" | "done";

export const Component0765 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0765] mounted");
    return () => console.log("[Component0765] unmounted");
  });
  ...
  return () => html`
    <div class="app-765">
      <header class="header-765">
        <h1>todos — Component0765</h1>
      </header>

      <main class="main-765">
        <input
          class="add-input-765 ${error() ? "add-input--error-765" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-765" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-765">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-765">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-765 ${filter() === f ? "filter-btn--active-765" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-765">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-765">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-765">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-765 ${todo.done ? "todo-item--done-765" : ""}">
                    <button
                      class="todo-check-765 ${todo.done ? "todo-check--done-765" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-765" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-765"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-765" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-765" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-765">
            <span class="todo-count-765">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-765" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
