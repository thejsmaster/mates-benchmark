// client/components/Component0065.ts
// Benchmark component #65 — identical logic to TodoPage, scoped class names.
...
import "./Component0065.css";

type Filter = "all" | "active" | "done";

export const Component0065 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0065] mounted");
    return () => console.log("[Component0065] unmounted");
  });
  ...
  return () => html`
    <div class="app-65">
      <header class="header-65">
        <h1>todos — Component0065</h1>
      </header>

      <main class="main-65">
        <input
          class="add-input-65 ${error() ? "add-input--error-65" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-65" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-65">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-65">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-65 ${filter() === f ? "filter-btn--active-65" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-65">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-65">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-65">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-65 ${todo.done ? "todo-item--done-65" : ""}">
                    <button
                      class="todo-check-65 ${todo.done ? "todo-check--done-65" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-65" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-65"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-65" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-65" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-65">
            <span class="todo-count-65">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-65" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
