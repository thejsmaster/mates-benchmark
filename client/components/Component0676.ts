// client/components/Component0676.ts
// Benchmark component #676 — identical logic to TodoPage, scoped class names.
...
import "./Component0676.css";

type Filter = "all" | "active" | "done";

export const Component0676 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0676] mounted");
    return () => console.log("[Component0676] unmounted");
  });
  ...
  return () => html`
    <div class="app-676">
      <header class="header-676">
        <h1>todos — Component0676</h1>
      </header>

      <main class="main-676">
        <input
          class="add-input-676 ${error() ? "add-input--error-676" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-676" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-676">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-676">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-676 ${filter() === f ? "filter-btn--active-676" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-676">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-676">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-676">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-676 ${todo.done ? "todo-item--done-676" : ""}">
                    <button
                      class="todo-check-676 ${todo.done ? "todo-check--done-676" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-676" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-676"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-676" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-676" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-676">
            <span class="todo-count-676">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-676" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
