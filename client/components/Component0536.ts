// client/components/Component0536.ts
// Benchmark component #536 — identical logic to TodoPage, scoped class names.
...
import "./Component0536.css";

type Filter = "all" | "active" | "done";

export const Component0536 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0536] mounted");
    return () => console.log("[Component0536] unmounted");
  });
  ...
  return () => html`
    <div class="app-536">
      <header class="header-536">
        <h1>todos — Component0536</h1>
      </header>

      <main class="main-536">
        <input
          class="add-input-536 ${error() ? "add-input--error-536" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-536" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-536">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-536">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-536 ${filter() === f ? "filter-btn--active-536" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-536">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-536">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-536">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-536 ${todo.done ? "todo-item--done-536" : ""}">
                    <button
                      class="todo-check-536 ${todo.done ? "todo-check--done-536" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-536" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-536"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-536" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-536" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-536">
            <span class="todo-count-536">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-536" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
