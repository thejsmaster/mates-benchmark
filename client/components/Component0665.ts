// client/components/Component0665.ts
// Benchmark component #665 — identical logic to TodoPage, scoped class names.
...
import "./Component0665.css";

type Filter = "all" | "active" | "done";

export const Component0665 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0665] mounted");
    return () => console.log("[Component0665] unmounted");
  });
  ...
  return () => html`
    <div class="app-665">
      <header class="header-665">
        <h1>todos — Component0665</h1>
      </header>

      <main class="main-665">
        <input
          class="add-input-665 ${error() ? "add-input--error-665" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-665" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-665">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-665">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-665 ${filter() === f ? "filter-btn--active-665" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-665">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-665">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-665">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-665 ${todo.done ? "todo-item--done-665" : ""}">
                    <button
                      class="todo-check-665 ${todo.done ? "todo-check--done-665" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-665" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-665"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-665" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-665" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-665">
            <span class="todo-count-665">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-665" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
