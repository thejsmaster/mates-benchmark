// client/components/Component0418.ts
// Benchmark component #418 — identical logic to TodoPage, scoped class names.
...
import "./Component0418.css";

type Filter = "all" | "active" | "done";

export const Component0418 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0418] mounted");
    return () => console.log("[Component0418] unmounted");
  });
  ...
  return () => html`
    <div class="app-418">
      <header class="header-418">
        <h1>todos — Component0418</h1>
      </header>

      <main class="main-418">
        <input
          class="add-input-418 ${error() ? "add-input--error-418" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-418" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-418">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-418">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-418 ${filter() === f ? "filter-btn--active-418" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-418">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-418">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-418">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-418 ${todo.done ? "todo-item--done-418" : ""}">
                    <button
                      class="todo-check-418 ${todo.done ? "todo-check--done-418" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-418" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-418"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-418" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-418" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-418">
            <span class="todo-count-418">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-418" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
