// client/components/Component0520.ts
// Benchmark component #520 — identical logic to TodoPage, scoped class names.
...
import "./Component0520.css";

type Filter = "all" | "active" | "done";

export const Component0520 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0520] mounted");
    return () => console.log("[Component0520] unmounted");
  });
  ...
  return () => html`
    <div class="app-520">
      <header class="header-520">
        <h1>todos — Component0520</h1>
      </header>

      <main class="main-520">
        <input
          class="add-input-520 ${error() ? "add-input--error-520" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-520" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-520">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-520">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-520 ${filter() === f ? "filter-btn--active-520" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-520">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-520">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-520">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-520 ${todo.done ? "todo-item--done-520" : ""}">
                    <button
                      class="todo-check-520 ${todo.done ? "todo-check--done-520" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-520" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-520"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-520" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-520" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-520">
            <span class="todo-count-520">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-520" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
