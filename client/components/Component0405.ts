// client/components/Component0405.ts
// Benchmark component #405 — identical logic to TodoPage, scoped class names.
...
import "./Component0405.css";

type Filter = "all" | "active" | "done";

export const Component0405 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0405] mounted");
    return () => console.log("[Component0405] unmounted");
  });
  ...
  return () => html`
    <div class="app-405">
      <header class="header-405">
        <h1>todos — Component0405</h1>
      </header>

      <main class="main-405">
        <input
          class="add-input-405 ${error() ? "add-input--error-405" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-405" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-405">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-405">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-405 ${filter() === f ? "filter-btn--active-405" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-405">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-405">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-405">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-405 ${todo.done ? "todo-item--done-405" : ""}">
                    <button
                      class="todo-check-405 ${todo.done ? "todo-check--done-405" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-405" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-405"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-405" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-405" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-405">
            <span class="todo-count-405">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-405" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
