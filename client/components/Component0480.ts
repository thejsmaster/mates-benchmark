// client/components/Component0480.ts
// Benchmark component #480 — identical logic to TodoPage, scoped class names.
...
import "./Component0480.css";

type Filter = "all" | "active" | "done";

export const Component0480 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0480] mounted");
    return () => console.log("[Component0480] unmounted");
  });
  ...
  return () => html`
    <div class="app-480">
      <header class="header-480">
        <h1>todos — Component0480</h1>
      </header>

      <main class="main-480">
        <input
          class="add-input-480 ${error() ? "add-input--error-480" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-480" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-480">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-480">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-480 ${filter() === f ? "filter-btn--active-480" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-480">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-480">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-480">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-480 ${todo.done ? "todo-item--done-480" : ""}">
                    <button
                      class="todo-check-480 ${todo.done ? "todo-check--done-480" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-480" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-480"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-480" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-480" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-480">
            <span class="todo-count-480">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-480" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
