// client/components/Component0555.ts
// Benchmark component #555 — identical logic to TodoPage, scoped class names.
...
import "./Component0555.css";

type Filter = "all" | "active" | "done";

export const Component0555 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0555] mounted");
    return () => console.log("[Component0555] unmounted");
  });
  ...
  return () => html`
    <div class="app-555">
      <header class="header-555">
        <h1>todos — Component0555</h1>
      </header>

      <main class="main-555">
        <input
          class="add-input-555 ${error() ? "add-input--error-555" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-555" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-555">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-555">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-555 ${filter() === f ? "filter-btn--active-555" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-555">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-555">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-555">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-555 ${todo.done ? "todo-item--done-555" : ""}">
                    <button
                      class="todo-check-555 ${todo.done ? "todo-check--done-555" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-555" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-555"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-555" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-555" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-555">
            <span class="todo-count-555">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-555" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
