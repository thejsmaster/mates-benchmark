// client/components/Component0710.ts
// Benchmark component #710 — identical logic to TodoPage, scoped class names.
...
import "./Component0710.css";

type Filter = "all" | "active" | "done";

export const Component0710 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0710] mounted");
    return () => console.log("[Component0710] unmounted");
  });
  ...
  return () => html`
    <div class="app-710">
      <header class="header-710">
        <h1>todos — Component0710</h1>
      </header>

      <main class="main-710">
        <input
          class="add-input-710 ${error() ? "add-input--error-710" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-710" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-710">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-710">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-710 ${filter() === f ? "filter-btn--active-710" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-710">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-710">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-710">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-710 ${todo.done ? "todo-item--done-710" : ""}">
                    <button
                      class="todo-check-710 ${todo.done ? "todo-check--done-710" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-710" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-710"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-710" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-710" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-710">
            <span class="todo-count-710">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-710" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
