// client/components/Component0248.ts
// Benchmark component #248 — identical logic to TodoPage, scoped class names.
...
import "./Component0248.css";

type Filter = "all" | "active" | "done";

export const Component0248 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0248] mounted");
    return () => console.log("[Component0248] unmounted");
  });
  ...
  return () => html`
    <div class="app-248">
      <header class="header-248">
        <h1>todos — Component0248</h1>
      </header>

      <main class="main-248">
        <input
          class="add-input-248 ${error() ? "add-input--error-248" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-248" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-248">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-248">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-248 ${filter() === f ? "filter-btn--active-248" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-248">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-248">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-248">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-248 ${todo.done ? "todo-item--done-248" : ""}">
                    <button
                      class="todo-check-248 ${todo.done ? "todo-check--done-248" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-248" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-248"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-248" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-248" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-248">
            <span class="todo-count-248">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-248" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
