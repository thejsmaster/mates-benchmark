// client/components/Component0808.ts
// Benchmark component #808 — identical logic to TodoPage, scoped class names.
...
import "./Component0808.css";

type Filter = "all" | "active" | "done";

export const Component0808 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0808] mounted");
    return () => console.log("[Component0808] unmounted");
  });
  ...
  return () => html`
    <div class="app-808">
      <header class="header-808">
        <h1>todos — Component0808</h1>
      </header>

      <main class="main-808">
        <input
          class="add-input-808 ${error() ? "add-input--error-808" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-808" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-808">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-808">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-808 ${filter() === f ? "filter-btn--active-808" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-808">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-808">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-808">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-808 ${todo.done ? "todo-item--done-808" : ""}">
                    <button
                      class="todo-check-808 ${todo.done ? "todo-check--done-808" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-808" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-808"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-808" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-808" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-808">
            <span class="todo-count-808">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-808" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
