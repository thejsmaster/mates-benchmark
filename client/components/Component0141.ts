// client/components/Component0141.ts
// Benchmark component #141 — identical logic to TodoPage, scoped class names.
...
import "./Component0141.css";

type Filter = "all" | "active" | "done";

export const Component0141 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0141] mounted");
    return () => console.log("[Component0141] unmounted");
  });
  ...
  return () => html`
    <div class="app-141">
      <header class="header-141">
        <h1>todos — Component0141</h1>
      </header>

      <main class="main-141">
        <input
          class="add-input-141 ${error() ? "add-input--error-141" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-141" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-141">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-141">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-141 ${filter() === f ? "filter-btn--active-141" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-141">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-141">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-141">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-141 ${todo.done ? "todo-item--done-141" : ""}">
                    <button
                      class="todo-check-141 ${todo.done ? "todo-check--done-141" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-141" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-141"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-141" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-141" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-141">
            <span class="todo-count-141">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-141" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
