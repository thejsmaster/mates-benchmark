// client/components/Component0107.ts
// Benchmark component #107 — identical logic to TodoPage, scoped class names.
...
import "./Component0107.css";

type Filter = "all" | "active" | "done";

export const Component0107 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0107] mounted");
    return () => console.log("[Component0107] unmounted");
  });
  ...
  return () => html`
    <div class="app-107">
      <header class="header-107">
        <h1>todos — Component0107</h1>
      </header>

      <main class="main-107">
        <input
          class="add-input-107 ${error() ? "add-input--error-107" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-107" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-107">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-107">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-107 ${filter() === f ? "filter-btn--active-107" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-107">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-107">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-107">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-107 ${todo.done ? "todo-item--done-107" : ""}">
                    <button
                      class="todo-check-107 ${todo.done ? "todo-check--done-107" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-107" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-107"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-107" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-107" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-107">
            <span class="todo-count-107">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-107" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
