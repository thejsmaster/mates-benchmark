// client/components/Component0625.ts
// Benchmark component #625 — identical logic to TodoPage, scoped class names.
...
import "./Component0625.css";

type Filter = "all" | "active" | "done";

export const Component0625 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0625] mounted");
    return () => console.log("[Component0625] unmounted");
  });
  ...
  return () => html`
    <div class="app-625">
      <header class="header-625">
        <h1>todos — Component0625</h1>
      </header>

      <main class="main-625">
        <input
          class="add-input-625 ${error() ? "add-input--error-625" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-625" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-625">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-625">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-625 ${filter() === f ? "filter-btn--active-625" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-625">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-625">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-625">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-625 ${todo.done ? "todo-item--done-625" : ""}">
                    <button
                      class="todo-check-625 ${todo.done ? "todo-check--done-625" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-625" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-625"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-625" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-625" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-625">
            <span class="todo-count-625">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-625" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
