// client/components/Component0351.ts
// Benchmark component #351 — identical logic to TodoPage, scoped class names.
...
import "./Component0351.css";

type Filter = "all" | "active" | "done";

export const Component0351 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0351] mounted");
    return () => console.log("[Component0351] unmounted");
  });
  ...
  return () => html`
    <div class="app-351">
      <header class="header-351">
        <h1>todos — Component0351</h1>
      </header>

      <main class="main-351">
        <input
          class="add-input-351 ${error() ? "add-input--error-351" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-351" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-351">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-351">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-351 ${filter() === f ? "filter-btn--active-351" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-351">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-351">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-351">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-351 ${todo.done ? "todo-item--done-351" : ""}">
                    <button
                      class="todo-check-351 ${todo.done ? "todo-check--done-351" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-351" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-351"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-351" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-351" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-351">
            <span class="todo-count-351">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-351" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
