// client/components/Component0636.ts
// Benchmark component #636 — identical logic to TodoPage, scoped class names.
...
import "./Component0636.css";

type Filter = "all" | "active" | "done";

export const Component0636 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0636] mounted");
    return () => console.log("[Component0636] unmounted");
  });
  ...
  return () => html`
    <div class="app-636">
      <header class="header-636">
        <h1>todos — Component0636</h1>
      </header>

      <main class="main-636">
        <input
          class="add-input-636 ${error() ? "add-input--error-636" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-636" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-636">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-636">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-636 ${filter() === f ? "filter-btn--active-636" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-636">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-636">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-636">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-636 ${todo.done ? "todo-item--done-636" : ""}">
                    <button
                      class="todo-check-636 ${todo.done ? "todo-check--done-636" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-636" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-636"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-636" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-636" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-636">
            <span class="todo-count-636">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-636" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
