// client/components/Component0716.ts
// Benchmark component #716 — identical logic to TodoPage, scoped class names.
...
import "./Component0716.css";

type Filter = "all" | "active" | "done";

export const Component0716 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0716] mounted");
    return () => console.log("[Component0716] unmounted");
  });
  ...
  return () => html`
    <div class="app-716">
      <header class="header-716">
        <h1>todos — Component0716</h1>
      </header>

      <main class="main-716">
        <input
          class="add-input-716 ${error() ? "add-input--error-716" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-716" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-716">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-716">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-716 ${filter() === f ? "filter-btn--active-716" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-716">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-716">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-716">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-716 ${todo.done ? "todo-item--done-716" : ""}">
                    <button
                      class="todo-check-716 ${todo.done ? "todo-check--done-716" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-716" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-716"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-716" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-716" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-716">
            <span class="todo-count-716">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-716" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
