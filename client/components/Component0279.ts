// client/components/Component0279.ts
// Benchmark component #279 — identical logic to TodoPage, scoped class names.
...
import "./Component0279.css";

type Filter = "all" | "active" | "done";

export const Component0279 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0279] mounted");
    return () => console.log("[Component0279] unmounted");
  });
  ...
  return () => html`
    <div class="app-279">
      <header class="header-279">
        <h1>todos — Component0279</h1>
      </header>

      <main class="main-279">
        <input
          class="add-input-279 ${error() ? "add-input--error-279" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-279" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-279">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-279">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-279 ${filter() === f ? "filter-btn--active-279" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-279">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-279">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-279">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-279 ${todo.done ? "todo-item--done-279" : ""}">
                    <button
                      class="todo-check-279 ${todo.done ? "todo-check--done-279" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-279" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-279"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-279" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-279" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-279">
            <span class="todo-count-279">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-279" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
