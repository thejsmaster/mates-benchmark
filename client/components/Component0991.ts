// client/components/Component0991.ts
// Benchmark component #991 — identical logic to TodoPage, scoped class names.
...
import "./Component0991.css";

type Filter = "all" | "active" | "done";

export const Component0991 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0991] mounted");
    return () => console.log("[Component0991] unmounted");
  });
  ...
  return () => html`
    <div class="app-991">
      <header class="header-991">
        <h1>todos — Component0991</h1>
      </header>

      <main class="main-991">
        <input
          class="add-input-991 ${error() ? "add-input--error-991" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-991" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-991">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-991">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-991 ${filter() === f ? "filter-btn--active-991" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-991">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-991">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-991">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-991 ${todo.done ? "todo-item--done-991" : ""}">
                    <button
                      class="todo-check-991 ${todo.done ? "todo-check--done-991" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-991" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-991"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-991" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-991" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-991">
            <span class="todo-count-991">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-991" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
