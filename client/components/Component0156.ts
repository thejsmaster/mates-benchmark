// client/components/Component0156.ts
// Benchmark component #156 — identical logic to TodoPage, scoped class names.
...
import "./Component0156.css";

type Filter = "all" | "active" | "done";

export const Component0156 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0156] mounted");
    return () => console.log("[Component0156] unmounted");
  });
  ...
  return () => html`
    <div class="app-156">
      <header class="header-156">
        <h1>todos — Component0156</h1>
      </header>

      <main class="main-156">
        <input
          class="add-input-156 ${error() ? "add-input--error-156" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-156" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-156">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-156">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-156 ${filter() === f ? "filter-btn--active-156" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-156">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-156">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-156">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-156 ${todo.done ? "todo-item--done-156" : ""}">
                    <button
                      class="todo-check-156 ${todo.done ? "todo-check--done-156" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-156" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-156"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-156" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-156" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-156">
            <span class="todo-count-156">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-156" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
