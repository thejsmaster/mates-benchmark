// client/components/Component0630.ts
// Benchmark component #630 — identical logic to TodoPage, scoped class names.
...
import "./Component0630.css";

type Filter = "all" | "active" | "done";

export const Component0630 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0630] mounted");
    return () => console.log("[Component0630] unmounted");
  });
  ...
  return () => html`
    <div class="app-630">
      <header class="header-630">
        <h1>todos — Component0630</h1>
      </header>

      <main class="main-630">
        <input
          class="add-input-630 ${error() ? "add-input--error-630" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-630" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-630">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-630">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-630 ${filter() === f ? "filter-btn--active-630" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-630">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-630">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-630">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-630 ${todo.done ? "todo-item--done-630" : ""}">
                    <button
                      class="todo-check-630 ${todo.done ? "todo-check--done-630" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-630" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-630"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-630" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-630" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-630">
            <span class="todo-count-630">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-630" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
