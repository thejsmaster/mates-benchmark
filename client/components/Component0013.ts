// client/components/Component0013.ts
// Benchmark component #13 — identical logic to TodoPage, scoped class names.
...
import "./Component0013.css";

type Filter = "all" | "active" | "done";

export const Component0013 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0013] mounted");
    return () => console.log("[Component0013] unmounted");
  });
  ...
  return () => html`
    <div class="app-13">
      <header class="header-13">
        <h1>todos — Component0013</h1>
      </header>

      <main class="main-13">
        <input
          class="add-input-13 ${error() ? "add-input--error-13" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-13" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-13">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-13">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-13 ${filter() === f ? "filter-btn--active-13" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-13">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-13">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-13">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-13 ${todo.done ? "todo-item--done-13" : ""}">
                    <button
                      class="todo-check-13 ${todo.done ? "todo-check--done-13" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-13" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-13"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-13" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-13" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-13">
            <span class="todo-count-13">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-13" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
