// client/components/Component0648.ts
// Benchmark component #648 — identical logic to TodoPage, scoped class names.
...
import "./Component0648.css";

type Filter = "all" | "active" | "done";

export const Component0648 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0648] mounted");
    return () => console.log("[Component0648] unmounted");
  });
  ...
  return () => html`
    <div class="app-648">
      <header class="header-648">
        <h1>todos — Component0648</h1>
      </header>

      <main class="main-648">
        <input
          class="add-input-648 ${error() ? "add-input--error-648" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-648" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-648">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-648">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-648 ${filter() === f ? "filter-btn--active-648" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-648">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-648">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-648">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-648 ${todo.done ? "todo-item--done-648" : ""}">
                    <button
                      class="todo-check-648 ${todo.done ? "todo-check--done-648" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-648" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-648"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-648" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-648" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-648">
            <span class="todo-count-648">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-648" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
