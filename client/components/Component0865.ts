// client/components/Component0865.ts
// Benchmark component #865 — identical logic to TodoPage, scoped class names.
...
import "./Component0865.css";

type Filter = "all" | "active" | "done";

export const Component0865 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0865] mounted");
    return () => console.log("[Component0865] unmounted");
  });
  ...
  return () => html`
    <div class="app-865">
      <header class="header-865">
        <h1>todos — Component0865</h1>
      </header>

      <main class="main-865">
        <input
          class="add-input-865 ${error() ? "add-input--error-865" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-865" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-865">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-865">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-865 ${filter() === f ? "filter-btn--active-865" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-865">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-865">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-865">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-865 ${todo.done ? "todo-item--done-865" : ""}">
                    <button
                      class="todo-check-865 ${todo.done ? "todo-check--done-865" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-865" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-865"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-865" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-865" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-865">
            <span class="todo-count-865">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-865" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
