// client/components/Component0250.ts
// Benchmark component #250 — identical logic to TodoPage, scoped class names.
...
import "./Component0250.css";

type Filter = "all" | "active" | "done";

export const Component0250 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0250] mounted");
    return () => console.log("[Component0250] unmounted");
  });
  ...
  return () => html`
    <div class="app-250">
      <header class="header-250">
        <h1>todos — Component0250</h1>
      </header>

      <main class="main-250">
        <input
          class="add-input-250 ${error() ? "add-input--error-250" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-250" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-250">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-250">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-250 ${filter() === f ? "filter-btn--active-250" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-250">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-250">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-250">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-250 ${todo.done ? "todo-item--done-250" : ""}">
                    <button
                      class="todo-check-250 ${todo.done ? "todo-check--done-250" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-250" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-250"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-250" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-250" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-250">
            <span class="todo-count-250">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-250" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
