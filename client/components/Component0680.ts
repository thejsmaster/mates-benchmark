// client/components/Component0680.ts
// Benchmark component #680 — identical logic to TodoPage, scoped class names.
...
import "./Component0680.css";

type Filter = "all" | "active" | "done";

export const Component0680 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0680] mounted");
    return () => console.log("[Component0680] unmounted");
  });
  ...
  return () => html`
    <div class="app-680">
      <header class="header-680">
        <h1>todos — Component0680</h1>
      </header>

      <main class="main-680">
        <input
          class="add-input-680 ${error() ? "add-input--error-680" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-680" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-680">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-680">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-680 ${filter() === f ? "filter-btn--active-680" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-680">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-680">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-680">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-680 ${todo.done ? "todo-item--done-680" : ""}">
                    <button
                      class="todo-check-680 ${todo.done ? "todo-check--done-680" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-680" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-680"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-680" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-680" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-680">
            <span class="todo-count-680">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-680" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
