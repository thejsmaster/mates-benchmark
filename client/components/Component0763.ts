// client/components/Component0763.ts
// Benchmark component #763 — identical logic to TodoPage, scoped class names.
...
import "./Component0763.css";

type Filter = "all" | "active" | "done";

export const Component0763 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0763] mounted");
    return () => console.log("[Component0763] unmounted");
  });
  ...
  return () => html`
    <div class="app-763">
      <header class="header-763">
        <h1>todos — Component0763</h1>
      </header>

      <main class="main-763">
        <input
          class="add-input-763 ${error() ? "add-input--error-763" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-763" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-763">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-763">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-763 ${filter() === f ? "filter-btn--active-763" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-763">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-763">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-763">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-763 ${todo.done ? "todo-item--done-763" : ""}">
                    <button
                      class="todo-check-763 ${todo.done ? "todo-check--done-763" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-763" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-763"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-763" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-763" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-763">
            <span class="todo-count-763">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-763" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
