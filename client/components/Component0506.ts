// client/components/Component0506.ts
// Benchmark component #506 — identical logic to TodoPage, scoped class names.
...
import "./Component0506.css";

type Filter = "all" | "active" | "done";

export const Component0506 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0506] mounted");
    return () => console.log("[Component0506] unmounted");
  });
  ...
  return () => html`
    <div class="app-506">
      <header class="header-506">
        <h1>todos — Component0506</h1>
      </header>

      <main class="main-506">
        <input
          class="add-input-506 ${error() ? "add-input--error-506" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-506" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-506">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-506">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-506 ${filter() === f ? "filter-btn--active-506" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-506">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-506">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-506">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-506 ${todo.done ? "todo-item--done-506" : ""}">
                    <button
                      class="todo-check-506 ${todo.done ? "todo-check--done-506" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-506" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-506"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-506" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-506" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-506">
            <span class="todo-count-506">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-506" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
