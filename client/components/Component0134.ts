// client/components/Component0134.ts
// Benchmark component #134 — identical logic to TodoPage, scoped class names.
...
import "./Component0134.css";

type Filter = "all" | "active" | "done";

export const Component0134 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0134] mounted");
    return () => console.log("[Component0134] unmounted");
  });
  ...
  return () => html`
    <div class="app-134">
      <header class="header-134">
        <h1>todos — Component0134</h1>
      </header>

      <main class="main-134">
        <input
          class="add-input-134 ${error() ? "add-input--error-134" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-134" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-134">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-134">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-134 ${filter() === f ? "filter-btn--active-134" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-134">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-134">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-134">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-134 ${todo.done ? "todo-item--done-134" : ""}">
                    <button
                      class="todo-check-134 ${todo.done ? "todo-check--done-134" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-134" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-134"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-134" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-134" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-134">
            <span class="todo-count-134">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-134" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
