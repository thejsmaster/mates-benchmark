// client/components/Component0195.ts
// Benchmark component #195 — identical logic to TodoPage, scoped class names.
...
import "./Component0195.css";

type Filter = "all" | "active" | "done";

export const Component0195 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0195] mounted");
    return () => console.log("[Component0195] unmounted");
  });
  ...
  return () => html`
    <div class="app-195">
      <header class="header-195">
        <h1>todos — Component0195</h1>
      </header>

      <main class="main-195">
        <input
          class="add-input-195 ${error() ? "add-input--error-195" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-195" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-195">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-195">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-195 ${filter() === f ? "filter-btn--active-195" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-195">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-195">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-195">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-195 ${todo.done ? "todo-item--done-195" : ""}">
                    <button
                      class="todo-check-195 ${todo.done ? "todo-check--done-195" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-195" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-195"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-195" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-195" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-195">
            <span class="todo-count-195">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-195" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
