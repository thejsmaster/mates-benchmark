// client/components/Component0434.ts
// Benchmark component #434 — identical logic to TodoPage, scoped class names.
...
import "./Component0434.css";

type Filter = "all" | "active" | "done";

export const Component0434 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0434] mounted");
    return () => console.log("[Component0434] unmounted");
  });
  ...
  return () => html`
    <div class="app-434">
      <header class="header-434">
        <h1>todos — Component0434</h1>
      </header>

      <main class="main-434">
        <input
          class="add-input-434 ${error() ? "add-input--error-434" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-434" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-434">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-434">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-434 ${filter() === f ? "filter-btn--active-434" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-434">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-434">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-434">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-434 ${todo.done ? "todo-item--done-434" : ""}">
                    <button
                      class="todo-check-434 ${todo.done ? "todo-check--done-434" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-434" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-434"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-434" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-434" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-434">
            <span class="todo-count-434">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-434" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
