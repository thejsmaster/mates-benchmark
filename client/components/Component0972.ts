// client/components/Component0972.ts
// Benchmark component #972 — identical logic to TodoPage, scoped class names.
...
import "./Component0972.css";

type Filter = "all" | "active" | "done";

export const Component0972 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0972] mounted");
    return () => console.log("[Component0972] unmounted");
  });
  ...
  return () => html`
    <div class="app-972">
      <header class="header-972">
        <h1>todos — Component0972</h1>
      </header>

      <main class="main-972">
        <input
          class="add-input-972 ${error() ? "add-input--error-972" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-972" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-972">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-972">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-972 ${filter() === f ? "filter-btn--active-972" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-972">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-972">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-972">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-972 ${todo.done ? "todo-item--done-972" : ""}">
                    <button
                      class="todo-check-972 ${todo.done ? "todo-check--done-972" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-972" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-972"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-972" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-972" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-972">
            <span class="todo-count-972">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-972" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
