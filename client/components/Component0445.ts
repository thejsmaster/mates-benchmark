// client/components/Component0445.ts
// Benchmark component #445 — identical logic to TodoPage, scoped class names.
...
import "./Component0445.css";

type Filter = "all" | "active" | "done";

export const Component0445 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0445] mounted");
    return () => console.log("[Component0445] unmounted");
  });
  ...
  return () => html`
    <div class="app-445">
      <header class="header-445">
        <h1>todos — Component0445</h1>
      </header>

      <main class="main-445">
        <input
          class="add-input-445 ${error() ? "add-input--error-445" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-445" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-445">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-445">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-445 ${filter() === f ? "filter-btn--active-445" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-445">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-445">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-445">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-445 ${todo.done ? "todo-item--done-445" : ""}">
                    <button
                      class="todo-check-445 ${todo.done ? "todo-check--done-445" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-445" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-445"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-445" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-445" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-445">
            <span class="todo-count-445">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-445" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
