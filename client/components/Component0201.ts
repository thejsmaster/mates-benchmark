// client/components/Component0201.ts
// Benchmark component #201 — identical logic to TodoPage, scoped class names.
...
import "./Component0201.css";

type Filter = "all" | "active" | "done";

export const Component0201 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0201] mounted");
    return () => console.log("[Component0201] unmounted");
  });
  ...
  return () => html`
    <div class="app-201">
      <header class="header-201">
        <h1>todos — Component0201</h1>
      </header>

      <main class="main-201">
        <input
          class="add-input-201 ${error() ? "add-input--error-201" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-201" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-201">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-201">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-201 ${filter() === f ? "filter-btn--active-201" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-201">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-201">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-201">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-201 ${todo.done ? "todo-item--done-201" : ""}">
                    <button
                      class="todo-check-201 ${todo.done ? "todo-check--done-201" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-201" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-201"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-201" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-201" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-201">
            <span class="todo-count-201">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-201" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
