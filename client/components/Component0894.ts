// client/components/Component0894.ts
// Benchmark component #894 — identical logic to TodoPage, scoped class names.
...
import "./Component0894.css";

type Filter = "all" | "active" | "done";

export const Component0894 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0894] mounted");
    return () => console.log("[Component0894] unmounted");
  });
  ...
  return () => html`
    <div class="app-894">
      <header class="header-894">
        <h1>todos — Component0894</h1>
      </header>

      <main class="main-894">
        <input
          class="add-input-894 ${error() ? "add-input--error-894" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-894" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-894">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-894">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-894 ${filter() === f ? "filter-btn--active-894" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-894">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-894">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-894">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-894 ${todo.done ? "todo-item--done-894" : ""}">
                    <button
                      class="todo-check-894 ${todo.done ? "todo-check--done-894" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-894" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-894"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-894" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-894" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-894">
            <span class="todo-count-894">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-894" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
