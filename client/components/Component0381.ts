// client/components/Component0381.ts
// Benchmark component #381 — identical logic to TodoPage, scoped class names.
...
import "./Component0381.css";

type Filter = "all" | "active" | "done";

export const Component0381 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0381] mounted");
    return () => console.log("[Component0381] unmounted");
  });
  ...
  return () => html`
    <div class="app-381">
      <header class="header-381">
        <h1>todos — Component0381</h1>
      </header>

      <main class="main-381">
        <input
          class="add-input-381 ${error() ? "add-input--error-381" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-381" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-381">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-381">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-381 ${filter() === f ? "filter-btn--active-381" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-381">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-381">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-381">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-381 ${todo.done ? "todo-item--done-381" : ""}">
                    <button
                      class="todo-check-381 ${todo.done ? "todo-check--done-381" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-381" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-381"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-381" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-381" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-381">
            <span class="todo-count-381">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-381" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
