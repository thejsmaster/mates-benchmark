// client/components/Component0729.ts
// Benchmark component #729 — identical logic to TodoPage, scoped class names.
...
import "./Component0729.css";

type Filter = "all" | "active" | "done";

export const Component0729 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0729] mounted");
    return () => console.log("[Component0729] unmounted");
  });
  ...
  return () => html`
    <div class="app-729">
      <header class="header-729">
        <h1>todos — Component0729</h1>
      </header>

      <main class="main-729">
        <input
          class="add-input-729 ${error() ? "add-input--error-729" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-729" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-729">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-729">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-729 ${filter() === f ? "filter-btn--active-729" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-729">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-729">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-729">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-729 ${todo.done ? "todo-item--done-729" : ""}">
                    <button
                      class="todo-check-729 ${todo.done ? "todo-check--done-729" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-729" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-729"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-729" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-729" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-729">
            <span class="todo-count-729">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-729" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
