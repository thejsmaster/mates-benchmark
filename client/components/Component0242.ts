// client/components/Component0242.ts
// Benchmark component #242 — identical logic to TodoPage, scoped class names.
...
import "./Component0242.css";

type Filter = "all" | "active" | "done";

export const Component0242 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0242] mounted");
    return () => console.log("[Component0242] unmounted");
  });
  ...
  return () => html`
    <div class="app-242">
      <header class="header-242">
        <h1>todos — Component0242</h1>
      </header>

      <main class="main-242">
        <input
          class="add-input-242 ${error() ? "add-input--error-242" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-242" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-242">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-242">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-242 ${filter() === f ? "filter-btn--active-242" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-242">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-242">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-242">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-242 ${todo.done ? "todo-item--done-242" : ""}">
                    <button
                      class="todo-check-242 ${todo.done ? "todo-check--done-242" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-242" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-242"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-242" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-242" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-242">
            <span class="todo-count-242">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-242" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
