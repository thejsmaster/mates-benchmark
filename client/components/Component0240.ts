// client/components/Component0240.ts
// Benchmark component #240 — identical logic to TodoPage, scoped class names.
...
import "./Component0240.css";

type Filter = "all" | "active" | "done";

export const Component0240 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0240] mounted");
    return () => console.log("[Component0240] unmounted");
  });
  ...
  return () => html`
    <div class="app-240">
      <header class="header-240">
        <h1>todos — Component0240</h1>
      </header>

      <main class="main-240">
        <input
          class="add-input-240 ${error() ? "add-input--error-240" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-240" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-240">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-240">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-240 ${filter() === f ? "filter-btn--active-240" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-240">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-240">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-240">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-240 ${todo.done ? "todo-item--done-240" : ""}">
                    <button
                      class="todo-check-240 ${todo.done ? "todo-check--done-240" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-240" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-240"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-240" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-240" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-240">
            <span class="todo-count-240">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-240" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
