// client/components/Component0664.ts
// Benchmark component #664 — identical logic to TodoPage, scoped class names.
...
import "./Component0664.css";

type Filter = "all" | "active" | "done";

export const Component0664 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0664] mounted");
    return () => console.log("[Component0664] unmounted");
  });
  ...
  return () => html`
    <div class="app-664">
      <header class="header-664">
        <h1>todos — Component0664</h1>
      </header>

      <main class="main-664">
        <input
          class="add-input-664 ${error() ? "add-input--error-664" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-664" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-664">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-664">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-664 ${filter() === f ? "filter-btn--active-664" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-664">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-664">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-664">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-664 ${todo.done ? "todo-item--done-664" : ""}">
                    <button
                      class="todo-check-664 ${todo.done ? "todo-check--done-664" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-664" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-664"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-664" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-664" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-664">
            <span class="todo-count-664">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-664" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
