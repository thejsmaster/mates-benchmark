// client/components/Component0528.ts
// Benchmark component #528 — identical logic to TodoPage, scoped class names.
...
import "./Component0528.css";

type Filter = "all" | "active" | "done";

export const Component0528 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0528] mounted");
    return () => console.log("[Component0528] unmounted");
  });
  ...
  return () => html`
    <div class="app-528">
      <header class="header-528">
        <h1>todos — Component0528</h1>
      </header>

      <main class="main-528">
        <input
          class="add-input-528 ${error() ? "add-input--error-528" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-528" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-528">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-528">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-528 ${filter() === f ? "filter-btn--active-528" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-528">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-528">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-528">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-528 ${todo.done ? "todo-item--done-528" : ""}">
                    <button
                      class="todo-check-528 ${todo.done ? "todo-check--done-528" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-528" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-528"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-528" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-528" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-528">
            <span class="todo-count-528">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-528" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
