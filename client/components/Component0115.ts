// client/components/Component0115.ts
// Benchmark component #115 — identical logic to TodoPage, scoped class names.
...
import "./Component0115.css";

type Filter = "all" | "active" | "done";

export const Component0115 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0115] mounted");
    return () => console.log("[Component0115] unmounted");
  });
  ...
  return () => html`
    <div class="app-115">
      <header class="header-115">
        <h1>todos — Component0115</h1>
      </header>

      <main class="main-115">
        <input
          class="add-input-115 ${error() ? "add-input--error-115" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-115" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-115">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-115">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-115 ${filter() === f ? "filter-btn--active-115" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-115">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-115">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-115">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-115 ${todo.done ? "todo-item--done-115" : ""}">
                    <button
                      class="todo-check-115 ${todo.done ? "todo-check--done-115" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-115" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-115"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-115" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-115" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-115">
            <span class="todo-count-115">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-115" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
