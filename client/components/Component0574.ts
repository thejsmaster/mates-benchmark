// client/components/Component0574.ts
// Benchmark component #574 — identical logic to TodoPage, scoped class names.
...
import "./Component0574.css";

type Filter = "all" | "active" | "done";

export const Component0574 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0574] mounted");
    return () => console.log("[Component0574] unmounted");
  });
  ...
  return () => html`
    <div class="app-574">
      <header class="header-574">
        <h1>todos — Component0574</h1>
      </header>

      <main class="main-574">
        <input
          class="add-input-574 ${error() ? "add-input--error-574" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-574" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-574">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-574">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-574 ${filter() === f ? "filter-btn--active-574" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-574">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-574">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-574">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-574 ${todo.done ? "todo-item--done-574" : ""}">
                    <button
                      class="todo-check-574 ${todo.done ? "todo-check--done-574" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-574" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-574"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-574" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-574" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-574">
            <span class="todo-count-574">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-574" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
