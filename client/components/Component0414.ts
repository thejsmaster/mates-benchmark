// client/components/Component0414.ts
// Benchmark component #414 — identical logic to TodoPage, scoped class names.
...
import "./Component0414.css";

type Filter = "all" | "active" | "done";

export const Component0414 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0414] mounted");
    return () => console.log("[Component0414] unmounted");
  });
  ...
  return () => html`
    <div class="app-414">
      <header class="header-414">
        <h1>todos — Component0414</h1>
      </header>

      <main class="main-414">
        <input
          class="add-input-414 ${error() ? "add-input--error-414" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-414" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-414">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-414">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-414 ${filter() === f ? "filter-btn--active-414" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-414">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-414">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-414">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-414 ${todo.done ? "todo-item--done-414" : ""}">
                    <button
                      class="todo-check-414 ${todo.done ? "todo-check--done-414" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-414" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-414"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-414" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-414" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-414">
            <span class="todo-count-414">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-414" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
