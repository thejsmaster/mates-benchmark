// client/components/Component0457.ts
// Benchmark component #457 — identical logic to TodoPage, scoped class names.
...
import "./Component0457.css";

type Filter = "all" | "active" | "done";

export const Component0457 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0457] mounted");
    return () => console.log("[Component0457] unmounted");
  });
  ...
  return () => html`
    <div class="app-457">
      <header class="header-457">
        <h1>todos — Component0457</h1>
      </header>

      <main class="main-457">
        <input
          class="add-input-457 ${error() ? "add-input--error-457" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-457" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-457">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-457">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-457 ${filter() === f ? "filter-btn--active-457" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-457">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-457">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-457">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-457 ${todo.done ? "todo-item--done-457" : ""}">
                    <button
                      class="todo-check-457 ${todo.done ? "todo-check--done-457" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-457" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-457"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-457" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-457" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-457">
            <span class="todo-count-457">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-457" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
