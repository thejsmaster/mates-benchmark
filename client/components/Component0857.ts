// client/components/Component0857.ts
// Benchmark component #857 — identical logic to TodoPage, scoped class names.
...
import "./Component0857.css";

type Filter = "all" | "active" | "done";

export const Component0857 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0857] mounted");
    return () => console.log("[Component0857] unmounted");
  });
  ...
  return () => html`
    <div class="app-857">
      <header class="header-857">
        <h1>todos — Component0857</h1>
      </header>

      <main class="main-857">
        <input
          class="add-input-857 ${error() ? "add-input--error-857" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-857" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-857">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-857">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-857 ${filter() === f ? "filter-btn--active-857" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-857">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-857">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-857">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-857 ${todo.done ? "todo-item--done-857" : ""}">
                    <button
                      class="todo-check-857 ${todo.done ? "todo-check--done-857" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-857" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-857"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-857" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-857" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-857">
            <span class="todo-count-857">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-857" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
