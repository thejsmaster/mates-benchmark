// client/components/Component0393.ts
// Benchmark component #393 — identical logic to TodoPage, scoped class names.
...
import "./Component0393.css";

type Filter = "all" | "active" | "done";

export const Component0393 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0393] mounted");
    return () => console.log("[Component0393] unmounted");
  });
  ...
  return () => html`
    <div class="app-393">
      <header class="header-393">
        <h1>todos — Component0393</h1>
      </header>

      <main class="main-393">
        <input
          class="add-input-393 ${error() ? "add-input--error-393" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-393" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-393">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-393">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-393 ${filter() === f ? "filter-btn--active-393" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-393">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-393">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-393">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-393 ${todo.done ? "todo-item--done-393" : ""}">
                    <button
                      class="todo-check-393 ${todo.done ? "todo-check--done-393" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-393" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-393"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-393" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-393" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-393">
            <span class="todo-count-393">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-393" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
