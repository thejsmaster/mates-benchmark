// client/components/Component0781.ts
// Benchmark component #781 — identical logic to TodoPage, scoped class names.
...
import "./Component0781.css";

type Filter = "all" | "active" | "done";

export const Component0781 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0781] mounted");
    return () => console.log("[Component0781] unmounted");
  });
  ...
  return () => html`
    <div class="app-781">
      <header class="header-781">
        <h1>todos — Component0781</h1>
      </header>

      <main class="main-781">
        <input
          class="add-input-781 ${error() ? "add-input--error-781" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-781" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-781">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-781">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-781 ${filter() === f ? "filter-btn--active-781" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-781">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-781">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-781">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-781 ${todo.done ? "todo-item--done-781" : ""}">
                    <button
                      class="todo-check-781 ${todo.done ? "todo-check--done-781" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-781" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-781"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-781" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-781" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-781">
            <span class="todo-count-781">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-781" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
