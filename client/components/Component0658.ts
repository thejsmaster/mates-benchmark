// client/components/Component0658.ts
// Benchmark component #658 — identical logic to TodoPage, scoped class names.
...
import "./Component0658.css";

type Filter = "all" | "active" | "done";

export const Component0658 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0658] mounted");
    return () => console.log("[Component0658] unmounted");
  });
  ...
  return () => html`
    <div class="app-658">
      <header class="header-658">
        <h1>todos — Component0658</h1>
      </header>

      <main class="main-658">
        <input
          class="add-input-658 ${error() ? "add-input--error-658" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-658" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-658">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-658">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-658 ${filter() === f ? "filter-btn--active-658" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-658">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-658">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-658">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-658 ${todo.done ? "todo-item--done-658" : ""}">
                    <button
                      class="todo-check-658 ${todo.done ? "todo-check--done-658" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-658" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-658"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-658" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-658" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-658">
            <span class="todo-count-658">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-658" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
