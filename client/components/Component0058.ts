// client/components/Component0058.ts
// Benchmark component #58 — identical logic to TodoPage, scoped class names.
...
import "./Component0058.css";

type Filter = "all" | "active" | "done";

export const Component0058 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0058] mounted");
    return () => console.log("[Component0058] unmounted");
  });
  ...
  return () => html`
    <div class="app-58">
      <header class="header-58">
        <h1>todos — Component0058</h1>
      </header>

      <main class="main-58">
        <input
          class="add-input-58 ${error() ? "add-input--error-58" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-58" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-58">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-58">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-58 ${filter() === f ? "filter-btn--active-58" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-58">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-58">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-58">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-58 ${todo.done ? "todo-item--done-58" : ""}">
                    <button
                      class="todo-check-58 ${todo.done ? "todo-check--done-58" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-58" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-58"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-58" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-58" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-58">
            <span class="todo-count-58">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-58" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
