// client/components/Component0698.ts
// Benchmark component #698 — identical logic to TodoPage, scoped class names.
...
import "./Component0698.css";

type Filter = "all" | "active" | "done";

export const Component0698 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0698] mounted");
    return () => console.log("[Component0698] unmounted");
  });
  ...
  return () => html`
    <div class="app-698">
      <header class="header-698">
        <h1>todos — Component0698</h1>
      </header>

      <main class="main-698">
        <input
          class="add-input-698 ${error() ? "add-input--error-698" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-698" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-698">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-698">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-698 ${filter() === f ? "filter-btn--active-698" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-698">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-698">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-698">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-698 ${todo.done ? "todo-item--done-698" : ""}">
                    <button
                      class="todo-check-698 ${todo.done ? "todo-check--done-698" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-698" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-698"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-698" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-698" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-698">
            <span class="todo-count-698">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-698" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
