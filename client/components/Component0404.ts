// client/components/Component0404.ts
// Benchmark component #404 — identical logic to TodoPage, scoped class names.
...
import "./Component0404.css";

type Filter = "all" | "active" | "done";

export const Component0404 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0404] mounted");
    return () => console.log("[Component0404] unmounted");
  });
  ...
  return () => html`
    <div class="app-404">
      <header class="header-404">
        <h1>todos — Component0404</h1>
      </header>

      <main class="main-404">
        <input
          class="add-input-404 ${error() ? "add-input--error-404" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-404" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-404">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-404">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-404 ${filter() === f ? "filter-btn--active-404" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-404">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-404">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-404">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-404 ${todo.done ? "todo-item--done-404" : ""}">
                    <button
                      class="todo-check-404 ${todo.done ? "todo-check--done-404" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-404" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-404"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-404" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-404" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-404">
            <span class="todo-count-404">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-404" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
