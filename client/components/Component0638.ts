// client/components/Component0638.ts
// Benchmark component #638 — identical logic to TodoPage, scoped class names.
...
import "./Component0638.css";

type Filter = "all" | "active" | "done";

export const Component0638 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0638] mounted");
    return () => console.log("[Component0638] unmounted");
  });
  ...
  return () => html`
    <div class="app-638">
      <header class="header-638">
        <h1>todos — Component0638</h1>
      </header>

      <main class="main-638">
        <input
          class="add-input-638 ${error() ? "add-input--error-638" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-638" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-638">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-638">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-638 ${filter() === f ? "filter-btn--active-638" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-638">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-638">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-638">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-638 ${todo.done ? "todo-item--done-638" : ""}">
                    <button
                      class="todo-check-638 ${todo.done ? "todo-check--done-638" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-638" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-638"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-638" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-638" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-638">
            <span class="todo-count-638">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-638" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
