// client/components/Component0132.ts
// Benchmark component #132 — identical logic to TodoPage, scoped class names.
...
import "./Component0132.css";

type Filter = "all" | "active" | "done";

export const Component0132 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0132] mounted");
    return () => console.log("[Component0132] unmounted");
  });
  ...
  return () => html`
    <div class="app-132">
      <header class="header-132">
        <h1>todos — Component0132</h1>
      </header>

      <main class="main-132">
        <input
          class="add-input-132 ${error() ? "add-input--error-132" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-132" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-132">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-132">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-132 ${filter() === f ? "filter-btn--active-132" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-132">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-132">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-132">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-132 ${todo.done ? "todo-item--done-132" : ""}">
                    <button
                      class="todo-check-132 ${todo.done ? "todo-check--done-132" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-132" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-132"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-132" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-132" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-132">
            <span class="todo-count-132">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-132" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
