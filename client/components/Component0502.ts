// client/components/Component0502.ts
// Benchmark component #502 — identical logic to TodoPage, scoped class names.
...
import "./Component0502.css";

type Filter = "all" | "active" | "done";

export const Component0502 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0502] mounted");
    return () => console.log("[Component0502] unmounted");
  });
  ...
  return () => html`
    <div class="app-502">
      <header class="header-502">
        <h1>todos — Component0502</h1>
      </header>

      <main class="main-502">
        <input
          class="add-input-502 ${error() ? "add-input--error-502" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-502" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-502">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-502">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-502 ${filter() === f ? "filter-btn--active-502" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-502">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-502">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-502">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-502 ${todo.done ? "todo-item--done-502" : ""}">
                    <button
                      class="todo-check-502 ${todo.done ? "todo-check--done-502" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-502" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-502"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-502" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-502" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-502">
            <span class="todo-count-502">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-502" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
