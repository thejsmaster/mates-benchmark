// client/components/Component0996.ts
// Benchmark component #996 — identical logic to TodoPage, scoped class names.
...
import "./Component0996.css";

type Filter = "all" | "active" | "done";

export const Component0996 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0996] mounted");
    return () => console.log("[Component0996] unmounted");
  });
  ...
  return () => html`
    <div class="app-996">
      <header class="header-996">
        <h1>todos — Component0996</h1>
      </header>

      <main class="main-996">
        <input
          class="add-input-996 ${error() ? "add-input--error-996" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-996" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-996">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-996">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-996 ${filter() === f ? "filter-btn--active-996" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-996">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-996">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-996">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-996 ${todo.done ? "todo-item--done-996" : ""}">
                    <button
                      class="todo-check-996 ${todo.done ? "todo-check--done-996" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-996" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-996"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-996" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-996" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-996">
            <span class="todo-count-996">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-996" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
