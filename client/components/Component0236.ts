// client/components/Component0236.ts
// Benchmark component #236 — identical logic to TodoPage, scoped class names.
...
import "./Component0236.css";

type Filter = "all" | "active" | "done";

export const Component0236 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0236] mounted");
    return () => console.log("[Component0236] unmounted");
  });
  ...
  return () => html`
    <div class="app-236">
      <header class="header-236">
        <h1>todos — Component0236</h1>
      </header>

      <main class="main-236">
        <input
          class="add-input-236 ${error() ? "add-input--error-236" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-236" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-236">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-236">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-236 ${filter() === f ? "filter-btn--active-236" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-236">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-236">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-236">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-236 ${todo.done ? "todo-item--done-236" : ""}">
                    <button
                      class="todo-check-236 ${todo.done ? "todo-check--done-236" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-236" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-236"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-236" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-236" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-236">
            <span class="todo-count-236">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-236" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
