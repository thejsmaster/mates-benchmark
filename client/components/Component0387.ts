// client/components/Component0387.ts
// Benchmark component #387 — identical logic to TodoPage, scoped class names.
...
import "./Component0387.css";

type Filter = "all" | "active" | "done";

export const Component0387 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0387] mounted");
    return () => console.log("[Component0387] unmounted");
  });
  ...
  return () => html`
    <div class="app-387">
      <header class="header-387">
        <h1>todos — Component0387</h1>
      </header>

      <main class="main-387">
        <input
          class="add-input-387 ${error() ? "add-input--error-387" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-387" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-387">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-387">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-387 ${filter() === f ? "filter-btn--active-387" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-387">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-387">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-387">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-387 ${todo.done ? "todo-item--done-387" : ""}">
                    <button
                      class="todo-check-387 ${todo.done ? "todo-check--done-387" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-387" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-387"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-387" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-387" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-387">
            <span class="todo-count-387">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-387" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
