// client/components/Component0294.ts
// Benchmark component #294 — identical logic to TodoPage, scoped class names.
...
import "./Component0294.css";

type Filter = "all" | "active" | "done";

export const Component0294 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0294] mounted");
    return () => console.log("[Component0294] unmounted");
  });
  ...
  return () => html`
    <div class="app-294">
      <header class="header-294">
        <h1>todos — Component0294</h1>
      </header>

      <main class="main-294">
        <input
          class="add-input-294 ${error() ? "add-input--error-294" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-294" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-294">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-294">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-294 ${filter() === f ? "filter-btn--active-294" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-294">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-294">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-294">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-294 ${todo.done ? "todo-item--done-294" : ""}">
                    <button
                      class="todo-check-294 ${todo.done ? "todo-check--done-294" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-294" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-294"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-294" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-294" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-294">
            <span class="todo-count-294">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-294" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
