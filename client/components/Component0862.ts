// client/components/Component0862.ts
// Benchmark component #862 — identical logic to TodoPage, scoped class names.
...
import "./Component0862.css";

type Filter = "all" | "active" | "done";

export const Component0862 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0862] mounted");
    return () => console.log("[Component0862] unmounted");
  });
  ...
  return () => html`
    <div class="app-862">
      <header class="header-862">
        <h1>todos — Component0862</h1>
      </header>

      <main class="main-862">
        <input
          class="add-input-862 ${error() ? "add-input--error-862" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-862" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-862">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-862">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-862 ${filter() === f ? "filter-btn--active-862" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-862">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-862">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-862">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-862 ${todo.done ? "todo-item--done-862" : ""}">
                    <button
                      class="todo-check-862 ${todo.done ? "todo-check--done-862" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-862" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-862"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-862" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-862" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-862">
            <span class="todo-count-862">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-862" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
