// client/components/Component0497.ts
// Benchmark component #497 — identical logic to TodoPage, scoped class names.
...
import "./Component0497.css";

type Filter = "all" | "active" | "done";

export const Component0497 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0497] mounted");
    return () => console.log("[Component0497] unmounted");
  });
  ...
  return () => html`
    <div class="app-497">
      <header class="header-497">
        <h1>todos — Component0497</h1>
      </header>

      <main class="main-497">
        <input
          class="add-input-497 ${error() ? "add-input--error-497" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-497" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-497">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-497">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-497 ${filter() === f ? "filter-btn--active-497" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-497">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-497">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-497">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-497 ${todo.done ? "todo-item--done-497" : ""}">
                    <button
                      class="todo-check-497 ${todo.done ? "todo-check--done-497" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-497" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-497"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-497" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-497" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-497">
            <span class="todo-count-497">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-497" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
