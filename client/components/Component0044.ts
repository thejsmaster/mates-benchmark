// client/components/Component0044.ts
// Benchmark component #44 — identical logic to TodoPage, scoped class names.
...
import "./Component0044.css";

type Filter = "all" | "active" | "done";

export const Component0044 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0044] mounted");
    return () => console.log("[Component0044] unmounted");
  });
  ...
  return () => html`
    <div class="app-44">
      <header class="header-44">
        <h1>todos — Component0044</h1>
      </header>

      <main class="main-44">
        <input
          class="add-input-44 ${error() ? "add-input--error-44" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-44" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-44">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-44">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-44 ${filter() === f ? "filter-btn--active-44" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-44">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-44">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-44">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-44 ${todo.done ? "todo-item--done-44" : ""}">
                    <button
                      class="todo-check-44 ${todo.done ? "todo-check--done-44" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-44" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-44"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-44" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-44" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-44">
            <span class="todo-count-44">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-44" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
