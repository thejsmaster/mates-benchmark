// client/components/Component0779.ts
// Benchmark component #779 — identical logic to TodoPage, scoped class names.
...
import "./Component0779.css";

type Filter = "all" | "active" | "done";

export const Component0779 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0779] mounted");
    return () => console.log("[Component0779] unmounted");
  });
  ...
  return () => html`
    <div class="app-779">
      <header class="header-779">
        <h1>todos — Component0779</h1>
      </header>

      <main class="main-779">
        <input
          class="add-input-779 ${error() ? "add-input--error-779" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-779" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-779">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-779">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-779 ${filter() === f ? "filter-btn--active-779" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-779">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-779">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-779">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-779 ${todo.done ? "todo-item--done-779" : ""}">
                    <button
                      class="todo-check-779 ${todo.done ? "todo-check--done-779" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-779" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-779"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-779" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-779" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-779">
            <span class="todo-count-779">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-779" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
