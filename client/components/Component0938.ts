// client/components/Component0938.ts
// Benchmark component #938 — identical logic to TodoPage, scoped class names.
...
import "./Component0938.css";

type Filter = "all" | "active" | "done";

export const Component0938 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0938] mounted");
    return () => console.log("[Component0938] unmounted");
  });
  ...
  return () => html`
    <div class="app-938">
      <header class="header-938">
        <h1>todos — Component0938</h1>
      </header>

      <main class="main-938">
        <input
          class="add-input-938 ${error() ? "add-input--error-938" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-938" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-938">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-938">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-938 ${filter() === f ? "filter-btn--active-938" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-938">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-938">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-938">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-938 ${todo.done ? "todo-item--done-938" : ""}">
                    <button
                      class="todo-check-938 ${todo.done ? "todo-check--done-938" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-938" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-938"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-938" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-938" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-938">
            <span class="todo-count-938">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-938" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
