// client/components/Component0530.ts
// Benchmark component #530 — identical logic to TodoPage, scoped class names.
...
import "./Component0530.css";

type Filter = "all" | "active" | "done";

export const Component0530 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0530] mounted");
    return () => console.log("[Component0530] unmounted");
  });
  ...
  return () => html`
    <div class="app-530">
      <header class="header-530">
        <h1>todos — Component0530</h1>
      </header>

      <main class="main-530">
        <input
          class="add-input-530 ${error() ? "add-input--error-530" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-530" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-530">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-530">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-530 ${filter() === f ? "filter-btn--active-530" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-530">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-530">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-530">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-530 ${todo.done ? "todo-item--done-530" : ""}">
                    <button
                      class="todo-check-530 ${todo.done ? "todo-check--done-530" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-530" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-530"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-530" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-530" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-530">
            <span class="todo-count-530">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-530" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
