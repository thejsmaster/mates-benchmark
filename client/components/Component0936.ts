// client/components/Component0936.ts
// Benchmark component #936 — identical logic to TodoPage, scoped class names.
...
import "./Component0936.css";

type Filter = "all" | "active" | "done";

export const Component0936 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0936] mounted");
    return () => console.log("[Component0936] unmounted");
  });
  ...
  return () => html`
    <div class="app-936">
      <header class="header-936">
        <h1>todos — Component0936</h1>
      </header>

      <main class="main-936">
        <input
          class="add-input-936 ${error() ? "add-input--error-936" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-936" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-936">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-936">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-936 ${filter() === f ? "filter-btn--active-936" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-936">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-936">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-936">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-936 ${todo.done ? "todo-item--done-936" : ""}">
                    <button
                      class="todo-check-936 ${todo.done ? "todo-check--done-936" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-936" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-936"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-936" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-936" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-936">
            <span class="todo-count-936">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-936" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
