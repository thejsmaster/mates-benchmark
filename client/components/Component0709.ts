// client/components/Component0709.ts
// Benchmark component #709 — identical logic to TodoPage, scoped class names.
...
import "./Component0709.css";

type Filter = "all" | "active" | "done";

export const Component0709 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0709] mounted");
    return () => console.log("[Component0709] unmounted");
  });
  ...
  return () => html`
    <div class="app-709">
      <header class="header-709">
        <h1>todos — Component0709</h1>
      </header>

      <main class="main-709">
        <input
          class="add-input-709 ${error() ? "add-input--error-709" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-709" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-709">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-709">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-709 ${filter() === f ? "filter-btn--active-709" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-709">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-709">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-709">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-709 ${todo.done ? "todo-item--done-709" : ""}">
                    <button
                      class="todo-check-709 ${todo.done ? "todo-check--done-709" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-709" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-709"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-709" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-709" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-709">
            <span class="todo-count-709">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-709" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
