// client/components/Component0599.ts
// Benchmark component #599 — identical logic to TodoPage, scoped class names.
...
import "./Component0599.css";

type Filter = "all" | "active" | "done";

export const Component0599 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0599] mounted");
    return () => console.log("[Component0599] unmounted");
  });
  ...
  return () => html`
    <div class="app-599">
      <header class="header-599">
        <h1>todos — Component0599</h1>
      </header>

      <main class="main-599">
        <input
          class="add-input-599 ${error() ? "add-input--error-599" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-599" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-599">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-599">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-599 ${filter() === f ? "filter-btn--active-599" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-599">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-599">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-599">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-599 ${todo.done ? "todo-item--done-599" : ""}">
                    <button
                      class="todo-check-599 ${todo.done ? "todo-check--done-599" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-599" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-599"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-599" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-599" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-599">
            <span class="todo-count-599">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-599" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
