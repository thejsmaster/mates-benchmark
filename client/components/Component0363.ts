// client/components/Component0363.ts
// Benchmark component #363 — identical logic to TodoPage, scoped class names.
...
import "./Component0363.css";

type Filter = "all" | "active" | "done";

export const Component0363 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0363] mounted");
    return () => console.log("[Component0363] unmounted");
  });
  ...
  return () => html`
    <div class="app-363">
      <header class="header-363">
        <h1>todos — Component0363</h1>
      </header>

      <main class="main-363">
        <input
          class="add-input-363 ${error() ? "add-input--error-363" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-363" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-363">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-363">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-363 ${filter() === f ? "filter-btn--active-363" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-363">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-363">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-363">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-363 ${todo.done ? "todo-item--done-363" : ""}">
                    <button
                      class="todo-check-363 ${todo.done ? "todo-check--done-363" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-363" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-363"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-363" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-363" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-363">
            <span class="todo-count-363">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-363" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
