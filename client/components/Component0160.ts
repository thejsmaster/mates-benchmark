// client/components/Component0160.ts
// Benchmark component #160 — identical logic to TodoPage, scoped class names.
...
import "./Component0160.css";

type Filter = "all" | "active" | "done";

export const Component0160 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0160] mounted");
    return () => console.log("[Component0160] unmounted");
  });
  ...
  return () => html`
    <div class="app-160">
      <header class="header-160">
        <h1>todos — Component0160</h1>
      </header>

      <main class="main-160">
        <input
          class="add-input-160 ${error() ? "add-input--error-160" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-160" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-160">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-160">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-160 ${filter() === f ? "filter-btn--active-160" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-160">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-160">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-160">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-160 ${todo.done ? "todo-item--done-160" : ""}">
                    <button
                      class="todo-check-160 ${todo.done ? "todo-check--done-160" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-160" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-160"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-160" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-160" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-160">
            <span class="todo-count-160">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-160" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
