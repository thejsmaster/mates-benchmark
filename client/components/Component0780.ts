// client/components/Component0780.ts
// Benchmark component #780 — identical logic to TodoPage, scoped class names.
...
import "./Component0780.css";

type Filter = "all" | "active" | "done";

export const Component0780 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0780] mounted");
    return () => console.log("[Component0780] unmounted");
  });
  ...
  return () => html`
    <div class="app-780">
      <header class="header-780">
        <h1>todos — Component0780</h1>
      </header>

      <main class="main-780">
        <input
          class="add-input-780 ${error() ? "add-input--error-780" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-780" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-780">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-780">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-780 ${filter() === f ? "filter-btn--active-780" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-780">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-780">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-780">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-780 ${todo.done ? "todo-item--done-780" : ""}">
                    <button
                      class="todo-check-780 ${todo.done ? "todo-check--done-780" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-780" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-780"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-780" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-780" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-780">
            <span class="todo-count-780">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-780" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
