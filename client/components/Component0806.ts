// client/components/Component0806.ts
// Benchmark component #806 — identical logic to TodoPage, scoped class names.
...
import "./Component0806.css";

type Filter = "all" | "active" | "done";

export const Component0806 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0806] mounted");
    return () => console.log("[Component0806] unmounted");
  });
  ...
  return () => html`
    <div class="app-806">
      <header class="header-806">
        <h1>todos — Component0806</h1>
      </header>

      <main class="main-806">
        <input
          class="add-input-806 ${error() ? "add-input--error-806" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-806" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-806">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-806">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-806 ${filter() === f ? "filter-btn--active-806" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-806">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-806">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-806">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-806 ${todo.done ? "todo-item--done-806" : ""}">
                    <button
                      class="todo-check-806 ${todo.done ? "todo-check--done-806" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-806" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-806"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-806" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-806" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-806">
            <span class="todo-count-806">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-806" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
