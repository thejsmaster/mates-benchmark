// client/components/Component0833.ts
// Benchmark component #833 — identical logic to TodoPage, scoped class names.
...
import "./Component0833.css";

type Filter = "all" | "active" | "done";

export const Component0833 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0833] mounted");
    return () => console.log("[Component0833] unmounted");
  });
  ...
  return () => html`
    <div class="app-833">
      <header class="header-833">
        <h1>todos — Component0833</h1>
      </header>

      <main class="main-833">
        <input
          class="add-input-833 ${error() ? "add-input--error-833" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-833" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-833">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-833">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-833 ${filter() === f ? "filter-btn--active-833" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-833">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-833">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-833">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-833 ${todo.done ? "todo-item--done-833" : ""}">
                    <button
                      class="todo-check-833 ${todo.done ? "todo-check--done-833" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-833" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-833"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-833" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-833" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-833">
            <span class="todo-count-833">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-833" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
