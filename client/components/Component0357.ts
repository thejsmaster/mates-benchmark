// client/components/Component0357.ts
// Benchmark component #357 — identical logic to TodoPage, scoped class names.
...
import "./Component0357.css";

type Filter = "all" | "active" | "done";

export const Component0357 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0357] mounted");
    return () => console.log("[Component0357] unmounted");
  });
  ...
  return () => html`
    <div class="app-357">
      <header class="header-357">
        <h1>todos — Component0357</h1>
      </header>

      <main class="main-357">
        <input
          class="add-input-357 ${error() ? "add-input--error-357" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-357" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-357">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-357">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-357 ${filter() === f ? "filter-btn--active-357" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-357">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-357">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-357">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-357 ${todo.done ? "todo-item--done-357" : ""}">
                    <button
                      class="todo-check-357 ${todo.done ? "todo-check--done-357" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-357" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-357"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-357" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-357" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-357">
            <span class="todo-count-357">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-357" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
