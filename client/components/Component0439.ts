// client/components/Component0439.ts
// Benchmark component #439 — identical logic to TodoPage, scoped class names.
...
import "./Component0439.css";

type Filter = "all" | "active" | "done";

export const Component0439 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0439] mounted");
    return () => console.log("[Component0439] unmounted");
  });
  ...
  return () => html`
    <div class="app-439">
      <header class="header-439">
        <h1>todos — Component0439</h1>
      </header>

      <main class="main-439">
        <input
          class="add-input-439 ${error() ? "add-input--error-439" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-439" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-439">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-439">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-439 ${filter() === f ? "filter-btn--active-439" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-439">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-439">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-439">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-439 ${todo.done ? "todo-item--done-439" : ""}">
                    <button
                      class="todo-check-439 ${todo.done ? "todo-check--done-439" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-439" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-439"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-439" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-439" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-439">
            <span class="todo-count-439">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-439" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
