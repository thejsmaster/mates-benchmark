// client/components/Component0697.ts
// Benchmark component #697 — identical logic to TodoPage, scoped class names.
...
import "./Component0697.css";

type Filter = "all" | "active" | "done";

export const Component0697 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0697] mounted");
    return () => console.log("[Component0697] unmounted");
  });
  ...
  return () => html`
    <div class="app-697">
      <header class="header-697">
        <h1>todos — Component0697</h1>
      </header>

      <main class="main-697">
        <input
          class="add-input-697 ${error() ? "add-input--error-697" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-697" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-697">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-697">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-697 ${filter() === f ? "filter-btn--active-697" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-697">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-697">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-697">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-697 ${todo.done ? "todo-item--done-697" : ""}">
                    <button
                      class="todo-check-697 ${todo.done ? "todo-check--done-697" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-697" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-697"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-697" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-697" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-697">
            <span class="todo-count-697">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-697" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
