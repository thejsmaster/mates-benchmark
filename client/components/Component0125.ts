// client/components/Component0125.ts
// Benchmark component #125 — identical logic to TodoPage, scoped class names.
...
import "./Component0125.css";

type Filter = "all" | "active" | "done";

export const Component0125 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0125] mounted");
    return () => console.log("[Component0125] unmounted");
  });
  ...
  return () => html`
    <div class="app-125">
      <header class="header-125">
        <h1>todos — Component0125</h1>
      </header>

      <main class="main-125">
        <input
          class="add-input-125 ${error() ? "add-input--error-125" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-125" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-125">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-125">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-125 ${filter() === f ? "filter-btn--active-125" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-125">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-125">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-125">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-125 ${todo.done ? "todo-item--done-125" : ""}">
                    <button
                      class="todo-check-125 ${todo.done ? "todo-check--done-125" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-125" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-125"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-125" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-125" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-125">
            <span class="todo-count-125">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-125" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
