// client/components/Component0744.ts
// Benchmark component #744 — identical logic to TodoPage, scoped class names.
...
import "./Component0744.css";

type Filter = "all" | "active" | "done";

export const Component0744 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0744] mounted");
    return () => console.log("[Component0744] unmounted");
  });
  ...
  return () => html`
    <div class="app-744">
      <header class="header-744">
        <h1>todos — Component0744</h1>
      </header>

      <main class="main-744">
        <input
          class="add-input-744 ${error() ? "add-input--error-744" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-744" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-744">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-744">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-744 ${filter() === f ? "filter-btn--active-744" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-744">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-744">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-744">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-744 ${todo.done ? "todo-item--done-744" : ""}">
                    <button
                      class="todo-check-744 ${todo.done ? "todo-check--done-744" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-744" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-744"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-744" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-744" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-744">
            <span class="todo-count-744">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-744" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
