// client/components/Component0153.ts
// Benchmark component #153 — identical logic to TodoPage, scoped class names.
...
import "./Component0153.css";

type Filter = "all" | "active" | "done";

export const Component0153 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0153] mounted");
    return () => console.log("[Component0153] unmounted");
  });
  ...
  return () => html`
    <div class="app-153">
      <header class="header-153">
        <h1>todos — Component0153</h1>
      </header>

      <main class="main-153">
        <input
          class="add-input-153 ${error() ? "add-input--error-153" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-153" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-153">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-153">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-153 ${filter() === f ? "filter-btn--active-153" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-153">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-153">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-153">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-153 ${todo.done ? "todo-item--done-153" : ""}">
                    <button
                      class="todo-check-153 ${todo.done ? "todo-check--done-153" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-153" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-153"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-153" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-153" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-153">
            <span class="todo-count-153">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-153" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
