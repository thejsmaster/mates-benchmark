// client/components/Component0451.ts
// Benchmark component #451 — identical logic to TodoPage, scoped class names.
...
import "./Component0451.css";

type Filter = "all" | "active" | "done";

export const Component0451 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0451] mounted");
    return () => console.log("[Component0451] unmounted");
  });
  ...
  return () => html`
    <div class="app-451">
      <header class="header-451">
        <h1>todos — Component0451</h1>
      </header>

      <main class="main-451">
        <input
          class="add-input-451 ${error() ? "add-input--error-451" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-451" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-451">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-451">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-451 ${filter() === f ? "filter-btn--active-451" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-451">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-451">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-451">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-451 ${todo.done ? "todo-item--done-451" : ""}">
                    <button
                      class="todo-check-451 ${todo.done ? "todo-check--done-451" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-451" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-451"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-451" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-451" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-451">
            <span class="todo-count-451">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-451" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
