// client/components/Component0382.ts
// Benchmark component #382 — identical logic to TodoPage, scoped class names.
...
import "./Component0382.css";

type Filter = "all" | "active" | "done";

export const Component0382 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0382] mounted");
    return () => console.log("[Component0382] unmounted");
  });
  ...
  return () => html`
    <div class="app-382">
      <header class="header-382">
        <h1>todos — Component0382</h1>
      </header>

      <main class="main-382">
        <input
          class="add-input-382 ${error() ? "add-input--error-382" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-382" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-382">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-382">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-382 ${filter() === f ? "filter-btn--active-382" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-382">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-382">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-382">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-382 ${todo.done ? "todo-item--done-382" : ""}">
                    <button
                      class="todo-check-382 ${todo.done ? "todo-check--done-382" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-382" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-382"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-382" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-382" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-382">
            <span class="todo-count-382">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-382" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
