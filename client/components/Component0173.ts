// client/components/Component0173.ts
// Benchmark component #173 — identical logic to TodoPage, scoped class names.
...
import "./Component0173.css";

type Filter = "all" | "active" | "done";

export const Component0173 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0173] mounted");
    return () => console.log("[Component0173] unmounted");
  });
  ...
  return () => html`
    <div class="app-173">
      <header class="header-173">
        <h1>todos — Component0173</h1>
      </header>

      <main class="main-173">
        <input
          class="add-input-173 ${error() ? "add-input--error-173" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-173" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-173">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-173">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-173 ${filter() === f ? "filter-btn--active-173" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-173">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-173">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-173">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-173 ${todo.done ? "todo-item--done-173" : ""}">
                    <button
                      class="todo-check-173 ${todo.done ? "todo-check--done-173" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-173" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-173"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-173" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-173" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-173">
            <span class="todo-count-173">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-173" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
