// client/components/Component0922.ts
// Benchmark component #922 — identical logic to TodoPage, scoped class names.
...
import "./Component0922.css";

type Filter = "all" | "active" | "done";

export const Component0922 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0922] mounted");
    return () => console.log("[Component0922] unmounted");
  });
  ...
  return () => html`
    <div class="app-922">
      <header class="header-922">
        <h1>todos — Component0922</h1>
      </header>

      <main class="main-922">
        <input
          class="add-input-922 ${error() ? "add-input--error-922" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-922" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-922">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-922">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-922 ${filter() === f ? "filter-btn--active-922" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-922">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-922">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-922">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-922 ${todo.done ? "todo-item--done-922" : ""}">
                    <button
                      class="todo-check-922 ${todo.done ? "todo-check--done-922" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-922" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-922"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-922" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-922" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-922">
            <span class="todo-count-922">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-922" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
