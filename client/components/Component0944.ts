// client/components/Component0944.ts
// Benchmark component #944 — identical logic to TodoPage, scoped class names.
...
import "./Component0944.css";

type Filter = "all" | "active" | "done";

export const Component0944 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0944] mounted");
    return () => console.log("[Component0944] unmounted");
  });
  ...
  return () => html`
    <div class="app-944">
      <header class="header-944">
        <h1>todos — Component0944</h1>
      </header>

      <main class="main-944">
        <input
          class="add-input-944 ${error() ? "add-input--error-944" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-944" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-944">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-944">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-944 ${filter() === f ? "filter-btn--active-944" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-944">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-944">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-944">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-944 ${todo.done ? "todo-item--done-944" : ""}">
                    <button
                      class="todo-check-944 ${todo.done ? "todo-check--done-944" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-944" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-944"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-944" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-944" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-944">
            <span class="todo-count-944">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-944" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
