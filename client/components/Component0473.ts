// client/components/Component0473.ts
// Benchmark component #473 — identical logic to TodoPage, scoped class names.
...
import "./Component0473.css";

type Filter = "all" | "active" | "done";

export const Component0473 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0473] mounted");
    return () => console.log("[Component0473] unmounted");
  });
  ...
  return () => html`
    <div class="app-473">
      <header class="header-473">
        <h1>todos — Component0473</h1>
      </header>

      <main class="main-473">
        <input
          class="add-input-473 ${error() ? "add-input--error-473" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-473" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-473">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-473">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-473 ${filter() === f ? "filter-btn--active-473" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-473">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-473">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-473">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-473 ${todo.done ? "todo-item--done-473" : ""}">
                    <button
                      class="todo-check-473 ${todo.done ? "todo-check--done-473" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-473" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-473"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-473" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-473" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-473">
            <span class="todo-count-473">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-473" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
