// client/components/Component0677.ts
// Benchmark component #677 — identical logic to TodoPage, scoped class names.
...
import "./Component0677.css";

type Filter = "all" | "active" | "done";

export const Component0677 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0677] mounted");
    return () => console.log("[Component0677] unmounted");
  });
  ...
  return () => html`
    <div class="app-677">
      <header class="header-677">
        <h1>todos — Component0677</h1>
      </header>

      <main class="main-677">
        <input
          class="add-input-677 ${error() ? "add-input--error-677" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-677" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-677">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-677">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-677 ${filter() === f ? "filter-btn--active-677" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-677">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-677">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-677">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-677 ${todo.done ? "todo-item--done-677" : ""}">
                    <button
                      class="todo-check-677 ${todo.done ? "todo-check--done-677" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-677" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-677"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-677" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-677" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-677">
            <span class="todo-count-677">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-677" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
