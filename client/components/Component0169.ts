// client/components/Component0169.ts
// Benchmark component #169 — identical logic to TodoPage, scoped class names.
...
import "./Component0169.css";

type Filter = "all" | "active" | "done";

export const Component0169 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0169] mounted");
    return () => console.log("[Component0169] unmounted");
  });
  ...
  return () => html`
    <div class="app-169">
      <header class="header-169">
        <h1>todos — Component0169</h1>
      </header>

      <main class="main-169">
        <input
          class="add-input-169 ${error() ? "add-input--error-169" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-169" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-169">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-169">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-169 ${filter() === f ? "filter-btn--active-169" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-169">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-169">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-169">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-169 ${todo.done ? "todo-item--done-169" : ""}">
                    <button
                      class="todo-check-169 ${todo.done ? "todo-check--done-169" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-169" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-169"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-169" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-169" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-169">
            <span class="todo-count-169">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-169" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
