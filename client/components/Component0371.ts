// client/components/Component0371.ts
// Benchmark component #371 — identical logic to TodoPage, scoped class names.
...
import "./Component0371.css";

type Filter = "all" | "active" | "done";

export const Component0371 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0371] mounted");
    return () => console.log("[Component0371] unmounted");
  });
  ...
  return () => html`
    <div class="app-371">
      <header class="header-371">
        <h1>todos — Component0371</h1>
      </header>

      <main class="main-371">
        <input
          class="add-input-371 ${error() ? "add-input--error-371" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-371" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-371">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-371">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-371 ${filter() === f ? "filter-btn--active-371" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-371">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-371">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-371">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-371 ${todo.done ? "todo-item--done-371" : ""}">
                    <button
                      class="todo-check-371 ${todo.done ? "todo-check--done-371" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-371" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-371"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-371" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-371" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-371">
            <span class="todo-count-371">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-371" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
