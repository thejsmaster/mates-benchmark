// client/components/Component0122.ts
// Benchmark component #122 — identical logic to TodoPage, scoped class names.
...
import "./Component0122.css";

type Filter = "all" | "active" | "done";

export const Component0122 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0122] mounted");
    return () => console.log("[Component0122] unmounted");
  });
  ...
  return () => html`
    <div class="app-122">
      <header class="header-122">
        <h1>todos — Component0122</h1>
      </header>

      <main class="main-122">
        <input
          class="add-input-122 ${error() ? "add-input--error-122" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-122" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-122">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-122">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-122 ${filter() === f ? "filter-btn--active-122" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-122">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-122">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-122">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-122 ${todo.done ? "todo-item--done-122" : ""}">
                    <button
                      class="todo-check-122 ${todo.done ? "todo-check--done-122" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-122" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-122"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-122" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-122" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-122">
            <span class="todo-count-122">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-122" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
