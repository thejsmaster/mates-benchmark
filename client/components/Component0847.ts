// client/components/Component0847.ts
// Benchmark component #847 — identical logic to TodoPage, scoped class names.
...
import "./Component0847.css";

type Filter = "all" | "active" | "done";

export const Component0847 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0847] mounted");
    return () => console.log("[Component0847] unmounted");
  });
  ...
  return () => html`
    <div class="app-847">
      <header class="header-847">
        <h1>todos — Component0847</h1>
      </header>

      <main class="main-847">
        <input
          class="add-input-847 ${error() ? "add-input--error-847" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-847" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-847">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-847">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-847 ${filter() === f ? "filter-btn--active-847" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-847">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-847">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-847">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-847 ${todo.done ? "todo-item--done-847" : ""}">
                    <button
                      class="todo-check-847 ${todo.done ? "todo-check--done-847" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-847" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-847"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-847" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-847" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-847">
            <span class="todo-count-847">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-847" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
