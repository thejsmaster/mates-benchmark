// client/components/Component0493.ts
// Benchmark component #493 — identical logic to TodoPage, scoped class names.
...
import "./Component0493.css";

type Filter = "all" | "active" | "done";

export const Component0493 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0493] mounted");
    return () => console.log("[Component0493] unmounted");
  });
  ...
  return () => html`
    <div class="app-493">
      <header class="header-493">
        <h1>todos — Component0493</h1>
      </header>

      <main class="main-493">
        <input
          class="add-input-493 ${error() ? "add-input--error-493" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-493" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-493">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-493">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-493 ${filter() === f ? "filter-btn--active-493" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-493">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-493">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-493">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-493 ${todo.done ? "todo-item--done-493" : ""}">
                    <button
                      class="todo-check-493 ${todo.done ? "todo-check--done-493" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-493" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-493"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-493" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-493" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-493">
            <span class="todo-count-493">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-493" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
