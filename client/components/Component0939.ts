// client/components/Component0939.ts
// Benchmark component #939 — identical logic to TodoPage, scoped class names.
...
import "./Component0939.css";

type Filter = "all" | "active" | "done";

export const Component0939 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0939] mounted");
    return () => console.log("[Component0939] unmounted");
  });
  ...
  return () => html`
    <div class="app-939">
      <header class="header-939">
        <h1>todos — Component0939</h1>
      </header>

      <main class="main-939">
        <input
          class="add-input-939 ${error() ? "add-input--error-939" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-939" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-939">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-939">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-939 ${filter() === f ? "filter-btn--active-939" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-939">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-939">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-939">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-939 ${todo.done ? "todo-item--done-939" : ""}">
                    <button
                      class="todo-check-939 ${todo.done ? "todo-check--done-939" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-939" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-939"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-939" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-939" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-939">
            <span class="todo-count-939">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-939" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
