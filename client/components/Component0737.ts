// client/components/Component0737.ts
// Benchmark component #737 — identical logic to TodoPage, scoped class names.
...
import "./Component0737.css";

type Filter = "all" | "active" | "done";

export const Component0737 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0737] mounted");
    return () => console.log("[Component0737] unmounted");
  });
  ...
  return () => html`
    <div class="app-737">
      <header class="header-737">
        <h1>todos — Component0737</h1>
      </header>

      <main class="main-737">
        <input
          class="add-input-737 ${error() ? "add-input--error-737" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-737" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-737">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-737">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-737 ${filter() === f ? "filter-btn--active-737" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-737">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-737">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-737">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-737 ${todo.done ? "todo-item--done-737" : ""}">
                    <button
                      class="todo-check-737 ${todo.done ? "todo-check--done-737" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-737" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-737"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-737" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-737" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-737">
            <span class="todo-count-737">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-737" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
