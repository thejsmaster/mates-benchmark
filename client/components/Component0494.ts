// client/components/Component0494.ts
// Benchmark component #494 — identical logic to TodoPage, scoped class names.
...
import "./Component0494.css";

type Filter = "all" | "active" | "done";

export const Component0494 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0494] mounted");
    return () => console.log("[Component0494] unmounted");
  });
  ...
  return () => html`
    <div class="app-494">
      <header class="header-494">
        <h1>todos — Component0494</h1>
      </header>

      <main class="main-494">
        <input
          class="add-input-494 ${error() ? "add-input--error-494" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-494" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-494">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-494">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-494 ${filter() === f ? "filter-btn--active-494" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-494">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-494">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-494">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-494 ${todo.done ? "todo-item--done-494" : ""}">
                    <button
                      class="todo-check-494 ${todo.done ? "todo-check--done-494" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-494" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-494"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-494" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-494" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-494">
            <span class="todo-count-494">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-494" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
