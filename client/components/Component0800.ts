// client/components/Component0800.ts
// Benchmark component #800 — identical logic to TodoPage, scoped class names.
...
import "./Component0800.css";

type Filter = "all" | "active" | "done";

export const Component0800 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0800] mounted");
    return () => console.log("[Component0800] unmounted");
  });
  ...
  return () => html`
    <div class="app-800">
      <header class="header-800">
        <h1>todos — Component0800</h1>
      </header>

      <main class="main-800">
        <input
          class="add-input-800 ${error() ? "add-input--error-800" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-800" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-800">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-800">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-800 ${filter() === f ? "filter-btn--active-800" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-800">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-800">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-800">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-800 ${todo.done ? "todo-item--done-800" : ""}">
                    <button
                      class="todo-check-800 ${todo.done ? "todo-check--done-800" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-800" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-800"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-800" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-800" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-800">
            <span class="todo-count-800">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-800" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
