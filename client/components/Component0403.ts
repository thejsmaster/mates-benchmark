// client/components/Component0403.ts
// Benchmark component #403 — identical logic to TodoPage, scoped class names.
...
import "./Component0403.css";

type Filter = "all" | "active" | "done";

export const Component0403 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0403] mounted");
    return () => console.log("[Component0403] unmounted");
  });
  ...
  return () => html`
    <div class="app-403">
      <header class="header-403">
        <h1>todos — Component0403</h1>
      </header>

      <main class="main-403">
        <input
          class="add-input-403 ${error() ? "add-input--error-403" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-403" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-403">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-403">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-403 ${filter() === f ? "filter-btn--active-403" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-403">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-403">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-403">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-403 ${todo.done ? "todo-item--done-403" : ""}">
                    <button
                      class="todo-check-403 ${todo.done ? "todo-check--done-403" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-403" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-403"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-403" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-403" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-403">
            <span class="todo-count-403">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-403" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
