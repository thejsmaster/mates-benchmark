// client/components/Component0438.ts
// Benchmark component #438 — identical logic to TodoPage, scoped class names.
...
import "./Component0438.css";

type Filter = "all" | "active" | "done";

export const Component0438 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0438] mounted");
    return () => console.log("[Component0438] unmounted");
  });
  ...
  return () => html`
    <div class="app-438">
      <header class="header-438">
        <h1>todos — Component0438</h1>
      </header>

      <main class="main-438">
        <input
          class="add-input-438 ${error() ? "add-input--error-438" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-438" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-438">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-438">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-438 ${filter() === f ? "filter-btn--active-438" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-438">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-438">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-438">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-438 ${todo.done ? "todo-item--done-438" : ""}">
                    <button
                      class="todo-check-438 ${todo.done ? "todo-check--done-438" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-438" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-438"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-438" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-438" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-438">
            <span class="todo-count-438">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-438" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
