// client/components/Component0918.ts
// Benchmark component #918 — identical logic to TodoPage, scoped class names.
...
import "./Component0918.css";

type Filter = "all" | "active" | "done";

export const Component0918 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0918] mounted");
    return () => console.log("[Component0918] unmounted");
  });
  ...
  return () => html`
    <div class="app-918">
      <header class="header-918">
        <h1>todos — Component0918</h1>
      </header>

      <main class="main-918">
        <input
          class="add-input-918 ${error() ? "add-input--error-918" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-918" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-918">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-918">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-918 ${filter() === f ? "filter-btn--active-918" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-918">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-918">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-918">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-918 ${todo.done ? "todo-item--done-918" : ""}">
                    <button
                      class="todo-check-918 ${todo.done ? "todo-check--done-918" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-918" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-918"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-918" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-918" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-918">
            <span class="todo-count-918">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-918" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
