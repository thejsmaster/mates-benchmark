// client/components/Component0605.ts
// Benchmark component #605 — identical logic to TodoPage, scoped class names.
...
import "./Component0605.css";

type Filter = "all" | "active" | "done";

export const Component0605 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0605] mounted");
    return () => console.log("[Component0605] unmounted");
  });
  ...
  return () => html`
    <div class="app-605">
      <header class="header-605">
        <h1>todos — Component0605</h1>
      </header>

      <main class="main-605">
        <input
          class="add-input-605 ${error() ? "add-input--error-605" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-605" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-605">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-605">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-605 ${filter() === f ? "filter-btn--active-605" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-605">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-605">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-605">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-605 ${todo.done ? "todo-item--done-605" : ""}">
                    <button
                      class="todo-check-605 ${todo.done ? "todo-check--done-605" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-605" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-605"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-605" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-605" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-605">
            <span class="todo-count-605">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-605" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
