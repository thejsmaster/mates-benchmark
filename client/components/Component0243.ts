// client/components/Component0243.ts
// Benchmark component #243 — identical logic to TodoPage, scoped class names.
...
import "./Component0243.css";

type Filter = "all" | "active" | "done";

export const Component0243 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0243] mounted");
    return () => console.log("[Component0243] unmounted");
  });
  ...
  return () => html`
    <div class="app-243">
      <header class="header-243">
        <h1>todos — Component0243</h1>
      </header>

      <main class="main-243">
        <input
          class="add-input-243 ${error() ? "add-input--error-243" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-243" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-243">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-243">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-243 ${filter() === f ? "filter-btn--active-243" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-243">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-243">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-243">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-243 ${todo.done ? "todo-item--done-243" : ""}">
                    <button
                      class="todo-check-243 ${todo.done ? "todo-check--done-243" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-243" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-243"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-243" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-243" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-243">
            <span class="todo-count-243">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-243" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
