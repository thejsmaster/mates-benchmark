// client/components/Component0486.ts
// Benchmark component #486 — identical logic to TodoPage, scoped class names.
...
import "./Component0486.css";

type Filter = "all" | "active" | "done";

export const Component0486 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0486] mounted");
    return () => console.log("[Component0486] unmounted");
  });
  ...
  return () => html`
    <div class="app-486">
      <header class="header-486">
        <h1>todos — Component0486</h1>
      </header>

      <main class="main-486">
        <input
          class="add-input-486 ${error() ? "add-input--error-486" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-486" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-486">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-486">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-486 ${filter() === f ? "filter-btn--active-486" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-486">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-486">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-486">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-486 ${todo.done ? "todo-item--done-486" : ""}">
                    <button
                      class="todo-check-486 ${todo.done ? "todo-check--done-486" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-486" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-486"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-486" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-486" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-486">
            <span class="todo-count-486">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-486" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
