// client/components/Component0811.ts
// Benchmark component #811 — identical logic to TodoPage, scoped class names.
...
import "./Component0811.css";

type Filter = "all" | "active" | "done";

export const Component0811 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0811] mounted");
    return () => console.log("[Component0811] unmounted");
  });
  ...
  return () => html`
    <div class="app-811">
      <header class="header-811">
        <h1>todos — Component0811</h1>
      </header>

      <main class="main-811">
        <input
          class="add-input-811 ${error() ? "add-input--error-811" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-811" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-811">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-811">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-811 ${filter() === f ? "filter-btn--active-811" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-811">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-811">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-811">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-811 ${todo.done ? "todo-item--done-811" : ""}">
                    <button
                      class="todo-check-811 ${todo.done ? "todo-check--done-811" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-811" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-811"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-811" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-811" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-811">
            <span class="todo-count-811">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-811" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
