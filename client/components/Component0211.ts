// client/components/Component0211.ts
// Benchmark component #211 — identical logic to TodoPage, scoped class names.
...
import "./Component0211.css";

type Filter = "all" | "active" | "done";

export const Component0211 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0211] mounted");
    return () => console.log("[Component0211] unmounted");
  });
  ...
  return () => html`
    <div class="app-211">
      <header class="header-211">
        <h1>todos — Component0211</h1>
      </header>

      <main class="main-211">
        <input
          class="add-input-211 ${error() ? "add-input--error-211" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-211" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-211">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-211">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-211 ${filter() === f ? "filter-btn--active-211" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-211">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-211">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-211">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-211 ${todo.done ? "todo-item--done-211" : ""}">
                    <button
                      class="todo-check-211 ${todo.done ? "todo-check--done-211" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-211" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-211"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-211" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-211" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-211">
            <span class="todo-count-211">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-211" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
