// client/components/Component0556.ts
// Benchmark component #556 — identical logic to TodoPage, scoped class names.
...
import "./Component0556.css";

type Filter = "all" | "active" | "done";

export const Component0556 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0556] mounted");
    return () => console.log("[Component0556] unmounted");
  });
  ...
  return () => html`
    <div class="app-556">
      <header class="header-556">
        <h1>todos — Component0556</h1>
      </header>

      <main class="main-556">
        <input
          class="add-input-556 ${error() ? "add-input--error-556" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-556" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-556">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-556">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-556 ${filter() === f ? "filter-btn--active-556" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-556">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-556">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-556">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-556 ${todo.done ? "todo-item--done-556" : ""}">
                    <button
                      class="todo-check-556 ${todo.done ? "todo-check--done-556" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-556" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-556"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-556" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-556" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-556">
            <span class="todo-count-556">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-556" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
