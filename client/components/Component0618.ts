// client/components/Component0618.ts
// Benchmark component #618 — identical logic to TodoPage, scoped class names.
...
import "./Component0618.css";

type Filter = "all" | "active" | "done";

export const Component0618 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0618] mounted");
    return () => console.log("[Component0618] unmounted");
  });
  ...
  return () => html`
    <div class="app-618">
      <header class="header-618">
        <h1>todos — Component0618</h1>
      </header>

      <main class="main-618">
        <input
          class="add-input-618 ${error() ? "add-input--error-618" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-618" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-618">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-618">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-618 ${filter() === f ? "filter-btn--active-618" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-618">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-618">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-618">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-618 ${todo.done ? "todo-item--done-618" : ""}">
                    <button
                      class="todo-check-618 ${todo.done ? "todo-check--done-618" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-618" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-618"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-618" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-618" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-618">
            <span class="todo-count-618">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-618" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
