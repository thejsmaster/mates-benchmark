// client/components/Component0521.ts
// Benchmark component #521 — identical logic to TodoPage, scoped class names.
...
import "./Component0521.css";

type Filter = "all" | "active" | "done";

export const Component0521 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0521] mounted");
    return () => console.log("[Component0521] unmounted");
  });
  ...
  return () => html`
    <div class="app-521">
      <header class="header-521">
        <h1>todos — Component0521</h1>
      </header>

      <main class="main-521">
        <input
          class="add-input-521 ${error() ? "add-input--error-521" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-521" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-521">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-521">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-521 ${filter() === f ? "filter-btn--active-521" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-521">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-521">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-521">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-521 ${todo.done ? "todo-item--done-521" : ""}">
                    <button
                      class="todo-check-521 ${todo.done ? "todo-check--done-521" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-521" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-521"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-521" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-521" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-521">
            <span class="todo-count-521">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-521" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
