// client/components/Component0563.ts
// Benchmark component #563 — identical logic to TodoPage, scoped class names.
...
import "./Component0563.css";

type Filter = "all" | "active" | "done";

export const Component0563 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0563] mounted");
    return () => console.log("[Component0563] unmounted");
  });
  ...
  return () => html`
    <div class="app-563">
      <header class="header-563">
        <h1>todos — Component0563</h1>
      </header>

      <main class="main-563">
        <input
          class="add-input-563 ${error() ? "add-input--error-563" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-563" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-563">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-563">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-563 ${filter() === f ? "filter-btn--active-563" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-563">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-563">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-563">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-563 ${todo.done ? "todo-item--done-563" : ""}">
                    <button
                      class="todo-check-563 ${todo.done ? "todo-check--done-563" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-563" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-563"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-563" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-563" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-563">
            <span class="todo-count-563">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-563" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
