// client/components/Component0975.ts
// Benchmark component #975 — identical logic to TodoPage, scoped class names.
...
import "./Component0975.css";

type Filter = "all" | "active" | "done";

export const Component0975 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0975] mounted");
    return () => console.log("[Component0975] unmounted");
  });
  ...
  return () => html`
    <div class="app-975">
      <header class="header-975">
        <h1>todos — Component0975</h1>
      </header>

      <main class="main-975">
        <input
          class="add-input-975 ${error() ? "add-input--error-975" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-975" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-975">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-975">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-975 ${filter() === f ? "filter-btn--active-975" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-975">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-975">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-975">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-975 ${todo.done ? "todo-item--done-975" : ""}">
                    <button
                      class="todo-check-975 ${todo.done ? "todo-check--done-975" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-975" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-975"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-975" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-975" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-975">
            <span class="todo-count-975">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-975" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
