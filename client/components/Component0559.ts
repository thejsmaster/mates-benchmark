// client/components/Component0559.ts
// Benchmark component #559 — identical logic to TodoPage, scoped class names.
...
import "./Component0559.css";

type Filter = "all" | "active" | "done";

export const Component0559 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0559] mounted");
    return () => console.log("[Component0559] unmounted");
  });
  ...
  return () => html`
    <div class="app-559">
      <header class="header-559">
        <h1>todos — Component0559</h1>
      </header>

      <main class="main-559">
        <input
          class="add-input-559 ${error() ? "add-input--error-559" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-559" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-559">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-559">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-559 ${filter() === f ? "filter-btn--active-559" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-559">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-559">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-559">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-559 ${todo.done ? "todo-item--done-559" : ""}">
                    <button
                      class="todo-check-559 ${todo.done ? "todo-check--done-559" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-559" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-559"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-559" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-559" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-559">
            <span class="todo-count-559">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-559" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
