// client/components/Component0757.ts
// Benchmark component #757 — identical logic to TodoPage, scoped class names.
...
import "./Component0757.css";

type Filter = "all" | "active" | "done";

export const Component0757 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0757] mounted");
    return () => console.log("[Component0757] unmounted");
  });
  ...
  return () => html`
    <div class="app-757">
      <header class="header-757">
        <h1>todos — Component0757</h1>
      </header>

      <main class="main-757">
        <input
          class="add-input-757 ${error() ? "add-input--error-757" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-757" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-757">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-757">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-757 ${filter() === f ? "filter-btn--active-757" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-757">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-757">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-757">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-757 ${todo.done ? "todo-item--done-757" : ""}">
                    <button
                      class="todo-check-757 ${todo.done ? "todo-check--done-757" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-757" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-757"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-757" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-757" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-757">
            <span class="todo-count-757">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-757" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
