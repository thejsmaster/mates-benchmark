// client/components/Component0272.ts
// Benchmark component #272 — identical logic to TodoPage, scoped class names.
...
import "./Component0272.css";

type Filter = "all" | "active" | "done";

export const Component0272 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0272] mounted");
    return () => console.log("[Component0272] unmounted");
  });
  ...
  return () => html`
    <div class="app-272">
      <header class="header-272">
        <h1>todos — Component0272</h1>
      </header>

      <main class="main-272">
        <input
          class="add-input-272 ${error() ? "add-input--error-272" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-272" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-272">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-272">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-272 ${filter() === f ? "filter-btn--active-272" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-272">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-272">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-272">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-272 ${todo.done ? "todo-item--done-272" : ""}">
                    <button
                      class="todo-check-272 ${todo.done ? "todo-check--done-272" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-272" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-272"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-272" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-272" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-272">
            <span class="todo-count-272">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-272" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
