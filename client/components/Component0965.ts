// client/components/Component0965.ts
// Benchmark component #965 — identical logic to TodoPage, scoped class names.
...
import "./Component0965.css";

type Filter = "all" | "active" | "done";

export const Component0965 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0965] mounted");
    return () => console.log("[Component0965] unmounted");
  });
  ...
  return () => html`
    <div class="app-965">
      <header class="header-965">
        <h1>todos — Component0965</h1>
      </header>

      <main class="main-965">
        <input
          class="add-input-965 ${error() ? "add-input--error-965" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-965" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-965">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-965">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-965 ${filter() === f ? "filter-btn--active-965" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-965">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-965">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-965">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-965 ${todo.done ? "todo-item--done-965" : ""}">
                    <button
                      class="todo-check-965 ${todo.done ? "todo-check--done-965" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-965" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-965"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-965" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-965" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-965">
            <span class="todo-count-965">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-965" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
