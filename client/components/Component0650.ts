// client/components/Component0650.ts
// Benchmark component #650 — identical logic to TodoPage, scoped class names.
...
import "./Component0650.css";

type Filter = "all" | "active" | "done";

export const Component0650 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0650] mounted");
    return () => console.log("[Component0650] unmounted");
  });
  ...
  return () => html`
    <div class="app-650">
      <header class="header-650">
        <h1>todos — Component0650</h1>
      </header>

      <main class="main-650">
        <input
          class="add-input-650 ${error() ? "add-input--error-650" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-650" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-650">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-650">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-650 ${filter() === f ? "filter-btn--active-650" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-650">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-650">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-650">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-650 ${todo.done ? "todo-item--done-650" : ""}">
                    <button
                      class="todo-check-650 ${todo.done ? "todo-check--done-650" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-650" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-650"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-650" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-650" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-650">
            <span class="todo-count-650">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-650" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
