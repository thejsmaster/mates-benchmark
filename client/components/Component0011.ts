// client/components/Component0011.ts
// Benchmark component #11 — identical logic to TodoPage, scoped class names.
...
import "./Component0011.css";

type Filter = "all" | "active" | "done";

export const Component0011 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0011] mounted");
    return () => console.log("[Component0011] unmounted");
  });
  ...
  return () => html`
    <div class="app-11">
      <header class="header-11">
        <h1>todos — Component0011</h1>
      </header>

      <main class="main-11">
        <input
          class="add-input-11 ${error() ? "add-input--error-11" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-11" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-11">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-11">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-11 ${filter() === f ? "filter-btn--active-11" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-11">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-11">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-11">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-11 ${todo.done ? "todo-item--done-11" : ""}">
                    <button
                      class="todo-check-11 ${todo.done ? "todo-check--done-11" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-11" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-11"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-11" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-11" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-11">
            <span class="todo-count-11">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-11" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
