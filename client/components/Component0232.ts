// client/components/Component0232.ts
// Benchmark component #232 — identical logic to TodoPage, scoped class names.
...
import "./Component0232.css";

type Filter = "all" | "active" | "done";

export const Component0232 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0232] mounted");
    return () => console.log("[Component0232] unmounted");
  });
  ...
  return () => html`
    <div class="app-232">
      <header class="header-232">
        <h1>todos — Component0232</h1>
      </header>

      <main class="main-232">
        <input
          class="add-input-232 ${error() ? "add-input--error-232" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-232" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-232">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-232">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-232 ${filter() === f ? "filter-btn--active-232" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-232">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-232">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-232">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-232 ${todo.done ? "todo-item--done-232" : ""}">
                    <button
                      class="todo-check-232 ${todo.done ? "todo-check--done-232" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-232" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-232"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-232" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-232" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-232">
            <span class="todo-count-232">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-232" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
