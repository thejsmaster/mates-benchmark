// client/components/Component0916.ts
// Benchmark component #916 — identical logic to TodoPage, scoped class names.
...
import "./Component0916.css";

type Filter = "all" | "active" | "done";

export const Component0916 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0916] mounted");
    return () => console.log("[Component0916] unmounted");
  });
  ...
  return () => html`
    <div class="app-916">
      <header class="header-916">
        <h1>todos — Component0916</h1>
      </header>

      <main class="main-916">
        <input
          class="add-input-916 ${error() ? "add-input--error-916" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-916" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-916">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-916">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-916 ${filter() === f ? "filter-btn--active-916" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-916">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-916">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-916">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-916 ${todo.done ? "todo-item--done-916" : ""}">
                    <button
                      class="todo-check-916 ${todo.done ? "todo-check--done-916" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-916" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-916"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-916" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-916" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-916">
            <span class="todo-count-916">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-916" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
