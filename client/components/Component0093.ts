// client/components/Component0093.ts
// Benchmark component #93 — identical logic to TodoPage, scoped class names.
...
import "./Component0093.css";

type Filter = "all" | "active" | "done";

export const Component0093 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0093] mounted");
    return () => console.log("[Component0093] unmounted");
  });
  ...
  return () => html`
    <div class="app-93">
      <header class="header-93">
        <h1>todos — Component0093</h1>
      </header>

      <main class="main-93">
        <input
          class="add-input-93 ${error() ? "add-input--error-93" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-93" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-93">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-93">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-93 ${filter() === f ? "filter-btn--active-93" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-93">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-93">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-93">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-93 ${todo.done ? "todo-item--done-93" : ""}">
                    <button
                      class="todo-check-93 ${todo.done ? "todo-check--done-93" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-93" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-93"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-93" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-93" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-93">
            <span class="todo-count-93">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-93" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
