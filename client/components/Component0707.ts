// client/components/Component0707.ts
// Benchmark component #707 — identical logic to TodoPage, scoped class names.
...
import "./Component0707.css";

type Filter = "all" | "active" | "done";

export const Component0707 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0707] mounted");
    return () => console.log("[Component0707] unmounted");
  });
  ...
  return () => html`
    <div class="app-707">
      <header class="header-707">
        <h1>todos — Component0707</h1>
      </header>

      <main class="main-707">
        <input
          class="add-input-707 ${error() ? "add-input--error-707" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-707" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-707">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-707">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-707 ${filter() === f ? "filter-btn--active-707" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-707">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-707">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-707">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-707 ${todo.done ? "todo-item--done-707" : ""}">
                    <button
                      class="todo-check-707 ${todo.done ? "todo-check--done-707" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-707" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-707"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-707" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-707" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-707">
            <span class="todo-count-707">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-707" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
