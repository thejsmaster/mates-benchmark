// client/components/Component0226.ts
// Benchmark component #226 — identical logic to TodoPage, scoped class names.
...
import "./Component0226.css";

type Filter = "all" | "active" | "done";

export const Component0226 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0226] mounted");
    return () => console.log("[Component0226] unmounted");
  });
  ...
  return () => html`
    <div class="app-226">
      <header class="header-226">
        <h1>todos — Component0226</h1>
      </header>

      <main class="main-226">
        <input
          class="add-input-226 ${error() ? "add-input--error-226" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-226" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-226">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-226">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-226 ${filter() === f ? "filter-btn--active-226" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-226">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-226">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-226">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-226 ${todo.done ? "todo-item--done-226" : ""}">
                    <button
                      class="todo-check-226 ${todo.done ? "todo-check--done-226" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-226" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-226"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-226" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-226" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-226">
            <span class="todo-count-226">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-226" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
