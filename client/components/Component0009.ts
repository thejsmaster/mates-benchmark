// client/components/Component0009.ts
// Benchmark component #9 — identical logic to TodoPage, scoped class names.
...
import "./Component0009.css";

type Filter = "all" | "active" | "done";

export const Component0009 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0009] mounted");
    return () => console.log("[Component0009] unmounted");
  });
  ...
  return () => html`
    <div class="app-9">
      <header class="header-9">
        <h1>todos — Component0009</h1>
      </header>

      <main class="main-9">
        <input
          class="add-input-9 ${error() ? "add-input--error-9" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-9" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-9">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-9">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-9 ${filter() === f ? "filter-btn--active-9" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-9">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-9">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-9">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-9 ${todo.done ? "todo-item--done-9" : ""}">
                    <button
                      class="todo-check-9 ${todo.done ? "todo-check--done-9" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-9" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-9"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-9" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-9" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-9">
            <span class="todo-count-9">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-9" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
