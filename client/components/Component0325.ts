// client/components/Component0325.ts
// Benchmark component #325 — identical logic to TodoPage, scoped class names.
...
import "./Component0325.css";

type Filter = "all" | "active" | "done";

export const Component0325 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0325] mounted");
    return () => console.log("[Component0325] unmounted");
  });
  ...
  return () => html`
    <div class="app-325">
      <header class="header-325">
        <h1>todos — Component0325</h1>
      </header>

      <main class="main-325">
        <input
          class="add-input-325 ${error() ? "add-input--error-325" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-325" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-325">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-325">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-325 ${filter() === f ? "filter-btn--active-325" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-325">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-325">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-325">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-325 ${todo.done ? "todo-item--done-325" : ""}">
                    <button
                      class="todo-check-325 ${todo.done ? "todo-check--done-325" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-325" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-325"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-325" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-325" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-325">
            <span class="todo-count-325">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-325" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
