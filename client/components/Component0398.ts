// client/components/Component0398.ts
// Benchmark component #398 — identical logic to TodoPage, scoped class names.
...
import "./Component0398.css";

type Filter = "all" | "active" | "done";

export const Component0398 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0398] mounted");
    return () => console.log("[Component0398] unmounted");
  });
  ...
  return () => html`
    <div class="app-398">
      <header class="header-398">
        <h1>todos — Component0398</h1>
      </header>

      <main class="main-398">
        <input
          class="add-input-398 ${error() ? "add-input--error-398" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-398" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-398">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-398">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-398 ${filter() === f ? "filter-btn--active-398" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-398">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-398">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-398">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-398 ${todo.done ? "todo-item--done-398" : ""}">
                    <button
                      class="todo-check-398 ${todo.done ? "todo-check--done-398" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-398" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-398"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-398" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-398" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-398">
            <span class="todo-count-398">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-398" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
