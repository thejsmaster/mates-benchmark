// client/components/Component0073.ts
// Benchmark component #73 — identical logic to TodoPage, scoped class names.
...
import "./Component0073.css";

type Filter = "all" | "active" | "done";

export const Component0073 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0073] mounted");
    return () => console.log("[Component0073] unmounted");
  });
  ...
  return () => html`
    <div class="app-73">
      <header class="header-73">
        <h1>todos — Component0073</h1>
      </header>

      <main class="main-73">
        <input
          class="add-input-73 ${error() ? "add-input--error-73" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-73" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-73">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-73">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-73 ${filter() === f ? "filter-btn--active-73" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-73">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-73">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-73">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-73 ${todo.done ? "todo-item--done-73" : ""}">
                    <button
                      class="todo-check-73 ${todo.done ? "todo-check--done-73" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-73" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-73"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-73" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-73" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-73">
            <span class="todo-count-73">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-73" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
