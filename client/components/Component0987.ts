// client/components/Component0987.ts
// Benchmark component #987 — identical logic to TodoPage, scoped class names.
...
import "./Component0987.css";

type Filter = "all" | "active" | "done";

export const Component0987 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0987] mounted");
    return () => console.log("[Component0987] unmounted");
  });
  ...
  return () => html`
    <div class="app-987">
      <header class="header-987">
        <h1>todos — Component0987</h1>
      </header>

      <main class="main-987">
        <input
          class="add-input-987 ${error() ? "add-input--error-987" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-987" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-987">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-987">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-987 ${filter() === f ? "filter-btn--active-987" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-987">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-987">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-987">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-987 ${todo.done ? "todo-item--done-987" : ""}">
                    <button
                      class="todo-check-987 ${todo.done ? "todo-check--done-987" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-987" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-987"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-987" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-987" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-987">
            <span class="todo-count-987">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-987" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
