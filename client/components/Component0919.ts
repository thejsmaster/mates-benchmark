// client/components/Component0919.ts
// Benchmark component #919 — identical logic to TodoPage, scoped class names.
...
import "./Component0919.css";

type Filter = "all" | "active" | "done";

export const Component0919 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0919] mounted");
    return () => console.log("[Component0919] unmounted");
  });
  ...
  return () => html`
    <div class="app-919">
      <header class="header-919">
        <h1>todos — Component0919</h1>
      </header>

      <main class="main-919">
        <input
          class="add-input-919 ${error() ? "add-input--error-919" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-919" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-919">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-919">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-919 ${filter() === f ? "filter-btn--active-919" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-919">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-919">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-919">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-919 ${todo.done ? "todo-item--done-919" : ""}">
                    <button
                      class="todo-check-919 ${todo.done ? "todo-check--done-919" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-919" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-919"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-919" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-919" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-919">
            <span class="todo-count-919">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-919" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
