// client/components/Component0863.ts
// Benchmark component #863 — identical logic to TodoPage, scoped class names.
...
import "./Component0863.css";

type Filter = "all" | "active" | "done";

export const Component0863 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0863] mounted");
    return () => console.log("[Component0863] unmounted");
  });
  ...
  return () => html`
    <div class="app-863">
      <header class="header-863">
        <h1>todos — Component0863</h1>
      </header>

      <main class="main-863">
        <input
          class="add-input-863 ${error() ? "add-input--error-863" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-863" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-863">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-863">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-863 ${filter() === f ? "filter-btn--active-863" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-863">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-863">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-863">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-863 ${todo.done ? "todo-item--done-863" : ""}">
                    <button
                      class="todo-check-863 ${todo.done ? "todo-check--done-863" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-863" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-863"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-863" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-863" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-863">
            <span class="todo-count-863">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-863" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
