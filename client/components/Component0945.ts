// client/components/Component0945.ts
// Benchmark component #945 — identical logic to TodoPage, scoped class names.
...
import "./Component0945.css";

type Filter = "all" | "active" | "done";

export const Component0945 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0945] mounted");
    return () => console.log("[Component0945] unmounted");
  });
  ...
  return () => html`
    <div class="app-945">
      <header class="header-945">
        <h1>todos — Component0945</h1>
      </header>

      <main class="main-945">
        <input
          class="add-input-945 ${error() ? "add-input--error-945" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-945" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-945">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-945">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-945 ${filter() === f ? "filter-btn--active-945" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-945">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-945">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-945">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-945 ${todo.done ? "todo-item--done-945" : ""}">
                    <button
                      class="todo-check-945 ${todo.done ? "todo-check--done-945" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-945" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-945"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-945" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-945" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-945">
            <span class="todo-count-945">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-945" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
