// client/components/Component0735.ts
// Benchmark component #735 — identical logic to TodoPage, scoped class names.
...
import "./Component0735.css";

type Filter = "all" | "active" | "done";

export const Component0735 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0735] mounted");
    return () => console.log("[Component0735] unmounted");
  });
  ...
  return () => html`
    <div class="app-735">
      <header class="header-735">
        <h1>todos — Component0735</h1>
      </header>

      <main class="main-735">
        <input
          class="add-input-735 ${error() ? "add-input--error-735" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-735" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-735">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-735">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-735 ${filter() === f ? "filter-btn--active-735" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-735">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-735">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-735">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-735 ${todo.done ? "todo-item--done-735" : ""}">
                    <button
                      class="todo-check-735 ${todo.done ? "todo-check--done-735" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-735" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-735"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-735" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-735" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-735">
            <span class="todo-count-735">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-735" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
