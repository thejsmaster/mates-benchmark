// client/components/Component0505.ts
// Benchmark component #505 — identical logic to TodoPage, scoped class names.
...
import "./Component0505.css";

type Filter = "all" | "active" | "done";

export const Component0505 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0505] mounted");
    return () => console.log("[Component0505] unmounted");
  });
  ...
  return () => html`
    <div class="app-505">
      <header class="header-505">
        <h1>todos — Component0505</h1>
      </header>

      <main class="main-505">
        <input
          class="add-input-505 ${error() ? "add-input--error-505" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-505" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-505">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-505">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-505 ${filter() === f ? "filter-btn--active-505" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-505">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-505">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-505">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-505 ${todo.done ? "todo-item--done-505" : ""}">
                    <button
                      class="todo-check-505 ${todo.done ? "todo-check--done-505" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-505" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-505"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-505" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-505" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-505">
            <span class="todo-count-505">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-505" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
