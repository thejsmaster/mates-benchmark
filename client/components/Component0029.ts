// client/components/Component0029.ts
// Benchmark component #29 — identical logic to TodoPage, scoped class names.
...
import "./Component0029.css";

type Filter = "all" | "active" | "done";

export const Component0029 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0029] mounted");
    return () => console.log("[Component0029] unmounted");
  });
  ...
  return () => html`
    <div class="app-29">
      <header class="header-29">
        <h1>todos — Component0029</h1>
      </header>

      <main class="main-29">
        <input
          class="add-input-29 ${error() ? "add-input--error-29" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-29" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-29">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-29">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-29 ${filter() === f ? "filter-btn--active-29" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-29">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-29">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-29">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-29 ${todo.done ? "todo-item--done-29" : ""}">
                    <button
                      class="todo-check-29 ${todo.done ? "todo-check--done-29" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-29" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-29"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-29" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-29" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-29">
            <span class="todo-count-29">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-29" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
