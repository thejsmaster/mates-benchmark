// client/components/Component0838.ts
// Benchmark component #838 — identical logic to TodoPage, scoped class names.
...
import "./Component0838.css";

type Filter = "all" | "active" | "done";

export const Component0838 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0838] mounted");
    return () => console.log("[Component0838] unmounted");
  });
  ...
  return () => html`
    <div class="app-838">
      <header class="header-838">
        <h1>todos — Component0838</h1>
      </header>

      <main class="main-838">
        <input
          class="add-input-838 ${error() ? "add-input--error-838" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-838" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-838">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-838">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-838 ${filter() === f ? "filter-btn--active-838" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-838">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-838">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-838">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-838 ${todo.done ? "todo-item--done-838" : ""}">
                    <button
                      class="todo-check-838 ${todo.done ? "todo-check--done-838" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-838" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-838"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-838" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-838" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-838">
            <span class="todo-count-838">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-838" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
