// client/components/Component0140.ts
// Benchmark component #140 — identical logic to TodoPage, scoped class names.
...
import "./Component0140.css";

type Filter = "all" | "active" | "done";

export const Component0140 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0140] mounted");
    return () => console.log("[Component0140] unmounted");
  });
  ...
  return () => html`
    <div class="app-140">
      <header class="header-140">
        <h1>todos — Component0140</h1>
      </header>

      <main class="main-140">
        <input
          class="add-input-140 ${error() ? "add-input--error-140" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-140" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-140">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-140">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-140 ${filter() === f ? "filter-btn--active-140" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-140">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-140">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-140">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-140 ${todo.done ? "todo-item--done-140" : ""}">
                    <button
                      class="todo-check-140 ${todo.done ? "todo-check--done-140" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-140" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-140"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-140" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-140" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-140">
            <span class="todo-count-140">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-140" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
