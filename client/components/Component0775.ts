// client/components/Component0775.ts
// Benchmark component #775 — identical logic to TodoPage, scoped class names.
...
import "./Component0775.css";

type Filter = "all" | "active" | "done";

export const Component0775 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0775] mounted");
    return () => console.log("[Component0775] unmounted");
  });
  ...
  return () => html`
    <div class="app-775">
      <header class="header-775">
        <h1>todos — Component0775</h1>
      </header>

      <main class="main-775">
        <input
          class="add-input-775 ${error() ? "add-input--error-775" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-775" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-775">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-775">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-775 ${filter() === f ? "filter-btn--active-775" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-775">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-775">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-775">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-775 ${todo.done ? "todo-item--done-775" : ""}">
                    <button
                      class="todo-check-775 ${todo.done ? "todo-check--done-775" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-775" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-775"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-775" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-775" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-775">
            <span class="todo-count-775">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-775" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
