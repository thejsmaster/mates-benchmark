// client/components/Component0813.ts
// Benchmark component #813 — identical logic to TodoPage, scoped class names.
...
import "./Component0813.css";

type Filter = "all" | "active" | "done";

export const Component0813 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0813] mounted");
    return () => console.log("[Component0813] unmounted");
  });
  ...
  return () => html`
    <div class="app-813">
      <header class="header-813">
        <h1>todos — Component0813</h1>
      </header>

      <main class="main-813">
        <input
          class="add-input-813 ${error() ? "add-input--error-813" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-813" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-813">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-813">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-813 ${filter() === f ? "filter-btn--active-813" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-813">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-813">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-813">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-813 ${todo.done ? "todo-item--done-813" : ""}">
                    <button
                      class="todo-check-813 ${todo.done ? "todo-check--done-813" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-813" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-813"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-813" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-813" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-813">
            <span class="todo-count-813">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-813" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
