// client/components/Component0076.ts
// Benchmark component #76 — identical logic to TodoPage, scoped class names.
...
import "./Component0076.css";

type Filter = "all" | "active" | "done";

export const Component0076 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0076] mounted");
    return () => console.log("[Component0076] unmounted");
  });
  ...
  return () => html`
    <div class="app-76">
      <header class="header-76">
        <h1>todos — Component0076</h1>
      </header>

      <main class="main-76">
        <input
          class="add-input-76 ${error() ? "add-input--error-76" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-76" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-76">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-76">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-76 ${filter() === f ? "filter-btn--active-76" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-76">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-76">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-76">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-76 ${todo.done ? "todo-item--done-76" : ""}">
                    <button
                      class="todo-check-76 ${todo.done ? "todo-check--done-76" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-76" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-76"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-76" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-76" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-76">
            <span class="todo-count-76">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-76" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
