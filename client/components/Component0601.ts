// client/components/Component0601.ts
// Benchmark component #601 — identical logic to TodoPage, scoped class names.
...
import "./Component0601.css";

type Filter = "all" | "active" | "done";

export const Component0601 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0601] mounted");
    return () => console.log("[Component0601] unmounted");
  });
  ...
  return () => html`
    <div class="app-601">
      <header class="header-601">
        <h1>todos — Component0601</h1>
      </header>

      <main class="main-601">
        <input
          class="add-input-601 ${error() ? "add-input--error-601" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-601" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-601">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-601">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-601 ${filter() === f ? "filter-btn--active-601" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-601">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-601">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-601">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-601 ${todo.done ? "todo-item--done-601" : ""}">
                    <button
                      class="todo-check-601 ${todo.done ? "todo-check--done-601" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-601" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-601"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-601" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-601" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-601">
            <span class="todo-count-601">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-601" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
