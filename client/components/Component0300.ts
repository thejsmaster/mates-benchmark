// client/components/Component0300.ts
// Benchmark component #300 — identical logic to TodoPage, scoped class names.
...
import "./Component0300.css";

type Filter = "all" | "active" | "done";

export const Component0300 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0300] mounted");
    return () => console.log("[Component0300] unmounted");
  });
  ...
  return () => html`
    <div class="app-300">
      <header class="header-300">
        <h1>todos — Component0300</h1>
      </header>

      <main class="main-300">
        <input
          class="add-input-300 ${error() ? "add-input--error-300" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-300" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-300">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-300">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-300 ${filter() === f ? "filter-btn--active-300" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-300">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-300">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-300">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-300 ${todo.done ? "todo-item--done-300" : ""}">
                    <button
                      class="todo-check-300 ${todo.done ? "todo-check--done-300" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-300" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-300"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-300" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-300" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-300">
            <span class="todo-count-300">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-300" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
