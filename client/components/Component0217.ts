// client/components/Component0217.ts
// Benchmark component #217 — identical logic to TodoPage, scoped class names.
...
import "./Component0217.css";

type Filter = "all" | "active" | "done";

export const Component0217 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0217] mounted");
    return () => console.log("[Component0217] unmounted");
  });
  ...
  return () => html`
    <div class="app-217">
      <header class="header-217">
        <h1>todos — Component0217</h1>
      </header>

      <main class="main-217">
        <input
          class="add-input-217 ${error() ? "add-input--error-217" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-217" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-217">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-217">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-217 ${filter() === f ? "filter-btn--active-217" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-217">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-217">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-217">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-217 ${todo.done ? "todo-item--done-217" : ""}">
                    <button
                      class="todo-check-217 ${todo.done ? "todo-check--done-217" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-217" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-217"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-217" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-217" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-217">
            <span class="todo-count-217">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-217" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
