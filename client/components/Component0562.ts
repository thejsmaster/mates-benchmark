// client/components/Component0562.ts
// Benchmark component #562 — identical logic to TodoPage, scoped class names.
...
import "./Component0562.css";

type Filter = "all" | "active" | "done";

export const Component0562 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0562] mounted");
    return () => console.log("[Component0562] unmounted");
  });
  ...
  return () => html`
    <div class="app-562">
      <header class="header-562">
        <h1>todos — Component0562</h1>
      </header>

      <main class="main-562">
        <input
          class="add-input-562 ${error() ? "add-input--error-562" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-562" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-562">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-562">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-562 ${filter() === f ? "filter-btn--active-562" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-562">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-562">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-562">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-562 ${todo.done ? "todo-item--done-562" : ""}">
                    <button
                      class="todo-check-562 ${todo.done ? "todo-check--done-562" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-562" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-562"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-562" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-562" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-562">
            <span class="todo-count-562">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-562" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
