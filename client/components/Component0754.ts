// client/components/Component0754.ts
// Benchmark component #754 — identical logic to TodoPage, scoped class names.
...
import "./Component0754.css";

type Filter = "all" | "active" | "done";

export const Component0754 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0754] mounted");
    return () => console.log("[Component0754] unmounted");
  });
  ...
  return () => html`
    <div class="app-754">
      <header class="header-754">
        <h1>todos — Component0754</h1>
      </header>

      <main class="main-754">
        <input
          class="add-input-754 ${error() ? "add-input--error-754" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-754" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-754">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-754">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-754 ${filter() === f ? "filter-btn--active-754" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-754">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-754">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-754">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-754 ${todo.done ? "todo-item--done-754" : ""}">
                    <button
                      class="todo-check-754 ${todo.done ? "todo-check--done-754" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-754" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-754"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-754" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-754" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-754">
            <span class="todo-count-754">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-754" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
