// client/components/Component0933.ts
// Benchmark component #933 — identical logic to TodoPage, scoped class names.
...
import "./Component0933.css";

type Filter = "all" | "active" | "done";

export const Component0933 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0933] mounted");
    return () => console.log("[Component0933] unmounted");
  });
  ...
  return () => html`
    <div class="app-933">
      <header class="header-933">
        <h1>todos — Component0933</h1>
      </header>

      <main class="main-933">
        <input
          class="add-input-933 ${error() ? "add-input--error-933" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-933" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-933">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-933">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-933 ${filter() === f ? "filter-btn--active-933" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-933">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-933">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-933">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-933 ${todo.done ? "todo-item--done-933" : ""}">
                    <button
                      class="todo-check-933 ${todo.done ? "todo-check--done-933" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-933" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-933"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-933" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-933" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-933">
            <span class="todo-count-933">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-933" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
