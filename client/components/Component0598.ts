// client/components/Component0598.ts
// Benchmark component #598 — identical logic to TodoPage, scoped class names.
...
import "./Component0598.css";

type Filter = "all" | "active" | "done";

export const Component0598 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0598] mounted");
    return () => console.log("[Component0598] unmounted");
  });
  ...
  return () => html`
    <div class="app-598">
      <header class="header-598">
        <h1>todos — Component0598</h1>
      </header>

      <main class="main-598">
        <input
          class="add-input-598 ${error() ? "add-input--error-598" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-598" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-598">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-598">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-598 ${filter() === f ? "filter-btn--active-598" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-598">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-598">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-598">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-598 ${todo.done ? "todo-item--done-598" : ""}">
                    <button
                      class="todo-check-598 ${todo.done ? "todo-check--done-598" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-598" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-598"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-598" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-598" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-598">
            <span class="todo-count-598">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-598" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
