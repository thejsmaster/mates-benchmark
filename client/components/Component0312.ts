// client/components/Component0312.ts
// Benchmark component #312 — identical logic to TodoPage, scoped class names.
...
import "./Component0312.css";

type Filter = "all" | "active" | "done";

export const Component0312 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0312] mounted");
    return () => console.log("[Component0312] unmounted");
  });
  ...
  return () => html`
    <div class="app-312">
      <header class="header-312">
        <h1>todos — Component0312</h1>
      </header>

      <main class="main-312">
        <input
          class="add-input-312 ${error() ? "add-input--error-312" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-312" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-312">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-312">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-312 ${filter() === f ? "filter-btn--active-312" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-312">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-312">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-312">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-312 ${todo.done ? "todo-item--done-312" : ""}">
                    <button
                      class="todo-check-312 ${todo.done ? "todo-check--done-312" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-312" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-312"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-312" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-312" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-312">
            <span class="todo-count-312">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-312" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
