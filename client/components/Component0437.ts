// client/components/Component0437.ts
// Benchmark component #437 — identical logic to TodoPage, scoped class names.
...
import "./Component0437.css";

type Filter = "all" | "active" | "done";

export const Component0437 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0437] mounted");
    return () => console.log("[Component0437] unmounted");
  });
  ...
  return () => html`
    <div class="app-437">
      <header class="header-437">
        <h1>todos — Component0437</h1>
      </header>

      <main class="main-437">
        <input
          class="add-input-437 ${error() ? "add-input--error-437" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-437" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-437">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-437">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-437 ${filter() === f ? "filter-btn--active-437" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-437">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-437">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-437">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-437 ${todo.done ? "todo-item--done-437" : ""}">
                    <button
                      class="todo-check-437 ${todo.done ? "todo-check--done-437" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-437" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-437"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-437" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-437" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-437">
            <span class="todo-count-437">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-437" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
