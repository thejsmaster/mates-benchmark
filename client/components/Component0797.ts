// client/components/Component0797.ts
// Benchmark component #797 — identical logic to TodoPage, scoped class names.
...
import "./Component0797.css";

type Filter = "all" | "active" | "done";

export const Component0797 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0797] mounted");
    return () => console.log("[Component0797] unmounted");
  });
  ...
  return () => html`
    <div class="app-797">
      <header class="header-797">
        <h1>todos — Component0797</h1>
      </header>

      <main class="main-797">
        <input
          class="add-input-797 ${error() ? "add-input--error-797" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-797" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-797">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-797">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-797 ${filter() === f ? "filter-btn--active-797" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-797">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-797">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-797">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-797 ${todo.done ? "todo-item--done-797" : ""}">
                    <button
                      class="todo-check-797 ${todo.done ? "todo-check--done-797" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-797" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-797"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-797" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-797" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-797">
            <span class="todo-count-797">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-797" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
