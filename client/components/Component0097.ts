// client/components/Component0097.ts
// Benchmark component #97 — identical logic to TodoPage, scoped class names.
...
import "./Component0097.css";

type Filter = "all" | "active" | "done";

export const Component0097 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0097] mounted");
    return () => console.log("[Component0097] unmounted");
  });
  ...
  return () => html`
    <div class="app-97">
      <header class="header-97">
        <h1>todos — Component0097</h1>
      </header>

      <main class="main-97">
        <input
          class="add-input-97 ${error() ? "add-input--error-97" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-97" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-97">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-97">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-97 ${filter() === f ? "filter-btn--active-97" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-97">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-97">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-97">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-97 ${todo.done ? "todo-item--done-97" : ""}">
                    <button
                      class="todo-check-97 ${todo.done ? "todo-check--done-97" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-97" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-97"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-97" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-97" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-97">
            <span class="todo-count-97">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-97" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
