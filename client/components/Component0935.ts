// client/components/Component0935.ts
// Benchmark component #935 — identical logic to TodoPage, scoped class names.
...
import "./Component0935.css";

type Filter = "all" | "active" | "done";

export const Component0935 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0935] mounted");
    return () => console.log("[Component0935] unmounted");
  });
  ...
  return () => html`
    <div class="app-935">
      <header class="header-935">
        <h1>todos — Component0935</h1>
      </header>

      <main class="main-935">
        <input
          class="add-input-935 ${error() ? "add-input--error-935" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-935" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-935">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-935">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-935 ${filter() === f ? "filter-btn--active-935" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-935">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-935">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-935">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-935 ${todo.done ? "todo-item--done-935" : ""}">
                    <button
                      class="todo-check-935 ${todo.done ? "todo-check--done-935" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-935" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-935"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-935" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-935" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-935">
            <span class="todo-count-935">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-935" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
