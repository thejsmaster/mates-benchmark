// client/components/Component0354.ts
// Benchmark component #354 — identical logic to TodoPage, scoped class names.
...
import "./Component0354.css";

type Filter = "all" | "active" | "done";

export const Component0354 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0354] mounted");
    return () => console.log("[Component0354] unmounted");
  });
  ...
  return () => html`
    <div class="app-354">
      <header class="header-354">
        <h1>todos — Component0354</h1>
      </header>

      <main class="main-354">
        <input
          class="add-input-354 ${error() ? "add-input--error-354" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-354" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-354">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-354">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-354 ${filter() === f ? "filter-btn--active-354" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-354">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-354">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-354">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-354 ${todo.done ? "todo-item--done-354" : ""}">
                    <button
                      class="todo-check-354 ${todo.done ? "todo-check--done-354" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-354" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-354"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-354" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-354" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-354">
            <span class="todo-count-354">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-354" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
