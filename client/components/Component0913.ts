// client/components/Component0913.ts
// Benchmark component #913 — identical logic to TodoPage, scoped class names.
...
import "./Component0913.css";

type Filter = "all" | "active" | "done";

export const Component0913 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0913] mounted");
    return () => console.log("[Component0913] unmounted");
  });
  ...
  return () => html`
    <div class="app-913">
      <header class="header-913">
        <h1>todos — Component0913</h1>
      </header>

      <main class="main-913">
        <input
          class="add-input-913 ${error() ? "add-input--error-913" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-913" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-913">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-913">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-913 ${filter() === f ? "filter-btn--active-913" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-913">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-913">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-913">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-913 ${todo.done ? "todo-item--done-913" : ""}">
                    <button
                      class="todo-check-913 ${todo.done ? "todo-check--done-913" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-913" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-913"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-913" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-913" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-913">
            <span class="todo-count-913">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-913" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
