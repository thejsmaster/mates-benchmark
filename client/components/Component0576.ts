// client/components/Component0576.ts
// Benchmark component #576 — identical logic to TodoPage, scoped class names.
...
import "./Component0576.css";

type Filter = "all" | "active" | "done";

export const Component0576 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0576] mounted");
    return () => console.log("[Component0576] unmounted");
  });
  ...
  return () => html`
    <div class="app-576">
      <header class="header-576">
        <h1>todos — Component0576</h1>
      </header>

      <main class="main-576">
        <input
          class="add-input-576 ${error() ? "add-input--error-576" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-576" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-576">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-576">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-576 ${filter() === f ? "filter-btn--active-576" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-576">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-576">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-576">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-576 ${todo.done ? "todo-item--done-576" : ""}">
                    <button
                      class="todo-check-576 ${todo.done ? "todo-check--done-576" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-576" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-576"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-576" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-576" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-576">
            <span class="todo-count-576">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-576" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
