// client/components/Component0687.ts
// Benchmark component #687 — identical logic to TodoPage, scoped class names.
...
import "./Component0687.css";

type Filter = "all" | "active" | "done";

export const Component0687 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0687] mounted");
    return () => console.log("[Component0687] unmounted");
  });
  ...
  return () => html`
    <div class="app-687">
      <header class="header-687">
        <h1>todos — Component0687</h1>
      </header>

      <main class="main-687">
        <input
          class="add-input-687 ${error() ? "add-input--error-687" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-687" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-687">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-687">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-687 ${filter() === f ? "filter-btn--active-687" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-687">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-687">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-687">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-687 ${todo.done ? "todo-item--done-687" : ""}">
                    <button
                      class="todo-check-687 ${todo.done ? "todo-check--done-687" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-687" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-687"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-687" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-687" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-687">
            <span class="todo-count-687">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-687" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
