// client/components/Component0391.ts
// Benchmark component #391 — identical logic to TodoPage, scoped class names.
...
import "./Component0391.css";

type Filter = "all" | "active" | "done";

export const Component0391 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0391] mounted");
    return () => console.log("[Component0391] unmounted");
  });
  ...
  return () => html`
    <div class="app-391">
      <header class="header-391">
        <h1>todos — Component0391</h1>
      </header>

      <main class="main-391">
        <input
          class="add-input-391 ${error() ? "add-input--error-391" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-391" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-391">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-391">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-391 ${filter() === f ? "filter-btn--active-391" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-391">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-391">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-391">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-391 ${todo.done ? "todo-item--done-391" : ""}">
                    <button
                      class="todo-check-391 ${todo.done ? "todo-check--done-391" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-391" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-391"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-391" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-391" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-391">
            <span class="todo-count-391">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-391" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
