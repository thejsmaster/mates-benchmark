// client/components/Component0257.ts
// Benchmark component #257 — identical logic to TodoPage, scoped class names.
...
import "./Component0257.css";

type Filter = "all" | "active" | "done";

export const Component0257 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0257] mounted");
    return () => console.log("[Component0257] unmounted");
  });
  ...
  return () => html`
    <div class="app-257">
      <header class="header-257">
        <h1>todos — Component0257</h1>
      </header>

      <main class="main-257">
        <input
          class="add-input-257 ${error() ? "add-input--error-257" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-257" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-257">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-257">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-257 ${filter() === f ? "filter-btn--active-257" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-257">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-257">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-257">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-257 ${todo.done ? "todo-item--done-257" : ""}">
                    <button
                      class="todo-check-257 ${todo.done ? "todo-check--done-257" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-257" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-257"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-257" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-257" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-257">
            <span class="todo-count-257">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-257" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
