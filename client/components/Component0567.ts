// client/components/Component0567.ts
// Benchmark component #567 — identical logic to TodoPage, scoped class names.
...
import "./Component0567.css";

type Filter = "all" | "active" | "done";

export const Component0567 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0567] mounted");
    return () => console.log("[Component0567] unmounted");
  });
  ...
  return () => html`
    <div class="app-567">
      <header class="header-567">
        <h1>todos — Component0567</h1>
      </header>

      <main class="main-567">
        <input
          class="add-input-567 ${error() ? "add-input--error-567" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-567" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-567">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-567">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-567 ${filter() === f ? "filter-btn--active-567" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-567">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-567">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-567">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-567 ${todo.done ? "todo-item--done-567" : ""}">
                    <button
                      class="todo-check-567 ${todo.done ? "todo-check--done-567" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-567" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-567"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-567" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-567" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-567">
            <span class="todo-count-567">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-567" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
