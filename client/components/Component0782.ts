// client/components/Component0782.ts
// Benchmark component #782 — identical logic to TodoPage, scoped class names.
...
import "./Component0782.css";

type Filter = "all" | "active" | "done";

export const Component0782 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0782] mounted");
    return () => console.log("[Component0782] unmounted");
  });
  ...
  return () => html`
    <div class="app-782">
      <header class="header-782">
        <h1>todos — Component0782</h1>
      </header>

      <main class="main-782">
        <input
          class="add-input-782 ${error() ? "add-input--error-782" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-782" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-782">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-782">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-782 ${filter() === f ? "filter-btn--active-782" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-782">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-782">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-782">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-782 ${todo.done ? "todo-item--done-782" : ""}">
                    <button
                      class="todo-check-782 ${todo.done ? "todo-check--done-782" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-782" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-782"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-782" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-782" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-782">
            <span class="todo-count-782">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-782" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
