// client/components/Component0743.ts
// Benchmark component #743 — identical logic to TodoPage, scoped class names.
...
import "./Component0743.css";

type Filter = "all" | "active" | "done";

export const Component0743 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0743] mounted");
    return () => console.log("[Component0743] unmounted");
  });
  ...
  return () => html`
    <div class="app-743">
      <header class="header-743">
        <h1>todos — Component0743</h1>
      </header>

      <main class="main-743">
        <input
          class="add-input-743 ${error() ? "add-input--error-743" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-743" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-743">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-743">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-743 ${filter() === f ? "filter-btn--active-743" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-743">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-743">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-743">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-743 ${todo.done ? "todo-item--done-743" : ""}">
                    <button
                      class="todo-check-743 ${todo.done ? "todo-check--done-743" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-743" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-743"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-743" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-743" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-743">
            <span class="todo-count-743">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-743" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
