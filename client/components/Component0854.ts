// client/components/Component0854.ts
// Benchmark component #854 — identical logic to TodoPage, scoped class names.
...
import "./Component0854.css";

type Filter = "all" | "active" | "done";

export const Component0854 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0854] mounted");
    return () => console.log("[Component0854] unmounted");
  });
  ...
  return () => html`
    <div class="app-854">
      <header class="header-854">
        <h1>todos — Component0854</h1>
      </header>

      <main class="main-854">
        <input
          class="add-input-854 ${error() ? "add-input--error-854" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-854" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-854">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-854">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-854 ${filter() === f ? "filter-btn--active-854" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-854">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-854">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-854">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-854 ${todo.done ? "todo-item--done-854" : ""}">
                    <button
                      class="todo-check-854 ${todo.done ? "todo-check--done-854" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-854" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-854"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-854" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-854" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-854">
            <span class="todo-count-854">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-854" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
