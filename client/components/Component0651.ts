// client/components/Component0651.ts
// Benchmark component #651 — identical logic to TodoPage, scoped class names.
...
import "./Component0651.css";

type Filter = "all" | "active" | "done";

export const Component0651 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0651] mounted");
    return () => console.log("[Component0651] unmounted");
  });
  ...
  return () => html`
    <div class="app-651">
      <header class="header-651">
        <h1>todos — Component0651</h1>
      </header>

      <main class="main-651">
        <input
          class="add-input-651 ${error() ? "add-input--error-651" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-651" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-651">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-651">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-651 ${filter() === f ? "filter-btn--active-651" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-651">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-651">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-651">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-651 ${todo.done ? "todo-item--done-651" : ""}">
                    <button
                      class="todo-check-651 ${todo.done ? "todo-check--done-651" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-651" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-651"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-651" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-651" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-651">
            <span class="todo-count-651">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-651" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
