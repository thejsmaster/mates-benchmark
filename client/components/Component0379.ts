// client/components/Component0379.ts
// Benchmark component #379 — identical logic to TodoPage, scoped class names.
...
import "./Component0379.css";

type Filter = "all" | "active" | "done";

export const Component0379 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0379] mounted");
    return () => console.log("[Component0379] unmounted");
  });
  ...
  return () => html`
    <div class="app-379">
      <header class="header-379">
        <h1>todos — Component0379</h1>
      </header>

      <main class="main-379">
        <input
          class="add-input-379 ${error() ? "add-input--error-379" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-379" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-379">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-379">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-379 ${filter() === f ? "filter-btn--active-379" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-379">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-379">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-379">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-379 ${todo.done ? "todo-item--done-379" : ""}">
                    <button
                      class="todo-check-379 ${todo.done ? "todo-check--done-379" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-379" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-379"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-379" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-379" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-379">
            <span class="todo-count-379">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-379" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
