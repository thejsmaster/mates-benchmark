// client/components/Component0510.ts
// Benchmark component #510 — identical logic to TodoPage, scoped class names.
...
import "./Component0510.css";

type Filter = "all" | "active" | "done";

export const Component0510 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0510] mounted");
    return () => console.log("[Component0510] unmounted");
  });
  ...
  return () => html`
    <div class="app-510">
      <header class="header-510">
        <h1>todos — Component0510</h1>
      </header>

      <main class="main-510">
        <input
          class="add-input-510 ${error() ? "add-input--error-510" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-510" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-510">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-510">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-510 ${filter() === f ? "filter-btn--active-510" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-510">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-510">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-510">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-510 ${todo.done ? "todo-item--done-510" : ""}">
                    <button
                      class="todo-check-510 ${todo.done ? "todo-check--done-510" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-510" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-510"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-510" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-510" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-510">
            <span class="todo-count-510">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-510" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
