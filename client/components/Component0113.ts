// client/components/Component0113.ts
// Benchmark component #113 — identical logic to TodoPage, scoped class names.
...
import "./Component0113.css";

type Filter = "all" | "active" | "done";

export const Component0113 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0113] mounted");
    return () => console.log("[Component0113] unmounted");
  });
  ...
  return () => html`
    <div class="app-113">
      <header class="header-113">
        <h1>todos — Component0113</h1>
      </header>

      <main class="main-113">
        <input
          class="add-input-113 ${error() ? "add-input--error-113" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-113" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-113">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-113">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-113 ${filter() === f ? "filter-btn--active-113" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-113">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-113">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-113">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-113 ${todo.done ? "todo-item--done-113" : ""}">
                    <button
                      class="todo-check-113 ${todo.done ? "todo-check--done-113" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-113" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-113"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-113" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-113" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-113">
            <span class="todo-count-113">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-113" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
