// client/components/Component0331.ts
// Benchmark component #331 — identical logic to TodoPage, scoped class names.
...
import "./Component0331.css";

type Filter = "all" | "active" | "done";

export const Component0331 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0331] mounted");
    return () => console.log("[Component0331] unmounted");
  });
  ...
  return () => html`
    <div class="app-331">
      <header class="header-331">
        <h1>todos — Component0331</h1>
      </header>

      <main class="main-331">
        <input
          class="add-input-331 ${error() ? "add-input--error-331" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-331" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-331">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-331">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-331 ${filter() === f ? "filter-btn--active-331" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-331">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-331">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-331">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-331 ${todo.done ? "todo-item--done-331" : ""}">
                    <button
                      class="todo-check-331 ${todo.done ? "todo-check--done-331" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-331" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-331"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-331" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-331" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-331">
            <span class="todo-count-331">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-331" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
