// client/components/Component0849.ts
// Benchmark component #849 — identical logic to TodoPage, scoped class names.
...
import "./Component0849.css";

type Filter = "all" | "active" | "done";

export const Component0849 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0849] mounted");
    return () => console.log("[Component0849] unmounted");
  });
  ...
  return () => html`
    <div class="app-849">
      <header class="header-849">
        <h1>todos — Component0849</h1>
      </header>

      <main class="main-849">
        <input
          class="add-input-849 ${error() ? "add-input--error-849" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-849" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-849">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-849">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-849 ${filter() === f ? "filter-btn--active-849" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-849">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-849">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-849">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-849 ${todo.done ? "todo-item--done-849" : ""}">
                    <button
                      class="todo-check-849 ${todo.done ? "todo-check--done-849" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-849" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-849"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-849" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-849" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-849">
            <span class="todo-count-849">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-849" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
