// client/components/Component0736.ts
// Benchmark component #736 — identical logic to TodoPage, scoped class names.
...
import "./Component0736.css";

type Filter = "all" | "active" | "done";

export const Component0736 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0736] mounted");
    return () => console.log("[Component0736] unmounted");
  });
  ...
  return () => html`
    <div class="app-736">
      <header class="header-736">
        <h1>todos — Component0736</h1>
      </header>

      <main class="main-736">
        <input
          class="add-input-736 ${error() ? "add-input--error-736" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-736" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-736">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-736">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-736 ${filter() === f ? "filter-btn--active-736" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-736">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-736">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-736">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-736 ${todo.done ? "todo-item--done-736" : ""}">
                    <button
                      class="todo-check-736 ${todo.done ? "todo-check--done-736" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-736" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-736"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-736" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-736" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-736">
            <span class="todo-count-736">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-736" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
