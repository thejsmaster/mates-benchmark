// client/components/Component0118.ts
// Benchmark component #118 — identical logic to TodoPage, scoped class names.
...
import "./Component0118.css";

type Filter = "all" | "active" | "done";

export const Component0118 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0118] mounted");
    return () => console.log("[Component0118] unmounted");
  });
  ...
  return () => html`
    <div class="app-118">
      <header class="header-118">
        <h1>todos — Component0118</h1>
      </header>

      <main class="main-118">
        <input
          class="add-input-118 ${error() ? "add-input--error-118" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-118" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-118">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-118">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-118 ${filter() === f ? "filter-btn--active-118" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-118">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-118">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-118">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-118 ${todo.done ? "todo-item--done-118" : ""}">
                    <button
                      class="todo-check-118 ${todo.done ? "todo-check--done-118" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-118" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-118"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-118" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-118" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-118">
            <span class="todo-count-118">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-118" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
