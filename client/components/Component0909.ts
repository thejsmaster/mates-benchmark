// client/components/Component0909.ts
// Benchmark component #909 — identical logic to TodoPage, scoped class names.
...
import "./Component0909.css";

type Filter = "all" | "active" | "done";

export const Component0909 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0909] mounted");
    return () => console.log("[Component0909] unmounted");
  });
  ...
  return () => html`
    <div class="app-909">
      <header class="header-909">
        <h1>todos — Component0909</h1>
      </header>

      <main class="main-909">
        <input
          class="add-input-909 ${error() ? "add-input--error-909" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-909" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-909">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-909">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-909 ${filter() === f ? "filter-btn--active-909" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-909">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-909">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-909">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-909 ${todo.done ? "todo-item--done-909" : ""}">
                    <button
                      class="todo-check-909 ${todo.done ? "todo-check--done-909" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-909" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-909"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-909" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-909" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-909">
            <span class="todo-count-909">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-909" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
