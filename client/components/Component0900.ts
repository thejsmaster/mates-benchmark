// client/components/Component0900.ts
// Benchmark component #900 — identical logic to TodoPage, scoped class names.
...
import "./Component0900.css";

type Filter = "all" | "active" | "done";

export const Component0900 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0900] mounted");
    return () => console.log("[Component0900] unmounted");
  });
  ...
  return () => html`
    <div class="app-900">
      <header class="header-900">
        <h1>todos — Component0900</h1>
      </header>

      <main class="main-900">
        <input
          class="add-input-900 ${error() ? "add-input--error-900" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-900" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-900">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-900">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-900 ${filter() === f ? "filter-btn--active-900" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-900">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-900">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-900">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-900 ${todo.done ? "todo-item--done-900" : ""}">
                    <button
                      class="todo-check-900 ${todo.done ? "todo-check--done-900" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-900" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-900"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-900" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-900" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-900">
            <span class="todo-count-900">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-900" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
