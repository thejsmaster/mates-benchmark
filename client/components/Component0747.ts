// client/components/Component0747.ts
// Benchmark component #747 — identical logic to TodoPage, scoped class names.
...
import "./Component0747.css";

type Filter = "all" | "active" | "done";

export const Component0747 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0747] mounted");
    return () => console.log("[Component0747] unmounted");
  });
  ...
  return () => html`
    <div class="app-747">
      <header class="header-747">
        <h1>todos — Component0747</h1>
      </header>

      <main class="main-747">
        <input
          class="add-input-747 ${error() ? "add-input--error-747" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-747" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-747">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-747">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-747 ${filter() === f ? "filter-btn--active-747" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-747">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-747">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-747">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-747 ${todo.done ? "todo-item--done-747" : ""}">
                    <button
                      class="todo-check-747 ${todo.done ? "todo-check--done-747" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-747" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-747"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-747" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-747" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-747">
            <span class="todo-count-747">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-747" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
