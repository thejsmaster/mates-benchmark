// client/components/Component0501.ts
// Benchmark component #501 — identical logic to TodoPage, scoped class names.
...
import "./Component0501.css";

type Filter = "all" | "active" | "done";

export const Component0501 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0501] mounted");
    return () => console.log("[Component0501] unmounted");
  });
  ...
  return () => html`
    <div class="app-501">
      <header class="header-501">
        <h1>todos — Component0501</h1>
      </header>

      <main class="main-501">
        <input
          class="add-input-501 ${error() ? "add-input--error-501" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-501" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-501">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-501">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-501 ${filter() === f ? "filter-btn--active-501" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-501">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-501">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-501">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-501 ${todo.done ? "todo-item--done-501" : ""}">
                    <button
                      class="todo-check-501 ${todo.done ? "todo-check--done-501" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-501" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-501"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-501" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-501" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-501">
            <span class="todo-count-501">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-501" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
