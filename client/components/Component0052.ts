// client/components/Component0052.ts
// Benchmark component #52 — identical logic to TodoPage, scoped class names.
...
import "./Component0052.css";

type Filter = "all" | "active" | "done";

export const Component0052 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0052] mounted");
    return () => console.log("[Component0052] unmounted");
  });
  ...
  return () => html`
    <div class="app-52">
      <header class="header-52">
        <h1>todos — Component0052</h1>
      </header>

      <main class="main-52">
        <input
          class="add-input-52 ${error() ? "add-input--error-52" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-52" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-52">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-52">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-52 ${filter() === f ? "filter-btn--active-52" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-52">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-52">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-52">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-52 ${todo.done ? "todo-item--done-52" : ""}">
                    <button
                      class="todo-check-52 ${todo.done ? "todo-check--done-52" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-52" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-52"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-52" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-52" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-52">
            <span class="todo-count-52">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-52" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
