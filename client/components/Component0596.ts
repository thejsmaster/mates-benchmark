// client/components/Component0596.ts
// Benchmark component #596 — identical logic to TodoPage, scoped class names.
...
import "./Component0596.css";

type Filter = "all" | "active" | "done";

export const Component0596 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0596] mounted");
    return () => console.log("[Component0596] unmounted");
  });
  ...
  return () => html`
    <div class="app-596">
      <header class="header-596">
        <h1>todos — Component0596</h1>
      </header>

      <main class="main-596">
        <input
          class="add-input-596 ${error() ? "add-input--error-596" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-596" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-596">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-596">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-596 ${filter() === f ? "filter-btn--active-596" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-596">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-596">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-596">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-596 ${todo.done ? "todo-item--done-596" : ""}">
                    <button
                      class="todo-check-596 ${todo.done ? "todo-check--done-596" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-596" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-596"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-596" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-596" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-596">
            <span class="todo-count-596">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-596" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
