// client/components/Component0246.ts
// Benchmark component #246 — identical logic to TodoPage, scoped class names.
...
import "./Component0246.css";

type Filter = "all" | "active" | "done";

export const Component0246 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0246] mounted");
    return () => console.log("[Component0246] unmounted");
  });
  ...
  return () => html`
    <div class="app-246">
      <header class="header-246">
        <h1>todos — Component0246</h1>
      </header>

      <main class="main-246">
        <input
          class="add-input-246 ${error() ? "add-input--error-246" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-246" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-246">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-246">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-246 ${filter() === f ? "filter-btn--active-246" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-246">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-246">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-246">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-246 ${todo.done ? "todo-item--done-246" : ""}">
                    <button
                      class="todo-check-246 ${todo.done ? "todo-check--done-246" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-246" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-246"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-246" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-246" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-246">
            <span class="todo-count-246">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-246" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
