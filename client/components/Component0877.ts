// client/components/Component0877.ts
// Benchmark component #877 — identical logic to TodoPage, scoped class names.
...
import "./Component0877.css";

type Filter = "all" | "active" | "done";

export const Component0877 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0877] mounted");
    return () => console.log("[Component0877] unmounted");
  });
  ...
  return () => html`
    <div class="app-877">
      <header class="header-877">
        <h1>todos — Component0877</h1>
      </header>

      <main class="main-877">
        <input
          class="add-input-877 ${error() ? "add-input--error-877" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-877" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-877">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-877">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-877 ${filter() === f ? "filter-btn--active-877" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-877">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-877">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-877">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-877 ${todo.done ? "todo-item--done-877" : ""}">
                    <button
                      class="todo-check-877 ${todo.done ? "todo-check--done-877" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-877" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-877"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-877" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-877" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-877">
            <span class="todo-count-877">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-877" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
