// client/components/Component0285.ts
// Benchmark component #285 — identical logic to TodoPage, scoped class names.
...
import "./Component0285.css";

type Filter = "all" | "active" | "done";

export const Component0285 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0285] mounted");
    return () => console.log("[Component0285] unmounted");
  });
  ...
  return () => html`
    <div class="app-285">
      <header class="header-285">
        <h1>todos — Component0285</h1>
      </header>

      <main class="main-285">
        <input
          class="add-input-285 ${error() ? "add-input--error-285" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-285" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-285">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-285">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-285 ${filter() === f ? "filter-btn--active-285" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-285">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-285">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-285">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-285 ${todo.done ? "todo-item--done-285" : ""}">
                    <button
                      class="todo-check-285 ${todo.done ? "todo-check--done-285" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-285" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-285"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-285" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-285" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-285">
            <span class="todo-count-285">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-285" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
