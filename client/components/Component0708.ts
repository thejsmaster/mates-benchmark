// client/components/Component0708.ts
// Benchmark component #708 — identical logic to TodoPage, scoped class names.
...
import "./Component0708.css";

type Filter = "all" | "active" | "done";

export const Component0708 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0708] mounted");
    return () => console.log("[Component0708] unmounted");
  });
  ...
  return () => html`
    <div class="app-708">
      <header class="header-708">
        <h1>todos — Component0708</h1>
      </header>

      <main class="main-708">
        <input
          class="add-input-708 ${error() ? "add-input--error-708" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-708" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-708">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-708">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-708 ${filter() === f ? "filter-btn--active-708" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-708">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-708">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-708">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-708 ${todo.done ? "todo-item--done-708" : ""}">
                    <button
                      class="todo-check-708 ${todo.done ? "todo-check--done-708" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-708" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-708"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-708" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-708" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-708">
            <span class="todo-count-708">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-708" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
