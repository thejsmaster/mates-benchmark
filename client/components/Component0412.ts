// client/components/Component0412.ts
// Benchmark component #412 — identical logic to TodoPage, scoped class names.
...
import "./Component0412.css";

type Filter = "all" | "active" | "done";

export const Component0412 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0412] mounted");
    return () => console.log("[Component0412] unmounted");
  });
  ...
  return () => html`
    <div class="app-412">
      <header class="header-412">
        <h1>todos — Component0412</h1>
      </header>

      <main class="main-412">
        <input
          class="add-input-412 ${error() ? "add-input--error-412" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-412" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-412">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-412">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-412 ${filter() === f ? "filter-btn--active-412" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-412">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-412">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-412">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-412 ${todo.done ? "todo-item--done-412" : ""}">
                    <button
                      class="todo-check-412 ${todo.done ? "todo-check--done-412" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-412" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-412"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-412" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-412" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-412">
            <span class="todo-count-412">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-412" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
