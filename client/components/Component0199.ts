// client/components/Component0199.ts
// Benchmark component #199 — identical logic to TodoPage, scoped class names.
...
import "./Component0199.css";

type Filter = "all" | "active" | "done";

export const Component0199 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0199] mounted");
    return () => console.log("[Component0199] unmounted");
  });
  ...
  return () => html`
    <div class="app-199">
      <header class="header-199">
        <h1>todos — Component0199</h1>
      </header>

      <main class="main-199">
        <input
          class="add-input-199 ${error() ? "add-input--error-199" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-199" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-199">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-199">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-199 ${filter() === f ? "filter-btn--active-199" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-199">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-199">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-199">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-199 ${todo.done ? "todo-item--done-199" : ""}">
                    <button
                      class="todo-check-199 ${todo.done ? "todo-check--done-199" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-199" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-199"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-199" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-199" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-199">
            <span class="todo-count-199">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-199" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
