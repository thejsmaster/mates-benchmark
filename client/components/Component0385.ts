// client/components/Component0385.ts
// Benchmark component #385 — identical logic to TodoPage, scoped class names.
...
import "./Component0385.css";

type Filter = "all" | "active" | "done";

export const Component0385 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0385] mounted");
    return () => console.log("[Component0385] unmounted");
  });
  ...
  return () => html`
    <div class="app-385">
      <header class="header-385">
        <h1>todos — Component0385</h1>
      </header>

      <main class="main-385">
        <input
          class="add-input-385 ${error() ? "add-input--error-385" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-385" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-385">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-385">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-385 ${filter() === f ? "filter-btn--active-385" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-385">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-385">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-385">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-385 ${todo.done ? "todo-item--done-385" : ""}">
                    <button
                      class="todo-check-385 ${todo.done ? "todo-check--done-385" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-385" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-385"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-385" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-385" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-385">
            <span class="todo-count-385">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-385" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
