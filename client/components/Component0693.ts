// client/components/Component0693.ts
// Benchmark component #693 — identical logic to TodoPage, scoped class names.
...
import "./Component0693.css";

type Filter = "all" | "active" | "done";

export const Component0693 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0693] mounted");
    return () => console.log("[Component0693] unmounted");
  });
  ...
  return () => html`
    <div class="app-693">
      <header class="header-693">
        <h1>todos — Component0693</h1>
      </header>

      <main class="main-693">
        <input
          class="add-input-693 ${error() ? "add-input--error-693" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-693" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-693">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-693">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-693 ${filter() === f ? "filter-btn--active-693" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-693">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-693">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-693">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-693 ${todo.done ? "todo-item--done-693" : ""}">
                    <button
                      class="todo-check-693 ${todo.done ? "todo-check--done-693" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-693" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-693"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-693" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-693" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-693">
            <span class="todo-count-693">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-693" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
