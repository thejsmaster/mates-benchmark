// client/components/Component0200.ts
// Benchmark component #200 — identical logic to TodoPage, scoped class names.
...
import "./Component0200.css";

type Filter = "all" | "active" | "done";

export const Component0200 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0200] mounted");
    return () => console.log("[Component0200] unmounted");
  });
  ...
  return () => html`
    <div class="app-200">
      <header class="header-200">
        <h1>todos — Component0200</h1>
      </header>

      <main class="main-200">
        <input
          class="add-input-200 ${error() ? "add-input--error-200" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-200" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-200">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-200">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-200 ${filter() === f ? "filter-btn--active-200" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-200">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-200">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-200">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-200 ${todo.done ? "todo-item--done-200" : ""}">
                    <button
                      class="todo-check-200 ${todo.done ? "todo-check--done-200" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-200" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-200"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-200" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-200" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-200">
            <span class="todo-count-200">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-200" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
