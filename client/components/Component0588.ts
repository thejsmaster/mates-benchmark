// client/components/Component0588.ts
// Benchmark component #588 — identical logic to TodoPage, scoped class names.
...
import "./Component0588.css";

type Filter = "all" | "active" | "done";

export const Component0588 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0588] mounted");
    return () => console.log("[Component0588] unmounted");
  });
  ...
  return () => html`
    <div class="app-588">
      <header class="header-588">
        <h1>todos — Component0588</h1>
      </header>

      <main class="main-588">
        <input
          class="add-input-588 ${error() ? "add-input--error-588" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-588" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-588">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-588">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-588 ${filter() === f ? "filter-btn--active-588" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-588">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-588">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-588">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-588 ${todo.done ? "todo-item--done-588" : ""}">
                    <button
                      class="todo-check-588 ${todo.done ? "todo-check--done-588" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-588" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-588"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-588" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-588" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-588">
            <span class="todo-count-588">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-588" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
