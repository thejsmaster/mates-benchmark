// client/components/Component0090.ts
// Benchmark component #90 — identical logic to TodoPage, scoped class names.
...
import "./Component0090.css";

type Filter = "all" | "active" | "done";

export const Component0090 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0090] mounted");
    return () => console.log("[Component0090] unmounted");
  });
  ...
  return () => html`
    <div class="app-90">
      <header class="header-90">
        <h1>todos — Component0090</h1>
      </header>

      <main class="main-90">
        <input
          class="add-input-90 ${error() ? "add-input--error-90" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-90" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-90">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-90">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-90 ${filter() === f ? "filter-btn--active-90" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-90">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-90">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-90">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-90 ${todo.done ? "todo-item--done-90" : ""}">
                    <button
                      class="todo-check-90 ${todo.done ? "todo-check--done-90" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-90" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-90"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-90" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-90" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-90">
            <span class="todo-count-90">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-90" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
