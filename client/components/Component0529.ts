// client/components/Component0529.ts
// Benchmark component #529 — identical logic to TodoPage, scoped class names.
...
import "./Component0529.css";

type Filter = "all" | "active" | "done";

export const Component0529 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0529] mounted");
    return () => console.log("[Component0529] unmounted");
  });
  ...
  return () => html`
    <div class="app-529">
      <header class="header-529">
        <h1>todos — Component0529</h1>
      </header>

      <main class="main-529">
        <input
          class="add-input-529 ${error() ? "add-input--error-529" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-529" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-529">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-529">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-529 ${filter() === f ? "filter-btn--active-529" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-529">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-529">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-529">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-529 ${todo.done ? "todo-item--done-529" : ""}">
                    <button
                      class="todo-check-529 ${todo.done ? "todo-check--done-529" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-529" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-529"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-529" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-529" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-529">
            <span class="todo-count-529">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-529" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
