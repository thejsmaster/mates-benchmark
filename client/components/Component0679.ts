// client/components/Component0679.ts
// Benchmark component #679 — identical logic to TodoPage, scoped class names.
...
import "./Component0679.css";

type Filter = "all" | "active" | "done";

export const Component0679 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0679] mounted");
    return () => console.log("[Component0679] unmounted");
  });
  ...
  return () => html`
    <div class="app-679">
      <header class="header-679">
        <h1>todos — Component0679</h1>
      </header>

      <main class="main-679">
        <input
          class="add-input-679 ${error() ? "add-input--error-679" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-679" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-679">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-679">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-679 ${filter() === f ? "filter-btn--active-679" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-679">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-679">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-679">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-679 ${todo.done ? "todo-item--done-679" : ""}">
                    <button
                      class="todo-check-679 ${todo.done ? "todo-check--done-679" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-679" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-679"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-679" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-679" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-679">
            <span class="todo-count-679">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-679" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
