// client/components/Component0552.ts
// Benchmark component #552 — identical logic to TodoPage, scoped class names.
...
import "./Component0552.css";

type Filter = "all" | "active" | "done";

export const Component0552 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0552] mounted");
    return () => console.log("[Component0552] unmounted");
  });
  ...
  return () => html`
    <div class="app-552">
      <header class="header-552">
        <h1>todos — Component0552</h1>
      </header>

      <main class="main-552">
        <input
          class="add-input-552 ${error() ? "add-input--error-552" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-552" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-552">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-552">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-552 ${filter() === f ? "filter-btn--active-552" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-552">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-552">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-552">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-552 ${todo.done ? "todo-item--done-552" : ""}">
                    <button
                      class="todo-check-552 ${todo.done ? "todo-check--done-552" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-552" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-552"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-552" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-552" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-552">
            <span class="todo-count-552">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-552" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
