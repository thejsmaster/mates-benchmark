// client/components/Component0538.ts
// Benchmark component #538 — identical logic to TodoPage, scoped class names.
...
import "./Component0538.css";

type Filter = "all" | "active" | "done";

export const Component0538 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0538] mounted");
    return () => console.log("[Component0538] unmounted");
  });
  ...
  return () => html`
    <div class="app-538">
      <header class="header-538">
        <h1>todos — Component0538</h1>
      </header>

      <main class="main-538">
        <input
          class="add-input-538 ${error() ? "add-input--error-538" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-538" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-538">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-538">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-538 ${filter() === f ? "filter-btn--active-538" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-538">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-538">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-538">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-538 ${todo.done ? "todo-item--done-538" : ""}">
                    <button
                      class="todo-check-538 ${todo.done ? "todo-check--done-538" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-538" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-538"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-538" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-538" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-538">
            <span class="todo-count-538">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-538" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
