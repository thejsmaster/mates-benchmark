// client/components/Component0959.ts
// Benchmark component #959 — identical logic to TodoPage, scoped class names.
...
import "./Component0959.css";

type Filter = "all" | "active" | "done";

export const Component0959 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0959] mounted");
    return () => console.log("[Component0959] unmounted");
  });
  ...
  return () => html`
    <div class="app-959">
      <header class="header-959">
        <h1>todos — Component0959</h1>
      </header>

      <main class="main-959">
        <input
          class="add-input-959 ${error() ? "add-input--error-959" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-959" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-959">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-959">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-959 ${filter() === f ? "filter-btn--active-959" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-959">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-959">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-959">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-959 ${todo.done ? "todo-item--done-959" : ""}">
                    <button
                      class="todo-check-959 ${todo.done ? "todo-check--done-959" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-959" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-959"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-959" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-959" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-959">
            <span class="todo-count-959">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-959" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
