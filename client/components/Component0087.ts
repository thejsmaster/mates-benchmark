// client/components/Component0087.ts
// Benchmark component #87 — identical logic to TodoPage, scoped class names.
...
import "./Component0087.css";

type Filter = "all" | "active" | "done";

export const Component0087 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0087] mounted");
    return () => console.log("[Component0087] unmounted");
  });
  ...
  return () => html`
    <div class="app-87">
      <header class="header-87">
        <h1>todos — Component0087</h1>
      </header>

      <main class="main-87">
        <input
          class="add-input-87 ${error() ? "add-input--error-87" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-87" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-87">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-87">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-87 ${filter() === f ? "filter-btn--active-87" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-87">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-87">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-87">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-87 ${todo.done ? "todo-item--done-87" : ""}">
                    <button
                      class="todo-check-87 ${todo.done ? "todo-check--done-87" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-87" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-87"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-87" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-87" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-87">
            <span class="todo-count-87">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-87" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
