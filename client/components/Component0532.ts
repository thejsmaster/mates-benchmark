// client/components/Component0532.ts
// Benchmark component #532 — identical logic to TodoPage, scoped class names.
...
import "./Component0532.css";

type Filter = "all" | "active" | "done";

export const Component0532 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0532] mounted");
    return () => console.log("[Component0532] unmounted");
  });
  ...
  return () => html`
    <div class="app-532">
      <header class="header-532">
        <h1>todos — Component0532</h1>
      </header>

      <main class="main-532">
        <input
          class="add-input-532 ${error() ? "add-input--error-532" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-532" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-532">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-532">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-532 ${filter() === f ? "filter-btn--active-532" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-532">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-532">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-532">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-532 ${todo.done ? "todo-item--done-532" : ""}">
                    <button
                      class="todo-check-532 ${todo.done ? "todo-check--done-532" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-532" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-532"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-532" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-532" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-532">
            <span class="todo-count-532">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-532" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
