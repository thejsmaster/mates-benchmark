// client/components/Component0409.ts
// Benchmark component #409 — identical logic to TodoPage, scoped class names.
...
import "./Component0409.css";

type Filter = "all" | "active" | "done";

export const Component0409 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0409] mounted");
    return () => console.log("[Component0409] unmounted");
  });
  ...
  return () => html`
    <div class="app-409">
      <header class="header-409">
        <h1>todos — Component0409</h1>
      </header>

      <main class="main-409">
        <input
          class="add-input-409 ${error() ? "add-input--error-409" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-409" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-409">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-409">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-409 ${filter() === f ? "filter-btn--active-409" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-409">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-409">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-409">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-409 ${todo.done ? "todo-item--done-409" : ""}">
                    <button
                      class="todo-check-409 ${todo.done ? "todo-check--done-409" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-409" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-409"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-409" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-409" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-409">
            <span class="todo-count-409">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-409" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
