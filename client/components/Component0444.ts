// client/components/Component0444.ts
// Benchmark component #444 — identical logic to TodoPage, scoped class names.
...
import "./Component0444.css";

type Filter = "all" | "active" | "done";

export const Component0444 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0444] mounted");
    return () => console.log("[Component0444] unmounted");
  });
  ...
  return () => html`
    <div class="app-444">
      <header class="header-444">
        <h1>todos — Component0444</h1>
      </header>

      <main class="main-444">
        <input
          class="add-input-444 ${error() ? "add-input--error-444" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-444" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-444">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-444">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-444 ${filter() === f ? "filter-btn--active-444" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-444">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-444">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-444">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-444 ${todo.done ? "todo-item--done-444" : ""}">
                    <button
                      class="todo-check-444 ${todo.done ? "todo-check--done-444" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-444" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-444"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-444" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-444" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-444">
            <span class="todo-count-444">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-444" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
