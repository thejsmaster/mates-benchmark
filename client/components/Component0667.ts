// client/components/Component0667.ts
// Benchmark component #667 — identical logic to TodoPage, scoped class names.
...
import "./Component0667.css";

type Filter = "all" | "active" | "done";

export const Component0667 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0667] mounted");
    return () => console.log("[Component0667] unmounted");
  });
  ...
  return () => html`
    <div class="app-667">
      <header class="header-667">
        <h1>todos — Component0667</h1>
      </header>

      <main class="main-667">
        <input
          class="add-input-667 ${error() ? "add-input--error-667" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-667" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-667">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-667">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-667 ${filter() === f ? "filter-btn--active-667" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-667">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-667">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-667">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-667 ${todo.done ? "todo-item--done-667" : ""}">
                    <button
                      class="todo-check-667 ${todo.done ? "todo-check--done-667" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-667" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-667"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-667" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-667" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-667">
            <span class="todo-count-667">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-667" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
