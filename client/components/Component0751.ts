// client/components/Component0751.ts
// Benchmark component #751 — identical logic to TodoPage, scoped class names.
...
import "./Component0751.css";

type Filter = "all" | "active" | "done";

export const Component0751 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0751] mounted");
    return () => console.log("[Component0751] unmounted");
  });
  ...
  return () => html`
    <div class="app-751">
      <header class="header-751">
        <h1>todos — Component0751</h1>
      </header>

      <main class="main-751">
        <input
          class="add-input-751 ${error() ? "add-input--error-751" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-751" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-751">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-751">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-751 ${filter() === f ? "filter-btn--active-751" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-751">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-751">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-751">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-751 ${todo.done ? "todo-item--done-751" : ""}">
                    <button
                      class="todo-check-751 ${todo.done ? "todo-check--done-751" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-751" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-751"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-751" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-751" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-751">
            <span class="todo-count-751">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-751" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
