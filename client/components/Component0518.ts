// client/components/Component0518.ts
// Benchmark component #518 — identical logic to TodoPage, scoped class names.
...
import "./Component0518.css";

type Filter = "all" | "active" | "done";

export const Component0518 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0518] mounted");
    return () => console.log("[Component0518] unmounted");
  });
  ...
  return () => html`
    <div class="app-518">
      <header class="header-518">
        <h1>todos — Component0518</h1>
      </header>

      <main class="main-518">
        <input
          class="add-input-518 ${error() ? "add-input--error-518" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-518" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-518">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-518">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-518 ${filter() === f ? "filter-btn--active-518" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-518">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-518">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-518">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-518 ${todo.done ? "todo-item--done-518" : ""}">
                    <button
                      class="todo-check-518 ${todo.done ? "todo-check--done-518" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-518" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-518"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-518" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-518" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-518">
            <span class="todo-count-518">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-518" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
