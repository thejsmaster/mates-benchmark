// client/components/Component0656.ts
// Benchmark component #656 — identical logic to TodoPage, scoped class names.
...
import "./Component0656.css";

type Filter = "all" | "active" | "done";

export const Component0656 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0656] mounted");
    return () => console.log("[Component0656] unmounted");
  });
  ...
  return () => html`
    <div class="app-656">
      <header class="header-656">
        <h1>todos — Component0656</h1>
      </header>

      <main class="main-656">
        <input
          class="add-input-656 ${error() ? "add-input--error-656" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-656" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-656">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-656">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-656 ${filter() === f ? "filter-btn--active-656" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-656">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-656">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-656">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-656 ${todo.done ? "todo-item--done-656" : ""}">
                    <button
                      class="todo-check-656 ${todo.done ? "todo-check--done-656" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-656" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-656"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-656" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-656" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-656">
            <span class="todo-count-656">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-656" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
