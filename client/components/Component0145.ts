// client/components/Component0145.ts
// Benchmark component #145 — identical logic to TodoPage, scoped class names.
...
import "./Component0145.css";

type Filter = "all" | "active" | "done";

export const Component0145 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0145] mounted");
    return () => console.log("[Component0145] unmounted");
  });
  ...
  return () => html`
    <div class="app-145">
      <header class="header-145">
        <h1>todos — Component0145</h1>
      </header>

      <main class="main-145">
        <input
          class="add-input-145 ${error() ? "add-input--error-145" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-145" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-145">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-145">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-145 ${filter() === f ? "filter-btn--active-145" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-145">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-145">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-145">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-145 ${todo.done ? "todo-item--done-145" : ""}">
                    <button
                      class="todo-check-145 ${todo.done ? "todo-check--done-145" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-145" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-145"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-145" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-145" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-145">
            <span class="todo-count-145">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-145" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
