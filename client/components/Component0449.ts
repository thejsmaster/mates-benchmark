// client/components/Component0449.ts
// Benchmark component #449 — identical logic to TodoPage, scoped class names.
...
import "./Component0449.css";

type Filter = "all" | "active" | "done";

export const Component0449 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0449] mounted");
    return () => console.log("[Component0449] unmounted");
  });
  ...
  return () => html`
    <div class="app-449">
      <header class="header-449">
        <h1>todos — Component0449</h1>
      </header>

      <main class="main-449">
        <input
          class="add-input-449 ${error() ? "add-input--error-449" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-449" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-449">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-449">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-449 ${filter() === f ? "filter-btn--active-449" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-449">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-449">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-449">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-449 ${todo.done ? "todo-item--done-449" : ""}">
                    <button
                      class="todo-check-449 ${todo.done ? "todo-check--done-449" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-449" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-449"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-449" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-449" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-449">
            <span class="todo-count-449">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-449" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
