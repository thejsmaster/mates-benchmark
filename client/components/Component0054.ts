// client/components/Component0054.ts
// Benchmark component #54 — identical logic to TodoPage, scoped class names.
...
import "./Component0054.css";

type Filter = "all" | "active" | "done";

export const Component0054 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0054] mounted");
    return () => console.log("[Component0054] unmounted");
  });
  ...
  return () => html`
    <div class="app-54">
      <header class="header-54">
        <h1>todos — Component0054</h1>
      </header>

      <main class="main-54">
        <input
          class="add-input-54 ${error() ? "add-input--error-54" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-54" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-54">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-54">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-54 ${filter() === f ? "filter-btn--active-54" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-54">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-54">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-54">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-54 ${todo.done ? "todo-item--done-54" : ""}">
                    <button
                      class="todo-check-54 ${todo.done ? "todo-check--done-54" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-54" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-54"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-54" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-54" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-54">
            <span class="todo-count-54">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-54" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
