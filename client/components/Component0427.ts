// client/components/Component0427.ts
// Benchmark component #427 — identical logic to TodoPage, scoped class names.
...
import "./Component0427.css";

type Filter = "all" | "active" | "done";

export const Component0427 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0427] mounted");
    return () => console.log("[Component0427] unmounted");
  });
  ...
  return () => html`
    <div class="app-427">
      <header class="header-427">
        <h1>todos — Component0427</h1>
      </header>

      <main class="main-427">
        <input
          class="add-input-427 ${error() ? "add-input--error-427" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-427" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-427">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-427">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-427 ${filter() === f ? "filter-btn--active-427" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-427">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-427">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-427">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-427 ${todo.done ? "todo-item--done-427" : ""}">
                    <button
                      class="todo-check-427 ${todo.done ? "todo-check--done-427" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-427" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-427"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-427" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-427" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-427">
            <span class="todo-count-427">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-427" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
