// client/components/Component0167.ts
// Benchmark component #167 — identical logic to TodoPage, scoped class names.
...
import "./Component0167.css";

type Filter = "all" | "active" | "done";

export const Component0167 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0167] mounted");
    return () => console.log("[Component0167] unmounted");
  });
  ...
  return () => html`
    <div class="app-167">
      <header class="header-167">
        <h1>todos — Component0167</h1>
      </header>

      <main class="main-167">
        <input
          class="add-input-167 ${error() ? "add-input--error-167" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-167" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-167">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-167">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-167 ${filter() === f ? "filter-btn--active-167" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-167">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-167">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-167">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-167 ${todo.done ? "todo-item--done-167" : ""}">
                    <button
                      class="todo-check-167 ${todo.done ? "todo-check--done-167" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-167" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-167"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-167" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-167" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-167">
            <span class="todo-count-167">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-167" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
