// client/components/Component0316.ts
// Benchmark component #316 — identical logic to TodoPage, scoped class names.
...
import "./Component0316.css";

type Filter = "all" | "active" | "done";

export const Component0316 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0316] mounted");
    return () => console.log("[Component0316] unmounted");
  });
  ...
  return () => html`
    <div class="app-316">
      <header class="header-316">
        <h1>todos — Component0316</h1>
      </header>

      <main class="main-316">
        <input
          class="add-input-316 ${error() ? "add-input--error-316" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-316" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-316">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-316">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-316 ${filter() === f ? "filter-btn--active-316" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-316">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-316">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-316">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-316 ${todo.done ? "todo-item--done-316" : ""}">
                    <button
                      class="todo-check-316 ${todo.done ? "todo-check--done-316" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-316" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-316"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-316" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-316" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-316">
            <span class="todo-count-316">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-316" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
