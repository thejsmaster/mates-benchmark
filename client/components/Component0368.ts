// client/components/Component0368.ts
// Benchmark component #368 — identical logic to TodoPage, scoped class names.
...
import "./Component0368.css";

type Filter = "all" | "active" | "done";

export const Component0368 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0368] mounted");
    return () => console.log("[Component0368] unmounted");
  });
  ...
  return () => html`
    <div class="app-368">
      <header class="header-368">
        <h1>todos — Component0368</h1>
      </header>

      <main class="main-368">
        <input
          class="add-input-368 ${error() ? "add-input--error-368" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-368" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-368">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-368">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-368 ${filter() === f ? "filter-btn--active-368" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-368">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-368">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-368">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-368 ${todo.done ? "todo-item--done-368" : ""}">
                    <button
                      class="todo-check-368 ${todo.done ? "todo-check--done-368" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-368" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-368"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-368" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-368" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-368">
            <span class="todo-count-368">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-368" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
