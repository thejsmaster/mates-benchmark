// client/components/Component0644.ts
// Benchmark component #644 — identical logic to TodoPage, scoped class names.
...
import "./Component0644.css";

type Filter = "all" | "active" | "done";

export const Component0644 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0644] mounted");
    return () => console.log("[Component0644] unmounted");
  });
  ...
  return () => html`
    <div class="app-644">
      <header class="header-644">
        <h1>todos — Component0644</h1>
      </header>

      <main class="main-644">
        <input
          class="add-input-644 ${error() ? "add-input--error-644" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-644" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-644">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-644">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-644 ${filter() === f ? "filter-btn--active-644" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-644">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-644">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-644">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-644 ${todo.done ? "todo-item--done-644" : ""}">
                    <button
                      class="todo-check-644 ${todo.done ? "todo-check--done-644" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-644" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-644"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-644" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-644" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-644">
            <span class="todo-count-644">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-644" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
