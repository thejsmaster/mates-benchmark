// client/components/Component0923.ts
// Benchmark component #923 — identical logic to TodoPage, scoped class names.
...
import "./Component0923.css";

type Filter = "all" | "active" | "done";

export const Component0923 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0923] mounted");
    return () => console.log("[Component0923] unmounted");
  });
  ...
  return () => html`
    <div class="app-923">
      <header class="header-923">
        <h1>todos — Component0923</h1>
      </header>

      <main class="main-923">
        <input
          class="add-input-923 ${error() ? "add-input--error-923" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-923" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-923">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-923">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-923 ${filter() === f ? "filter-btn--active-923" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-923">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-923">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-923">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-923 ${todo.done ? "todo-item--done-923" : ""}">
                    <button
                      class="todo-check-923 ${todo.done ? "todo-check--done-923" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-923" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-923"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-923" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-923" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-923">
            <span class="todo-count-923">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-923" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
