// client/components/Component0010.ts
// Benchmark component #10 — identical logic to TodoPage, scoped class names.
...
import "./Component0010.css";

type Filter = "all" | "active" | "done";

export const Component0010 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0010] mounted");
    return () => console.log("[Component0010] unmounted");
  });
  ...
  return () => html`
    <div class="app-10">
      <header class="header-10">
        <h1>todos — Component0010</h1>
      </header>

      <main class="main-10">
        <input
          class="add-input-10 ${error() ? "add-input--error-10" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-10" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-10">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-10">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-10 ${filter() === f ? "filter-btn--active-10" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-10">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-10">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-10">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-10 ${todo.done ? "todo-item--done-10" : ""}">
                    <button
                      class="todo-check-10 ${todo.done ? "todo-check--done-10" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-10" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-10"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-10" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-10" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-10">
            <span class="todo-count-10">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-10" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
