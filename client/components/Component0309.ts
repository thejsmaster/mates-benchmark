// client/components/Component0309.ts
// Benchmark component #309 — identical logic to TodoPage, scoped class names.
...
import "./Component0309.css";

type Filter = "all" | "active" | "done";

export const Component0309 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0309] mounted");
    return () => console.log("[Component0309] unmounted");
  });
  ...
  return () => html`
    <div class="app-309">
      <header class="header-309">
        <h1>todos — Component0309</h1>
      </header>

      <main class="main-309">
        <input
          class="add-input-309 ${error() ? "add-input--error-309" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-309" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-309">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-309">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-309 ${filter() === f ? "filter-btn--active-309" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-309">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-309">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-309">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-309 ${todo.done ? "todo-item--done-309" : ""}">
                    <button
                      class="todo-check-309 ${todo.done ? "todo-check--done-309" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-309" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-309"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-309" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-309" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-309">
            <span class="todo-count-309">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-309" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
