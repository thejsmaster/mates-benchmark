// client/components/Component0209.ts
// Benchmark component #209 — identical logic to TodoPage, scoped class names.
...
import "./Component0209.css";

type Filter = "all" | "active" | "done";

export const Component0209 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0209] mounted");
    return () => console.log("[Component0209] unmounted");
  });
  ...
  return () => html`
    <div class="app-209">
      <header class="header-209">
        <h1>todos — Component0209</h1>
      </header>

      <main class="main-209">
        <input
          class="add-input-209 ${error() ? "add-input--error-209" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-209" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-209">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-209">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-209 ${filter() === f ? "filter-btn--active-209" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-209">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-209">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-209">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-209 ${todo.done ? "todo-item--done-209" : ""}">
                    <button
                      class="todo-check-209 ${todo.done ? "todo-check--done-209" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-209" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-209"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-209" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-209" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-209">
            <span class="todo-count-209">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-209" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
