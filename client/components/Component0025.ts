// client/components/Component0025.ts
// Benchmark component #25 — identical logic to TodoPage, scoped class names.
...
import "./Component0025.css";

type Filter = "all" | "active" | "done";

export const Component0025 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0025] mounted");
    return () => console.log("[Component0025] unmounted");
  });
  ...
  return () => html`
    <div class="app-25">
      <header class="header-25">
        <h1>todos — Component0025</h1>
      </header>

      <main class="main-25">
        <input
          class="add-input-25 ${error() ? "add-input--error-25" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-25" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-25">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-25">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-25 ${filter() === f ? "filter-btn--active-25" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-25">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-25">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-25">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-25 ${todo.done ? "todo-item--done-25" : ""}">
                    <button
                      class="todo-check-25 ${todo.done ? "todo-check--done-25" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-25" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-25"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-25" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-25" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-25">
            <span class="todo-count-25">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-25" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
