// client/components/Component0539.ts
// Benchmark component #539 — identical logic to TodoPage, scoped class names.
...
import "./Component0539.css";

type Filter = "all" | "active" | "done";

export const Component0539 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0539] mounted");
    return () => console.log("[Component0539] unmounted");
  });
  ...
  return () => html`
    <div class="app-539">
      <header class="header-539">
        <h1>todos — Component0539</h1>
      </header>

      <main class="main-539">
        <input
          class="add-input-539 ${error() ? "add-input--error-539" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-539" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-539">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-539">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-539 ${filter() === f ? "filter-btn--active-539" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-539">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-539">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-539">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-539 ${todo.done ? "todo-item--done-539" : ""}">
                    <button
                      class="todo-check-539 ${todo.done ? "todo-check--done-539" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-539" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-539"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-539" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-539" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-539">
            <span class="todo-count-539">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-539" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
