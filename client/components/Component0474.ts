// client/components/Component0474.ts
// Benchmark component #474 — identical logic to TodoPage, scoped class names.
...
import "./Component0474.css";

type Filter = "all" | "active" | "done";

export const Component0474 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0474] mounted");
    return () => console.log("[Component0474] unmounted");
  });
  ...
  return () => html`
    <div class="app-474">
      <header class="header-474">
        <h1>todos — Component0474</h1>
      </header>

      <main class="main-474">
        <input
          class="add-input-474 ${error() ? "add-input--error-474" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-474" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-474">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-474">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-474 ${filter() === f ? "filter-btn--active-474" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-474">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-474">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-474">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-474 ${todo.done ? "todo-item--done-474" : ""}">
                    <button
                      class="todo-check-474 ${todo.done ? "todo-check--done-474" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-474" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-474"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-474" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-474" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-474">
            <span class="todo-count-474">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-474" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
