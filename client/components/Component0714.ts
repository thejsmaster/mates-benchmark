// client/components/Component0714.ts
// Benchmark component #714 — identical logic to TodoPage, scoped class names.
...
import "./Component0714.css";

type Filter = "all" | "active" | "done";

export const Component0714 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0714] mounted");
    return () => console.log("[Component0714] unmounted");
  });
  ...
  return () => html`
    <div class="app-714">
      <header class="header-714">
        <h1>todos — Component0714</h1>
      </header>

      <main class="main-714">
        <input
          class="add-input-714 ${error() ? "add-input--error-714" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-714" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-714">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-714">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-714 ${filter() === f ? "filter-btn--active-714" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-714">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-714">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-714">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-714 ${todo.done ? "todo-item--done-714" : ""}">
                    <button
                      class="todo-check-714 ${todo.done ? "todo-check--done-714" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-714" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-714"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-714" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-714" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-714">
            <span class="todo-count-714">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-714" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
