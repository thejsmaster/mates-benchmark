// client/components/Component0795.ts
// Benchmark component #795 — identical logic to TodoPage, scoped class names.
...
import "./Component0795.css";

type Filter = "all" | "active" | "done";

export const Component0795 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0795] mounted");
    return () => console.log("[Component0795] unmounted");
  });
  ...
  return () => html`
    <div class="app-795">
      <header class="header-795">
        <h1>todos — Component0795</h1>
      </header>

      <main class="main-795">
        <input
          class="add-input-795 ${error() ? "add-input--error-795" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-795" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-795">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-795">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-795 ${filter() === f ? "filter-btn--active-795" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-795">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-795">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-795">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-795 ${todo.done ? "todo-item--done-795" : ""}">
                    <button
                      class="todo-check-795 ${todo.done ? "todo-check--done-795" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-795" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-795"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-795" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-795" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-795">
            <span class="todo-count-795">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-795" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
