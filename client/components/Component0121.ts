// client/components/Component0121.ts
// Benchmark component #121 — identical logic to TodoPage, scoped class names.
...
import "./Component0121.css";

type Filter = "all" | "active" | "done";

export const Component0121 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0121] mounted");
    return () => console.log("[Component0121] unmounted");
  });
  ...
  return () => html`
    <div class="app-121">
      <header class="header-121">
        <h1>todos — Component0121</h1>
      </header>

      <main class="main-121">
        <input
          class="add-input-121 ${error() ? "add-input--error-121" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-121" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-121">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-121">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-121 ${filter() === f ? "filter-btn--active-121" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-121">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-121">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-121">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-121 ${todo.done ? "todo-item--done-121" : ""}">
                    <button
                      class="todo-check-121 ${todo.done ? "todo-check--done-121" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-121" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-121"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-121" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-121" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-121">
            <span class="todo-count-121">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-121" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
