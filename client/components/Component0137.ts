// client/components/Component0137.ts
// Benchmark component #137 — identical logic to TodoPage, scoped class names.
...
import "./Component0137.css";

type Filter = "all" | "active" | "done";

export const Component0137 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0137] mounted");
    return () => console.log("[Component0137] unmounted");
  });
  ...
  return () => html`
    <div class="app-137">
      <header class="header-137">
        <h1>todos — Component0137</h1>
      </header>

      <main class="main-137">
        <input
          class="add-input-137 ${error() ? "add-input--error-137" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-137" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-137">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-137">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-137 ${filter() === f ? "filter-btn--active-137" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-137">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-137">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-137">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-137 ${todo.done ? "todo-item--done-137" : ""}">
                    <button
                      class="todo-check-137 ${todo.done ? "todo-check--done-137" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-137" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-137"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-137" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-137" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-137">
            <span class="todo-count-137">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-137" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
