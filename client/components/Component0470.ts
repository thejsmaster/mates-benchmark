// client/components/Component0470.ts
// Benchmark component #470 — identical logic to TodoPage, scoped class names.
...
import "./Component0470.css";

type Filter = "all" | "active" | "done";

export const Component0470 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0470] mounted");
    return () => console.log("[Component0470] unmounted");
  });
  ...
  return () => html`
    <div class="app-470">
      <header class="header-470">
        <h1>todos — Component0470</h1>
      </header>

      <main class="main-470">
        <input
          class="add-input-470 ${error() ? "add-input--error-470" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-470" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-470">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-470">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-470 ${filter() === f ? "filter-btn--active-470" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-470">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-470">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-470">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-470 ${todo.done ? "todo-item--done-470" : ""}">
                    <button
                      class="todo-check-470 ${todo.done ? "todo-check--done-470" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-470" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-470"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-470" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-470" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-470">
            <span class="todo-count-470">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-470" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
