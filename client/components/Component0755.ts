// client/components/Component0755.ts
// Benchmark component #755 — identical logic to TodoPage, scoped class names.
...
import "./Component0755.css";

type Filter = "all" | "active" | "done";

export const Component0755 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0755] mounted");
    return () => console.log("[Component0755] unmounted");
  });
  ...
  return () => html`
    <div class="app-755">
      <header class="header-755">
        <h1>todos — Component0755</h1>
      </header>

      <main class="main-755">
        <input
          class="add-input-755 ${error() ? "add-input--error-755" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-755" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-755">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-755">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-755 ${filter() === f ? "filter-btn--active-755" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-755">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-755">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-755">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-755 ${todo.done ? "todo-item--done-755" : ""}">
                    <button
                      class="todo-check-755 ${todo.done ? "todo-check--done-755" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-755" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-755"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-755" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-755" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-755">
            <span class="todo-count-755">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-755" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
