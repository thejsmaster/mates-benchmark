// client/components/Component0646.ts
// Benchmark component #646 — identical logic to TodoPage, scoped class names.
...
import "./Component0646.css";

type Filter = "all" | "active" | "done";

export const Component0646 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0646] mounted");
    return () => console.log("[Component0646] unmounted");
  });
  ...
  return () => html`
    <div class="app-646">
      <header class="header-646">
        <h1>todos — Component0646</h1>
      </header>

      <main class="main-646">
        <input
          class="add-input-646 ${error() ? "add-input--error-646" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-646" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-646">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-646">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-646 ${filter() === f ? "filter-btn--active-646" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-646">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-646">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-646">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-646 ${todo.done ? "todo-item--done-646" : ""}">
                    <button
                      class="todo-check-646 ${todo.done ? "todo-check--done-646" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-646" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-646"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-646" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-646" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-646">
            <span class="todo-count-646">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-646" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
