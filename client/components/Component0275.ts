// client/components/Component0275.ts
// Benchmark component #275 — identical logic to TodoPage, scoped class names.
...
import "./Component0275.css";

type Filter = "all" | "active" | "done";

export const Component0275 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0275] mounted");
    return () => console.log("[Component0275] unmounted");
  });
  ...
  return () => html`
    <div class="app-275">
      <header class="header-275">
        <h1>todos — Component0275</h1>
      </header>

      <main class="main-275">
        <input
          class="add-input-275 ${error() ? "add-input--error-275" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-275" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-275">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-275">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-275 ${filter() === f ? "filter-btn--active-275" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-275">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-275">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-275">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-275 ${todo.done ? "todo-item--done-275" : ""}">
                    <button
                      class="todo-check-275 ${todo.done ? "todo-check--done-275" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-275" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-275"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-275" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-275" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-275">
            <span class="todo-count-275">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-275" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
