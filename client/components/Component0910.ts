// client/components/Component0910.ts
// Benchmark component #910 — identical logic to TodoPage, scoped class names.
...
import "./Component0910.css";

type Filter = "all" | "active" | "done";

export const Component0910 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0910] mounted");
    return () => console.log("[Component0910] unmounted");
  });
  ...
  return () => html`
    <div class="app-910">
      <header class="header-910">
        <h1>todos — Component0910</h1>
      </header>

      <main class="main-910">
        <input
          class="add-input-910 ${error() ? "add-input--error-910" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-910" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-910">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-910">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-910 ${filter() === f ? "filter-btn--active-910" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-910">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-910">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-910">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-910 ${todo.done ? "todo-item--done-910" : ""}">
                    <button
                      class="todo-check-910 ${todo.done ? "todo-check--done-910" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-910" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-910"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-910" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-910" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-910">
            <span class="todo-count-910">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-910" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
