// client/components/Component0685.ts
// Benchmark component #685 — identical logic to TodoPage, scoped class names.
...
import "./Component0685.css";

type Filter = "all" | "active" | "done";

export const Component0685 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0685] mounted");
    return () => console.log("[Component0685] unmounted");
  });
  ...
  return () => html`
    <div class="app-685">
      <header class="header-685">
        <h1>todos — Component0685</h1>
      </header>

      <main class="main-685">
        <input
          class="add-input-685 ${error() ? "add-input--error-685" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-685" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-685">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-685">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-685 ${filter() === f ? "filter-btn--active-685" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-685">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-685">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-685">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-685 ${todo.done ? "todo-item--done-685" : ""}">
                    <button
                      class="todo-check-685 ${todo.done ? "todo-check--done-685" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-685" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-685"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-685" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-685" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-685">
            <span class="todo-count-685">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-685" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
