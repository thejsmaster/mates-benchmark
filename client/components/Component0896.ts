// client/components/Component0896.ts
// Benchmark component #896 — identical logic to TodoPage, scoped class names.
...
import "./Component0896.css";

type Filter = "all" | "active" | "done";

export const Component0896 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0896] mounted");
    return () => console.log("[Component0896] unmounted");
  });
  ...
  return () => html`
    <div class="app-896">
      <header class="header-896">
        <h1>todos — Component0896</h1>
      </header>

      <main class="main-896">
        <input
          class="add-input-896 ${error() ? "add-input--error-896" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-896" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-896">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-896">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-896 ${filter() === f ? "filter-btn--active-896" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-896">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-896">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-896">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-896 ${todo.done ? "todo-item--done-896" : ""}">
                    <button
                      class="todo-check-896 ${todo.done ? "todo-check--done-896" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-896" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-896"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-896" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-896" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-896">
            <span class="todo-count-896">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-896" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
