// client/components/Component0515.ts
// Benchmark component #515 — identical logic to TodoPage, scoped class names.
...
import "./Component0515.css";

type Filter = "all" | "active" | "done";

export const Component0515 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0515] mounted");
    return () => console.log("[Component0515] unmounted");
  });
  ...
  return () => html`
    <div class="app-515">
      <header class="header-515">
        <h1>todos — Component0515</h1>
      </header>

      <main class="main-515">
        <input
          class="add-input-515 ${error() ? "add-input--error-515" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-515" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-515">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-515">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-515 ${filter() === f ? "filter-btn--active-515" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-515">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-515">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-515">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-515 ${todo.done ? "todo-item--done-515" : ""}">
                    <button
                      class="todo-check-515 ${todo.done ? "todo-check--done-515" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-515" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-515"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-515" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-515" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-515">
            <span class="todo-count-515">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-515" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
