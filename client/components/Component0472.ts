// client/components/Component0472.ts
// Benchmark component #472 — identical logic to TodoPage, scoped class names.
...
import "./Component0472.css";

type Filter = "all" | "active" | "done";

export const Component0472 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0472] mounted");
    return () => console.log("[Component0472] unmounted");
  });
  ...
  return () => html`
    <div class="app-472">
      <header class="header-472">
        <h1>todos — Component0472</h1>
      </header>

      <main class="main-472">
        <input
          class="add-input-472 ${error() ? "add-input--error-472" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-472" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-472">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-472">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-472 ${filter() === f ? "filter-btn--active-472" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-472">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-472">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-472">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-472 ${todo.done ? "todo-item--done-472" : ""}">
                    <button
                      class="todo-check-472 ${todo.done ? "todo-check--done-472" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-472" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-472"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-472" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-472" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-472">
            <span class="todo-count-472">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-472" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
