// client/components/Component0722.ts
// Benchmark component #722 — identical logic to TodoPage, scoped class names.
...
import "./Component0722.css";

type Filter = "all" | "active" | "done";

export const Component0722 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0722] mounted");
    return () => console.log("[Component0722] unmounted");
  });
  ...
  return () => html`
    <div class="app-722">
      <header class="header-722">
        <h1>todos — Component0722</h1>
      </header>

      <main class="main-722">
        <input
          class="add-input-722 ${error() ? "add-input--error-722" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-722" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-722">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-722">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-722 ${filter() === f ? "filter-btn--active-722" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-722">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-722">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-722">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-722 ${todo.done ? "todo-item--done-722" : ""}">
                    <button
                      class="todo-check-722 ${todo.done ? "todo-check--done-722" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-722" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-722"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-722" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-722" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-722">
            <span class="todo-count-722">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-722" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
