// client/components/Component0603.ts
// Benchmark component #603 — identical logic to TodoPage, scoped class names.
...
import "./Component0603.css";

type Filter = "all" | "active" | "done";

export const Component0603 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0603] mounted");
    return () => console.log("[Component0603] unmounted");
  });
  ...
  return () => html`
    <div class="app-603">
      <header class="header-603">
        <h1>todos — Component0603</h1>
      </header>

      <main class="main-603">
        <input
          class="add-input-603 ${error() ? "add-input--error-603" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-603" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-603">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-603">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-603 ${filter() === f ? "filter-btn--active-603" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-603">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-603">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-603">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-603 ${todo.done ? "todo-item--done-603" : ""}">
                    <button
                      class="todo-check-603 ${todo.done ? "todo-check--done-603" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-603" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-603"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-603" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-603" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-603">
            <span class="todo-count-603">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-603" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
