// client/components/Component0742.ts
// Benchmark component #742 — identical logic to TodoPage, scoped class names.
...
import "./Component0742.css";

type Filter = "all" | "active" | "done";

export const Component0742 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0742] mounted");
    return () => console.log("[Component0742] unmounted");
  });
  ...
  return () => html`
    <div class="app-742">
      <header class="header-742">
        <h1>todos — Component0742</h1>
      </header>

      <main class="main-742">
        <input
          class="add-input-742 ${error() ? "add-input--error-742" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-742" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-742">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-742">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-742 ${filter() === f ? "filter-btn--active-742" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-742">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-742">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-742">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-742 ${todo.done ? "todo-item--done-742" : ""}">
                    <button
                      class="todo-check-742 ${todo.done ? "todo-check--done-742" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-742" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-742"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-742" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-742" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-742">
            <span class="todo-count-742">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-742" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
