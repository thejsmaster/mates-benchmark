// client/components/Component0019.ts
// Benchmark component #19 — identical logic to TodoPage, scoped class names.
...
import "./Component0019.css";

type Filter = "all" | "active" | "done";

export const Component0019 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0019] mounted");
    return () => console.log("[Component0019] unmounted");
  });
  ...
  return () => html`
    <div class="app-19">
      <header class="header-19">
        <h1>todos — Component0019</h1>
      </header>

      <main class="main-19">
        <input
          class="add-input-19 ${error() ? "add-input--error-19" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-19" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-19">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-19">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-19 ${filter() === f ? "filter-btn--active-19" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-19">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-19">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-19">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-19 ${todo.done ? "todo-item--done-19" : ""}">
                    <button
                      class="todo-check-19 ${todo.done ? "todo-check--done-19" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-19" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-19"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-19" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-19" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-19">
            <span class="todo-count-19">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-19" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
