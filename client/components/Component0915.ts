// client/components/Component0915.ts
// Benchmark component #915 — identical logic to TodoPage, scoped class names.
...
import "./Component0915.css";

type Filter = "all" | "active" | "done";

export const Component0915 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0915] mounted");
    return () => console.log("[Component0915] unmounted");
  });
  ...
  return () => html`
    <div class="app-915">
      <header class="header-915">
        <h1>todos — Component0915</h1>
      </header>

      <main class="main-915">
        <input
          class="add-input-915 ${error() ? "add-input--error-915" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-915" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-915">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-915">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-915 ${filter() === f ? "filter-btn--active-915" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-915">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-915">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-915">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-915 ${todo.done ? "todo-item--done-915" : ""}">
                    <button
                      class="todo-check-915 ${todo.done ? "todo-check--done-915" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-915" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-915"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-915" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-915" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-915">
            <span class="todo-count-915">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-915" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
