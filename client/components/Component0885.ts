// client/components/Component0885.ts
// Benchmark component #885 — identical logic to TodoPage, scoped class names.
...
import "./Component0885.css";

type Filter = "all" | "active" | "done";

export const Component0885 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0885] mounted");
    return () => console.log("[Component0885] unmounted");
  });
  ...
  return () => html`
    <div class="app-885">
      <header class="header-885">
        <h1>todos — Component0885</h1>
      </header>

      <main class="main-885">
        <input
          class="add-input-885 ${error() ? "add-input--error-885" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-885" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-885">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-885">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-885 ${filter() === f ? "filter-btn--active-885" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-885">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-885">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-885">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-885 ${todo.done ? "todo-item--done-885" : ""}">
                    <button
                      class="todo-check-885 ${todo.done ? "todo-check--done-885" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-885" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-885"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-885" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-885" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-885">
            <span class="todo-count-885">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-885" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
