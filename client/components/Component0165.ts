// client/components/Component0165.ts
// Benchmark component #165 — identical logic to TodoPage, scoped class names.
...
import "./Component0165.css";

type Filter = "all" | "active" | "done";

export const Component0165 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0165] mounted");
    return () => console.log("[Component0165] unmounted");
  });
  ...
  return () => html`
    <div class="app-165">
      <header class="header-165">
        <h1>todos — Component0165</h1>
      </header>

      <main class="main-165">
        <input
          class="add-input-165 ${error() ? "add-input--error-165" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-165" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-165">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-165">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-165 ${filter() === f ? "filter-btn--active-165" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-165">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-165">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-165">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-165 ${todo.done ? "todo-item--done-165" : ""}">
                    <button
                      class="todo-check-165 ${todo.done ? "todo-check--done-165" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-165" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-165"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-165" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-165" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-165">
            <span class="todo-count-165">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-165" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
