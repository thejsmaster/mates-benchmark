// client/components/Component0845.ts
// Benchmark component #845 — identical logic to TodoPage, scoped class names.
...
import "./Component0845.css";

type Filter = "all" | "active" | "done";

export const Component0845 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0845] mounted");
    return () => console.log("[Component0845] unmounted");
  });
  ...
  return () => html`
    <div class="app-845">
      <header class="header-845">
        <h1>todos — Component0845</h1>
      </header>

      <main class="main-845">
        <input
          class="add-input-845 ${error() ? "add-input--error-845" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-845" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-845">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-845">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-845 ${filter() === f ? "filter-btn--active-845" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-845">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-845">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-845">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-845 ${todo.done ? "todo-item--done-845" : ""}">
                    <button
                      class="todo-check-845 ${todo.done ? "todo-check--done-845" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-845" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-845"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-845" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-845" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-845">
            <span class="todo-count-845">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-845" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
