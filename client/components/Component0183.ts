// client/components/Component0183.ts
// Benchmark component #183 — identical logic to TodoPage, scoped class names.
...
import "./Component0183.css";

type Filter = "all" | "active" | "done";

export const Component0183 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0183] mounted");
    return () => console.log("[Component0183] unmounted");
  });
  ...
  return () => html`
    <div class="app-183">
      <header class="header-183">
        <h1>todos — Component0183</h1>
      </header>

      <main class="main-183">
        <input
          class="add-input-183 ${error() ? "add-input--error-183" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-183" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-183">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-183">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-183 ${filter() === f ? "filter-btn--active-183" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-183">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-183">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-183">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-183 ${todo.done ? "todo-item--done-183" : ""}">
                    <button
                      class="todo-check-183 ${todo.done ? "todo-check--done-183" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-183" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-183"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-183" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-183" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-183">
            <span class="todo-count-183">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-183" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
