// client/components/Component0094.ts
// Benchmark component #94 — identical logic to TodoPage, scoped class names.
...
import "./Component0094.css";

type Filter = "all" | "active" | "done";

export const Component0094 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0094] mounted");
    return () => console.log("[Component0094] unmounted");
  });
  ...
  return () => html`
    <div class="app-94">
      <header class="header-94">
        <h1>todos — Component0094</h1>
      </header>

      <main class="main-94">
        <input
          class="add-input-94 ${error() ? "add-input--error-94" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-94" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-94">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-94">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-94 ${filter() === f ? "filter-btn--active-94" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-94">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-94">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-94">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-94 ${todo.done ? "todo-item--done-94" : ""}">
                    <button
                      class="todo-check-94 ${todo.done ? "todo-check--done-94" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-94" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-94"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-94" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-94" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-94">
            <span class="todo-count-94">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-94" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
