// client/components/Component0202.ts
// Benchmark component #202 — identical logic to TodoPage, scoped class names.
...
import "./Component0202.css";

type Filter = "all" | "active" | "done";

export const Component0202 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0202] mounted");
    return () => console.log("[Component0202] unmounted");
  });
  ...
  return () => html`
    <div class="app-202">
      <header class="header-202">
        <h1>todos — Component0202</h1>
      </header>

      <main class="main-202">
        <input
          class="add-input-202 ${error() ? "add-input--error-202" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-202" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-202">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-202">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-202 ${filter() === f ? "filter-btn--active-202" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-202">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-202">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-202">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-202 ${todo.done ? "todo-item--done-202" : ""}">
                    <button
                      class="todo-check-202 ${todo.done ? "todo-check--done-202" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-202" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-202"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-202" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-202" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-202">
            <span class="todo-count-202">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-202" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
