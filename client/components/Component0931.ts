// client/components/Component0931.ts
// Benchmark component #931 — identical logic to TodoPage, scoped class names.
...
import "./Component0931.css";

type Filter = "all" | "active" | "done";

export const Component0931 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0931] mounted");
    return () => console.log("[Component0931] unmounted");
  });
  ...
  return () => html`
    <div class="app-931">
      <header class="header-931">
        <h1>todos — Component0931</h1>
      </header>

      <main class="main-931">
        <input
          class="add-input-931 ${error() ? "add-input--error-931" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-931" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-931">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-931">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-931 ${filter() === f ? "filter-btn--active-931" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-931">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-931">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-931">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-931 ${todo.done ? "todo-item--done-931" : ""}">
                    <button
                      class="todo-check-931 ${todo.done ? "todo-check--done-931" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-931" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-931"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-931" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-931" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-931">
            <span class="todo-count-931">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-931" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
