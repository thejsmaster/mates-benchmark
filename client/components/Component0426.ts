// client/components/Component0426.ts
// Benchmark component #426 — identical logic to TodoPage, scoped class names.
...
import "./Component0426.css";

type Filter = "all" | "active" | "done";

export const Component0426 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0426] mounted");
    return () => console.log("[Component0426] unmounted");
  });
  ...
  return () => html`
    <div class="app-426">
      <header class="header-426">
        <h1>todos — Component0426</h1>
      </header>

      <main class="main-426">
        <input
          class="add-input-426 ${error() ? "add-input--error-426" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-426" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-426">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-426">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-426 ${filter() === f ? "filter-btn--active-426" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-426">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-426">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-426">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-426 ${todo.done ? "todo-item--done-426" : ""}">
                    <button
                      class="todo-check-426 ${todo.done ? "todo-check--done-426" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-426" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-426"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-426" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-426" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-426">
            <span class="todo-count-426">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-426" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
