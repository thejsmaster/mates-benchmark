// client/components/Component0615.ts
// Benchmark component #615 — identical logic to TodoPage, scoped class names.
...
import "./Component0615.css";

type Filter = "all" | "active" | "done";

export const Component0615 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0615] mounted");
    return () => console.log("[Component0615] unmounted");
  });
  ...
  return () => html`
    <div class="app-615">
      <header class="header-615">
        <h1>todos — Component0615</h1>
      </header>

      <main class="main-615">
        <input
          class="add-input-615 ${error() ? "add-input--error-615" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-615" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-615">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-615">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-615 ${filter() === f ? "filter-btn--active-615" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-615">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-615">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-615">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-615 ${todo.done ? "todo-item--done-615" : ""}">
                    <button
                      class="todo-check-615 ${todo.done ? "todo-check--done-615" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-615" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-615"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-615" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-615" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-615">
            <span class="todo-count-615">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-615" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
