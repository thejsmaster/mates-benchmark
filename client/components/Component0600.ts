// client/components/Component0600.ts
// Benchmark component #600 — identical logic to TodoPage, scoped class names.
...
import "./Component0600.css";

type Filter = "all" | "active" | "done";

export const Component0600 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0600] mounted");
    return () => console.log("[Component0600] unmounted");
  });
  ...
  return () => html`
    <div class="app-600">
      <header class="header-600">
        <h1>todos — Component0600</h1>
      </header>

      <main class="main-600">
        <input
          class="add-input-600 ${error() ? "add-input--error-600" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-600" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-600">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-600">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-600 ${filter() === f ? "filter-btn--active-600" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-600">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-600">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-600">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-600 ${todo.done ? "todo-item--done-600" : ""}">
                    <button
                      class="todo-check-600 ${todo.done ? "todo-check--done-600" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-600" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-600"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-600" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-600" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-600">
            <span class="todo-count-600">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-600" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
