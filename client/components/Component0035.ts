// client/components/Component0035.ts
// Benchmark component #35 — identical logic to TodoPage, scoped class names.
...
import "./Component0035.css";

type Filter = "all" | "active" | "done";

export const Component0035 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0035] mounted");
    return () => console.log("[Component0035] unmounted");
  });
  ...
  return () => html`
    <div class="app-35">
      <header class="header-35">
        <h1>todos — Component0035</h1>
      </header>

      <main class="main-35">
        <input
          class="add-input-35 ${error() ? "add-input--error-35" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-35" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-35">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-35">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-35 ${filter() === f ? "filter-btn--active-35" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-35">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-35">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-35">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-35 ${todo.done ? "todo-item--done-35" : ""}">
                    <button
                      class="todo-check-35 ${todo.done ? "todo-check--done-35" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-35" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-35"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-35" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-35" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-35">
            <span class="todo-count-35">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-35" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
