// client/components/Component0263.ts
// Benchmark component #263 — identical logic to TodoPage, scoped class names.
...
import "./Component0263.css";

type Filter = "all" | "active" | "done";

export const Component0263 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0263] mounted");
    return () => console.log("[Component0263] unmounted");
  });
  ...
  return () => html`
    <div class="app-263">
      <header class="header-263">
        <h1>todos — Component0263</h1>
      </header>

      <main class="main-263">
        <input
          class="add-input-263 ${error() ? "add-input--error-263" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-263" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-263">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-263">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-263 ${filter() === f ? "filter-btn--active-263" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-263">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-263">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-263">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-263 ${todo.done ? "todo-item--done-263" : ""}">
                    <button
                      class="todo-check-263 ${todo.done ? "todo-check--done-263" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-263" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-263"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-263" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-263" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-263">
            <span class="todo-count-263">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-263" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
