// client/components/Component0342.ts
// Benchmark component #342 — identical logic to TodoPage, scoped class names.
...
import "./Component0342.css";

type Filter = "all" | "active" | "done";

export const Component0342 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0342] mounted");
    return () => console.log("[Component0342] unmounted");
  });
  ...
  return () => html`
    <div class="app-342">
      <header class="header-342">
        <h1>todos — Component0342</h1>
      </header>

      <main class="main-342">
        <input
          class="add-input-342 ${error() ? "add-input--error-342" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-342" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-342">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-342">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-342 ${filter() === f ? "filter-btn--active-342" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-342">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-342">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-342">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-342 ${todo.done ? "todo-item--done-342" : ""}">
                    <button
                      class="todo-check-342 ${todo.done ? "todo-check--done-342" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-342" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-342"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-342" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-342" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-342">
            <span class="todo-count-342">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-342" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
