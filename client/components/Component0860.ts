// client/components/Component0860.ts
// Benchmark component #860 — identical logic to TodoPage, scoped class names.
...
import "./Component0860.css";

type Filter = "all" | "active" | "done";

export const Component0860 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0860] mounted");
    return () => console.log("[Component0860] unmounted");
  });
  ...
  return () => html`
    <div class="app-860">
      <header class="header-860">
        <h1>todos — Component0860</h1>
      </header>

      <main class="main-860">
        <input
          class="add-input-860 ${error() ? "add-input--error-860" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-860" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-860">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-860">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-860 ${filter() === f ? "filter-btn--active-860" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-860">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-860">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-860">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-860 ${todo.done ? "todo-item--done-860" : ""}">
                    <button
                      class="todo-check-860 ${todo.done ? "todo-check--done-860" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-860" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-860"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-860" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-860" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-860">
            <span class="todo-count-860">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-860" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
