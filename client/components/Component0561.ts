// client/components/Component0561.ts
// Benchmark component #561 — identical logic to TodoPage, scoped class names.
...
import "./Component0561.css";

type Filter = "all" | "active" | "done";

export const Component0561 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0561] mounted");
    return () => console.log("[Component0561] unmounted");
  });
  ...
  return () => html`
    <div class="app-561">
      <header class="header-561">
        <h1>todos — Component0561</h1>
      </header>

      <main class="main-561">
        <input
          class="add-input-561 ${error() ? "add-input--error-561" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-561" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-561">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-561">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-561 ${filter() === f ? "filter-btn--active-561" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-561">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-561">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-561">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-561 ${todo.done ? "todo-item--done-561" : ""}">
                    <button
                      class="todo-check-561 ${todo.done ? "todo-check--done-561" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-561" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-561"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-561" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-561" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-561">
            <span class="todo-count-561">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-561" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
