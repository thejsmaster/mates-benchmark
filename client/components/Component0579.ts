// client/components/Component0579.ts
// Benchmark component #579 — identical logic to TodoPage, scoped class names.
...
import "./Component0579.css";

type Filter = "all" | "active" | "done";

export const Component0579 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0579] mounted");
    return () => console.log("[Component0579] unmounted");
  });
  ...
  return () => html`
    <div class="app-579">
      <header class="header-579">
        <h1>todos — Component0579</h1>
      </header>

      <main class="main-579">
        <input
          class="add-input-579 ${error() ? "add-input--error-579" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-579" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-579">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-579">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-579 ${filter() === f ? "filter-btn--active-579" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-579">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-579">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-579">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-579 ${todo.done ? "todo-item--done-579" : ""}">
                    <button
                      class="todo-check-579 ${todo.done ? "todo-check--done-579" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-579" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-579"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-579" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-579" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-579">
            <span class="todo-count-579">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-579" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
