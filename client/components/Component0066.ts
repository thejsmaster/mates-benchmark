// client/components/Component0066.ts
// Benchmark component #66 — identical logic to TodoPage, scoped class names.
...
import "./Component0066.css";

type Filter = "all" | "active" | "done";

export const Component0066 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0066] mounted");
    return () => console.log("[Component0066] unmounted");
  });
  ...
  return () => html`
    <div class="app-66">
      <header class="header-66">
        <h1>todos — Component0066</h1>
      </header>

      <main class="main-66">
        <input
          class="add-input-66 ${error() ? "add-input--error-66" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-66" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-66">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-66">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-66 ${filter() === f ? "filter-btn--active-66" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-66">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-66">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-66">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-66 ${todo.done ? "todo-item--done-66" : ""}">
                    <button
                      class="todo-check-66 ${todo.done ? "todo-check--done-66" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-66" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-66"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-66" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-66" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-66">
            <span class="todo-count-66">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-66" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
