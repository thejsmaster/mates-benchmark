// client/components/Component0130.ts
// Benchmark component #130 — identical logic to TodoPage, scoped class names.
...
import "./Component0130.css";

type Filter = "all" | "active" | "done";

export const Component0130 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0130] mounted");
    return () => console.log("[Component0130] unmounted");
  });
  ...
  return () => html`
    <div class="app-130">
      <header class="header-130">
        <h1>todos — Component0130</h1>
      </header>

      <main class="main-130">
        <input
          class="add-input-130 ${error() ? "add-input--error-130" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-130" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-130">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-130">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-130 ${filter() === f ? "filter-btn--active-130" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-130">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-130">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-130">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-130 ${todo.done ? "todo-item--done-130" : ""}">
                    <button
                      class="todo-check-130 ${todo.done ? "todo-check--done-130" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-130" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-130"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-130" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-130" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-130">
            <span class="todo-count-130">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-130" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
