// client/components/Component0432.ts
// Benchmark component #432 — identical logic to TodoPage, scoped class names.
...
import "./Component0432.css";

type Filter = "all" | "active" | "done";

export const Component0432 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0432] mounted");
    return () => console.log("[Component0432] unmounted");
  });
  ...
  return () => html`
    <div class="app-432">
      <header class="header-432">
        <h1>todos — Component0432</h1>
      </header>

      <main class="main-432">
        <input
          class="add-input-432 ${error() ? "add-input--error-432" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-432" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-432">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-432">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-432 ${filter() === f ? "filter-btn--active-432" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-432">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-432">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-432">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-432 ${todo.done ? "todo-item--done-432" : ""}">
                    <button
                      class="todo-check-432 ${todo.done ? "todo-check--done-432" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-432" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-432"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-432" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-432" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-432">
            <span class="todo-count-432">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-432" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
