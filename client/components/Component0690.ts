// client/components/Component0690.ts
// Benchmark component #690 — identical logic to TodoPage, scoped class names.
...
import "./Component0690.css";

type Filter = "all" | "active" | "done";

export const Component0690 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0690] mounted");
    return () => console.log("[Component0690] unmounted");
  });
  ...
  return () => html`
    <div class="app-690">
      <header class="header-690">
        <h1>todos — Component0690</h1>
      </header>

      <main class="main-690">
        <input
          class="add-input-690 ${error() ? "add-input--error-690" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-690" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-690">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-690">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-690 ${filter() === f ? "filter-btn--active-690" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-690">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-690">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-690">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-690 ${todo.done ? "todo-item--done-690" : ""}">
                    <button
                      class="todo-check-690 ${todo.done ? "todo-check--done-690" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-690" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-690"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-690" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-690" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-690">
            <span class="todo-count-690">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-690" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
