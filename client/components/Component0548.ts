// client/components/Component0548.ts
// Benchmark component #548 — identical logic to TodoPage, scoped class names.
...
import "./Component0548.css";

type Filter = "all" | "active" | "done";

export const Component0548 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0548] mounted");
    return () => console.log("[Component0548] unmounted");
  });
  ...
  return () => html`
    <div class="app-548">
      <header class="header-548">
        <h1>todos — Component0548</h1>
      </header>

      <main class="main-548">
        <input
          class="add-input-548 ${error() ? "add-input--error-548" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-548" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-548">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-548">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-548 ${filter() === f ? "filter-btn--active-548" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-548">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-548">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-548">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-548 ${todo.done ? "todo-item--done-548" : ""}">
                    <button
                      class="todo-check-548 ${todo.done ? "todo-check--done-548" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-548" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-548"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-548" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-548" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-548">
            <span class="todo-count-548">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-548" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
