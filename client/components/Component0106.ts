// client/components/Component0106.ts
// Benchmark component #106 — identical logic to TodoPage, scoped class names.
...
import "./Component0106.css";

type Filter = "all" | "active" | "done";

export const Component0106 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0106] mounted");
    return () => console.log("[Component0106] unmounted");
  });
  ...
  return () => html`
    <div class="app-106">
      <header class="header-106">
        <h1>todos — Component0106</h1>
      </header>

      <main class="main-106">
        <input
          class="add-input-106 ${error() ? "add-input--error-106" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-106" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-106">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-106">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-106 ${filter() === f ? "filter-btn--active-106" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-106">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-106">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-106">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-106 ${todo.done ? "todo-item--done-106" : ""}">
                    <button
                      class="todo-check-106 ${todo.done ? "todo-check--done-106" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-106" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-106"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-106" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-106" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-106">
            <span class="todo-count-106">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-106" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
