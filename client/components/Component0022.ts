// client/components/Component0022.ts
// Benchmark component #22 — identical logic to TodoPage, scoped class names.
...
import "./Component0022.css";

type Filter = "all" | "active" | "done";

export const Component0022 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0022] mounted");
    return () => console.log("[Component0022] unmounted");
  });
  ...
  return () => html`
    <div class="app-22">
      <header class="header-22">
        <h1>todos — Component0022</h1>
      </header>

      <main class="main-22">
        <input
          class="add-input-22 ${error() ? "add-input--error-22" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-22" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-22">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-22">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-22 ${filter() === f ? "filter-btn--active-22" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-22">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-22">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-22">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-22 ${todo.done ? "todo-item--done-22" : ""}">
                    <button
                      class="todo-check-22 ${todo.done ? "todo-check--done-22" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-22" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-22"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-22" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-22" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-22">
            <span class="todo-count-22">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-22" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
