// client/components/Component0410.ts
// Benchmark component #410 — identical logic to TodoPage, scoped class names.
...
import "./Component0410.css";

type Filter = "all" | "active" | "done";

export const Component0410 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0410] mounted");
    return () => console.log("[Component0410] unmounted");
  });
  ...
  return () => html`
    <div class="app-410">
      <header class="header-410">
        <h1>todos — Component0410</h1>
      </header>

      <main class="main-410">
        <input
          class="add-input-410 ${error() ? "add-input--error-410" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-410" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-410">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-410">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-410 ${filter() === f ? "filter-btn--active-410" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-410">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-410">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-410">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-410 ${todo.done ? "todo-item--done-410" : ""}">
                    <button
                      class="todo-check-410 ${todo.done ? "todo-check--done-410" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-410" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-410"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-410" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-410" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-410">
            <span class="todo-count-410">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-410" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
