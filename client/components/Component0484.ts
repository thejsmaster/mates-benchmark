// client/components/Component0484.ts
// Benchmark component #484 — identical logic to TodoPage, scoped class names.
...
import "./Component0484.css";

type Filter = "all" | "active" | "done";

export const Component0484 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0484] mounted");
    return () => console.log("[Component0484] unmounted");
  });
  ...
  return () => html`
    <div class="app-484">
      <header class="header-484">
        <h1>todos — Component0484</h1>
      </header>

      <main class="main-484">
        <input
          class="add-input-484 ${error() ? "add-input--error-484" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-484" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-484">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-484">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-484 ${filter() === f ? "filter-btn--active-484" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-484">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-484">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-484">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-484 ${todo.done ? "todo-item--done-484" : ""}">
                    <button
                      class="todo-check-484 ${todo.done ? "todo-check--done-484" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-484" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-484"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-484" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-484" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-484">
            <span class="todo-count-484">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-484" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
