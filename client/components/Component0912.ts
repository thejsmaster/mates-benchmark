// client/components/Component0912.ts
// Benchmark component #912 — identical logic to TodoPage, scoped class names.
...
import "./Component0912.css";

type Filter = "all" | "active" | "done";

export const Component0912 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0912] mounted");
    return () => console.log("[Component0912] unmounted");
  });
  ...
  return () => html`
    <div class="app-912">
      <header class="header-912">
        <h1>todos — Component0912</h1>
      </header>

      <main class="main-912">
        <input
          class="add-input-912 ${error() ? "add-input--error-912" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-912" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-912">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-912">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-912 ${filter() === f ? "filter-btn--active-912" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-912">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-912">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-912">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-912 ${todo.done ? "todo-item--done-912" : ""}">
                    <button
                      class="todo-check-912 ${todo.done ? "todo-check--done-912" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-912" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-912"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-912" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-912" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-912">
            <span class="todo-count-912">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-912" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
