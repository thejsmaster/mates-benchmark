// client/components/Component0305.ts
// Benchmark component #305 — identical logic to TodoPage, scoped class names.
...
import "./Component0305.css";

type Filter = "all" | "active" | "done";

export const Component0305 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0305] mounted");
    return () => console.log("[Component0305] unmounted");
  });
  ...
  return () => html`
    <div class="app-305">
      <header class="header-305">
        <h1>todos — Component0305</h1>
      </header>

      <main class="main-305">
        <input
          class="add-input-305 ${error() ? "add-input--error-305" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-305" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-305">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-305">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-305 ${filter() === f ? "filter-btn--active-305" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-305">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-305">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-305">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-305 ${todo.done ? "todo-item--done-305" : ""}">
                    <button
                      class="todo-check-305 ${todo.done ? "todo-check--done-305" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-305" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-305"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-305" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-305" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-305">
            <span class="todo-count-305">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-305" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
