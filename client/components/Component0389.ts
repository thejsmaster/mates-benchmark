// client/components/Component0389.ts
// Benchmark component #389 — identical logic to TodoPage, scoped class names.
...
import "./Component0389.css";

type Filter = "all" | "active" | "done";

export const Component0389 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0389] mounted");
    return () => console.log("[Component0389] unmounted");
  });
  ...
  return () => html`
    <div class="app-389">
      <header class="header-389">
        <h1>todos — Component0389</h1>
      </header>

      <main class="main-389">
        <input
          class="add-input-389 ${error() ? "add-input--error-389" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-389" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-389">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-389">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-389 ${filter() === f ? "filter-btn--active-389" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-389">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-389">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-389">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-389 ${todo.done ? "todo-item--done-389" : ""}">
                    <button
                      class="todo-check-389 ${todo.done ? "todo-check--done-389" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-389" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-389"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-389" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-389" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-389">
            <span class="todo-count-389">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-389" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
