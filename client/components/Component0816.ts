// client/components/Component0816.ts
// Benchmark component #816 — identical logic to TodoPage, scoped class names.
...
import "./Component0816.css";

type Filter = "all" | "active" | "done";

export const Component0816 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0816] mounted");
    return () => console.log("[Component0816] unmounted");
  });
  ...
  return () => html`
    <div class="app-816">
      <header class="header-816">
        <h1>todos — Component0816</h1>
      </header>

      <main class="main-816">
        <input
          class="add-input-816 ${error() ? "add-input--error-816" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-816" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-816">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-816">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-816 ${filter() === f ? "filter-btn--active-816" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-816">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-816">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-816">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-816 ${todo.done ? "todo-item--done-816" : ""}">
                    <button
                      class="todo-check-816 ${todo.done ? "todo-check--done-816" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-816" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-816"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-816" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-816" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-816">
            <span class="todo-count-816">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-816" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
