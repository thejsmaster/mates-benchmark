// client/components/Component0428.ts
// Benchmark component #428 — identical logic to TodoPage, scoped class names.
...
import "./Component0428.css";

type Filter = "all" | "active" | "done";

export const Component0428 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0428] mounted");
    return () => console.log("[Component0428] unmounted");
  });
  ...
  return () => html`
    <div class="app-428">
      <header class="header-428">
        <h1>todos — Component0428</h1>
      </header>

      <main class="main-428">
        <input
          class="add-input-428 ${error() ? "add-input--error-428" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-428" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-428">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-428">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-428 ${filter() === f ? "filter-btn--active-428" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-428">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-428">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-428">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-428 ${todo.done ? "todo-item--done-428" : ""}">
                    <button
                      class="todo-check-428 ${todo.done ? "todo-check--done-428" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-428" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-428"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-428" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-428" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-428">
            <span class="todo-count-428">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-428" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
