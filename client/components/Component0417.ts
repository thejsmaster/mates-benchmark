// client/components/Component0417.ts
// Benchmark component #417 — identical logic to TodoPage, scoped class names.
...
import "./Component0417.css";

type Filter = "all" | "active" | "done";

export const Component0417 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0417] mounted");
    return () => console.log("[Component0417] unmounted");
  });
  ...
  return () => html`
    <div class="app-417">
      <header class="header-417">
        <h1>todos — Component0417</h1>
      </header>

      <main class="main-417">
        <input
          class="add-input-417 ${error() ? "add-input--error-417" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-417" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-417">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-417">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-417 ${filter() === f ? "filter-btn--active-417" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-417">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-417">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-417">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-417 ${todo.done ? "todo-item--done-417" : ""}">
                    <button
                      class="todo-check-417 ${todo.done ? "todo-check--done-417" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-417" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-417"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-417" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-417" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-417">
            <span class="todo-count-417">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-417" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
