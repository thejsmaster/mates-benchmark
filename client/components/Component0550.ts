// client/components/Component0550.ts
// Benchmark component #550 — identical logic to TodoPage, scoped class names.
...
import "./Component0550.css";

type Filter = "all" | "active" | "done";

export const Component0550 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0550] mounted");
    return () => console.log("[Component0550] unmounted");
  });
  ...
  return () => html`
    <div class="app-550">
      <header class="header-550">
        <h1>todos — Component0550</h1>
      </header>

      <main class="main-550">
        <input
          class="add-input-550 ${error() ? "add-input--error-550" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-550" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-550">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-550">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-550 ${filter() === f ? "filter-btn--active-550" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-550">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-550">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-550">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-550 ${todo.done ? "todo-item--done-550" : ""}">
                    <button
                      class="todo-check-550 ${todo.done ? "todo-check--done-550" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-550" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-550"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-550" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-550" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-550">
            <span class="todo-count-550">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-550" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
