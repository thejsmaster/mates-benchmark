// client/components/Component0039.ts
// Benchmark component #39 — identical logic to TodoPage, scoped class names.
...
import "./Component0039.css";

type Filter = "all" | "active" | "done";

export const Component0039 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0039] mounted");
    return () => console.log("[Component0039] unmounted");
  });
  ...
  return () => html`
    <div class="app-39">
      <header class="header-39">
        <h1>todos — Component0039</h1>
      </header>

      <main class="main-39">
        <input
          class="add-input-39 ${error() ? "add-input--error-39" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-39" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-39">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-39">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-39 ${filter() === f ? "filter-btn--active-39" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-39">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-39">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-39">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-39 ${todo.done ? "todo-item--done-39" : ""}">
                    <button
                      class="todo-check-39 ${todo.done ? "todo-check--done-39" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-39" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-39"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-39" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-39" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-39">
            <span class="todo-count-39">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-39" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
