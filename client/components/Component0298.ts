// client/components/Component0298.ts
// Benchmark component #298 — identical logic to TodoPage, scoped class names.
...
import "./Component0298.css";

type Filter = "all" | "active" | "done";

export const Component0298 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0298] mounted");
    return () => console.log("[Component0298] unmounted");
  });
  ...
  return () => html`
    <div class="app-298">
      <header class="header-298">
        <h1>todos — Component0298</h1>
      </header>

      <main class="main-298">
        <input
          class="add-input-298 ${error() ? "add-input--error-298" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-298" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-298">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-298">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-298 ${filter() === f ? "filter-btn--active-298" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-298">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-298">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-298">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-298 ${todo.done ? "todo-item--done-298" : ""}">
                    <button
                      class="todo-check-298 ${todo.done ? "todo-check--done-298" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-298" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-298"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-298" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-298" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-298">
            <span class="todo-count-298">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-298" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
