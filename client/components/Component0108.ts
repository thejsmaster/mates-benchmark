// client/components/Component0108.ts
// Benchmark component #108 — identical logic to TodoPage, scoped class names.
...
import "./Component0108.css";

type Filter = "all" | "active" | "done";

export const Component0108 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0108] mounted");
    return () => console.log("[Component0108] unmounted");
  });
  ...
  return () => html`
    <div class="app-108">
      <header class="header-108">
        <h1>todos — Component0108</h1>
      </header>

      <main class="main-108">
        <input
          class="add-input-108 ${error() ? "add-input--error-108" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-108" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-108">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-108">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-108 ${filter() === f ? "filter-btn--active-108" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-108">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-108">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-108">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-108 ${todo.done ? "todo-item--done-108" : ""}">
                    <button
                      class="todo-check-108 ${todo.done ? "todo-check--done-108" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-108" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-108"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-108" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-108" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-108">
            <span class="todo-count-108">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-108" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
