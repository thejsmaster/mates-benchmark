// client/components/Component0008.ts
// Benchmark component #8 — identical logic to TodoPage, scoped class names.
...
import "./Component0008.css";

type Filter = "all" | "active" | "done";

export const Component0008 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0008] mounted");
    return () => console.log("[Component0008] unmounted");
  });
  ...
  return () => html`
    <div class="app-8">
      <header class="header-8">
        <h1>todos — Component0008</h1>
      </header>

      <main class="main-8">
        <input
          class="add-input-8 ${error() ? "add-input--error-8" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-8" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-8">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-8">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-8 ${filter() === f ? "filter-btn--active-8" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-8">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-8">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-8">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-8 ${todo.done ? "todo-item--done-8" : ""}">
                    <button
                      class="todo-check-8 ${todo.done ? "todo-check--done-8" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-8" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-8"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-8" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-8" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-8">
            <span class="todo-count-8">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-8" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
