// client/components/Component0626.ts
// Benchmark component #626 — identical logic to TodoPage, scoped class names.
...
import "./Component0626.css";

type Filter = "all" | "active" | "done";

export const Component0626 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0626] mounted");
    return () => console.log("[Component0626] unmounted");
  });
  ...
  return () => html`
    <div class="app-626">
      <header class="header-626">
        <h1>todos — Component0626</h1>
      </header>

      <main class="main-626">
        <input
          class="add-input-626 ${error() ? "add-input--error-626" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-626" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-626">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-626">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-626 ${filter() === f ? "filter-btn--active-626" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-626">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-626">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-626">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-626 ${todo.done ? "todo-item--done-626" : ""}">
                    <button
                      class="todo-check-626 ${todo.done ? "todo-check--done-626" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-626" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-626"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-626" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-626" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-626">
            <span class="todo-count-626">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-626" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
