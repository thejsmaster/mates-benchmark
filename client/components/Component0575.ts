// client/components/Component0575.ts
// Benchmark component #575 — identical logic to TodoPage, scoped class names.
...
import "./Component0575.css";

type Filter = "all" | "active" | "done";

export const Component0575 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0575] mounted");
    return () => console.log("[Component0575] unmounted");
  });
  ...
  return () => html`
    <div class="app-575">
      <header class="header-575">
        <h1>todos — Component0575</h1>
      </header>

      <main class="main-575">
        <input
          class="add-input-575 ${error() ? "add-input--error-575" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-575" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-575">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-575">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-575 ${filter() === f ? "filter-btn--active-575" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-575">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-575">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-575">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-575 ${todo.done ? "todo-item--done-575" : ""}">
                    <button
                      class="todo-check-575 ${todo.done ? "todo-check--done-575" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-575" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-575"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-575" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-575" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-575">
            <span class="todo-count-575">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-575" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
