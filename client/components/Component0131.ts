// client/components/Component0131.ts
// Benchmark component #131 — identical logic to TodoPage, scoped class names.
...
import "./Component0131.css";

type Filter = "all" | "active" | "done";

export const Component0131 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0131] mounted");
    return () => console.log("[Component0131] unmounted");
  });
  ...
  return () => html`
    <div class="app-131">
      <header class="header-131">
        <h1>todos — Component0131</h1>
      </header>

      <main class="main-131">
        <input
          class="add-input-131 ${error() ? "add-input--error-131" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-131" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-131">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-131">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-131 ${filter() === f ? "filter-btn--active-131" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-131">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-131">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-131">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-131 ${todo.done ? "todo-item--done-131" : ""}">
                    <button
                      class="todo-check-131 ${todo.done ? "todo-check--done-131" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-131" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-131"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-131" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-131" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-131">
            <span class="todo-count-131">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-131" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
