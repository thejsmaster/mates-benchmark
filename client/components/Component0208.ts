// client/components/Component0208.ts
// Benchmark component #208 — identical logic to TodoPage, scoped class names.
...
import "./Component0208.css";

type Filter = "all" | "active" | "done";

export const Component0208 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0208] mounted");
    return () => console.log("[Component0208] unmounted");
  });
  ...
  return () => html`
    <div class="app-208">
      <header class="header-208">
        <h1>todos — Component0208</h1>
      </header>

      <main class="main-208">
        <input
          class="add-input-208 ${error() ? "add-input--error-208" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-208" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-208">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-208">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-208 ${filter() === f ? "filter-btn--active-208" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-208">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-208">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-208">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-208 ${todo.done ? "todo-item--done-208" : ""}">
                    <button
                      class="todo-check-208 ${todo.done ? "todo-check--done-208" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-208" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-208"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-208" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-208" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-208">
            <span class="todo-count-208">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-208" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
