// client/components/Component0383.ts
// Benchmark component #383 — identical logic to TodoPage, scoped class names.
...
import "./Component0383.css";

type Filter = "all" | "active" | "done";

export const Component0383 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0383] mounted");
    return () => console.log("[Component0383] unmounted");
  });
  ...
  return () => html`
    <div class="app-383">
      <header class="header-383">
        <h1>todos — Component0383</h1>
      </header>

      <main class="main-383">
        <input
          class="add-input-383 ${error() ? "add-input--error-383" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-383" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-383">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-383">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-383 ${filter() === f ? "filter-btn--active-383" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-383">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-383">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-383">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-383 ${todo.done ? "todo-item--done-383" : ""}">
                    <button
                      class="todo-check-383 ${todo.done ? "todo-check--done-383" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-383" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-383"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-383" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-383" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-383">
            <span class="todo-count-383">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-383" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
