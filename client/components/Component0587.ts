// client/components/Component0587.ts
// Benchmark component #587 — identical logic to TodoPage, scoped class names.
...
import "./Component0587.css";

type Filter = "all" | "active" | "done";

export const Component0587 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0587] mounted");
    return () => console.log("[Component0587] unmounted");
  });
  ...
  return () => html`
    <div class="app-587">
      <header class="header-587">
        <h1>todos — Component0587</h1>
      </header>

      <main class="main-587">
        <input
          class="add-input-587 ${error() ? "add-input--error-587" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-587" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-587">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-587">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-587 ${filter() === f ? "filter-btn--active-587" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-587">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-587">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-587">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-587 ${todo.done ? "todo-item--done-587" : ""}">
                    <button
                      class="todo-check-587 ${todo.done ? "todo-check--done-587" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-587" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-587"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-587" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-587" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-587">
            <span class="todo-count-587">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-587" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
