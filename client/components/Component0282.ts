// client/components/Component0282.ts
// Benchmark component #282 — identical logic to TodoPage, scoped class names.
...
import "./Component0282.css";

type Filter = "all" | "active" | "done";

export const Component0282 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0282] mounted");
    return () => console.log("[Component0282] unmounted");
  });
  ...
  return () => html`
    <div class="app-282">
      <header class="header-282">
        <h1>todos — Component0282</h1>
      </header>

      <main class="main-282">
        <input
          class="add-input-282 ${error() ? "add-input--error-282" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-282" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-282">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-282">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-282 ${filter() === f ? "filter-btn--active-282" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-282">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-282">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-282">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-282 ${todo.done ? "todo-item--done-282" : ""}">
                    <button
                      class="todo-check-282 ${todo.done ? "todo-check--done-282" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-282" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-282"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-282" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-282" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-282">
            <span class="todo-count-282">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-282" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
