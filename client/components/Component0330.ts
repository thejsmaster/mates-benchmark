// client/components/Component0330.ts
// Benchmark component #330 — identical logic to TodoPage, scoped class names.
...
import "./Component0330.css";

type Filter = "all" | "active" | "done";

export const Component0330 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0330] mounted");
    return () => console.log("[Component0330] unmounted");
  });
  ...
  return () => html`
    <div class="app-330">
      <header class="header-330">
        <h1>todos — Component0330</h1>
      </header>

      <main class="main-330">
        <input
          class="add-input-330 ${error() ? "add-input--error-330" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-330" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-330">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-330">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-330 ${filter() === f ? "filter-btn--active-330" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-330">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-330">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-330">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-330 ${todo.done ? "todo-item--done-330" : ""}">
                    <button
                      class="todo-check-330 ${todo.done ? "todo-check--done-330" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-330" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-330"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-330" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-330" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-330">
            <span class="todo-count-330">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-330" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
