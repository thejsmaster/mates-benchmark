// client/components/Component0024.ts
// Benchmark component #24 — identical logic to TodoPage, scoped class names.
...
import "./Component0024.css";

type Filter = "all" | "active" | "done";

export const Component0024 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0024] mounted");
    return () => console.log("[Component0024] unmounted");
  });
  ...
  return () => html`
    <div class="app-24">
      <header class="header-24">
        <h1>todos — Component0024</h1>
      </header>

      <main class="main-24">
        <input
          class="add-input-24 ${error() ? "add-input--error-24" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-24" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-24">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-24">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-24 ${filter() === f ? "filter-btn--active-24" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-24">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-24">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-24">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-24 ${todo.done ? "todo-item--done-24" : ""}">
                    <button
                      class="todo-check-24 ${todo.done ? "todo-check--done-24" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-24" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-24"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-24" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-24" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-24">
            <span class="todo-count-24">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-24" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
