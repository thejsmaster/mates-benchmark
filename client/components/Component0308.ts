// client/components/Component0308.ts
// Benchmark component #308 — identical logic to TodoPage, scoped class names.
...
import "./Component0308.css";

type Filter = "all" | "active" | "done";

export const Component0308 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0308] mounted");
    return () => console.log("[Component0308] unmounted");
  });
  ...
  return () => html`
    <div class="app-308">
      <header class="header-308">
        <h1>todos — Component0308</h1>
      </header>

      <main class="main-308">
        <input
          class="add-input-308 ${error() ? "add-input--error-308" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-308" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-308">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-308">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-308 ${filter() === f ? "filter-btn--active-308" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-308">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-308">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-308">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-308 ${todo.done ? "todo-item--done-308" : ""}">
                    <button
                      class="todo-check-308 ${todo.done ? "todo-check--done-308" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-308" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-308"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-308" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-308" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-308">
            <span class="todo-count-308">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-308" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
