// client/components/Component0193.ts
// Benchmark component #193 — identical logic to TodoPage, scoped class names.
...
import "./Component0193.css";

type Filter = "all" | "active" | "done";

export const Component0193 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0193] mounted");
    return () => console.log("[Component0193] unmounted");
  });
  ...
  return () => html`
    <div class="app-193">
      <header class="header-193">
        <h1>todos — Component0193</h1>
      </header>

      <main class="main-193">
        <input
          class="add-input-193 ${error() ? "add-input--error-193" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-193" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-193">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-193">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-193 ${filter() === f ? "filter-btn--active-193" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-193">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-193">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-193">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-193 ${todo.done ? "todo-item--done-193" : ""}">
                    <button
                      class="todo-check-193 ${todo.done ? "todo-check--done-193" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-193" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-193"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-193" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-193" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-193">
            <span class="todo-count-193">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-193" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
