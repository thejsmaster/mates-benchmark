// client/components/Component0483.ts
// Benchmark component #483 — identical logic to TodoPage, scoped class names.
...
import "./Component0483.css";

type Filter = "all" | "active" | "done";

export const Component0483 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0483] mounted");
    return () => console.log("[Component0483] unmounted");
  });
  ...
  return () => html`
    <div class="app-483">
      <header class="header-483">
        <h1>todos — Component0483</h1>
      </header>

      <main class="main-483">
        <input
          class="add-input-483 ${error() ? "add-input--error-483" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-483" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-483">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-483">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-483 ${filter() === f ? "filter-btn--active-483" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-483">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-483">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-483">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-483 ${todo.done ? "todo-item--done-483" : ""}">
                    <button
                      class="todo-check-483 ${todo.done ? "todo-check--done-483" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-483" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-483"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-483" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-483" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-483">
            <span class="todo-count-483">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-483" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
