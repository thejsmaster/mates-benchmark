// client/components/Component0148.ts
// Benchmark component #148 — identical logic to TodoPage, scoped class names.
...
import "./Component0148.css";

type Filter = "all" | "active" | "done";

export const Component0148 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0148] mounted");
    return () => console.log("[Component0148] unmounted");
  });
  ...
  return () => html`
    <div class="app-148">
      <header class="header-148">
        <h1>todos — Component0148</h1>
      </header>

      <main class="main-148">
        <input
          class="add-input-148 ${error() ? "add-input--error-148" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-148" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-148">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-148">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-148 ${filter() === f ? "filter-btn--active-148" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-148">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-148">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-148">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-148 ${todo.done ? "todo-item--done-148" : ""}">
                    <button
                      class="todo-check-148 ${todo.done ? "todo-check--done-148" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-148" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-148"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-148" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-148" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-148">
            <span class="todo-count-148">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-148" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
