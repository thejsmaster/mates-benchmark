// client/components/Component0489.ts
// Benchmark component #489 — identical logic to TodoPage, scoped class names.
...
import "./Component0489.css";

type Filter = "all" | "active" | "done";

export const Component0489 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0489] mounted");
    return () => console.log("[Component0489] unmounted");
  });
  ...
  return () => html`
    <div class="app-489">
      <header class="header-489">
        <h1>todos — Component0489</h1>
      </header>

      <main class="main-489">
        <input
          class="add-input-489 ${error() ? "add-input--error-489" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-489" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-489">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-489">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-489 ${filter() === f ? "filter-btn--active-489" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-489">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-489">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-489">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-489 ${todo.done ? "todo-item--done-489" : ""}">
                    <button
                      class="todo-check-489 ${todo.done ? "todo-check--done-489" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-489" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-489"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-489" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-489" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-489">
            <span class="todo-count-489">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-489" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
