// client/components/Component0844.ts
// Benchmark component #844 — identical logic to TodoPage, scoped class names.
...
import "./Component0844.css";

type Filter = "all" | "active" | "done";

export const Component0844 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0844] mounted");
    return () => console.log("[Component0844] unmounted");
  });
  ...
  return () => html`
    <div class="app-844">
      <header class="header-844">
        <h1>todos — Component0844</h1>
      </header>

      <main class="main-844">
        <input
          class="add-input-844 ${error() ? "add-input--error-844" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-844" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-844">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-844">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-844 ${filter() === f ? "filter-btn--active-844" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-844">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-844">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-844">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-844 ${todo.done ? "todo-item--done-844" : ""}">
                    <button
                      class="todo-check-844 ${todo.done ? "todo-check--done-844" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-844" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-844"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-844" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-844" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-844">
            <span class="todo-count-844">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-844" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
