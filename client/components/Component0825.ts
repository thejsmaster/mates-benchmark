// client/components/Component0825.ts
// Benchmark component #825 — identical logic to TodoPage, scoped class names.
...
import "./Component0825.css";

type Filter = "all" | "active" | "done";

export const Component0825 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0825] mounted");
    return () => console.log("[Component0825] unmounted");
  });
  ...
  return () => html`
    <div class="app-825">
      <header class="header-825">
        <h1>todos — Component0825</h1>
      </header>

      <main class="main-825">
        <input
          class="add-input-825 ${error() ? "add-input--error-825" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-825" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-825">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-825">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-825 ${filter() === f ? "filter-btn--active-825" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-825">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-825">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-825">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-825 ${todo.done ? "todo-item--done-825" : ""}">
                    <button
                      class="todo-check-825 ${todo.done ? "todo-check--done-825" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-825" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-825"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-825" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-825" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-825">
            <span class="todo-count-825">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-825" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
