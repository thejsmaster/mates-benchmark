// client/components/Component0826.ts
// Benchmark component #826 — identical logic to TodoPage, scoped class names.
...
import "./Component0826.css";

type Filter = "all" | "active" | "done";

export const Component0826 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0826] mounted");
    return () => console.log("[Component0826] unmounted");
  });
  ...
  return () => html`
    <div class="app-826">
      <header class="header-826">
        <h1>todos — Component0826</h1>
      </header>

      <main class="main-826">
        <input
          class="add-input-826 ${error() ? "add-input--error-826" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-826" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-826">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-826">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-826 ${filter() === f ? "filter-btn--active-826" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-826">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-826">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-826">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-826 ${todo.done ? "todo-item--done-826" : ""}">
                    <button
                      class="todo-check-826 ${todo.done ? "todo-check--done-826" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-826" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-826"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-826" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-826" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-826">
            <span class="todo-count-826">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-826" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
