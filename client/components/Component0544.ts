// client/components/Component0544.ts
// Benchmark component #544 — identical logic to TodoPage, scoped class names.
...
import "./Component0544.css";

type Filter = "all" | "active" | "done";

export const Component0544 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0544] mounted");
    return () => console.log("[Component0544] unmounted");
  });
  ...
  return () => html`
    <div class="app-544">
      <header class="header-544">
        <h1>todos — Component0544</h1>
      </header>

      <main class="main-544">
        <input
          class="add-input-544 ${error() ? "add-input--error-544" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-544" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-544">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-544">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-544 ${filter() === f ? "filter-btn--active-544" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-544">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-544">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-544">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-544 ${todo.done ? "todo-item--done-544" : ""}">
                    <button
                      class="todo-check-544 ${todo.done ? "todo-check--done-544" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-544" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-544"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-544" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-544" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-544">
            <span class="todo-count-544">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-544" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
