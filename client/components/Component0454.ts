// client/components/Component0454.ts
// Benchmark component #454 — identical logic to TodoPage, scoped class names.
...
import "./Component0454.css";

type Filter = "all" | "active" | "done";

export const Component0454 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0454] mounted");
    return () => console.log("[Component0454] unmounted");
  });
  ...
  return () => html`
    <div class="app-454">
      <header class="header-454">
        <h1>todos — Component0454</h1>
      </header>

      <main class="main-454">
        <input
          class="add-input-454 ${error() ? "add-input--error-454" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-454" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-454">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-454">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-454 ${filter() === f ? "filter-btn--active-454" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-454">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-454">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-454">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-454 ${todo.done ? "todo-item--done-454" : ""}">
                    <button
                      class="todo-check-454 ${todo.done ? "todo-check--done-454" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-454" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-454"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-454" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-454" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-454">
            <span class="todo-count-454">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-454" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
