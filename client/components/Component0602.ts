// client/components/Component0602.ts
// Benchmark component #602 — identical logic to TodoPage, scoped class names.
...
import "./Component0602.css";

type Filter = "all" | "active" | "done";

export const Component0602 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0602] mounted");
    return () => console.log("[Component0602] unmounted");
  });
  ...
  return () => html`
    <div class="app-602">
      <header class="header-602">
        <h1>todos — Component0602</h1>
      </header>

      <main class="main-602">
        <input
          class="add-input-602 ${error() ? "add-input--error-602" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-602" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-602">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-602">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-602 ${filter() === f ? "filter-btn--active-602" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-602">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-602">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-602">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-602 ${todo.done ? "todo-item--done-602" : ""}">
                    <button
                      class="todo-check-602 ${todo.done ? "todo-check--done-602" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-602" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-602"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-602" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-602" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-602">
            <span class="todo-count-602">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-602" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
