// client/components/Component0289.ts
// Benchmark component #289 — identical logic to TodoPage, scoped class names.
...
import "./Component0289.css";

type Filter = "all" | "active" | "done";

export const Component0289 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0289] mounted");
    return () => console.log("[Component0289] unmounted");
  });
  ...
  return () => html`
    <div class="app-289">
      <header class="header-289">
        <h1>todos — Component0289</h1>
      </header>

      <main class="main-289">
        <input
          class="add-input-289 ${error() ? "add-input--error-289" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-289" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-289">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-289">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-289 ${filter() === f ? "filter-btn--active-289" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-289">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-289">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-289">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-289 ${todo.done ? "todo-item--done-289" : ""}">
                    <button
                      class="todo-check-289 ${todo.done ? "todo-check--done-289" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-289" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-289"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-289" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-289" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-289">
            <span class="todo-count-289">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-289" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
