// client/components/Component0234.ts
// Benchmark component #234 — identical logic to TodoPage, scoped class names.
...
import "./Component0234.css";

type Filter = "all" | "active" | "done";

export const Component0234 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0234] mounted");
    return () => console.log("[Component0234] unmounted");
  });
  ...
  return () => html`
    <div class="app-234">
      <header class="header-234">
        <h1>todos — Component0234</h1>
      </header>

      <main class="main-234">
        <input
          class="add-input-234 ${error() ? "add-input--error-234" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-234" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-234">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-234">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-234 ${filter() === f ? "filter-btn--active-234" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-234">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-234">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-234">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-234 ${todo.done ? "todo-item--done-234" : ""}">
                    <button
                      class="todo-check-234 ${todo.done ? "todo-check--done-234" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-234" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-234"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-234" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-234" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-234">
            <span class="todo-count-234">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-234" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
