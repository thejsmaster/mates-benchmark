// client/components/Component0448.ts
// Benchmark component #448 — identical logic to TodoPage, scoped class names.
...
import "./Component0448.css";

type Filter = "all" | "active" | "done";

export const Component0448 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0448] mounted");
    return () => console.log("[Component0448] unmounted");
  });
  ...
  return () => html`
    <div class="app-448">
      <header class="header-448">
        <h1>todos — Component0448</h1>
      </header>

      <main class="main-448">
        <input
          class="add-input-448 ${error() ? "add-input--error-448" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-448" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-448">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-448">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-448 ${filter() === f ? "filter-btn--active-448" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-448">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-448">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-448">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-448 ${todo.done ? "todo-item--done-448" : ""}">
                    <button
                      class="todo-check-448 ${todo.done ? "todo-check--done-448" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-448" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-448"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-448" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-448" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-448">
            <span class="todo-count-448">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-448" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
