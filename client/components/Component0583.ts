// client/components/Component0583.ts
// Benchmark component #583 — identical logic to TodoPage, scoped class names.
...
import "./Component0583.css";

type Filter = "all" | "active" | "done";

export const Component0583 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0583] mounted");
    return () => console.log("[Component0583] unmounted");
  });
  ...
  return () => html`
    <div class="app-583">
      <header class="header-583">
        <h1>todos — Component0583</h1>
      </header>

      <main class="main-583">
        <input
          class="add-input-583 ${error() ? "add-input--error-583" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-583" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-583">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-583">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-583 ${filter() === f ? "filter-btn--active-583" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-583">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-583">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-583">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-583 ${todo.done ? "todo-item--done-583" : ""}">
                    <button
                      class="todo-check-583 ${todo.done ? "todo-check--done-583" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-583" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-583"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-583" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-583" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-583">
            <span class="todo-count-583">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-583" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
