// client/components/Component0720.ts
// Benchmark component #720 — identical logic to TodoPage, scoped class names.
...
import "./Component0720.css";

type Filter = "all" | "active" | "done";

export const Component0720 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0720] mounted");
    return () => console.log("[Component0720] unmounted");
  });
  ...
  return () => html`
    <div class="app-720">
      <header class="header-720">
        <h1>todos — Component0720</h1>
      </header>

      <main class="main-720">
        <input
          class="add-input-720 ${error() ? "add-input--error-720" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-720" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-720">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-720">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-720 ${filter() === f ? "filter-btn--active-720" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-720">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-720">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-720">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-720 ${todo.done ? "todo-item--done-720" : ""}">
                    <button
                      class="todo-check-720 ${todo.done ? "todo-check--done-720" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-720" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-720"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-720" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-720" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-720">
            <span class="todo-count-720">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-720" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
