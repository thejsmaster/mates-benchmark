// client/components/Component0950.ts
// Benchmark component #950 — identical logic to TodoPage, scoped class names.
...
import "./Component0950.css";

type Filter = "all" | "active" | "done";

export const Component0950 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0950] mounted");
    return () => console.log("[Component0950] unmounted");
  });
  ...
  return () => html`
    <div class="app-950">
      <header class="header-950">
        <h1>todos — Component0950</h1>
      </header>

      <main class="main-950">
        <input
          class="add-input-950 ${error() ? "add-input--error-950" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-950" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-950">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-950">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-950 ${filter() === f ? "filter-btn--active-950" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-950">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-950">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-950">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-950 ${todo.done ? "todo-item--done-950" : ""}">
                    <button
                      class="todo-check-950 ${todo.done ? "todo-check--done-950" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-950" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-950"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-950" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-950" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-950">
            <span class="todo-count-950">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-950" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
