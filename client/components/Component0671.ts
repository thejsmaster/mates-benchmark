// client/components/Component0671.ts
// Benchmark component #671 — identical logic to TodoPage, scoped class names.
...
import "./Component0671.css";

type Filter = "all" | "active" | "done";

export const Component0671 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0671] mounted");
    return () => console.log("[Component0671] unmounted");
  });
  ...
  return () => html`
    <div class="app-671">
      <header class="header-671">
        <h1>todos — Component0671</h1>
      </header>

      <main class="main-671">
        <input
          class="add-input-671 ${error() ? "add-input--error-671" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-671" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-671">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-671">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-671 ${filter() === f ? "filter-btn--active-671" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-671">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-671">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-671">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-671 ${todo.done ? "todo-item--done-671" : ""}">
                    <button
                      class="todo-check-671 ${todo.done ? "todo-check--done-671" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-671" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-671"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-671" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-671" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-671">
            <span class="todo-count-671">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-671" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
