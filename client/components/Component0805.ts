// client/components/Component0805.ts
// Benchmark component #805 — identical logic to TodoPage, scoped class names.
...
import "./Component0805.css";

type Filter = "all" | "active" | "done";

export const Component0805 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0805] mounted");
    return () => console.log("[Component0805] unmounted");
  });
  ...
  return () => html`
    <div class="app-805">
      <header class="header-805">
        <h1>todos — Component0805</h1>
      </header>

      <main class="main-805">
        <input
          class="add-input-805 ${error() ? "add-input--error-805" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-805" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-805">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-805">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-805 ${filter() === f ? "filter-btn--active-805" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-805">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-805">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-805">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-805 ${todo.done ? "todo-item--done-805" : ""}">
                    <button
                      class="todo-check-805 ${todo.done ? "todo-check--done-805" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-805" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-805"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-805" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-805" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-805">
            <span class="todo-count-805">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-805" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
