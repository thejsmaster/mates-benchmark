// client/components/Component0721.ts
// Benchmark component #721 — identical logic to TodoPage, scoped class names.
...
import "./Component0721.css";

type Filter = "all" | "active" | "done";

export const Component0721 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0721] mounted");
    return () => console.log("[Component0721] unmounted");
  });
  ...
  return () => html`
    <div class="app-721">
      <header class="header-721">
        <h1>todos — Component0721</h1>
      </header>

      <main class="main-721">
        <input
          class="add-input-721 ${error() ? "add-input--error-721" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-721" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-721">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-721">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-721 ${filter() === f ? "filter-btn--active-721" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-721">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-721">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-721">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-721 ${todo.done ? "todo-item--done-721" : ""}">
                    <button
                      class="todo-check-721 ${todo.done ? "todo-check--done-721" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-721" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-721"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-721" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-721" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-721">
            <span class="todo-count-721">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-721" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
