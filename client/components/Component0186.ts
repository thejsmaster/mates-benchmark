// client/components/Component0186.ts
// Benchmark component #186 — identical logic to TodoPage, scoped class names.
...
import "./Component0186.css";

type Filter = "all" | "active" | "done";

export const Component0186 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0186] mounted");
    return () => console.log("[Component0186] unmounted");
  });
  ...
  return () => html`
    <div class="app-186">
      <header class="header-186">
        <h1>todos — Component0186</h1>
      </header>

      <main class="main-186">
        <input
          class="add-input-186 ${error() ? "add-input--error-186" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-186" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-186">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-186">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-186 ${filter() === f ? "filter-btn--active-186" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-186">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-186">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-186">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-186 ${todo.done ? "todo-item--done-186" : ""}">
                    <button
                      class="todo-check-186 ${todo.done ? "todo-check--done-186" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-186" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-186"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-186" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-186" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-186">
            <span class="todo-count-186">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-186" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
