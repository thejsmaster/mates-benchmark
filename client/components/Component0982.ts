// client/components/Component0982.ts
// Benchmark component #982 — identical logic to TodoPage, scoped class names.
...
import "./Component0982.css";

type Filter = "all" | "active" | "done";

export const Component0982 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0982] mounted");
    return () => console.log("[Component0982] unmounted");
  });
  ...
  return () => html`
    <div class="app-982">
      <header class="header-982">
        <h1>todos — Component0982</h1>
      </header>

      <main class="main-982">
        <input
          class="add-input-982 ${error() ? "add-input--error-982" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-982" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-982">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-982">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-982 ${filter() === f ? "filter-btn--active-982" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-982">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-982">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-982">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-982 ${todo.done ? "todo-item--done-982" : ""}">
                    <button
                      class="todo-check-982 ${todo.done ? "todo-check--done-982" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-982" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-982"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-982" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-982" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-982">
            <span class="todo-count-982">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-982" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
