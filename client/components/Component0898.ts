// client/components/Component0898.ts
// Benchmark component #898 — identical logic to TodoPage, scoped class names.
...
import "./Component0898.css";

type Filter = "all" | "active" | "done";

export const Component0898 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0898] mounted");
    return () => console.log("[Component0898] unmounted");
  });
  ...
  return () => html`
    <div class="app-898">
      <header class="header-898">
        <h1>todos — Component0898</h1>
      </header>

      <main class="main-898">
        <input
          class="add-input-898 ${error() ? "add-input--error-898" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-898" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-898">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-898">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-898 ${filter() === f ? "filter-btn--active-898" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-898">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-898">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-898">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-898 ${todo.done ? "todo-item--done-898" : ""}">
                    <button
                      class="todo-check-898 ${todo.done ? "todo-check--done-898" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-898" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-898"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-898" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-898" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-898">
            <span class="todo-count-898">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-898" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
