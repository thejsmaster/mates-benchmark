// client/components/Component0373.ts
// Benchmark component #373 — identical logic to TodoPage, scoped class names.
...
import "./Component0373.css";

type Filter = "all" | "active" | "done";

export const Component0373 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0373] mounted");
    return () => console.log("[Component0373] unmounted");
  });
  ...
  return () => html`
    <div class="app-373">
      <header class="header-373">
        <h1>todos — Component0373</h1>
      </header>

      <main class="main-373">
        <input
          class="add-input-373 ${error() ? "add-input--error-373" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-373" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-373">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-373">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-373 ${filter() === f ? "filter-btn--active-373" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-373">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-373">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-373">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-373 ${todo.done ? "todo-item--done-373" : ""}">
                    <button
                      class="todo-check-373 ${todo.done ? "todo-check--done-373" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-373" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-373"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-373" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-373" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-373">
            <span class="todo-count-373">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-373" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
