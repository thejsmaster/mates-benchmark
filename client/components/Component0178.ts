// client/components/Component0178.ts
// Benchmark component #178 — identical logic to TodoPage, scoped class names.
...
import "./Component0178.css";

type Filter = "all" | "active" | "done";

export const Component0178 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0178] mounted");
    return () => console.log("[Component0178] unmounted");
  });
  ...
  return () => html`
    <div class="app-178">
      <header class="header-178">
        <h1>todos — Component0178</h1>
      </header>

      <main class="main-178">
        <input
          class="add-input-178 ${error() ? "add-input--error-178" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-178" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-178">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-178">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-178 ${filter() === f ? "filter-btn--active-178" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-178">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-178">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-178">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-178 ${todo.done ? "todo-item--done-178" : ""}">
                    <button
                      class="todo-check-178 ${todo.done ? "todo-check--done-178" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-178" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-178"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-178" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-178" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-178">
            <span class="todo-count-178">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-178" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
