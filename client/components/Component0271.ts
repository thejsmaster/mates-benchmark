// client/components/Component0271.ts
// Benchmark component #271 — identical logic to TodoPage, scoped class names.
...
import "./Component0271.css";

type Filter = "all" | "active" | "done";

export const Component0271 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0271] mounted");
    return () => console.log("[Component0271] unmounted");
  });
  ...
  return () => html`
    <div class="app-271">
      <header class="header-271">
        <h1>todos — Component0271</h1>
      </header>

      <main class="main-271">
        <input
          class="add-input-271 ${error() ? "add-input--error-271" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-271" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-271">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-271">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-271 ${filter() === f ? "filter-btn--active-271" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-271">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-271">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-271">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-271 ${todo.done ? "todo-item--done-271" : ""}">
                    <button
                      class="todo-check-271 ${todo.done ? "todo-check--done-271" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-271" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-271"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-271" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-271" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-271">
            <span class="todo-count-271">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-271" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
