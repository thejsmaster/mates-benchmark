// client/components/Component0622.ts
// Benchmark component #622 — identical logic to TodoPage, scoped class names.
...
import "./Component0622.css";

type Filter = "all" | "active" | "done";

export const Component0622 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0622] mounted");
    return () => console.log("[Component0622] unmounted");
  });
  ...
  return () => html`
    <div class="app-622">
      <header class="header-622">
        <h1>todos — Component0622</h1>
      </header>

      <main class="main-622">
        <input
          class="add-input-622 ${error() ? "add-input--error-622" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-622" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-622">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-622">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-622 ${filter() === f ? "filter-btn--active-622" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-622">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-622">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-622">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-622 ${todo.done ? "todo-item--done-622" : ""}">
                    <button
                      class="todo-check-622 ${todo.done ? "todo-check--done-622" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-622" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-622"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-622" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-622" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-622">
            <span class="todo-count-622">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-622" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
