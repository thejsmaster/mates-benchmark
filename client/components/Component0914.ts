// client/components/Component0914.ts
// Benchmark component #914 — identical logic to TodoPage, scoped class names.
...
import "./Component0914.css";

type Filter = "all" | "active" | "done";

export const Component0914 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0914] mounted");
    return () => console.log("[Component0914] unmounted");
  });
  ...
  return () => html`
    <div class="app-914">
      <header class="header-914">
        <h1>todos — Component0914</h1>
      </header>

      <main class="main-914">
        <input
          class="add-input-914 ${error() ? "add-input--error-914" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-914" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-914">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-914">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-914 ${filter() === f ? "filter-btn--active-914" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-914">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-914">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-914">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-914 ${todo.done ? "todo-item--done-914" : ""}">
                    <button
                      class="todo-check-914 ${todo.done ? "todo-check--done-914" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-914" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-914"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-914" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-914" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-914">
            <span class="todo-count-914">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-914" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
