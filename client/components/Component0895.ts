// client/components/Component0895.ts
// Benchmark component #895 — identical logic to TodoPage, scoped class names.
...
import "./Component0895.css";

type Filter = "all" | "active" | "done";

export const Component0895 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0895] mounted");
    return () => console.log("[Component0895] unmounted");
  });
  ...
  return () => html`
    <div class="app-895">
      <header class="header-895">
        <h1>todos — Component0895</h1>
      </header>

      <main class="main-895">
        <input
          class="add-input-895 ${error() ? "add-input--error-895" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-895" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-895">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-895">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-895 ${filter() === f ? "filter-btn--active-895" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-895">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-895">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-895">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-895 ${todo.done ? "todo-item--done-895" : ""}">
                    <button
                      class="todo-check-895 ${todo.done ? "todo-check--done-895" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-895" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-895"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-895" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-895" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-895">
            <span class="todo-count-895">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-895" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
