// client/components/Component0286.ts
// Benchmark component #286 — identical logic to TodoPage, scoped class names.
...
import "./Component0286.css";

type Filter = "all" | "active" | "done";

export const Component0286 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0286] mounted");
    return () => console.log("[Component0286] unmounted");
  });
  ...
  return () => html`
    <div class="app-286">
      <header class="header-286">
        <h1>todos — Component0286</h1>
      </header>

      <main class="main-286">
        <input
          class="add-input-286 ${error() ? "add-input--error-286" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-286" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-286">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-286">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-286 ${filter() === f ? "filter-btn--active-286" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-286">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-286">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-286">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-286 ${todo.done ? "todo-item--done-286" : ""}">
                    <button
                      class="todo-check-286 ${todo.done ? "todo-check--done-286" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-286" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-286"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-286" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-286" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-286">
            <span class="todo-count-286">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-286" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
