// client/components/Component0336.ts
// Benchmark component #336 — identical logic to TodoPage, scoped class names.
...
import "./Component0336.css";

type Filter = "all" | "active" | "done";

export const Component0336 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0336] mounted");
    return () => console.log("[Component0336] unmounted");
  });
  ...
  return () => html`
    <div class="app-336">
      <header class="header-336">
        <h1>todos — Component0336</h1>
      </header>

      <main class="main-336">
        <input
          class="add-input-336 ${error() ? "add-input--error-336" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-336" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-336">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-336">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-336 ${filter() === f ? "filter-btn--active-336" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-336">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-336">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-336">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-336 ${todo.done ? "todo-item--done-336" : ""}">
                    <button
                      class="todo-check-336 ${todo.done ? "todo-check--done-336" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-336" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-336"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-336" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-336" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-336">
            <span class="todo-count-336">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-336" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
