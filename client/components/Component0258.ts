// client/components/Component0258.ts
// Benchmark component #258 — identical logic to TodoPage, scoped class names.
...
import "./Component0258.css";

type Filter = "all" | "active" | "done";

export const Component0258 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0258] mounted");
    return () => console.log("[Component0258] unmounted");
  });
  ...
  return () => html`
    <div class="app-258">
      <header class="header-258">
        <h1>todos — Component0258</h1>
      </header>

      <main class="main-258">
        <input
          class="add-input-258 ${error() ? "add-input--error-258" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-258" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-258">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-258">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-258 ${filter() === f ? "filter-btn--active-258" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-258">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-258">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-258">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-258 ${todo.done ? "todo-item--done-258" : ""}">
                    <button
                      class="todo-check-258 ${todo.done ? "todo-check--done-258" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-258" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-258"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-258" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-258" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-258">
            <span class="todo-count-258">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-258" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
