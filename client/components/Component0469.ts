// client/components/Component0469.ts
// Benchmark component #469 — identical logic to TodoPage, scoped class names.
...
import "./Component0469.css";

type Filter = "all" | "active" | "done";

export const Component0469 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0469] mounted");
    return () => console.log("[Component0469] unmounted");
  });
  ...
  return () => html`
    <div class="app-469">
      <header class="header-469">
        <h1>todos — Component0469</h1>
      </header>

      <main class="main-469">
        <input
          class="add-input-469 ${error() ? "add-input--error-469" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-469" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-469">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-469">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-469 ${filter() === f ? "filter-btn--active-469" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-469">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-469">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-469">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-469 ${todo.done ? "todo-item--done-469" : ""}">
                    <button
                      class="todo-check-469 ${todo.done ? "todo-check--done-469" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-469" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-469"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-469" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-469" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-469">
            <span class="todo-count-469">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-469" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
