// client/components/Component0979.ts
// Benchmark component #979 — identical logic to TodoPage, scoped class names.
...
import "./Component0979.css";

type Filter = "all" | "active" | "done";

export const Component0979 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0979] mounted");
    return () => console.log("[Component0979] unmounted");
  });
  ...
  return () => html`
    <div class="app-979">
      <header class="header-979">
        <h1>todos — Component0979</h1>
      </header>

      <main class="main-979">
        <input
          class="add-input-979 ${error() ? "add-input--error-979" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-979" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-979">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-979">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-979 ${filter() === f ? "filter-btn--active-979" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-979">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-979">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-979">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-979 ${todo.done ? "todo-item--done-979" : ""}">
                    <button
                      class="todo-check-979 ${todo.done ? "todo-check--done-979" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-979" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-979"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-979" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-979" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-979">
            <span class="todo-count-979">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-979" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
