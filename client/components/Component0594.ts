// client/components/Component0594.ts
// Benchmark component #594 — identical logic to TodoPage, scoped class names.
...
import "./Component0594.css";

type Filter = "all" | "active" | "done";

export const Component0594 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0594] mounted");
    return () => console.log("[Component0594] unmounted");
  });
  ...
  return () => html`
    <div class="app-594">
      <header class="header-594">
        <h1>todos — Component0594</h1>
      </header>

      <main class="main-594">
        <input
          class="add-input-594 ${error() ? "add-input--error-594" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-594" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-594">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-594">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-594 ${filter() === f ? "filter-btn--active-594" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-594">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-594">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-594">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-594 ${todo.done ? "todo-item--done-594" : ""}">
                    <button
                      class="todo-check-594 ${todo.done ? "todo-check--done-594" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-594" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-594"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-594" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-594" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-594">
            <span class="todo-count-594">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-594" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
