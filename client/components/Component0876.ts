// client/components/Component0876.ts
// Benchmark component #876 — identical logic to TodoPage, scoped class names.
...
import "./Component0876.css";

type Filter = "all" | "active" | "done";

export const Component0876 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0876] mounted");
    return () => console.log("[Component0876] unmounted");
  });
  ...
  return () => html`
    <div class="app-876">
      <header class="header-876">
        <h1>todos — Component0876</h1>
      </header>

      <main class="main-876">
        <input
          class="add-input-876 ${error() ? "add-input--error-876" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-876" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-876">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-876">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-876 ${filter() === f ? "filter-btn--active-876" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-876">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-876">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-876">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-876 ${todo.done ? "todo-item--done-876" : ""}">
                    <button
                      class="todo-check-876 ${todo.done ? "todo-check--done-876" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-876" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-876"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-876" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-876" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-876">
            <span class="todo-count-876">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-876" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
