// client/components/Component0792.ts
// Benchmark component #792 — identical logic to TodoPage, scoped class names.
...
import "./Component0792.css";

type Filter = "all" | "active" | "done";

export const Component0792 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0792] mounted");
    return () => console.log("[Component0792] unmounted");
  });
  ...
  return () => html`
    <div class="app-792">
      <header class="header-792">
        <h1>todos — Component0792</h1>
      </header>

      <main class="main-792">
        <input
          class="add-input-792 ${error() ? "add-input--error-792" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-792" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-792">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-792">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-792 ${filter() === f ? "filter-btn--active-792" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-792">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-792">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-792">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-792 ${todo.done ? "todo-item--done-792" : ""}">
                    <button
                      class="todo-check-792 ${todo.done ? "todo-check--done-792" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-792" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-792"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-792" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-792" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-792">
            <span class="todo-count-792">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-792" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
