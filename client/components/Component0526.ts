// client/components/Component0526.ts
// Benchmark component #526 — identical logic to TodoPage, scoped class names.
...
import "./Component0526.css";

type Filter = "all" | "active" | "done";

export const Component0526 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0526] mounted");
    return () => console.log("[Component0526] unmounted");
  });
  ...
  return () => html`
    <div class="app-526">
      <header class="header-526">
        <h1>todos — Component0526</h1>
      </header>

      <main class="main-526">
        <input
          class="add-input-526 ${error() ? "add-input--error-526" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-526" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-526">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-526">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-526 ${filter() === f ? "filter-btn--active-526" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-526">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-526">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-526">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-526 ${todo.done ? "todo-item--done-526" : ""}">
                    <button
                      class="todo-check-526 ${todo.done ? "todo-check--done-526" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-526" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-526"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-526" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-526" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-526">
            <span class="todo-count-526">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-526" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
