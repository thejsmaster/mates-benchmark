// client/components/Component0441.ts
// Benchmark component #441 — identical logic to TodoPage, scoped class names.
...
import "./Component0441.css";

type Filter = "all" | "active" | "done";

export const Component0441 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0441] mounted");
    return () => console.log("[Component0441] unmounted");
  });
  ...
  return () => html`
    <div class="app-441">
      <header class="header-441">
        <h1>todos — Component0441</h1>
      </header>

      <main class="main-441">
        <input
          class="add-input-441 ${error() ? "add-input--error-441" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-441" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-441">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-441">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-441 ${filter() === f ? "filter-btn--active-441" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-441">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-441">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-441">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-441 ${todo.done ? "todo-item--done-441" : ""}">
                    <button
                      class="todo-check-441 ${todo.done ? "todo-check--done-441" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-441" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-441"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-441" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-441" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-441">
            <span class="todo-count-441">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-441" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
