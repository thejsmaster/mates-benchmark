// client/components/Component0335.ts
// Benchmark component #335 — identical logic to TodoPage, scoped class names.
...
import "./Component0335.css";

type Filter = "all" | "active" | "done";

export const Component0335 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0335] mounted");
    return () => console.log("[Component0335] unmounted");
  });
  ...
  return () => html`
    <div class="app-335">
      <header class="header-335">
        <h1>todos — Component0335</h1>
      </header>

      <main class="main-335">
        <input
          class="add-input-335 ${error() ? "add-input--error-335" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-335" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-335">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-335">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-335 ${filter() === f ? "filter-btn--active-335" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-335">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-335">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-335">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-335 ${todo.done ? "todo-item--done-335" : ""}">
                    <button
                      class="todo-check-335 ${todo.done ? "todo-check--done-335" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-335" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-335"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-335" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-335" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-335">
            <span class="todo-count-335">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-335" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
