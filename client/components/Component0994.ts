// client/components/Component0994.ts
// Benchmark component #994 — identical logic to TodoPage, scoped class names.
...
import "./Component0994.css";

type Filter = "all" | "active" | "done";

export const Component0994 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0994] mounted");
    return () => console.log("[Component0994] unmounted");
  });
  ...
  return () => html`
    <div class="app-994">
      <header class="header-994">
        <h1>todos — Component0994</h1>
      </header>

      <main class="main-994">
        <input
          class="add-input-994 ${error() ? "add-input--error-994" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-994" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-994">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-994">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-994 ${filter() === f ? "filter-btn--active-994" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-994">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-994">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-994">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-994 ${todo.done ? "todo-item--done-994" : ""}">
                    <button
                      class="todo-check-994 ${todo.done ? "todo-check--done-994" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-994" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-994"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-994" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-994" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-994">
            <span class="todo-count-994">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-994" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
