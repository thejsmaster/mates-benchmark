// client/components/Component0407.ts
// Benchmark component #407 — identical logic to TodoPage, scoped class names.
...
import "./Component0407.css";

type Filter = "all" | "active" | "done";

export const Component0407 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0407] mounted");
    return () => console.log("[Component0407] unmounted");
  });
  ...
  return () => html`
    <div class="app-407">
      <header class="header-407">
        <h1>todos — Component0407</h1>
      </header>

      <main class="main-407">
        <input
          class="add-input-407 ${error() ? "add-input--error-407" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-407" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-407">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-407">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-407 ${filter() === f ? "filter-btn--active-407" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-407">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-407">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-407">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-407 ${todo.done ? "todo-item--done-407" : ""}">
                    <button
                      class="todo-check-407 ${todo.done ? "todo-check--done-407" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-407" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-407"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-407" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-407" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-407">
            <span class="todo-count-407">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-407" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
