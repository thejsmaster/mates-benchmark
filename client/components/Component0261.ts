// client/components/Component0261.ts
// Benchmark component #261 — identical logic to TodoPage, scoped class names.
...
import "./Component0261.css";

type Filter = "all" | "active" | "done";

export const Component0261 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0261] mounted");
    return () => console.log("[Component0261] unmounted");
  });
  ...
  return () => html`
    <div class="app-261">
      <header class="header-261">
        <h1>todos — Component0261</h1>
      </header>

      <main class="main-261">
        <input
          class="add-input-261 ${error() ? "add-input--error-261" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-261" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-261">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-261">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-261 ${filter() === f ? "filter-btn--active-261" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-261">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-261">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-261">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-261 ${todo.done ? "todo-item--done-261" : ""}">
                    <button
                      class="todo-check-261 ${todo.done ? "todo-check--done-261" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-261" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-261"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-261" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-261" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-261">
            <span class="todo-count-261">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-261" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
