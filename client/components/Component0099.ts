// client/components/Component0099.ts
// Benchmark component #99 — identical logic to TodoPage, scoped class names.
...
import "./Component0099.css";

type Filter = "all" | "active" | "done";

export const Component0099 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0099] mounted");
    return () => console.log("[Component0099] unmounted");
  });
  ...
  return () => html`
    <div class="app-99">
      <header class="header-99">
        <h1>todos — Component0099</h1>
      </header>

      <main class="main-99">
        <input
          class="add-input-99 ${error() ? "add-input--error-99" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-99" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-99">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-99">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-99 ${filter() === f ? "filter-btn--active-99" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-99">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-99">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-99">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-99 ${todo.done ? "todo-item--done-99" : ""}">
                    <button
                      class="todo-check-99 ${todo.done ? "todo-check--done-99" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-99" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-99"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-99" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-99" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-99">
            <span class="todo-count-99">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-99" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
