// client/components/Component0074.ts
// Benchmark component #74 — identical logic to TodoPage, scoped class names.
...
import "./Component0074.css";

type Filter = "all" | "active" | "done";

export const Component0074 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0074] mounted");
    return () => console.log("[Component0074] unmounted");
  });
  ...
  return () => html`
    <div class="app-74">
      <header class="header-74">
        <h1>todos — Component0074</h1>
      </header>

      <main class="main-74">
        <input
          class="add-input-74 ${error() ? "add-input--error-74" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-74" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-74">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-74">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-74 ${filter() === f ? "filter-btn--active-74" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-74">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-74">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-74">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-74 ${todo.done ? "todo-item--done-74" : ""}">
                    <button
                      class="todo-check-74 ${todo.done ? "todo-check--done-74" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-74" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-74"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-74" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-74" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-74">
            <span class="todo-count-74">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-74" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
