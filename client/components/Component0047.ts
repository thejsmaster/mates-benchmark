// client/components/Component0047.ts
// Benchmark component #47 — identical logic to TodoPage, scoped class names.
...
import "./Component0047.css";

type Filter = "all" | "active" | "done";

export const Component0047 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0047] mounted");
    return () => console.log("[Component0047] unmounted");
  });
  ...
  return () => html`
    <div class="app-47">
      <header class="header-47">
        <h1>todos — Component0047</h1>
      </header>

      <main class="main-47">
        <input
          class="add-input-47 ${error() ? "add-input--error-47" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-47" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-47">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-47">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-47 ${filter() === f ? "filter-btn--active-47" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-47">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-47">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-47">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-47 ${todo.done ? "todo-item--done-47" : ""}">
                    <button
                      class="todo-check-47 ${todo.done ? "todo-check--done-47" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-47" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-47"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-47" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-47" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-47">
            <span class="todo-count-47">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-47" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
