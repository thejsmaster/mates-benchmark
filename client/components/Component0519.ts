// client/components/Component0519.ts
// Benchmark component #519 — identical logic to TodoPage, scoped class names.
...
import "./Component0519.css";

type Filter = "all" | "active" | "done";

export const Component0519 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0519] mounted");
    return () => console.log("[Component0519] unmounted");
  });
  ...
  return () => html`
    <div class="app-519">
      <header class="header-519">
        <h1>todos — Component0519</h1>
      </header>

      <main class="main-519">
        <input
          class="add-input-519 ${error() ? "add-input--error-519" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-519" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-519">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-519">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-519 ${filter() === f ? "filter-btn--active-519" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-519">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-519">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-519">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-519 ${todo.done ? "todo-item--done-519" : ""}">
                    <button
                      class="todo-check-519 ${todo.done ? "todo-check--done-519" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-519" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-519"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-519" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-519" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-519">
            <span class="todo-count-519">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-519" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
