// client/components/Component0077.ts
// Benchmark component #77 — identical logic to TodoPage, scoped class names.
...
import "./Component0077.css";

type Filter = "all" | "active" | "done";

export const Component0077 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0077] mounted");
    return () => console.log("[Component0077] unmounted");
  });
  ...
  return () => html`
    <div class="app-77">
      <header class="header-77">
        <h1>todos — Component0077</h1>
      </header>

      <main class="main-77">
        <input
          class="add-input-77 ${error() ? "add-input--error-77" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-77" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-77">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-77">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-77 ${filter() === f ? "filter-btn--active-77" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-77">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-77">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-77">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-77 ${todo.done ? "todo-item--done-77" : ""}">
                    <button
                      class="todo-check-77 ${todo.done ? "todo-check--done-77" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-77" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-77"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-77" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-77" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-77">
            <span class="todo-count-77">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-77" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
