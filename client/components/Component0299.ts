// client/components/Component0299.ts
// Benchmark component #299 — identical logic to TodoPage, scoped class names.
...
import "./Component0299.css";

type Filter = "all" | "active" | "done";

export const Component0299 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0299] mounted");
    return () => console.log("[Component0299] unmounted");
  });
  ...
  return () => html`
    <div class="app-299">
      <header class="header-299">
        <h1>todos — Component0299</h1>
      </header>

      <main class="main-299">
        <input
          class="add-input-299 ${error() ? "add-input--error-299" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-299" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-299">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-299">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-299 ${filter() === f ? "filter-btn--active-299" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-299">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-299">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-299">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-299 ${todo.done ? "todo-item--done-299" : ""}">
                    <button
                      class="todo-check-299 ${todo.done ? "todo-check--done-299" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-299" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-299"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-299" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-299" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-299">
            <span class="todo-count-299">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-299" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
