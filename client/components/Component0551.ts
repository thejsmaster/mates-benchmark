// client/components/Component0551.ts
// Benchmark component #551 — identical logic to TodoPage, scoped class names.
...
import "./Component0551.css";

type Filter = "all" | "active" | "done";

export const Component0551 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0551] mounted");
    return () => console.log("[Component0551] unmounted");
  });
  ...
  return () => html`
    <div class="app-551">
      <header class="header-551">
        <h1>todos — Component0551</h1>
      </header>

      <main class="main-551">
        <input
          class="add-input-551 ${error() ? "add-input--error-551" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-551" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-551">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-551">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-551 ${filter() === f ? "filter-btn--active-551" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-551">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-551">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-551">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-551 ${todo.done ? "todo-item--done-551" : ""}">
                    <button
                      class="todo-check-551 ${todo.done ? "todo-check--done-551" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-551" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-551"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-551" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-551" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-551">
            <span class="todo-count-551">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-551" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
