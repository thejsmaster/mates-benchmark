// client/components/Component0771.ts
// Benchmark component #771 — identical logic to TodoPage, scoped class names.
...
import "./Component0771.css";

type Filter = "all" | "active" | "done";

export const Component0771 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0771] mounted");
    return () => console.log("[Component0771] unmounted");
  });
  ...
  return () => html`
    <div class="app-771">
      <header class="header-771">
        <h1>todos — Component0771</h1>
      </header>

      <main class="main-771">
        <input
          class="add-input-771 ${error() ? "add-input--error-771" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-771" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-771">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-771">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-771 ${filter() === f ? "filter-btn--active-771" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-771">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-771">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-771">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-771 ${todo.done ? "todo-item--done-771" : ""}">
                    <button
                      class="todo-check-771 ${todo.done ? "todo-check--done-771" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-771" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-771"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-771" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-771" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-771">
            <span class="todo-count-771">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-771" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
