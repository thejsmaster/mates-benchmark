// client/components/Component0492.ts
// Benchmark component #492 — identical logic to TodoPage, scoped class names.
...
import "./Component0492.css";

type Filter = "all" | "active" | "done";

export const Component0492 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0492] mounted");
    return () => console.log("[Component0492] unmounted");
  });
  ...
  return () => html`
    <div class="app-492">
      <header class="header-492">
        <h1>todos — Component0492</h1>
      </header>

      <main class="main-492">
        <input
          class="add-input-492 ${error() ? "add-input--error-492" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-492" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-492">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-492">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-492 ${filter() === f ? "filter-btn--active-492" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-492">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-492">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-492">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-492 ${todo.done ? "todo-item--done-492" : ""}">
                    <button
                      class="todo-check-492 ${todo.done ? "todo-check--done-492" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-492" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-492"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-492" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-492" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-492">
            <span class="todo-count-492">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-492" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
