// client/components/Component0192.ts
// Benchmark component #192 — identical logic to TodoPage, scoped class names.
...
import "./Component0192.css";

type Filter = "all" | "active" | "done";

export const Component0192 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0192] mounted");
    return () => console.log("[Component0192] unmounted");
  });
  ...
  return () => html`
    <div class="app-192">
      <header class="header-192">
        <h1>todos — Component0192</h1>
      </header>

      <main class="main-192">
        <input
          class="add-input-192 ${error() ? "add-input--error-192" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-192" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-192">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-192">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-192 ${filter() === f ? "filter-btn--active-192" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-192">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-192">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-192">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-192 ${todo.done ? "todo-item--done-192" : ""}">
                    <button
                      class="todo-check-192 ${todo.done ? "todo-check--done-192" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-192" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-192"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-192" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-192" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-192">
            <span class="todo-count-192">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-192" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
