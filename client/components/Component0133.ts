// client/components/Component0133.ts
// Benchmark component #133 — identical logic to TodoPage, scoped class names.
...
import "./Component0133.css";

type Filter = "all" | "active" | "done";

export const Component0133 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0133] mounted");
    return () => console.log("[Component0133] unmounted");
  });
  ...
  return () => html`
    <div class="app-133">
      <header class="header-133">
        <h1>todos — Component0133</h1>
      </header>

      <main class="main-133">
        <input
          class="add-input-133 ${error() ? "add-input--error-133" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-133" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-133">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-133">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-133 ${filter() === f ? "filter-btn--active-133" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-133">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-133">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-133">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-133 ${todo.done ? "todo-item--done-133" : ""}">
                    <button
                      class="todo-check-133 ${todo.done ? "todo-check--done-133" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-133" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-133"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-133" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-133" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-133">
            <span class="todo-count-133">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-133" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
