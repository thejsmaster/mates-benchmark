// client/components/Component0142.ts
// Benchmark component #142 — identical logic to TodoPage, scoped class names.
...
import "./Component0142.css";

type Filter = "all" | "active" | "done";

export const Component0142 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0142] mounted");
    return () => console.log("[Component0142] unmounted");
  });
  ...
  return () => html`
    <div class="app-142">
      <header class="header-142">
        <h1>todos — Component0142</h1>
      </header>

      <main class="main-142">
        <input
          class="add-input-142 ${error() ? "add-input--error-142" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-142" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-142">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-142">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-142 ${filter() === f ? "filter-btn--active-142" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-142">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-142">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-142">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-142 ${todo.done ? "todo-item--done-142" : ""}">
                    <button
                      class="todo-check-142 ${todo.done ? "todo-check--done-142" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-142" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-142"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-142" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-142" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-142">
            <span class="todo-count-142">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-142" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
