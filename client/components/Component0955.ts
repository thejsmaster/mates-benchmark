// client/components/Component0955.ts
// Benchmark component #955 — identical logic to TodoPage, scoped class names.
...
import "./Component0955.css";

type Filter = "all" | "active" | "done";

export const Component0955 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0955] mounted");
    return () => console.log("[Component0955] unmounted");
  });
  ...
  return () => html`
    <div class="app-955">
      <header class="header-955">
        <h1>todos — Component0955</h1>
      </header>

      <main class="main-955">
        <input
          class="add-input-955 ${error() ? "add-input--error-955" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-955" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-955">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-955">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-955 ${filter() === f ? "filter-btn--active-955" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-955">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-955">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-955">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-955 ${todo.done ? "todo-item--done-955" : ""}">
                    <button
                      class="todo-check-955 ${todo.done ? "todo-check--done-955" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-955" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-955"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-955" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-955" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-955">
            <span class="todo-count-955">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-955" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
