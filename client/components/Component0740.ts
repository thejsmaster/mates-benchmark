// client/components/Component0740.ts
// Benchmark component #740 — identical logic to TodoPage, scoped class names.
...
import "./Component0740.css";

type Filter = "all" | "active" | "done";

export const Component0740 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0740] mounted");
    return () => console.log("[Component0740] unmounted");
  });
  ...
  return () => html`
    <div class="app-740">
      <header class="header-740">
        <h1>todos — Component0740</h1>
      </header>

      <main class="main-740">
        <input
          class="add-input-740 ${error() ? "add-input--error-740" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-740" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-740">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-740">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-740 ${filter() === f ? "filter-btn--active-740" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-740">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-740">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-740">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-740 ${todo.done ? "todo-item--done-740" : ""}">
                    <button
                      class="todo-check-740 ${todo.done ? "todo-check--done-740" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-740" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-740"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-740" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-740" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-740">
            <span class="todo-count-740">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-740" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
