// client/components/Component0443.ts
// Benchmark component #443 — identical logic to TodoPage, scoped class names.
...
import "./Component0443.css";

type Filter = "all" | "active" | "done";

export const Component0443 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0443] mounted");
    return () => console.log("[Component0443] unmounted");
  });
  ...
  return () => html`
    <div class="app-443">
      <header class="header-443">
        <h1>todos — Component0443</h1>
      </header>

      <main class="main-443">
        <input
          class="add-input-443 ${error() ? "add-input--error-443" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-443" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-443">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-443">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-443 ${filter() === f ? "filter-btn--active-443" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-443">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-443">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-443">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-443 ${todo.done ? "todo-item--done-443" : ""}">
                    <button
                      class="todo-check-443 ${todo.done ? "todo-check--done-443" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-443" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-443"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-443" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-443" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-443">
            <span class="todo-count-443">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-443" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
