// client/components/Component0705.ts
// Benchmark component #705 — identical logic to TodoPage, scoped class names.
...
import "./Component0705.css";

type Filter = "all" | "active" | "done";

export const Component0705 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0705] mounted");
    return () => console.log("[Component0705] unmounted");
  });
  ...
  return () => html`
    <div class="app-705">
      <header class="header-705">
        <h1>todos — Component0705</h1>
      </header>

      <main class="main-705">
        <input
          class="add-input-705 ${error() ? "add-input--error-705" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-705" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-705">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-705">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-705 ${filter() === f ? "filter-btn--active-705" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-705">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-705">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-705">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-705 ${todo.done ? "todo-item--done-705" : ""}">
                    <button
                      class="todo-check-705 ${todo.done ? "todo-check--done-705" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-705" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-705"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-705" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-705" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-705">
            <span class="todo-count-705">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-705" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
