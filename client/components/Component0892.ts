// client/components/Component0892.ts
// Benchmark component #892 — identical logic to TodoPage, scoped class names.
...
import "./Component0892.css";

type Filter = "all" | "active" | "done";

export const Component0892 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0892] mounted");
    return () => console.log("[Component0892] unmounted");
  });
  ...
  return () => html`
    <div class="app-892">
      <header class="header-892">
        <h1>todos — Component0892</h1>
      </header>

      <main class="main-892">
        <input
          class="add-input-892 ${error() ? "add-input--error-892" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-892" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-892">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-892">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-892 ${filter() === f ? "filter-btn--active-892" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-892">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-892">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-892">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-892 ${todo.done ? "todo-item--done-892" : ""}">
                    <button
                      class="todo-check-892 ${todo.done ? "todo-check--done-892" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-892" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-892"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-892" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-892" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-892">
            <span class="todo-count-892">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-892" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
