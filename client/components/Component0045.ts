// client/components/Component0045.ts
// Benchmark component #45 — identical logic to TodoPage, scoped class names.
...
import "./Component0045.css";

type Filter = "all" | "active" | "done";

export const Component0045 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0045] mounted");
    return () => console.log("[Component0045] unmounted");
  });
  ...
  return () => html`
    <div class="app-45">
      <header class="header-45">
        <h1>todos — Component0045</h1>
      </header>

      <main class="main-45">
        <input
          class="add-input-45 ${error() ? "add-input--error-45" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-45" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-45">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-45">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-45 ${filter() === f ? "filter-btn--active-45" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-45">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-45">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-45">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-45 ${todo.done ? "todo-item--done-45" : ""}">
                    <button
                      class="todo-check-45 ${todo.done ? "todo-check--done-45" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-45" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-45"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-45" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-45" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-45">
            <span class="todo-count-45">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-45" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
