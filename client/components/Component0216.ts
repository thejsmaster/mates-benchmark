// client/components/Component0216.ts
// Benchmark component #216 — identical logic to TodoPage, scoped class names.
...
import "./Component0216.css";

type Filter = "all" | "active" | "done";

export const Component0216 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0216] mounted");
    return () => console.log("[Component0216] unmounted");
  });
  ...
  return () => html`
    <div class="app-216">
      <header class="header-216">
        <h1>todos — Component0216</h1>
      </header>

      <main class="main-216">
        <input
          class="add-input-216 ${error() ? "add-input--error-216" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-216" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-216">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-216">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-216 ${filter() === f ? "filter-btn--active-216" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-216">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-216">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-216">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-216 ${todo.done ? "todo-item--done-216" : ""}">
                    <button
                      class="todo-check-216 ${todo.done ? "todo-check--done-216" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-216" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-216"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-216" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-216" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-216">
            <span class="todo-count-216">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-216" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
