// client/components/Component0104.ts
// Benchmark component #104 — identical logic to TodoPage, scoped class names.
...
import "./Component0104.css";

type Filter = "all" | "active" | "done";

export const Component0104 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0104] mounted");
    return () => console.log("[Component0104] unmounted");
  });
  ...
  return () => html`
    <div class="app-104">
      <header class="header-104">
        <h1>todos — Component0104</h1>
      </header>

      <main class="main-104">
        <input
          class="add-input-104 ${error() ? "add-input--error-104" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-104" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-104">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-104">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-104 ${filter() === f ? "filter-btn--active-104" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-104">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-104">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-104">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-104 ${todo.done ? "todo-item--done-104" : ""}">
                    <button
                      class="todo-check-104 ${todo.done ? "todo-check--done-104" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-104" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-104"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-104" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-104" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-104">
            <span class="todo-count-104">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-104" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
