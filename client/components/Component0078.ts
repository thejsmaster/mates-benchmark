// client/components/Component0078.ts
// Benchmark component #78 — identical logic to TodoPage, scoped class names.
...
import "./Component0078.css";

type Filter = "all" | "active" | "done";

export const Component0078 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0078] mounted");
    return () => console.log("[Component0078] unmounted");
  });
  ...
  return () => html`
    <div class="app-78">
      <header class="header-78">
        <h1>todos — Component0078</h1>
      </header>

      <main class="main-78">
        <input
          class="add-input-78 ${error() ? "add-input--error-78" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-78" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-78">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-78">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-78 ${filter() === f ? "filter-btn--active-78" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-78">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-78">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-78">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-78 ${todo.done ? "todo-item--done-78" : ""}">
                    <button
                      class="todo-check-78 ${todo.done ? "todo-check--done-78" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-78" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-78"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-78" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-78" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-78">
            <span class="todo-count-78">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-78" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
