// client/components/Component0691.ts
// Benchmark component #691 — identical logic to TodoPage, scoped class names.
...
import "./Component0691.css";

type Filter = "all" | "active" | "done";

export const Component0691 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0691] mounted");
    return () => console.log("[Component0691] unmounted");
  });
  ...
  return () => html`
    <div class="app-691">
      <header class="header-691">
        <h1>todos — Component0691</h1>
      </header>

      <main class="main-691">
        <input
          class="add-input-691 ${error() ? "add-input--error-691" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-691" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-691">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-691">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-691 ${filter() === f ? "filter-btn--active-691" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-691">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-691">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-691">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-691 ${todo.done ? "todo-item--done-691" : ""}">
                    <button
                      class="todo-check-691 ${todo.done ? "todo-check--done-691" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-691" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-691"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-691" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-691" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-691">
            <span class="todo-count-691">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-691" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
