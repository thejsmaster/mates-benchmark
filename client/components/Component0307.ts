// client/components/Component0307.ts
// Benchmark component #307 — identical logic to TodoPage, scoped class names.
...
import "./Component0307.css";

type Filter = "all" | "active" | "done";

export const Component0307 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0307] mounted");
    return () => console.log("[Component0307] unmounted");
  });
  ...
  return () => html`
    <div class="app-307">
      <header class="header-307">
        <h1>todos — Component0307</h1>
      </header>

      <main class="main-307">
        <input
          class="add-input-307 ${error() ? "add-input--error-307" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-307" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-307">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-307">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-307 ${filter() === f ? "filter-btn--active-307" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-307">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-307">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-307">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-307 ${todo.done ? "todo-item--done-307" : ""}">
                    <button
                      class="todo-check-307 ${todo.done ? "todo-check--done-307" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-307" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-307"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-307" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-307" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-307">
            <span class="todo-count-307">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-307" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
