// client/components/Component0085.ts
// Benchmark component #85 — identical logic to TodoPage, scoped class names.
...
import "./Component0085.css";

type Filter = "all" | "active" | "done";

export const Component0085 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0085] mounted");
    return () => console.log("[Component0085] unmounted");
  });
  ...
  return () => html`
    <div class="app-85">
      <header class="header-85">
        <h1>todos — Component0085</h1>
      </header>

      <main class="main-85">
        <input
          class="add-input-85 ${error() ? "add-input--error-85" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-85" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-85">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-85">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-85 ${filter() === f ? "filter-btn--active-85" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-85">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-85">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-85">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-85 ${todo.done ? "todo-item--done-85" : ""}">
                    <button
                      class="todo-check-85 ${todo.done ? "todo-check--done-85" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-85" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-85"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-85" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-85" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-85">
            <span class="todo-count-85">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-85" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
