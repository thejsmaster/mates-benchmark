// client/components/Component0003.ts
// Benchmark component #3 — identical logic to TodoPage, scoped class names.
...
import "./Component0003.css";

type Filter = "all" | "active" | "done";

export const Component0003 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0003] mounted");
    return () => console.log("[Component0003] unmounted");
  });
  ...
  return () => html`
    <div class="app-3">
      <header class="header-3">
        <h1>todos — Component0003</h1>
      </header>

      <main class="main-3">
        <input
          class="add-input-3 ${error() ? "add-input--error-3" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-3" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-3">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-3">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-3 ${filter() === f ? "filter-btn--active-3" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-3">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-3">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-3">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-3 ${todo.done ? "todo-item--done-3" : ""}">
                    <button
                      class="todo-check-3 ${todo.done ? "todo-check--done-3" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-3" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-3"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-3" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-3" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-3">
            <span class="todo-count-3">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-3" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
