// client/components/Component0422.ts
// Benchmark component #422 — identical logic to TodoPage, scoped class names.
...
import "./Component0422.css";

type Filter = "all" | "active" | "done";

export const Component0422 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0422] mounted");
    return () => console.log("[Component0422] unmounted");
  });
  ...
  return () => html`
    <div class="app-422">
      <header class="header-422">
        <h1>todos — Component0422</h1>
      </header>

      <main class="main-422">
        <input
          class="add-input-422 ${error() ? "add-input--error-422" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-422" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-422">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-422">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-422 ${filter() === f ? "filter-btn--active-422" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-422">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-422">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-422">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-422 ${todo.done ? "todo-item--done-422" : ""}">
                    <button
                      class="todo-check-422 ${todo.done ? "todo-check--done-422" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-422" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-422"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-422" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-422" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-422">
            <span class="todo-count-422">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-422" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
