// client/components/Component0522.ts
// Benchmark component #522 — identical logic to TodoPage, scoped class names.
...
import "./Component0522.css";

type Filter = "all" | "active" | "done";

export const Component0522 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0522] mounted");
    return () => console.log("[Component0522] unmounted");
  });
  ...
  return () => html`
    <div class="app-522">
      <header class="header-522">
        <h1>todos — Component0522</h1>
      </header>

      <main class="main-522">
        <input
          class="add-input-522 ${error() ? "add-input--error-522" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-522" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-522">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-522">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-522 ${filter() === f ? "filter-btn--active-522" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-522">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-522">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-522">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-522 ${todo.done ? "todo-item--done-522" : ""}">
                    <button
                      class="todo-check-522 ${todo.done ? "todo-check--done-522" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-522" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-522"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-522" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-522" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-522">
            <span class="todo-count-522">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-522" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
