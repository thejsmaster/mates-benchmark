// client/components/Component0479.ts
// Benchmark component #479 — identical logic to TodoPage, scoped class names.
...
import "./Component0479.css";

type Filter = "all" | "active" | "done";

export const Component0479 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0479] mounted");
    return () => console.log("[Component0479] unmounted");
  });
  ...
  return () => html`
    <div class="app-479">
      <header class="header-479">
        <h1>todos — Component0479</h1>
      </header>

      <main class="main-479">
        <input
          class="add-input-479 ${error() ? "add-input--error-479" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-479" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-479">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-479">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-479 ${filter() === f ? "filter-btn--active-479" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-479">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-479">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-479">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-479 ${todo.done ? "todo-item--done-479" : ""}">
                    <button
                      class="todo-check-479 ${todo.done ? "todo-check--done-479" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-479" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-479"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-479" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-479" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-479">
            <span class="todo-count-479">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-479" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
