// client/components/Component0089.ts
// Benchmark component #89 — identical logic to TodoPage, scoped class names.
...
import "./Component0089.css";

type Filter = "all" | "active" | "done";

export const Component0089 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0089] mounted");
    return () => console.log("[Component0089] unmounted");
  });
  ...
  return () => html`
    <div class="app-89">
      <header class="header-89">
        <h1>todos — Component0089</h1>
      </header>

      <main class="main-89">
        <input
          class="add-input-89 ${error() ? "add-input--error-89" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-89" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-89">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-89">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-89 ${filter() === f ? "filter-btn--active-89" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-89">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-89">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-89">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-89 ${todo.done ? "todo-item--done-89" : ""}">
                    <button
                      class="todo-check-89 ${todo.done ? "todo-check--done-89" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-89" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-89"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-89" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-89" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-89">
            <span class="todo-count-89">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-89" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
