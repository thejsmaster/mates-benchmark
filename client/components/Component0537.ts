// client/components/Component0537.ts
// Benchmark component #537 — identical logic to TodoPage, scoped class names.
...
import "./Component0537.css";

type Filter = "all" | "active" | "done";

export const Component0537 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0537] mounted");
    return () => console.log("[Component0537] unmounted");
  });
  ...
  return () => html`
    <div class="app-537">
      <header class="header-537">
        <h1>todos — Component0537</h1>
      </header>

      <main class="main-537">
        <input
          class="add-input-537 ${error() ? "add-input--error-537" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-537" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-537">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-537">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-537 ${filter() === f ? "filter-btn--active-537" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-537">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-537">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-537">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-537 ${todo.done ? "todo-item--done-537" : ""}">
                    <button
                      class="todo-check-537 ${todo.done ? "todo-check--done-537" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-537" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-537"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-537" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-537" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-537">
            <span class="todo-count-537">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-537" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
