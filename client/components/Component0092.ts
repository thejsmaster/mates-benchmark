// client/components/Component0092.ts
// Benchmark component #92 — identical logic to TodoPage, scoped class names.
...
import "./Component0092.css";

type Filter = "all" | "active" | "done";

export const Component0092 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0092] mounted");
    return () => console.log("[Component0092] unmounted");
  });
  ...
  return () => html`
    <div class="app-92">
      <header class="header-92">
        <h1>todos — Component0092</h1>
      </header>

      <main class="main-92">
        <input
          class="add-input-92 ${error() ? "add-input--error-92" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-92" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-92">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-92">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-92 ${filter() === f ? "filter-btn--active-92" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-92">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-92">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-92">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-92 ${todo.done ? "todo-item--done-92" : ""}">
                    <button
                      class="todo-check-92 ${todo.done ? "todo-check--done-92" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-92" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-92"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-92" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-92" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-92">
            <span class="todo-count-92">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-92" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
