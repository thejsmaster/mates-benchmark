// client/components/Component0475.ts
// Benchmark component #475 — identical logic to TodoPage, scoped class names.
...
import "./Component0475.css";

type Filter = "all" | "active" | "done";

export const Component0475 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0475] mounted");
    return () => console.log("[Component0475] unmounted");
  });
  ...
  return () => html`
    <div class="app-475">
      <header class="header-475">
        <h1>todos — Component0475</h1>
      </header>

      <main class="main-475">
        <input
          class="add-input-475 ${error() ? "add-input--error-475" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-475" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-475">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-475">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-475 ${filter() === f ? "filter-btn--active-475" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-475">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-475">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-475">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-475 ${todo.done ? "todo-item--done-475" : ""}">
                    <button
                      class="todo-check-475 ${todo.done ? "todo-check--done-475" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-475" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-475"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-475" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-475" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-475">
            <span class="todo-count-475">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-475" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
