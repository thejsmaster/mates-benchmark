// client/components/Component0062.ts
// Benchmark component #62 — identical logic to TodoPage, scoped class names.
...
import "./Component0062.css";

type Filter = "all" | "active" | "done";

export const Component0062 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0062] mounted");
    return () => console.log("[Component0062] unmounted");
  });
  ...
  return () => html`
    <div class="app-62">
      <header class="header-62">
        <h1>todos — Component0062</h1>
      </header>

      <main class="main-62">
        <input
          class="add-input-62 ${error() ? "add-input--error-62" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-62" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-62">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-62">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-62 ${filter() === f ? "filter-btn--active-62" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-62">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-62">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-62">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-62 ${todo.done ? "todo-item--done-62" : ""}">
                    <button
                      class="todo-check-62 ${todo.done ? "todo-check--done-62" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-62" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-62"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-62" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-62" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-62">
            <span class="todo-count-62">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-62" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
