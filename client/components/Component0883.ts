// client/components/Component0883.ts
// Benchmark component #883 — identical logic to TodoPage, scoped class names.
...
import "./Component0883.css";

type Filter = "all" | "active" | "done";

export const Component0883 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0883] mounted");
    return () => console.log("[Component0883] unmounted");
  });
  ...
  return () => html`
    <div class="app-883">
      <header class="header-883">
        <h1>todos — Component0883</h1>
      </header>

      <main class="main-883">
        <input
          class="add-input-883 ${error() ? "add-input--error-883" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-883" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-883">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-883">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-883 ${filter() === f ? "filter-btn--active-883" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-883">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-883">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-883">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-883 ${todo.done ? "todo-item--done-883" : ""}">
                    <button
                      class="todo-check-883 ${todo.done ? "todo-check--done-883" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-883" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-883"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-883" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-883" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-883">
            <span class="todo-count-883">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-883" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
