// client/components/Component0673.ts
// Benchmark component #673 — identical logic to TodoPage, scoped class names.
...
import "./Component0673.css";

type Filter = "all" | "active" | "done";

export const Component0673 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0673] mounted");
    return () => console.log("[Component0673] unmounted");
  });
  ...
  return () => html`
    <div class="app-673">
      <header class="header-673">
        <h1>todos — Component0673</h1>
      </header>

      <main class="main-673">
        <input
          class="add-input-673 ${error() ? "add-input--error-673" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-673" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-673">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-673">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-673 ${filter() === f ? "filter-btn--active-673" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-673">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-673">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-673">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-673 ${todo.done ? "todo-item--done-673" : ""}">
                    <button
                      class="todo-check-673 ${todo.done ? "todo-check--done-673" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-673" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-673"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-673" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-673" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-673">
            <span class="todo-count-673">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-673" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
