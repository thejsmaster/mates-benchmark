// client/components/Component0748.ts
// Benchmark component #748 — identical logic to TodoPage, scoped class names.
...
import "./Component0748.css";

type Filter = "all" | "active" | "done";

export const Component0748 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0748] mounted");
    return () => console.log("[Component0748] unmounted");
  });
  ...
  return () => html`
    <div class="app-748">
      <header class="header-748">
        <h1>todos — Component0748</h1>
      </header>

      <main class="main-748">
        <input
          class="add-input-748 ${error() ? "add-input--error-748" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-748" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-748">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-748">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-748 ${filter() === f ? "filter-btn--active-748" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-748">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-748">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-748">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-748 ${todo.done ? "todo-item--done-748" : ""}">
                    <button
                      class="todo-check-748 ${todo.done ? "todo-check--done-748" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-748" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-748"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-748" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-748" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-748">
            <span class="todo-count-748">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-748" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
