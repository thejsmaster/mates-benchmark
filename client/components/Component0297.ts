// client/components/Component0297.ts
// Benchmark component #297 — identical logic to TodoPage, scoped class names.
...
import "./Component0297.css";

type Filter = "all" | "active" | "done";

export const Component0297 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0297] mounted");
    return () => console.log("[Component0297] unmounted");
  });
  ...
  return () => html`
    <div class="app-297">
      <header class="header-297">
        <h1>todos — Component0297</h1>
      </header>

      <main class="main-297">
        <input
          class="add-input-297 ${error() ? "add-input--error-297" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-297" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-297">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-297">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-297 ${filter() === f ? "filter-btn--active-297" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-297">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-297">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-297">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-297 ${todo.done ? "todo-item--done-297" : ""}">
                    <button
                      class="todo-check-297 ${todo.done ? "todo-check--done-297" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-297" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-297"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-297" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-297" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-297">
            <span class="todo-count-297">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-297" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
