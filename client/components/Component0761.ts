// client/components/Component0761.ts
// Benchmark component #761 — identical logic to TodoPage, scoped class names.
...
import "./Component0761.css";

type Filter = "all" | "active" | "done";

export const Component0761 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0761] mounted");
    return () => console.log("[Component0761] unmounted");
  });
  ...
  return () => html`
    <div class="app-761">
      <header class="header-761">
        <h1>todos — Component0761</h1>
      </header>

      <main class="main-761">
        <input
          class="add-input-761 ${error() ? "add-input--error-761" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-761" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-761">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-761">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-761 ${filter() === f ? "filter-btn--active-761" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-761">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-761">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-761">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-761 ${todo.done ? "todo-item--done-761" : ""}">
                    <button
                      class="todo-check-761 ${todo.done ? "todo-check--done-761" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-761" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-761"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-761" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-761" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-761">
            <span class="todo-count-761">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-761" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
