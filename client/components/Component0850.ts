// client/components/Component0850.ts
// Benchmark component #850 — identical logic to TodoPage, scoped class names.
...
import "./Component0850.css";

type Filter = "all" | "active" | "done";

export const Component0850 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0850] mounted");
    return () => console.log("[Component0850] unmounted");
  });
  ...
  return () => html`
    <div class="app-850">
      <header class="header-850">
        <h1>todos — Component0850</h1>
      </header>

      <main class="main-850">
        <input
          class="add-input-850 ${error() ? "add-input--error-850" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-850" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-850">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-850">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-850 ${filter() === f ? "filter-btn--active-850" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-850">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-850">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-850">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-850 ${todo.done ? "todo-item--done-850" : ""}">
                    <button
                      class="todo-check-850 ${todo.done ? "todo-check--done-850" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-850" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-850"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-850" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-850" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-850">
            <span class="todo-count-850">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-850" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
