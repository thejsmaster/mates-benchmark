// client/components/Component0545.ts
// Benchmark component #545 — identical logic to TodoPage, scoped class names.
...
import "./Component0545.css";

type Filter = "all" | "active" | "done";

export const Component0545 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0545] mounted");
    return () => console.log("[Component0545] unmounted");
  });
  ...
  return () => html`
    <div class="app-545">
      <header class="header-545">
        <h1>todos — Component0545</h1>
      </header>

      <main class="main-545">
        <input
          class="add-input-545 ${error() ? "add-input--error-545" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-545" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-545">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-545">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-545 ${filter() === f ? "filter-btn--active-545" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-545">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-545">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-545">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-545 ${todo.done ? "todo-item--done-545" : ""}">
                    <button
                      class="todo-check-545 ${todo.done ? "todo-check--done-545" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-545" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-545"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-545" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-545" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-545">
            <span class="todo-count-545">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-545" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
