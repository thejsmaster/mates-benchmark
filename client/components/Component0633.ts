// client/components/Component0633.ts
// Benchmark component #633 — identical logic to TodoPage, scoped class names.
...
import "./Component0633.css";

type Filter = "all" | "active" | "done";

export const Component0633 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0633] mounted");
    return () => console.log("[Component0633] unmounted");
  });
  ...
  return () => html`
    <div class="app-633">
      <header class="header-633">
        <h1>todos — Component0633</h1>
      </header>

      <main class="main-633">
        <input
          class="add-input-633 ${error() ? "add-input--error-633" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-633" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-633">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-633">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-633 ${filter() === f ? "filter-btn--active-633" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-633">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-633">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-633">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-633 ${todo.done ? "todo-item--done-633" : ""}">
                    <button
                      class="todo-check-633 ${todo.done ? "todo-check--done-633" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-633" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-633"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-633" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-633" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-633">
            <span class="todo-count-633">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-633" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
