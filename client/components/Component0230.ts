// client/components/Component0230.ts
// Benchmark component #230 — identical logic to TodoPage, scoped class names.
...
import "./Component0230.css";

type Filter = "all" | "active" | "done";

export const Component0230 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0230] mounted");
    return () => console.log("[Component0230] unmounted");
  });
  ...
  return () => html`
    <div class="app-230">
      <header class="header-230">
        <h1>todos — Component0230</h1>
      </header>

      <main class="main-230">
        <input
          class="add-input-230 ${error() ? "add-input--error-230" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-230" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-230">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-230">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-230 ${filter() === f ? "filter-btn--active-230" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-230">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-230">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-230">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-230 ${todo.done ? "todo-item--done-230" : ""}">
                    <button
                      class="todo-check-230 ${todo.done ? "todo-check--done-230" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-230" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-230"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-230" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-230" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-230">
            <span class="todo-count-230">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-230" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
