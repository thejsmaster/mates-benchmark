// client/components/Component0172.ts
// Benchmark component #172 — identical logic to TodoPage, scoped class names.
...
import "./Component0172.css";

type Filter = "all" | "active" | "done";

export const Component0172 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0172] mounted");
    return () => console.log("[Component0172] unmounted");
  });
  ...
  return () => html`
    <div class="app-172">
      <header class="header-172">
        <h1>todos — Component0172</h1>
      </header>

      <main class="main-172">
        <input
          class="add-input-172 ${error() ? "add-input--error-172" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-172" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-172">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-172">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-172 ${filter() === f ? "filter-btn--active-172" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-172">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-172">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-172">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-172 ${todo.done ? "todo-item--done-172" : ""}">
                    <button
                      class="todo-check-172 ${todo.done ? "todo-check--done-172" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-172" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-172"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-172" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-172" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-172">
            <span class="todo-count-172">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-172" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
