// client/components/Component0244.ts
// Benchmark component #244 — identical logic to TodoPage, scoped class names.
...
import "./Component0244.css";

type Filter = "all" | "active" | "done";

export const Component0244 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0244] mounted");
    return () => console.log("[Component0244] unmounted");
  });
  ...
  return () => html`
    <div class="app-244">
      <header class="header-244">
        <h1>todos — Component0244</h1>
      </header>

      <main class="main-244">
        <input
          class="add-input-244 ${error() ? "add-input--error-244" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-244" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-244">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-244">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-244 ${filter() === f ? "filter-btn--active-244" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-244">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-244">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-244">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-244 ${todo.done ? "todo-item--done-244" : ""}">
                    <button
                      class="todo-check-244 ${todo.done ? "todo-check--done-244" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-244" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-244"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-244" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-244" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-244">
            <span class="todo-count-244">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-244" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
