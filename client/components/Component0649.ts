// client/components/Component0649.ts
// Benchmark component #649 — identical logic to TodoPage, scoped class names.
...
import "./Component0649.css";

type Filter = "all" | "active" | "done";

export const Component0649 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0649] mounted");
    return () => console.log("[Component0649] unmounted");
  });
  ...
  return () => html`
    <div class="app-649">
      <header class="header-649">
        <h1>todos — Component0649</h1>
      </header>

      <main class="main-649">
        <input
          class="add-input-649 ${error() ? "add-input--error-649" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-649" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-649">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-649">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-649 ${filter() === f ? "filter-btn--active-649" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-649">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-649">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-649">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-649 ${todo.done ? "todo-item--done-649" : ""}">
                    <button
                      class="todo-check-649 ${todo.done ? "todo-check--done-649" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-649" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-649"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-649" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-649" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-649">
            <span class="todo-count-649">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-649" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
