// client/components/Component0424.ts
// Benchmark component #424 — identical logic to TodoPage, scoped class names.
...
import "./Component0424.css";

type Filter = "all" | "active" | "done";

export const Component0424 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0424] mounted");
    return () => console.log("[Component0424] unmounted");
  });
  ...
  return () => html`
    <div class="app-424">
      <header class="header-424">
        <h1>todos — Component0424</h1>
      </header>

      <main class="main-424">
        <input
          class="add-input-424 ${error() ? "add-input--error-424" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-424" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-424">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-424">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-424 ${filter() === f ? "filter-btn--active-424" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-424">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-424">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-424">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-424 ${todo.done ? "todo-item--done-424" : ""}">
                    <button
                      class="todo-check-424 ${todo.done ? "todo-check--done-424" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-424" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-424"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-424" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-424" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-424">
            <span class="todo-count-424">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-424" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
