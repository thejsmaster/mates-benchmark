// client/components/Component0344.ts
// Benchmark component #344 — identical logic to TodoPage, scoped class names.
...
import "./Component0344.css";

type Filter = "all" | "active" | "done";

export const Component0344 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0344] mounted");
    return () => console.log("[Component0344] unmounted");
  });
  ...
  return () => html`
    <div class="app-344">
      <header class="header-344">
        <h1>todos — Component0344</h1>
      </header>

      <main class="main-344">
        <input
          class="add-input-344 ${error() ? "add-input--error-344" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-344" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-344">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-344">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-344 ${filter() === f ? "filter-btn--active-344" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-344">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-344">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-344">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-344 ${todo.done ? "todo-item--done-344" : ""}">
                    <button
                      class="todo-check-344 ${todo.done ? "todo-check--done-344" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-344" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-344"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-344" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-344" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-344">
            <span class="todo-count-344">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-344" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
