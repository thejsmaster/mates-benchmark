// client/components/Component0509.ts
// Benchmark component #509 — identical logic to TodoPage, scoped class names.
...
import "./Component0509.css";

type Filter = "all" | "active" | "done";

export const Component0509 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0509] mounted");
    return () => console.log("[Component0509] unmounted");
  });
  ...
  return () => html`
    <div class="app-509">
      <header class="header-509">
        <h1>todos — Component0509</h1>
      </header>

      <main class="main-509">
        <input
          class="add-input-509 ${error() ? "add-input--error-509" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-509" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-509">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-509">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-509 ${filter() === f ? "filter-btn--active-509" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-509">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-509">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-509">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-509 ${todo.done ? "todo-item--done-509" : ""}">
                    <button
                      class="todo-check-509 ${todo.done ? "todo-check--done-509" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-509" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-509"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-509" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-509" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-509">
            <span class="todo-count-509">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-509" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
