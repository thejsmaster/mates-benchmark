// client/components/Component0425.ts
// Benchmark component #425 — identical logic to TodoPage, scoped class names.
...
import "./Component0425.css";

type Filter = "all" | "active" | "done";

export const Component0425 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0425] mounted");
    return () => console.log("[Component0425] unmounted");
  });
  ...
  return () => html`
    <div class="app-425">
      <header class="header-425">
        <h1>todos — Component0425</h1>
      </header>

      <main class="main-425">
        <input
          class="add-input-425 ${error() ? "add-input--error-425" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-425" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-425">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-425">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-425 ${filter() === f ? "filter-btn--active-425" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-425">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-425">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-425">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-425 ${todo.done ? "todo-item--done-425" : ""}">
                    <button
                      class="todo-check-425 ${todo.done ? "todo-check--done-425" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-425" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-425"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-425" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-425" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-425">
            <span class="todo-count-425">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-425" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
