// client/components/Component0712.ts
// Benchmark component #712 — identical logic to TodoPage, scoped class names.
...
import "./Component0712.css";

type Filter = "all" | "active" | "done";

export const Component0712 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0712] mounted");
    return () => console.log("[Component0712] unmounted");
  });
  ...
  return () => html`
    <div class="app-712">
      <header class="header-712">
        <h1>todos — Component0712</h1>
      </header>

      <main class="main-712">
        <input
          class="add-input-712 ${error() ? "add-input--error-712" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-712" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-712">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-712">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-712 ${filter() === f ? "filter-btn--active-712" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-712">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-712">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-712">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-712 ${todo.done ? "todo-item--done-712" : ""}">
                    <button
                      class="todo-check-712 ${todo.done ? "todo-check--done-712" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-712" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-712"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-712" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-712" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-712">
            <span class="todo-count-712">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-712" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
