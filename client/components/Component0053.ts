// client/components/Component0053.ts
// Benchmark component #53 — identical logic to TodoPage, scoped class names.
...
import "./Component0053.css";

type Filter = "all" | "active" | "done";

export const Component0053 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0053] mounted");
    return () => console.log("[Component0053] unmounted");
  });
  ...
  return () => html`
    <div class="app-53">
      <header class="header-53">
        <h1>todos — Component0053</h1>
      </header>

      <main class="main-53">
        <input
          class="add-input-53 ${error() ? "add-input--error-53" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-53" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-53">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-53">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-53 ${filter() === f ? "filter-btn--active-53" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-53">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-53">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-53">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-53 ${todo.done ? "todo-item--done-53" : ""}">
                    <button
                      class="todo-check-53 ${todo.done ? "todo-check--done-53" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-53" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-53"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-53" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-53" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-53">
            <span class="todo-count-53">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-53" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
