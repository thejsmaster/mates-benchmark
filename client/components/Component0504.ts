// client/components/Component0504.ts
// Benchmark component #504 — identical logic to TodoPage, scoped class names.
...
import "./Component0504.css";

type Filter = "all" | "active" | "done";

export const Component0504 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0504] mounted");
    return () => console.log("[Component0504] unmounted");
  });
  ...
  return () => html`
    <div class="app-504">
      <header class="header-504">
        <h1>todos — Component0504</h1>
      </header>

      <main class="main-504">
        <input
          class="add-input-504 ${error() ? "add-input--error-504" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-504" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-504">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-504">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-504 ${filter() === f ? "filter-btn--active-504" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-504">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-504">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-504">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-504 ${todo.done ? "todo-item--done-504" : ""}">
                    <button
                      class="todo-check-504 ${todo.done ? "todo-check--done-504" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-504" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-504"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-504" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-504" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-504">
            <span class="todo-count-504">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-504" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
