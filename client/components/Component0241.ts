// client/components/Component0241.ts
// Benchmark component #241 — identical logic to TodoPage, scoped class names.
...
import "./Component0241.css";

type Filter = "all" | "active" | "done";

export const Component0241 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0241] mounted");
    return () => console.log("[Component0241] unmounted");
  });
  ...
  return () => html`
    <div class="app-241">
      <header class="header-241">
        <h1>todos — Component0241</h1>
      </header>

      <main class="main-241">
        <input
          class="add-input-241 ${error() ? "add-input--error-241" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-241" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-241">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-241">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-241 ${filter() === f ? "filter-btn--active-241" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-241">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-241">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-241">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-241 ${todo.done ? "todo-item--done-241" : ""}">
                    <button
                      class="todo-check-241 ${todo.done ? "todo-check--done-241" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-241" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-241"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-241" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-241" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-241">
            <span class="todo-count-241">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-241" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
