// client/components/Component0446.ts
// Benchmark component #446 — identical logic to TodoPage, scoped class names.
...
import "./Component0446.css";

type Filter = "all" | "active" | "done";

export const Component0446 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0446] mounted");
    return () => console.log("[Component0446] unmounted");
  });
  ...
  return () => html`
    <div class="app-446">
      <header class="header-446">
        <h1>todos — Component0446</h1>
      </header>

      <main class="main-446">
        <input
          class="add-input-446 ${error() ? "add-input--error-446" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-446" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-446">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-446">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-446 ${filter() === f ? "filter-btn--active-446" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-446">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-446">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-446">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-446 ${todo.done ? "todo-item--done-446" : ""}">
                    <button
                      class="todo-check-446 ${todo.done ? "todo-check--done-446" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-446" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-446"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-446" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-446" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-446">
            <span class="todo-count-446">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-446" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
