// client/components/Component0929.ts
// Benchmark component #929 — identical logic to TodoPage, scoped class names.
...
import "./Component0929.css";

type Filter = "all" | "active" | "done";

export const Component0929 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0929] mounted");
    return () => console.log("[Component0929] unmounted");
  });
  ...
  return () => html`
    <div class="app-929">
      <header class="header-929">
        <h1>todos — Component0929</h1>
      </header>

      <main class="main-929">
        <input
          class="add-input-929 ${error() ? "add-input--error-929" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-929" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-929">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-929">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-929 ${filter() === f ? "filter-btn--active-929" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-929">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-929">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-929">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-929 ${todo.done ? "todo-item--done-929" : ""}">
                    <button
                      class="todo-check-929 ${todo.done ? "todo-check--done-929" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-929" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-929"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-929" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-929" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-929">
            <span class="todo-count-929">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-929" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
