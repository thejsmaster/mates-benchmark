// client/components/Component0095.ts
// Benchmark component #95 — identical logic to TodoPage, scoped class names.
...
import "./Component0095.css";

type Filter = "all" | "active" | "done";

export const Component0095 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0095] mounted");
    return () => console.log("[Component0095] unmounted");
  });
  ...
  return () => html`
    <div class="app-95">
      <header class="header-95">
        <h1>todos — Component0095</h1>
      </header>

      <main class="main-95">
        <input
          class="add-input-95 ${error() ? "add-input--error-95" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-95" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-95">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-95">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-95 ${filter() === f ? "filter-btn--active-95" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-95">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-95">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-95">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-95 ${todo.done ? "todo-item--done-95" : ""}">
                    <button
                      class="todo-check-95 ${todo.done ? "todo-check--done-95" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-95" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-95"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-95" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-95" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-95">
            <span class="todo-count-95">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-95" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
