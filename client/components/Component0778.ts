// client/components/Component0778.ts
// Benchmark component #778 — identical logic to TodoPage, scoped class names.
...
import "./Component0778.css";

type Filter = "all" | "active" | "done";

export const Component0778 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0778] mounted");
    return () => console.log("[Component0778] unmounted");
  });
  ...
  return () => html`
    <div class="app-778">
      <header class="header-778">
        <h1>todos — Component0778</h1>
      </header>

      <main class="main-778">
        <input
          class="add-input-778 ${error() ? "add-input--error-778" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-778" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-778">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-778">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-778 ${filter() === f ? "filter-btn--active-778" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-778">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-778">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-778">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-778 ${todo.done ? "todo-item--done-778" : ""}">
                    <button
                      class="todo-check-778 ${todo.done ? "todo-check--done-778" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-778" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-778"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-778" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-778" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-778">
            <span class="todo-count-778">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-778" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
