// client/components/Component0235.ts
// Benchmark component #235 — identical logic to TodoPage, scoped class names.
...
import "./Component0235.css";

type Filter = "all" | "active" | "done";

export const Component0235 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0235] mounted");
    return () => console.log("[Component0235] unmounted");
  });
  ...
  return () => html`
    <div class="app-235">
      <header class="header-235">
        <h1>todos — Component0235</h1>
      </header>

      <main class="main-235">
        <input
          class="add-input-235 ${error() ? "add-input--error-235" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-235" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-235">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-235">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-235 ${filter() === f ? "filter-btn--active-235" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-235">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-235">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-235">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-235 ${todo.done ? "todo-item--done-235" : ""}">
                    <button
                      class="todo-check-235 ${todo.done ? "todo-check--done-235" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-235" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-235"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-235" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-235" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-235">
            <span class="todo-count-235">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-235" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
