// client/components/Component0723.ts
// Benchmark component #723 — identical logic to TodoPage, scoped class names.
...
import "./Component0723.css";

type Filter = "all" | "active" | "done";

export const Component0723 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0723] mounted");
    return () => console.log("[Component0723] unmounted");
  });
  ...
  return () => html`
    <div class="app-723">
      <header class="header-723">
        <h1>todos — Component0723</h1>
      </header>

      <main class="main-723">
        <input
          class="add-input-723 ${error() ? "add-input--error-723" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-723" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-723">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-723">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-723 ${filter() === f ? "filter-btn--active-723" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-723">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-723">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-723">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-723 ${todo.done ? "todo-item--done-723" : ""}">
                    <button
                      class="todo-check-723 ${todo.done ? "todo-check--done-723" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-723" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-723"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-723" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-723" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-723">
            <span class="todo-count-723">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-723" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
