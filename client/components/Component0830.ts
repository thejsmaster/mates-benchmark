// client/components/Component0830.ts
// Benchmark component #830 — identical logic to TodoPage, scoped class names.
...
import "./Component0830.css";

type Filter = "all" | "active" | "done";

export const Component0830 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0830] mounted");
    return () => console.log("[Component0830] unmounted");
  });
  ...
  return () => html`
    <div class="app-830">
      <header class="header-830">
        <h1>todos — Component0830</h1>
      </header>

      <main class="main-830">
        <input
          class="add-input-830 ${error() ? "add-input--error-830" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-830" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-830">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-830">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-830 ${filter() === f ? "filter-btn--active-830" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-830">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-830">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-830">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-830 ${todo.done ? "todo-item--done-830" : ""}">
                    <button
                      class="todo-check-830 ${todo.done ? "todo-check--done-830" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-830" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-830"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-830" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-830" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-830">
            <span class="todo-count-830">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-830" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
