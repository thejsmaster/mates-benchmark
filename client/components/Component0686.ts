// client/components/Component0686.ts
// Benchmark component #686 — identical logic to TodoPage, scoped class names.
...
import "./Component0686.css";

type Filter = "all" | "active" | "done";

export const Component0686 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0686] mounted");
    return () => console.log("[Component0686] unmounted");
  });
  ...
  return () => html`
    <div class="app-686">
      <header class="header-686">
        <h1>todos — Component0686</h1>
      </header>

      <main class="main-686">
        <input
          class="add-input-686 ${error() ? "add-input--error-686" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-686" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-686">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-686">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-686 ${filter() === f ? "filter-btn--active-686" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-686">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-686">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-686">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-686 ${todo.done ? "todo-item--done-686" : ""}">
                    <button
                      class="todo-check-686 ${todo.done ? "todo-check--done-686" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-686" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-686"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-686" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-686" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-686">
            <span class="todo-count-686">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-686" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
