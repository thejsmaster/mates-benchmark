// client/components/Component0954.ts
// Benchmark component #954 — identical logic to TodoPage, scoped class names.
...
import "./Component0954.css";

type Filter = "all" | "active" | "done";

export const Component0954 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0954] mounted");
    return () => console.log("[Component0954] unmounted");
  });
  ...
  return () => html`
    <div class="app-954">
      <header class="header-954">
        <h1>todos — Component0954</h1>
      </header>

      <main class="main-954">
        <input
          class="add-input-954 ${error() ? "add-input--error-954" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-954" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-954">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-954">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-954 ${filter() === f ? "filter-btn--active-954" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-954">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-954">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-954">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-954 ${todo.done ? "todo-item--done-954" : ""}">
                    <button
                      class="todo-check-954 ${todo.done ? "todo-check--done-954" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-954" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-954"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-954" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-954" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-954">
            <span class="todo-count-954">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-954" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
