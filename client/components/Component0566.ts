// client/components/Component0566.ts
// Benchmark component #566 — identical logic to TodoPage, scoped class names.
...
import "./Component0566.css";

type Filter = "all" | "active" | "done";

export const Component0566 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0566] mounted");
    return () => console.log("[Component0566] unmounted");
  });
  ...
  return () => html`
    <div class="app-566">
      <header class="header-566">
        <h1>todos — Component0566</h1>
      </header>

      <main class="main-566">
        <input
          class="add-input-566 ${error() ? "add-input--error-566" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-566" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-566">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-566">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-566 ${filter() === f ? "filter-btn--active-566" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-566">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-566">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-566">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-566 ${todo.done ? "todo-item--done-566" : ""}">
                    <button
                      class="todo-check-566 ${todo.done ? "todo-check--done-566" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-566" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-566"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-566" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-566" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-566">
            <span class="todo-count-566">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-566" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
