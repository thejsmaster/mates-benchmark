// client/components/Component0810.ts
// Benchmark component #810 — identical logic to TodoPage, scoped class names.
...
import "./Component0810.css";

type Filter = "all" | "active" | "done";

export const Component0810 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0810] mounted");
    return () => console.log("[Component0810] unmounted");
  });
  ...
  return () => html`
    <div class="app-810">
      <header class="header-810">
        <h1>todos — Component0810</h1>
      </header>

      <main class="main-810">
        <input
          class="add-input-810 ${error() ? "add-input--error-810" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-810" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-810">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-810">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-810 ${filter() === f ? "filter-btn--active-810" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-810">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-810">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-810">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-810 ${todo.done ? "todo-item--done-810" : ""}">
                    <button
                      class="todo-check-810 ${todo.done ? "todo-check--done-810" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-810" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-810"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-810" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-810" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-810">
            <span class="todo-count-810">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-810" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
