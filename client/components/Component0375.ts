// client/components/Component0375.ts
// Benchmark component #375 — identical logic to TodoPage, scoped class names.
...
import "./Component0375.css";

type Filter = "all" | "active" | "done";

export const Component0375 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0375] mounted");
    return () => console.log("[Component0375] unmounted");
  });
  ...
  return () => html`
    <div class="app-375">
      <header class="header-375">
        <h1>todos — Component0375</h1>
      </header>

      <main class="main-375">
        <input
          class="add-input-375 ${error() ? "add-input--error-375" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-375" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-375">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-375">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-375 ${filter() === f ? "filter-btn--active-375" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-375">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-375">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-375">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-375 ${todo.done ? "todo-item--done-375" : ""}">
                    <button
                      class="todo-check-375 ${todo.done ? "todo-check--done-375" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-375" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-375"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-375" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-375" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-375">
            <span class="todo-count-375">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-375" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
