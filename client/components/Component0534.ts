// client/components/Component0534.ts
// Benchmark component #534 — identical logic to TodoPage, scoped class names.
...
import "./Component0534.css";

type Filter = "all" | "active" | "done";

export const Component0534 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0534] mounted");
    return () => console.log("[Component0534] unmounted");
  });
  ...
  return () => html`
    <div class="app-534">
      <header class="header-534">
        <h1>todos — Component0534</h1>
      </header>

      <main class="main-534">
        <input
          class="add-input-534 ${error() ? "add-input--error-534" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-534" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-534">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-534">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-534 ${filter() === f ? "filter-btn--active-534" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-534">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-534">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-534">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-534 ${todo.done ? "todo-item--done-534" : ""}">
                    <button
                      class="todo-check-534 ${todo.done ? "todo-check--done-534" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-534" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-534"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-534" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-534" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-534">
            <span class="todo-count-534">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-534" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
