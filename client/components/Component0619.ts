// client/components/Component0619.ts
// Benchmark component #619 — identical logic to TodoPage, scoped class names.
...
import "./Component0619.css";

type Filter = "all" | "active" | "done";

export const Component0619 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0619] mounted");
    return () => console.log("[Component0619] unmounted");
  });
  ...
  return () => html`
    <div class="app-619">
      <header class="header-619">
        <h1>todos — Component0619</h1>
      </header>

      <main class="main-619">
        <input
          class="add-input-619 ${error() ? "add-input--error-619" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-619" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-619">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-619">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-619 ${filter() === f ? "filter-btn--active-619" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-619">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-619">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-619">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-619 ${todo.done ? "todo-item--done-619" : ""}">
                    <button
                      class="todo-check-619 ${todo.done ? "todo-check--done-619" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-619" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-619"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-619" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-619" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-619">
            <span class="todo-count-619">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-619" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
