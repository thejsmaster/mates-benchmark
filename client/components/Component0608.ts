// client/components/Component0608.ts
// Benchmark component #608 — identical logic to TodoPage, scoped class names.
...
import "./Component0608.css";

type Filter = "all" | "active" | "done";

export const Component0608 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0608] mounted");
    return () => console.log("[Component0608] unmounted");
  });
  ...
  return () => html`
    <div class="app-608">
      <header class="header-608">
        <h1>todos — Component0608</h1>
      </header>

      <main class="main-608">
        <input
          class="add-input-608 ${error() ? "add-input--error-608" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-608" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-608">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-608">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-608 ${filter() === f ? "filter-btn--active-608" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-608">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-608">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-608">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-608 ${todo.done ? "todo-item--done-608" : ""}">
                    <button
                      class="todo-check-608 ${todo.done ? "todo-check--done-608" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-608" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-608"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-608" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-608" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-608">
            <span class="todo-count-608">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-608" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
