// client/components/Component0139.ts
// Benchmark component #139 — identical logic to TodoPage, scoped class names.
...
import "./Component0139.css";

type Filter = "all" | "active" | "done";

export const Component0139 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0139] mounted");
    return () => console.log("[Component0139] unmounted");
  });
  ...
  return () => html`
    <div class="app-139">
      <header class="header-139">
        <h1>todos — Component0139</h1>
      </header>

      <main class="main-139">
        <input
          class="add-input-139 ${error() ? "add-input--error-139" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-139" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-139">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-139">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-139 ${filter() === f ? "filter-btn--active-139" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-139">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-139">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-139">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-139 ${todo.done ? "todo-item--done-139" : ""}">
                    <button
                      class="todo-check-139 ${todo.done ? "todo-check--done-139" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-139" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-139"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-139" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-139" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-139">
            <span class="todo-count-139">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-139" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
