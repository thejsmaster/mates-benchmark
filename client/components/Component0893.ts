// client/components/Component0893.ts
// Benchmark component #893 — identical logic to TodoPage, scoped class names.
...
import "./Component0893.css";

type Filter = "all" | "active" | "done";

export const Component0893 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0893] mounted");
    return () => console.log("[Component0893] unmounted");
  });
  ...
  return () => html`
    <div class="app-893">
      <header class="header-893">
        <h1>todos — Component0893</h1>
      </header>

      <main class="main-893">
        <input
          class="add-input-893 ${error() ? "add-input--error-893" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-893" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-893">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-893">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-893 ${filter() === f ? "filter-btn--active-893" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-893">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-893">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-893">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-893 ${todo.done ? "todo-item--done-893" : ""}">
                    <button
                      class="todo-check-893 ${todo.done ? "todo-check--done-893" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-893" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-893"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-893" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-893" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-893">
            <span class="todo-count-893">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-893" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
