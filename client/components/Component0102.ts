// client/components/Component0102.ts
// Benchmark component #102 — identical logic to TodoPage, scoped class names.
...
import "./Component0102.css";

type Filter = "all" | "active" | "done";

export const Component0102 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0102] mounted");
    return () => console.log("[Component0102] unmounted");
  });
  ...
  return () => html`
    <div class="app-102">
      <header class="header-102">
        <h1>todos — Component0102</h1>
      </header>

      <main class="main-102">
        <input
          class="add-input-102 ${error() ? "add-input--error-102" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-102" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-102">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-102">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-102 ${filter() === f ? "filter-btn--active-102" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-102">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-102">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-102">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-102 ${todo.done ? "todo-item--done-102" : ""}">
                    <button
                      class="todo-check-102 ${todo.done ? "todo-check--done-102" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-102" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-102"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-102" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-102" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-102">
            <span class="todo-count-102">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-102" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
