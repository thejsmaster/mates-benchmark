// client/components/Component0992.ts
// Benchmark component #992 — identical logic to TodoPage, scoped class names.
...
import "./Component0992.css";

type Filter = "all" | "active" | "done";

export const Component0992 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0992] mounted");
    return () => console.log("[Component0992] unmounted");
  });
  ...
  return () => html`
    <div class="app-992">
      <header class="header-992">
        <h1>todos — Component0992</h1>
      </header>

      <main class="main-992">
        <input
          class="add-input-992 ${error() ? "add-input--error-992" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-992" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-992">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-992">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-992 ${filter() === f ? "filter-btn--active-992" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-992">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-992">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-992">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-992 ${todo.done ? "todo-item--done-992" : ""}">
                    <button
                      class="todo-check-992 ${todo.done ? "todo-check--done-992" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-992" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-992"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-992" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-992" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-992">
            <span class="todo-count-992">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-992" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
