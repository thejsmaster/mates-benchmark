// client/components/Component0396.ts
// Benchmark component #396 — identical logic to TodoPage, scoped class names.
...
import "./Component0396.css";

type Filter = "all" | "active" | "done";

export const Component0396 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0396] mounted");
    return () => console.log("[Component0396] unmounted");
  });
  ...
  return () => html`
    <div class="app-396">
      <header class="header-396">
        <h1>todos — Component0396</h1>
      </header>

      <main class="main-396">
        <input
          class="add-input-396 ${error() ? "add-input--error-396" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-396" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-396">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-396">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-396 ${filter() === f ? "filter-btn--active-396" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-396">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-396">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-396">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-396 ${todo.done ? "todo-item--done-396" : ""}">
                    <button
                      class="todo-check-396 ${todo.done ? "todo-check--done-396" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-396" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-396"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-396" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-396" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-396">
            <span class="todo-count-396">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-396" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
