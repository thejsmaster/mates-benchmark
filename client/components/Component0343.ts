// client/components/Component0343.ts
// Benchmark component #343 — identical logic to TodoPage, scoped class names.
...
import "./Component0343.css";

type Filter = "all" | "active" | "done";

export const Component0343 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0343] mounted");
    return () => console.log("[Component0343] unmounted");
  });
  ...
  return () => html`
    <div class="app-343">
      <header class="header-343">
        <h1>todos — Component0343</h1>
      </header>

      <main class="main-343">
        <input
          class="add-input-343 ${error() ? "add-input--error-343" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-343" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-343">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-343">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-343 ${filter() === f ? "filter-btn--active-343" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-343">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-343">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-343">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-343 ${todo.done ? "todo-item--done-343" : ""}">
                    <button
                      class="todo-check-343 ${todo.done ? "todo-check--done-343" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-343" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-343"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-343" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-343" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-343">
            <span class="todo-count-343">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-343" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
