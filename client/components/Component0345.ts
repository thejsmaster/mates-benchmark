// client/components/Component0345.ts
// Benchmark component #345 — identical logic to TodoPage, scoped class names.
...
import "./Component0345.css";

type Filter = "all" | "active" | "done";

export const Component0345 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0345] mounted");
    return () => console.log("[Component0345] unmounted");
  });
  ...
  return () => html`
    <div class="app-345">
      <header class="header-345">
        <h1>todos — Component0345</h1>
      </header>

      <main class="main-345">
        <input
          class="add-input-345 ${error() ? "add-input--error-345" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-345" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-345">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-345">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-345 ${filter() === f ? "filter-btn--active-345" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-345">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-345">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-345">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-345 ${todo.done ? "todo-item--done-345" : ""}">
                    <button
                      class="todo-check-345 ${todo.done ? "todo-check--done-345" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-345" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-345"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-345" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-345" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-345">
            <span class="todo-count-345">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-345" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
