// client/components/Component0482.ts
// Benchmark component #482 — identical logic to TodoPage, scoped class names.
...
import "./Component0482.css";

type Filter = "all" | "active" | "done";

export const Component0482 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0482] mounted");
    return () => console.log("[Component0482] unmounted");
  });
  ...
  return () => html`
    <div class="app-482">
      <header class="header-482">
        <h1>todos — Component0482</h1>
      </header>

      <main class="main-482">
        <input
          class="add-input-482 ${error() ? "add-input--error-482" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-482" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-482">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-482">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-482 ${filter() === f ? "filter-btn--active-482" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-482">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-482">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-482">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-482 ${todo.done ? "todo-item--done-482" : ""}">
                    <button
                      class="todo-check-482 ${todo.done ? "todo-check--done-482" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-482" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-482"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-482" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-482" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-482">
            <span class="todo-count-482">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-482" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
