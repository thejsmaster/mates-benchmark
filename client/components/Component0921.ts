// client/components/Component0921.ts
// Benchmark component #921 — identical logic to TodoPage, scoped class names.
...
import "./Component0921.css";

type Filter = "all" | "active" | "done";

export const Component0921 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0921] mounted");
    return () => console.log("[Component0921] unmounted");
  });
  ...
  return () => html`
    <div class="app-921">
      <header class="header-921">
        <h1>todos — Component0921</h1>
      </header>

      <main class="main-921">
        <input
          class="add-input-921 ${error() ? "add-input--error-921" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-921" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-921">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-921">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-921 ${filter() === f ? "filter-btn--active-921" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-921">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-921">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-921">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-921 ${todo.done ? "todo-item--done-921" : ""}">
                    <button
                      class="todo-check-921 ${todo.done ? "todo-check--done-921" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-921" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-921"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-921" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-921" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-921">
            <span class="todo-count-921">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-921" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
