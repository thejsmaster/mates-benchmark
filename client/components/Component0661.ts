// client/components/Component0661.ts
// Benchmark component #661 — identical logic to TodoPage, scoped class names.
...
import "./Component0661.css";

type Filter = "all" | "active" | "done";

export const Component0661 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0661] mounted");
    return () => console.log("[Component0661] unmounted");
  });
  ...
  return () => html`
    <div class="app-661">
      <header class="header-661">
        <h1>todos — Component0661</h1>
      </header>

      <main class="main-661">
        <input
          class="add-input-661 ${error() ? "add-input--error-661" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-661" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-661">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-661">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-661 ${filter() === f ? "filter-btn--active-661" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-661">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-661">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-661">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-661 ${todo.done ? "todo-item--done-661" : ""}">
                    <button
                      class="todo-check-661 ${todo.done ? "todo-check--done-661" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-661" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-661"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-661" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-661" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-661">
            <span class="todo-count-661">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-661" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
