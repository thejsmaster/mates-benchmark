// client/components/Component0969.ts
// Benchmark component #969 — identical logic to TodoPage, scoped class names.
...
import "./Component0969.css";

type Filter = "all" | "active" | "done";

export const Component0969 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0969] mounted");
    return () => console.log("[Component0969] unmounted");
  });
  ...
  return () => html`
    <div class="app-969">
      <header class="header-969">
        <h1>todos — Component0969</h1>
      </header>

      <main class="main-969">
        <input
          class="add-input-969 ${error() ? "add-input--error-969" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-969" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-969">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-969">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-969 ${filter() === f ? "filter-btn--active-969" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-969">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-969">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-969">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-969 ${todo.done ? "todo-item--done-969" : ""}">
                    <button
                      class="todo-check-969 ${todo.done ? "todo-check--done-969" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-969" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-969"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-969" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-969" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-969">
            <span class="todo-count-969">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-969" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
