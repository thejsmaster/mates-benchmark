// client/components/Component0848.ts
// Benchmark component #848 — identical logic to TodoPage, scoped class names.
...
import "./Component0848.css";

type Filter = "all" | "active" | "done";

export const Component0848 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0848] mounted");
    return () => console.log("[Component0848] unmounted");
  });
  ...
  return () => html`
    <div class="app-848">
      <header class="header-848">
        <h1>todos — Component0848</h1>
      </header>

      <main class="main-848">
        <input
          class="add-input-848 ${error() ? "add-input--error-848" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-848" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-848">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-848">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-848 ${filter() === f ? "filter-btn--active-848" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-848">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-848">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-848">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-848 ${todo.done ? "todo-item--done-848" : ""}">
                    <button
                      class="todo-check-848 ${todo.done ? "todo-check--done-848" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-848" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-848"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-848" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-848" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-848">
            <span class="todo-count-848">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-848" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
