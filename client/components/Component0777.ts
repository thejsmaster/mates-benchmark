// client/components/Component0777.ts
// Benchmark component #777 — identical logic to TodoPage, scoped class names.
...
import "./Component0777.css";

type Filter = "all" | "active" | "done";

export const Component0777 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0777] mounted");
    return () => console.log("[Component0777] unmounted");
  });
  ...
  return () => html`
    <div class="app-777">
      <header class="header-777">
        <h1>todos — Component0777</h1>
      </header>

      <main class="main-777">
        <input
          class="add-input-777 ${error() ? "add-input--error-777" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-777" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-777">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-777">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-777 ${filter() === f ? "filter-btn--active-777" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-777">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-777">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-777">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-777 ${todo.done ? "todo-item--done-777" : ""}">
                    <button
                      class="todo-check-777 ${todo.done ? "todo-check--done-777" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-777" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-777"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-777" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-777" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-777">
            <span class="todo-count-777">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-777" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
