// client/components/Component0926.ts
// Benchmark component #926 — identical logic to TodoPage, scoped class names.
...
import "./Component0926.css";

type Filter = "all" | "active" | "done";

export const Component0926 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0926] mounted");
    return () => console.log("[Component0926] unmounted");
  });
  ...
  return () => html`
    <div class="app-926">
      <header class="header-926">
        <h1>todos — Component0926</h1>
      </header>

      <main class="main-926">
        <input
          class="add-input-926 ${error() ? "add-input--error-926" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-926" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-926">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-926">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-926 ${filter() === f ? "filter-btn--active-926" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-926">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-926">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-926">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-926 ${todo.done ? "todo-item--done-926" : ""}">
                    <button
                      class="todo-check-926 ${todo.done ? "todo-check--done-926" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-926" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-926"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-926" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-926" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-926">
            <span class="todo-count-926">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-926" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
