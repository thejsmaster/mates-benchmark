// client/components/Component0549.ts
// Benchmark component #549 — identical logic to TodoPage, scoped class names.
...
import "./Component0549.css";

type Filter = "all" | "active" | "done";

export const Component0549 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0549] mounted");
    return () => console.log("[Component0549] unmounted");
  });
  ...
  return () => html`
    <div class="app-549">
      <header class="header-549">
        <h1>todos — Component0549</h1>
      </header>

      <main class="main-549">
        <input
          class="add-input-549 ${error() ? "add-input--error-549" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-549" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-549">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-549">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-549 ${filter() === f ? "filter-btn--active-549" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-549">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-549">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-549">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-549 ${todo.done ? "todo-item--done-549" : ""}">
                    <button
                      class="todo-check-549 ${todo.done ? "todo-check--done-549" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-549" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-549"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-549" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-549" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-549">
            <span class="todo-count-549">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-549" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
