// client/components/Component0339.ts
// Benchmark component #339 — identical logic to TodoPage, scoped class names.
...
import "./Component0339.css";

type Filter = "all" | "active" | "done";

export const Component0339 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0339] mounted");
    return () => console.log("[Component0339] unmounted");
  });
  ...
  return () => html`
    <div class="app-339">
      <header class="header-339">
        <h1>todos — Component0339</h1>
      </header>

      <main class="main-339">
        <input
          class="add-input-339 ${error() ? "add-input--error-339" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-339" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-339">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-339">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-339 ${filter() === f ? "filter-btn--active-339" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-339">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-339">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-339">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-339 ${todo.done ? "todo-item--done-339" : ""}">
                    <button
                      class="todo-check-339 ${todo.done ? "todo-check--done-339" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-339" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-339"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-339" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-339" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-339">
            <span class="todo-count-339">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-339" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
