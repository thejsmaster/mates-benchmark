// client/components/Component0146.ts
// Benchmark component #146 — identical logic to TodoPage, scoped class names.
...
import "./Component0146.css";

type Filter = "all" | "active" | "done";

export const Component0146 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0146] mounted");
    return () => console.log("[Component0146] unmounted");
  });
  ...
  return () => html`
    <div class="app-146">
      <header class="header-146">
        <h1>todos — Component0146</h1>
      </header>

      <main class="main-146">
        <input
          class="add-input-146 ${error() ? "add-input--error-146" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-146" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-146">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-146">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-146 ${filter() === f ? "filter-btn--active-146" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-146">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-146">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-146">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-146 ${todo.done ? "todo-item--done-146" : ""}">
                    <button
                      class="todo-check-146 ${todo.done ? "todo-check--done-146" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-146" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-146"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-146" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-146" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-146">
            <span class="todo-count-146">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-146" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
