// client/components/Component0174.ts
// Benchmark component #174 — identical logic to TodoPage, scoped class names.
...
import "./Component0174.css";

type Filter = "all" | "active" | "done";

export const Component0174 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0174] mounted");
    return () => console.log("[Component0174] unmounted");
  });
  ...
  return () => html`
    <div class="app-174">
      <header class="header-174">
        <h1>todos — Component0174</h1>
      </header>

      <main class="main-174">
        <input
          class="add-input-174 ${error() ? "add-input--error-174" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-174" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-174">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-174">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-174 ${filter() === f ? "filter-btn--active-174" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-174">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-174">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-174">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-174 ${todo.done ? "todo-item--done-174" : ""}">
                    <button
                      class="todo-check-174 ${todo.done ? "todo-check--done-174" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-174" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-174"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-174" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-174" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-174">
            <span class="todo-count-174">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-174" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
