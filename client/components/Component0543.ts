// client/components/Component0543.ts
// Benchmark component #543 — identical logic to TodoPage, scoped class names.
...
import "./Component0543.css";

type Filter = "all" | "active" | "done";

export const Component0543 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0543] mounted");
    return () => console.log("[Component0543] unmounted");
  });
  ...
  return () => html`
    <div class="app-543">
      <header class="header-543">
        <h1>todos — Component0543</h1>
      </header>

      <main class="main-543">
        <input
          class="add-input-543 ${error() ? "add-input--error-543" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-543" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-543">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-543">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-543 ${filter() === f ? "filter-btn--active-543" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-543">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-543">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-543">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-543 ${todo.done ? "todo-item--done-543" : ""}">
                    <button
                      class="todo-check-543 ${todo.done ? "todo-check--done-543" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-543" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-543"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-543" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-543" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-543">
            <span class="todo-count-543">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-543" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
