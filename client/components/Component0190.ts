// client/components/Component0190.ts
// Benchmark component #190 — identical logic to TodoPage, scoped class names.
...
import "./Component0190.css";

type Filter = "all" | "active" | "done";

export const Component0190 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0190] mounted");
    return () => console.log("[Component0190] unmounted");
  });
  ...
  return () => html`
    <div class="app-190">
      <header class="header-190">
        <h1>todos — Component0190</h1>
      </header>

      <main class="main-190">
        <input
          class="add-input-190 ${error() ? "add-input--error-190" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-190" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-190">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-190">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-190 ${filter() === f ? "filter-btn--active-190" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-190">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-190">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-190">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-190 ${todo.done ? "todo-item--done-190" : ""}">
                    <button
                      class="todo-check-190 ${todo.done ? "todo-check--done-190" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-190" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-190"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-190" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-190" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-190">
            <span class="todo-count-190">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-190" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
