// client/components/Component0961.ts
// Benchmark component #961 — identical logic to TodoPage, scoped class names.
...
import "./Component0961.css";

type Filter = "all" | "active" | "done";

export const Component0961 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0961] mounted");
    return () => console.log("[Component0961] unmounted");
  });
  ...
  return () => html`
    <div class="app-961">
      <header class="header-961">
        <h1>todos — Component0961</h1>
      </header>

      <main class="main-961">
        <input
          class="add-input-961 ${error() ? "add-input--error-961" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-961" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-961">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-961">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-961 ${filter() === f ? "filter-btn--active-961" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-961">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-961">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-961">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-961 ${todo.done ? "todo-item--done-961" : ""}">
                    <button
                      class="todo-check-961 ${todo.done ? "todo-check--done-961" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-961" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-961"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-961" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-961" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-961">
            <span class="todo-count-961">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-961" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
