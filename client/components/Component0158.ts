// client/components/Component0158.ts
// Benchmark component #158 — identical logic to TodoPage, scoped class names.
...
import "./Component0158.css";

type Filter = "all" | "active" | "done";

export const Component0158 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0158] mounted");
    return () => console.log("[Component0158] unmounted");
  });
  ...
  return () => html`
    <div class="app-158">
      <header class="header-158">
        <h1>todos — Component0158</h1>
      </header>

      <main class="main-158">
        <input
          class="add-input-158 ${error() ? "add-input--error-158" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-158" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-158">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-158">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-158 ${filter() === f ? "filter-btn--active-158" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-158">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-158">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-158">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-158 ${todo.done ? "todo-item--done-158" : ""}">
                    <button
                      class="todo-check-158 ${todo.done ? "todo-check--done-158" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-158" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-158"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-158" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-158" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-158">
            <span class="todo-count-158">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-158" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
