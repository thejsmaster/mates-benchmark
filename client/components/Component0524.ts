// client/components/Component0524.ts
// Benchmark component #524 — identical logic to TodoPage, scoped class names.
...
import "./Component0524.css";

type Filter = "all" | "active" | "done";

export const Component0524 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0524] mounted");
    return () => console.log("[Component0524] unmounted");
  });
  ...
  return () => html`
    <div class="app-524">
      <header class="header-524">
        <h1>todos — Component0524</h1>
      </header>

      <main class="main-524">
        <input
          class="add-input-524 ${error() ? "add-input--error-524" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-524" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-524">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-524">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-524 ${filter() === f ? "filter-btn--active-524" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-524">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-524">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-524">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-524 ${todo.done ? "todo-item--done-524" : ""}">
                    <button
                      class="todo-check-524 ${todo.done ? "todo-check--done-524" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-524" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-524"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-524" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-524" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-524">
            <span class="todo-count-524">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-524" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
