// client/components/Component0266.ts
// Benchmark component #266 — identical logic to TodoPage, scoped class names.
...
import "./Component0266.css";

type Filter = "all" | "active" | "done";

export const Component0266 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0266] mounted");
    return () => console.log("[Component0266] unmounted");
  });
  ...
  return () => html`
    <div class="app-266">
      <header class="header-266">
        <h1>todos — Component0266</h1>
      </header>

      <main class="main-266">
        <input
          class="add-input-266 ${error() ? "add-input--error-266" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-266" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-266">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-266">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-266 ${filter() === f ? "filter-btn--active-266" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-266">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-266">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-266">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-266 ${todo.done ? "todo-item--done-266" : ""}">
                    <button
                      class="todo-check-266 ${todo.done ? "todo-check--done-266" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-266" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-266"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-266" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-266" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-266">
            <span class="todo-count-266">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-266" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
