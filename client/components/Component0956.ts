// client/components/Component0956.ts
// Benchmark component #956 — identical logic to TodoPage, scoped class names.
...
import "./Component0956.css";

type Filter = "all" | "active" | "done";

export const Component0956 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0956] mounted");
    return () => console.log("[Component0956] unmounted");
  });
  ...
  return () => html`
    <div class="app-956">
      <header class="header-956">
        <h1>todos — Component0956</h1>
      </header>

      <main class="main-956">
        <input
          class="add-input-956 ${error() ? "add-input--error-956" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-956" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-956">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-956">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-956 ${filter() === f ? "filter-btn--active-956" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-956">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-956">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-956">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-956 ${todo.done ? "todo-item--done-956" : ""}">
                    <button
                      class="todo-check-956 ${todo.done ? "todo-check--done-956" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-956" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-956"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-956" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-956" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-956">
            <span class="todo-count-956">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-956" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
