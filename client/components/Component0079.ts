// client/components/Component0079.ts
// Benchmark component #79 — identical logic to TodoPage, scoped class names.
...
import "./Component0079.css";

type Filter = "all" | "active" | "done";

export const Component0079 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0079] mounted");
    return () => console.log("[Component0079] unmounted");
  });
  ...
  return () => html`
    <div class="app-79">
      <header class="header-79">
        <h1>todos — Component0079</h1>
      </header>

      <main class="main-79">
        <input
          class="add-input-79 ${error() ? "add-input--error-79" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-79" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-79">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-79">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-79 ${filter() === f ? "filter-btn--active-79" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-79">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-79">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-79">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-79 ${todo.done ? "todo-item--done-79" : ""}">
                    <button
                      class="todo-check-79 ${todo.done ? "todo-check--done-79" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-79" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-79"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-79" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-79" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-79">
            <span class="todo-count-79">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-79" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
