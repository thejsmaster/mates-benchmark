// client/components/Component0277.ts
// Benchmark component #277 — identical logic to TodoPage, scoped class names.
...
import "./Component0277.css";

type Filter = "all" | "active" | "done";

export const Component0277 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0277] mounted");
    return () => console.log("[Component0277] unmounted");
  });
  ...
  return () => html`
    <div class="app-277">
      <header class="header-277">
        <h1>todos — Component0277</h1>
      </header>

      <main class="main-277">
        <input
          class="add-input-277 ${error() ? "add-input--error-277" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-277" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-277">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-277">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-277 ${filter() === f ? "filter-btn--active-277" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-277">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-277">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-277">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-277 ${todo.done ? "todo-item--done-277" : ""}">
                    <button
                      class="todo-check-277 ${todo.done ? "todo-check--done-277" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-277" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-277"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-277" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-277" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-277">
            <span class="todo-count-277">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-277" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
