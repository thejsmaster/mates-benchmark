// client/components/Component0231.ts
// Benchmark component #231 — identical logic to TodoPage, scoped class names.
...
import "./Component0231.css";

type Filter = "all" | "active" | "done";

export const Component0231 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0231] mounted");
    return () => console.log("[Component0231] unmounted");
  });
  ...
  return () => html`
    <div class="app-231">
      <header class="header-231">
        <h1>todos — Component0231</h1>
      </header>

      <main class="main-231">
        <input
          class="add-input-231 ${error() ? "add-input--error-231" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-231" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-231">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-231">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-231 ${filter() === f ? "filter-btn--active-231" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-231">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-231">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-231">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-231 ${todo.done ? "todo-item--done-231" : ""}">
                    <button
                      class="todo-check-231 ${todo.done ? "todo-check--done-231" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-231" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-231"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-231" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-231" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-231">
            <span class="todo-count-231">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-231" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
