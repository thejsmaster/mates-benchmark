// client/components/Component0713.ts
// Benchmark component #713 — identical logic to TodoPage, scoped class names.
...
import "./Component0713.css";

type Filter = "all" | "active" | "done";

export const Component0713 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0713] mounted");
    return () => console.log("[Component0713] unmounted");
  });
  ...
  return () => html`
    <div class="app-713">
      <header class="header-713">
        <h1>todos — Component0713</h1>
      </header>

      <main class="main-713">
        <input
          class="add-input-713 ${error() ? "add-input--error-713" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-713" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-713">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-713">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-713 ${filter() === f ? "filter-btn--active-713" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-713">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-713">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-713">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-713 ${todo.done ? "todo-item--done-713" : ""}">
                    <button
                      class="todo-check-713 ${todo.done ? "todo-check--done-713" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-713" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-713"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-713" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-713" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-713">
            <span class="todo-count-713">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-713" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
