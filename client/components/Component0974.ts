// client/components/Component0974.ts
// Benchmark component #974 — identical logic to TodoPage, scoped class names.
...
import "./Component0974.css";

type Filter = "all" | "active" | "done";

export const Component0974 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0974] mounted");
    return () => console.log("[Component0974] unmounted");
  });
  ...
  return () => html`
    <div class="app-974">
      <header class="header-974">
        <h1>todos — Component0974</h1>
      </header>

      <main class="main-974">
        <input
          class="add-input-974 ${error() ? "add-input--error-974" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-974" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-974">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-974">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-974 ${filter() === f ? "filter-btn--active-974" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-974">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-974">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-974">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-974 ${todo.done ? "todo-item--done-974" : ""}">
                    <button
                      class="todo-check-974 ${todo.done ? "todo-check--done-974" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-974" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-974"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-974" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-974" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-974">
            <span class="todo-count-974">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-974" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
