// client/components/Component0109.ts
// Benchmark component #109 — identical logic to TodoPage, scoped class names.
...
import "./Component0109.css";

type Filter = "all" | "active" | "done";

export const Component0109 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0109] mounted");
    return () => console.log("[Component0109] unmounted");
  });
  ...
  return () => html`
    <div class="app-109">
      <header class="header-109">
        <h1>todos — Component0109</h1>
      </header>

      <main class="main-109">
        <input
          class="add-input-109 ${error() ? "add-input--error-109" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-109" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-109">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-109">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-109 ${filter() === f ? "filter-btn--active-109" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-109">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-109">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-109">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-109 ${todo.done ? "todo-item--done-109" : ""}">
                    <button
                      class="todo-check-109 ${todo.done ? "todo-check--done-109" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-109" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-109"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-109" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-109" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-109">
            <span class="todo-count-109">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-109" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
