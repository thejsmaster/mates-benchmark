// client/components/Component0789.ts
// Benchmark component #789 — identical logic to TodoPage, scoped class names.
...
import "./Component0789.css";

type Filter = "all" | "active" | "done";

export const Component0789 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0789] mounted");
    return () => console.log("[Component0789] unmounted");
  });
  ...
  return () => html`
    <div class="app-789">
      <header class="header-789">
        <h1>todos — Component0789</h1>
      </header>

      <main class="main-789">
        <input
          class="add-input-789 ${error() ? "add-input--error-789" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-789" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-789">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-789">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-789 ${filter() === f ? "filter-btn--active-789" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-789">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-789">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-789">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-789 ${todo.done ? "todo-item--done-789" : ""}">
                    <button
                      class="todo-check-789 ${todo.done ? "todo-check--done-789" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-789" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-789"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-789" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-789" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-789">
            <span class="todo-count-789">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-789" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
