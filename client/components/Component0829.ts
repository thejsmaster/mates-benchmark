// client/components/Component0829.ts
// Benchmark component #829 — identical logic to TodoPage, scoped class names.
...
import "./Component0829.css";

type Filter = "all" | "active" | "done";

export const Component0829 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0829] mounted");
    return () => console.log("[Component0829] unmounted");
  });
  ...
  return () => html`
    <div class="app-829">
      <header class="header-829">
        <h1>todos — Component0829</h1>
      </header>

      <main class="main-829">
        <input
          class="add-input-829 ${error() ? "add-input--error-829" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-829" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-829">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-829">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-829 ${filter() === f ? "filter-btn--active-829" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-829">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-829">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-829">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-829 ${todo.done ? "todo-item--done-829" : ""}">
                    <button
                      class="todo-check-829 ${todo.done ? "todo-check--done-829" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-829" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-829"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-829" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-829" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-829">
            <span class="todo-count-829">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-829" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
