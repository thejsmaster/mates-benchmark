// client/components/Component0809.ts
// Benchmark component #809 — identical logic to TodoPage, scoped class names.
...
import "./Component0809.css";

type Filter = "all" | "active" | "done";

export const Component0809 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0809] mounted");
    return () => console.log("[Component0809] unmounted");
  });
  ...
  return () => html`
    <div class="app-809">
      <header class="header-809">
        <h1>todos — Component0809</h1>
      </header>

      <main class="main-809">
        <input
          class="add-input-809 ${error() ? "add-input--error-809" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-809" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-809">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-809">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-809 ${filter() === f ? "filter-btn--active-809" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-809">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-809">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-809">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-809 ${todo.done ? "todo-item--done-809" : ""}">
                    <button
                      class="todo-check-809 ${todo.done ? "todo-check--done-809" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-809" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-809"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-809" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-809" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-809">
            <span class="todo-count-809">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-809" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
