// client/components/Component0578.ts
// Benchmark component #578 — identical logic to TodoPage, scoped class names.
...
import "./Component0578.css";

type Filter = "all" | "active" | "done";

export const Component0578 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0578] mounted");
    return () => console.log("[Component0578] unmounted");
  });
  ...
  return () => html`
    <div class="app-578">
      <header class="header-578">
        <h1>todos — Component0578</h1>
      </header>

      <main class="main-578">
        <input
          class="add-input-578 ${error() ? "add-input--error-578" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-578" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-578">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-578">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-578 ${filter() === f ? "filter-btn--active-578" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-578">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-578">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-578">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-578 ${todo.done ? "todo-item--done-578" : ""}">
                    <button
                      class="todo-check-578 ${todo.done ? "todo-check--done-578" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-578" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-578"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-578" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-578" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-578">
            <span class="todo-count-578">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-578" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
