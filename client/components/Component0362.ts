// client/components/Component0362.ts
// Benchmark component #362 — identical logic to TodoPage, scoped class names.
...
import "./Component0362.css";

type Filter = "all" | "active" | "done";

export const Component0362 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0362] mounted");
    return () => console.log("[Component0362] unmounted");
  });
  ...
  return () => html`
    <div class="app-362">
      <header class="header-362">
        <h1>todos — Component0362</h1>
      </header>

      <main class="main-362">
        <input
          class="add-input-362 ${error() ? "add-input--error-362" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-362" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-362">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-362">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-362 ${filter() === f ? "filter-btn--active-362" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-362">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-362">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-362">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-362 ${todo.done ? "todo-item--done-362" : ""}">
                    <button
                      class="todo-check-362 ${todo.done ? "todo-check--done-362" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-362" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-362"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-362" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-362" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-362">
            <span class="todo-count-362">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-362" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
