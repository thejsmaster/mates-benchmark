// client/components/Component0730.ts
// Benchmark component #730 — identical logic to TodoPage, scoped class names.
...
import "./Component0730.css";

type Filter = "all" | "active" | "done";

export const Component0730 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0730] mounted");
    return () => console.log("[Component0730] unmounted");
  });
  ...
  return () => html`
    <div class="app-730">
      <header class="header-730">
        <h1>todos — Component0730</h1>
      </header>

      <main class="main-730">
        <input
          class="add-input-730 ${error() ? "add-input--error-730" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-730" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-730">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-730">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-730 ${filter() === f ? "filter-btn--active-730" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-730">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-730">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-730">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-730 ${todo.done ? "todo-item--done-730" : ""}">
                    <button
                      class="todo-check-730 ${todo.done ? "todo-check--done-730" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-730" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-730"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-730" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-730" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-730">
            <span class="todo-count-730">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-730" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
