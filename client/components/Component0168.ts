// client/components/Component0168.ts
// Benchmark component #168 — identical logic to TodoPage, scoped class names.
...
import "./Component0168.css";

type Filter = "all" | "active" | "done";

export const Component0168 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0168] mounted");
    return () => console.log("[Component0168] unmounted");
  });
  ...
  return () => html`
    <div class="app-168">
      <header class="header-168">
        <h1>todos — Component0168</h1>
      </header>

      <main class="main-168">
        <input
          class="add-input-168 ${error() ? "add-input--error-168" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-168" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-168">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-168">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-168 ${filter() === f ? "filter-btn--active-168" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-168">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-168">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-168">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-168 ${todo.done ? "todo-item--done-168" : ""}">
                    <button
                      class="todo-check-168 ${todo.done ? "todo-check--done-168" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-168" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-168"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-168" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-168" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-168">
            <span class="todo-count-168">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-168" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
