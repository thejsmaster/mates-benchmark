// client/components/Component0352.ts
// Benchmark component #352 — identical logic to TodoPage, scoped class names.
...
import "./Component0352.css";

type Filter = "all" | "active" | "done";

export const Component0352 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0352] mounted");
    return () => console.log("[Component0352] unmounted");
  });
  ...
  return () => html`
    <div class="app-352">
      <header class="header-352">
        <h1>todos — Component0352</h1>
      </header>

      <main class="main-352">
        <input
          class="add-input-352 ${error() ? "add-input--error-352" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-352" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-352">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-352">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-352 ${filter() === f ? "filter-btn--active-352" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-352">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-352">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-352">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-352 ${todo.done ? "todo-item--done-352" : ""}">
                    <button
                      class="todo-check-352 ${todo.done ? "todo-check--done-352" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-352" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-352"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-352" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-352" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-352">
            <span class="todo-count-352">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-352" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
