// client/components/Component0358.ts
// Benchmark component #358 — identical logic to TodoPage, scoped class names.
...
import "./Component0358.css";

type Filter = "all" | "active" | "done";

export const Component0358 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0358] mounted");
    return () => console.log("[Component0358] unmounted");
  });
  ...
  return () => html`
    <div class="app-358">
      <header class="header-358">
        <h1>todos — Component0358</h1>
      </header>

      <main class="main-358">
        <input
          class="add-input-358 ${error() ? "add-input--error-358" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-358" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-358">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-358">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-358 ${filter() === f ? "filter-btn--active-358" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-358">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-358">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-358">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-358 ${todo.done ? "todo-item--done-358" : ""}">
                    <button
                      class="todo-check-358 ${todo.done ? "todo-check--done-358" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-358" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-358"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-358" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-358" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-358">
            <span class="todo-count-358">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-358" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
