// client/components/Component0182.ts
// Benchmark component #182 — identical logic to TodoPage, scoped class names.
...
import "./Component0182.css";

type Filter = "all" | "active" | "done";

export const Component0182 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0182] mounted");
    return () => console.log("[Component0182] unmounted");
  });
  ...
  return () => html`
    <div class="app-182">
      <header class="header-182">
        <h1>todos — Component0182</h1>
      </header>

      <main class="main-182">
        <input
          class="add-input-182 ${error() ? "add-input--error-182" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-182" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-182">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-182">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-182 ${filter() === f ? "filter-btn--active-182" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-182">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-182">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-182">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-182 ${todo.done ? "todo-item--done-182" : ""}">
                    <button
                      class="todo-check-182 ${todo.done ? "todo-check--done-182" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-182" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-182"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-182" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-182" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-182">
            <span class="todo-count-182">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-182" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
