// client/components/Component0942.ts
// Benchmark component #942 — identical logic to TodoPage, scoped class names.
...
import "./Component0942.css";

type Filter = "all" | "active" | "done";

export const Component0942 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0942] mounted");
    return () => console.log("[Component0942] unmounted");
  });
  ...
  return () => html`
    <div class="app-942">
      <header class="header-942">
        <h1>todos — Component0942</h1>
      </header>

      <main class="main-942">
        <input
          class="add-input-942 ${error() ? "add-input--error-942" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-942" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-942">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-942">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-942 ${filter() === f ? "filter-btn--active-942" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-942">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-942">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-942">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-942 ${todo.done ? "todo-item--done-942" : ""}">
                    <button
                      class="todo-check-942 ${todo.done ? "todo-check--done-942" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-942" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-942"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-942" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-942" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-942">
            <span class="todo-count-942">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-942" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
