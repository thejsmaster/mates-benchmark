// client/components/Component0866.ts
// Benchmark component #866 — identical logic to TodoPage, scoped class names.
...
import "./Component0866.css";

type Filter = "all" | "active" | "done";

export const Component0866 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0866] mounted");
    return () => console.log("[Component0866] unmounted");
  });
  ...
  return () => html`
    <div class="app-866">
      <header class="header-866">
        <h1>todos — Component0866</h1>
      </header>

      <main class="main-866">
        <input
          class="add-input-866 ${error() ? "add-input--error-866" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-866" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-866">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-866">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-866 ${filter() === f ? "filter-btn--active-866" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-866">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-866">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-866">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-866 ${todo.done ? "todo-item--done-866" : ""}">
                    <button
                      class="todo-check-866 ${todo.done ? "todo-check--done-866" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-866" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-866"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-866" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-866" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-866">
            <span class="todo-count-866">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-866" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
