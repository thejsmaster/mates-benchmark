// client/components/Component0734.ts
// Benchmark component #734 — identical logic to TodoPage, scoped class names.
...
import "./Component0734.css";

type Filter = "all" | "active" | "done";

export const Component0734 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0734] mounted");
    return () => console.log("[Component0734] unmounted");
  });
  ...
  return () => html`
    <div class="app-734">
      <header class="header-734">
        <h1>todos — Component0734</h1>
      </header>

      <main class="main-734">
        <input
          class="add-input-734 ${error() ? "add-input--error-734" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-734" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-734">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-734">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-734 ${filter() === f ? "filter-btn--active-734" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-734">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-734">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-734">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-734 ${todo.done ? "todo-item--done-734" : ""}">
                    <button
                      class="todo-check-734 ${todo.done ? "todo-check--done-734" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-734" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-734"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-734" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-734" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-734">
            <span class="todo-count-734">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-734" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
