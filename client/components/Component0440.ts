// client/components/Component0440.ts
// Benchmark component #440 — identical logic to TodoPage, scoped class names.
...
import "./Component0440.css";

type Filter = "all" | "active" | "done";

export const Component0440 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0440] mounted");
    return () => console.log("[Component0440] unmounted");
  });
  ...
  return () => html`
    <div class="app-440">
      <header class="header-440">
        <h1>todos — Component0440</h1>
      </header>

      <main class="main-440">
        <input
          class="add-input-440 ${error() ? "add-input--error-440" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-440" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-440">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-440">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-440 ${filter() === f ? "filter-btn--active-440" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-440">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-440">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-440">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-440 ${todo.done ? "todo-item--done-440" : ""}">
                    <button
                      class="todo-check-440 ${todo.done ? "todo-check--done-440" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-440" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-440"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-440" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-440" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-440">
            <span class="todo-count-440">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-440" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
