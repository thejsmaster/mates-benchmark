// client/components/Component0880.ts
// Benchmark component #880 — identical logic to TodoPage, scoped class names.
...
import "./Component0880.css";

type Filter = "all" | "active" | "done";

export const Component0880 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0880] mounted");
    return () => console.log("[Component0880] unmounted");
  });
  ...
  return () => html`
    <div class="app-880">
      <header class="header-880">
        <h1>todos — Component0880</h1>
      </header>

      <main class="main-880">
        <input
          class="add-input-880 ${error() ? "add-input--error-880" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-880" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-880">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-880">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-880 ${filter() === f ? "filter-btn--active-880" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-880">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-880">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-880">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-880 ${todo.done ? "todo-item--done-880" : ""}">
                    <button
                      class="todo-check-880 ${todo.done ? "todo-check--done-880" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-880" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-880"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-880" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-880" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-880">
            <span class="todo-count-880">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-880" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
