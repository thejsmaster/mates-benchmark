// client/components/Component0290.ts
// Benchmark component #290 — identical logic to TodoPage, scoped class names.
...
import "./Component0290.css";

type Filter = "all" | "active" | "done";

export const Component0290 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0290] mounted");
    return () => console.log("[Component0290] unmounted");
  });
  ...
  return () => html`
    <div class="app-290">
      <header class="header-290">
        <h1>todos — Component0290</h1>
      </header>

      <main class="main-290">
        <input
          class="add-input-290 ${error() ? "add-input--error-290" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-290" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-290">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-290">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-290 ${filter() === f ? "filter-btn--active-290" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-290">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-290">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-290">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-290 ${todo.done ? "todo-item--done-290" : ""}">
                    <button
                      class="todo-check-290 ${todo.done ? "todo-check--done-290" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-290" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-290"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-290" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-290" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-290">
            <span class="todo-count-290">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-290" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
