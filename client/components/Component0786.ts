// client/components/Component0786.ts
// Benchmark component #786 — identical logic to TodoPage, scoped class names.
...
import "./Component0786.css";

type Filter = "all" | "active" | "done";

export const Component0786 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0786] mounted");
    return () => console.log("[Component0786] unmounted");
  });
  ...
  return () => html`
    <div class="app-786">
      <header class="header-786">
        <h1>todos — Component0786</h1>
      </header>

      <main class="main-786">
        <input
          class="add-input-786 ${error() ? "add-input--error-786" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-786" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-786">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-786">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-786 ${filter() === f ? "filter-btn--active-786" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-786">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-786">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-786">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-786 ${todo.done ? "todo-item--done-786" : ""}">
                    <button
                      class="todo-check-786 ${todo.done ? "todo-check--done-786" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-786" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-786"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-786" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-786" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-786">
            <span class="todo-count-786">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-786" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
