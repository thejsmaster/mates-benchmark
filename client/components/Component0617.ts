// client/components/Component0617.ts
// Benchmark component #617 — identical logic to TodoPage, scoped class names.
...
import "./Component0617.css";

type Filter = "all" | "active" | "done";

export const Component0617 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0617] mounted");
    return () => console.log("[Component0617] unmounted");
  });
  ...
  return () => html`
    <div class="app-617">
      <header class="header-617">
        <h1>todos — Component0617</h1>
      </header>

      <main class="main-617">
        <input
          class="add-input-617 ${error() ? "add-input--error-617" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-617" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-617">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-617">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-617 ${filter() === f ? "filter-btn--active-617" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-617">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-617">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-617">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-617 ${todo.done ? "todo-item--done-617" : ""}">
                    <button
                      class="todo-check-617 ${todo.done ? "todo-check--done-617" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-617" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-617"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-617" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-617" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-617">
            <span class="todo-count-617">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-617" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
