// client/components/Component0204.ts
// Benchmark component #204 — identical logic to TodoPage, scoped class names.
...
import "./Component0204.css";

type Filter = "all" | "active" | "done";

export const Component0204 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0204] mounted");
    return () => console.log("[Component0204] unmounted");
  });
  ...
  return () => html`
    <div class="app-204">
      <header class="header-204">
        <h1>todos — Component0204</h1>
      </header>

      <main class="main-204">
        <input
          class="add-input-204 ${error() ? "add-input--error-204" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-204" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-204">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-204">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-204 ${filter() === f ? "filter-btn--active-204" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-204">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-204">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-204">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-204 ${todo.done ? "todo-item--done-204" : ""}">
                    <button
                      class="todo-check-204 ${todo.done ? "todo-check--done-204" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-204" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-204"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-204" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-204" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-204">
            <span class="todo-count-204">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-204" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
