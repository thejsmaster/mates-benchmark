// client/components/Component0037.ts
// Benchmark component #37 — identical logic to TodoPage, scoped class names.
...
import "./Component0037.css";

type Filter = "all" | "active" | "done";

export const Component0037 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0037] mounted");
    return () => console.log("[Component0037] unmounted");
  });
  ...
  return () => html`
    <div class="app-37">
      <header class="header-37">
        <h1>todos — Component0037</h1>
      </header>

      <main class="main-37">
        <input
          class="add-input-37 ${error() ? "add-input--error-37" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-37" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-37">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-37">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-37 ${filter() === f ? "filter-btn--active-37" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-37">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-37">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-37">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-37 ${todo.done ? "todo-item--done-37" : ""}">
                    <button
                      class="todo-check-37 ${todo.done ? "todo-check--done-37" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-37" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-37"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-37" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-37" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-37">
            <span class="todo-count-37">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-37" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
