// client/components/Component0496.ts
// Benchmark component #496 — identical logic to TodoPage, scoped class names.
...
import "./Component0496.css";

type Filter = "all" | "active" | "done";

export const Component0496 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0496] mounted");
    return () => console.log("[Component0496] unmounted");
  });
  ...
  return () => html`
    <div class="app-496">
      <header class="header-496">
        <h1>todos — Component0496</h1>
      </header>

      <main class="main-496">
        <input
          class="add-input-496 ${error() ? "add-input--error-496" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-496" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-496">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-496">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-496 ${filter() === f ? "filter-btn--active-496" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-496">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-496">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-496">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-496 ${todo.done ? "todo-item--done-496" : ""}">
                    <button
                      class="todo-check-496 ${todo.done ? "todo-check--done-496" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-496" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-496"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-496" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-496" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-496">
            <span class="todo-count-496">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-496" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
