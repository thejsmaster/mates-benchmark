// client/components/Component0270.ts
// Benchmark component #270 — identical logic to TodoPage, scoped class names.
...
import "./Component0270.css";

type Filter = "all" | "active" | "done";

export const Component0270 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0270] mounted");
    return () => console.log("[Component0270] unmounted");
  });
  ...
  return () => html`
    <div class="app-270">
      <header class="header-270">
        <h1>todos — Component0270</h1>
      </header>

      <main class="main-270">
        <input
          class="add-input-270 ${error() ? "add-input--error-270" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-270" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-270">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-270">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-270 ${filter() === f ? "filter-btn--active-270" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-270">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-270">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-270">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-270 ${todo.done ? "todo-item--done-270" : ""}">
                    <button
                      class="todo-check-270 ${todo.done ? "todo-check--done-270" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-270" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-270"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-270" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-270" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-270">
            <span class="todo-count-270">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-270" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
