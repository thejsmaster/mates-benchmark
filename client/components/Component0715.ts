// client/components/Component0715.ts
// Benchmark component #715 — identical logic to TodoPage, scoped class names.
...
import "./Component0715.css";

type Filter = "all" | "active" | "done";

export const Component0715 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0715] mounted");
    return () => console.log("[Component0715] unmounted");
  });
  ...
  return () => html`
    <div class="app-715">
      <header class="header-715">
        <h1>todos — Component0715</h1>
      </header>

      <main class="main-715">
        <input
          class="add-input-715 ${error() ? "add-input--error-715" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-715" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-715">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-715">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-715 ${filter() === f ? "filter-btn--active-715" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-715">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-715">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-715">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-715 ${todo.done ? "todo-item--done-715" : ""}">
                    <button
                      class="todo-check-715 ${todo.done ? "todo-check--done-715" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-715" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-715"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-715" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-715" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-715">
            <span class="todo-count-715">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-715" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
