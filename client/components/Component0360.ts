// client/components/Component0360.ts
// Benchmark component #360 — identical logic to TodoPage, scoped class names.
...
import "./Component0360.css";

type Filter = "all" | "active" | "done";

export const Component0360 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0360] mounted");
    return () => console.log("[Component0360] unmounted");
  });
  ...
  return () => html`
    <div class="app-360">
      <header class="header-360">
        <h1>todos — Component0360</h1>
      </header>

      <main class="main-360">
        <input
          class="add-input-360 ${error() ? "add-input--error-360" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-360" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-360">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-360">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-360 ${filter() === f ? "filter-btn--active-360" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-360">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-360">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-360">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-360 ${todo.done ? "todo-item--done-360" : ""}">
                    <button
                      class="todo-check-360 ${todo.done ? "todo-check--done-360" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-360" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-360"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-360" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-360" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-360">
            <span class="todo-count-360">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-360" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
