// client/components/Component0565.ts
// Benchmark component #565 — identical logic to TodoPage, scoped class names.
...
import "./Component0565.css";

type Filter = "all" | "active" | "done";

export const Component0565 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0565] mounted");
    return () => console.log("[Component0565] unmounted");
  });
  ...
  return () => html`
    <div class="app-565">
      <header class="header-565">
        <h1>todos — Component0565</h1>
      </header>

      <main class="main-565">
        <input
          class="add-input-565 ${error() ? "add-input--error-565" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-565" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-565">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-565">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-565 ${filter() === f ? "filter-btn--active-565" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-565">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-565">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-565">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-565 ${todo.done ? "todo-item--done-565" : ""}">
                    <button
                      class="todo-check-565 ${todo.done ? "todo-check--done-565" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-565" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-565"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-565" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-565" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-565">
            <span class="todo-count-565">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-565" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
