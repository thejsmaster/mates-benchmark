// client/components/Component0920.ts
// Benchmark component #920 — identical logic to TodoPage, scoped class names.
...
import "./Component0920.css";

type Filter = "all" | "active" | "done";

export const Component0920 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0920] mounted");
    return () => console.log("[Component0920] unmounted");
  });
  ...
  return () => html`
    <div class="app-920">
      <header class="header-920">
        <h1>todos — Component0920</h1>
      </header>

      <main class="main-920">
        <input
          class="add-input-920 ${error() ? "add-input--error-920" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-920" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-920">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-920">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-920 ${filter() === f ? "filter-btn--active-920" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-920">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-920">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-920">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-920 ${todo.done ? "todo-item--done-920" : ""}">
                    <button
                      class="todo-check-920 ${todo.done ? "todo-check--done-920" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-920" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-920"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-920" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-920" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-920">
            <span class="todo-count-920">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-920" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
