// client/components/Component0288.ts
// Benchmark component #288 — identical logic to TodoPage, scoped class names.
...
import "./Component0288.css";

type Filter = "all" | "active" | "done";

export const Component0288 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0288] mounted");
    return () => console.log("[Component0288] unmounted");
  });
  ...
  return () => html`
    <div class="app-288">
      <header class="header-288">
        <h1>todos — Component0288</h1>
      </header>

      <main class="main-288">
        <input
          class="add-input-288 ${error() ? "add-input--error-288" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-288" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-288">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-288">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-288 ${filter() === f ? "filter-btn--active-288" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-288">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-288">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-288">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-288 ${todo.done ? "todo-item--done-288" : ""}">
                    <button
                      class="todo-check-288 ${todo.done ? "todo-check--done-288" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-288" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-288"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-288" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-288" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-288">
            <span class="todo-count-288">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-288" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
