// client/components/Component0905.ts
// Benchmark component #905 — identical logic to TodoPage, scoped class names.
...
import "./Component0905.css";

type Filter = "all" | "active" | "done";

export const Component0905 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0905] mounted");
    return () => console.log("[Component0905] unmounted");
  });
  ...
  return () => html`
    <div class="app-905">
      <header class="header-905">
        <h1>todos — Component0905</h1>
      </header>

      <main class="main-905">
        <input
          class="add-input-905 ${error() ? "add-input--error-905" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-905" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-905">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-905">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-905 ${filter() === f ? "filter-btn--active-905" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-905">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-905">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-905">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-905 ${todo.done ? "todo-item--done-905" : ""}">
                    <button
                      class="todo-check-905 ${todo.done ? "todo-check--done-905" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-905" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-905"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-905" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-905" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-905">
            <span class="todo-count-905">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-905" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
