// client/components/Component0027.ts
// Benchmark component #27 — identical logic to TodoPage, scoped class names.
...
import "./Component0027.css";

type Filter = "all" | "active" | "done";

export const Component0027 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0027] mounted");
    return () => console.log("[Component0027] unmounted");
  });
  ...
  return () => html`
    <div class="app-27">
      <header class="header-27">
        <h1>todos — Component0027</h1>
      </header>

      <main class="main-27">
        <input
          class="add-input-27 ${error() ? "add-input--error-27" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-27" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-27">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-27">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-27 ${filter() === f ? "filter-btn--active-27" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-27">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-27">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-27">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-27 ${todo.done ? "todo-item--done-27" : ""}">
                    <button
                      class="todo-check-27 ${todo.done ? "todo-check--done-27" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-27" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-27"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-27" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-27" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-27">
            <span class="todo-count-27">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-27" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
