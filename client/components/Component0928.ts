// client/components/Component0928.ts
// Benchmark component #928 — identical logic to TodoPage, scoped class names.
...
import "./Component0928.css";

type Filter = "all" | "active" | "done";

export const Component0928 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0928] mounted");
    return () => console.log("[Component0928] unmounted");
  });
  ...
  return () => html`
    <div class="app-928">
      <header class="header-928">
        <h1>todos — Component0928</h1>
      </header>

      <main class="main-928">
        <input
          class="add-input-928 ${error() ? "add-input--error-928" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-928" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-928">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-928">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-928 ${filter() === f ? "filter-btn--active-928" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-928">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-928">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-928">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-928 ${todo.done ? "todo-item--done-928" : ""}">
                    <button
                      class="todo-check-928 ${todo.done ? "todo-check--done-928" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-928" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-928"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-928" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-928" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-928">
            <span class="todo-count-928">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-928" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
