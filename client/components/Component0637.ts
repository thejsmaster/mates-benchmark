// client/components/Component0637.ts
// Benchmark component #637 — identical logic to TodoPage, scoped class names.
...
import "./Component0637.css";

type Filter = "all" | "active" | "done";

export const Component0637 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0637] mounted");
    return () => console.log("[Component0637] unmounted");
  });
  ...
  return () => html`
    <div class="app-637">
      <header class="header-637">
        <h1>todos — Component0637</h1>
      </header>

      <main class="main-637">
        <input
          class="add-input-637 ${error() ? "add-input--error-637" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-637" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-637">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-637">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-637 ${filter() === f ? "filter-btn--active-637" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-637">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-637">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-637">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-637 ${todo.done ? "todo-item--done-637" : ""}">
                    <button
                      class="todo-check-637 ${todo.done ? "todo-check--done-637" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-637" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-637"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-637" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-637" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-637">
            <span class="todo-count-637">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-637" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
