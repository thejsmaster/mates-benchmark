// client/components/Component0932.ts
// Benchmark component #932 — identical logic to TodoPage, scoped class names.
...
import "./Component0932.css";

type Filter = "all" | "active" | "done";

export const Component0932 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0932] mounted");
    return () => console.log("[Component0932] unmounted");
  });
  ...
  return () => html`
    <div class="app-932">
      <header class="header-932">
        <h1>todos — Component0932</h1>
      </header>

      <main class="main-932">
        <input
          class="add-input-932 ${error() ? "add-input--error-932" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-932" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-932">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-932">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-932 ${filter() === f ? "filter-btn--active-932" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-932">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-932">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-932">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-932 ${todo.done ? "todo-item--done-932" : ""}">
                    <button
                      class="todo-check-932 ${todo.done ? "todo-check--done-932" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-932" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-932"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-932" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-932" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-932">
            <span class="todo-count-932">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-932" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
