// client/components/Component0997.ts
// Benchmark component #997 — identical logic to TodoPage, scoped class names.
...
import "./Component0997.css";

type Filter = "all" | "active" | "done";

export const Component0997 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0997] mounted");
    return () => console.log("[Component0997] unmounted");
  });
  ...
  return () => html`
    <div class="app-997">
      <header class="header-997">
        <h1>todos — Component0997</h1>
      </header>

      <main class="main-997">
        <input
          class="add-input-997 ${error() ? "add-input--error-997" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-997" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-997">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-997">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-997 ${filter() === f ? "filter-btn--active-997" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-997">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-997">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-997">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-997 ${todo.done ? "todo-item--done-997" : ""}">
                    <button
                      class="todo-check-997 ${todo.done ? "todo-check--done-997" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-997" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-997"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-997" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-997" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-997">
            <span class="todo-count-997">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-997" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
