// client/components/Component0287.ts
// Benchmark component #287 — identical logic to TodoPage, scoped class names.
...
import "./Component0287.css";

type Filter = "all" | "active" | "done";

export const Component0287 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0287] mounted");
    return () => console.log("[Component0287] unmounted");
  });
  ...
  return () => html`
    <div class="app-287">
      <header class="header-287">
        <h1>todos — Component0287</h1>
      </header>

      <main class="main-287">
        <input
          class="add-input-287 ${error() ? "add-input--error-287" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-287" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-287">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-287">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-287 ${filter() === f ? "filter-btn--active-287" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-287">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-287">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-287">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-287 ${todo.done ? "todo-item--done-287" : ""}">
                    <button
                      class="todo-check-287 ${todo.done ? "todo-check--done-287" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-287" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-287"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-287" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-287" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-287">
            <span class="todo-count-287">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-287" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
