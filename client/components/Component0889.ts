// client/components/Component0889.ts
// Benchmark component #889 — identical logic to TodoPage, scoped class names.
...
import "./Component0889.css";

type Filter = "all" | "active" | "done";

export const Component0889 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0889] mounted");
    return () => console.log("[Component0889] unmounted");
  });
  ...
  return () => html`
    <div class="app-889">
      <header class="header-889">
        <h1>todos — Component0889</h1>
      </header>

      <main class="main-889">
        <input
          class="add-input-889 ${error() ? "add-input--error-889" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-889" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-889">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-889">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-889 ${filter() === f ? "filter-btn--active-889" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-889">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-889">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-889">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-889 ${todo.done ? "todo-item--done-889" : ""}">
                    <button
                      class="todo-check-889 ${todo.done ? "todo-check--done-889" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-889" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-889"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-889" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-889" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-889">
            <span class="todo-count-889">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-889" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
