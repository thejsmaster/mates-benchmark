// client/components/Component0868.ts
// Benchmark component #868 — identical logic to TodoPage, scoped class names.
...
import "./Component0868.css";

type Filter = "all" | "active" | "done";

export const Component0868 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0868] mounted");
    return () => console.log("[Component0868] unmounted");
  });
  ...
  return () => html`
    <div class="app-868">
      <header class="header-868">
        <h1>todos — Component0868</h1>
      </header>

      <main class="main-868">
        <input
          class="add-input-868 ${error() ? "add-input--error-868" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-868" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-868">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-868">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-868 ${filter() === f ? "filter-btn--active-868" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-868">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-868">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-868">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-868 ${todo.done ? "todo-item--done-868" : ""}">
                    <button
                      class="todo-check-868 ${todo.done ? "todo-check--done-868" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-868" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-868"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-868" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-868" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-868">
            <span class="todo-count-868">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-868" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
