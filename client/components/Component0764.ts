// client/components/Component0764.ts
// Benchmark component #764 — identical logic to TodoPage, scoped class names.
...
import "./Component0764.css";

type Filter = "all" | "active" | "done";

export const Component0764 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0764] mounted");
    return () => console.log("[Component0764] unmounted");
  });
  ...
  return () => html`
    <div class="app-764">
      <header class="header-764">
        <h1>todos — Component0764</h1>
      </header>

      <main class="main-764">
        <input
          class="add-input-764 ${error() ? "add-input--error-764" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-764" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-764">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-764">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-764 ${filter() === f ? "filter-btn--active-764" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-764">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-764">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-764">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-764 ${todo.done ? "todo-item--done-764" : ""}">
                    <button
                      class="todo-check-764 ${todo.done ? "todo-check--done-764" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-764" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-764"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-764" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-764" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-764">
            <span class="todo-count-764">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-764" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
