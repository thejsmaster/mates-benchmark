// client/components/Component0824.ts
// Benchmark component #824 — identical logic to TodoPage, scoped class names.
...
import "./Component0824.css";

type Filter = "all" | "active" | "done";

export const Component0824 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0824] mounted");
    return () => console.log("[Component0824] unmounted");
  });
  ...
  return () => html`
    <div class="app-824">
      <header class="header-824">
        <h1>todos — Component0824</h1>
      </header>

      <main class="main-824">
        <input
          class="add-input-824 ${error() ? "add-input--error-824" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-824" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-824">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-824">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-824 ${filter() === f ? "filter-btn--active-824" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-824">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-824">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-824">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-824 ${todo.done ? "todo-item--done-824" : ""}">
                    <button
                      class="todo-check-824 ${todo.done ? "todo-check--done-824" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-824" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-824"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-824" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-824" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-824">
            <span class="todo-count-824">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-824" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
