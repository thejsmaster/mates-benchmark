// client/components/Component0370.ts
// Benchmark component #370 — identical logic to TodoPage, scoped class names.
...
import "./Component0370.css";

type Filter = "all" | "active" | "done";

export const Component0370 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0370] mounted");
    return () => console.log("[Component0370] unmounted");
  });
  ...
  return () => html`
    <div class="app-370">
      <header class="header-370">
        <h1>todos — Component0370</h1>
      </header>

      <main class="main-370">
        <input
          class="add-input-370 ${error() ? "add-input--error-370" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-370" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-370">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-370">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-370 ${filter() === f ? "filter-btn--active-370" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-370">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-370">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-370">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-370 ${todo.done ? "todo-item--done-370" : ""}">
                    <button
                      class="todo-check-370 ${todo.done ? "todo-check--done-370" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-370" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-370"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-370" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-370" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-370">
            <span class="todo-count-370">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-370" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
