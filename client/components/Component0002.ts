// client/components/Component0002.ts
// Benchmark component #2 — identical logic to TodoPage, scoped class names.
...
import "./Component0002.css";

type Filter = "all" | "active" | "done";

export const Component0002 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0002] mounted");
    return () => console.log("[Component0002] unmounted");
  });
  ...
  return () => html`
    <div class="app-2">
      <header class="header-2">
        <h1>todos — Component0002</h1>
      </header>

      <main class="main-2">
        <input
          class="add-input-2 ${error() ? "add-input--error-2" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-2" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-2">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-2">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-2 ${filter() === f ? "filter-btn--active-2" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-2">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-2">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-2">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-2 ${todo.done ? "todo-item--done-2" : ""}">
                    <button
                      class="todo-check-2 ${todo.done ? "todo-check--done-2" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-2" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-2"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-2" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-2" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-2">
            <span class="todo-count-2">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-2" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
