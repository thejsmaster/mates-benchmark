// client/components/Component0245.ts
// Benchmark component #245 — identical logic to TodoPage, scoped class names.
...
import "./Component0245.css";

type Filter = "all" | "active" | "done";

export const Component0245 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0245] mounted");
    return () => console.log("[Component0245] unmounted");
  });
  ...
  return () => html`
    <div class="app-245">
      <header class="header-245">
        <h1>todos — Component0245</h1>
      </header>

      <main class="main-245">
        <input
          class="add-input-245 ${error() ? "add-input--error-245" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-245" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-245">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-245">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-245 ${filter() === f ? "filter-btn--active-245" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-245">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-245">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-245">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-245 ${todo.done ? "todo-item--done-245" : ""}">
                    <button
                      class="todo-check-245 ${todo.done ? "todo-check--done-245" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-245" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-245"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-245" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-245" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-245">
            <span class="todo-count-245">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-245" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
