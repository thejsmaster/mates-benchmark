// client/components/Component0196.ts
// Benchmark component #196 — identical logic to TodoPage, scoped class names.
...
import "./Component0196.css";

type Filter = "all" | "active" | "done";

export const Component0196 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0196] mounted");
    return () => console.log("[Component0196] unmounted");
  });
  ...
  return () => html`
    <div class="app-196">
      <header class="header-196">
        <h1>todos — Component0196</h1>
      </header>

      <main class="main-196">
        <input
          class="add-input-196 ${error() ? "add-input--error-196" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-196" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-196">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-196">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-196 ${filter() === f ? "filter-btn--active-196" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-196">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-196">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-196">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-196 ${todo.done ? "todo-item--done-196" : ""}">
                    <button
                      class="todo-check-196 ${todo.done ? "todo-check--done-196" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-196" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-196"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-196" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-196" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-196">
            <span class="todo-count-196">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-196" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
