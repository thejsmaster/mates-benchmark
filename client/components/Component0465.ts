// client/components/Component0465.ts
// Benchmark component #465 — identical logic to TodoPage, scoped class names.
...
import "./Component0465.css";

type Filter = "all" | "active" | "done";

export const Component0465 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0465] mounted");
    return () => console.log("[Component0465] unmounted");
  });
  ...
  return () => html`
    <div class="app-465">
      <header class="header-465">
        <h1>todos — Component0465</h1>
      </header>

      <main class="main-465">
        <input
          class="add-input-465 ${error() ? "add-input--error-465" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-465" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-465">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-465">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-465 ${filter() === f ? "filter-btn--active-465" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-465">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-465">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-465">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-465 ${todo.done ? "todo-item--done-465" : ""}">
                    <button
                      class="todo-check-465 ${todo.done ? "todo-check--done-465" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-465" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-465"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-465" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-465" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-465">
            <span class="todo-count-465">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-465" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
