// client/components/Component0464.ts
// Benchmark component #464 — identical logic to TodoPage, scoped class names.
...
import "./Component0464.css";

type Filter = "all" | "active" | "done";

export const Component0464 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0464] mounted");
    return () => console.log("[Component0464] unmounted");
  });
  ...
  return () => html`
    <div class="app-464">
      <header class="header-464">
        <h1>todos — Component0464</h1>
      </header>

      <main class="main-464">
        <input
          class="add-input-464 ${error() ? "add-input--error-464" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-464" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-464">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-464">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-464 ${filter() === f ? "filter-btn--active-464" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-464">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-464">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-464">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-464 ${todo.done ? "todo-item--done-464" : ""}">
                    <button
                      class="todo-check-464 ${todo.done ? "todo-check--done-464" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-464" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-464"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-464" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-464" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-464">
            <span class="todo-count-464">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-464" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
