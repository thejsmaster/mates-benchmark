// client/components/Component0820.ts
// Benchmark component #820 — identical logic to TodoPage, scoped class names.
...
import "./Component0820.css";

type Filter = "all" | "active" | "done";

export const Component0820 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0820] mounted");
    return () => console.log("[Component0820] unmounted");
  });
  ...
  return () => html`
    <div class="app-820">
      <header class="header-820">
        <h1>todos — Component0820</h1>
      </header>

      <main class="main-820">
        <input
          class="add-input-820 ${error() ? "add-input--error-820" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-820" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-820">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-820">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-820 ${filter() === f ? "filter-btn--active-820" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-820">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-820">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-820">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-820 ${todo.done ? "todo-item--done-820" : ""}">
                    <button
                      class="todo-check-820 ${todo.done ? "todo-check--done-820" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-820" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-820"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-820" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-820" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-820">
            <span class="todo-count-820">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-820" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
