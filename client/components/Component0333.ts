// client/components/Component0333.ts
// Benchmark component #333 — identical logic to TodoPage, scoped class names.
...
import "./Component0333.css";

type Filter = "all" | "active" | "done";

export const Component0333 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0333] mounted");
    return () => console.log("[Component0333] unmounted");
  });
  ...
  return () => html`
    <div class="app-333">
      <header class="header-333">
        <h1>todos — Component0333</h1>
      </header>

      <main class="main-333">
        <input
          class="add-input-333 ${error() ? "add-input--error-333" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-333" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-333">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-333">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-333 ${filter() === f ? "filter-btn--active-333" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-333">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-333">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-333">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-333 ${todo.done ? "todo-item--done-333" : ""}">
                    <button
                      class="todo-check-333 ${todo.done ? "todo-check--done-333" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-333" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-333"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-333" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-333" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-333">
            <span class="todo-count-333">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-333" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
