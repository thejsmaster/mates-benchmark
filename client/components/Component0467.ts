// client/components/Component0467.ts
// Benchmark component #467 — identical logic to TodoPage, scoped class names.
...
import "./Component0467.css";

type Filter = "all" | "active" | "done";

export const Component0467 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0467] mounted");
    return () => console.log("[Component0467] unmounted");
  });
  ...
  return () => html`
    <div class="app-467">
      <header class="header-467">
        <h1>todos — Component0467</h1>
      </header>

      <main class="main-467">
        <input
          class="add-input-467 ${error() ? "add-input--error-467" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-467" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-467">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-467">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-467 ${filter() === f ? "filter-btn--active-467" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-467">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-467">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-467">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-467 ${todo.done ? "todo-item--done-467" : ""}">
                    <button
                      class="todo-check-467 ${todo.done ? "todo-check--done-467" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-467" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-467"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-467" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-467" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-467">
            <span class="todo-count-467">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-467" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
