// client/components/Component0466.ts
// Benchmark component #466 — identical logic to TodoPage, scoped class names.
...
import "./Component0466.css";

type Filter = "all" | "active" | "done";

export const Component0466 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0466] mounted");
    return () => console.log("[Component0466] unmounted");
  });
  ...
  return () => html`
    <div class="app-466">
      <header class="header-466">
        <h1>todos — Component0466</h1>
      </header>

      <main class="main-466">
        <input
          class="add-input-466 ${error() ? "add-input--error-466" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-466" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-466">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-466">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-466 ${filter() === f ? "filter-btn--active-466" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-466">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-466">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-466">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-466 ${todo.done ? "todo-item--done-466" : ""}">
                    <button
                      class="todo-check-466 ${todo.done ? "todo-check--done-466" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-466" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-466"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-466" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-466" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-466">
            <span class="todo-count-466">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-466" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
