// client/components/Component0999.ts
// Benchmark component #999 — identical logic to TodoPage, scoped class names.
...
import "./Component0999.css";

type Filter = "all" | "active" | "done";

export const Component0999 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0999] mounted");
    return () => console.log("[Component0999] unmounted");
  });
  ...
  return () => html`
    <div class="app-999">
      <header class="header-999">
        <h1>todos — Component0999</h1>
      </header>

      <main class="main-999">
        <input
          class="add-input-999 ${error() ? "add-input--error-999" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-999" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-999">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-999">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-999 ${filter() === f ? "filter-btn--active-999" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-999">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-999">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-999">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-999 ${todo.done ? "todo-item--done-999" : ""}">
                    <button
                      class="todo-check-999 ${todo.done ? "todo-check--done-999" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-999" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-999"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-999" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-999" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-999">
            <span class="todo-count-999">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-999" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
