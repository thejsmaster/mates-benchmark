// client/components/Component0206.ts
// Benchmark component #206 — identical logic to TodoPage, scoped class names.
...
import "./Component0206.css";

type Filter = "all" | "active" | "done";

export const Component0206 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0206] mounted");
    return () => console.log("[Component0206] unmounted");
  });
  ...
  return () => html`
    <div class="app-206">
      <header class="header-206">
        <h1>todos — Component0206</h1>
      </header>

      <main class="main-206">
        <input
          class="add-input-206 ${error() ? "add-input--error-206" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-206" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-206">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-206">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-206 ${filter() === f ? "filter-btn--active-206" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-206">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-206">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-206">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-206 ${todo.done ? "todo-item--done-206" : ""}">
                    <button
                      class="todo-check-206 ${todo.done ? "todo-check--done-206" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-206" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-206"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-206" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-206" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-206">
            <span class="todo-count-206">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-206" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
