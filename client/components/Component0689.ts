// client/components/Component0689.ts
// Benchmark component #689 — identical logic to TodoPage, scoped class names.
...
import "./Component0689.css";

type Filter = "all" | "active" | "done";

export const Component0689 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0689] mounted");
    return () => console.log("[Component0689] unmounted");
  });
  ...
  return () => html`
    <div class="app-689">
      <header class="header-689">
        <h1>todos — Component0689</h1>
      </header>

      <main class="main-689">
        <input
          class="add-input-689 ${error() ? "add-input--error-689" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-689" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-689">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-689">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-689 ${filter() === f ? "filter-btn--active-689" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-689">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-689">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-689">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-689 ${todo.done ? "todo-item--done-689" : ""}">
                    <button
                      class="todo-check-689 ${todo.done ? "todo-check--done-689" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-689" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-689"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-689" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-689" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-689">
            <span class="todo-count-689">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-689" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
