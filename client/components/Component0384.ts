// client/components/Component0384.ts
// Benchmark component #384 — identical logic to TodoPage, scoped class names.
...
import "./Component0384.css";

type Filter = "all" | "active" | "done";

export const Component0384 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0384] mounted");
    return () => console.log("[Component0384] unmounted");
  });
  ...
  return () => html`
    <div class="app-384">
      <header class="header-384">
        <h1>todos — Component0384</h1>
      </header>

      <main class="main-384">
        <input
          class="add-input-384 ${error() ? "add-input--error-384" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-384" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-384">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-384">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-384 ${filter() === f ? "filter-btn--active-384" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-384">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-384">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-384">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-384 ${todo.done ? "todo-item--done-384" : ""}">
                    <button
                      class="todo-check-384 ${todo.done ? "todo-check--done-384" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-384" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-384"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-384" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-384" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-384">
            <span class="todo-count-384">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-384" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
