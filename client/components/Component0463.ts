// client/components/Component0463.ts
// Benchmark component #463 — identical logic to TodoPage, scoped class names.
...
import "./Component0463.css";

type Filter = "all" | "active" | "done";

export const Component0463 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0463] mounted");
    return () => console.log("[Component0463] unmounted");
  });
  ...
  return () => html`
    <div class="app-463">
      <header class="header-463">
        <h1>todos — Component0463</h1>
      </header>

      <main class="main-463">
        <input
          class="add-input-463 ${error() ? "add-input--error-463" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-463" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-463">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-463">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-463 ${filter() === f ? "filter-btn--active-463" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-463">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-463">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-463">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-463 ${todo.done ? "todo-item--done-463" : ""}">
                    <button
                      class="todo-check-463 ${todo.done ? "todo-check--done-463" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-463" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-463"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-463" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-463" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-463">
            <span class="todo-count-463">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-463" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
