// client/components/Component0264.ts
// Benchmark component #264 — identical logic to TodoPage, scoped class names.
...
import "./Component0264.css";

type Filter = "all" | "active" | "done";

export const Component0264 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0264] mounted");
    return () => console.log("[Component0264] unmounted");
  });
  ...
  return () => html`
    <div class="app-264">
      <header class="header-264">
        <h1>todos — Component0264</h1>
      </header>

      <main class="main-264">
        <input
          class="add-input-264 ${error() ? "add-input--error-264" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-264" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-264">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-264">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-264 ${filter() === f ? "filter-btn--active-264" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-264">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-264">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-264">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-264 ${todo.done ? "todo-item--done-264" : ""}">
                    <button
                      class="todo-check-264 ${todo.done ? "todo-check--done-264" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-264" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-264"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-264" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-264" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-264">
            <span class="todo-count-264">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-264" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
