// client/components/Component0793.ts
// Benchmark component #793 — identical logic to TodoPage, scoped class names.
...
import "./Component0793.css";

type Filter = "all" | "active" | "done";

export const Component0793 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0793] mounted");
    return () => console.log("[Component0793] unmounted");
  });
  ...
  return () => html`
    <div class="app-793">
      <header class="header-793">
        <h1>todos — Component0793</h1>
      </header>

      <main class="main-793">
        <input
          class="add-input-793 ${error() ? "add-input--error-793" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-793" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-793">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-793">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-793 ${filter() === f ? "filter-btn--active-793" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-793">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-793">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-793">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-793 ${todo.done ? "todo-item--done-793" : ""}">
                    <button
                      class="todo-check-793 ${todo.done ? "todo-check--done-793" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-793" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-793"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-793" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-793" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-793">
            <span class="todo-count-793">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-793" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
