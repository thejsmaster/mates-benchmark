// client/components/Component0988.ts
// Benchmark component #988 — identical logic to TodoPage, scoped class names.
...
import "./Component0988.css";

type Filter = "all" | "active" | "done";

export const Component0988 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0988] mounted");
    return () => console.log("[Component0988] unmounted");
  });
  ...
  return () => html`
    <div class="app-988">
      <header class="header-988">
        <h1>todos — Component0988</h1>
      </header>

      <main class="main-988">
        <input
          class="add-input-988 ${error() ? "add-input--error-988" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-988" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-988">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-988">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-988 ${filter() === f ? "filter-btn--active-988" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-988">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-988">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-988">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-988 ${todo.done ? "todo-item--done-988" : ""}">
                    <button
                      class="todo-check-988 ${todo.done ? "todo-check--done-988" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-988" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-988"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-988" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-988" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-988">
            <span class="todo-count-988">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-988" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
