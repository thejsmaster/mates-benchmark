// client/components/Component0701.ts
// Benchmark component #701 — identical logic to TodoPage, scoped class names.
...
import "./Component0701.css";

type Filter = "all" | "active" | "done";

export const Component0701 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0701] mounted");
    return () => console.log("[Component0701] unmounted");
  });
  ...
  return () => html`
    <div class="app-701">
      <header class="header-701">
        <h1>todos — Component0701</h1>
      </header>

      <main class="main-701">
        <input
          class="add-input-701 ${error() ? "add-input--error-701" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-701" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-701">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-701">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-701 ${filter() === f ? "filter-btn--active-701" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-701">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-701">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-701">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-701 ${todo.done ? "todo-item--done-701" : ""}">
                    <button
                      class="todo-check-701 ${todo.done ? "todo-check--done-701" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-701" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-701"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-701" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-701" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-701">
            <span class="todo-count-701">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-701" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
