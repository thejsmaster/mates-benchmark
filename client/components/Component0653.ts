// client/components/Component0653.ts
// Benchmark component #653 — identical logic to TodoPage, scoped class names.
...
import "./Component0653.css";

type Filter = "all" | "active" | "done";

export const Component0653 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0653] mounted");
    return () => console.log("[Component0653] unmounted");
  });
  ...
  return () => html`
    <div class="app-653">
      <header class="header-653">
        <h1>todos — Component0653</h1>
      </header>

      <main class="main-653">
        <input
          class="add-input-653 ${error() ? "add-input--error-653" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-653" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-653">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-653">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-653 ${filter() === f ? "filter-btn--active-653" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-653">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-653">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-653">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-653 ${todo.done ? "todo-item--done-653" : ""}">
                    <button
                      class="todo-check-653 ${todo.done ? "todo-check--done-653" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-653" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-653"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-653" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-653" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-653">
            <span class="todo-count-653">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-653" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
