// client/components/Component0112.ts
// Benchmark component #112 — identical logic to TodoPage, scoped class names.
...
import "./Component0112.css";

type Filter = "all" | "active" | "done";

export const Component0112 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0112] mounted");
    return () => console.log("[Component0112] unmounted");
  });
  ...
  return () => html`
    <div class="app-112">
      <header class="header-112">
        <h1>todos — Component0112</h1>
      </header>

      <main class="main-112">
        <input
          class="add-input-112 ${error() ? "add-input--error-112" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-112" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-112">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-112">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-112 ${filter() === f ? "filter-btn--active-112" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-112">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-112">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-112">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-112 ${todo.done ? "todo-item--done-112" : ""}">
                    <button
                      class="todo-check-112 ${todo.done ? "todo-check--done-112" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-112" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-112"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-112" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-112" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-112">
            <span class="todo-count-112">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-112" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
