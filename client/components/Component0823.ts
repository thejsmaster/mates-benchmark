// client/components/Component0823.ts
// Benchmark component #823 — identical logic to TodoPage, scoped class names.
...
import "./Component0823.css";

type Filter = "all" | "active" | "done";

export const Component0823 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0823] mounted");
    return () => console.log("[Component0823] unmounted");
  });
  ...
  return () => html`
    <div class="app-823">
      <header class="header-823">
        <h1>todos — Component0823</h1>
      </header>

      <main class="main-823">
        <input
          class="add-input-823 ${error() ? "add-input--error-823" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-823" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-823">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-823">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-823 ${filter() === f ? "filter-btn--active-823" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-823">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-823">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-823">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-823 ${todo.done ? "todo-item--done-823" : ""}">
                    <button
                      class="todo-check-823 ${todo.done ? "todo-check--done-823" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-823" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-823"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-823" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-823" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-823">
            <span class="todo-count-823">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-823" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
