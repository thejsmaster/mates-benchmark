// client/components/Component0872.ts
// Benchmark component #872 — identical logic to TodoPage, scoped class names.
...
import "./Component0872.css";

type Filter = "all" | "active" | "done";

export const Component0872 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0872] mounted");
    return () => console.log("[Component0872] unmounted");
  });
  ...
  return () => html`
    <div class="app-872">
      <header class="header-872">
        <h1>todos — Component0872</h1>
      </header>

      <main class="main-872">
        <input
          class="add-input-872 ${error() ? "add-input--error-872" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-872" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-872">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-872">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-872 ${filter() === f ? "filter-btn--active-872" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-872">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-872">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-872">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-872 ${todo.done ? "todo-item--done-872" : ""}">
                    <button
                      class="todo-check-872 ${todo.done ? "todo-check--done-872" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-872" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-872"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-872" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-872" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-872">
            <span class="todo-count-872">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-872" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
