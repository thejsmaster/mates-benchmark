// client/components/Component0364.ts
// Benchmark component #364 — identical logic to TodoPage, scoped class names.
...
import "./Component0364.css";

type Filter = "all" | "active" | "done";

export const Component0364 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0364] mounted");
    return () => console.log("[Component0364] unmounted");
  });
  ...
  return () => html`
    <div class="app-364">
      <header class="header-364">
        <h1>todos — Component0364</h1>
      </header>

      <main class="main-364">
        <input
          class="add-input-364 ${error() ? "add-input--error-364" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-364" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-364">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-364">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-364 ${filter() === f ? "filter-btn--active-364" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-364">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-364">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-364">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-364 ${todo.done ? "todo-item--done-364" : ""}">
                    <button
                      class="todo-check-364 ${todo.done ? "todo-check--done-364" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-364" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-364"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-364" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-364" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-364">
            <span class="todo-count-364">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-364" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
