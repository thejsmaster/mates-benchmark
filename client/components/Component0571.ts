// client/components/Component0571.ts
// Benchmark component #571 — identical logic to TodoPage, scoped class names.
...
import "./Component0571.css";

type Filter = "all" | "active" | "done";

export const Component0571 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0571] mounted");
    return () => console.log("[Component0571] unmounted");
  });
  ...
  return () => html`
    <div class="app-571">
      <header class="header-571">
        <h1>todos — Component0571</h1>
      </header>

      <main class="main-571">
        <input
          class="add-input-571 ${error() ? "add-input--error-571" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-571" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-571">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-571">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-571 ${filter() === f ? "filter-btn--active-571" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-571">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-571">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-571">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-571 ${todo.done ? "todo-item--done-571" : ""}">
                    <button
                      class="todo-check-571 ${todo.done ? "todo-check--done-571" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-571" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-571"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-571" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-571" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-571">
            <span class="todo-count-571">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-571" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
