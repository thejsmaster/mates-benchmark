// client/components/Component0725.ts
// Benchmark component #725 — identical logic to TodoPage, scoped class names.
...
import "./Component0725.css";

type Filter = "all" | "active" | "done";

export const Component0725 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0725] mounted");
    return () => console.log("[Component0725] unmounted");
  });
  ...
  return () => html`
    <div class="app-725">
      <header class="header-725">
        <h1>todos — Component0725</h1>
      </header>

      <main class="main-725">
        <input
          class="add-input-725 ${error() ? "add-input--error-725" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-725" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-725">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-725">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-725 ${filter() === f ? "filter-btn--active-725" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-725">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-725">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-725">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-725 ${todo.done ? "todo-item--done-725" : ""}">
                    <button
                      class="todo-check-725 ${todo.done ? "todo-check--done-725" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-725" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-725"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-725" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-725" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-725">
            <span class="todo-count-725">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-725" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
