// client/components/Component0406.ts
// Benchmark component #406 — identical logic to TodoPage, scoped class names.
...
import "./Component0406.css";

type Filter = "all" | "active" | "done";

export const Component0406 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0406] mounted");
    return () => console.log("[Component0406] unmounted");
  });
  ...
  return () => html`
    <div class="app-406">
      <header class="header-406">
        <h1>todos — Component0406</h1>
      </header>

      <main class="main-406">
        <input
          class="add-input-406 ${error() ? "add-input--error-406" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-406" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-406">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-406">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-406 ${filter() === f ? "filter-btn--active-406" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-406">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-406">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-406">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-406 ${todo.done ? "todo-item--done-406" : ""}">
                    <button
                      class="todo-check-406 ${todo.done ? "todo-check--done-406" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-406" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-406"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-406" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-406" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-406">
            <span class="todo-count-406">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-406" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
