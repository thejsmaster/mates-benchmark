// client/components/Component0064.ts
// Benchmark component #64 — identical logic to TodoPage, scoped class names.
...
import "./Component0064.css";

type Filter = "all" | "active" | "done";

export const Component0064 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0064] mounted");
    return () => console.log("[Component0064] unmounted");
  });
  ...
  return () => html`
    <div class="app-64">
      <header class="header-64">
        <h1>todos — Component0064</h1>
      </header>

      <main class="main-64">
        <input
          class="add-input-64 ${error() ? "add-input--error-64" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-64" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-64">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-64">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-64 ${filter() === f ? "filter-btn--active-64" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-64">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-64">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-64">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-64 ${todo.done ? "todo-item--done-64" : ""}">
                    <button
                      class="todo-check-64 ${todo.done ? "todo-check--done-64" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-64" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-64"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-64" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-64" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-64">
            <span class="todo-count-64">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-64" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
