// client/components/Component0392.ts
// Benchmark component #392 — identical logic to TodoPage, scoped class names.
...
import "./Component0392.css";

type Filter = "all" | "active" | "done";

export const Component0392 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0392] mounted");
    return () => console.log("[Component0392] unmounted");
  });
  ...
  return () => html`
    <div class="app-392">
      <header class="header-392">
        <h1>todos — Component0392</h1>
      </header>

      <main class="main-392">
        <input
          class="add-input-392 ${error() ? "add-input--error-392" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-392" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-392">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-392">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-392 ${filter() === f ? "filter-btn--active-392" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-392">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-392">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-392">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-392 ${todo.done ? "todo-item--done-392" : ""}">
                    <button
                      class="todo-check-392 ${todo.done ? "todo-check--done-392" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-392" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-392"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-392" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-392" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-392">
            <span class="todo-count-392">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-392" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
