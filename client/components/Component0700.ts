// client/components/Component0700.ts
// Benchmark component #700 — identical logic to TodoPage, scoped class names.
...
import "./Component0700.css";

type Filter = "all" | "active" | "done";

export const Component0700 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0700] mounted");
    return () => console.log("[Component0700] unmounted");
  });
  ...
  return () => html`
    <div class="app-700">
      <header class="header-700">
        <h1>todos — Component0700</h1>
      </header>

      <main class="main-700">
        <input
          class="add-input-700 ${error() ? "add-input--error-700" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-700" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-700">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-700">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-700 ${filter() === f ? "filter-btn--active-700" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-700">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-700">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-700">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-700 ${todo.done ? "todo-item--done-700" : ""}">
                    <button
                      class="todo-check-700 ${todo.done ? "todo-check--done-700" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-700" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-700"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-700" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-700" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-700">
            <span class="todo-count-700">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-700" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
