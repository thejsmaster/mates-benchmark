// client/components/Component0542.ts
// Benchmark component #542 — identical logic to TodoPage, scoped class names.
...
import "./Component0542.css";

type Filter = "all" | "active" | "done";

export const Component0542 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0542] mounted");
    return () => console.log("[Component0542] unmounted");
  });
  ...
  return () => html`
    <div class="app-542">
      <header class="header-542">
        <h1>todos — Component0542</h1>
      </header>

      <main class="main-542">
        <input
          class="add-input-542 ${error() ? "add-input--error-542" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-542" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-542">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-542">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-542 ${filter() === f ? "filter-btn--active-542" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-542">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-542">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-542">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-542 ${todo.done ? "todo-item--done-542" : ""}">
                    <button
                      class="todo-check-542 ${todo.done ? "todo-check--done-542" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-542" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-542"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-542" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-542" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-542">
            <span class="todo-count-542">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-542" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
