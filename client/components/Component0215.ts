// client/components/Component0215.ts
// Benchmark component #215 — identical logic to TodoPage, scoped class names.
...
import "./Component0215.css";

type Filter = "all" | "active" | "done";

export const Component0215 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0215] mounted");
    return () => console.log("[Component0215] unmounted");
  });
  ...
  return () => html`
    <div class="app-215">
      <header class="header-215">
        <h1>todos — Component0215</h1>
      </header>

      <main class="main-215">
        <input
          class="add-input-215 ${error() ? "add-input--error-215" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-215" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-215">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-215">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-215 ${filter() === f ? "filter-btn--active-215" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-215">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-215">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-215">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-215 ${todo.done ? "todo-item--done-215" : ""}">
                    <button
                      class="todo-check-215 ${todo.done ? "todo-check--done-215" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-215" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-215"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-215" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-215" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-215">
            <span class="todo-count-215">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-215" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
