// client/components/Component0419.ts
// Benchmark component #419 — identical logic to TodoPage, scoped class names.
...
import "./Component0419.css";

type Filter = "all" | "active" | "done";

export const Component0419 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0419] mounted");
    return () => console.log("[Component0419] unmounted");
  });
  ...
  return () => html`
    <div class="app-419">
      <header class="header-419">
        <h1>todos — Component0419</h1>
      </header>

      <main class="main-419">
        <input
          class="add-input-419 ${error() ? "add-input--error-419" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-419" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-419">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-419">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-419 ${filter() === f ? "filter-btn--active-419" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-419">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-419">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-419">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-419 ${todo.done ? "todo-item--done-419" : ""}">
                    <button
                      class="todo-check-419 ${todo.done ? "todo-check--done-419" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-419" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-419"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-419" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-419" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-419">
            <span class="todo-count-419">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-419" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
