// client/components/Component0415.ts
// Benchmark component #415 — identical logic to TodoPage, scoped class names.
...
import "./Component0415.css";

type Filter = "all" | "active" | "done";

export const Component0415 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0415] mounted");
    return () => console.log("[Component0415] unmounted");
  });
  ...
  return () => html`
    <div class="app-415">
      <header class="header-415">
        <h1>todos — Component0415</h1>
      </header>

      <main class="main-415">
        <input
          class="add-input-415 ${error() ? "add-input--error-415" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-415" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-415">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-415">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-415 ${filter() === f ? "filter-btn--active-415" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-415">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-415">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-415">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-415 ${todo.done ? "todo-item--done-415" : ""}">
                    <button
                      class="todo-check-415 ${todo.done ? "todo-check--done-415" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-415" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-415"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-415" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-415" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-415">
            <span class="todo-count-415">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-415" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
