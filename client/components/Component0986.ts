// client/components/Component0986.ts
// Benchmark component #986 — identical logic to TodoPage, scoped class names.
...
import "./Component0986.css";

type Filter = "all" | "active" | "done";

export const Component0986 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0986] mounted");
    return () => console.log("[Component0986] unmounted");
  });
  ...
  return () => html`
    <div class="app-986">
      <header class="header-986">
        <h1>todos — Component0986</h1>
      </header>

      <main class="main-986">
        <input
          class="add-input-986 ${error() ? "add-input--error-986" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-986" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-986">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-986">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-986 ${filter() === f ? "filter-btn--active-986" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-986">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-986">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-986">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-986 ${todo.done ? "todo-item--done-986" : ""}">
                    <button
                      class="todo-check-986 ${todo.done ? "todo-check--done-986" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-986" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-986"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-986" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-986" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-986">
            <span class="todo-count-986">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-986" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
