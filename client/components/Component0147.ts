// client/components/Component0147.ts
// Benchmark component #147 — identical logic to TodoPage, scoped class names.
...
import "./Component0147.css";

type Filter = "all" | "active" | "done";

export const Component0147 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0147] mounted");
    return () => console.log("[Component0147] unmounted");
  });
  ...
  return () => html`
    <div class="app-147">
      <header class="header-147">
        <h1>todos — Component0147</h1>
      </header>

      <main class="main-147">
        <input
          class="add-input-147 ${error() ? "add-input--error-147" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-147" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-147">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-147">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-147 ${filter() === f ? "filter-btn--active-147" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-147">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-147">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-147">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-147 ${todo.done ? "todo-item--done-147" : ""}">
                    <button
                      class="todo-check-147 ${todo.done ? "todo-check--done-147" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-147" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-147"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-147" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-147" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-147">
            <span class="todo-count-147">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-147" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
