// client/components/Component0963.ts
// Benchmark component #963 — identical logic to TodoPage, scoped class names.
...
import "./Component0963.css";

type Filter = "all" | "active" | "done";

export const Component0963 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0963] mounted");
    return () => console.log("[Component0963] unmounted");
  });
  ...
  return () => html`
    <div class="app-963">
      <header class="header-963">
        <h1>todos — Component0963</h1>
      </header>

      <main class="main-963">
        <input
          class="add-input-963 ${error() ? "add-input--error-963" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-963" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-963">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-963">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-963 ${filter() === f ? "filter-btn--active-963" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-963">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-963">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-963">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-963 ${todo.done ? "todo-item--done-963" : ""}">
                    <button
                      class="todo-check-963 ${todo.done ? "todo-check--done-963" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-963" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-963"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-963" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-963" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-963">
            <span class="todo-count-963">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-963" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
