// client/components/Component0684.ts
// Benchmark component #684 — identical logic to TodoPage, scoped class names.
...
import "./Component0684.css";

type Filter = "all" | "active" | "done";

export const Component0684 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0684] mounted");
    return () => console.log("[Component0684] unmounted");
  });
  ...
  return () => html`
    <div class="app-684">
      <header class="header-684">
        <h1>todos — Component0684</h1>
      </header>

      <main class="main-684">
        <input
          class="add-input-684 ${error() ? "add-input--error-684" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-684" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-684">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-684">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-684 ${filter() === f ? "filter-btn--active-684" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-684">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-684">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-684">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-684 ${todo.done ? "todo-item--done-684" : ""}">
                    <button
                      class="todo-check-684 ${todo.done ? "todo-check--done-684" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-684" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-684"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-684" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-684" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-684">
            <span class="todo-count-684">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-684" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
