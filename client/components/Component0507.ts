// client/components/Component0507.ts
// Benchmark component #507 — identical logic to TodoPage, scoped class names.
...
import "./Component0507.css";

type Filter = "all" | "active" | "done";

export const Component0507 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0507] mounted");
    return () => console.log("[Component0507] unmounted");
  });
  ...
  return () => html`
    <div class="app-507">
      <header class="header-507">
        <h1>todos — Component0507</h1>
      </header>

      <main class="main-507">
        <input
          class="add-input-507 ${error() ? "add-input--error-507" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-507" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-507">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-507">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-507 ${filter() === f ? "filter-btn--active-507" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-507">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-507">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-507">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-507 ${todo.done ? "todo-item--done-507" : ""}">
                    <button
                      class="todo-check-507 ${todo.done ? "todo-check--done-507" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-507" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-507"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-507" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-507" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-507">
            <span class="todo-count-507">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-507" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
