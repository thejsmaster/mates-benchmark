// client/components/Component0129.ts
// Benchmark component #129 — identical logic to TodoPage, scoped class names.
...
import "./Component0129.css";

type Filter = "all" | "active" | "done";

export const Component0129 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0129] mounted");
    return () => console.log("[Component0129] unmounted");
  });
  ...
  return () => html`
    <div class="app-129">
      <header class="header-129">
        <h1>todos — Component0129</h1>
      </header>

      <main class="main-129">
        <input
          class="add-input-129 ${error() ? "add-input--error-129" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-129" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-129">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-129">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-129 ${filter() === f ? "filter-btn--active-129" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-129">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-129">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-129">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-129 ${todo.done ? "todo-item--done-129" : ""}">
                    <button
                      class="todo-check-129 ${todo.done ? "todo-check--done-129" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-129" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-129"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-129" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-129" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-129">
            <span class="todo-count-129">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-129" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
