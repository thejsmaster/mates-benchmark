// client/components/Component0814.ts
// Benchmark component #814 — identical logic to TodoPage, scoped class names.
...
import "./Component0814.css";

type Filter = "all" | "active" | "done";

export const Component0814 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0814] mounted");
    return () => console.log("[Component0814] unmounted");
  });
  ...
  return () => html`
    <div class="app-814">
      <header class="header-814">
        <h1>todos — Component0814</h1>
      </header>

      <main class="main-814">
        <input
          class="add-input-814 ${error() ? "add-input--error-814" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-814" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-814">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-814">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-814 ${filter() === f ? "filter-btn--active-814" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-814">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-814">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-814">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-814 ${todo.done ? "todo-item--done-814" : ""}">
                    <button
                      class="todo-check-814 ${todo.done ? "todo-check--done-814" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-814" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-814"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-814" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-814" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-814">
            <span class="todo-count-814">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-814" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
