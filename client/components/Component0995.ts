// client/components/Component0995.ts
// Benchmark component #995 — identical logic to TodoPage, scoped class names.
...
import "./Component0995.css";

type Filter = "all" | "active" | "done";

export const Component0995 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0995] mounted");
    return () => console.log("[Component0995] unmounted");
  });
  ...
  return () => html`
    <div class="app-995">
      <header class="header-995">
        <h1>todos — Component0995</h1>
      </header>

      <main class="main-995">
        <input
          class="add-input-995 ${error() ? "add-input--error-995" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-995" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-995">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-995">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-995 ${filter() === f ? "filter-btn--active-995" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-995">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-995">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-995">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-995 ${todo.done ? "todo-item--done-995" : ""}">
                    <button
                      class="todo-check-995 ${todo.done ? "todo-check--done-995" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-995" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-995"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-995" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-995" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-995">
            <span class="todo-count-995">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-995" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
