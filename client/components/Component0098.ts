// client/components/Component0098.ts
// Benchmark component #98 — identical logic to TodoPage, scoped class names.
...
import "./Component0098.css";

type Filter = "all" | "active" | "done";

export const Component0098 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0098] mounted");
    return () => console.log("[Component0098] unmounted");
  });
  ...
  return () => html`
    <div class="app-98">
      <header class="header-98">
        <h1>todos — Component0098</h1>
      </header>

      <main class="main-98">
        <input
          class="add-input-98 ${error() ? "add-input--error-98" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-98" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-98">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-98">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-98 ${filter() === f ? "filter-btn--active-98" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-98">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-98">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-98">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-98 ${todo.done ? "todo-item--done-98" : ""}">
                    <button
                      class="todo-check-98 ${todo.done ? "todo-check--done-98" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-98" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-98"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-98" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-98" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-98">
            <span class="todo-count-98">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-98" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
