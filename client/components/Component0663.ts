// client/components/Component0663.ts
// Benchmark component #663 — identical logic to TodoPage, scoped class names.
...
import "./Component0663.css";

type Filter = "all" | "active" | "done";

export const Component0663 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0663] mounted");
    return () => console.log("[Component0663] unmounted");
  });
  ...
  return () => html`
    <div class="app-663">
      <header class="header-663">
        <h1>todos — Component0663</h1>
      </header>

      <main class="main-663">
        <input
          class="add-input-663 ${error() ? "add-input--error-663" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-663" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-663">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-663">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-663 ${filter() === f ? "filter-btn--active-663" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-663">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-663">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-663">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-663 ${todo.done ? "todo-item--done-663" : ""}">
                    <button
                      class="todo-check-663 ${todo.done ? "todo-check--done-663" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-663" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-663"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-663" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-663" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-663">
            <span class="todo-count-663">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-663" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
