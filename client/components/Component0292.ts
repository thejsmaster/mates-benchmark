// client/components/Component0292.ts
// Benchmark component #292 — identical logic to TodoPage, scoped class names.
...
import "./Component0292.css";

type Filter = "all" | "active" | "done";

export const Component0292 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0292] mounted");
    return () => console.log("[Component0292] unmounted");
  });
  ...
  return () => html`
    <div class="app-292">
      <header class="header-292">
        <h1>todos — Component0292</h1>
      </header>

      <main class="main-292">
        <input
          class="add-input-292 ${error() ? "add-input--error-292" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-292" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-292">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-292">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-292 ${filter() === f ? "filter-btn--active-292" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-292">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-292">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-292">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-292 ${todo.done ? "todo-item--done-292" : ""}">
                    <button
                      class="todo-check-292 ${todo.done ? "todo-check--done-292" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-292" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-292"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-292" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-292" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-292">
            <span class="todo-count-292">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-292" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
