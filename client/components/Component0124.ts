// client/components/Component0124.ts
// Benchmark component #124 — identical logic to TodoPage, scoped class names.
...
import "./Component0124.css";

type Filter = "all" | "active" | "done";

export const Component0124 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0124] mounted");
    return () => console.log("[Component0124] unmounted");
  });
  ...
  return () => html`
    <div class="app-124">
      <header class="header-124">
        <h1>todos — Component0124</h1>
      </header>

      <main class="main-124">
        <input
          class="add-input-124 ${error() ? "add-input--error-124" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-124" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-124">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-124">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-124 ${filter() === f ? "filter-btn--active-124" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-124">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-124">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-124">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-124 ${todo.done ? "todo-item--done-124" : ""}">
                    <button
                      class="todo-check-124 ${todo.done ? "todo-check--done-124" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-124" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-124"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-124" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-124" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-124">
            <span class="todo-count-124">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-124" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
