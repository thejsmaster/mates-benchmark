// client/components/Component0500.ts
// Benchmark component #500 — identical logic to TodoPage, scoped class names.
...
import "./Component0500.css";

type Filter = "all" | "active" | "done";

export const Component0500 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0500] mounted");
    return () => console.log("[Component0500] unmounted");
  });
  ...
  return () => html`
    <div class="app-500">
      <header class="header-500">
        <h1>todos — Component0500</h1>
      </header>

      <main class="main-500">
        <input
          class="add-input-500 ${error() ? "add-input--error-500" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-500" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-500">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-500">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-500 ${filter() === f ? "filter-btn--active-500" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-500">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-500">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-500">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-500 ${todo.done ? "todo-item--done-500" : ""}">
                    <button
                      class="todo-check-500 ${todo.done ? "todo-check--done-500" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-500" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-500"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-500" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-500" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-500">
            <span class="todo-count-500">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-500" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
