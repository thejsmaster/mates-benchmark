// client/components/Component0787.ts
// Benchmark component #787 — identical logic to TodoPage, scoped class names.
...
import "./Component0787.css";

type Filter = "all" | "active" | "done";

export const Component0787 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0787] mounted");
    return () => console.log("[Component0787] unmounted");
  });
  ...
  return () => html`
    <div class="app-787">
      <header class="header-787">
        <h1>todos — Component0787</h1>
      </header>

      <main class="main-787">
        <input
          class="add-input-787 ${error() ? "add-input--error-787" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-787" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-787">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-787">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-787 ${filter() === f ? "filter-btn--active-787" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-787">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-787">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-787">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-787 ${todo.done ? "todo-item--done-787" : ""}">
                    <button
                      class="todo-check-787 ${todo.done ? "todo-check--done-787" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-787" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-787"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-787" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-787" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-787">
            <span class="todo-count-787">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-787" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
