// client/components/Component0430.ts
// Benchmark component #430 — identical logic to TodoPage, scoped class names.
...
import "./Component0430.css";

type Filter = "all" | "active" | "done";

export const Component0430 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0430] mounted");
    return () => console.log("[Component0430] unmounted");
  });
  ...
  return () => html`
    <div class="app-430">
      <header class="header-430">
        <h1>todos — Component0430</h1>
      </header>

      <main class="main-430">
        <input
          class="add-input-430 ${error() ? "add-input--error-430" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-430" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-430">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-430">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-430 ${filter() === f ? "filter-btn--active-430" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-430">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-430">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-430">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-430 ${todo.done ? "todo-item--done-430" : ""}">
                    <button
                      class="todo-check-430 ${todo.done ? "todo-check--done-430" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-430" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-430"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-430" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-430" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-430">
            <span class="todo-count-430">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-430" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
