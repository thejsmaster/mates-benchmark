// client/components/Component0423.ts
// Benchmark component #423 — identical logic to TodoPage, scoped class names.
...
import "./Component0423.css";

type Filter = "all" | "active" | "done";

export const Component0423 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0423] mounted");
    return () => console.log("[Component0423] unmounted");
  });
  ...
  return () => html`
    <div class="app-423">
      <header class="header-423">
        <h1>todos — Component0423</h1>
      </header>

      <main class="main-423">
        <input
          class="add-input-423 ${error() ? "add-input--error-423" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-423" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-423">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-423">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-423 ${filter() === f ? "filter-btn--active-423" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-423">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-423">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-423">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-423 ${todo.done ? "todo-item--done-423" : ""}">
                    <button
                      class="todo-check-423 ${todo.done ? "todo-check--done-423" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-423" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-423"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-423" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-423" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-423">
            <span class="todo-count-423">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-423" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
