// client/components/Component0310.ts
// Benchmark component #310 — identical logic to TodoPage, scoped class names.
...
import "./Component0310.css";

type Filter = "all" | "active" | "done";

export const Component0310 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0310] mounted");
    return () => console.log("[Component0310] unmounted");
  });
  ...
  return () => html`
    <div class="app-310">
      <header class="header-310">
        <h1>todos — Component0310</h1>
      </header>

      <main class="main-310">
        <input
          class="add-input-310 ${error() ? "add-input--error-310" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-310" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-310">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-310">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-310 ${filter() === f ? "filter-btn--active-310" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-310">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-310">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-310">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-310 ${todo.done ? "todo-item--done-310" : ""}">
                    <button
                      class="todo-check-310 ${todo.done ? "todo-check--done-310" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-310" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-310"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-310" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-310" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-310">
            <span class="todo-count-310">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-310" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
