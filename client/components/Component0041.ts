// client/components/Component0041.ts
// Benchmark component #41 — identical logic to TodoPage, scoped class names.
...
import "./Component0041.css";

type Filter = "all" | "active" | "done";

export const Component0041 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0041] mounted");
    return () => console.log("[Component0041] unmounted");
  });
  ...
  return () => html`
    <div class="app-41">
      <header class="header-41">
        <h1>todos — Component0041</h1>
      </header>

      <main class="main-41">
        <input
          class="add-input-41 ${error() ? "add-input--error-41" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-41" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-41">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-41">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-41 ${filter() === f ? "filter-btn--active-41" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-41">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-41">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-41">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-41 ${todo.done ? "todo-item--done-41" : ""}">
                    <button
                      class="todo-check-41 ${todo.done ? "todo-check--done-41" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-41" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-41"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-41" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-41" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-41">
            <span class="todo-count-41">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-41" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
