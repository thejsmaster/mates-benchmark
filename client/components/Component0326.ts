// client/components/Component0326.ts
// Benchmark component #326 — identical logic to TodoPage, scoped class names.
...
import "./Component0326.css";

type Filter = "all" | "active" | "done";

export const Component0326 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0326] mounted");
    return () => console.log("[Component0326] unmounted");
  });
  ...
  return () => html`
    <div class="app-326">
      <header class="header-326">
        <h1>todos — Component0326</h1>
      </header>

      <main class="main-326">
        <input
          class="add-input-326 ${error() ? "add-input--error-326" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-326" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-326">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-326">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-326 ${filter() === f ? "filter-btn--active-326" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-326">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-326">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-326">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-326 ${todo.done ? "todo-item--done-326" : ""}">
                    <button
                      class="todo-check-326 ${todo.done ? "todo-check--done-326" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-326" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-326"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-326" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-326" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-326">
            <span class="todo-count-326">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-326" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
