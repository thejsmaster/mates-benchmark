// client/components/Component0554.ts
// Benchmark component #554 — identical logic to TodoPage, scoped class names.
...
import "./Component0554.css";

type Filter = "all" | "active" | "done";

export const Component0554 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0554] mounted");
    return () => console.log("[Component0554] unmounted");
  });
  ...
  return () => html`
    <div class="app-554">
      <header class="header-554">
        <h1>todos — Component0554</h1>
      </header>

      <main class="main-554">
        <input
          class="add-input-554 ${error() ? "add-input--error-554" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-554" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-554">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-554">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-554 ${filter() === f ? "filter-btn--active-554" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-554">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-554">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-554">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-554 ${todo.done ? "todo-item--done-554" : ""}">
                    <button
                      class="todo-check-554 ${todo.done ? "todo-check--done-554" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-554" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-554"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-554" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-554" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-554">
            <span class="todo-count-554">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-554" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
