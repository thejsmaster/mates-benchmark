// client/components/Component0128.ts
// Benchmark component #128 — identical logic to TodoPage, scoped class names.
...
import "./Component0128.css";

type Filter = "all" | "active" | "done";

export const Component0128 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0128] mounted");
    return () => console.log("[Component0128] unmounted");
  });
  ...
  return () => html`
    <div class="app-128">
      <header class="header-128">
        <h1>todos — Component0128</h1>
      </header>

      <main class="main-128">
        <input
          class="add-input-128 ${error() ? "add-input--error-128" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-128" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-128">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-128">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-128 ${filter() === f ? "filter-btn--active-128" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-128">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-128">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-128">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-128 ${todo.done ? "todo-item--done-128" : ""}">
                    <button
                      class="todo-check-128 ${todo.done ? "todo-check--done-128" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-128" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-128"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-128" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-128" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-128">
            <span class="todo-count-128">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-128" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
