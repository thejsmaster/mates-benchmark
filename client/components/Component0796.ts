// client/components/Component0796.ts
// Benchmark component #796 — identical logic to TodoPage, scoped class names.
...
import "./Component0796.css";

type Filter = "all" | "active" | "done";

export const Component0796 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0796] mounted");
    return () => console.log("[Component0796] unmounted");
  });
  ...
  return () => html`
    <div class="app-796">
      <header class="header-796">
        <h1>todos — Component0796</h1>
      </header>

      <main class="main-796">
        <input
          class="add-input-796 ${error() ? "add-input--error-796" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-796" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-796">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-796">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-796 ${filter() === f ? "filter-btn--active-796" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-796">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-796">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-796">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-796 ${todo.done ? "todo-item--done-796" : ""}">
                    <button
                      class="todo-check-796 ${todo.done ? "todo-check--done-796" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-796" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-796"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-796" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-796" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-796">
            <span class="todo-count-796">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-796" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
