// client/components/Component0613.ts
// Benchmark component #613 — identical logic to TodoPage, scoped class names.
...
import "./Component0613.css";

type Filter = "all" | "active" | "done";

export const Component0613 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0613] mounted");
    return () => console.log("[Component0613] unmounted");
  });
  ...
  return () => html`
    <div class="app-613">
      <header class="header-613">
        <h1>todos — Component0613</h1>
      </header>

      <main class="main-613">
        <input
          class="add-input-613 ${error() ? "add-input--error-613" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-613" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-613">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-613">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-613 ${filter() === f ? "filter-btn--active-613" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-613">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-613">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-613">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-613 ${todo.done ? "todo-item--done-613" : ""}">
                    <button
                      class="todo-check-613 ${todo.done ? "todo-check--done-613" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-613" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-613"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-613" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-613" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-613">
            <span class="todo-count-613">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-613" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
