// client/components/Component0049.ts
// Benchmark component #49 — identical logic to TodoPage, scoped class names.
...
import "./Component0049.css";

type Filter = "all" | "active" | "done";

export const Component0049 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0049] mounted");
    return () => console.log("[Component0049] unmounted");
  });
  ...
  return () => html`
    <div class="app-49">
      <header class="header-49">
        <h1>todos — Component0049</h1>
      </header>

      <main class="main-49">
        <input
          class="add-input-49 ${error() ? "add-input--error-49" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-49" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-49">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-49">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-49 ${filter() === f ? "filter-btn--active-49" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-49">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-49">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-49">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-49 ${todo.done ? "todo-item--done-49" : ""}">
                    <button
                      class="todo-check-49 ${todo.done ? "todo-check--done-49" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-49" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-49"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-49" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-49" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-49">
            <span class="todo-count-49">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-49" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
