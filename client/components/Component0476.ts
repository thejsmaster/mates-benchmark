// client/components/Component0476.ts
// Benchmark component #476 — identical logic to TodoPage, scoped class names.
...
import "./Component0476.css";

type Filter = "all" | "active" | "done";

export const Component0476 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0476] mounted");
    return () => console.log("[Component0476] unmounted");
  });
  ...
  return () => html`
    <div class="app-476">
      <header class="header-476">
        <h1>todos — Component0476</h1>
      </header>

      <main class="main-476">
        <input
          class="add-input-476 ${error() ? "add-input--error-476" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-476" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-476">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-476">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-476 ${filter() === f ? "filter-btn--active-476" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-476">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-476">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-476">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-476 ${todo.done ? "todo-item--done-476" : ""}">
                    <button
                      class="todo-check-476 ${todo.done ? "todo-check--done-476" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-476" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-476"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-476" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-476" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-476">
            <span class="todo-count-476">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-476" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
