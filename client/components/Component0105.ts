// client/components/Component0105.ts
// Benchmark component #105 — identical logic to TodoPage, scoped class names.
...
import "./Component0105.css";

type Filter = "all" | "active" | "done";

export const Component0105 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0105] mounted");
    return () => console.log("[Component0105] unmounted");
  });
  ...
  return () => html`
    <div class="app-105">
      <header class="header-105">
        <h1>todos — Component0105</h1>
      </header>

      <main class="main-105">
        <input
          class="add-input-105 ${error() ? "add-input--error-105" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-105" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-105">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-105">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-105 ${filter() === f ? "filter-btn--active-105" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-105">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-105">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-105">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-105 ${todo.done ? "todo-item--done-105" : ""}">
                    <button
                      class="todo-check-105 ${todo.done ? "todo-check--done-105" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-105" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-105"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-105" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-105" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-105">
            <span class="todo-count-105">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-105" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
