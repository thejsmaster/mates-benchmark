// client/components/Component0855.ts
// Benchmark component #855 — identical logic to TodoPage, scoped class names.
...
import "./Component0855.css";

type Filter = "all" | "active" | "done";

export const Component0855 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0855] mounted");
    return () => console.log("[Component0855] unmounted");
  });
  ...
  return () => html`
    <div class="app-855">
      <header class="header-855">
        <h1>todos — Component0855</h1>
      </header>

      <main class="main-855">
        <input
          class="add-input-855 ${error() ? "add-input--error-855" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-855" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-855">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-855">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-855 ${filter() === f ? "filter-btn--active-855" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-855">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-855">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-855">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-855 ${todo.done ? "todo-item--done-855" : ""}">
                    <button
                      class="todo-check-855 ${todo.done ? "todo-check--done-855" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-855" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-855"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-855" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-855" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-855">
            <span class="todo-count-855">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-855" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
