// client/components/Component0348.ts
// Benchmark component #348 — identical logic to TodoPage, scoped class names.
...
import "./Component0348.css";

type Filter = "all" | "active" | "done";

export const Component0348 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0348] mounted");
    return () => console.log("[Component0348] unmounted");
  });
  ...
  return () => html`
    <div class="app-348">
      <header class="header-348">
        <h1>todos — Component0348</h1>
      </header>

      <main class="main-348">
        <input
          class="add-input-348 ${error() ? "add-input--error-348" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-348" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-348">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-348">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-348 ${filter() === f ? "filter-btn--active-348" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-348">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-348">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-348">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-348 ${todo.done ? "todo-item--done-348" : ""}">
                    <button
                      class="todo-check-348 ${todo.done ? "todo-check--done-348" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-348" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-348"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-348" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-348" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-348">
            <span class="todo-count-348">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-348" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
