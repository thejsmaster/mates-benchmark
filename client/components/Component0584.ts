// client/components/Component0584.ts
// Benchmark component #584 — identical logic to TodoPage, scoped class names.
...
import "./Component0584.css";

type Filter = "all" | "active" | "done";

export const Component0584 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0584] mounted");
    return () => console.log("[Component0584] unmounted");
  });
  ...
  return () => html`
    <div class="app-584">
      <header class="header-584">
        <h1>todos — Component0584</h1>
      </header>

      <main class="main-584">
        <input
          class="add-input-584 ${error() ? "add-input--error-584" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-584" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-584">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-584">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-584 ${filter() === f ? "filter-btn--active-584" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-584">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-584">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-584">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-584 ${todo.done ? "todo-item--done-584" : ""}">
                    <button
                      class="todo-check-584 ${todo.done ? "todo-check--done-584" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-584" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-584"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-584" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-584" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-584">
            <span class="todo-count-584">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-584" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
