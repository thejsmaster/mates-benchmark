// client/components/Component0957.ts
// Benchmark component #957 — identical logic to TodoPage, scoped class names.
...
import "./Component0957.css";

type Filter = "all" | "active" | "done";

export const Component0957 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0957] mounted");
    return () => console.log("[Component0957] unmounted");
  });
  ...
  return () => html`
    <div class="app-957">
      <header class="header-957">
        <h1>todos — Component0957</h1>
      </header>

      <main class="main-957">
        <input
          class="add-input-957 ${error() ? "add-input--error-957" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-957" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-957">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-957">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-957 ${filter() === f ? "filter-btn--active-957" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-957">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-957">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-957">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-957 ${todo.done ? "todo-item--done-957" : ""}">
                    <button
                      class="todo-check-957 ${todo.done ? "todo-check--done-957" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-957" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-957"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-957" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-957" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-957">
            <span class="todo-count-957">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-957" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
