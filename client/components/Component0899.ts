// client/components/Component0899.ts
// Benchmark component #899 — identical logic to TodoPage, scoped class names.
...
import "./Component0899.css";

type Filter = "all" | "active" | "done";

export const Component0899 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0899] mounted");
    return () => console.log("[Component0899] unmounted");
  });
  ...
  return () => html`
    <div class="app-899">
      <header class="header-899">
        <h1>todos — Component0899</h1>
      </header>

      <main class="main-899">
        <input
          class="add-input-899 ${error() ? "add-input--error-899" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-899" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-899">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-899">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-899 ${filter() === f ? "filter-btn--active-899" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-899">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-899">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-899">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-899 ${todo.done ? "todo-item--done-899" : ""}">
                    <button
                      class="todo-check-899 ${todo.done ? "todo-check--done-899" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-899" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-899"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-899" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-899" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-899">
            <span class="todo-count-899">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-899" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
