// client/components/Component0318.ts
// Benchmark component #318 — identical logic to TodoPage, scoped class names.
...
import "./Component0318.css";

type Filter = "all" | "active" | "done";

export const Component0318 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0318] mounted");
    return () => console.log("[Component0318] unmounted");
  });
  ...
  return () => html`
    <div class="app-318">
      <header class="header-318">
        <h1>todos — Component0318</h1>
      </header>

      <main class="main-318">
        <input
          class="add-input-318 ${error() ? "add-input--error-318" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-318" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-318">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-318">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-318 ${filter() === f ? "filter-btn--active-318" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-318">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-318">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-318">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-318 ${todo.done ? "todo-item--done-318" : ""}">
                    <button
                      class="todo-check-318 ${todo.done ? "todo-check--done-318" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-318" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-318"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-318" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-318" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-318">
            <span class="todo-count-318">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-318" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
