// client/components/Component0233.ts
// Benchmark component #233 — identical logic to TodoPage, scoped class names.
...
import "./Component0233.css";

type Filter = "all" | "active" | "done";

export const Component0233 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0233] mounted");
    return () => console.log("[Component0233] unmounted");
  });
  ...
  return () => html`
    <div class="app-233">
      <header class="header-233">
        <h1>todos — Component0233</h1>
      </header>

      <main class="main-233">
        <input
          class="add-input-233 ${error() ? "add-input--error-233" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-233" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-233">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-233">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-233 ${filter() === f ? "filter-btn--active-233" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-233">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-233">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-233">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-233 ${todo.done ? "todo-item--done-233" : ""}">
                    <button
                      class="todo-check-233 ${todo.done ? "todo-check--done-233" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-233" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-233"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-233" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-233" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-233">
            <span class="todo-count-233">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-233" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
