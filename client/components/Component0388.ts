// client/components/Component0388.ts
// Benchmark component #388 — identical logic to TodoPage, scoped class names.
...
import "./Component0388.css";

type Filter = "all" | "active" | "done";

export const Component0388 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0388] mounted");
    return () => console.log("[Component0388] unmounted");
  });
  ...
  return () => html`
    <div class="app-388">
      <header class="header-388">
        <h1>todos — Component0388</h1>
      </header>

      <main class="main-388">
        <input
          class="add-input-388 ${error() ? "add-input--error-388" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-388" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-388">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-388">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-388 ${filter() === f ? "filter-btn--active-388" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-388">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-388">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-388">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-388 ${todo.done ? "todo-item--done-388" : ""}">
                    <button
                      class="todo-check-388 ${todo.done ? "todo-check--done-388" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-388" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-388"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-388" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-388" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-388">
            <span class="todo-count-388">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-388" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
