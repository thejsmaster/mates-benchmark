// client/components/Component0332.ts
// Benchmark component #332 — identical logic to TodoPage, scoped class names.
...
import "./Component0332.css";

type Filter = "all" | "active" | "done";

export const Component0332 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0332] mounted");
    return () => console.log("[Component0332] unmounted");
  });
  ...
  return () => html`
    <div class="app-332">
      <header class="header-332">
        <h1>todos — Component0332</h1>
      </header>

      <main class="main-332">
        <input
          class="add-input-332 ${error() ? "add-input--error-332" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-332" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-332">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-332">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-332 ${filter() === f ? "filter-btn--active-332" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-332">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-332">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-332">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-332 ${todo.done ? "todo-item--done-332" : ""}">
                    <button
                      class="todo-check-332 ${todo.done ? "todo-check--done-332" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-332" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-332"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-332" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-332" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-332">
            <span class="todo-count-332">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-332" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
