// client/components/Component0614.ts
// Benchmark component #614 — identical logic to TodoPage, scoped class names.
...
import "./Component0614.css";

type Filter = "all" | "active" | "done";

export const Component0614 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0614] mounted");
    return () => console.log("[Component0614] unmounted");
  });
  ...
  return () => html`
    <div class="app-614">
      <header class="header-614">
        <h1>todos — Component0614</h1>
      </header>

      <main class="main-614">
        <input
          class="add-input-614 ${error() ? "add-input--error-614" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-614" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-614">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-614">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-614 ${filter() === f ? "filter-btn--active-614" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-614">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-614">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-614">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-614 ${todo.done ? "todo-item--done-614" : ""}">
                    <button
                      class="todo-check-614 ${todo.done ? "todo-check--done-614" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-614" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-614"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-614" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-614" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-614">
            <span class="todo-count-614">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-614" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
