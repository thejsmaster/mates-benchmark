// client/components/Component0239.ts
// Benchmark component #239 — identical logic to TodoPage, scoped class names.
...
import "./Component0239.css";

type Filter = "all" | "active" | "done";

export const Component0239 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0239] mounted");
    return () => console.log("[Component0239] unmounted");
  });
  ...
  return () => html`
    <div class="app-239">
      <header class="header-239">
        <h1>todos — Component0239</h1>
      </header>

      <main class="main-239">
        <input
          class="add-input-239 ${error() ? "add-input--error-239" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-239" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-239">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-239">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-239 ${filter() === f ? "filter-btn--active-239" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-239">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-239">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-239">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-239 ${todo.done ? "todo-item--done-239" : ""}">
                    <button
                      class="todo-check-239 ${todo.done ? "todo-check--done-239" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-239" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-239"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-239" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-239" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-239">
            <span class="todo-count-239">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-239" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
