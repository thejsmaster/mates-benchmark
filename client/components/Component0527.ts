// client/components/Component0527.ts
// Benchmark component #527 — identical logic to TodoPage, scoped class names.
...
import "./Component0527.css";

type Filter = "all" | "active" | "done";

export const Component0527 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0527] mounted");
    return () => console.log("[Component0527] unmounted");
  });
  ...
  return () => html`
    <div class="app-527">
      <header class="header-527">
        <h1>todos — Component0527</h1>
      </header>

      <main class="main-527">
        <input
          class="add-input-527 ${error() ? "add-input--error-527" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-527" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-527">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-527">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-527 ${filter() === f ? "filter-btn--active-527" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-527">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-527">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-527">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-527 ${todo.done ? "todo-item--done-527" : ""}">
                    <button
                      class="todo-check-527 ${todo.done ? "todo-check--done-527" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-527" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-527"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-527" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-527" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-527">
            <span class="todo-count-527">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-527" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
