// client/components/Component0675.ts
// Benchmark component #675 — identical logic to TodoPage, scoped class names.
...
import "./Component0675.css";

type Filter = "all" | "active" | "done";

export const Component0675 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0675] mounted");
    return () => console.log("[Component0675] unmounted");
  });
  ...
  return () => html`
    <div class="app-675">
      <header class="header-675">
        <h1>todos — Component0675</h1>
      </header>

      <main class="main-675">
        <input
          class="add-input-675 ${error() ? "add-input--error-675" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-675" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-675">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-675">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-675 ${filter() === f ? "filter-btn--active-675" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-675">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-675">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-675">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-675 ${todo.done ? "todo-item--done-675" : ""}">
                    <button
                      class="todo-check-675 ${todo.done ? "todo-check--done-675" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-675" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-675"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-675" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-675" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-675">
            <span class="todo-count-675">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-675" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
