// client/components/Component0334.ts
// Benchmark component #334 — identical logic to TodoPage, scoped class names.
...
import "./Component0334.css";

type Filter = "all" | "active" | "done";

export const Component0334 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0334] mounted");
    return () => console.log("[Component0334] unmounted");
  });
  ...
  return () => html`
    <div class="app-334">
      <header class="header-334">
        <h1>todos — Component0334</h1>
      </header>

      <main class="main-334">
        <input
          class="add-input-334 ${error() ? "add-input--error-334" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-334" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-334">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-334">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-334 ${filter() === f ? "filter-btn--active-334" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-334">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-334">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-334">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-334 ${todo.done ? "todo-item--done-334" : ""}">
                    <button
                      class="todo-check-334 ${todo.done ? "todo-check--done-334" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-334" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-334"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-334" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-334" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-334">
            <span class="todo-count-334">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-334" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
