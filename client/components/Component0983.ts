// client/components/Component0983.ts
// Benchmark component #983 — identical logic to TodoPage, scoped class names.
...
import "./Component0983.css";

type Filter = "all" | "active" | "done";

export const Component0983 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0983] mounted");
    return () => console.log("[Component0983] unmounted");
  });
  ...
  return () => html`
    <div class="app-983">
      <header class="header-983">
        <h1>todos — Component0983</h1>
      </header>

      <main class="main-983">
        <input
          class="add-input-983 ${error() ? "add-input--error-983" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-983" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-983">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-983">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-983 ${filter() === f ? "filter-btn--active-983" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-983">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-983">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-983">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-983 ${todo.done ? "todo-item--done-983" : ""}">
                    <button
                      class="todo-check-983 ${todo.done ? "todo-check--done-983" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-983" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-983"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-983" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-983" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-983">
            <span class="todo-count-983">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-983" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
