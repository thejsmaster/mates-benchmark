// client/components/Component0541.ts
// Benchmark component #541 — identical logic to TodoPage, scoped class names.
...
import "./Component0541.css";

type Filter = "all" | "active" | "done";

export const Component0541 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0541] mounted");
    return () => console.log("[Component0541] unmounted");
  });
  ...
  return () => html`
    <div class="app-541">
      <header class="header-541">
        <h1>todos — Component0541</h1>
      </header>

      <main class="main-541">
        <input
          class="add-input-541 ${error() ? "add-input--error-541" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-541" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-541">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-541">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-541 ${filter() === f ? "filter-btn--active-541" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-541">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-541">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-541">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-541 ${todo.done ? "todo-item--done-541" : ""}">
                    <button
                      class="todo-check-541 ${todo.done ? "todo-check--done-541" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-541" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-541"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-541" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-541" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-541">
            <span class="todo-count-541">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-541" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
