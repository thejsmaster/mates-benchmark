// client/components/Component0376.ts
// Benchmark component #376 — identical logic to TodoPage, scoped class names.
...
import "./Component0376.css";

type Filter = "all" | "active" | "done";

export const Component0376 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0376] mounted");
    return () => console.log("[Component0376] unmounted");
  });
  ...
  return () => html`
    <div class="app-376">
      <header class="header-376">
        <h1>todos — Component0376</h1>
      </header>

      <main class="main-376">
        <input
          class="add-input-376 ${error() ? "add-input--error-376" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-376" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-376">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-376">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-376 ${filter() === f ? "filter-btn--active-376" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-376">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-376">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-376">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-376 ${todo.done ? "todo-item--done-376" : ""}">
                    <button
                      class="todo-check-376 ${todo.done ? "todo-check--done-376" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-376" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-376"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-376" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-376" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-376">
            <span class="todo-count-376">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-376" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
