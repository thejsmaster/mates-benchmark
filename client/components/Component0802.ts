// client/components/Component0802.ts
// Benchmark component #802 — identical logic to TodoPage, scoped class names.
...
import "./Component0802.css";

type Filter = "all" | "active" | "done";

export const Component0802 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0802] mounted");
    return () => console.log("[Component0802] unmounted");
  });
  ...
  return () => html`
    <div class="app-802">
      <header class="header-802">
        <h1>todos — Component0802</h1>
      </header>

      <main class="main-802">
        <input
          class="add-input-802 ${error() ? "add-input--error-802" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-802" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-802">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-802">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-802 ${filter() === f ? "filter-btn--active-802" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-802">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-802">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-802">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-802 ${todo.done ? "todo-item--done-802" : ""}">
                    <button
                      class="todo-check-802 ${todo.done ? "todo-check--done-802" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-802" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-802"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-802" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-802" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-802">
            <span class="todo-count-802">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-802" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
