// client/components/Component0155.ts
// Benchmark component #155 — identical logic to TodoPage, scoped class names.
...
import "./Component0155.css";

type Filter = "all" | "active" | "done";

export const Component0155 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0155] mounted");
    return () => console.log("[Component0155] unmounted");
  });
  ...
  return () => html`
    <div class="app-155">
      <header class="header-155">
        <h1>todos — Component0155</h1>
      </header>

      <main class="main-155">
        <input
          class="add-input-155 ${error() ? "add-input--error-155" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-155" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-155">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-155">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-155 ${filter() === f ? "filter-btn--active-155" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-155">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-155">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-155">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-155 ${todo.done ? "todo-item--done-155" : ""}">
                    <button
                      class="todo-check-155 ${todo.done ? "todo-check--done-155" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-155" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-155"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-155" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-155" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-155">
            <span class="todo-count-155">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-155" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
