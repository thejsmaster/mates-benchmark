// client/components/Component0890.ts
// Benchmark component #890 — identical logic to TodoPage, scoped class names.
...
import "./Component0890.css";

type Filter = "all" | "active" | "done";

export const Component0890 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0890] mounted");
    return () => console.log("[Component0890] unmounted");
  });
  ...
  return () => html`
    <div class="app-890">
      <header class="header-890">
        <h1>todos — Component0890</h1>
      </header>

      <main class="main-890">
        <input
          class="add-input-890 ${error() ? "add-input--error-890" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-890" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-890">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-890">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-890 ${filter() === f ? "filter-btn--active-890" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-890">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-890">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-890">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-890 ${todo.done ? "todo-item--done-890" : ""}">
                    <button
                      class="todo-check-890 ${todo.done ? "todo-check--done-890" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-890" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-890"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-890" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-890" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-890">
            <span class="todo-count-890">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-890" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
