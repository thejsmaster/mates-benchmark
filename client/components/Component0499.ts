// client/components/Component0499.ts
// Benchmark component #499 — identical logic to TodoPage, scoped class names.
...
import "./Component0499.css";

type Filter = "all" | "active" | "done";

export const Component0499 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0499] mounted");
    return () => console.log("[Component0499] unmounted");
  });
  ...
  return () => html`
    <div class="app-499">
      <header class="header-499">
        <h1>todos — Component0499</h1>
      </header>

      <main class="main-499">
        <input
          class="add-input-499 ${error() ? "add-input--error-499" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-499" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-499">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-499">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-499 ${filter() === f ? "filter-btn--active-499" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-499">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-499">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-499">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-499 ${todo.done ? "todo-item--done-499" : ""}">
                    <button
                      class="todo-check-499 ${todo.done ? "todo-check--done-499" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-499" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-499"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-499" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-499" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-499">
            <span class="todo-count-499">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-499" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
