// client/components/Component0503.ts
// Benchmark component #503 — identical logic to TodoPage, scoped class names.
...
import "./Component0503.css";

type Filter = "all" | "active" | "done";

export const Component0503 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0503] mounted");
    return () => console.log("[Component0503] unmounted");
  });
  ...
  return () => html`
    <div class="app-503">
      <header class="header-503">
        <h1>todos — Component0503</h1>
      </header>

      <main class="main-503">
        <input
          class="add-input-503 ${error() ? "add-input--error-503" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-503" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-503">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-503">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-503 ${filter() === f ? "filter-btn--active-503" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-503">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-503">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-503">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-503 ${todo.done ? "todo-item--done-503" : ""}">
                    <button
                      class="todo-check-503 ${todo.done ? "todo-check--done-503" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-503" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-503"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-503" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-503" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-503">
            <span class="todo-count-503">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-503" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
