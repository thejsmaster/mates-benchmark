// client/components/Component0695.ts
// Benchmark component #695 — identical logic to TodoPage, scoped class names.
...
import "./Component0695.css";

type Filter = "all" | "active" | "done";

export const Component0695 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0695] mounted");
    return () => console.log("[Component0695] unmounted");
  });
  ...
  return () => html`
    <div class="app-695">
      <header class="header-695">
        <h1>todos — Component0695</h1>
      </header>

      <main class="main-695">
        <input
          class="add-input-695 ${error() ? "add-input--error-695" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-695" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-695">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-695">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-695 ${filter() === f ? "filter-btn--active-695" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-695">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-695">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-695">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-695 ${todo.done ? "todo-item--done-695" : ""}">
                    <button
                      class="todo-check-695 ${todo.done ? "todo-check--done-695" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-695" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-695"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-695" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-695" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-695">
            <span class="todo-count-695">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-695" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
