// client/components/Component0059.ts
// Benchmark component #59 — identical logic to TodoPage, scoped class names.
...
import "./Component0059.css";

type Filter = "all" | "active" | "done";

export const Component0059 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0059] mounted");
    return () => console.log("[Component0059] unmounted");
  });
  ...
  return () => html`
    <div class="app-59">
      <header class="header-59">
        <h1>todos — Component0059</h1>
      </header>

      <main class="main-59">
        <input
          class="add-input-59 ${error() ? "add-input--error-59" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-59" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-59">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-59">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-59 ${filter() === f ? "filter-btn--active-59" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-59">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-59">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-59">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-59 ${todo.done ? "todo-item--done-59" : ""}">
                    <button
                      class="todo-check-59 ${todo.done ? "todo-check--done-59" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-59" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-59"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-59" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-59" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-59">
            <span class="todo-count-59">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-59" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
