// client/components/Component0645.ts
// Benchmark component #645 — identical logic to TodoPage, scoped class names.
...
import "./Component0645.css";

type Filter = "all" | "active" | "done";

export const Component0645 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0645] mounted");
    return () => console.log("[Component0645] unmounted");
  });
  ...
  return () => html`
    <div class="app-645">
      <header class="header-645">
        <h1>todos — Component0645</h1>
      </header>

      <main class="main-645">
        <input
          class="add-input-645 ${error() ? "add-input--error-645" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-645" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-645">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-645">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-645 ${filter() === f ? "filter-btn--active-645" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-645">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-645">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-645">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-645 ${todo.done ? "todo-item--done-645" : ""}">
                    <button
                      class="todo-check-645 ${todo.done ? "todo-check--done-645" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-645" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-645"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-645" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-645" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-645">
            <span class="todo-count-645">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-645" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
