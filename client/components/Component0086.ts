// client/components/Component0086.ts
// Benchmark component #86 — identical logic to TodoPage, scoped class names.
...
import "./Component0086.css";

type Filter = "all" | "active" | "done";

export const Component0086 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0086] mounted");
    return () => console.log("[Component0086] unmounted");
  });
  ...
  return () => html`
    <div class="app-86">
      <header class="header-86">
        <h1>todos — Component0086</h1>
      </header>

      <main class="main-86">
        <input
          class="add-input-86 ${error() ? "add-input--error-86" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-86" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-86">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-86">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-86 ${filter() === f ? "filter-btn--active-86" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-86">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-86">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-86">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-86 ${todo.done ? "todo-item--done-86" : ""}">
                    <button
                      class="todo-check-86 ${todo.done ? "todo-check--done-86" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-86" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-86"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-86" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-86" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-86">
            <span class="todo-count-86">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-86" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
