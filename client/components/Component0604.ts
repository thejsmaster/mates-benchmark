// client/components/Component0604.ts
// Benchmark component #604 — identical logic to TodoPage, scoped class names.
...
import "./Component0604.css";

type Filter = "all" | "active" | "done";

export const Component0604 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0604] mounted");
    return () => console.log("[Component0604] unmounted");
  });
  ...
  return () => html`
    <div class="app-604">
      <header class="header-604">
        <h1>todos — Component0604</h1>
      </header>

      <main class="main-604">
        <input
          class="add-input-604 ${error() ? "add-input--error-604" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-604" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-604">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-604">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-604 ${filter() === f ? "filter-btn--active-604" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-604">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-604">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-604">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-604 ${todo.done ? "todo-item--done-604" : ""}">
                    <button
                      class="todo-check-604 ${todo.done ? "todo-check--done-604" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-604" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-604"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-604" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-604" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-604">
            <span class="todo-count-604">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-604" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
