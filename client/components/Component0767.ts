// client/components/Component0767.ts
// Benchmark component #767 — identical logic to TodoPage, scoped class names.
...
import "./Component0767.css";

type Filter = "all" | "active" | "done";

export const Component0767 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0767] mounted");
    return () => console.log("[Component0767] unmounted");
  });
  ...
  return () => html`
    <div class="app-767">
      <header class="header-767">
        <h1>todos — Component0767</h1>
      </header>

      <main class="main-767">
        <input
          class="add-input-767 ${error() ? "add-input--error-767" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-767" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-767">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-767">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-767 ${filter() === f ? "filter-btn--active-767" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-767">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-767">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-767">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-767 ${todo.done ? "todo-item--done-767" : ""}">
                    <button
                      class="todo-check-767 ${todo.done ? "todo-check--done-767" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-767" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-767"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-767" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-767" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-767">
            <span class="todo-count-767">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-767" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
