// client/components/Component0435.ts
// Benchmark component #435 — identical logic to TodoPage, scoped class names.
...
import "./Component0435.css";

type Filter = "all" | "active" | "done";

export const Component0435 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0435] mounted");
    return () => console.log("[Component0435] unmounted");
  });
  ...
  return () => html`
    <div class="app-435">
      <header class="header-435">
        <h1>todos — Component0435</h1>
      </header>

      <main class="main-435">
        <input
          class="add-input-435 ${error() ? "add-input--error-435" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-435" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-435">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-435">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-435 ${filter() === f ? "filter-btn--active-435" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-435">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-435">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-435">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-435 ${todo.done ? "todo-item--done-435" : ""}">
                    <button
                      class="todo-check-435 ${todo.done ? "todo-check--done-435" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-435" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-435"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-435" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-435" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-435">
            <span class="todo-count-435">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-435" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
