// client/components/Component0055.ts
// Benchmark component #55 — identical logic to TodoPage, scoped class names.
...
import "./Component0055.css";

type Filter = "all" | "active" | "done";

export const Component0055 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0055] mounted");
    return () => console.log("[Component0055] unmounted");
  });
  ...
  return () => html`
    <div class="app-55">
      <header class="header-55">
        <h1>todos — Component0055</h1>
      </header>

      <main class="main-55">
        <input
          class="add-input-55 ${error() ? "add-input--error-55" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-55" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-55">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-55">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-55 ${filter() === f ? "filter-btn--active-55" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-55">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-55">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-55">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-55 ${todo.done ? "todo-item--done-55" : ""}">
                    <button
                      class="todo-check-55 ${todo.done ? "todo-check--done-55" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-55" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-55"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-55" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-55" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-55">
            <span class="todo-count-55">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-55" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
