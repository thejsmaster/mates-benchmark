// client/components/Component0819.ts
// Benchmark component #819 — identical logic to TodoPage, scoped class names.
...
import "./Component0819.css";

type Filter = "all" | "active" | "done";

export const Component0819 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0819] mounted");
    return () => console.log("[Component0819] unmounted");
  });
  ...
  return () => html`
    <div class="app-819">
      <header class="header-819">
        <h1>todos — Component0819</h1>
      </header>

      <main class="main-819">
        <input
          class="add-input-819 ${error() ? "add-input--error-819" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-819" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-819">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-819">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-819 ${filter() === f ? "filter-btn--active-819" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-819">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-819">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-819">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-819 ${todo.done ? "todo-item--done-819" : ""}">
                    <button
                      class="todo-check-819 ${todo.done ? "todo-check--done-819" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-819" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-819"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-819" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-819" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-819">
            <span class="todo-count-819">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-819" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
