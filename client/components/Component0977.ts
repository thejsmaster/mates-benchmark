// client/components/Component0977.ts
// Benchmark component #977 — identical logic to TodoPage, scoped class names.
...
import "./Component0977.css";

type Filter = "all" | "active" | "done";

export const Component0977 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0977] mounted");
    return () => console.log("[Component0977] unmounted");
  });
  ...
  return () => html`
    <div class="app-977">
      <header class="header-977">
        <h1>todos — Component0977</h1>
      </header>

      <main class="main-977">
        <input
          class="add-input-977 ${error() ? "add-input--error-977" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-977" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-977">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-977">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-977 ${filter() === f ? "filter-btn--active-977" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-977">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-977">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-977">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-977 ${todo.done ? "todo-item--done-977" : ""}">
                    <button
                      class="todo-check-977 ${todo.done ? "todo-check--done-977" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-977" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-977"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-977" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-977" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-977">
            <span class="todo-count-977">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-977" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
