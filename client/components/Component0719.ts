// client/components/Component0719.ts
// Benchmark component #719 — identical logic to TodoPage, scoped class names.
...
import "./Component0719.css";

type Filter = "all" | "active" | "done";

export const Component0719 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0719] mounted");
    return () => console.log("[Component0719] unmounted");
  });
  ...
  return () => html`
    <div class="app-719">
      <header class="header-719">
        <h1>todos — Component0719</h1>
      </header>

      <main class="main-719">
        <input
          class="add-input-719 ${error() ? "add-input--error-719" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-719" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-719">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-719">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-719 ${filter() === f ? "filter-btn--active-719" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-719">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-719">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-719">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-719 ${todo.done ? "todo-item--done-719" : ""}">
                    <button
                      class="todo-check-719 ${todo.done ? "todo-check--done-719" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-719" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-719"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-719" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-719" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-719">
            <span class="todo-count-719">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-719" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
