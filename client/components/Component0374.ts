// client/components/Component0374.ts
// Benchmark component #374 — identical logic to TodoPage, scoped class names.
...
import "./Component0374.css";

type Filter = "all" | "active" | "done";

export const Component0374 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0374] mounted");
    return () => console.log("[Component0374] unmounted");
  });
  ...
  return () => html`
    <div class="app-374">
      <header class="header-374">
        <h1>todos — Component0374</h1>
      </header>

      <main class="main-374">
        <input
          class="add-input-374 ${error() ? "add-input--error-374" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-374" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-374">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-374">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-374 ${filter() === f ? "filter-btn--active-374" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-374">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-374">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-374">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-374 ${todo.done ? "todo-item--done-374" : ""}">
                    <button
                      class="todo-check-374 ${todo.done ? "todo-check--done-374" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-374" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-374"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-374" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-374" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-374">
            <span class="todo-count-374">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-374" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
