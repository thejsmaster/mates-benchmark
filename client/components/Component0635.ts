// client/components/Component0635.ts
// Benchmark component #635 — identical logic to TodoPage, scoped class names.
...
import "./Component0635.css";

type Filter = "all" | "active" | "done";

export const Component0635 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0635] mounted");
    return () => console.log("[Component0635] unmounted");
  });
  ...
  return () => html`
    <div class="app-635">
      <header class="header-635">
        <h1>todos — Component0635</h1>
      </header>

      <main class="main-635">
        <input
          class="add-input-635 ${error() ? "add-input--error-635" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-635" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-635">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-635">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-635 ${filter() === f ? "filter-btn--active-635" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-635">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-635">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-635">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-635 ${todo.done ? "todo-item--done-635" : ""}">
                    <button
                      class="todo-check-635 ${todo.done ? "todo-check--done-635" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-635" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-635"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-635" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-635" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-635">
            <span class="todo-count-635">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-635" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
