// client/components/Component0032.ts
// Benchmark component #32 — identical logic to TodoPage, scoped class names.
...
import "./Component0032.css";

type Filter = "all" | "active" | "done";

export const Component0032 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0032] mounted");
    return () => console.log("[Component0032] unmounted");
  });
  ...
  return () => html`
    <div class="app-32">
      <header class="header-32">
        <h1>todos — Component0032</h1>
      </header>

      <main class="main-32">
        <input
          class="add-input-32 ${error() ? "add-input--error-32" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-32" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-32">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-32">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-32 ${filter() === f ? "filter-btn--active-32" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-32">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-32">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-32">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-32 ${todo.done ? "todo-item--done-32" : ""}">
                    <button
                      class="todo-check-32 ${todo.done ? "todo-check--done-32" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-32" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-32"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-32" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-32" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-32">
            <span class="todo-count-32">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-32" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
