// client/components/Component0311.ts
// Benchmark component #311 — identical logic to TodoPage, scoped class names.
...
import "./Component0311.css";

type Filter = "all" | "active" | "done";

export const Component0311 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0311] mounted");
    return () => console.log("[Component0311] unmounted");
  });
  ...
  return () => html`
    <div class="app-311">
      <header class="header-311">
        <h1>todos — Component0311</h1>
      </header>

      <main class="main-311">
        <input
          class="add-input-311 ${error() ? "add-input--error-311" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-311" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-311">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-311">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-311 ${filter() === f ? "filter-btn--active-311" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-311">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-311">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-311">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-311 ${todo.done ? "todo-item--done-311" : ""}">
                    <button
                      class="todo-check-311 ${todo.done ? "todo-check--done-311" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-311" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-311"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-311" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-311" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-311">
            <span class="todo-count-311">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-311" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
