// client/components/Component0273.ts
// Benchmark component #273 — identical logic to TodoPage, scoped class names.
...
import "./Component0273.css";

type Filter = "all" | "active" | "done";

export const Component0273 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0273] mounted");
    return () => console.log("[Component0273] unmounted");
  });
  ...
  return () => html`
    <div class="app-273">
      <header class="header-273">
        <h1>todos — Component0273</h1>
      </header>

      <main class="main-273">
        <input
          class="add-input-273 ${error() ? "add-input--error-273" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-273" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-273">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-273">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-273 ${filter() === f ? "filter-btn--active-273" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-273">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-273">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-273">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-273 ${todo.done ? "todo-item--done-273" : ""}">
                    <button
                      class="todo-check-273 ${todo.done ? "todo-check--done-273" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-273" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-273"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-273" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-273" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-273">
            <span class="todo-count-273">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-273" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
