// client/components/Component0080.ts
// Benchmark component #80 — identical logic to TodoPage, scoped class names.
...
import "./Component0080.css";

type Filter = "all" | "active" | "done";

export const Component0080 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0080] mounted");
    return () => console.log("[Component0080] unmounted");
  });
  ...
  return () => html`
    <div class="app-80">
      <header class="header-80">
        <h1>todos — Component0080</h1>
      </header>

      <main class="main-80">
        <input
          class="add-input-80 ${error() ? "add-input--error-80" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-80" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-80">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-80">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-80 ${filter() === f ? "filter-btn--active-80" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-80">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-80">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-80">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-80 ${todo.done ? "todo-item--done-80" : ""}">
                    <button
                      class="todo-check-80 ${todo.done ? "todo-check--done-80" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-80" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-80"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-80" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-80" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-80">
            <span class="todo-count-80">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-80" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
