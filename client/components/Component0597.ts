// client/components/Component0597.ts
// Benchmark component #597 — identical logic to TodoPage, scoped class names.
...
import "./Component0597.css";

type Filter = "all" | "active" | "done";

export const Component0597 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0597] mounted");
    return () => console.log("[Component0597] unmounted");
  });
  ...
  return () => html`
    <div class="app-597">
      <header class="header-597">
        <h1>todos — Component0597</h1>
      </header>

      <main class="main-597">
        <input
          class="add-input-597 ${error() ? "add-input--error-597" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-597" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-597">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-597">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-597 ${filter() === f ? "filter-btn--active-597" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-597">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-597">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-597">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-597 ${todo.done ? "todo-item--done-597" : ""}">
                    <button
                      class="todo-check-597 ${todo.done ? "todo-check--done-597" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-597" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-597"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-597" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-597" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-597">
            <span class="todo-count-597">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-597" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
