// client/components/Component0103.ts
// Benchmark component #103 — identical logic to TodoPage, scoped class names.
...
import "./Component0103.css";

type Filter = "all" | "active" | "done";

export const Component0103 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0103] mounted");
    return () => console.log("[Component0103] unmounted");
  });
  ...
  return () => html`
    <div class="app-103">
      <header class="header-103">
        <h1>todos — Component0103</h1>
      </header>

      <main class="main-103">
        <input
          class="add-input-103 ${error() ? "add-input--error-103" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-103" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-103">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-103">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-103 ${filter() === f ? "filter-btn--active-103" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-103">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-103">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-103">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-103 ${todo.done ? "todo-item--done-103" : ""}">
                    <button
                      class="todo-check-103 ${todo.done ? "todo-check--done-103" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-103" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-103"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-103" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-103" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-103">
            <span class="todo-count-103">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-103" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
