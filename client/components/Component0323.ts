// client/components/Component0323.ts
// Benchmark component #323 — identical logic to TodoPage, scoped class names.
...
import "./Component0323.css";

type Filter = "all" | "active" | "done";

export const Component0323 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0323] mounted");
    return () => console.log("[Component0323] unmounted");
  });
  ...
  return () => html`
    <div class="app-323">
      <header class="header-323">
        <h1>todos — Component0323</h1>
      </header>

      <main class="main-323">
        <input
          class="add-input-323 ${error() ? "add-input--error-323" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-323" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-323">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-323">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-323 ${filter() === f ? "filter-btn--active-323" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-323">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-323">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-323">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-323 ${todo.done ? "todo-item--done-323" : ""}">
                    <button
                      class="todo-check-323 ${todo.done ? "todo-check--done-323" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-323" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-323"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-323" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-323" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-323">
            <span class="todo-count-323">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-323" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
