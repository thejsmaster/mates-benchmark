// client/components/Component0582.ts
// Benchmark component #582 — identical logic to TodoPage, scoped class names.
...
import "./Component0582.css";

type Filter = "all" | "active" | "done";

export const Component0582 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0582] mounted");
    return () => console.log("[Component0582] unmounted");
  });
  ...
  return () => html`
    <div class="app-582">
      <header class="header-582">
        <h1>todos — Component0582</h1>
      </header>

      <main class="main-582">
        <input
          class="add-input-582 ${error() ? "add-input--error-582" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-582" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-582">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-582">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-582 ${filter() === f ? "filter-btn--active-582" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-582">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-582">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-582">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-582 ${todo.done ? "todo-item--done-582" : ""}">
                    <button
                      class="todo-check-582 ${todo.done ? "todo-check--done-582" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-582" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-582"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-582" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-582" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-582">
            <span class="todo-count-582">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-582" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
