// client/components/Component0276.ts
// Benchmark component #276 — identical logic to TodoPage, scoped class names.
...
import "./Component0276.css";

type Filter = "all" | "active" | "done";

export const Component0276 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0276] mounted");
    return () => console.log("[Component0276] unmounted");
  });
  ...
  return () => html`
    <div class="app-276">
      <header class="header-276">
        <h1>todos — Component0276</h1>
      </header>

      <main class="main-276">
        <input
          class="add-input-276 ${error() ? "add-input--error-276" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-276" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-276">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-276">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-276 ${filter() === f ? "filter-btn--active-276" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-276">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-276">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-276">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-276 ${todo.done ? "todo-item--done-276" : ""}">
                    <button
                      class="todo-check-276 ${todo.done ? "todo-check--done-276" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-276" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-276"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-276" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-276" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-276">
            <span class="todo-count-276">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-276" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
