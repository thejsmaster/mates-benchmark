// client/components/Component0634.ts
// Benchmark component #634 — identical logic to TodoPage, scoped class names.
...
import "./Component0634.css";

type Filter = "all" | "active" | "done";

export const Component0634 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0634] mounted");
    return () => console.log("[Component0634] unmounted");
  });
  ...
  return () => html`
    <div class="app-634">
      <header class="header-634">
        <h1>todos — Component0634</h1>
      </header>

      <main class="main-634">
        <input
          class="add-input-634 ${error() ? "add-input--error-634" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-634" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-634">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-634">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-634 ${filter() === f ? "filter-btn--active-634" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-634">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-634">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-634">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-634 ${todo.done ? "todo-item--done-634" : ""}">
                    <button
                      class="todo-check-634 ${todo.done ? "todo-check--done-634" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-634" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-634"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-634" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-634" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-634">
            <span class="todo-count-634">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-634" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
