// client/components/Component0973.ts
// Benchmark component #973 — identical logic to TodoPage, scoped class names.
...
import "./Component0973.css";

type Filter = "all" | "active" | "done";

export const Component0973 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0973] mounted");
    return () => console.log("[Component0973] unmounted");
  });
  ...
  return () => html`
    <div class="app-973">
      <header class="header-973">
        <h1>todos — Component0973</h1>
      </header>

      <main class="main-973">
        <input
          class="add-input-973 ${error() ? "add-input--error-973" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-973" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-973">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-973">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-973 ${filter() === f ? "filter-btn--active-973" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-973">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-973">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-973">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-973 ${todo.done ? "todo-item--done-973" : ""}">
                    <button
                      class="todo-check-973 ${todo.done ? "todo-check--done-973" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-973" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-973"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-973" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-973" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-973">
            <span class="todo-count-973">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-973" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
