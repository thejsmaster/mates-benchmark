// client/components/Component0924.ts
// Benchmark component #924 — identical logic to TodoPage, scoped class names.
...
import "./Component0924.css";

type Filter = "all" | "active" | "done";

export const Component0924 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0924] mounted");
    return () => console.log("[Component0924] unmounted");
  });
  ...
  return () => html`
    <div class="app-924">
      <header class="header-924">
        <h1>todos — Component0924</h1>
      </header>

      <main class="main-924">
        <input
          class="add-input-924 ${error() ? "add-input--error-924" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-924" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-924">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-924">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-924 ${filter() === f ? "filter-btn--active-924" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-924">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-924">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-924">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-924 ${todo.done ? "todo-item--done-924" : ""}">
                    <button
                      class="todo-check-924 ${todo.done ? "todo-check--done-924" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-924" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-924"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-924" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-924" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-924">
            <span class="todo-count-924">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-924" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
