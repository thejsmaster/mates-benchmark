// client/components/Component0154.ts
// Benchmark component #154 — identical logic to TodoPage, scoped class names.
...
import "./Component0154.css";

type Filter = "all" | "active" | "done";

export const Component0154 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0154] mounted");
    return () => console.log("[Component0154] unmounted");
  });
  ...
  return () => html`
    <div class="app-154">
      <header class="header-154">
        <h1>todos — Component0154</h1>
      </header>

      <main class="main-154">
        <input
          class="add-input-154 ${error() ? "add-input--error-154" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-154" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-154">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-154">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-154 ${filter() === f ? "filter-btn--active-154" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-154">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-154">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-154">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-154 ${todo.done ? "todo-item--done-154" : ""}">
                    <button
                      class="todo-check-154 ${todo.done ? "todo-check--done-154" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-154" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-154"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-154" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-154" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-154">
            <span class="todo-count-154">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-154" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
