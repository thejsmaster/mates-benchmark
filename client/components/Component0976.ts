// client/components/Component0976.ts
// Benchmark component #976 — identical logic to TodoPage, scoped class names.
...
import "./Component0976.css";

type Filter = "all" | "active" | "done";

export const Component0976 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0976] mounted");
    return () => console.log("[Component0976] unmounted");
  });
  ...
  return () => html`
    <div class="app-976">
      <header class="header-976">
        <h1>todos — Component0976</h1>
      </header>

      <main class="main-976">
        <input
          class="add-input-976 ${error() ? "add-input--error-976" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-976" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-976">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-976">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-976 ${filter() === f ? "filter-btn--active-976" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-976">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-976">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-976">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-976 ${todo.done ? "todo-item--done-976" : ""}">
                    <button
                      class="todo-check-976 ${todo.done ? "todo-check--done-976" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-976" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-976"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-976" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-976" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-976">
            <span class="todo-count-976">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-976" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
