// client/components/Component0329.ts
// Benchmark component #329 — identical logic to TodoPage, scoped class names.
...
import "./Component0329.css";

type Filter = "all" | "active" | "done";

export const Component0329 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0329] mounted");
    return () => console.log("[Component0329] unmounted");
  });
  ...
  return () => html`
    <div class="app-329">
      <header class="header-329">
        <h1>todos — Component0329</h1>
      </header>

      <main class="main-329">
        <input
          class="add-input-329 ${error() ? "add-input--error-329" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-329" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-329">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-329">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-329 ${filter() === f ? "filter-btn--active-329" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-329">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-329">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-329">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-329 ${todo.done ? "todo-item--done-329" : ""}">
                    <button
                      class="todo-check-329 ${todo.done ? "todo-check--done-329" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-329" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-329"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-329" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-329" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-329">
            <span class="todo-count-329">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-329" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
