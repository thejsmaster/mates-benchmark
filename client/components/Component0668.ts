// client/components/Component0668.ts
// Benchmark component #668 — identical logic to TodoPage, scoped class names.
...
import "./Component0668.css";

type Filter = "all" | "active" | "done";

export const Component0668 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0668] mounted");
    return () => console.log("[Component0668] unmounted");
  });
  ...
  return () => html`
    <div class="app-668">
      <header class="header-668">
        <h1>todos — Component0668</h1>
      </header>

      <main class="main-668">
        <input
          class="add-input-668 ${error() ? "add-input--error-668" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-668" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-668">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-668">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-668 ${filter() === f ? "filter-btn--active-668" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-668">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-668">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-668">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-668 ${todo.done ? "todo-item--done-668" : ""}">
                    <button
                      class="todo-check-668 ${todo.done ? "todo-check--done-668" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-668" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-668"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-668" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-668" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-668">
            <span class="todo-count-668">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-668" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
