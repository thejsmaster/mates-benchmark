// client/components/Component0828.ts
// Benchmark component #828 — identical logic to TodoPage, scoped class names.
...
import "./Component0828.css";

type Filter = "all" | "active" | "done";

export const Component0828 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0828] mounted");
    return () => console.log("[Component0828] unmounted");
  });
  ...
  return () => html`
    <div class="app-828">
      <header class="header-828">
        <h1>todos — Component0828</h1>
      </header>

      <main class="main-828">
        <input
          class="add-input-828 ${error() ? "add-input--error-828" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-828" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-828">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-828">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-828 ${filter() === f ? "filter-btn--active-828" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-828">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-828">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-828">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-828 ${todo.done ? "todo-item--done-828" : ""}">
                    <button
                      class="todo-check-828 ${todo.done ? "todo-check--done-828" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-828" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-828"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-828" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-828" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-828">
            <span class="todo-count-828">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-828" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
