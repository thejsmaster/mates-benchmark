// client/components/Component0247.ts
// Benchmark component #247 — identical logic to TodoPage, scoped class names.
...
import "./Component0247.css";

type Filter = "all" | "active" | "done";

export const Component0247 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0247] mounted");
    return () => console.log("[Component0247] unmounted");
  });
  ...
  return () => html`
    <div class="app-247">
      <header class="header-247">
        <h1>todos — Component0247</h1>
      </header>

      <main class="main-247">
        <input
          class="add-input-247 ${error() ? "add-input--error-247" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-247" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-247">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-247">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-247 ${filter() === f ? "filter-btn--active-247" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-247">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-247">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-247">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-247 ${todo.done ? "todo-item--done-247" : ""}">
                    <button
                      class="todo-check-247 ${todo.done ? "todo-check--done-247" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-247" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-247"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-247" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-247" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-247">
            <span class="todo-count-247">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-247" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
