// client/components/Component0739.ts
// Benchmark component #739 — identical logic to TodoPage, scoped class names.
...
import "./Component0739.css";

type Filter = "all" | "active" | "done";

export const Component0739 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0739] mounted");
    return () => console.log("[Component0739] unmounted");
  });
  ...
  return () => html`
    <div class="app-739">
      <header class="header-739">
        <h1>todos — Component0739</h1>
      </header>

      <main class="main-739">
        <input
          class="add-input-739 ${error() ? "add-input--error-739" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-739" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-739">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-739">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-739 ${filter() === f ? "filter-btn--active-739" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-739">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-739">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-739">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-739 ${todo.done ? "todo-item--done-739" : ""}">
                    <button
                      class="todo-check-739 ${todo.done ? "todo-check--done-739" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-739" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-739"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-739" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-739" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-739">
            <span class="todo-count-739">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-739" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
