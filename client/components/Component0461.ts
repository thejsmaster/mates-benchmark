// client/components/Component0461.ts
// Benchmark component #461 — identical logic to TodoPage, scoped class names.
...
import "./Component0461.css";

type Filter = "all" | "active" | "done";

export const Component0461 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0461] mounted");
    return () => console.log("[Component0461] unmounted");
  });
  ...
  return () => html`
    <div class="app-461">
      <header class="header-461">
        <h1>todos — Component0461</h1>
      </header>

      <main class="main-461">
        <input
          class="add-input-461 ${error() ? "add-input--error-461" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-461" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-461">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-461">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-461 ${filter() === f ? "filter-btn--active-461" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-461">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-461">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-461">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-461 ${todo.done ? "todo-item--done-461" : ""}">
                    <button
                      class="todo-check-461 ${todo.done ? "todo-check--done-461" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-461" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-461"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-461" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-461" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-461">
            <span class="todo-count-461">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-461" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
