// client/components/Component0072.ts
// Benchmark component #72 — identical logic to TodoPage, scoped class names.
...
import "./Component0072.css";

type Filter = "all" | "active" | "done";

export const Component0072 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0072] mounted");
    return () => console.log("[Component0072] unmounted");
  });
  ...
  return () => html`
    <div class="app-72">
      <header class="header-72">
        <h1>todos — Component0072</h1>
      </header>

      <main class="main-72">
        <input
          class="add-input-72 ${error() ? "add-input--error-72" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-72" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-72">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-72">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-72 ${filter() === f ? "filter-btn--active-72" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-72">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-72">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-72">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-72 ${todo.done ? "todo-item--done-72" : ""}">
                    <button
                      class="todo-check-72 ${todo.done ? "todo-check--done-72" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-72" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-72"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-72" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-72" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-72">
            <span class="todo-count-72">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-72" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
