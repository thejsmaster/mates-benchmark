// client/components/Component0350.ts
// Benchmark component #350 — identical logic to TodoPage, scoped class names.
...
import "./Component0350.css";

type Filter = "all" | "active" | "done";

export const Component0350 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0350] mounted");
    return () => console.log("[Component0350] unmounted");
  });
  ...
  return () => html`
    <div class="app-350">
      <header class="header-350">
        <h1>todos — Component0350</h1>
      </header>

      <main class="main-350">
        <input
          class="add-input-350 ${error() ? "add-input--error-350" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-350" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-350">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-350">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-350 ${filter() === f ? "filter-btn--active-350" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-350">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-350">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-350">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-350 ${todo.done ? "todo-item--done-350" : ""}">
                    <button
                      class="todo-check-350 ${todo.done ? "todo-check--done-350" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-350" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-350"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-350" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-350" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-350">
            <span class="todo-count-350">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-350" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
