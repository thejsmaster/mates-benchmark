// client/components/Component0993.ts
// Benchmark component #993 — identical logic to TodoPage, scoped class names.
...
import "./Component0993.css";

type Filter = "all" | "active" | "done";

export const Component0993 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0993] mounted");
    return () => console.log("[Component0993] unmounted");
  });
  ...
  return () => html`
    <div class="app-993">
      <header class="header-993">
        <h1>todos — Component0993</h1>
      </header>

      <main class="main-993">
        <input
          class="add-input-993 ${error() ? "add-input--error-993" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-993" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-993">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-993">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-993 ${filter() === f ? "filter-btn--active-993" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-993">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-993">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-993">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-993 ${todo.done ? "todo-item--done-993" : ""}">
                    <button
                      class="todo-check-993 ${todo.done ? "todo-check--done-993" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-993" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-993"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-993" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-993" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-993">
            <span class="todo-count-993">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-993" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
