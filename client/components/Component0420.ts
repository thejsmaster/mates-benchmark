// client/components/Component0420.ts
// Benchmark component #420 — identical logic to TodoPage, scoped class names.
...
import "./Component0420.css";

type Filter = "all" | "active" | "done";

export const Component0420 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0420] mounted");
    return () => console.log("[Component0420] unmounted");
  });
  ...
  return () => html`
    <div class="app-420">
      <header class="header-420">
        <h1>todos — Component0420</h1>
      </header>

      <main class="main-420">
        <input
          class="add-input-420 ${error() ? "add-input--error-420" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-420" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-420">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-420">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-420 ${filter() === f ? "filter-btn--active-420" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-420">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-420">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-420">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-420 ${todo.done ? "todo-item--done-420" : ""}">
                    <button
                      class="todo-check-420 ${todo.done ? "todo-check--done-420" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-420" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-420"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-420" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-420" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-420">
            <span class="todo-count-420">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-420" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
