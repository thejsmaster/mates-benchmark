// client/components/Component0681.ts
// Benchmark component #681 — identical logic to TodoPage, scoped class names.
...
import "./Component0681.css";

type Filter = "all" | "active" | "done";

export const Component0681 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0681] mounted");
    return () => console.log("[Component0681] unmounted");
  });
  ...
  return () => html`
    <div class="app-681">
      <header class="header-681">
        <h1>todos — Component0681</h1>
      </header>

      <main class="main-681">
        <input
          class="add-input-681 ${error() ? "add-input--error-681" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-681" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-681">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-681">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-681 ${filter() === f ? "filter-btn--active-681" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-681">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-681">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-681">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-681 ${todo.done ? "todo-item--done-681" : ""}">
                    <button
                      class="todo-check-681 ${todo.done ? "todo-check--done-681" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-681" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-681"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-681" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-681" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-681">
            <span class="todo-count-681">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-681" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
