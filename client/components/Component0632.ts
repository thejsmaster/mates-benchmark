// client/components/Component0632.ts
// Benchmark component #632 — identical logic to TodoPage, scoped class names.
...
import "./Component0632.css";

type Filter = "all" | "active" | "done";

export const Component0632 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0632] mounted");
    return () => console.log("[Component0632] unmounted");
  });
  ...
  return () => html`
    <div class="app-632">
      <header class="header-632">
        <h1>todos — Component0632</h1>
      </header>

      <main class="main-632">
        <input
          class="add-input-632 ${error() ? "add-input--error-632" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-632" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-632">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-632">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-632 ${filter() === f ? "filter-btn--active-632" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-632">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-632">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-632">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-632 ${todo.done ? "todo-item--done-632" : ""}">
                    <button
                      class="todo-check-632 ${todo.done ? "todo-check--done-632" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-632" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-632"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-632" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-632" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-632">
            <span class="todo-count-632">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-632" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
