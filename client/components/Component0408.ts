// client/components/Component0408.ts
// Benchmark component #408 — identical logic to TodoPage, scoped class names.
...
import "./Component0408.css";

type Filter = "all" | "active" | "done";

export const Component0408 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0408] mounted");
    return () => console.log("[Component0408] unmounted");
  });
  ...
  return () => html`
    <div class="app-408">
      <header class="header-408">
        <h1>todos — Component0408</h1>
      </header>

      <main class="main-408">
        <input
          class="add-input-408 ${error() ? "add-input--error-408" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-408" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-408">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-408">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-408 ${filter() === f ? "filter-btn--active-408" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-408">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-408">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-408">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-408 ${todo.done ? "todo-item--done-408" : ""}">
                    <button
                      class="todo-check-408 ${todo.done ? "todo-check--done-408" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-408" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-408"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-408" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-408" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-408">
            <span class="todo-count-408">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-408" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
