// client/components/Component0056.ts
// Benchmark component #56 — identical logic to TodoPage, scoped class names.
...
import "./Component0056.css";

type Filter = "all" | "active" | "done";

export const Component0056 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0056] mounted");
    return () => console.log("[Component0056] unmounted");
  });
  ...
  return () => html`
    <div class="app-56">
      <header class="header-56">
        <h1>todos — Component0056</h1>
      </header>

      <main class="main-56">
        <input
          class="add-input-56 ${error() ? "add-input--error-56" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-56" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-56">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-56">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-56 ${filter() === f ? "filter-btn--active-56" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-56">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-56">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-56">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-56 ${todo.done ? "todo-item--done-56" : ""}">
                    <button
                      class="todo-check-56 ${todo.done ? "todo-check--done-56" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-56" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-56"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-56" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-56" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-56">
            <span class="todo-count-56">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-56" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
