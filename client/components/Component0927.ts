// client/components/Component0927.ts
// Benchmark component #927 — identical logic to TodoPage, scoped class names.
...
import "./Component0927.css";

type Filter = "all" | "active" | "done";

export const Component0927 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0927] mounted");
    return () => console.log("[Component0927] unmounted");
  });
  ...
  return () => html`
    <div class="app-927">
      <header class="header-927">
        <h1>todos — Component0927</h1>
      </header>

      <main class="main-927">
        <input
          class="add-input-927 ${error() ? "add-input--error-927" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-927" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-927">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-927">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-927 ${filter() === f ? "filter-btn--active-927" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-927">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-927">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-927">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-927 ${todo.done ? "todo-item--done-927" : ""}">
                    <button
                      class="todo-check-927 ${todo.done ? "todo-check--done-927" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-927" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-927"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-927" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-927" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-927">
            <span class="todo-count-927">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-927" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
