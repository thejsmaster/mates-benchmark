// client/components/Component0171.ts
// Benchmark component #171 — identical logic to TodoPage, scoped class names.
...
import "./Component0171.css";

type Filter = "all" | "active" | "done";

export const Component0171 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0171] mounted");
    return () => console.log("[Component0171] unmounted");
  });
  ...
  return () => html`
    <div class="app-171">
      <header class="header-171">
        <h1>todos — Component0171</h1>
      </header>

      <main class="main-171">
        <input
          class="add-input-171 ${error() ? "add-input--error-171" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-171" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-171">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-171">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-171 ${filter() === f ? "filter-btn--active-171" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-171">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-171">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-171">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-171 ${todo.done ? "todo-item--done-171" : ""}">
                    <button
                      class="todo-check-171 ${todo.done ? "todo-check--done-171" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-171" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-171"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-171" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-171" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-171">
            <span class="todo-count-171">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-171" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
