// client/components/Component0794.ts
// Benchmark component #794 — identical logic to TodoPage, scoped class names.
...
import "./Component0794.css";

type Filter = "all" | "active" | "done";

export const Component0794 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0794] mounted");
    return () => console.log("[Component0794] unmounted");
  });
  ...
  return () => html`
    <div class="app-794">
      <header class="header-794">
        <h1>todos — Component0794</h1>
      </header>

      <main class="main-794">
        <input
          class="add-input-794 ${error() ? "add-input--error-794" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-794" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-794">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-794">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-794 ${filter() === f ? "filter-btn--active-794" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-794">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-794">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-794">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-794 ${todo.done ? "todo-item--done-794" : ""}">
                    <button
                      class="todo-check-794 ${todo.done ? "todo-check--done-794" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-794" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-794"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-794" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-794" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-794">
            <span class="todo-count-794">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-794" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
