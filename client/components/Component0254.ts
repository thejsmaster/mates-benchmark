// client/components/Component0254.ts
// Benchmark component #254 — identical logic to TodoPage, scoped class names.
...
import "./Component0254.css";

type Filter = "all" | "active" | "done";

export const Component0254 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0254] mounted");
    return () => console.log("[Component0254] unmounted");
  });
  ...
  return () => html`
    <div class="app-254">
      <header class="header-254">
        <h1>todos — Component0254</h1>
      </header>

      <main class="main-254">
        <input
          class="add-input-254 ${error() ? "add-input--error-254" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-254" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-254">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-254">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-254 ${filter() === f ? "filter-btn--active-254" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-254">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-254">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-254">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-254 ${todo.done ? "todo-item--done-254" : ""}">
                    <button
                      class="todo-check-254 ${todo.done ? "todo-check--done-254" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-254" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-254"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-254" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-254" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-254">
            <span class="todo-count-254">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-254" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
