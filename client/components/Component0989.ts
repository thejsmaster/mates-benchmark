// client/components/Component0989.ts
// Benchmark component #989 — identical logic to TodoPage, scoped class names.
...
import "./Component0989.css";

type Filter = "all" | "active" | "done";

export const Component0989 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0989] mounted");
    return () => console.log("[Component0989] unmounted");
  });
  ...
  return () => html`
    <div class="app-989">
      <header class="header-989">
        <h1>todos — Component0989</h1>
      </header>

      <main class="main-989">
        <input
          class="add-input-989 ${error() ? "add-input--error-989" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-989" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-989">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-989">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-989 ${filter() === f ? "filter-btn--active-989" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-989">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-989">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-989">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-989 ${todo.done ? "todo-item--done-989" : ""}">
                    <button
                      class="todo-check-989 ${todo.done ? "todo-check--done-989" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-989" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-989"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-989" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-989" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-989">
            <span class="todo-count-989">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-989" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
