// client/components/Component0696.ts
// Benchmark component #696 — identical logic to TodoPage, scoped class names.
...
import "./Component0696.css";

type Filter = "all" | "active" | "done";

export const Component0696 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0696] mounted");
    return () => console.log("[Component0696] unmounted");
  });
  ...
  return () => html`
    <div class="app-696">
      <header class="header-696">
        <h1>todos — Component0696</h1>
      </header>

      <main class="main-696">
        <input
          class="add-input-696 ${error() ? "add-input--error-696" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-696" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-696">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-696">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-696 ${filter() === f ? "filter-btn--active-696" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-696">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-696">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-696">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-696 ${todo.done ? "todo-item--done-696" : ""}">
                    <button
                      class="todo-check-696 ${todo.done ? "todo-check--done-696" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-696" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-696"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-696" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-696" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-696">
            <span class="todo-count-696">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-696" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
