// client/components/Component0030.ts
// Benchmark component #30 — identical logic to TodoPage, scoped class names.
...
import "./Component0030.css";

type Filter = "all" | "active" | "done";

export const Component0030 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0030] mounted");
    return () => console.log("[Component0030] unmounted");
  });
  ...
  return () => html`
    <div class="app-30">
      <header class="header-30">
        <h1>todos — Component0030</h1>
      </header>

      <main class="main-30">
        <input
          class="add-input-30 ${error() ? "add-input--error-30" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-30" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-30">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-30">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-30 ${filter() === f ? "filter-btn--active-30" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-30">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-30">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-30">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-30 ${todo.done ? "todo-item--done-30" : ""}">
                    <button
                      class="todo-check-30 ${todo.done ? "todo-check--done-30" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-30" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-30"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-30" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-30" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-30">
            <span class="todo-count-30">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-30" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
