// client/components/Component0207.ts
// Benchmark component #207 — identical logic to TodoPage, scoped class names.
...
import "./Component0207.css";

type Filter = "all" | "active" | "done";

export const Component0207 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0207] mounted");
    return () => console.log("[Component0207] unmounted");
  });
  ...
  return () => html`
    <div class="app-207">
      <header class="header-207">
        <h1>todos — Component0207</h1>
      </header>

      <main class="main-207">
        <input
          class="add-input-207 ${error() ? "add-input--error-207" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-207" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-207">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-207">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-207 ${filter() === f ? "filter-btn--active-207" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-207">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-207">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-207">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-207 ${todo.done ? "todo-item--done-207" : ""}">
                    <button
                      class="todo-check-207 ${todo.done ? "todo-check--done-207" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-207" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-207"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-207" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-207" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-207">
            <span class="todo-count-207">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-207" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
