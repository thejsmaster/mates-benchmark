// client/components/Component0453.ts
// Benchmark component #453 — identical logic to TodoPage, scoped class names.
...
import "./Component0453.css";

type Filter = "all" | "active" | "done";

export const Component0453 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0453] mounted");
    return () => console.log("[Component0453] unmounted");
  });
  ...
  return () => html`
    <div class="app-453">
      <header class="header-453">
        <h1>todos — Component0453</h1>
      </header>

      <main class="main-453">
        <input
          class="add-input-453 ${error() ? "add-input--error-453" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-453" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-453">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-453">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-453 ${filter() === f ? "filter-btn--active-453" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-453">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-453">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-453">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-453 ${todo.done ? "todo-item--done-453" : ""}">
                    <button
                      class="todo-check-453 ${todo.done ? "todo-check--done-453" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-453" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-453"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-453" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-453" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-453">
            <span class="todo-count-453">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-453" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
