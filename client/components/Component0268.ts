// client/components/Component0268.ts
// Benchmark component #268 — identical logic to TodoPage, scoped class names.
...
import "./Component0268.css";

type Filter = "all" | "active" | "done";

export const Component0268 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0268] mounted");
    return () => console.log("[Component0268] unmounted");
  });
  ...
  return () => html`
    <div class="app-268">
      <header class="header-268">
        <h1>todos — Component0268</h1>
      </header>

      <main class="main-268">
        <input
          class="add-input-268 ${error() ? "add-input--error-268" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-268" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-268">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-268">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-268 ${filter() === f ? "filter-btn--active-268" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-268">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-268">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-268">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-268 ${todo.done ? "todo-item--done-268" : ""}">
                    <button
                      class="todo-check-268 ${todo.done ? "todo-check--done-268" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-268" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-268"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-268" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-268" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-268">
            <span class="todo-count-268">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-268" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
