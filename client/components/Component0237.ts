// client/components/Component0237.ts
// Benchmark component #237 — identical logic to TodoPage, scoped class names.
...
import "./Component0237.css";

type Filter = "all" | "active" | "done";

export const Component0237 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0237] mounted");
    return () => console.log("[Component0237] unmounted");
  });
  ...
  return () => html`
    <div class="app-237">
      <header class="header-237">
        <h1>todos — Component0237</h1>
      </header>

      <main class="main-237">
        <input
          class="add-input-237 ${error() ? "add-input--error-237" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-237" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-237">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-237">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-237 ${filter() === f ? "filter-btn--active-237" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-237">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-237">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-237">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-237 ${todo.done ? "todo-item--done-237" : ""}">
                    <button
                      class="todo-check-237 ${todo.done ? "todo-check--done-237" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-237" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-237"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-237" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-237" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-237">
            <span class="todo-count-237">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-237" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
