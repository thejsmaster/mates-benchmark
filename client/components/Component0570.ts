// client/components/Component0570.ts
// Benchmark component #570 — identical logic to TodoPage, scoped class names.
...
import "./Component0570.css";

type Filter = "all" | "active" | "done";

export const Component0570 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0570] mounted");
    return () => console.log("[Component0570] unmounted");
  });
  ...
  return () => html`
    <div class="app-570">
      <header class="header-570">
        <h1>todos — Component0570</h1>
      </header>

      <main class="main-570">
        <input
          class="add-input-570 ${error() ? "add-input--error-570" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-570" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-570">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-570">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-570 ${filter() === f ? "filter-btn--active-570" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-570">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-570">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-570">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-570 ${todo.done ? "todo-item--done-570" : ""}">
                    <button
                      class="todo-check-570 ${todo.done ? "todo-check--done-570" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-570" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-570"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-570" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-570" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-570">
            <span class="todo-count-570">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-570" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
