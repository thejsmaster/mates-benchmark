// client/components/Component0034.ts
// Benchmark component #34 — identical logic to TodoPage, scoped class names.
...
import "./Component0034.css";

type Filter = "all" | "active" | "done";

export const Component0034 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0034] mounted");
    return () => console.log("[Component0034] unmounted");
  });
  ...
  return () => html`
    <div class="app-34">
      <header class="header-34">
        <h1>todos — Component0034</h1>
      </header>

      <main class="main-34">
        <input
          class="add-input-34 ${error() ? "add-input--error-34" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-34" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-34">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-34">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-34 ${filter() === f ? "filter-btn--active-34" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-34">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-34">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-34">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-34 ${todo.done ? "todo-item--done-34" : ""}">
                    <button
                      class="todo-check-34 ${todo.done ? "todo-check--done-34" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-34" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-34"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-34" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-34" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-34">
            <span class="todo-count-34">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-34" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
