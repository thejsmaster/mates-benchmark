// client/components/Component0265.ts
// Benchmark component #265 — identical logic to TodoPage, scoped class names.
...
import "./Component0265.css";

type Filter = "all" | "active" | "done";

export const Component0265 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0265] mounted");
    return () => console.log("[Component0265] unmounted");
  });
  ...
  return () => html`
    <div class="app-265">
      <header class="header-265">
        <h1>todos — Component0265</h1>
      </header>

      <main class="main-265">
        <input
          class="add-input-265 ${error() ? "add-input--error-265" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-265" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-265">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-265">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-265 ${filter() === f ? "filter-btn--active-265" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-265">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-265">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-265">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-265 ${todo.done ? "todo-item--done-265" : ""}">
                    <button
                      class="todo-check-265 ${todo.done ? "todo-check--done-265" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-265" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-265"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-265" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-265" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-265">
            <span class="todo-count-265">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-265" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
