// client/components/Component0728.ts
// Benchmark component #728 — identical logic to TodoPage, scoped class names.
...
import "./Component0728.css";

type Filter = "all" | "active" | "done";

export const Component0728 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0728] mounted");
    return () => console.log("[Component0728] unmounted");
  });
  ...
  return () => html`
    <div class="app-728">
      <header class="header-728">
        <h1>todos — Component0728</h1>
      </header>

      <main class="main-728">
        <input
          class="add-input-728 ${error() ? "add-input--error-728" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-728" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-728">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-728">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-728 ${filter() === f ? "filter-btn--active-728" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-728">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-728">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-728">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-728 ${todo.done ? "todo-item--done-728" : ""}">
                    <button
                      class="todo-check-728 ${todo.done ? "todo-check--done-728" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-728" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-728"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-728" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-728" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-728">
            <span class="todo-count-728">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-728" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
