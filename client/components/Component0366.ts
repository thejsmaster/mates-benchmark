// client/components/Component0366.ts
// Benchmark component #366 — identical logic to TodoPage, scoped class names.
...
import "./Component0366.css";

type Filter = "all" | "active" | "done";

export const Component0366 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0366] mounted");
    return () => console.log("[Component0366] unmounted");
  });
  ...
  return () => html`
    <div class="app-366">
      <header class="header-366">
        <h1>todos — Component0366</h1>
      </header>

      <main class="main-366">
        <input
          class="add-input-366 ${error() ? "add-input--error-366" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-366" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-366">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-366">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-366 ${filter() === f ? "filter-btn--active-366" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-366">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-366">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-366">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-366 ${todo.done ? "todo-item--done-366" : ""}">
                    <button
                      class="todo-check-366 ${todo.done ? "todo-check--done-366" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-366" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-366"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-366" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-366" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-366">
            <span class="todo-count-366">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-366" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
