// client/components/Component0980.ts
// Benchmark component #980 — identical logic to TodoPage, scoped class names.
...
import "./Component0980.css";

type Filter = "all" | "active" | "done";

export const Component0980 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0980] mounted");
    return () => console.log("[Component0980] unmounted");
  });
  ...
  return () => html`
    <div class="app-980">
      <header class="header-980">
        <h1>todos — Component0980</h1>
      </header>

      <main class="main-980">
        <input
          class="add-input-980 ${error() ? "add-input--error-980" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-980" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-980">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-980">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-980 ${filter() === f ? "filter-btn--active-980" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-980">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-980">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-980">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-980 ${todo.done ? "todo-item--done-980" : ""}">
                    <button
                      class="todo-check-980 ${todo.done ? "todo-check--done-980" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-980" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-980"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-980" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-980" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-980">
            <span class="todo-count-980">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-980" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
