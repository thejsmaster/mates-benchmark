// client/components/Component0004.ts
// Benchmark component #4 — identical logic to TodoPage, scoped class names.
...
import "./Component0004.css";

type Filter = "all" | "active" | "done";

export const Component0004 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0004] mounted");
    return () => console.log("[Component0004] unmounted");
  });
  ...
  return () => html`
    <div class="app-4">
      <header class="header-4">
        <h1>todos — Component0004</h1>
      </header>

      <main class="main-4">
        <input
          class="add-input-4 ${error() ? "add-input--error-4" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-4" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-4">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-4">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-4 ${filter() === f ? "filter-btn--active-4" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-4">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-4">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-4">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-4 ${todo.done ? "todo-item--done-4" : ""}">
                    <button
                      class="todo-check-4 ${todo.done ? "todo-check--done-4" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-4" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-4"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-4" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-4" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-4">
            <span class="todo-count-4">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-4" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
