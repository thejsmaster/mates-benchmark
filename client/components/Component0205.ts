// client/components/Component0205.ts
// Benchmark component #205 — identical logic to TodoPage, scoped class names.
...
import "./Component0205.css";

type Filter = "all" | "active" | "done";

export const Component0205 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0205] mounted");
    return () => console.log("[Component0205] unmounted");
  });
  ...
  return () => html`
    <div class="app-205">
      <header class="header-205">
        <h1>todos — Component0205</h1>
      </header>

      <main class="main-205">
        <input
          class="add-input-205 ${error() ? "add-input--error-205" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-205" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-205">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-205">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-205 ${filter() === f ? "filter-btn--active-205" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-205">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-205">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-205">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-205 ${todo.done ? "todo-item--done-205" : ""}">
                    <button
                      class="todo-check-205 ${todo.done ? "todo-check--done-205" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-205" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-205"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-205" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-205" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-205">
            <span class="todo-count-205">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-205" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
