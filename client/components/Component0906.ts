// client/components/Component0906.ts
// Benchmark component #906 — identical logic to TodoPage, scoped class names.
...
import "./Component0906.css";

type Filter = "all" | "active" | "done";

export const Component0906 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0906] mounted");
    return () => console.log("[Component0906] unmounted");
  });
  ...
  return () => html`
    <div class="app-906">
      <header class="header-906">
        <h1>todos — Component0906</h1>
      </header>

      <main class="main-906">
        <input
          class="add-input-906 ${error() ? "add-input--error-906" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-906" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-906">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-906">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-906 ${filter() === f ? "filter-btn--active-906" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-906">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-906">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-906">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-906 ${todo.done ? "todo-item--done-906" : ""}">
                    <button
                      class="todo-check-906 ${todo.done ? "todo-check--done-906" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-906" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-906"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-906" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-906" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-906">
            <span class="todo-count-906">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-906" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
