// client/components/Component0592.ts
// Benchmark component #592 — identical logic to TodoPage, scoped class names.
...
import "./Component0592.css";

type Filter = "all" | "active" | "done";

export const Component0592 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0592] mounted");
    return () => console.log("[Component0592] unmounted");
  });
  ...
  return () => html`
    <div class="app-592">
      <header class="header-592">
        <h1>todos — Component0592</h1>
      </header>

      <main class="main-592">
        <input
          class="add-input-592 ${error() ? "add-input--error-592" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-592" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-592">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-592">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-592 ${filter() === f ? "filter-btn--active-592" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-592">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-592">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-592">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-592 ${todo.done ? "todo-item--done-592" : ""}">
                    <button
                      class="todo-check-592 ${todo.done ? "todo-check--done-592" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-592" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-592"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-592" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-592" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-592">
            <span class="todo-count-592">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-592" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
