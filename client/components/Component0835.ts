// client/components/Component0835.ts
// Benchmark component #835 — identical logic to TodoPage, scoped class names.
...
import "./Component0835.css";

type Filter = "all" | "active" | "done";

export const Component0835 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0835] mounted");
    return () => console.log("[Component0835] unmounted");
  });
  ...
  return () => html`
    <div class="app-835">
      <header class="header-835">
        <h1>todos — Component0835</h1>
      </header>

      <main class="main-835">
        <input
          class="add-input-835 ${error() ? "add-input--error-835" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-835" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-835">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-835">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-835 ${filter() === f ? "filter-btn--active-835" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-835">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-835">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-835">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-835 ${todo.done ? "todo-item--done-835" : ""}">
                    <button
                      class="todo-check-835 ${todo.done ? "todo-check--done-835" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-835" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-835"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-835" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-835" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-835">
            <span class="todo-count-835">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-835" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
