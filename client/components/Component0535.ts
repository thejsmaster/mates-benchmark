// client/components/Component0535.ts
// Benchmark component #535 — identical logic to TodoPage, scoped class names.
...
import "./Component0535.css";

type Filter = "all" | "active" | "done";

export const Component0535 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0535] mounted");
    return () => console.log("[Component0535] unmounted");
  });
  ...
  return () => html`
    <div class="app-535">
      <header class="header-535">
        <h1>todos — Component0535</h1>
      </header>

      <main class="main-535">
        <input
          class="add-input-535 ${error() ? "add-input--error-535" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-535" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-535">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-535">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-535 ${filter() === f ? "filter-btn--active-535" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-535">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-535">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-535">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-535 ${todo.done ? "todo-item--done-535" : ""}">
                    <button
                      class="todo-check-535 ${todo.done ? "todo-check--done-535" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-535" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-535"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-535" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-535" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-535">
            <span class="todo-count-535">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-535" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
