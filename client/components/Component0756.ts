// client/components/Component0756.ts
// Benchmark component #756 — identical logic to TodoPage, scoped class names.
...
import "./Component0756.css";

type Filter = "all" | "active" | "done";

export const Component0756 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0756] mounted");
    return () => console.log("[Component0756] unmounted");
  });
  ...
  return () => html`
    <div class="app-756">
      <header class="header-756">
        <h1>todos — Component0756</h1>
      </header>

      <main class="main-756">
        <input
          class="add-input-756 ${error() ? "add-input--error-756" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-756" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-756">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-756">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-756 ${filter() === f ? "filter-btn--active-756" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-756">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-756">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-756">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-756 ${todo.done ? "todo-item--done-756" : ""}">
                    <button
                      class="todo-check-756 ${todo.done ? "todo-check--done-756" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-756" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-756"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-756" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-756" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-756">
            <span class="todo-count-756">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-756" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
