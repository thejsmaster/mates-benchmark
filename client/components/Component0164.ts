// client/components/Component0164.ts
// Benchmark component #164 — identical logic to TodoPage, scoped class names.
...
import "./Component0164.css";

type Filter = "all" | "active" | "done";

export const Component0164 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0164] mounted");
    return () => console.log("[Component0164] unmounted");
  });
  ...
  return () => html`
    <div class="app-164">
      <header class="header-164">
        <h1>todos — Component0164</h1>
      </header>

      <main class="main-164">
        <input
          class="add-input-164 ${error() ? "add-input--error-164" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-164" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-164">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-164">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-164 ${filter() === f ? "filter-btn--active-164" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-164">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-164">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-164">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-164 ${todo.done ? "todo-item--done-164" : ""}">
                    <button
                      class="todo-check-164 ${todo.done ? "todo-check--done-164" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-164" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-164"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-164" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-164" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-164">
            <span class="todo-count-164">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-164" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
