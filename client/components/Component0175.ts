// client/components/Component0175.ts
// Benchmark component #175 — identical logic to TodoPage, scoped class names.
...
import "./Component0175.css";

type Filter = "all" | "active" | "done";

export const Component0175 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0175] mounted");
    return () => console.log("[Component0175] unmounted");
  });
  ...
  return () => html`
    <div class="app-175">
      <header class="header-175">
        <h1>todos — Component0175</h1>
      </header>

      <main class="main-175">
        <input
          class="add-input-175 ${error() ? "add-input--error-175" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-175" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-175">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-175">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-175 ${filter() === f ? "filter-btn--active-175" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-175">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-175">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-175">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-175 ${todo.done ? "todo-item--done-175" : ""}">
                    <button
                      class="todo-check-175 ${todo.done ? "todo-check--done-175" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-175" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-175"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-175" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-175" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-175">
            <span class="todo-count-175">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-175" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
