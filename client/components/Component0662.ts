// client/components/Component0662.ts
// Benchmark component #662 — identical logic to TodoPage, scoped class names.
...
import "./Component0662.css";

type Filter = "all" | "active" | "done";

export const Component0662 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0662] mounted");
    return () => console.log("[Component0662] unmounted");
  });
  ...
  return () => html`
    <div class="app-662">
      <header class="header-662">
        <h1>todos — Component0662</h1>
      </header>

      <main class="main-662">
        <input
          class="add-input-662 ${error() ? "add-input--error-662" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-662" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-662">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-662">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-662 ${filter() === f ? "filter-btn--active-662" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-662">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-662">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-662">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-662 ${todo.done ? "todo-item--done-662" : ""}">
                    <button
                      class="todo-check-662 ${todo.done ? "todo-check--done-662" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-662" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-662"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-662" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-662" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-662">
            <span class="todo-count-662">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-662" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
