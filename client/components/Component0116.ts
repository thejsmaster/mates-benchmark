// client/components/Component0116.ts
// Benchmark component #116 — identical logic to TodoPage, scoped class names.
...
import "./Component0116.css";

type Filter = "all" | "active" | "done";

export const Component0116 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0116] mounted");
    return () => console.log("[Component0116] unmounted");
  });
  ...
  return () => html`
    <div class="app-116">
      <header class="header-116">
        <h1>todos — Component0116</h1>
      </header>

      <main class="main-116">
        <input
          class="add-input-116 ${error() ? "add-input--error-116" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-116" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-116">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-116">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-116 ${filter() === f ? "filter-btn--active-116" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-116">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-116">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-116">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-116 ${todo.done ? "todo-item--done-116" : ""}">
                    <button
                      class="todo-check-116 ${todo.done ? "todo-check--done-116" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-116" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-116"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-116" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-116" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-116">
            <span class="todo-count-116">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-116" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
