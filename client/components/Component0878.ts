// client/components/Component0878.ts
// Benchmark component #878 — identical logic to TodoPage, scoped class names.
...
import "./Component0878.css";

type Filter = "all" | "active" | "done";

export const Component0878 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0878] mounted");
    return () => console.log("[Component0878] unmounted");
  });
  ...
  return () => html`
    <div class="app-878">
      <header class="header-878">
        <h1>todos — Component0878</h1>
      </header>

      <main class="main-878">
        <input
          class="add-input-878 ${error() ? "add-input--error-878" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-878" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-878">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-878">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-878 ${filter() === f ? "filter-btn--active-878" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-878">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-878">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-878">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-878 ${todo.done ? "todo-item--done-878" : ""}">
                    <button
                      class="todo-check-878 ${todo.done ? "todo-check--done-878" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-878" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-878"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-878" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-878" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-878">
            <span class="todo-count-878">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-878" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
