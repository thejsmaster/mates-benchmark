// client/components/Component0227.ts
// Benchmark component #227 — identical logic to TodoPage, scoped class names.
...
import "./Component0227.css";

type Filter = "all" | "active" | "done";

export const Component0227 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0227] mounted");
    return () => console.log("[Component0227] unmounted");
  });
  ...
  return () => html`
    <div class="app-227">
      <header class="header-227">
        <h1>todos — Component0227</h1>
      </header>

      <main class="main-227">
        <input
          class="add-input-227 ${error() ? "add-input--error-227" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-227" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-227">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-227">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-227 ${filter() === f ? "filter-btn--active-227" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-227">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-227">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-227">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-227 ${todo.done ? "todo-item--done-227" : ""}">
                    <button
                      class="todo-check-227 ${todo.done ? "todo-check--done-227" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-227" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-227"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-227" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-227" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-227">
            <span class="todo-count-227">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-227" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
