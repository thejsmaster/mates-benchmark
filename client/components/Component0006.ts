// client/components/Component0006.ts
// Benchmark component #6 — identical logic to TodoPage, scoped class names.
...
import "./Component0006.css";

type Filter = "all" | "active" | "done";

export const Component0006 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0006] mounted");
    return () => console.log("[Component0006] unmounted");
  });
  ...
  return () => html`
    <div class="app-6">
      <header class="header-6">
        <h1>todos — Component0006</h1>
      </header>

      <main class="main-6">
        <input
          class="add-input-6 ${error() ? "add-input--error-6" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-6" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-6">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-6">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-6 ${filter() === f ? "filter-btn--active-6" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-6">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-6">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-6">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-6 ${todo.done ? "todo-item--done-6" : ""}">
                    <button
                      class="todo-check-6 ${todo.done ? "todo-check--done-6" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-6" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-6"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-6" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-6" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-6">
            <span class="todo-count-6">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-6" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
