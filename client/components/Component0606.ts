// client/components/Component0606.ts
// Benchmark component #606 — identical logic to TodoPage, scoped class names.
...
import "./Component0606.css";

type Filter = "all" | "active" | "done";

export const Component0606 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0606] mounted");
    return () => console.log("[Component0606] unmounted");
  });
  ...
  return () => html`
    <div class="app-606">
      <header class="header-606">
        <h1>todos — Component0606</h1>
      </header>

      <main class="main-606">
        <input
          class="add-input-606 ${error() ? "add-input--error-606" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-606" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-606">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-606">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-606 ${filter() === f ? "filter-btn--active-606" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-606">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-606">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-606">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-606 ${todo.done ? "todo-item--done-606" : ""}">
                    <button
                      class="todo-check-606 ${todo.done ? "todo-check--done-606" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-606" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-606"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-606" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-606" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-606">
            <span class="todo-count-606">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-606" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
