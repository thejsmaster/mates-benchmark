// client/components/Component0213.ts
// Benchmark component #213 — identical logic to TodoPage, scoped class names.
...
import "./Component0213.css";

type Filter = "all" | "active" | "done";

export const Component0213 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0213] mounted");
    return () => console.log("[Component0213] unmounted");
  });
  ...
  return () => html`
    <div class="app-213">
      <header class="header-213">
        <h1>todos — Component0213</h1>
      </header>

      <main class="main-213">
        <input
          class="add-input-213 ${error() ? "add-input--error-213" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-213" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-213">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-213">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-213 ${filter() === f ? "filter-btn--active-213" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-213">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-213">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-213">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-213 ${todo.done ? "todo-item--done-213" : ""}">
                    <button
                      class="todo-check-213 ${todo.done ? "todo-check--done-213" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-213" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-213"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-213" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-213" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-213">
            <span class="todo-count-213">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-213" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
