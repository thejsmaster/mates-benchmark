// client/components/Component0639.ts
// Benchmark component #639 — identical logic to TodoPage, scoped class names.
...
import "./Component0639.css";

type Filter = "all" | "active" | "done";

export const Component0639 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0639] mounted");
    return () => console.log("[Component0639] unmounted");
  });
  ...
  return () => html`
    <div class="app-639">
      <header class="header-639">
        <h1>todos — Component0639</h1>
      </header>

      <main class="main-639">
        <input
          class="add-input-639 ${error() ? "add-input--error-639" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-639" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-639">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-639">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-639 ${filter() === f ? "filter-btn--active-639" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-639">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-639">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-639">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-639 ${todo.done ? "todo-item--done-639" : ""}">
                    <button
                      class="todo-check-639 ${todo.done ? "todo-check--done-639" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-639" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-639"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-639" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-639" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-639">
            <span class="todo-count-639">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-639" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
