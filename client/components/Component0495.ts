// client/components/Component0495.ts
// Benchmark component #495 — identical logic to TodoPage, scoped class names.
...
import "./Component0495.css";

type Filter = "all" | "active" | "done";

export const Component0495 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0495] mounted");
    return () => console.log("[Component0495] unmounted");
  });
  ...
  return () => html`
    <div class="app-495">
      <header class="header-495">
        <h1>todos — Component0495</h1>
      </header>

      <main class="main-495">
        <input
          class="add-input-495 ${error() ? "add-input--error-495" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-495" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-495">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-495">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-495 ${filter() === f ? "filter-btn--active-495" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-495">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-495">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-495">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-495 ${todo.done ? "todo-item--done-495" : ""}">
                    <button
                      class="todo-check-495 ${todo.done ? "todo-check--done-495" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-495" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-495"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-495" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-495" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-495">
            <span class="todo-count-495">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-495" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
