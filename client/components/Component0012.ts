// client/components/Component0012.ts
// Benchmark component #12 — identical logic to TodoPage, scoped class names.
...
import "./Component0012.css";

type Filter = "all" | "active" | "done";

export const Component0012 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0012] mounted");
    return () => console.log("[Component0012] unmounted");
  });
  ...
  return () => html`
    <div class="app-12">
      <header class="header-12">
        <h1>todos — Component0012</h1>
      </header>

      <main class="main-12">
        <input
          class="add-input-12 ${error() ? "add-input--error-12" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-12" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-12">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-12">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-12 ${filter() === f ? "filter-btn--active-12" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-12">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-12">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-12">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-12 ${todo.done ? "todo-item--done-12" : ""}">
                    <button
                      class="todo-check-12 ${todo.done ? "todo-check--done-12" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-12" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-12"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-12" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-12" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-12">
            <span class="todo-count-12">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-12" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
