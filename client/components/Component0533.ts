// client/components/Component0533.ts
// Benchmark component #533 — identical logic to TodoPage, scoped class names.
...
import "./Component0533.css";

type Filter = "all" | "active" | "done";

export const Component0533 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0533] mounted");
    return () => console.log("[Component0533] unmounted");
  });
  ...
  return () => html`
    <div class="app-533">
      <header class="header-533">
        <h1>todos — Component0533</h1>
      </header>

      <main class="main-533">
        <input
          class="add-input-533 ${error() ? "add-input--error-533" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-533" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-533">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-533">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-533 ${filter() === f ? "filter-btn--active-533" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-533">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-533">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-533">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-533 ${todo.done ? "todo-item--done-533" : ""}">
                    <button
                      class="todo-check-533 ${todo.done ? "todo-check--done-533" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-533" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-533"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-533" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-533" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-533">
            <span class="todo-count-533">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-533" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
