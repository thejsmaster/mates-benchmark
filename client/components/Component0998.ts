// client/components/Component0998.ts
// Benchmark component #998 — identical logic to TodoPage, scoped class names.
...
import "./Component0998.css";

type Filter = "all" | "active" | "done";

export const Component0998 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0998] mounted");
    return () => console.log("[Component0998] unmounted");
  });
  ...
  return () => html`
    <div class="app-998">
      <header class="header-998">
        <h1>todos — Component0998</h1>
      </header>

      <main class="main-998">
        <input
          class="add-input-998 ${error() ? "add-input--error-998" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-998" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-998">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-998">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-998 ${filter() === f ? "filter-btn--active-998" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-998">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-998">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-998">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-998 ${todo.done ? "todo-item--done-998" : ""}">
                    <button
                      class="todo-check-998 ${todo.done ? "todo-check--done-998" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-998" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-998"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-998" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-998" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-998">
            <span class="todo-count-998">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-998" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
