// client/components/Component0990.ts
// Benchmark component #990 — identical logic to TodoPage, scoped class names.
...
import "./Component0990.css";

type Filter = "all" | "active" | "done";

export const Component0990 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0990] mounted");
    return () => console.log("[Component0990] unmounted");
  });
  ...
  return () => html`
    <div class="app-990">
      <header class="header-990">
        <h1>todos — Component0990</h1>
      </header>

      <main class="main-990">
        <input
          class="add-input-990 ${error() ? "add-input--error-990" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-990" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-990">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-990">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-990 ${filter() === f ? "filter-btn--active-990" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-990">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-990">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-990">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-990 ${todo.done ? "todo-item--done-990" : ""}">
                    <button
                      class="todo-check-990 ${todo.done ? "todo-check--done-990" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-990" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-990"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-990" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-990" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-990">
            <span class="todo-count-990">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-990" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
