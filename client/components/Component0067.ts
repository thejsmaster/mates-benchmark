// client/components/Component0067.ts
// Benchmark component #67 — identical logic to TodoPage, scoped class names.
...
import "./Component0067.css";

type Filter = "all" | "active" | "done";

export const Component0067 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0067] mounted");
    return () => console.log("[Component0067] unmounted");
  });
  ...
  return () => html`
    <div class="app-67">
      <header class="header-67">
        <h1>todos — Component0067</h1>
      </header>

      <main class="main-67">
        <input
          class="add-input-67 ${error() ? "add-input--error-67" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-67" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-67">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-67">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-67 ${filter() === f ? "filter-btn--active-67" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-67">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-67">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-67">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-67 ${todo.done ? "todo-item--done-67" : ""}">
                    <button
                      class="todo-check-67 ${todo.done ? "todo-check--done-67" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-67" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-67"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-67" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-67" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-67">
            <span class="todo-count-67">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-67" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
