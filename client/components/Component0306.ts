// client/components/Component0306.ts
// Benchmark component #306 — identical logic to TodoPage, scoped class names.
...
import "./Component0306.css";

type Filter = "all" | "active" | "done";

export const Component0306 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0306] mounted");
    return () => console.log("[Component0306] unmounted");
  });
  ...
  return () => html`
    <div class="app-306">
      <header class="header-306">
        <h1>todos — Component0306</h1>
      </header>

      <main class="main-306">
        <input
          class="add-input-306 ${error() ? "add-input--error-306" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-306" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-306">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-306">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-306 ${filter() === f ? "filter-btn--active-306" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-306">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-306">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-306">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-306 ${todo.done ? "todo-item--done-306" : ""}">
                    <button
                      class="todo-check-306 ${todo.done ? "todo-check--done-306" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-306" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-306"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-306" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-306" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-306">
            <span class="todo-count-306">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-306" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
