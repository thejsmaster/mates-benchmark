// client/components/Component0315.ts
// Benchmark component #315 — identical logic to TodoPage, scoped class names.
...
import "./Component0315.css";

type Filter = "all" | "active" | "done";

export const Component0315 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0315] mounted");
    return () => console.log("[Component0315] unmounted");
  });
  ...
  return () => html`
    <div class="app-315">
      <header class="header-315">
        <h1>todos — Component0315</h1>
      </header>

      <main class="main-315">
        <input
          class="add-input-315 ${error() ? "add-input--error-315" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-315" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-315">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-315">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-315 ${filter() === f ? "filter-btn--active-315" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-315">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-315">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-315">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-315 ${todo.done ? "todo-item--done-315" : ""}">
                    <button
                      class="todo-check-315 ${todo.done ? "todo-check--done-315" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-315" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-315"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-315" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-315" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-315">
            <span class="todo-count-315">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-315" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
