// client/components/Component0875.ts
// Benchmark component #875 — identical logic to TodoPage, scoped class names.
...
import "./Component0875.css";

type Filter = "all" | "active" | "done";

export const Component0875 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0875] mounted");
    return () => console.log("[Component0875] unmounted");
  });
  ...
  return () => html`
    <div class="app-875">
      <header class="header-875">
        <h1>todos — Component0875</h1>
      </header>

      <main class="main-875">
        <input
          class="add-input-875 ${error() ? "add-input--error-875" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-875" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-875">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-875">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-875 ${filter() === f ? "filter-btn--active-875" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-875">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-875">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-875">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-875 ${todo.done ? "todo-item--done-875" : ""}">
                    <button
                      class="todo-check-875 ${todo.done ? "todo-check--done-875" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-875" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-875"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-875" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-875" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-875">
            <span class="todo-count-875">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-875" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
