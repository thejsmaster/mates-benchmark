// client/components/Component0189.ts
// Benchmark component #189 — identical logic to TodoPage, scoped class names.
...
import "./Component0189.css";

type Filter = "all" | "active" | "done";

export const Component0189 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0189] mounted");
    return () => console.log("[Component0189] unmounted");
  });
  ...
  return () => html`
    <div class="app-189">
      <header class="header-189">
        <h1>todos — Component0189</h1>
      </header>

      <main class="main-189">
        <input
          class="add-input-189 ${error() ? "add-input--error-189" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-189" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-189">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-189">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-189 ${filter() === f ? "filter-btn--active-189" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-189">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-189">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-189">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-189 ${todo.done ? "todo-item--done-189" : ""}">
                    <button
                      class="todo-check-189 ${todo.done ? "todo-check--done-189" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-189" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-189"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-189" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-189" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-189">
            <span class="todo-count-189">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-189" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
