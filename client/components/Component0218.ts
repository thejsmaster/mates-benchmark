// client/components/Component0218.ts
// Benchmark component #218 — identical logic to TodoPage, scoped class names.
...
import "./Component0218.css";

type Filter = "all" | "active" | "done";

export const Component0218 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0218] mounted");
    return () => console.log("[Component0218] unmounted");
  });
  ...
  return () => html`
    <div class="app-218">
      <header class="header-218">
        <h1>todos — Component0218</h1>
      </header>

      <main class="main-218">
        <input
          class="add-input-218 ${error() ? "add-input--error-218" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-218" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-218">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-218">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-218 ${filter() === f ? "filter-btn--active-218" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-218">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-218">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-218">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-218 ${todo.done ? "todo-item--done-218" : ""}">
                    <button
                      class="todo-check-218 ${todo.done ? "todo-check--done-218" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-218" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-218"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-218" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-218" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-218">
            <span class="todo-count-218">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-218" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
