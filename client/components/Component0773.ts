// client/components/Component0773.ts
// Benchmark component #773 — identical logic to TodoPage, scoped class names.
...
import "./Component0773.css";

type Filter = "all" | "active" | "done";

export const Component0773 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0773] mounted");
    return () => console.log("[Component0773] unmounted");
  });
  ...
  return () => html`
    <div class="app-773">
      <header class="header-773">
        <h1>todos — Component0773</h1>
      </header>

      <main class="main-773">
        <input
          class="add-input-773 ${error() ? "add-input--error-773" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-773" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-773">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-773">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-773 ${filter() === f ? "filter-btn--active-773" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-773">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-773">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-773">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-773 ${todo.done ? "todo-item--done-773" : ""}">
                    <button
                      class="todo-check-773 ${todo.done ? "todo-check--done-773" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-773" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-773"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-773" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-773" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-773">
            <span class="todo-count-773">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-773" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
