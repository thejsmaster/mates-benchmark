// client/components/Component0717.ts
// Benchmark component #717 — identical logic to TodoPage, scoped class names.
...
import "./Component0717.css";

type Filter = "all" | "active" | "done";

export const Component0717 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0717] mounted");
    return () => console.log("[Component0717] unmounted");
  });
  ...
  return () => html`
    <div class="app-717">
      <header class="header-717">
        <h1>todos — Component0717</h1>
      </header>

      <main class="main-717">
        <input
          class="add-input-717 ${error() ? "add-input--error-717" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-717" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-717">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-717">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-717 ${filter() === f ? "filter-btn--active-717" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-717">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-717">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-717">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-717 ${todo.done ? "todo-item--done-717" : ""}">
                    <button
                      class="todo-check-717 ${todo.done ? "todo-check--done-717" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-717" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-717"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-717" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-717" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-717">
            <span class="todo-count-717">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-717" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
