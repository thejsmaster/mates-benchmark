// client/components/Component0337.ts
// Benchmark component #337 — identical logic to TodoPage, scoped class names.
...
import "./Component0337.css";

type Filter = "all" | "active" | "done";

export const Component0337 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0337] mounted");
    return () => console.log("[Component0337] unmounted");
  });
  ...
  return () => html`
    <div class="app-337">
      <header class="header-337">
        <h1>todos — Component0337</h1>
      </header>

      <main class="main-337">
        <input
          class="add-input-337 ${error() ? "add-input--error-337" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-337" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-337">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-337">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-337 ${filter() === f ? "filter-btn--active-337" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-337">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-337">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-337">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-337 ${todo.done ? "todo-item--done-337" : ""}">
                    <button
                      class="todo-check-337 ${todo.done ? "todo-check--done-337" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-337" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-337"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-337" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-337" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-337">
            <span class="todo-count-337">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-337" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
