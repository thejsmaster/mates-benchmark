// client/components/Component0772.ts
// Benchmark component #772 — identical logic to TodoPage, scoped class names.
...
import "./Component0772.css";

type Filter = "all" | "active" | "done";

export const Component0772 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0772] mounted");
    return () => console.log("[Component0772] unmounted");
  });
  ...
  return () => html`
    <div class="app-772">
      <header class="header-772">
        <h1>todos — Component0772</h1>
      </header>

      <main class="main-772">
        <input
          class="add-input-772 ${error() ? "add-input--error-772" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-772" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-772">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-772">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-772 ${filter() === f ? "filter-btn--active-772" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-772">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-772">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-772">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-772 ${todo.done ? "todo-item--done-772" : ""}">
                    <button
                      class="todo-check-772 ${todo.done ? "todo-check--done-772" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-772" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-772"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-772" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-772" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-772">
            <span class="todo-count-772">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-772" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
