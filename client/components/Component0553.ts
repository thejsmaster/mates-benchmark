// client/components/Component0553.ts
// Benchmark component #553 — identical logic to TodoPage, scoped class names.
...
import "./Component0553.css";

type Filter = "all" | "active" | "done";

export const Component0553 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0553] mounted");
    return () => console.log("[Component0553] unmounted");
  });
  ...
  return () => html`
    <div class="app-553">
      <header class="header-553">
        <h1>todos — Component0553</h1>
      </header>

      <main class="main-553">
        <input
          class="add-input-553 ${error() ? "add-input--error-553" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-553" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-553">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-553">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-553 ${filter() === f ? "filter-btn--active-553" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-553">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-553">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-553">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-553 ${todo.done ? "todo-item--done-553" : ""}">
                    <button
                      class="todo-check-553 ${todo.done ? "todo-check--done-553" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-553" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-553"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-553" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-553" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-553">
            <span class="todo-count-553">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-553" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
