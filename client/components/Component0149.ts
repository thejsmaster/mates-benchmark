// client/components/Component0149.ts
// Benchmark component #149 — identical logic to TodoPage, scoped class names.
...
import "./Component0149.css";

type Filter = "all" | "active" | "done";

export const Component0149 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0149] mounted");
    return () => console.log("[Component0149] unmounted");
  });
  ...
  return () => html`
    <div class="app-149">
      <header class="header-149">
        <h1>todos — Component0149</h1>
      </header>

      <main class="main-149">
        <input
          class="add-input-149 ${error() ? "add-input--error-149" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-149" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-149">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-149">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-149 ${filter() === f ? "filter-btn--active-149" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-149">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-149">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-149">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-149 ${todo.done ? "todo-item--done-149" : ""}">
                    <button
                      class="todo-check-149 ${todo.done ? "todo-check--done-149" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-149" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-149"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-149" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-149" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-149">
            <span class="todo-count-149">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-149" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
