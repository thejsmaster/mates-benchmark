// client/components/Component0834.ts
// Benchmark component #834 — identical logic to TodoPage, scoped class names.
...
import "./Component0834.css";

type Filter = "all" | "active" | "done";

export const Component0834 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0834] mounted");
    return () => console.log("[Component0834] unmounted");
  });
  ...
  return () => html`
    <div class="app-834">
      <header class="header-834">
        <h1>todos — Component0834</h1>
      </header>

      <main class="main-834">
        <input
          class="add-input-834 ${error() ? "add-input--error-834" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-834" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-834">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-834">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-834 ${filter() === f ? "filter-btn--active-834" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-834">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-834">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-834">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-834 ${todo.done ? "todo-item--done-834" : ""}">
                    <button
                      class="todo-check-834 ${todo.done ? "todo-check--done-834" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-834" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-834"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-834" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-834" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-834">
            <span class="todo-count-834">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-834" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
