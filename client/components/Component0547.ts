// client/components/Component0547.ts
// Benchmark component #547 — identical logic to TodoPage, scoped class names.
...
import "./Component0547.css";

type Filter = "all" | "active" | "done";

export const Component0547 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0547] mounted");
    return () => console.log("[Component0547] unmounted");
  });
  ...
  return () => html`
    <div class="app-547">
      <header class="header-547">
        <h1>todos — Component0547</h1>
      </header>

      <main class="main-547">
        <input
          class="add-input-547 ${error() ? "add-input--error-547" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-547" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-547">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-547">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-547 ${filter() === f ? "filter-btn--active-547" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-547">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-547">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-547">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-547 ${todo.done ? "todo-item--done-547" : ""}">
                    <button
                      class="todo-check-547 ${todo.done ? "todo-check--done-547" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-547" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-547"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-547" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-547" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-547">
            <span class="todo-count-547">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-547" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
