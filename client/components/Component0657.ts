// client/components/Component0657.ts
// Benchmark component #657 — identical logic to TodoPage, scoped class names.
...
import "./Component0657.css";

type Filter = "all" | "active" | "done";

export const Component0657 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0657] mounted");
    return () => console.log("[Component0657] unmounted");
  });
  ...
  return () => html`
    <div class="app-657">
      <header class="header-657">
        <h1>todos — Component0657</h1>
      </header>

      <main class="main-657">
        <input
          class="add-input-657 ${error() ? "add-input--error-657" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-657" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-657">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-657">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-657 ${filter() === f ? "filter-btn--active-657" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-657">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-657">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-657">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-657 ${todo.done ? "todo-item--done-657" : ""}">
                    <button
                      class="todo-check-657 ${todo.done ? "todo-check--done-657" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-657" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-657"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-657" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-657" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-657">
            <span class="todo-count-657">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-657" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
