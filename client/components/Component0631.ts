// client/components/Component0631.ts
// Benchmark component #631 — identical logic to TodoPage, scoped class names.
...
import "./Component0631.css";

type Filter = "all" | "active" | "done";

export const Component0631 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0631] mounted");
    return () => console.log("[Component0631] unmounted");
  });
  ...
  return () => html`
    <div class="app-631">
      <header class="header-631">
        <h1>todos — Component0631</h1>
      </header>

      <main class="main-631">
        <input
          class="add-input-631 ${error() ? "add-input--error-631" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-631" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-631">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-631">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-631 ${filter() === f ? "filter-btn--active-631" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-631">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-631">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-631">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-631 ${todo.done ? "todo-item--done-631" : ""}">
                    <button
                      class="todo-check-631 ${todo.done ? "todo-check--done-631" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-631" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-631"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-631" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-631" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-631">
            <span class="todo-count-631">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-631" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
