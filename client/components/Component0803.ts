// client/components/Component0803.ts
// Benchmark component #803 — identical logic to TodoPage, scoped class names.
...
import "./Component0803.css";

type Filter = "all" | "active" | "done";

export const Component0803 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0803] mounted");
    return () => console.log("[Component0803] unmounted");
  });
  ...
  return () => html`
    <div class="app-803">
      <header class="header-803">
        <h1>todos — Component0803</h1>
      </header>

      <main class="main-803">
        <input
          class="add-input-803 ${error() ? "add-input--error-803" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-803" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-803">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-803">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-803 ${filter() === f ? "filter-btn--active-803" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-803">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-803">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-803">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-803 ${todo.done ? "todo-item--done-803" : ""}">
                    <button
                      class="todo-check-803 ${todo.done ? "todo-check--done-803" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-803" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-803"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-803" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-803" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-803">
            <span class="todo-count-803">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-803" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
