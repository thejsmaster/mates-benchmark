// client/components/Component0120.ts
// Benchmark component #120 — identical logic to TodoPage, scoped class names.
...
import "./Component0120.css";

type Filter = "all" | "active" | "done";

export const Component0120 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0120] mounted");
    return () => console.log("[Component0120] unmounted");
  });
  ...
  return () => html`
    <div class="app-120">
      <header class="header-120">
        <h1>todos — Component0120</h1>
      </header>

      <main class="main-120">
        <input
          class="add-input-120 ${error() ? "add-input--error-120" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-120" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-120">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-120">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-120 ${filter() === f ? "filter-btn--active-120" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-120">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-120">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-120">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-120 ${todo.done ? "todo-item--done-120" : ""}">
                    <button
                      class="todo-check-120 ${todo.done ? "todo-check--done-120" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-120" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-120"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-120" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-120" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-120">
            <span class="todo-count-120">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-120" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
