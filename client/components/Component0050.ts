// client/components/Component0050.ts
// Benchmark component #50 — identical logic to TodoPage, scoped class names.
...
import "./Component0050.css";

type Filter = "all" | "active" | "done";

export const Component0050 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0050] mounted");
    return () => console.log("[Component0050] unmounted");
  });
  ...
  return () => html`
    <div class="app-50">
      <header class="header-50">
        <h1>todos — Component0050</h1>
      </header>

      <main class="main-50">
        <input
          class="add-input-50 ${error() ? "add-input--error-50" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-50" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-50">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-50">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-50 ${filter() === f ? "filter-btn--active-50" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-50">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-50">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-50">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-50 ${todo.done ? "todo-item--done-50" : ""}">
                    <button
                      class="todo-check-50 ${todo.done ? "todo-check--done-50" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-50" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-50"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-50" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-50" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-50">
            <span class="todo-count-50">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-50" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
