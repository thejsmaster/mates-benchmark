// client/components/Component0365.ts
// Benchmark component #365 — identical logic to TodoPage, scoped class names.
...
import "./Component0365.css";

type Filter = "all" | "active" | "done";

export const Component0365 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0365] mounted");
    return () => console.log("[Component0365] unmounted");
  });
  ...
  return () => html`
    <div class="app-365">
      <header class="header-365">
        <h1>todos — Component0365</h1>
      </header>

      <main class="main-365">
        <input
          class="add-input-365 ${error() ? "add-input--error-365" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-365" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-365">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-365">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-365 ${filter() === f ? "filter-btn--active-365" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-365">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-365">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-365">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-365 ${todo.done ? "todo-item--done-365" : ""}">
                    <button
                      class="todo-check-365 ${todo.done ? "todo-check--done-365" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-365" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-365"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-365" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-365" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-365">
            <span class="todo-count-365">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-365" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
