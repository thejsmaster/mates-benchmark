// client/components/Component0699.ts
// Benchmark component #699 — identical logic to TodoPage, scoped class names.
...
import "./Component0699.css";

type Filter = "all" | "active" | "done";

export const Component0699 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0699] mounted");
    return () => console.log("[Component0699] unmounted");
  });
  ...
  return () => html`
    <div class="app-699">
      <header class="header-699">
        <h1>todos — Component0699</h1>
      </header>

      <main class="main-699">
        <input
          class="add-input-699 ${error() ? "add-input--error-699" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-699" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-699">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-699">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-699 ${filter() === f ? "filter-btn--active-699" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-699">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-699">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-699">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-699 ${todo.done ? "todo-item--done-699" : ""}">
                    <button
                      class="todo-check-699 ${todo.done ? "todo-check--done-699" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-699" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-699"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-699" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-699" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-699">
            <span class="todo-count-699">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-699" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
