// client/components/Component0655.ts
// Benchmark component #655 — identical logic to TodoPage, scoped class names.
...
import "./Component0655.css";

type Filter = "all" | "active" | "done";

export const Component0655 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0655] mounted");
    return () => console.log("[Component0655] unmounted");
  });
  ...
  return () => html`
    <div class="app-655">
      <header class="header-655">
        <h1>todos — Component0655</h1>
      </header>

      <main class="main-655">
        <input
          class="add-input-655 ${error() ? "add-input--error-655" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-655" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-655">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-655">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-655 ${filter() === f ? "filter-btn--active-655" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-655">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-655">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-655">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-655 ${todo.done ? "todo-item--done-655" : ""}">
                    <button
                      class="todo-check-655 ${todo.done ? "todo-check--done-655" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-655" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-655"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-655" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-655" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-655">
            <span class="todo-count-655">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-655" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
