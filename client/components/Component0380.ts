// client/components/Component0380.ts
// Benchmark component #380 — identical logic to TodoPage, scoped class names.
...
import "./Component0380.css";

type Filter = "all" | "active" | "done";

export const Component0380 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0380] mounted");
    return () => console.log("[Component0380] unmounted");
  });
  ...
  return () => html`
    <div class="app-380">
      <header class="header-380">
        <h1>todos — Component0380</h1>
      </header>

      <main class="main-380">
        <input
          class="add-input-380 ${error() ? "add-input--error-380" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-380" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-380">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-380">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-380 ${filter() === f ? "filter-btn--active-380" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-380">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-380">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-380">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-380 ${todo.done ? "todo-item--done-380" : ""}">
                    <button
                      class="todo-check-380 ${todo.done ? "todo-check--done-380" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-380" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-380"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-380" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-380" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-380">
            <span class="todo-count-380">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-380" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
