// client/components/Component0220.ts
// Benchmark component #220 — identical logic to TodoPage, scoped class names.
...
import "./Component0220.css";

type Filter = "all" | "active" | "done";

export const Component0220 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0220] mounted");
    return () => console.log("[Component0220] unmounted");
  });
  ...
  return () => html`
    <div class="app-220">
      <header class="header-220">
        <h1>todos — Component0220</h1>
      </header>

      <main class="main-220">
        <input
          class="add-input-220 ${error() ? "add-input--error-220" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-220" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-220">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-220">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-220 ${filter() === f ? "filter-btn--active-220" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-220">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-220">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-220">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-220 ${todo.done ? "todo-item--done-220" : ""}">
                    <button
                      class="todo-check-220 ${todo.done ? "todo-check--done-220" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-220" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-220"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-220" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-220" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-220">
            <span class="todo-count-220">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-220" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
