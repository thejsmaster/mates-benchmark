// client/components/Component0197.ts
// Benchmark component #197 — identical logic to TodoPage, scoped class names.
...
import "./Component0197.css";

type Filter = "all" | "active" | "done";

export const Component0197 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0197] mounted");
    return () => console.log("[Component0197] unmounted");
  });
  ...
  return () => html`
    <div class="app-197">
      <header class="header-197">
        <h1>todos — Component0197</h1>
      </header>

      <main class="main-197">
        <input
          class="add-input-197 ${error() ? "add-input--error-197" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-197" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-197">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-197">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-197 ${filter() === f ? "filter-btn--active-197" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-197">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-197">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-197">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-197 ${todo.done ? "todo-item--done-197" : ""}">
                    <button
                      class="todo-check-197 ${todo.done ? "todo-check--done-197" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-197" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-197"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-197" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-197" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-197">
            <span class="todo-count-197">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-197" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
