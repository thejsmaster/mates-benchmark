// client/components/Component0051.ts
// Benchmark component #51 — identical logic to TodoPage, scoped class names.
...
import "./Component0051.css";

type Filter = "all" | "active" | "done";

export const Component0051 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0051] mounted");
    return () => console.log("[Component0051] unmounted");
  });
  ...
  return () => html`
    <div class="app-51">
      <header class="header-51">
        <h1>todos — Component0051</h1>
      </header>

      <main class="main-51">
        <input
          class="add-input-51 ${error() ? "add-input--error-51" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-51" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-51">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-51">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-51 ${filter() === f ? "filter-btn--active-51" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-51">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-51">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-51">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-51 ${todo.done ? "todo-item--done-51" : ""}">
                    <button
                      class="todo-check-51 ${todo.done ? "todo-check--done-51" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-51" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-51"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-51" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-51" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-51">
            <span class="todo-count-51">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-51" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
