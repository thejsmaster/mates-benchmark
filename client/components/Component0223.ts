// client/components/Component0223.ts
// Benchmark component #223 — identical logic to TodoPage, scoped class names.
...
import "./Component0223.css";

type Filter = "all" | "active" | "done";

export const Component0223 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0223] mounted");
    return () => console.log("[Component0223] unmounted");
  });
  ...
  return () => html`
    <div class="app-223">
      <header class="header-223">
        <h1>todos — Component0223</h1>
      </header>

      <main class="main-223">
        <input
          class="add-input-223 ${error() ? "add-input--error-223" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-223" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-223">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-223">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-223 ${filter() === f ? "filter-btn--active-223" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-223">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-223">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-223">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-223 ${todo.done ? "todo-item--done-223" : ""}">
                    <button
                      class="todo-check-223 ${todo.done ? "todo-check--done-223" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-223" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-223"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-223" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-223" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-223">
            <span class="todo-count-223">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-223" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
