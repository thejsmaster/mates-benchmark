// client/components/Component0096.ts
// Benchmark component #96 — identical logic to TodoPage, scoped class names.
...
import "./Component0096.css";

type Filter = "all" | "active" | "done";

export const Component0096 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0096] mounted");
    return () => console.log("[Component0096] unmounted");
  });
  ...
  return () => html`
    <div class="app-96">
      <header class="header-96">
        <h1>todos — Component0096</h1>
      </header>

      <main class="main-96">
        <input
          class="add-input-96 ${error() ? "add-input--error-96" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-96" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-96">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-96">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-96 ${filter() === f ? "filter-btn--active-96" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-96">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-96">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-96">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-96 ${todo.done ? "todo-item--done-96" : ""}">
                    <button
                      class="todo-check-96 ${todo.done ? "todo-check--done-96" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-96" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-96"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-96" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-96" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-96">
            <span class="todo-count-96">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-96" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
