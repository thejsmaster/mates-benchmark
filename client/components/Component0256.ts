// client/components/Component0256.ts
// Benchmark component #256 — identical logic to TodoPage, scoped class names.
...
import "./Component0256.css";

type Filter = "all" | "active" | "done";

export const Component0256 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0256] mounted");
    return () => console.log("[Component0256] unmounted");
  });
  ...
  return () => html`
    <div class="app-256">
      <header class="header-256">
        <h1>todos — Component0256</h1>
      </header>

      <main class="main-256">
        <input
          class="add-input-256 ${error() ? "add-input--error-256" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-256" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-256">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-256">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-256 ${filter() === f ? "filter-btn--active-256" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-256">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-256">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-256">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-256 ${todo.done ? "todo-item--done-256" : ""}">
                    <button
                      class="todo-check-256 ${todo.done ? "todo-check--done-256" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-256" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-256"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-256" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-256" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-256">
            <span class="todo-count-256">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-256" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
