// client/components/Component0557.ts
// Benchmark component #557 — identical logic to TodoPage, scoped class names.
...
import "./Component0557.css";

type Filter = "all" | "active" | "done";

export const Component0557 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0557] mounted");
    return () => console.log("[Component0557] unmounted");
  });
  ...
  return () => html`
    <div class="app-557">
      <header class="header-557">
        <h1>todos — Component0557</h1>
      </header>

      <main class="main-557">
        <input
          class="add-input-557 ${error() ? "add-input--error-557" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-557" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-557">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-557">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-557 ${filter() === f ? "filter-btn--active-557" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-557">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-557">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-557">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-557 ${todo.done ? "todo-item--done-557" : ""}">
                    <button
                      class="todo-check-557 ${todo.done ? "todo-check--done-557" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-557" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-557"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-557" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-557" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-557">
            <span class="todo-count-557">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-557" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
