// client/components/Component0726.ts
// Benchmark component #726 — identical logic to TodoPage, scoped class names.
...
import "./Component0726.css";

type Filter = "all" | "active" | "done";

export const Component0726 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0726] mounted");
    return () => console.log("[Component0726] unmounted");
  });
  ...
  return () => html`
    <div class="app-726">
      <header class="header-726">
        <h1>todos — Component0726</h1>
      </header>

      <main class="main-726">
        <input
          class="add-input-726 ${error() ? "add-input--error-726" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-726" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-726">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-726">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-726 ${filter() === f ? "filter-btn--active-726" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-726">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-726">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-726">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-726 ${todo.done ? "todo-item--done-726" : ""}">
                    <button
                      class="todo-check-726 ${todo.done ? "todo-check--done-726" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-726" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-726"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-726" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-726" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-726">
            <span class="todo-count-726">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-726" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
