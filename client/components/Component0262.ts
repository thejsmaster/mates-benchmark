// client/components/Component0262.ts
// Benchmark component #262 — identical logic to TodoPage, scoped class names.
...
import "./Component0262.css";

type Filter = "all" | "active" | "done";

export const Component0262 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0262] mounted");
    return () => console.log("[Component0262] unmounted");
  });
  ...
  return () => html`
    <div class="app-262">
      <header class="header-262">
        <h1>todos — Component0262</h1>
      </header>

      <main class="main-262">
        <input
          class="add-input-262 ${error() ? "add-input--error-262" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-262" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-262">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-262">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-262 ${filter() === f ? "filter-btn--active-262" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-262">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-262">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-262">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-262 ${todo.done ? "todo-item--done-262" : ""}">
                    <button
                      class="todo-check-262 ${todo.done ? "todo-check--done-262" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-262" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-262"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-262" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-262" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-262">
            <span class="todo-count-262">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-262" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
