// client/components/Component0702.ts
// Benchmark component #702 — identical logic to TodoPage, scoped class names.
...
import "./Component0702.css";

type Filter = "all" | "active" | "done";

export const Component0702 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0702] mounted");
    return () => console.log("[Component0702] unmounted");
  });
  ...
  return () => html`
    <div class="app-702">
      <header class="header-702">
        <h1>todos — Component0702</h1>
      </header>

      <main class="main-702">
        <input
          class="add-input-702 ${error() ? "add-input--error-702" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-702" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-702">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-702">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-702 ${filter() === f ? "filter-btn--active-702" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-702">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-702">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-702">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-702 ${todo.done ? "todo-item--done-702" : ""}">
                    <button
                      class="todo-check-702 ${todo.done ? "todo-check--done-702" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-702" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-702"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-702" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-702" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-702">
            <span class="todo-count-702">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-702" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
