// client/components/Component0908.ts
// Benchmark component #908 — identical logic to TodoPage, scoped class names.
...
import "./Component0908.css";

type Filter = "all" | "active" | "done";

export const Component0908 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0908] mounted");
    return () => console.log("[Component0908] unmounted");
  });
  ...
  return () => html`
    <div class="app-908">
      <header class="header-908">
        <h1>todos — Component0908</h1>
      </header>

      <main class="main-908">
        <input
          class="add-input-908 ${error() ? "add-input--error-908" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-908" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-908">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-908">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-908 ${filter() === f ? "filter-btn--active-908" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-908">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-908">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-908">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-908 ${todo.done ? "todo-item--done-908" : ""}">
                    <button
                      class="todo-check-908 ${todo.done ? "todo-check--done-908" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-908" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-908"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-908" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-908" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-908">
            <span class="todo-count-908">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-908" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
