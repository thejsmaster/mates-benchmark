// client/components/Component0367.ts
// Benchmark component #367 — identical logic to TodoPage, scoped class names.
...
import "./Component0367.css";

type Filter = "all" | "active" | "done";

export const Component0367 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0367] mounted");
    return () => console.log("[Component0367] unmounted");
  });
  ...
  return () => html`
    <div class="app-367">
      <header class="header-367">
        <h1>todos — Component0367</h1>
      </header>

      <main class="main-367">
        <input
          class="add-input-367 ${error() ? "add-input--error-367" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-367" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-367">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-367">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-367 ${filter() === f ? "filter-btn--active-367" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-367">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-367">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-367">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-367 ${todo.done ? "todo-item--done-367" : ""}">
                    <button
                      class="todo-check-367 ${todo.done ? "todo-check--done-367" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-367" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-367"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-367" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-367" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-367">
            <span class="todo-count-367">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-367" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
