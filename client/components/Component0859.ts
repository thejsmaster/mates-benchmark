// client/components/Component0859.ts
// Benchmark component #859 — identical logic to TodoPage, scoped class names.
...
import "./Component0859.css";

type Filter = "all" | "active" | "done";

export const Component0859 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0859] mounted");
    return () => console.log("[Component0859] unmounted");
  });
  ...
  return () => html`
    <div class="app-859">
      <header class="header-859">
        <h1>todos — Component0859</h1>
      </header>

      <main class="main-859">
        <input
          class="add-input-859 ${error() ? "add-input--error-859" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-859" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-859">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-859">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-859 ${filter() === f ? "filter-btn--active-859" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-859">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-859">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-859">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-859 ${todo.done ? "todo-item--done-859" : ""}">
                    <button
                      class="todo-check-859 ${todo.done ? "todo-check--done-859" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-859" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-859"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-859" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-859" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-859">
            <span class="todo-count-859">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-859" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
