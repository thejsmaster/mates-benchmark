// client/components/Component0623.ts
// Benchmark component #623 — identical logic to TodoPage, scoped class names.
...
import "./Component0623.css";

type Filter = "all" | "active" | "done";

export const Component0623 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0623] mounted");
    return () => console.log("[Component0623] unmounted");
  });
  ...
  return () => html`
    <div class="app-623">
      <header class="header-623">
        <h1>todos — Component0623</h1>
      </header>

      <main class="main-623">
        <input
          class="add-input-623 ${error() ? "add-input--error-623" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-623" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-623">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-623">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-623 ${filter() === f ? "filter-btn--active-623" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-623">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-623">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-623">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-623 ${todo.done ? "todo-item--done-623" : ""}">
                    <button
                      class="todo-check-623 ${todo.done ? "todo-check--done-623" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-623" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-623"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-623" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-623" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-623">
            <span class="todo-count-623">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-623" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
