// client/components/Component0511.ts
// Benchmark component #511 — identical logic to TodoPage, scoped class names.
...
import "./Component0511.css";

type Filter = "all" | "active" | "done";

export const Component0511 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0511] mounted");
    return () => console.log("[Component0511] unmounted");
  });
  ...
  return () => html`
    <div class="app-511">
      <header class="header-511">
        <h1>todos — Component0511</h1>
      </header>

      <main class="main-511">
        <input
          class="add-input-511 ${error() ? "add-input--error-511" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-511" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-511">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-511">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-511 ${filter() === f ? "filter-btn--active-511" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-511">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-511">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-511">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-511 ${todo.done ? "todo-item--done-511" : ""}">
                    <button
                      class="todo-check-511 ${todo.done ? "todo-check--done-511" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-511" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-511"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-511" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-511" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-511">
            <span class="todo-count-511">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-511" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
