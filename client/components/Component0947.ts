// client/components/Component0947.ts
// Benchmark component #947 — identical logic to TodoPage, scoped class names.
...
import "./Component0947.css";

type Filter = "all" | "active" | "done";

export const Component0947 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0947] mounted");
    return () => console.log("[Component0947] unmounted");
  });
  ...
  return () => html`
    <div class="app-947">
      <header class="header-947">
        <h1>todos — Component0947</h1>
      </header>

      <main class="main-947">
        <input
          class="add-input-947 ${error() ? "add-input--error-947" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-947" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-947">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-947">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-947 ${filter() === f ? "filter-btn--active-947" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-947">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-947">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-947">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-947 ${todo.done ? "todo-item--done-947" : ""}">
                    <button
                      class="todo-check-947 ${todo.done ? "todo-check--done-947" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-947" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-947"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-947" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-947" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-947">
            <span class="todo-count-947">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-947" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
