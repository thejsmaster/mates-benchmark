// client/components/Component0377.ts
// Benchmark component #377 — identical logic to TodoPage, scoped class names.
...
import "./Component0377.css";

type Filter = "all" | "active" | "done";

export const Component0377 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0377] mounted");
    return () => console.log("[Component0377] unmounted");
  });
  ...
  return () => html`
    <div class="app-377">
      <header class="header-377">
        <h1>todos — Component0377</h1>
      </header>

      <main class="main-377">
        <input
          class="add-input-377 ${error() ? "add-input--error-377" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-377" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-377">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-377">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-377 ${filter() === f ? "filter-btn--active-377" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-377">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-377">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-377">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-377 ${todo.done ? "todo-item--done-377" : ""}">
                    <button
                      class="todo-check-377 ${todo.done ? "todo-check--done-377" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-377" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-377"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-377" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-377" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-377">
            <span class="todo-count-377">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-377" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
