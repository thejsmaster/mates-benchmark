// client/components/Component0624.ts
// Benchmark component #624 — identical logic to TodoPage, scoped class names.
...
import "./Component0624.css";

type Filter = "all" | "active" | "done";

export const Component0624 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0624] mounted");
    return () => console.log("[Component0624] unmounted");
  });
  ...
  return () => html`
    <div class="app-624">
      <header class="header-624">
        <h1>todos — Component0624</h1>
      </header>

      <main class="main-624">
        <input
          class="add-input-624 ${error() ? "add-input--error-624" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-624" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-624">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-624">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-624 ${filter() === f ? "filter-btn--active-624" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-624">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-624">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-624">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-624 ${todo.done ? "todo-item--done-624" : ""}">
                    <button
                      class="todo-check-624 ${todo.done ? "todo-check--done-624" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-624" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-624"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-624" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-624" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-624">
            <span class="todo-count-624">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-624" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
