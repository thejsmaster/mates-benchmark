// client/components/Component0724.ts
// Benchmark component #724 — identical logic to TodoPage, scoped class names.
...
import "./Component0724.css";

type Filter = "all" | "active" | "done";

export const Component0724 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0724] mounted");
    return () => console.log("[Component0724] unmounted");
  });
  ...
  return () => html`
    <div class="app-724">
      <header class="header-724">
        <h1>todos — Component0724</h1>
      </header>

      <main class="main-724">
        <input
          class="add-input-724 ${error() ? "add-input--error-724" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-724" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-724">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-724">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-724 ${filter() === f ? "filter-btn--active-724" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-724">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-724">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-724">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-724 ${todo.done ? "todo-item--done-724" : ""}">
                    <button
                      class="todo-check-724 ${todo.done ? "todo-check--done-724" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-724" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-724"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-724" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-724" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-724">
            <span class="todo-count-724">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-724" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
