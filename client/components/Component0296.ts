// client/components/Component0296.ts
// Benchmark component #296 — identical logic to TodoPage, scoped class names.
...
import "./Component0296.css";

type Filter = "all" | "active" | "done";

export const Component0296 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0296] mounted");
    return () => console.log("[Component0296] unmounted");
  });
  ...
  return () => html`
    <div class="app-296">
      <header class="header-296">
        <h1>todos — Component0296</h1>
      </header>

      <main class="main-296">
        <input
          class="add-input-296 ${error() ? "add-input--error-296" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-296" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-296">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-296">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-296 ${filter() === f ? "filter-btn--active-296" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-296">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-296">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-296">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-296 ${todo.done ? "todo-item--done-296" : ""}">
                    <button
                      class="todo-check-296 ${todo.done ? "todo-check--done-296" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-296" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-296"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-296" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-296" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-296">
            <span class="todo-count-296">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-296" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
