// client/components/Component0585.ts
// Benchmark component #585 — identical logic to TodoPage, scoped class names.
...
import "./Component0585.css";

type Filter = "all" | "active" | "done";

export const Component0585 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0585] mounted");
    return () => console.log("[Component0585] unmounted");
  });
  ...
  return () => html`
    <div class="app-585">
      <header class="header-585">
        <h1>todos — Component0585</h1>
      </header>

      <main class="main-585">
        <input
          class="add-input-585 ${error() ? "add-input--error-585" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-585" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-585">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-585">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-585 ${filter() === f ? "filter-btn--active-585" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-585">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-585">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-585">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-585 ${todo.done ? "todo-item--done-585" : ""}">
                    <button
                      class="todo-check-585 ${todo.done ? "todo-check--done-585" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-585" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-585"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-585" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-585" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-585">
            <span class="todo-count-585">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-585" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
