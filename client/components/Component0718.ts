// client/components/Component0718.ts
// Benchmark component #718 — identical logic to TodoPage, scoped class names.
...
import "./Component0718.css";

type Filter = "all" | "active" | "done";

export const Component0718 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0718] mounted");
    return () => console.log("[Component0718] unmounted");
  });
  ...
  return () => html`
    <div class="app-718">
      <header class="header-718">
        <h1>todos — Component0718</h1>
      </header>

      <main class="main-718">
        <input
          class="add-input-718 ${error() ? "add-input--error-718" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-718" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-718">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-718">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-718 ${filter() === f ? "filter-btn--active-718" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-718">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-718">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-718">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-718 ${todo.done ? "todo-item--done-718" : ""}">
                    <button
                      class="todo-check-718 ${todo.done ? "todo-check--done-718" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-718" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-718"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-718" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-718" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-718">
            <span class="todo-count-718">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-718" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
