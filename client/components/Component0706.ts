// client/components/Component0706.ts
// Benchmark component #706 — identical logic to TodoPage, scoped class names.
...
import "./Component0706.css";

type Filter = "all" | "active" | "done";

export const Component0706 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0706] mounted");
    return () => console.log("[Component0706] unmounted");
  });
  ...
  return () => html`
    <div class="app-706">
      <header class="header-706">
        <h1>todos — Component0706</h1>
      </header>

      <main class="main-706">
        <input
          class="add-input-706 ${error() ? "add-input--error-706" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-706" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-706">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-706">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-706 ${filter() === f ? "filter-btn--active-706" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-706">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-706">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-706">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-706 ${todo.done ? "todo-item--done-706" : ""}">
                    <button
                      class="todo-check-706 ${todo.done ? "todo-check--done-706" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-706" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-706"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-706" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-706" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-706">
            <span class="todo-count-706">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-706" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
