// client/components/Component0891.ts
// Benchmark component #891 — identical logic to TodoPage, scoped class names.
...
import "./Component0891.css";

type Filter = "all" | "active" | "done";

export const Component0891 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0891] mounted");
    return () => console.log("[Component0891] unmounted");
  });
  ...
  return () => html`
    <div class="app-891">
      <header class="header-891">
        <h1>todos — Component0891</h1>
      </header>

      <main class="main-891">
        <input
          class="add-input-891 ${error() ? "add-input--error-891" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-891" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-891">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-891">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-891 ${filter() === f ? "filter-btn--active-891" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-891">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-891">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-891">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-891 ${todo.done ? "todo-item--done-891" : ""}">
                    <button
                      class="todo-check-891 ${todo.done ? "todo-check--done-891" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-891" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-891"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-891" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-891" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-891">
            <span class="todo-count-891">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-891" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
