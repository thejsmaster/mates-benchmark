// client/components/Component0817.ts
// Benchmark component #817 — identical logic to TodoPage, scoped class names.
...
import "./Component0817.css";

type Filter = "all" | "active" | "done";

export const Component0817 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0817] mounted");
    return () => console.log("[Component0817] unmounted");
  });
  ...
  return () => html`
    <div class="app-817">
      <header class="header-817">
        <h1>todos — Component0817</h1>
      </header>

      <main class="main-817">
        <input
          class="add-input-817 ${error() ? "add-input--error-817" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-817" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-817">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-817">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-817 ${filter() === f ? "filter-btn--active-817" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-817">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-817">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-817">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-817 ${todo.done ? "todo-item--done-817" : ""}">
                    <button
                      class="todo-check-817 ${todo.done ? "todo-check--done-817" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-817" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-817"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-817" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-817" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-817">
            <span class="todo-count-817">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-817" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
