// client/components/Component0746.ts
// Benchmark component #746 — identical logic to TodoPage, scoped class names.
...
import "./Component0746.css";

type Filter = "all" | "active" | "done";

export const Component0746 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0746] mounted");
    return () => console.log("[Component0746] unmounted");
  });
  ...
  return () => html`
    <div class="app-746">
      <header class="header-746">
        <h1>todos — Component0746</h1>
      </header>

      <main class="main-746">
        <input
          class="add-input-746 ${error() ? "add-input--error-746" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-746" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-746">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-746">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-746 ${filter() === f ? "filter-btn--active-746" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-746">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-746">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-746">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-746 ${todo.done ? "todo-item--done-746" : ""}">
                    <button
                      class="todo-check-746 ${todo.done ? "todo-check--done-746" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-746" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-746"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-746" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-746" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-746">
            <span class="todo-count-746">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-746" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
