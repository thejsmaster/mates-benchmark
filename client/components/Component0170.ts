// client/components/Component0170.ts
// Benchmark component #170 — identical logic to TodoPage, scoped class names.
...
import "./Component0170.css";

type Filter = "all" | "active" | "done";

export const Component0170 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0170] mounted");
    return () => console.log("[Component0170] unmounted");
  });
  ...
  return () => html`
    <div class="app-170">
      <header class="header-170">
        <h1>todos — Component0170</h1>
      </header>

      <main class="main-170">
        <input
          class="add-input-170 ${error() ? "add-input--error-170" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-170" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-170">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-170">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-170 ${filter() === f ? "filter-btn--active-170" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-170">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-170">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-170">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-170 ${todo.done ? "todo-item--done-170" : ""}">
                    <button
                      class="todo-check-170 ${todo.done ? "todo-check--done-170" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-170" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-170"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-170" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-170" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-170">
            <span class="todo-count-170">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-170" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
