// client/components/Component0144.ts
// Benchmark component #144 — identical logic to TodoPage, scoped class names.
...
import "./Component0144.css";

type Filter = "all" | "active" | "done";

export const Component0144 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0144] mounted");
    return () => console.log("[Component0144] unmounted");
  });
  ...
  return () => html`
    <div class="app-144">
      <header class="header-144">
        <h1>todos — Component0144</h1>
      </header>

      <main class="main-144">
        <input
          class="add-input-144 ${error() ? "add-input--error-144" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-144" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-144">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-144">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-144 ${filter() === f ? "filter-btn--active-144" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-144">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-144">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-144">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-144 ${todo.done ? "todo-item--done-144" : ""}">
                    <button
                      class="todo-check-144 ${todo.done ? "todo-check--done-144" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-144" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-144"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-144" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-144" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-144">
            <span class="todo-count-144">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-144" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
