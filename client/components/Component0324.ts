// client/components/Component0324.ts
// Benchmark component #324 — identical logic to TodoPage, scoped class names.
...
import "./Component0324.css";

type Filter = "all" | "active" | "done";

export const Component0324 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0324] mounted");
    return () => console.log("[Component0324] unmounted");
  });
  ...
  return () => html`
    <div class="app-324">
      <header class="header-324">
        <h1>todos — Component0324</h1>
      </header>

      <main class="main-324">
        <input
          class="add-input-324 ${error() ? "add-input--error-324" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-324" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-324">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-324">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-324 ${filter() === f ? "filter-btn--active-324" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-324">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-324">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-324">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-324 ${todo.done ? "todo-item--done-324" : ""}">
                    <button
                      class="todo-check-324 ${todo.done ? "todo-check--done-324" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-324" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-324"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-324" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-324" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-324">
            <span class="todo-count-324">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-324" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
