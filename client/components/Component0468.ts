// client/components/Component0468.ts
// Benchmark component #468 — identical logic to TodoPage, scoped class names.
...
import "./Component0468.css";

type Filter = "all" | "active" | "done";

export const Component0468 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0468] mounted");
    return () => console.log("[Component0468] unmounted");
  });
  ...
  return () => html`
    <div class="app-468">
      <header class="header-468">
        <h1>todos — Component0468</h1>
      </header>

      <main class="main-468">
        <input
          class="add-input-468 ${error() ? "add-input--error-468" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-468" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-468">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-468">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-468 ${filter() === f ? "filter-btn--active-468" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-468">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-468">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-468">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-468 ${todo.done ? "todo-item--done-468" : ""}">
                    <button
                      class="todo-check-468 ${todo.done ? "todo-check--done-468" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-468" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-468"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-468" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-468" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-468">
            <span class="todo-count-468">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-468" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
