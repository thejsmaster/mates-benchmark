// client/components/Component0280.ts
// Benchmark component #280 — identical logic to TodoPage, scoped class names.
...
import "./Component0280.css";

type Filter = "all" | "active" | "done";

export const Component0280 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0280] mounted");
    return () => console.log("[Component0280] unmounted");
  });
  ...
  return () => html`
    <div class="app-280">
      <header class="header-280">
        <h1>todos — Component0280</h1>
      </header>

      <main class="main-280">
        <input
          class="add-input-280 ${error() ? "add-input--error-280" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-280" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-280">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-280">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-280 ${filter() === f ? "filter-btn--active-280" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-280">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-280">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-280">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-280 ${todo.done ? "todo-item--done-280" : ""}">
                    <button
                      class="todo-check-280 ${todo.done ? "todo-check--done-280" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-280" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-280"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-280" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-280" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-280">
            <span class="todo-count-280">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-280" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
