// client/components/Component0799.ts
// Benchmark component #799 — identical logic to TodoPage, scoped class names.
...
import "./Component0799.css";

type Filter = "all" | "active" | "done";

export const Component0799 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0799] mounted");
    return () => console.log("[Component0799] unmounted");
  });
  ...
  return () => html`
    <div class="app-799">
      <header class="header-799">
        <h1>todos — Component0799</h1>
      </header>

      <main class="main-799">
        <input
          class="add-input-799 ${error() ? "add-input--error-799" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-799" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-799">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-799">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-799 ${filter() === f ? "filter-btn--active-799" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-799">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-799">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-799">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-799 ${todo.done ? "todo-item--done-799" : ""}">
                    <button
                      class="todo-check-799 ${todo.done ? "todo-check--done-799" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-799" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-799"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-799" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-799" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-799">
            <span class="todo-count-799">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-799" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
