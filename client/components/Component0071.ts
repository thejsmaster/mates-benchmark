// client/components/Component0071.ts
// Benchmark component #71 — identical logic to TodoPage, scoped class names.
...
import "./Component0071.css";

type Filter = "all" | "active" | "done";

export const Component0071 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0071] mounted");
    return () => console.log("[Component0071] unmounted");
  });
  ...
  return () => html`
    <div class="app-71">
      <header class="header-71">
        <h1>todos — Component0071</h1>
      </header>

      <main class="main-71">
        <input
          class="add-input-71 ${error() ? "add-input--error-71" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-71" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-71">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-71">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-71 ${filter() === f ? "filter-btn--active-71" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-71">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-71">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-71">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-71 ${todo.done ? "todo-item--done-71" : ""}">
                    <button
                      class="todo-check-71 ${todo.done ? "todo-check--done-71" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-71" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-71"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-71" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-71" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-71">
            <span class="todo-count-71">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-71" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
