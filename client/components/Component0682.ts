// client/components/Component0682.ts
// Benchmark component #682 — identical logic to TodoPage, scoped class names.
...
import "./Component0682.css";

type Filter = "all" | "active" | "done";

export const Component0682 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0682] mounted");
    return () => console.log("[Component0682] unmounted");
  });
  ...
  return () => html`
    <div class="app-682">
      <header class="header-682">
        <h1>todos — Component0682</h1>
      </header>

      <main class="main-682">
        <input
          class="add-input-682 ${error() ? "add-input--error-682" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-682" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-682">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-682">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-682 ${filter() === f ? "filter-btn--active-682" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-682">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-682">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-682">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-682 ${todo.done ? "todo-item--done-682" : ""}">
                    <button
                      class="todo-check-682 ${todo.done ? "todo-check--done-682" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-682" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-682"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-682" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-682" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-682">
            <span class="todo-count-682">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-682" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
