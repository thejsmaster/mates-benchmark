// client/components/Component0274.ts
// Benchmark component #274 — identical logic to TodoPage, scoped class names.
...
import "./Component0274.css";

type Filter = "all" | "active" | "done";

export const Component0274 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0274] mounted");
    return () => console.log("[Component0274] unmounted");
  });
  ...
  return () => html`
    <div class="app-274">
      <header class="header-274">
        <h1>todos — Component0274</h1>
      </header>

      <main class="main-274">
        <input
          class="add-input-274 ${error() ? "add-input--error-274" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-274" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-274">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-274">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-274 ${filter() === f ? "filter-btn--active-274" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-274">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-274">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-274">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-274 ${todo.done ? "todo-item--done-274" : ""}">
                    <button
                      class="todo-check-274 ${todo.done ? "todo-check--done-274" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-274" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-274"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-274" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-274" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-274">
            <span class="todo-count-274">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-274" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
