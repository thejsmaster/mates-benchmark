// client/components/Component0372.ts
// Benchmark component #372 — identical logic to TodoPage, scoped class names.
...
import "./Component0372.css";

type Filter = "all" | "active" | "done";

export const Component0372 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0372] mounted");
    return () => console.log("[Component0372] unmounted");
  });
  ...
  return () => html`
    <div class="app-372">
      <header class="header-372">
        <h1>todos — Component0372</h1>
      </header>

      <main class="main-372">
        <input
          class="add-input-372 ${error() ? "add-input--error-372" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-372" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-372">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-372">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-372 ${filter() === f ? "filter-btn--active-372" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-372">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-372">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-372">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-372 ${todo.done ? "todo-item--done-372" : ""}">
                    <button
                      class="todo-check-372 ${todo.done ? "todo-check--done-372" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-372" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-372"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-372" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-372" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-372">
            <span class="todo-count-372">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-372" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
