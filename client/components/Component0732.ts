// client/components/Component0732.ts
// Benchmark component #732 — identical logic to TodoPage, scoped class names.
...
import "./Component0732.css";

type Filter = "all" | "active" | "done";

export const Component0732 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0732] mounted");
    return () => console.log("[Component0732] unmounted");
  });
  ...
  return () => html`
    <div class="app-732">
      <header class="header-732">
        <h1>todos — Component0732</h1>
      </header>

      <main class="main-732">
        <input
          class="add-input-732 ${error() ? "add-input--error-732" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-732" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-732">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-732">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-732 ${filter() === f ? "filter-btn--active-732" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-732">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-732">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-732">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-732 ${todo.done ? "todo-item--done-732" : ""}">
                    <button
                      class="todo-check-732 ${todo.done ? "todo-check--done-732" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-732" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-732"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-732" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-732" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-732">
            <span class="todo-count-732">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-732" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
