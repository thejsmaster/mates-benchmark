// client/components/Component0400.ts
// Benchmark component #400 — identical logic to TodoPage, scoped class names.
...
import "./Component0400.css";

type Filter = "all" | "active" | "done";

export const Component0400 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0400] mounted");
    return () => console.log("[Component0400] unmounted");
  });
  ...
  return () => html`
    <div class="app-400">
      <header class="header-400">
        <h1>todos — Component0400</h1>
      </header>

      <main class="main-400">
        <input
          class="add-input-400 ${error() ? "add-input--error-400" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-400" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-400">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-400">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-400 ${filter() === f ? "filter-btn--active-400" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-400">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-400">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-400">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-400 ${todo.done ? "todo-item--done-400" : ""}">
                    <button
                      class="todo-check-400 ${todo.done ? "todo-check--done-400" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-400" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-400"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-400" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-400" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-400">
            <span class="todo-count-400">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-400" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
