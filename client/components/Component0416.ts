// client/components/Component0416.ts
// Benchmark component #416 — identical logic to TodoPage, scoped class names.
...
import "./Component0416.css";

type Filter = "all" | "active" | "done";

export const Component0416 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0416] mounted");
    return () => console.log("[Component0416] unmounted");
  });
  ...
  return () => html`
    <div class="app-416">
      <header class="header-416">
        <h1>todos — Component0416</h1>
      </header>

      <main class="main-416">
        <input
          class="add-input-416 ${error() ? "add-input--error-416" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-416" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-416">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-416">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-416 ${filter() === f ? "filter-btn--active-416" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-416">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-416">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-416">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-416 ${todo.done ? "todo-item--done-416" : ""}">
                    <button
                      class="todo-check-416 ${todo.done ? "todo-check--done-416" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-416" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-416"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-416" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-416" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-416">
            <span class="todo-count-416">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-416" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
