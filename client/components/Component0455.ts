// client/components/Component0455.ts
// Benchmark component #455 — identical logic to TodoPage, scoped class names.
...
import "./Component0455.css";

type Filter = "all" | "active" | "done";

export const Component0455 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0455] mounted");
    return () => console.log("[Component0455] unmounted");
  });
  ...
  return () => html`
    <div class="app-455">
      <header class="header-455">
        <h1>todos — Component0455</h1>
      </header>

      <main class="main-455">
        <input
          class="add-input-455 ${error() ? "add-input--error-455" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-455" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-455">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-455">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-455 ${filter() === f ? "filter-btn--active-455" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-455">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-455">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-455">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-455 ${todo.done ? "todo-item--done-455" : ""}">
                    <button
                      class="todo-check-455 ${todo.done ? "todo-check--done-455" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-455" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-455"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-455" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-455" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-455">
            <span class="todo-count-455">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-455" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
