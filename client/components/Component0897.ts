// client/components/Component0897.ts
// Benchmark component #897 — identical logic to TodoPage, scoped class names.
...
import "./Component0897.css";

type Filter = "all" | "active" | "done";

export const Component0897 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0897] mounted");
    return () => console.log("[Component0897] unmounted");
  });
  ...
  return () => html`
    <div class="app-897">
      <header class="header-897">
        <h1>todos — Component0897</h1>
      </header>

      <main class="main-897">
        <input
          class="add-input-897 ${error() ? "add-input--error-897" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-897" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-897">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-897">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-897 ${filter() === f ? "filter-btn--active-897" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-897">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-897">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-897">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-897 ${todo.done ? "todo-item--done-897" : ""}">
                    <button
                      class="todo-check-897 ${todo.done ? "todo-check--done-897" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-897" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-897"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-897" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-897" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-897">
            <span class="todo-count-897">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-897" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
