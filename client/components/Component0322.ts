// client/components/Component0322.ts
// Benchmark component #322 — identical logic to TodoPage, scoped class names.
...
import "./Component0322.css";

type Filter = "all" | "active" | "done";

export const Component0322 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0322] mounted");
    return () => console.log("[Component0322] unmounted");
  });
  ...
  return () => html`
    <div class="app-322">
      <header class="header-322">
        <h1>todos — Component0322</h1>
      </header>

      <main class="main-322">
        <input
          class="add-input-322 ${error() ? "add-input--error-322" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-322" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-322">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-322">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-322 ${filter() === f ? "filter-btn--active-322" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-322">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-322">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-322">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-322 ${todo.done ? "todo-item--done-322" : ""}">
                    <button
                      class="todo-check-322 ${todo.done ? "todo-check--done-322" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-322" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-322"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-322" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-322" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-322">
            <span class="todo-count-322">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-322" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
