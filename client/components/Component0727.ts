// client/components/Component0727.ts
// Benchmark component #727 — identical logic to TodoPage, scoped class names.
...
import "./Component0727.css";

type Filter = "all" | "active" | "done";

export const Component0727 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0727] mounted");
    return () => console.log("[Component0727] unmounted");
  });
  ...
  return () => html`
    <div class="app-727">
      <header class="header-727">
        <h1>todos — Component0727</h1>
      </header>

      <main class="main-727">
        <input
          class="add-input-727 ${error() ? "add-input--error-727" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-727" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-727">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-727">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-727 ${filter() === f ? "filter-btn--active-727" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-727">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-727">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-727">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-727 ${todo.done ? "todo-item--done-727" : ""}">
                    <button
                      class="todo-check-727 ${todo.done ? "todo-check--done-727" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-727" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-727"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-727" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-727" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-727">
            <span class="todo-count-727">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-727" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
