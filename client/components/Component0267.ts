// client/components/Component0267.ts
// Benchmark component #267 — identical logic to TodoPage, scoped class names.
...
import "./Component0267.css";

type Filter = "all" | "active" | "done";

export const Component0267 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0267] mounted");
    return () => console.log("[Component0267] unmounted");
  });
  ...
  return () => html`
    <div class="app-267">
      <header class="header-267">
        <h1>todos — Component0267</h1>
      </header>

      <main class="main-267">
        <input
          class="add-input-267 ${error() ? "add-input--error-267" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-267" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-267">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-267">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-267 ${filter() === f ? "filter-btn--active-267" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-267">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-267">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-267">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-267 ${todo.done ? "todo-item--done-267" : ""}">
                    <button
                      class="todo-check-267 ${todo.done ? "todo-check--done-267" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-267" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-267"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-267" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-267" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-267">
            <span class="todo-count-267">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-267" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
