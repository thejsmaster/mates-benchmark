// client/components/Component0871.ts
// Benchmark component #871 — identical logic to TodoPage, scoped class names.
...
import "./Component0871.css";

type Filter = "all" | "active" | "done";

export const Component0871 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0871] mounted");
    return () => console.log("[Component0871] unmounted");
  });
  ...
  return () => html`
    <div class="app-871">
      <header class="header-871">
        <h1>todos — Component0871</h1>
      </header>

      <main class="main-871">
        <input
          class="add-input-871 ${error() ? "add-input--error-871" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-871" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-871">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-871">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-871 ${filter() === f ? "filter-btn--active-871" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-871">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-871">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-871">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-871 ${todo.done ? "todo-item--done-871" : ""}">
                    <button
                      class="todo-check-871 ${todo.done ? "todo-check--done-871" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-871" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-871"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-871" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-871" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-871">
            <span class="todo-count-871">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-871" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
