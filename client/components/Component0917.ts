// client/components/Component0917.ts
// Benchmark component #917 — identical logic to TodoPage, scoped class names.
...
import "./Component0917.css";

type Filter = "all" | "active" | "done";

export const Component0917 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0917] mounted");
    return () => console.log("[Component0917] unmounted");
  });
  ...
  return () => html`
    <div class="app-917">
      <header class="header-917">
        <h1>todos — Component0917</h1>
      </header>

      <main class="main-917">
        <input
          class="add-input-917 ${error() ? "add-input--error-917" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-917" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-917">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-917">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-917 ${filter() === f ? "filter-btn--active-917" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-917">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-917">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-917">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-917 ${todo.done ? "todo-item--done-917" : ""}">
                    <button
                      class="todo-check-917 ${todo.done ? "todo-check--done-917" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-917" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-917"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-917" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-917" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-917">
            <span class="todo-count-917">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-917" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
