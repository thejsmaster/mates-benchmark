// client/components/Component0531.ts
// Benchmark component #531 — identical logic to TodoPage, scoped class names.
...
import "./Component0531.css";

type Filter = "all" | "active" | "done";

export const Component0531 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0531] mounted");
    return () => console.log("[Component0531] unmounted");
  });
  ...
  return () => html`
    <div class="app-531">
      <header class="header-531">
        <h1>todos — Component0531</h1>
      </header>

      <main class="main-531">
        <input
          class="add-input-531 ${error() ? "add-input--error-531" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-531" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-531">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-531">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-531 ${filter() === f ? "filter-btn--active-531" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-531">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-531">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-531">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-531 ${todo.done ? "todo-item--done-531" : ""}">
                    <button
                      class="todo-check-531 ${todo.done ? "todo-check--done-531" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-531" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-531"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-531" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-531" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-531">
            <span class="todo-count-531">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-531" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
