// client/components/Component0487.ts
// Benchmark component #487 — identical logic to TodoPage, scoped class names.
...
import "./Component0487.css";

type Filter = "all" | "active" | "done";

export const Component0487 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0487] mounted");
    return () => console.log("[Component0487] unmounted");
  });
  ...
  return () => html`
    <div class="app-487">
      <header class="header-487">
        <h1>todos — Component0487</h1>
      </header>

      <main class="main-487">
        <input
          class="add-input-487 ${error() ? "add-input--error-487" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-487" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-487">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-487">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-487 ${filter() === f ? "filter-btn--active-487" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-487">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-487">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-487">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-487 ${todo.done ? "todo-item--done-487" : ""}">
                    <button
                      class="todo-check-487 ${todo.done ? "todo-check--done-487" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-487" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-487"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-487" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-487" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-487">
            <span class="todo-count-487">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-487" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
