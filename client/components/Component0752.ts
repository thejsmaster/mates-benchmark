// client/components/Component0752.ts
// Benchmark component #752 — identical logic to TodoPage, scoped class names.
...
import "./Component0752.css";

type Filter = "all" | "active" | "done";

export const Component0752 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0752] mounted");
    return () => console.log("[Component0752] unmounted");
  });
  ...
  return () => html`
    <div class="app-752">
      <header class="header-752">
        <h1>todos — Component0752</h1>
      </header>

      <main class="main-752">
        <input
          class="add-input-752 ${error() ? "add-input--error-752" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-752" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-752">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-752">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-752 ${filter() === f ? "filter-btn--active-752" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-752">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-752">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-752">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-752 ${todo.done ? "todo-item--done-752" : ""}">
                    <button
                      class="todo-check-752 ${todo.done ? "todo-check--done-752" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-752" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-752"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-752" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-752" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-752">
            <span class="todo-count-752">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-752" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
