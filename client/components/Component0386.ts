// client/components/Component0386.ts
// Benchmark component #386 — identical logic to TodoPage, scoped class names.
...
import "./Component0386.css";

type Filter = "all" | "active" | "done";

export const Component0386 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0386] mounted");
    return () => console.log("[Component0386] unmounted");
  });
  ...
  return () => html`
    <div class="app-386">
      <header class="header-386">
        <h1>todos — Component0386</h1>
      </header>

      <main class="main-386">
        <input
          class="add-input-386 ${error() ? "add-input--error-386" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-386" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-386">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-386">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-386 ${filter() === f ? "filter-btn--active-386" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-386">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-386">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-386">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-386 ${todo.done ? "todo-item--done-386" : ""}">
                    <button
                      class="todo-check-386 ${todo.done ? "todo-check--done-386" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-386" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-386"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-386" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-386" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-386">
            <span class="todo-count-386">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-386" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
