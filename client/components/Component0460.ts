// client/components/Component0460.ts
// Benchmark component #460 — identical logic to TodoPage, scoped class names.
...
import "./Component0460.css";

type Filter = "all" | "active" | "done";

export const Component0460 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0460] mounted");
    return () => console.log("[Component0460] unmounted");
  });
  ...
  return () => html`
    <div class="app-460">
      <header class="header-460">
        <h1>todos — Component0460</h1>
      </header>

      <main class="main-460">
        <input
          class="add-input-460 ${error() ? "add-input--error-460" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-460" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-460">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-460">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-460 ${filter() === f ? "filter-btn--active-460" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-460">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-460">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-460">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-460 ${todo.done ? "todo-item--done-460" : ""}">
                    <button
                      class="todo-check-460 ${todo.done ? "todo-check--done-460" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-460" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-460"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-460" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-460" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-460">
            <span class="todo-count-460">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-460" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
