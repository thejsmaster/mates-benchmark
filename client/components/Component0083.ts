// client/components/Component0083.ts
// Benchmark component #83 — identical logic to TodoPage, scoped class names.
...
import "./Component0083.css";

type Filter = "all" | "active" | "done";

export const Component0083 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0083] mounted");
    return () => console.log("[Component0083] unmounted");
  });
  ...
  return () => html`
    <div class="app-83">
      <header class="header-83">
        <h1>todos — Component0083</h1>
      </header>

      <main class="main-83">
        <input
          class="add-input-83 ${error() ? "add-input--error-83" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-83" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-83">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-83">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-83 ${filter() === f ? "filter-btn--active-83" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-83">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-83">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-83">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-83 ${todo.done ? "todo-item--done-83" : ""}">
                    <button
                      class="todo-check-83 ${todo.done ? "todo-check--done-83" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-83" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-83"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-83" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-83" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-83">
            <span class="todo-count-83">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-83" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
