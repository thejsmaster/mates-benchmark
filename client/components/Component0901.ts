// client/components/Component0901.ts
// Benchmark component #901 — identical logic to TodoPage, scoped class names.
...
import "./Component0901.css";

type Filter = "all" | "active" | "done";

export const Component0901 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0901] mounted");
    return () => console.log("[Component0901] unmounted");
  });
  ...
  return () => html`
    <div class="app-901">
      <header class="header-901">
        <h1>todos — Component0901</h1>
      </header>

      <main class="main-901">
        <input
          class="add-input-901 ${error() ? "add-input--error-901" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-901" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-901">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-901">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-901 ${filter() === f ? "filter-btn--active-901" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-901">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-901">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-901">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-901 ${todo.done ? "todo-item--done-901" : ""}">
                    <button
                      class="todo-check-901 ${todo.done ? "todo-check--done-901" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-901" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-901"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-901" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-901" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-901">
            <span class="todo-count-901">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-901" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
