// client/components/Component0303.ts
// Benchmark component #303 — identical logic to TodoPage, scoped class names.
...
import "./Component0303.css";

type Filter = "all" | "active" | "done";

export const Component0303 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0303] mounted");
    return () => console.log("[Component0303] unmounted");
  });
  ...
  return () => html`
    <div class="app-303">
      <header class="header-303">
        <h1>todos — Component0303</h1>
      </header>

      <main class="main-303">
        <input
          class="add-input-303 ${error() ? "add-input--error-303" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-303" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-303">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-303">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-303 ${filter() === f ? "filter-btn--active-303" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-303">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-303">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-303">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-303 ${todo.done ? "todo-item--done-303" : ""}">
                    <button
                      class="todo-check-303 ${todo.done ? "todo-check--done-303" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-303" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-303"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-303" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-303" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-303">
            <span class="todo-count-303">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-303" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
