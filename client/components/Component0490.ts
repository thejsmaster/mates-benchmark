// client/components/Component0490.ts
// Benchmark component #490 — identical logic to TodoPage, scoped class names.
...
import "./Component0490.css";

type Filter = "all" | "active" | "done";

export const Component0490 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0490] mounted");
    return () => console.log("[Component0490] unmounted");
  });
  ...
  return () => html`
    <div class="app-490">
      <header class="header-490">
        <h1>todos — Component0490</h1>
      </header>

      <main class="main-490">
        <input
          class="add-input-490 ${error() ? "add-input--error-490" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-490" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-490">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-490">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-490 ${filter() === f ? "filter-btn--active-490" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-490">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-490">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-490">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-490 ${todo.done ? "todo-item--done-490" : ""}">
                    <button
                      class="todo-check-490 ${todo.done ? "todo-check--done-490" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-490" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-490"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-490" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-490" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-490">
            <span class="todo-count-490">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-490" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
