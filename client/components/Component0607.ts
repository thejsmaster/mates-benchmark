// client/components/Component0607.ts
// Benchmark component #607 — identical logic to TodoPage, scoped class names.
...
import "./Component0607.css";

type Filter = "all" | "active" | "done";

export const Component0607 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0607] mounted");
    return () => console.log("[Component0607] unmounted");
  });
  ...
  return () => html`
    <div class="app-607">
      <header class="header-607">
        <h1>todos — Component0607</h1>
      </header>

      <main class="main-607">
        <input
          class="add-input-607 ${error() ? "add-input--error-607" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-607" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-607">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-607">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-607 ${filter() === f ? "filter-btn--active-607" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-607">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-607">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-607">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-607 ${todo.done ? "todo-item--done-607" : ""}">
                    <button
                      class="todo-check-607 ${todo.done ? "todo-check--done-607" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-607" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-607"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-607" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-607" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-607">
            <span class="todo-count-607">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-607" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
