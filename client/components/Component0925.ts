// client/components/Component0925.ts
// Benchmark component #925 — identical logic to TodoPage, scoped class names.
...
import "./Component0925.css";

type Filter = "all" | "active" | "done";

export const Component0925 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0925] mounted");
    return () => console.log("[Component0925] unmounted");
  });
  ...
  return () => html`
    <div class="app-925">
      <header class="header-925">
        <h1>todos — Component0925</h1>
      </header>

      <main class="main-925">
        <input
          class="add-input-925 ${error() ? "add-input--error-925" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-925" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-925">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-925">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-925 ${filter() === f ? "filter-btn--active-925" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-925">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-925">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-925">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-925 ${todo.done ? "todo-item--done-925" : ""}">
                    <button
                      class="todo-check-925 ${todo.done ? "todo-check--done-925" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-925" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-925"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-925" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-925" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-925">
            <span class="todo-count-925">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-925" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
