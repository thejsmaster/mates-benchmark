// client/components/Component0842.ts
// Benchmark component #842 — identical logic to TodoPage, scoped class names.
...
import "./Component0842.css";

type Filter = "all" | "active" | "done";

export const Component0842 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0842] mounted");
    return () => console.log("[Component0842] unmounted");
  });
  ...
  return () => html`
    <div class="app-842">
      <header class="header-842">
        <h1>todos — Component0842</h1>
      </header>

      <main class="main-842">
        <input
          class="add-input-842 ${error() ? "add-input--error-842" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-842" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-842">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-842">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-842 ${filter() === f ? "filter-btn--active-842" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-842">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-842">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-842">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-842 ${todo.done ? "todo-item--done-842" : ""}">
                    <button
                      class="todo-check-842 ${todo.done ? "todo-check--done-842" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-842" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-842"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-842" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-842" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-842">
            <span class="todo-count-842">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-842" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
