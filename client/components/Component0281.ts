// client/components/Component0281.ts
// Benchmark component #281 — identical logic to TodoPage, scoped class names.
...
import "./Component0281.css";

type Filter = "all" | "active" | "done";

export const Component0281 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0281] mounted");
    return () => console.log("[Component0281] unmounted");
  });
  ...
  return () => html`
    <div class="app-281">
      <header class="header-281">
        <h1>todos — Component0281</h1>
      </header>

      <main class="main-281">
        <input
          class="add-input-281 ${error() ? "add-input--error-281" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-281" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-281">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-281">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-281 ${filter() === f ? "filter-btn--active-281" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-281">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-281">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-281">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-281 ${todo.done ? "todo-item--done-281" : ""}">
                    <button
                      class="todo-check-281 ${todo.done ? "todo-check--done-281" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-281" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-281"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-281" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-281" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-281">
            <span class="todo-count-281">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-281" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
