// client/components/Component0194.ts
// Benchmark component #194 — identical logic to TodoPage, scoped class names.
...
import "./Component0194.css";

type Filter = "all" | "active" | "done";

export const Component0194 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0194] mounted");
    return () => console.log("[Component0194] unmounted");
  });
  ...
  return () => html`
    <div class="app-194">
      <header class="header-194">
        <h1>todos — Component0194</h1>
      </header>

      <main class="main-194">
        <input
          class="add-input-194 ${error() ? "add-input--error-194" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-194" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-194">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-194">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-194 ${filter() === f ? "filter-btn--active-194" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-194">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-194">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-194">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-194 ${todo.done ? "todo-item--done-194" : ""}">
                    <button
                      class="todo-check-194 ${todo.done ? "todo-check--done-194" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-194" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-194"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-194" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-194" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-194">
            <span class="todo-count-194">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-194" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
