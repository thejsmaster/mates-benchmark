// client/components/Component0023.ts
// Benchmark component #23 — identical logic to TodoPage, scoped class names.
...
import "./Component0023.css";

type Filter = "all" | "active" | "done";

export const Component0023 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0023] mounted");
    return () => console.log("[Component0023] unmounted");
  });
  ...
  return () => html`
    <div class="app-23">
      <header class="header-23">
        <h1>todos — Component0023</h1>
      </header>

      <main class="main-23">
        <input
          class="add-input-23 ${error() ? "add-input--error-23" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-23" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-23">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-23">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-23 ${filter() === f ? "filter-btn--active-23" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-23">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-23">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-23">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-23 ${todo.done ? "todo-item--done-23" : ""}">
                    <button
                      class="todo-check-23 ${todo.done ? "todo-check--done-23" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-23" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-23"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-23" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-23" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-23">
            <span class="todo-count-23">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-23" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
