// client/components/Component0867.ts
// Benchmark component #867 — identical logic to TodoPage, scoped class names.
...
import "./Component0867.css";

type Filter = "all" | "active" | "done";

export const Component0867 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0867] mounted");
    return () => console.log("[Component0867] unmounted");
  });
  ...
  return () => html`
    <div class="app-867">
      <header class="header-867">
        <h1>todos — Component0867</h1>
      </header>

      <main class="main-867">
        <input
          class="add-input-867 ${error() ? "add-input--error-867" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-867" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-867">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-867">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-867 ${filter() === f ? "filter-btn--active-867" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-867">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-867">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-867">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-867 ${todo.done ? "todo-item--done-867" : ""}">
                    <button
                      class="todo-check-867 ${todo.done ? "todo-check--done-867" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-867" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-867"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-867" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-867" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-867">
            <span class="todo-count-867">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-867" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
