// client/components/Component0369.ts
// Benchmark component #369 — identical logic to TodoPage, scoped class names.
...
import "./Component0369.css";

type Filter = "all" | "active" | "done";

export const Component0369 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0369] mounted");
    return () => console.log("[Component0369] unmounted");
  });
  ...
  return () => html`
    <div class="app-369">
      <header class="header-369">
        <h1>todos — Component0369</h1>
      </header>

      <main class="main-369">
        <input
          class="add-input-369 ${error() ? "add-input--error-369" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-369" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-369">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-369">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-369 ${filter() === f ? "filter-btn--active-369" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-369">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-369">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-369">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-369 ${todo.done ? "todo-item--done-369" : ""}">
                    <button
                      class="todo-check-369 ${todo.done ? "todo-check--done-369" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-369" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-369"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-369" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-369" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-369">
            <span class="todo-count-369">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-369" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
