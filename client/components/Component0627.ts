// client/components/Component0627.ts
// Benchmark component #627 — identical logic to TodoPage, scoped class names.
...
import "./Component0627.css";

type Filter = "all" | "active" | "done";

export const Component0627 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0627] mounted");
    return () => console.log("[Component0627] unmounted");
  });
  ...
  return () => html`
    <div class="app-627">
      <header class="header-627">
        <h1>todos — Component0627</h1>
      </header>

      <main class="main-627">
        <input
          class="add-input-627 ${error() ? "add-input--error-627" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-627" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-627">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-627">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-627 ${filter() === f ? "filter-btn--active-627" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-627">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-627">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-627">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-627 ${todo.done ? "todo-item--done-627" : ""}">
                    <button
                      class="todo-check-627 ${todo.done ? "todo-check--done-627" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-627" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-627"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-627" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-627" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-627">
            <span class="todo-count-627">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-627" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
