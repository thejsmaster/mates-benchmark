// client/components/Component0110.ts
// Benchmark component #110 — identical logic to TodoPage, scoped class names.
...
import "./Component0110.css";

type Filter = "all" | "active" | "done";

export const Component0110 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0110] mounted");
    return () => console.log("[Component0110] unmounted");
  });
  ...
  return () => html`
    <div class="app-110">
      <header class="header-110">
        <h1>todos — Component0110</h1>
      </header>

      <main class="main-110">
        <input
          class="add-input-110 ${error() ? "add-input--error-110" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-110" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-110">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-110">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-110 ${filter() === f ? "filter-btn--active-110" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-110">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-110">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-110">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-110 ${todo.done ? "todo-item--done-110" : ""}">
                    <button
                      class="todo-check-110 ${todo.done ? "todo-check--done-110" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-110" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-110"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-110" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-110" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-110">
            <span class="todo-count-110">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-110" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
