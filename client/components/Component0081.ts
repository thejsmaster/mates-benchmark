// client/components/Component0081.ts
// Benchmark component #81 — identical logic to TodoPage, scoped class names.
...
import "./Component0081.css";

type Filter = "all" | "active" | "done";

export const Component0081 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0081] mounted");
    return () => console.log("[Component0081] unmounted");
  });
  ...
  return () => html`
    <div class="app-81">
      <header class="header-81">
        <h1>todos — Component0081</h1>
      </header>

      <main class="main-81">
        <input
          class="add-input-81 ${error() ? "add-input--error-81" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-81" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-81">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-81">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-81 ${filter() === f ? "filter-btn--active-81" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-81">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-81">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-81">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-81 ${todo.done ? "todo-item--done-81" : ""}">
                    <button
                      class="todo-check-81 ${todo.done ? "todo-check--done-81" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-81" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-81"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-81" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-81" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-81">
            <span class="todo-count-81">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-81" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
