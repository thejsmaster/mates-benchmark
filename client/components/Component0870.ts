// client/components/Component0870.ts
// Benchmark component #870 — identical logic to TodoPage, scoped class names.
...
import "./Component0870.css";

type Filter = "all" | "active" | "done";

export const Component0870 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0870] mounted");
    return () => console.log("[Component0870] unmounted");
  });
  ...
  return () => html`
    <div class="app-870">
      <header class="header-870">
        <h1>todos — Component0870</h1>
      </header>

      <main class="main-870">
        <input
          class="add-input-870 ${error() ? "add-input--error-870" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-870" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-870">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-870">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-870 ${filter() === f ? "filter-btn--active-870" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-870">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-870">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-870">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-870 ${todo.done ? "todo-item--done-870" : ""}">
                    <button
                      class="todo-check-870 ${todo.done ? "todo-check--done-870" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-870" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-870"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-870" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-870" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-870">
            <span class="todo-count-870">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-870" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
