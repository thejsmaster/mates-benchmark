// client/components/Component0184.ts
// Benchmark component #184 — identical logic to TodoPage, scoped class names.
...
import "./Component0184.css";

type Filter = "all" | "active" | "done";

export const Component0184 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0184] mounted");
    return () => console.log("[Component0184] unmounted");
  });
  ...
  return () => html`
    <div class="app-184">
      <header class="header-184">
        <h1>todos — Component0184</h1>
      </header>

      <main class="main-184">
        <input
          class="add-input-184 ${error() ? "add-input--error-184" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-184" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-184">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-184">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-184 ${filter() === f ? "filter-btn--active-184" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-184">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-184">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-184">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-184 ${todo.done ? "todo-item--done-184" : ""}">
                    <button
                      class="todo-check-184 ${todo.done ? "todo-check--done-184" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-184" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-184"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-184" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-184" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-184">
            <span class="todo-count-184">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-184" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
