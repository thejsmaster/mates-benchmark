// client/components/Component0491.ts
// Benchmark component #491 — identical logic to TodoPage, scoped class names.
...
import "./Component0491.css";

type Filter = "all" | "active" | "done";

export const Component0491 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0491] mounted");
    return () => console.log("[Component0491] unmounted");
  });
  ...
  return () => html`
    <div class="app-491">
      <header class="header-491">
        <h1>todos — Component0491</h1>
      </header>

      <main class="main-491">
        <input
          class="add-input-491 ${error() ? "add-input--error-491" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-491" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-491">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-491">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-491 ${filter() === f ? "filter-btn--active-491" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-491">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-491">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-491">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-491 ${todo.done ? "todo-item--done-491" : ""}">
                    <button
                      class="todo-check-491 ${todo.done ? "todo-check--done-491" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-491" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-491"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-491" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-491" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-491">
            <span class="todo-count-491">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-491" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
