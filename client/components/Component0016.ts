// client/components/Component0016.ts
// Benchmark component #16 — identical logic to TodoPage, scoped class names.
...
import "./Component0016.css";

type Filter = "all" | "active" | "done";

export const Component0016 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0016] mounted");
    return () => console.log("[Component0016] unmounted");
  });
  ...
  return () => html`
    <div class="app-16">
      <header class="header-16">
        <h1>todos — Component0016</h1>
      </header>

      <main class="main-16">
        <input
          class="add-input-16 ${error() ? "add-input--error-16" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-16" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-16">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-16">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-16 ${filter() === f ? "filter-btn--active-16" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-16">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-16">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-16">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-16 ${todo.done ? "todo-item--done-16" : ""}">
                    <button
                      class="todo-check-16 ${todo.done ? "todo-check--done-16" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-16" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-16"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-16" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-16" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-16">
            <span class="todo-count-16">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-16" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
