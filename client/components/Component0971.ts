// client/components/Component0971.ts
// Benchmark component #971 — identical logic to TodoPage, scoped class names.
...
import "./Component0971.css";

type Filter = "all" | "active" | "done";

export const Component0971 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0971] mounted");
    return () => console.log("[Component0971] unmounted");
  });
  ...
  return () => html`
    <div class="app-971">
      <header class="header-971">
        <h1>todos — Component0971</h1>
      </header>

      <main class="main-971">
        <input
          class="add-input-971 ${error() ? "add-input--error-971" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-971" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-971">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-971">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-971 ${filter() === f ? "filter-btn--active-971" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-971">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-971">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-971">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-971 ${todo.done ? "todo-item--done-971" : ""}">
                    <button
                      class="todo-check-971 ${todo.done ? "todo-check--done-971" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-971" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-971"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-971" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-971" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-971">
            <span class="todo-count-971">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-971" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
