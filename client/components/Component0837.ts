// client/components/Component0837.ts
// Benchmark component #837 — identical logic to TodoPage, scoped class names.
...
import "./Component0837.css";

type Filter = "all" | "active" | "done";

export const Component0837 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0837] mounted");
    return () => console.log("[Component0837] unmounted");
  });
  ...
  return () => html`
    <div class="app-837">
      <header class="header-837">
        <h1>todos — Component0837</h1>
      </header>

      <main class="main-837">
        <input
          class="add-input-837 ${error() ? "add-input--error-837" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-837" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-837">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-837">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-837 ${filter() === f ? "filter-btn--active-837" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-837">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-837">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-837">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-837 ${todo.done ? "todo-item--done-837" : ""}">
                    <button
                      class="todo-check-837 ${todo.done ? "todo-check--done-837" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-837" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-837"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-837" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-837" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-837">
            <span class="todo-count-837">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-837" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
