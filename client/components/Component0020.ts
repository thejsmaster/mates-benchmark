// client/components/Component0020.ts
// Benchmark component #20 — identical logic to TodoPage, scoped class names.
...
import "./Component0020.css";

type Filter = "all" | "active" | "done";

export const Component0020 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0020] mounted");
    return () => console.log("[Component0020] unmounted");
  });
  ...
  return () => html`
    <div class="app-20">
      <header class="header-20">
        <h1>todos — Component0020</h1>
      </header>

      <main class="main-20">
        <input
          class="add-input-20 ${error() ? "add-input--error-20" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-20" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-20">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-20">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-20 ${filter() === f ? "filter-btn--active-20" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-20">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-20">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-20">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-20 ${todo.done ? "todo-item--done-20" : ""}">
                    <button
                      class="todo-check-20 ${todo.done ? "todo-check--done-20" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-20" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-20"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-20" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-20" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-20">
            <span class="todo-count-20">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-20" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
