// client/components/Component0907.ts
// Benchmark component #907 — identical logic to TodoPage, scoped class names.
...
import "./Component0907.css";

type Filter = "all" | "active" | "done";

export const Component0907 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0907] mounted");
    return () => console.log("[Component0907] unmounted");
  });
  ...
  return () => html`
    <div class="app-907">
      <header class="header-907">
        <h1>todos — Component0907</h1>
      </header>

      <main class="main-907">
        <input
          class="add-input-907 ${error() ? "add-input--error-907" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-907" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-907">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-907">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-907 ${filter() === f ? "filter-btn--active-907" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-907">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-907">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-907">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-907 ${todo.done ? "todo-item--done-907" : ""}">
                    <button
                      class="todo-check-907 ${todo.done ? "todo-check--done-907" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-907" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-907"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-907" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-907" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-907">
            <span class="todo-count-907">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-907" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
