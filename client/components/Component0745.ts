// client/components/Component0745.ts
// Benchmark component #745 — identical logic to TodoPage, scoped class names.
...
import "./Component0745.css";

type Filter = "all" | "active" | "done";

export const Component0745 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0745] mounted");
    return () => console.log("[Component0745] unmounted");
  });
  ...
  return () => html`
    <div class="app-745">
      <header class="header-745">
        <h1>todos — Component0745</h1>
      </header>

      <main class="main-745">
        <input
          class="add-input-745 ${error() ? "add-input--error-745" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-745" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-745">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-745">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-745 ${filter() === f ? "filter-btn--active-745" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-745">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-745">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-745">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-745 ${todo.done ? "todo-item--done-745" : ""}">
                    <button
                      class="todo-check-745 ${todo.done ? "todo-check--done-745" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-745" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-745"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-745" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-745" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-745">
            <span class="todo-count-745">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-745" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
