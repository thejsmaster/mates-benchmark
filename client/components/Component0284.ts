// client/components/Component0284.ts
// Benchmark component #284 — identical logic to TodoPage, scoped class names.
...
import "./Component0284.css";

type Filter = "all" | "active" | "done";

export const Component0284 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0284] mounted");
    return () => console.log("[Component0284] unmounted");
  });
  ...
  return () => html`
    <div class="app-284">
      <header class="header-284">
        <h1>todos — Component0284</h1>
      </header>

      <main class="main-284">
        <input
          class="add-input-284 ${error() ? "add-input--error-284" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-284" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-284">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-284">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-284 ${filter() === f ? "filter-btn--active-284" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-284">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-284">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-284">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-284 ${todo.done ? "todo-item--done-284" : ""}">
                    <button
                      class="todo-check-284 ${todo.done ? "todo-check--done-284" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-284" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-284"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-284" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-284" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-284">
            <span class="todo-count-284">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-284" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
