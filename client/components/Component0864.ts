// client/components/Component0864.ts
// Benchmark component #864 — identical logic to TodoPage, scoped class names.
...
import "./Component0864.css";

type Filter = "all" | "active" | "done";

export const Component0864 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0864] mounted");
    return () => console.log("[Component0864] unmounted");
  });
  ...
  return () => html`
    <div class="app-864">
      <header class="header-864">
        <h1>todos — Component0864</h1>
      </header>

      <main class="main-864">
        <input
          class="add-input-864 ${error() ? "add-input--error-864" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-864" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-864">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-864">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-864 ${filter() === f ? "filter-btn--active-864" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-864">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-864">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-864">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-864 ${todo.done ? "todo-item--done-864" : ""}">
                    <button
                      class="todo-check-864 ${todo.done ? "todo-check--done-864" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-864" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-864"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-864" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-864" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-864">
            <span class="todo-count-864">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-864" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
