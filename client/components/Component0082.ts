// client/components/Component0082.ts
// Benchmark component #82 — identical logic to TodoPage, scoped class names.
...
import "./Component0082.css";

type Filter = "all" | "active" | "done";

export const Component0082 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0082] mounted");
    return () => console.log("[Component0082] unmounted");
  });
  ...
  return () => html`
    <div class="app-82">
      <header class="header-82">
        <h1>todos — Component0082</h1>
      </header>

      <main class="main-82">
        <input
          class="add-input-82 ${error() ? "add-input--error-82" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-82" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-82">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-82">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-82 ${filter() === f ? "filter-btn--active-82" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-82">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-82">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-82">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-82 ${todo.done ? "todo-item--done-82" : ""}">
                    <button
                      class="todo-check-82 ${todo.done ? "todo-check--done-82" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-82" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-82"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-82" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-82" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-82">
            <span class="todo-count-82">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-82" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
