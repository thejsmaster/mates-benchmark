// client/components/Component0433.ts
// Benchmark component #433 — identical logic to TodoPage, scoped class names.
...
import "./Component0433.css";

type Filter = "all" | "active" | "done";

export const Component0433 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0433] mounted");
    return () => console.log("[Component0433] unmounted");
  });
  ...
  return () => html`
    <div class="app-433">
      <header class="header-433">
        <h1>todos — Component0433</h1>
      </header>

      <main class="main-433">
        <input
          class="add-input-433 ${error() ? "add-input--error-433" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-433" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-433">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-433">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-433 ${filter() === f ? "filter-btn--active-433" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-433">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-433">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-433">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-433 ${todo.done ? "todo-item--done-433" : ""}">
                    <button
                      class="todo-check-433 ${todo.done ? "todo-check--done-433" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-433" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-433"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-433" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-433" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-433">
            <span class="todo-count-433">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-433" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
