// client/components/Component0886.ts
// Benchmark component #886 — identical logic to TodoPage, scoped class names.
...
import "./Component0886.css";

type Filter = "all" | "active" | "done";

export const Component0886 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0886] mounted");
    return () => console.log("[Component0886] unmounted");
  });
  ...
  return () => html`
    <div class="app-886">
      <header class="header-886">
        <h1>todos — Component0886</h1>
      </header>

      <main class="main-886">
        <input
          class="add-input-886 ${error() ? "add-input--error-886" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-886" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-886">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-886">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-886 ${filter() === f ? "filter-btn--active-886" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-886">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-886">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-886">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-886 ${todo.done ? "todo-item--done-886" : ""}">
                    <button
                      class="todo-check-886 ${todo.done ? "todo-check--done-886" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-886" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-886"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-886" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-886" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-886">
            <span class="todo-count-886">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-886" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
