// client/components/Component0442.ts
// Benchmark component #442 — identical logic to TodoPage, scoped class names.
...
import "./Component0442.css";

type Filter = "all" | "active" | "done";

export const Component0442 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0442] mounted");
    return () => console.log("[Component0442] unmounted");
  });
  ...
  return () => html`
    <div class="app-442">
      <header class="header-442">
        <h1>todos — Component0442</h1>
      </header>

      <main class="main-442">
        <input
          class="add-input-442 ${error() ? "add-input--error-442" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-442" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-442">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-442">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-442 ${filter() === f ? "filter-btn--active-442" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-442">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-442">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-442">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-442 ${todo.done ? "todo-item--done-442" : ""}">
                    <button
                      class="todo-check-442 ${todo.done ? "todo-check--done-442" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-442" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-442"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-442" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-442" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-442">
            <span class="todo-count-442">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-442" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
