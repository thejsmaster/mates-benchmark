// client/components/Component0033.ts
// Benchmark component #33 — identical logic to TodoPage, scoped class names.
...
import "./Component0033.css";

type Filter = "all" | "active" | "done";

export const Component0033 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0033] mounted");
    return () => console.log("[Component0033] unmounted");
  });
  ...
  return () => html`
    <div class="app-33">
      <header class="header-33">
        <h1>todos — Component0033</h1>
      </header>

      <main class="main-33">
        <input
          class="add-input-33 ${error() ? "add-input--error-33" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-33" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-33">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-33">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-33 ${filter() === f ? "filter-btn--active-33" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-33">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-33">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-33">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-33 ${todo.done ? "todo-item--done-33" : ""}">
                    <button
                      class="todo-check-33 ${todo.done ? "todo-check--done-33" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-33" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-33"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-33" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-33" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-33">
            <span class="todo-count-33">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-33" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
