// client/components/Component0609.ts
// Benchmark component #609 — identical logic to TodoPage, scoped class names.
...
import "./Component0609.css";

type Filter = "all" | "active" | "done";

export const Component0609 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0609] mounted");
    return () => console.log("[Component0609] unmounted");
  });
  ...
  return () => html`
    <div class="app-609">
      <header class="header-609">
        <h1>todos — Component0609</h1>
      </header>

      <main class="main-609">
        <input
          class="add-input-609 ${error() ? "add-input--error-609" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-609" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-609">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-609">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-609 ${filter() === f ? "filter-btn--active-609" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-609">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-609">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-609">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-609 ${todo.done ? "todo-item--done-609" : ""}">
                    <button
                      class="todo-check-609 ${todo.done ? "todo-check--done-609" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-609" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-609"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-609" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-609" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-609">
            <span class="todo-count-609">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-609" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
