// client/components/Component0769.ts
// Benchmark component #769 — identical logic to TodoPage, scoped class names.
...
import "./Component0769.css";

type Filter = "all" | "active" | "done";

export const Component0769 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0769] mounted");
    return () => console.log("[Component0769] unmounted");
  });
  ...
  return () => html`
    <div class="app-769">
      <header class="header-769">
        <h1>todos — Component0769</h1>
      </header>

      <main class="main-769">
        <input
          class="add-input-769 ${error() ? "add-input--error-769" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-769" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-769">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-769">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-769 ${filter() === f ? "filter-btn--active-769" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-769">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-769">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-769">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-769 ${todo.done ? "todo-item--done-769" : ""}">
                    <button
                      class="todo-check-769 ${todo.done ? "todo-check--done-769" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-769" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-769"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-769" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-769" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-769">
            <span class="todo-count-769">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-769" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
