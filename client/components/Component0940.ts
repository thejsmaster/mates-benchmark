// client/components/Component0940.ts
// Benchmark component #940 — identical logic to TodoPage, scoped class names.
...
import "./Component0940.css";

type Filter = "all" | "active" | "done";

export const Component0940 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0940] mounted");
    return () => console.log("[Component0940] unmounted");
  });
  ...
  return () => html`
    <div class="app-940">
      <header class="header-940">
        <h1>todos — Component0940</h1>
      </header>

      <main class="main-940">
        <input
          class="add-input-940 ${error() ? "add-input--error-940" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-940" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-940">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-940">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-940 ${filter() === f ? "filter-btn--active-940" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-940">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-940">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-940">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-940 ${todo.done ? "todo-item--done-940" : ""}">
                    <button
                      class="todo-check-940 ${todo.done ? "todo-check--done-940" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-940" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-940"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-940" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-940" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-940">
            <span class="todo-count-940">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-940" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
