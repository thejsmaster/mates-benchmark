// client/components/Component0564.ts
// Benchmark component #564 — identical logic to TodoPage, scoped class names.
...
import "./Component0564.css";

type Filter = "all" | "active" | "done";

export const Component0564 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0564] mounted");
    return () => console.log("[Component0564] unmounted");
  });
  ...
  return () => html`
    <div class="app-564">
      <header class="header-564">
        <h1>todos — Component0564</h1>
      </header>

      <main class="main-564">
        <input
          class="add-input-564 ${error() ? "add-input--error-564" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-564" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-564">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-564">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-564 ${filter() === f ? "filter-btn--active-564" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-564">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-564">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-564">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-564 ${todo.done ? "todo-item--done-564" : ""}">
                    <button
                      class="todo-check-564 ${todo.done ? "todo-check--done-564" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-564" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-564"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-564" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-564" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-564">
            <span class="todo-count-564">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-564" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
