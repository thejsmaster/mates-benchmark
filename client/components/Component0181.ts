// client/components/Component0181.ts
// Benchmark component #181 — identical logic to TodoPage, scoped class names.
...
import "./Component0181.css";

type Filter = "all" | "active" | "done";

export const Component0181 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0181] mounted");
    return () => console.log("[Component0181] unmounted");
  });
  ...
  return () => html`
    <div class="app-181">
      <header class="header-181">
        <h1>todos — Component0181</h1>
      </header>

      <main class="main-181">
        <input
          class="add-input-181 ${error() ? "add-input--error-181" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-181" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-181">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-181">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-181 ${filter() === f ? "filter-btn--active-181" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-181">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-181">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-181">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-181 ${todo.done ? "todo-item--done-181" : ""}">
                    <button
                      class="todo-check-181 ${todo.done ? "todo-check--done-181" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-181" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-181"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-181" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-181" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-181">
            <span class="todo-count-181">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-181" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
