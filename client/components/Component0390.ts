// client/components/Component0390.ts
// Benchmark component #390 — identical logic to TodoPage, scoped class names.
...
import "./Component0390.css";

type Filter = "all" | "active" | "done";

export const Component0390 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0390] mounted");
    return () => console.log("[Component0390] unmounted");
  });
  ...
  return () => html`
    <div class="app-390">
      <header class="header-390">
        <h1>todos — Component0390</h1>
      </header>

      <main class="main-390">
        <input
          class="add-input-390 ${error() ? "add-input--error-390" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-390" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-390">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-390">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-390 ${filter() === f ? "filter-btn--active-390" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-390">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-390">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-390">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-390 ${todo.done ? "todo-item--done-390" : ""}">
                    <button
                      class="todo-check-390 ${todo.done ? "todo-check--done-390" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-390" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-390"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-390" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-390" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-390">
            <span class="todo-count-390">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-390" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
