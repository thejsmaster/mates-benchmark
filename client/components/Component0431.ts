// client/components/Component0431.ts
// Benchmark component #431 — identical logic to TodoPage, scoped class names.
...
import "./Component0431.css";

type Filter = "all" | "active" | "done";

export const Component0431 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0431] mounted");
    return () => console.log("[Component0431] unmounted");
  });
  ...
  return () => html`
    <div class="app-431">
      <header class="header-431">
        <h1>todos — Component0431</h1>
      </header>

      <main class="main-431">
        <input
          class="add-input-431 ${error() ? "add-input--error-431" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-431" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-431">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-431">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-431 ${filter() === f ? "filter-btn--active-431" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-431">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-431">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-431">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-431 ${todo.done ? "todo-item--done-431" : ""}">
                    <button
                      class="todo-check-431 ${todo.done ? "todo-check--done-431" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-431" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-431"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-431" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-431" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-431">
            <span class="todo-count-431">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-431" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
