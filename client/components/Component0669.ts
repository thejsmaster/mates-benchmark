// client/components/Component0669.ts
// Benchmark component #669 — identical logic to TodoPage, scoped class names.
...
import "./Component0669.css";

type Filter = "all" | "active" | "done";

export const Component0669 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0669] mounted");
    return () => console.log("[Component0669] unmounted");
  });
  ...
  return () => html`
    <div class="app-669">
      <header class="header-669">
        <h1>todos — Component0669</h1>
      </header>

      <main class="main-669">
        <input
          class="add-input-669 ${error() ? "add-input--error-669" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-669" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-669">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-669">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-669 ${filter() === f ? "filter-btn--active-669" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-669">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-669">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-669">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-669 ${todo.done ? "todo-item--done-669" : ""}">
                    <button
                      class="todo-check-669 ${todo.done ? "todo-check--done-669" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-669" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-669"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-669" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-669" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-669">
            <span class="todo-count-669">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-669" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
