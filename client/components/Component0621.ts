// client/components/Component0621.ts
// Benchmark component #621 — identical logic to TodoPage, scoped class names.
...
import "./Component0621.css";

type Filter = "all" | "active" | "done";

export const Component0621 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0621] mounted");
    return () => console.log("[Component0621] unmounted");
  });
  ...
  return () => html`
    <div class="app-621">
      <header class="header-621">
        <h1>todos — Component0621</h1>
      </header>

      <main class="main-621">
        <input
          class="add-input-621 ${error() ? "add-input--error-621" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-621" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-621">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-621">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-621 ${filter() === f ? "filter-btn--active-621" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-621">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-621">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-621">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-621 ${todo.done ? "todo-item--done-621" : ""}">
                    <button
                      class="todo-check-621 ${todo.done ? "todo-check--done-621" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-621" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-621"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-621" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-621" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-621">
            <span class="todo-count-621">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-621" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
