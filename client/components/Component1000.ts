// client/components/Component1000.ts
// Benchmark component #1000 — identical logic to TodoPage, scoped class names.
...
import "./Component1000.css";

type Filter = "all" | "active" | "done";

export const Component1000 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component1000] mounted");
    return () => console.log("[Component1000] unmounted");
  });
  ...
  return () => html`
    <div class="app-1000">
      <header class="header-1000">
        <h1>todos — Component1000</h1>
      </header>

      <main class="main-1000">
        <input
          class="add-input-1000 ${error() ? "add-input--error-1000" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-1000" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-1000">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-1000">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-1000 ${filter() === f ? "filter-btn--active-1000" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-1000">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-1000">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-1000">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-1000 ${todo.done ? "todo-item--done-1000" : ""}">
                    <button
                      class="todo-check-1000 ${todo.done ? "todo-check--done-1000" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-1000" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-1000"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-1000" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-1000" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-1000">
            <span class="todo-count-1000">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-1000" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
