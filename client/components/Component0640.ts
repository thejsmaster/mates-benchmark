// client/components/Component0640.ts
// Benchmark component #640 — identical logic to TodoPage, scoped class names.
...
import "./Component0640.css";

type Filter = "all" | "active" | "done";

export const Component0640 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0640] mounted");
    return () => console.log("[Component0640] unmounted");
  });
  ...
  return () => html`
    <div class="app-640">
      <header class="header-640">
        <h1>todos — Component0640</h1>
      </header>

      <main class="main-640">
        <input
          class="add-input-640 ${error() ? "add-input--error-640" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-640" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-640">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-640">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-640 ${filter() === f ? "filter-btn--active-640" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-640">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-640">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-640">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-640 ${todo.done ? "todo-item--done-640" : ""}">
                    <button
                      class="todo-check-640 ${todo.done ? "todo-check--done-640" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-640" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-640"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-640" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-640" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-640">
            <span class="todo-count-640">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-640" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
