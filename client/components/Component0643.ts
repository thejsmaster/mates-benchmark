// client/components/Component0643.ts
// Benchmark component #643 — identical logic to TodoPage, scoped class names.
...
import "./Component0643.css";

type Filter = "all" | "active" | "done";

export const Component0643 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0643] mounted");
    return () => console.log("[Component0643] unmounted");
  });
  ...
  return () => html`
    <div class="app-643">
      <header class="header-643">
        <h1>todos — Component0643</h1>
      </header>

      <main class="main-643">
        <input
          class="add-input-643 ${error() ? "add-input--error-643" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-643" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-643">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-643">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-643 ${filter() === f ? "filter-btn--active-643" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-643">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-643">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-643">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-643 ${todo.done ? "todo-item--done-643" : ""}">
                    <button
                      class="todo-check-643 ${todo.done ? "todo-check--done-643" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-643" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-643"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-643" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-643" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-643">
            <span class="todo-count-643">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-643" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
