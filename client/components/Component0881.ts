// client/components/Component0881.ts
// Benchmark component #881 — identical logic to TodoPage, scoped class names.
...
import "./Component0881.css";

type Filter = "all" | "active" | "done";

export const Component0881 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0881] mounted");
    return () => console.log("[Component0881] unmounted");
  });
  ...
  return () => html`
    <div class="app-881">
      <header class="header-881">
        <h1>todos — Component0881</h1>
      </header>

      <main class="main-881">
        <input
          class="add-input-881 ${error() ? "add-input--error-881" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-881" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-881">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-881">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-881 ${filter() === f ? "filter-btn--active-881" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-881">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-881">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-881">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-881 ${todo.done ? "todo-item--done-881" : ""}">
                    <button
                      class="todo-check-881 ${todo.done ? "todo-check--done-881" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-881" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-881"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-881" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-881" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-881">
            <span class="todo-count-881">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-881" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
