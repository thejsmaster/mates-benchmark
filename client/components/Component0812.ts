// client/components/Component0812.ts
// Benchmark component #812 — identical logic to TodoPage, scoped class names.
...
import "./Component0812.css";

type Filter = "all" | "active" | "done";

export const Component0812 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0812] mounted");
    return () => console.log("[Component0812] unmounted");
  });
  ...
  return () => html`
    <div class="app-812">
      <header class="header-812">
        <h1>todos — Component0812</h1>
      </header>

      <main class="main-812">
        <input
          class="add-input-812 ${error() ? "add-input--error-812" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-812" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-812">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-812">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-812 ${filter() === f ? "filter-btn--active-812" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-812">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-812">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-812">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-812 ${todo.done ? "todo-item--done-812" : ""}">
                    <button
                      class="todo-check-812 ${todo.done ? "todo-check--done-812" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-812" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-812"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-812" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-812" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-812">
            <span class="todo-count-812">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-812" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
