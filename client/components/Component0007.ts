// client/components/Component0007.ts
// Benchmark component #7 — identical logic to TodoPage, scoped class names.
...
import "./Component0007.css";

type Filter = "all" | "active" | "done";

export const Component0007 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0007] mounted");
    return () => console.log("[Component0007] unmounted");
  });
  ...
  return () => html`
    <div class="app-7">
      <header class="header-7">
        <h1>todos — Component0007</h1>
      </header>

      <main class="main-7">
        <input
          class="add-input-7 ${error() ? "add-input--error-7" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-7" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-7">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-7">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-7 ${filter() === f ? "filter-btn--active-7" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-7">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-7">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-7">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-7 ${todo.done ? "todo-item--done-7" : ""}">
                    <button
                      class="todo-check-7 ${todo.done ? "todo-check--done-7" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-7" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-7"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-7" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-7" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-7">
            <span class="todo-count-7">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-7" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
