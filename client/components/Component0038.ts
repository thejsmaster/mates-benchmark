// client/components/Component0038.ts
// Benchmark component #38 — identical logic to TodoPage, scoped class names.
...
import "./Component0038.css";

type Filter = "all" | "active" | "done";

export const Component0038 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0038] mounted");
    return () => console.log("[Component0038] unmounted");
  });
  ...
  return () => html`
    <div class="app-38">
      <header class="header-38">
        <h1>todos — Component0038</h1>
      </header>

      <main class="main-38">
        <input
          class="add-input-38 ${error() ? "add-input--error-38" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-38" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-38">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-38">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-38 ${filter() === f ? "filter-btn--active-38" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-38">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-38">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-38">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-38 ${todo.done ? "todo-item--done-38" : ""}">
                    <button
                      class="todo-check-38 ${todo.done ? "todo-check--done-38" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-38" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-38"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-38" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-38" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-38">
            <span class="todo-count-38">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-38" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
