// client/components/Component0293.ts
// Benchmark component #293 — identical logic to TodoPage, scoped class names.
...
import "./Component0293.css";

type Filter = "all" | "active" | "done";

export const Component0293 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0293] mounted");
    return () => console.log("[Component0293] unmounted");
  });
  ...
  return () => html`
    <div class="app-293">
      <header class="header-293">
        <h1>todos — Component0293</h1>
      </header>

      <main class="main-293">
        <input
          class="add-input-293 ${error() ? "add-input--error-293" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-293" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-293">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-293">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-293 ${filter() === f ? "filter-btn--active-293" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-293">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-293">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-293">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-293 ${todo.done ? "todo-item--done-293" : ""}">
                    <button
                      class="todo-check-293 ${todo.done ? "todo-check--done-293" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-293" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-293"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-293" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-293" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-293">
            <span class="todo-count-293">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-293" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
