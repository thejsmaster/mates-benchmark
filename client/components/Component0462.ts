// client/components/Component0462.ts
// Benchmark component #462 — identical logic to TodoPage, scoped class names.
...
import "./Component0462.css";

type Filter = "all" | "active" | "done";

export const Component0462 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0462] mounted");
    return () => console.log("[Component0462] unmounted");
  });
  ...
  return () => html`
    <div class="app-462">
      <header class="header-462">
        <h1>todos — Component0462</h1>
      </header>

      <main class="main-462">
        <input
          class="add-input-462 ${error() ? "add-input--error-462" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-462" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-462">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-462">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-462 ${filter() === f ? "filter-btn--active-462" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-462">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-462">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-462">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-462 ${todo.done ? "todo-item--done-462" : ""}">
                    <button
                      class="todo-check-462 ${todo.done ? "todo-check--done-462" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-462" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-462"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-462" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-462" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-462">
            <span class="todo-count-462">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-462" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
