// client/components/Component0827.ts
// Benchmark component #827 — identical logic to TodoPage, scoped class names.
...
import "./Component0827.css";

type Filter = "all" | "active" | "done";

export const Component0827 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0827] mounted");
    return () => console.log("[Component0827] unmounted");
  });
  ...
  return () => html`
    <div class="app-827">
      <header class="header-827">
        <h1>todos — Component0827</h1>
      </header>

      <main class="main-827">
        <input
          class="add-input-827 ${error() ? "add-input--error-827" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-827" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-827">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-827">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-827 ${filter() === f ? "filter-btn--active-827" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-827">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-827">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-827">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-827 ${todo.done ? "todo-item--done-827" : ""}">
                    <button
                      class="todo-check-827 ${todo.done ? "todo-check--done-827" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-827" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-827"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-827" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-827" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-827">
            <span class="todo-count-827">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-827" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
