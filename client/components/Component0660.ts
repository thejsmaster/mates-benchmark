// client/components/Component0660.ts
// Benchmark component #660 — identical logic to TodoPage, scoped class names.
...
import "./Component0660.css";

type Filter = "all" | "active" | "done";

export const Component0660 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0660] mounted");
    return () => console.log("[Component0660] unmounted");
  });
  ...
  return () => html`
    <div class="app-660">
      <header class="header-660">
        <h1>todos — Component0660</h1>
      </header>

      <main class="main-660">
        <input
          class="add-input-660 ${error() ? "add-input--error-660" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-660" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-660">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-660">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-660 ${filter() === f ? "filter-btn--active-660" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-660">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-660">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-660">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-660 ${todo.done ? "todo-item--done-660" : ""}">
                    <button
                      class="todo-check-660 ${todo.done ? "todo-check--done-660" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-660" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-660"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-660" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-660" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-660">
            <span class="todo-count-660">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-660" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
