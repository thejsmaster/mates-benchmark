// client/components/Component0666.ts
// Benchmark component #666 — identical logic to TodoPage, scoped class names.
...
import "./Component0666.css";

type Filter = "all" | "active" | "done";

export const Component0666 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0666] mounted");
    return () => console.log("[Component0666] unmounted");
  });
  ...
  return () => html`
    <div class="app-666">
      <header class="header-666">
        <h1>todos — Component0666</h1>
      </header>

      <main class="main-666">
        <input
          class="add-input-666 ${error() ? "add-input--error-666" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-666" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-666">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-666">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-666 ${filter() === f ? "filter-btn--active-666" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-666">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-666">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-666">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-666 ${todo.done ? "todo-item--done-666" : ""}">
                    <button
                      class="todo-check-666 ${todo.done ? "todo-check--done-666" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-666" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-666"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-666" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-666" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-666">
            <span class="todo-count-666">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-666" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
