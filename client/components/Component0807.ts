// client/components/Component0807.ts
// Benchmark component #807 — identical logic to TodoPage, scoped class names.
...
import "./Component0807.css";

type Filter = "all" | "active" | "done";

export const Component0807 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0807] mounted");
    return () => console.log("[Component0807] unmounted");
  });
  ...
  return () => html`
    <div class="app-807">
      <header class="header-807">
        <h1>todos — Component0807</h1>
      </header>

      <main class="main-807">
        <input
          class="add-input-807 ${error() ? "add-input--error-807" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-807" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-807">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-807">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-807 ${filter() === f ? "filter-btn--active-807" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-807">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-807">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-807">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-807 ${todo.done ? "todo-item--done-807" : ""}">
                    <button
                      class="todo-check-807 ${todo.done ? "todo-check--done-807" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-807" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-807"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-807" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-807" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-807">
            <span class="todo-count-807">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-807" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
