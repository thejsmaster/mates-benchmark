// client/components/Component0269.ts
// Benchmark component #269 — identical logic to TodoPage, scoped class names.
...
import "./Component0269.css";

type Filter = "all" | "active" | "done";

export const Component0269 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0269] mounted");
    return () => console.log("[Component0269] unmounted");
  });
  ...
  return () => html`
    <div class="app-269">
      <header class="header-269">
        <h1>todos — Component0269</h1>
      </header>

      <main class="main-269">
        <input
          class="add-input-269 ${error() ? "add-input--error-269" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-269" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-269">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-269">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-269 ${filter() === f ? "filter-btn--active-269" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-269">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-269">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-269">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-269 ${todo.done ? "todo-item--done-269" : ""}">
                    <button
                      class="todo-check-269 ${todo.done ? "todo-check--done-269" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-269" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-269"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-269" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-269" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-269">
            <span class="todo-count-269">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-269" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
