// client/components/Component0776.ts
// Benchmark component #776 — identical logic to TodoPage, scoped class names.
...
import "./Component0776.css";

type Filter = "all" | "active" | "done";

export const Component0776 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0776] mounted");
    return () => console.log("[Component0776] unmounted");
  });
  ...
  return () => html`
    <div class="app-776">
      <header class="header-776">
        <h1>todos — Component0776</h1>
      </header>

      <main class="main-776">
        <input
          class="add-input-776 ${error() ? "add-input--error-776" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-776" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-776">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-776">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-776 ${filter() === f ? "filter-btn--active-776" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-776">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-776">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-776">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-776 ${todo.done ? "todo-item--done-776" : ""}">
                    <button
                      class="todo-check-776 ${todo.done ? "todo-check--done-776" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-776" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-776"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-776" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-776" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-776">
            <span class="todo-count-776">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-776" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
