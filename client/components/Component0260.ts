// client/components/Component0260.ts
// Benchmark component #260 — identical logic to TodoPage, scoped class names.
...
import "./Component0260.css";

type Filter = "all" | "active" | "done";

export const Component0260 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0260] mounted");
    return () => console.log("[Component0260] unmounted");
  });
  ...
  return () => html`
    <div class="app-260">
      <header class="header-260">
        <h1>todos — Component0260</h1>
      </header>

      <main class="main-260">
        <input
          class="add-input-260 ${error() ? "add-input--error-260" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-260" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-260">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-260">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-260 ${filter() === f ? "filter-btn--active-260" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-260">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-260">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-260">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-260 ${todo.done ? "todo-item--done-260" : ""}">
                    <button
                      class="todo-check-260 ${todo.done ? "todo-check--done-260" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-260" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-260"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-260" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-260" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-260">
            <span class="todo-count-260">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-260" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
