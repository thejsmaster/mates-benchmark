// client/components/Component0568.ts
// Benchmark component #568 — identical logic to TodoPage, scoped class names.
...
import "./Component0568.css";

type Filter = "all" | "active" | "done";

export const Component0568 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0568] mounted");
    return () => console.log("[Component0568] unmounted");
  });
  ...
  return () => html`
    <div class="app-568">
      <header class="header-568">
        <h1>todos — Component0568</h1>
      </header>

      <main class="main-568">
        <input
          class="add-input-568 ${error() ? "add-input--error-568" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-568" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-568">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-568">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-568 ${filter() === f ? "filter-btn--active-568" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-568">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-568">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-568">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-568 ${todo.done ? "todo-item--done-568" : ""}">
                    <button
                      class="todo-check-568 ${todo.done ? "todo-check--done-568" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-568" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-568"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-568" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-568" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-568">
            <span class="todo-count-568">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-568" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
