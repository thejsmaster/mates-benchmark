// client/components/Component0191.ts
// Benchmark component #191 — identical logic to TodoPage, scoped class names.
...
import "./Component0191.css";

type Filter = "all" | "active" | "done";

export const Component0191 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0191] mounted");
    return () => console.log("[Component0191] unmounted");
  });
  ...
  return () => html`
    <div class="app-191">
      <header class="header-191">
        <h1>todos — Component0191</h1>
      </header>

      <main class="main-191">
        <input
          class="add-input-191 ${error() ? "add-input--error-191" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-191" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-191">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-191">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-191 ${filter() === f ? "filter-btn--active-191" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-191">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-191">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-191">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-191 ${todo.done ? "todo-item--done-191" : ""}">
                    <button
                      class="todo-check-191 ${todo.done ? "todo-check--done-191" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-191" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-191"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-191" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-191" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-191">
            <span class="todo-count-191">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-191" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
