// client/components/Component0672.ts
// Benchmark component #672 — identical logic to TodoPage, scoped class names.
...
import "./Component0672.css";

type Filter = "all" | "active" | "done";

export const Component0672 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0672] mounted");
    return () => console.log("[Component0672] unmounted");
  });
  ...
  return () => html`
    <div class="app-672">
      <header class="header-672">
        <h1>todos — Component0672</h1>
      </header>

      <main class="main-672">
        <input
          class="add-input-672 ${error() ? "add-input--error-672" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-672" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-672">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-672">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-672 ${filter() === f ? "filter-btn--active-672" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-672">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-672">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-672">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-672 ${todo.done ? "todo-item--done-672" : ""}">
                    <button
                      class="todo-check-672 ${todo.done ? "todo-check--done-672" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-672" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-672"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-672" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-672" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-672">
            <span class="todo-count-672">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-672" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
