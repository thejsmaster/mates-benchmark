// client/components/Component0960.ts
// Benchmark component #960 — identical logic to TodoPage, scoped class names.
...
import "./Component0960.css";

type Filter = "all" | "active" | "done";

export const Component0960 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0960] mounted");
    return () => console.log("[Component0960] unmounted");
  });
  ...
  return () => html`
    <div class="app-960">
      <header class="header-960">
        <h1>todos — Component0960</h1>
      </header>

      <main class="main-960">
        <input
          class="add-input-960 ${error() ? "add-input--error-960" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-960" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-960">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-960">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-960 ${filter() === f ? "filter-btn--active-960" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-960">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-960">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-960">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-960 ${todo.done ? "todo-item--done-960" : ""}">
                    <button
                      class="todo-check-960 ${todo.done ? "todo-check--done-960" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-960" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-960"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-960" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-960" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-960">
            <span class="todo-count-960">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-960" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
