// client/components/Component0831.ts
// Benchmark component #831 — identical logic to TodoPage, scoped class names.
...
import "./Component0831.css";

type Filter = "all" | "active" | "done";

export const Component0831 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0831] mounted");
    return () => console.log("[Component0831] unmounted");
  });
  ...
  return () => html`
    <div class="app-831">
      <header class="header-831">
        <h1>todos — Component0831</h1>
      </header>

      <main class="main-831">
        <input
          class="add-input-831 ${error() ? "add-input--error-831" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-831" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-831">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-831">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-831 ${filter() === f ? "filter-btn--active-831" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-831">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-831">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-831">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-831 ${todo.done ? "todo-item--done-831" : ""}">
                    <button
                      class="todo-check-831 ${todo.done ? "todo-check--done-831" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-831" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-831"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-831" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-831" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-831">
            <span class="todo-count-831">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-831" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
