// client/components/Component0152.ts
// Benchmark component #152 — identical logic to TodoPage, scoped class names.
...
import "./Component0152.css";

type Filter = "all" | "active" | "done";

export const Component0152 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0152] mounted");
    return () => console.log("[Component0152] unmounted");
  });
  ...
  return () => html`
    <div class="app-152">
      <header class="header-152">
        <h1>todos — Component0152</h1>
      </header>

      <main class="main-152">
        <input
          class="add-input-152 ${error() ? "add-input--error-152" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-152" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-152">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-152">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-152 ${filter() === f ? "filter-btn--active-152" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-152">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-152">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-152">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-152 ${todo.done ? "todo-item--done-152" : ""}">
                    <button
                      class="todo-check-152 ${todo.done ? "todo-check--done-152" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-152" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-152"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-152" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-152" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-152">
            <span class="todo-count-152">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-152" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
