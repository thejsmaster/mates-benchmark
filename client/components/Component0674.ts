// client/components/Component0674.ts
// Benchmark component #674 — identical logic to TodoPage, scoped class names.
...
import "./Component0674.css";

type Filter = "all" | "active" | "done";

export const Component0674 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0674] mounted");
    return () => console.log("[Component0674] unmounted");
  });
  ...
  return () => html`
    <div class="app-674">
      <header class="header-674">
        <h1>todos — Component0674</h1>
      </header>

      <main class="main-674">
        <input
          class="add-input-674 ${error() ? "add-input--error-674" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-674" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-674">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-674">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-674 ${filter() === f ? "filter-btn--active-674" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-674">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-674">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-674">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-674 ${todo.done ? "todo-item--done-674" : ""}">
                    <button
                      class="todo-check-674 ${todo.done ? "todo-check--done-674" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-674" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-674"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-674" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-674" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-674">
            <span class="todo-count-674">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-674" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
