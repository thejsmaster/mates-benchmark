// client/components/Component0413.ts
// Benchmark component #413 — identical logic to TodoPage, scoped class names.
...
import "./Component0413.css";

type Filter = "all" | "active" | "done";

export const Component0413 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0413] mounted");
    return () => console.log("[Component0413] unmounted");
  });
  ...
  return () => html`
    <div class="app-413">
      <header class="header-413">
        <h1>todos — Component0413</h1>
      </header>

      <main class="main-413">
        <input
          class="add-input-413 ${error() ? "add-input--error-413" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-413" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-413">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-413">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-413 ${filter() === f ? "filter-btn--active-413" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-413">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-413">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-413">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-413 ${todo.done ? "todo-item--done-413" : ""}">
                    <button
                      class="todo-check-413 ${todo.done ? "todo-check--done-413" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-413" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-413"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-413" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-413" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-413">
            <span class="todo-count-413">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-413" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
