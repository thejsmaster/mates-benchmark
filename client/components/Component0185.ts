// client/components/Component0185.ts
// Benchmark component #185 — identical logic to TodoPage, scoped class names.
...
import "./Component0185.css";

type Filter = "all" | "active" | "done";

export const Component0185 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0185] mounted");
    return () => console.log("[Component0185] unmounted");
  });
  ...
  return () => html`
    <div class="app-185">
      <header class="header-185">
        <h1>todos — Component0185</h1>
      </header>

      <main class="main-185">
        <input
          class="add-input-185 ${error() ? "add-input--error-185" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-185" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-185">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-185">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-185 ${filter() === f ? "filter-btn--active-185" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-185">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-185">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-185">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-185 ${todo.done ? "todo-item--done-185" : ""}">
                    <button
                      class="todo-check-185 ${todo.done ? "todo-check--done-185" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-185" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-185"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-185" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-185" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-185">
            <span class="todo-count-185">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-185" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
