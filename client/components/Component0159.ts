// client/components/Component0159.ts
// Benchmark component #159 — identical logic to TodoPage, scoped class names.
...
import "./Component0159.css";

type Filter = "all" | "active" | "done";

export const Component0159 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0159] mounted");
    return () => console.log("[Component0159] unmounted");
  });
  ...
  return () => html`
    <div class="app-159">
      <header class="header-159">
        <h1>todos — Component0159</h1>
      </header>

      <main class="main-159">
        <input
          class="add-input-159 ${error() ? "add-input--error-159" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-159" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-159">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-159">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-159 ${filter() === f ? "filter-btn--active-159" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-159">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-159">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-159">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-159 ${todo.done ? "todo-item--done-159" : ""}">
                    <button
                      class="todo-check-159 ${todo.done ? "todo-check--done-159" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-159" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-159"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-159" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-159" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-159">
            <span class="todo-count-159">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-159" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
