// client/components/Component0949.ts
// Benchmark component #949 — identical logic to TodoPage, scoped class names.
...
import "./Component0949.css";

type Filter = "all" | "active" | "done";

export const Component0949 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0949] mounted");
    return () => console.log("[Component0949] unmounted");
  });
  ...
  return () => html`
    <div class="app-949">
      <header class="header-949">
        <h1>todos — Component0949</h1>
      </header>

      <main class="main-949">
        <input
          class="add-input-949 ${error() ? "add-input--error-949" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-949" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-949">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-949">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-949 ${filter() === f ? "filter-btn--active-949" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-949">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-949">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-949">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-949 ${todo.done ? "todo-item--done-949" : ""}">
                    <button
                      class="todo-check-949 ${todo.done ? "todo-check--done-949" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-949" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-949"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-949" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-949" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-949">
            <span class="todo-count-949">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-949" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
