// client/components/Component0485.ts
// Benchmark component #485 — identical logic to TodoPage, scoped class names.
...
import "./Component0485.css";

type Filter = "all" | "active" | "done";

export const Component0485 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0485] mounted");
    return () => console.log("[Component0485] unmounted");
  });
  ...
  return () => html`
    <div class="app-485">
      <header class="header-485">
        <h1>todos — Component0485</h1>
      </header>

      <main class="main-485">
        <input
          class="add-input-485 ${error() ? "add-input--error-485" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-485" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-485">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-485">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-485 ${filter() === f ? "filter-btn--active-485" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-485">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-485">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-485">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-485 ${todo.done ? "todo-item--done-485" : ""}">
                    <button
                      class="todo-check-485 ${todo.done ? "todo-check--done-485" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-485" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-485"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-485" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-485" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-485">
            <span class="todo-count-485">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-485" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
