// client/components/Component0711.ts
// Benchmark component #711 — identical logic to TodoPage, scoped class names.
...
import "./Component0711.css";

type Filter = "all" | "active" | "done";

export const Component0711 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0711] mounted");
    return () => console.log("[Component0711] unmounted");
  });
  ...
  return () => html`
    <div class="app-711">
      <header class="header-711">
        <h1>todos — Component0711</h1>
      </header>

      <main class="main-711">
        <input
          class="add-input-711 ${error() ? "add-input--error-711" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-711" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-711">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-711">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-711 ${filter() === f ? "filter-btn--active-711" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-711">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-711">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-711">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-711 ${todo.done ? "todo-item--done-711" : ""}">
                    <button
                      class="todo-check-711 ${todo.done ? "todo-check--done-711" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-711" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-711"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-711" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-711" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-711">
            <span class="todo-count-711">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-711" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
