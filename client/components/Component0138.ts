// client/components/Component0138.ts
// Benchmark component #138 — identical logic to TodoPage, scoped class names.
...
import "./Component0138.css";

type Filter = "all" | "active" | "done";

export const Component0138 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0138] mounted");
    return () => console.log("[Component0138] unmounted");
  });
  ...
  return () => html`
    <div class="app-138">
      <header class="header-138">
        <h1>todos — Component0138</h1>
      </header>

      <main class="main-138">
        <input
          class="add-input-138 ${error() ? "add-input--error-138" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-138" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-138">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-138">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-138 ${filter() === f ? "filter-btn--active-138" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-138">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-138">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-138">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-138 ${todo.done ? "todo-item--done-138" : ""}">
                    <button
                      class="todo-check-138 ${todo.done ? "todo-check--done-138" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-138" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-138"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-138" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-138" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-138">
            <span class="todo-count-138">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-138" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
