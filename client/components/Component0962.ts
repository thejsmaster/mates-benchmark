// client/components/Component0962.ts
// Benchmark component #962 — identical logic to TodoPage, scoped class names.
...
import "./Component0962.css";

type Filter = "all" | "active" | "done";

export const Component0962 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0962] mounted");
    return () => console.log("[Component0962] unmounted");
  });
  ...
  return () => html`
    <div class="app-962">
      <header class="header-962">
        <h1>todos — Component0962</h1>
      </header>

      <main class="main-962">
        <input
          class="add-input-962 ${error() ? "add-input--error-962" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-962" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-962">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-962">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-962 ${filter() === f ? "filter-btn--active-962" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-962">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-962">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-962">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-962 ${todo.done ? "todo-item--done-962" : ""}">
                    <button
                      class="todo-check-962 ${todo.done ? "todo-check--done-962" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-962" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-962"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-962" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-962" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-962">
            <span class="todo-count-962">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-962" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
