// client/components/Component0166.ts
// Benchmark component #166 — identical logic to TodoPage, scoped class names.
...
import "./Component0166.css";

type Filter = "all" | "active" | "done";

export const Component0166 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0166] mounted");
    return () => console.log("[Component0166] unmounted");
  });
  ...
  return () => html`
    <div class="app-166">
      <header class="header-166">
        <h1>todos — Component0166</h1>
      </header>

      <main class="main-166">
        <input
          class="add-input-166 ${error() ? "add-input--error-166" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-166" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-166">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-166">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-166 ${filter() === f ? "filter-btn--active-166" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-166">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-166">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-166">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-166 ${todo.done ? "todo-item--done-166" : ""}">
                    <button
                      class="todo-check-166 ${todo.done ? "todo-check--done-166" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-166" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-166"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-166" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-166" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-166">
            <span class="todo-count-166">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-166" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
