// client/components/Component0815.ts
// Benchmark component #815 — identical logic to TodoPage, scoped class names.
...
import "./Component0815.css";

type Filter = "all" | "active" | "done";

export const Component0815 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0815] mounted");
    return () => console.log("[Component0815] unmounted");
  });
  ...
  return () => html`
    <div class="app-815">
      <header class="header-815">
        <h1>todos — Component0815</h1>
      </header>

      <main class="main-815">
        <input
          class="add-input-815 ${error() ? "add-input--error-815" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-815" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-815">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-815">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-815 ${filter() === f ? "filter-btn--active-815" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-815">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-815">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-815">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-815 ${todo.done ? "todo-item--done-815" : ""}">
                    <button
                      class="todo-check-815 ${todo.done ? "todo-check--done-815" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-815" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-815"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-815" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-815" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-815">
            <span class="todo-count-815">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-815" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
