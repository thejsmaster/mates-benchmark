// client/components/Component0670.ts
// Benchmark component #670 — identical logic to TodoPage, scoped class names.
...
import "./Component0670.css";

type Filter = "all" | "active" | "done";

export const Component0670 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0670] mounted");
    return () => console.log("[Component0670] unmounted");
  });
  ...
  return () => html`
    <div class="app-670">
      <header class="header-670">
        <h1>todos — Component0670</h1>
      </header>

      <main class="main-670">
        <input
          class="add-input-670 ${error() ? "add-input--error-670" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-670" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-670">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-670">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-670 ${filter() === f ? "filter-btn--active-670" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-670">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-670">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-670">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-670 ${todo.done ? "todo-item--done-670" : ""}">
                    <button
                      class="todo-check-670 ${todo.done ? "todo-check--done-670" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-670" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-670"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-670" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-670" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-670">
            <span class="todo-count-670">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-670" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
