// client/components/Component0652.ts
// Benchmark component #652 — identical logic to TodoPage, scoped class names.
...
import "./Component0652.css";

type Filter = "all" | "active" | "done";

export const Component0652 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0652] mounted");
    return () => console.log("[Component0652] unmounted");
  });
  ...
  return () => html`
    <div class="app-652">
      <header class="header-652">
        <h1>todos — Component0652</h1>
      </header>

      <main class="main-652">
        <input
          class="add-input-652 ${error() ? "add-input--error-652" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-652" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-652">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-652">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-652 ${filter() === f ? "filter-btn--active-652" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-652">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-652">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-652">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-652 ${todo.done ? "todo-item--done-652" : ""}">
                    <button
                      class="todo-check-652 ${todo.done ? "todo-check--done-652" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-652" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-652"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-652" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-652" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-652">
            <span class="todo-count-652">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-652" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
