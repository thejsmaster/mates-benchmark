// client/components/Component0589.ts
// Benchmark component #589 — identical logic to TodoPage, scoped class names.
...
import "./Component0589.css";

type Filter = "all" | "active" | "done";

export const Component0589 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0589] mounted");
    return () => console.log("[Component0589] unmounted");
  });
  ...
  return () => html`
    <div class="app-589">
      <header class="header-589">
        <h1>todos — Component0589</h1>
      </header>

      <main class="main-589">
        <input
          class="add-input-589 ${error() ? "add-input--error-589" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-589" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-589">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-589">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-589 ${filter() === f ? "filter-btn--active-589" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-589">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-589">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-589">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-589 ${todo.done ? "todo-item--done-589" : ""}">
                    <button
                      class="todo-check-589 ${todo.done ? "todo-check--done-589" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-589" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-589"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-589" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-589" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-589">
            <span class="todo-count-589">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-589" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
