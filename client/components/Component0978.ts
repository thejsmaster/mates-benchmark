// client/components/Component0978.ts
// Benchmark component #978 — identical logic to TodoPage, scoped class names.
...
import "./Component0978.css";

type Filter = "all" | "active" | "done";

export const Component0978 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0978] mounted");
    return () => console.log("[Component0978] unmounted");
  });
  ...
  return () => html`
    <div class="app-978">
      <header class="header-978">
        <h1>todos — Component0978</h1>
      </header>

      <main class="main-978">
        <input
          class="add-input-978 ${error() ? "add-input--error-978" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-978" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-978">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-978">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-978 ${filter() === f ? "filter-btn--active-978" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-978">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-978">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-978">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-978 ${todo.done ? "todo-item--done-978" : ""}">
                    <button
                      class="todo-check-978 ${todo.done ? "todo-check--done-978" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-978" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-978"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-978" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-978" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-978">
            <span class="todo-count-978">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-978" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
