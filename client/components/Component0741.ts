// client/components/Component0741.ts
// Benchmark component #741 — identical logic to TodoPage, scoped class names.
...
import "./Component0741.css";

type Filter = "all" | "active" | "done";

export const Component0741 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0741] mounted");
    return () => console.log("[Component0741] unmounted");
  });
  ...
  return () => html`
    <div class="app-741">
      <header class="header-741">
        <h1>todos — Component0741</h1>
      </header>

      <main class="main-741">
        <input
          class="add-input-741 ${error() ? "add-input--error-741" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-741" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-741">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-741">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-741 ${filter() === f ? "filter-btn--active-741" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-741">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-741">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-741">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-741 ${todo.done ? "todo-item--done-741" : ""}">
                    <button
                      class="todo-check-741 ${todo.done ? "todo-check--done-741" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-741" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-741"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-741" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-741" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-741">
            <span class="todo-count-741">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-741" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
