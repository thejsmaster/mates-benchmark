// client/components/Component0014.ts
// Benchmark component #14 — identical logic to TodoPage, scoped class names.
...
import "./Component0014.css";

type Filter = "all" | "active" | "done";

export const Component0014 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0014] mounted");
    return () => console.log("[Component0014] unmounted");
  });
  ...
  return () => html`
    <div class="app-14">
      <header class="header-14">
        <h1>todos — Component0014</h1>
      </header>

      <main class="main-14">
        <input
          class="add-input-14 ${error() ? "add-input--error-14" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-14" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-14">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-14">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-14 ${filter() === f ? "filter-btn--active-14" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-14">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-14">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-14">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-14 ${todo.done ? "todo-item--done-14" : ""}">
                    <button
                      class="todo-check-14 ${todo.done ? "todo-check--done-14" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-14" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-14"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-14" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-14" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-14">
            <span class="todo-count-14">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-14" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
