// client/components/Component0628.ts
// Benchmark component #628 — identical logic to TodoPage, scoped class names.
...
import "./Component0628.css";

type Filter = "all" | "active" | "done";

export const Component0628 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0628] mounted");
    return () => console.log("[Component0628] unmounted");
  });
  ...
  return () => html`
    <div class="app-628">
      <header class="header-628">
        <h1>todos — Component0628</h1>
      </header>

      <main class="main-628">
        <input
          class="add-input-628 ${error() ? "add-input--error-628" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-628" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-628">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-628">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-628 ${filter() === f ? "filter-btn--active-628" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-628">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-628">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-628">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-628 ${todo.done ? "todo-item--done-628" : ""}">
                    <button
                      class="todo-check-628 ${todo.done ? "todo-check--done-628" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-628" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-628"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-628" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-628" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-628">
            <span class="todo-count-628">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-628" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
