// client/components/Component0546.ts
// Benchmark component #546 — identical logic to TodoPage, scoped class names.
...
import "./Component0546.css";

type Filter = "all" | "active" | "done";

export const Component0546 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0546] mounted");
    return () => console.log("[Component0546] unmounted");
  });
  ...
  return () => html`
    <div class="app-546">
      <header class="header-546">
        <h1>todos — Component0546</h1>
      </header>

      <main class="main-546">
        <input
          class="add-input-546 ${error() ? "add-input--error-546" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-546" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-546">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-546">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-546 ${filter() === f ? "filter-btn--active-546" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-546">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-546">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-546">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-546 ${todo.done ? "todo-item--done-546" : ""}">
                    <button
                      class="todo-check-546 ${todo.done ? "todo-check--done-546" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-546" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-546"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-546" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-546" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-546">
            <span class="todo-count-546">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-546" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
