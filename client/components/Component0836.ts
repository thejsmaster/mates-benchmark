// client/components/Component0836.ts
// Benchmark component #836 — identical logic to TodoPage, scoped class names.
...
import "./Component0836.css";

type Filter = "all" | "active" | "done";

export const Component0836 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0836] mounted");
    return () => console.log("[Component0836] unmounted");
  });
  ...
  return () => html`
    <div class="app-836">
      <header class="header-836">
        <h1>todos — Component0836</h1>
      </header>

      <main class="main-836">
        <input
          class="add-input-836 ${error() ? "add-input--error-836" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-836" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-836">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-836">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-836 ${filter() === f ? "filter-btn--active-836" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-836">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-836">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-836">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-836 ${todo.done ? "todo-item--done-836" : ""}">
                    <button
                      class="todo-check-836 ${todo.done ? "todo-check--done-836" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-836" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-836"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-836" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-836" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-836">
            <span class="todo-count-836">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-836" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
