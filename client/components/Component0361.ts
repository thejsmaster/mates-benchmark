// client/components/Component0361.ts
// Benchmark component #361 — identical logic to TodoPage, scoped class names.
...
import "./Component0361.css";

type Filter = "all" | "active" | "done";

export const Component0361 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0361] mounted");
    return () => console.log("[Component0361] unmounted");
  });
  ...
  return () => html`
    <div class="app-361">
      <header class="header-361">
        <h1>todos — Component0361</h1>
      </header>

      <main class="main-361">
        <input
          class="add-input-361 ${error() ? "add-input--error-361" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-361" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-361">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-361">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-361 ${filter() === f ? "filter-btn--active-361" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-361">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-361">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-361">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-361 ${todo.done ? "todo-item--done-361" : ""}">
                    <button
                      class="todo-check-361 ${todo.done ? "todo-check--done-361" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-361" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-361"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-361" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-361" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-361">
            <span class="todo-count-361">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-361" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
