// client/components/Component0659.ts
// Benchmark component #659 — identical logic to TodoPage, scoped class names.
...
import "./Component0659.css";

type Filter = "all" | "active" | "done";

export const Component0659 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0659] mounted");
    return () => console.log("[Component0659] unmounted");
  });
  ...
  return () => html`
    <div class="app-659">
      <header class="header-659">
        <h1>todos — Component0659</h1>
      </header>

      <main class="main-659">
        <input
          class="add-input-659 ${error() ? "add-input--error-659" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-659" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-659">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-659">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-659 ${filter() === f ? "filter-btn--active-659" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-659">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-659">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-659">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-659 ${todo.done ? "todo-item--done-659" : ""}">
                    <button
                      class="todo-check-659 ${todo.done ? "todo-check--done-659" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-659" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-659"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-659" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-659" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-659">
            <span class="todo-count-659">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-659" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
