// client/components/Component0163.ts
// Benchmark component #163 — identical logic to TodoPage, scoped class names.
...
import "./Component0163.css";

type Filter = "all" | "active" | "done";

export const Component0163 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0163] mounted");
    return () => console.log("[Component0163] unmounted");
  });
  ...
  return () => html`
    <div class="app-163">
      <header class="header-163">
        <h1>todos — Component0163</h1>
      </header>

      <main class="main-163">
        <input
          class="add-input-163 ${error() ? "add-input--error-163" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-163" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-163">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-163">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-163 ${filter() === f ? "filter-btn--active-163" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-163">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-163">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-163">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-163 ${todo.done ? "todo-item--done-163" : ""}">
                    <button
                      class="todo-check-163 ${todo.done ? "todo-check--done-163" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-163" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-163"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-163" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-163" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-163">
            <span class="todo-count-163">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-163" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
