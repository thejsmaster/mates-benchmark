// client/components/Component0610.ts
// Benchmark component #610 — identical logic to TodoPage, scoped class names.
...
import "./Component0610.css";

type Filter = "all" | "active" | "done";

export const Component0610 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0610] mounted");
    return () => console.log("[Component0610] unmounted");
  });
  ...
  return () => html`
    <div class="app-610">
      <header class="header-610">
        <h1>todos — Component0610</h1>
      </header>

      <main class="main-610">
        <input
          class="add-input-610 ${error() ? "add-input--error-610" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-610" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-610">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-610">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-610 ${filter() === f ? "filter-btn--active-610" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-610">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-610">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-610">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-610 ${todo.done ? "todo-item--done-610" : ""}">
                    <button
                      class="todo-check-610 ${todo.done ? "todo-check--done-610" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-610" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-610"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-610" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-610" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-610">
            <span class="todo-count-610">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-610" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
