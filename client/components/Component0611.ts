// client/components/Component0611.ts
// Benchmark component #611 — identical logic to TodoPage, scoped class names.
...
import "./Component0611.css";

type Filter = "all" | "active" | "done";

export const Component0611 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0611] mounted");
    return () => console.log("[Component0611] unmounted");
  });
  ...
  return () => html`
    <div class="app-611">
      <header class="header-611">
        <h1>todos — Component0611</h1>
      </header>

      <main class="main-611">
        <input
          class="add-input-611 ${error() ? "add-input--error-611" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-611" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-611">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-611">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-611 ${filter() === f ? "filter-btn--active-611" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-611">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-611">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-611">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-611 ${todo.done ? "todo-item--done-611" : ""}">
                    <button
                      class="todo-check-611 ${todo.done ? "todo-check--done-611" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-611" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-611"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-611" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-611" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-611">
            <span class="todo-count-611">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-611" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
