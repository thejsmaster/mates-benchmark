// client/components/Component0301.ts
// Benchmark component #301 — identical logic to TodoPage, scoped class names.
...
import "./Component0301.css";

type Filter = "all" | "active" | "done";

export const Component0301 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0301] mounted");
    return () => console.log("[Component0301] unmounted");
  });
  ...
  return () => html`
    <div class="app-301">
      <header class="header-301">
        <h1>todos — Component0301</h1>
      </header>

      <main class="main-301">
        <input
          class="add-input-301 ${error() ? "add-input--error-301" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-301" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-301">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-301">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-301 ${filter() === f ? "filter-btn--active-301" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-301">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-301">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-301">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-301 ${todo.done ? "todo-item--done-301" : ""}">
                    <button
                      class="todo-check-301 ${todo.done ? "todo-check--done-301" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-301" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-301"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-301" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-301" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-301">
            <span class="todo-count-301">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-301" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
