// client/components/Component0958.ts
// Benchmark component #958 — identical logic to TodoPage, scoped class names.
...
import "./Component0958.css";

type Filter = "all" | "active" | "done";

export const Component0958 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0958] mounted");
    return () => console.log("[Component0958] unmounted");
  });
  ...
  return () => html`
    <div class="app-958">
      <header class="header-958">
        <h1>todos — Component0958</h1>
      </header>

      <main class="main-958">
        <input
          class="add-input-958 ${error() ? "add-input--error-958" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-958" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-958">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-958">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-958 ${filter() === f ? "filter-btn--active-958" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-958">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-958">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-958">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-958 ${todo.done ? "todo-item--done-958" : ""}">
                    <button
                      class="todo-check-958 ${todo.done ? "todo-check--done-958" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-958" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-958"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-958" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-958" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-958">
            <span class="todo-count-958">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-958" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
