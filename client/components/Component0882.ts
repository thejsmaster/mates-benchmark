// client/components/Component0882.ts
// Benchmark component #882 — identical logic to TodoPage, scoped class names.
...
import "./Component0882.css";

type Filter = "all" | "active" | "done";

export const Component0882 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0882] mounted");
    return () => console.log("[Component0882] unmounted");
  });
  ...
  return () => html`
    <div class="app-882">
      <header class="header-882">
        <h1>todos — Component0882</h1>
      </header>

      <main class="main-882">
        <input
          class="add-input-882 ${error() ? "add-input--error-882" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-882" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-882">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-882">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-882 ${filter() === f ? "filter-btn--active-882" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-882">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-882">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-882">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-882 ${todo.done ? "todo-item--done-882" : ""}">
                    <button
                      class="todo-check-882 ${todo.done ? "todo-check--done-882" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-882" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-882"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-882" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-882" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-882">
            <span class="todo-count-882">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-882" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
