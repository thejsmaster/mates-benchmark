// client/components/Component0948.ts
// Benchmark component #948 — identical logic to TodoPage, scoped class names.
...
import "./Component0948.css";

type Filter = "all" | "active" | "done";

export const Component0948 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0948] mounted");
    return () => console.log("[Component0948] unmounted");
  });
  ...
  return () => html`
    <div class="app-948">
      <header class="header-948">
        <h1>todos — Component0948</h1>
      </header>

      <main class="main-948">
        <input
          class="add-input-948 ${error() ? "add-input--error-948" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-948" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-948">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-948">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-948 ${filter() === f ? "filter-btn--active-948" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-948">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-948">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-948">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-948 ${todo.done ? "todo-item--done-948" : ""}">
                    <button
                      class="todo-check-948 ${todo.done ? "todo-check--done-948" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-948" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-948"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-948" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-948" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-948">
            <span class="todo-count-948">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-948" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
