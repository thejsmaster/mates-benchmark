// client/components/Component0591.ts
// Benchmark component #591 — identical logic to TodoPage, scoped class names.
...
import "./Component0591.css";

type Filter = "all" | "active" | "done";

export const Component0591 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0591] mounted");
    return () => console.log("[Component0591] unmounted");
  });
  ...
  return () => html`
    <div class="app-591">
      <header class="header-591">
        <h1>todos — Component0591</h1>
      </header>

      <main class="main-591">
        <input
          class="add-input-591 ${error() ? "add-input--error-591" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-591" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-591">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-591">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-591 ${filter() === f ? "filter-btn--active-591" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-591">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-591">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-591">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-591 ${todo.done ? "todo-item--done-591" : ""}">
                    <button
                      class="todo-check-591 ${todo.done ? "todo-check--done-591" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-591" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-591"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-591" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-591" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-591">
            <span class="todo-count-591">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-591" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
