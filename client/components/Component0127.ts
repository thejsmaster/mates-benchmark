// client/components/Component0127.ts
// Benchmark component #127 — identical logic to TodoPage, scoped class names.
...
import "./Component0127.css";

type Filter = "all" | "active" | "done";

export const Component0127 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0127] mounted");
    return () => console.log("[Component0127] unmounted");
  });
  ...
  return () => html`
    <div class="app-127">
      <header class="header-127">
        <h1>todos — Component0127</h1>
      </header>

      <main class="main-127">
        <input
          class="add-input-127 ${error() ? "add-input--error-127" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-127" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-127">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-127">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-127 ${filter() === f ? "filter-btn--active-127" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-127">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-127">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-127">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-127 ${todo.done ? "todo-item--done-127" : ""}">
                    <button
                      class="todo-check-127 ${todo.done ? "todo-check--done-127" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-127" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-127"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-127" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-127" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-127">
            <span class="todo-count-127">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-127" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
