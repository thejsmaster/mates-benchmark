// client/components/Component0753.ts
// Benchmark component #753 — identical logic to TodoPage, scoped class names.
...
import "./Component0753.css";

type Filter = "all" | "active" | "done";

export const Component0753 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0753] mounted");
    return () => console.log("[Component0753] unmounted");
  });
  ...
  return () => html`
    <div class="app-753">
      <header class="header-753">
        <h1>todos — Component0753</h1>
      </header>

      <main class="main-753">
        <input
          class="add-input-753 ${error() ? "add-input--error-753" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-753" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-753">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-753">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-753 ${filter() === f ? "filter-btn--active-753" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-753">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-753">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-753">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-753 ${todo.done ? "todo-item--done-753" : ""}">
                    <button
                      class="todo-check-753 ${todo.done ? "todo-check--done-753" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-753" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-753"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-753" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-753" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-753">
            <span class="todo-count-753">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-753" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
