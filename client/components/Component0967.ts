// client/components/Component0967.ts
// Benchmark component #967 — identical logic to TodoPage, scoped class names.
...
import "./Component0967.css";

type Filter = "all" | "active" | "done";

export const Component0967 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0967] mounted");
    return () => console.log("[Component0967] unmounted");
  });
  ...
  return () => html`
    <div class="app-967">
      <header class="header-967">
        <h1>todos — Component0967</h1>
      </header>

      <main class="main-967">
        <input
          class="add-input-967 ${error() ? "add-input--error-967" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-967" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-967">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-967">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-967 ${filter() === f ? "filter-btn--active-967" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-967">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-967">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-967">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-967 ${todo.done ? "todo-item--done-967" : ""}">
                    <button
                      class="todo-check-967 ${todo.done ? "todo-check--done-967" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-967" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-967"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-967" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-967" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-967">
            <span class="todo-count-967">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-967" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
