// client/components/Component0397.ts
// Benchmark component #397 — identical logic to TodoPage, scoped class names.
...
import "./Component0397.css";

type Filter = "all" | "active" | "done";

export const Component0397 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0397] mounted");
    return () => console.log("[Component0397] unmounted");
  });
  ...
  return () => html`
    <div class="app-397">
      <header class="header-397">
        <h1>todos — Component0397</h1>
      </header>

      <main class="main-397">
        <input
          class="add-input-397 ${error() ? "add-input--error-397" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-397" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-397">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-397">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-397 ${filter() === f ? "filter-btn--active-397" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-397">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-397">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-397">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-397 ${todo.done ? "todo-item--done-397" : ""}">
                    <button
                      class="todo-check-397 ${todo.done ? "todo-check--done-397" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-397" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-397"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-397" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-397" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-397">
            <span class="todo-count-397">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-397" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
