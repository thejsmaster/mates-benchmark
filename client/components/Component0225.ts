// client/components/Component0225.ts
// Benchmark component #225 — identical logic to TodoPage, scoped class names.
...
import "./Component0225.css";

type Filter = "all" | "active" | "done";

export const Component0225 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0225] mounted");
    return () => console.log("[Component0225] unmounted");
  });
  ...
  return () => html`
    <div class="app-225">
      <header class="header-225">
        <h1>todos — Component0225</h1>
      </header>

      <main class="main-225">
        <input
          class="add-input-225 ${error() ? "add-input--error-225" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-225" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-225">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-225">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-225 ${filter() === f ? "filter-btn--active-225" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-225">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-225">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-225">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-225 ${todo.done ? "todo-item--done-225" : ""}">
                    <button
                      class="todo-check-225 ${todo.done ? "todo-check--done-225" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-225" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-225"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-225" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-225" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-225">
            <span class="todo-count-225">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-225" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
