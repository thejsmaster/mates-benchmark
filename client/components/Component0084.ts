// client/components/Component0084.ts
// Benchmark component #84 — identical logic to TodoPage, scoped class names.
...
import "./Component0084.css";

type Filter = "all" | "active" | "done";

export const Component0084 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0084] mounted");
    return () => console.log("[Component0084] unmounted");
  });
  ...
  return () => html`
    <div class="app-84">
      <header class="header-84">
        <h1>todos — Component0084</h1>
      </header>

      <main class="main-84">
        <input
          class="add-input-84 ${error() ? "add-input--error-84" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-84" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-84">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-84">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-84 ${filter() === f ? "filter-btn--active-84" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-84">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-84">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-84">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-84 ${todo.done ? "todo-item--done-84" : ""}">
                    <button
                      class="todo-check-84 ${todo.done ? "todo-check--done-84" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-84" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-84"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-84" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-84" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-84">
            <span class="todo-count-84">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-84" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
