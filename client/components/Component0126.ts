// client/components/Component0126.ts
// Benchmark component #126 — identical logic to TodoPage, scoped class names.
...
import "./Component0126.css";

type Filter = "all" | "active" | "done";

export const Component0126 = (_: Props<{}>) => {
  ...
  onMount(() => {
    console.log("[Component0126] mounted");
    return () => console.log("[Component0126] unmounted");
  });
  ...
  return () => html`
    <div class="app-126">
      <header class="header-126">
        <h1>todos — Component0126</h1>
      </header>

      <main class="main-126">
        <input
          class="add-input-126 ${error() ? "add-input--error-126" : ""}"
          type="text"
        ...
        />
        <button class="add-btn-126" type="submit" ?disabled=${add.isLoading()}>
          ${add.isLoading() ? "Adding…" : "Add"}
        </button>

        ${error() ? html`<p class="field-error-126">${error()}</p>` : null}

        ${todos().length > 0 ? html`
          <div class="filters-126">
            ${(["all", "active", "done"] as Filter[]).map((f) => html`
              <button
                class="filter-btn-126 ${filter() === f ? "filter-btn--active-126" : ""}"
                @click=${() => filter.set(f)}
              >${f}</button>
            `)}
          </div>
        ...
        ${fetchTodos.isLoading()
          ? html`<p class="loading-126">Loading…</p>`
          : filteredTodos().length === 0
            ? html`<p class="empty-126">${todos().length === 0 ? "No todos yet." : `No ${filter()} todos.`}</p>`
            : html`
              <ul class="todo-list-126">
                ${filteredTodos().map((todo) => html`
                  <li class="todo-item-126 ${todo.done ? "todo-item--done-126" : ""}">
                    <button
                      class="todo-check-126 ${todo.done ? "todo-check--done-126" : ""}"
                    ...
                    ${editingId() === todo.id
                      ? html`
                          <form class="edit-form-126" @submit=${(e: Event) => { e.preventDefault(); saveEdit(); }}>
                            <input
                              class="edit-input-126"
                              .value=${editingText()}
                        ...
                        `
                      : html`
                          <span class="todo-text-126" @dblclick=${() => startEdit(todo)} title="Double-click to edit">
                            ${todo.text}
                          </span>
                        `}

                    <button class="todo-delete-126" @click=${() => remove(todo.id)} aria-label="Delete">×</button>
                  </li>
                `)}
              </ul>
            `}
        ...
        ${todos().length > 0 ? html`
          <footer class="footer-126">
            <span class="todo-count-126">${activeCount()} item${activeCount() === 1 ? "" : "s"} left</span>
            ${doneCount() > 0
              ? html`<button class="clear-btn-126" @click=${() => clearCompleted()}>Clear completed (${doneCount()})</button>`
              : null}
          </footer>
        ` : null}
      </main>
